...
const static std::unordered_map<std::string, uint32_t> basic_information_map{
    ...
    { "ManufacturerName", Attributes::VendorID::Id },             // string, vendor id {can not map to one to one}
    ...
    { "ApplicationVersion", Attributes::UniqueID::Id }
}; // uint8_t, string
...

{
    ...
    node_state_monitor.register_event_listener(BridgedDeviceBasicInfoAttributeAccess::unify_node_reachable_state_update);
}
...
CHIP_ERROR BridgedDeviceBasicInfoAttributeAccess::Read(const ConcreteReadAttributePath & aPath, AttributeValueEncoder & aEncoder)
{
    ...
    ConcreteAttributePath attr_path = ConcreteAttributePath(aPath.mEndpointId, aPath.mClusterId, aPath.mAttributeId);
    try
    {
        switch (aPath.mAttributeId)
        {
        case VendorName::Id: {
            std::string read_vendor_name = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_vendor_name.c_str()));
        }
        case VendorID::Id: {
            uint16_t read_vendor_id = attribute_state_cache::get_instance().get<uint16_t>(attr_path);
            return aEncoder.Encode(read_vendor_id);
        }
        case ProductName::Id: {
            std::string read_product_name = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_product_name.c_str()));
        }
        case HardwareVersion::Id: {
            uint16_t read_hardware_version = attribute_state_cache::get_instance().get<uint16_t>(attr_path);
            return aEncoder.Encode(read_hardware_version);
        }
        case HardwareVersionString::Id: {
            std::string read_hardware_version_string = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_hardware_version_string.c_str()));
        }
        case SoftwareVersion::Id: {
            uint32_t read_software_version = attribute_state_cache::get_instance().get<uint32_t>(attr_path);
            return aEncoder.Encode(read_software_version);
        }
        case SoftwareVersionString::Id: {
            std::string read_software_version_string = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_software_version_string.c_str()));
        }
        case ManufacturingDate::Id: {
            std::string read_manufacturing_date = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_manufacturing_date.c_str()));
        }
        case ProductURL::Id: {
            std::string read_product_url = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_product_url.c_str()));
        }
        case ProductLabel::Id: {
            std::string read_product_label = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_product_label.c_str()));
        }
        case SerialNumber::Id: {
            std::string read_serial_number = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_serial_number.c_str()));
        }
        case NodeLabel::Id: {
            std::string read_node_label = attribute_state_cache::get_instance().get<std::string>(attr_path);
            return aEncoder.Encode(chip::CharSpan::fromCharString(read_node_label.c_str()));
        }
        case Reachable::Id: {
            Reachable::TypeInfo::Type state = attribute_state_cache::get_instance().get<Reachable::TypeInfo::Type>(attr_path);
...
CHIP_ERROR BridgedDeviceBasicInfoAttributeAccess::Write(const ConcreteDataAttributePath & aPath, AttributeValueDecoder & aDecoder)
{
    ...
    if (aPath.mAttributeId == NodeLabel::Id)
    {
        ...
        nlohmann::json jsn;
        std::string node_label = std::string(node_label_span.begin(), node_label_span.end());
        ...
        std::string name_topic = "ucl/by-unid/" + unify_node->unify_unid + "/ep" + std::to_string(unify_node->unify_endpoint) +
            "/NameAndLocation/Attributes/Name/Desired";
        std::string payload_str = jsn.dump();
        uic_mqtt_publish(name_topic.c_str(), payload_str.c_str(), payload_str.length(), true);
        return CHIP_NO_ERROR;
    }
    return CHIP_ERROR_NO_MESSAGE_HANDLER;
}

void BridgedDeviceBasicInfoAttributeAccess::reported_updated(const bridged_endpoint * ep, const std::string & cluster,
...
void BridgedDeviceBasicInfoAttributeAccess::unify_node_reachable_state_update(const bridged_endpoint & ep,
                                                                              matter_node_state_monitor::update_t state)
{
...
uint32_t BridgedDeviceBasicInfoAttributeAccess::map_basic_cluster_attributes(const std::string & dot_dot_basic_attribute) const
{
