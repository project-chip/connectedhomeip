...
/* In a real product this should be changed by the manufacturer as appropriate */
#define WATER_HEATER_ENDPOINT 2
...
static std::unique_ptr<EnergyEvseDelegate> gWaterHeaterDelegate;
...
CHIP_ERROR DeviceEnergyManagementInit()
{
    ...
    gDEMInstance = std::make_unique<DeviceEnergyManagementManager>(EndpointId(WATER_HEATER_ENDPOINT), *gDEMDelegate, featureMap);
...
/*
 *  @brief  Creates a Delegate and Instance for WaterHeaterManagement cluster
 *
 * The Instance is a container around the Delegate, so
 * create the Delegate first, then wrap it in the Instance
 * Then call the Instance->Init() to register the attribute and command handlers
 */
CHIP_ERROR WaterHeaterManagementInit()
{
    CHIP_ERROR err;
    ...
    gEvseInstance = std::make_unique<EnergyEvseManager>(
        EndpointId(WATER_HEATER_ENDPOINT), *gEvseDelegate,
...
CHIP_ERROR PowerTopologyInit()
{
    ...
    gPTInstance =
        std::make_unique<PowerTopologyInstance>(EndpointId(WATER_HEATER_ENDPOINT), *gPTDelegate,
...
CHIP_ERROR EnergyMeterInit()
{
    ...
    gEPMInstance = std::make_unique<ElectricalPowerMeasurementInstance>(
        EndpointId(WATER_HEATER_ENDPOINT), *gEPMDelegate,
...
{
    ChipLogDetail(AppServer, "Water Heater Management: ApplicationShutdown()");

    /* Shutdown in reverse order that they were created */
    WaterHeaterManufacturerShutdown(); /* Free the WaterHeaterManufacturer */
...
{
    VerifyOrDie(endpointId == WATER_HEATER_ENDPOINT); // this cluster is only enabled for endpoint 1.
