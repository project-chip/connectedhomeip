...
Protocols::InteractionModel::Status ThermostatSetpointsDelegate::Init()
{
    ...
    if (auto status = AbsMinCoolSetpointLimit::GetDefault(mEndpointId, &mAbsMinCoolSetpointLimit);
        status != Status::Success)
    {
        mAbsMinCoolSetpointLimit = kDefaultAbsMinCoolSetpointLimit;
    }

    if (auto status = AbsMaxCoolSetpointLimit::GetDefault(mEndpointId, &mAbsMaxCoolSetpointLimit);
        status != Status::Success)
    {
    ...
    if (auto status = MinCoolSetpointLimit::GetDefault(mEndpointId, &defaultMinCoolLimit);
        status != Status::Success)
    {
    ...
    if (auto status = MaxCoolSetpointLimit::GetDefault(mEndpointId, &defaultMaxCoolLimit);
        status != Status::Success)
    {
    ...
    if (auto status = MinHeatSetpointLimit::GetDefault(mEndpointId, &defaultMinHeatLimit);
        status != Status::Success)
    {
    ...
    if (auto status = MaxHeatSetpointLimit::GetDefault(mEndpointId, &defaultMaxHeatLimit);
        status != Status::Success)
    {
    ...
    if (auto status = UnoccupiedCoolingSetpoint::GetDefault(mEndpointId, &mUnoccupiedCoolingSetpoint);
        status != Status::Success)
    {
        mUnoccupiedCoolingSetpoint = kDefaultCoolingSetpoint;
    }

    if (auto status = OccupiedCoolingSetpoint::GetDefault(mEndpointId, &mOccupiedCoolingSetpoint);
        status != Status::Success)
    {
        mOccupiedCoolingSetpoint = kDefaultCoolingSetpoint;
    }

        if (auto status = AbsMinHeatSetpointLimit::GetDefault(mEndpointId, &mAbsMinHeatSetpointLimit);
        status != Status::Success)
    {
        mAbsMinHeatSetpointLimit = kDefaultAbsMinHeatSetpointLimit;
    }

    if (auto status = AbsMaxHeatSetpointLimit::GetDefault(mEndpointId, &mAbsMaxHeatSetpointLimit);
        status != Status::Success)
    {
    ...
    if (auto status = MinHeatSetpointLimit::GetDefault(mEndpointId, &mMinHeatSetpointLimit);
        status != Status::Success)
    {
        mMinHeatSetpointLimit = mAbsMinHeatSetpointLimit    ;
    }

    if (auto status = MaxHeatSetpointLimit::GetDefault(mEndpointId, &mMaxHeatSetpointLimit);
        status != Status::Success)
    {
        mMaxHeatSetpointLimit = mAbsMaxHeatSetpointLimit;
    }

    if (auto status = OccupiedHeatingSetpoint::GetDefault(mEndpointId, &mOccupiedHeatingSetpoint);
        status != Status::Success)
    {
        mOccupiedHeatingSetpoint = kDefaultHeatingSetpoint;
    }

    if (auto status = UnoccupiedHeatingSetpoint::GetDefault(mEndpointId, &mUnoccupiedHeatingSetpoint);
        status != Status::Success)
    {
...
Protocols::InteractionModel::Status ThermostatSetpointsDelegate::SaveSetpoint(AttributeId attributeId, temperature value) {
    ...
    VerifyOrReturnError(provider != nullptr, Status::Failure);
    AttributePersistence persistence(*provider);
    if (auto status = persistence.StoreNativeEndianValue({ mEndpointId, Thermostat::Id, attributeId }, value); status != CHIP_NO_ERROR) {
        return ClusterStatusCode(status).GetStatus();
    }
    return Status::Success;
}

Protocols::InteractionModel::Status ThermostatSetpointsDelegate::GetAbsMinCoolSetpointLimit(temperature & absMinCoolSetpointLimit) const
{
    absMinCoolSetpointLimit = mAbsMinCoolSetpointLimit;
    return Status::Success;
}

Protocols::InteractionModel::Status ThermostatSetpointsDelegate::GetAbsMaxCoolSetpointLimit(temperature & absMaxCoolSetpointLimit) const
{
...
Protocols::InteractionModel::Status ThermostatSetpointsDelegate::GetOccupiedCoolingSetpoint(temperature & occupiedCoolingSetpoint) const
{
...
Protocols::InteractionModel::Status ThermostatSetpointsDelegate::GetUnoccupiedCoolingSetpoint(temperature & unoccupiedCoolingSetpoint) const
{
...
Protocols::InteractionModel::Status ThermostatSetpointsDelegate::GetAbsMinHeatSetpointLimit(temperature & absMinHeatSetpointLimit) const {
    absMinHeatSetpointLimit = mAbsMinHeatSetpointLimit;
    return Status::Success;
}

Protocols::InteractionModel::Status ThermostatSetpointsDelegate::GetAbsMaxHeatSetpointLimit(temperature & absMaxHeatSetpointLimit) const {
...
Protocols::InteractionModel::Status ThermostatSetpointsDelegate::GetOccupiedHeatingSetpoint(temperature & occupiedHeatingSetpoint) const
{
...
Protocols::InteractionModel::Status ThermostatSetpointsDelegate::GetUnoccupiedHeatingSetpoint(temperature & unoccupiedHeatingSetpoint) const
{
