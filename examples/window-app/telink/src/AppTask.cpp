...
void AppTask::OpenHandler(const AppEvent & event)
{
    ...
    if (event.ButtonEvent.Action == static_cast<uint8_t>(AppEvent::kEventType_ButtonPushed))
    {
        sAppTask.mOpenButtonIsPressed = true;
        if (sAppTask.mCloseButtonIsPressed)
        {
            sAppTask.ToggleMoveType();
        }
    }
    else if (event.ButtonEvent.Action == static_cast<uint8_t>(AppEvent::kEventType_ButtonReleased))
    {
        if (!sAppTask.mCloseButtonIsPressed)
        {
            if (!sAppTask.mMoveTypeRecentlyChanged)
            {
                WindowCovering::Instance().SetSingleStepTarget(OperationalState::MovingUpOrOpen);
            }
            else
            {
                sAppTask.mMoveTypeRecentlyChanged = false;
            }
        }
        sAppTask.mOpenButtonIsPressed = false;
    }
}
...
void AppTask::CloseHandler(const AppEvent & event)
{
    ...
    if (event.ButtonEvent.Action == static_cast<uint8_t>(AppEvent::kEventType_ButtonPushed))
    {
        sAppTask.mCloseButtonIsPressed = true;
        if (sAppTask.mOpenButtonIsPressed)
        {
            sAppTask.ToggleMoveType();
        }
    }
    else if (event.ButtonEvent.Action == static_cast<uint8_t>(AppEvent::kEventType_ButtonReleased))
    {
        ...
        sAppTask.mCloseButtonIsPressed = false;
    }
}
