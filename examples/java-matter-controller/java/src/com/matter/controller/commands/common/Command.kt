...
abstract class Command(private val name: String, private val helpText: String? = null) {
     ...
     * @brief Get attribute argument if it exists, there is at most one Attribute argument per
     *   command
     */
    ...
    fun initArguments(args: Array<String>) {
        ...
        while (i < argc) {
            ...
            if (
                args[i].substring(OPTIONAL_ARGUMENT_PREFIX_LENGTH) == arguments[currentIndex].name
            ) {
                require(i + 1 < argc) {
