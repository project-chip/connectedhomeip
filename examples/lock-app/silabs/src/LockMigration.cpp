...
static constexpr uint8_t kMaxCredentialsPerUser      = 64;
...
{
    return VerifyKey(Legacy::kConfigKey_LockUser) ||
           VerifyKey(Legacy::kConfigKey_Credential) ||
           VerifyKey(Legacy::kConfigKey_LockUserName) ||
           VerifyKey(Legacy::kConfigKey_CredentialData) ||
           VerifyKey(Legacy::kConfigKey_UserCredentials) ||
           VerifyKey(Legacy::kConfigKey_WeekDaySchedules) ||
           VerifyKey(Legacy::kConfigKey_YearDaySchedules) ||
           VerifyKey(Legacy::kConfigKey_HolidaySchedules);
}
    ...
    //   kConfigKey_Credential = 0x87311
    {
        ...
        for (size_t type_idx = 0; type_idx < type_info_count; type_idx++)
        {
            ...
            for (size_t cred_idx = 0; success && cred_idx < params.numberOfCredentialsPerUser; cred_idx++)
            {
                ...
                Legacy::EmberAfPluginDoorLockCredentialInfo &info = creds_info[(type_idx *  params.numberOfCredentialsPerUser) + cred_idx];
    ...
    if(IsMigrationNeeded())
    {
        ...
        for (chip::EndpointId endpoint_id = 1; endpoint_id <= kEnpointCount; ++endpoint_id)
        {
            if (emberAfContainsServer(endpoint_id, DoorLock::Id))
            {
                ...
                bool success = MigrateCredentials(endpoint_id, params) &&
                               MigrateUsers(endpoint_id, params) &&
