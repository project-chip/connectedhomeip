...
struct chip_os_timer sLockTimer;

int BoltLockManager::Init()
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    chip_os_timer_init(&sLockTimer, TimerEventHandler, this);
...
{
    chip_os_timer_start(&sLockTimer, aTimeoutMs);
}

void BoltLockManager::CancelTimer(void)
{
    chip_os_timer_stop(&sLockTimer);
}
...
void BoltLockManager::AutoReLockTimerEventHandler(AppEvent * aEvent)
{
    ...
    APP_LOG("Auto Re-Lock has been triggered!");
...
void BoltLockManager::ActuatorMovementTimerEventHandler(AppEvent * aEvent)
{
    ...
    if (actionCompleted != INVALID_ACTION)
    {
        ...
        if (lock->mAutoRelock && actionCompleted == UNLOCK_ACTION)
        {
            ...
            APP_LOG("Auto Re-lock enabled. Will be triggered in %u seconds", lock->mAutoLockDuration);
        }
    }
}
