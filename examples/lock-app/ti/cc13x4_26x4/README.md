...
-   The lock-app ZAP file needs an additional attribute enabled in order to be
    able to commission successfully. Please enable the ConnectTimeMaxSeconds
    attribute in the NetworkCommissioning cluster on Endpoint 0 using the ZAP
    GUI, save the changes in the lock-app ZAP file, and generate the
    lock-app.matter file. The ZAP GUI can be accessed by running 'zap' at the
    root of the connectedhomeip repo, and the lock-app ZAP and .matter files are
    located at `{chip_root}/examples/lock-app/lock-common`. The lock-app.matter
    file can be generated by navigating to `{chip_root}/scripts/tools/zap` and
    running `./generate.py examples/lock-app/lock-common/lock-app.zap`.
