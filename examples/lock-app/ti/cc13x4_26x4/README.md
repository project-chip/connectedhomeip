...
-   The lock-app ZAP file needs an additional attribute enabled in order to be
    ...
    root of the connectedhomeip repo, and the lock-app ZAP and .matter files
    are located at `{chip_root}/examples/lock-app/lock-common`. The 
    lock-app.matter file can be generated by navigating to 
    `{chip_root}/scripts/tools/zap` and running 
    `./generate.py examples/lock-app/lock-common/lock-app.zap`.
