...
def generate_device_manifest(
    ...
    for device_name in _DEVICE_LIST:
        matter_file_path = os.path.join(_DEVICE_FOLDER, device_name + ".matter")
        zap_file_path = os.path.join(_DEVICE_FOLDER, device_name + ".zap")
        device_zzz_dir = os.path.join(_CHEF_ZZZ_ROOT, device_name)

        matter_file_md5 = generate_hash_from_file(matter_file_path)
        zap_file_md5 = generate_hash_from_file(zap_file_path)
        zzz_gen_dir_md5 = generate_hash_from_dir(device_zzz_dir)

        device_manifest_object = {
            "matter_hash": matter_file_md5,
            "zap_hash": zap_file_md5,
            "zzz_gen_hash": zzz_gen_dir_md5,
        }
    ...
    if write_manifest_file:
        with open(_CI_MANIFEST_FILE, "w") as f:
            f.write(json.dumps(ci_manifest, indent=4))
    return ci_manifest
...
def main(argv: Sequence[str]) -> None:
    ...
    if options.validate_zzz:
        ...
        To fix:
          ...
          cd ./third_party/zap/repo && git reset HEAD --hard
          cd ../../..
          git submodule update ./third_party/zap/repo
          npm install ./third_party/zap/repo
