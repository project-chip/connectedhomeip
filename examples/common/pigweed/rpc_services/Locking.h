...
public:
    ...
    virtual pw::Status Set(ServerContext &, const chip_rpc_LockingState & request, pw_protobuf_Empty & response)
    {
        ...
        RETURN_STATUS_IF_NOT_OK(
            emberAfWriteServerAttribute(1, ZCL_ON_OFF_CLUSTER_ID, ZCL_ON_OFF_ATTRIBUTE_ID, &locked, ZCL_BOOLEAN_ATTRIBUTE_TYPE));
        return pw::OkStatus();
    }
    ...
    virtual pw::Status Get(ServerContext &, const pw_protobuf_Empty & request, chip_rpc_LockingState & response)
    {
        uint8_t locked;
        RETURN_STATUS_IF_NOT_OK(
            emberAfReadServerAttribute(1, ZCL_ON_OFF_CLUSTER_ID, ZCL_ON_OFF_ATTRIBUTE_ID, &locked, sizeof(locked)));
