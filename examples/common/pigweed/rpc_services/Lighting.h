...
{
public:
    Lighting(bool support_level = true, bool support_color = true) : _support_level(support_level), _support_color(support_color) {}
    ...
    virtual pw::Status Set(ServerContext &, const chip_rpc_LightingState & request, pw_protobuf_Empty & response)
    {
        ...
        if (_support_level && request.has_level)
        {
            // Clip level to max
            uint8_t level = std::min(request.level, static_cast<uint32_t>(std::numeric_limits<uint8_t>::max()));
            RETURN_STATUS_IF_NOT_OK(emberAfWriteServerAttribute(1, ZCL_LEVEL_CONTROL_CLUSTER_ID, ZCL_CURRENT_LEVEL_ATTRIBUTE_ID,
                                                                &level, ZCL_INT8U_ATTRIBUTE_TYPE));
        }

        if (_support_color && request.has_color)
        {
            ...
            RETURN_STATUS_IF_NOT_OK(emberAfWriteServerAttribute(1, ZCL_COLOR_CONTROL_CLUSTER_ID,
    ...
    virtual pw::Status Get(ServerContext &, const pw_protobuf_Empty & request, chip_rpc_LightingState & response)
    {
        ...
        if (_support_color)
        {
            RETURN_STATUS_IF_NOT_OK(emberAfReadServerAttribute(1, ZCL_COLOR_CONTROL_CLUSTER_ID,
