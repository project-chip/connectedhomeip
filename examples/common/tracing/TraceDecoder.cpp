...
CHIP_ERROR TraceDecoder::Log(Json::Value & json)
{
...
{
    ChipLogDetail(DataManagement, "Header Flags =");
...
CHIP_ERROR TraceDecoder::MaybeLogAndConsumeEncryptedPayload(Json::Value & json)
{
    ...
    {
        auto size = static_cast<uint16_t>(json[kPayloadEncryptedSizeKey].asLargestUInt());
        if (size)
        {
            auto payload   = json[kPayloadEncryptedDataKey].asString();
            auto bufferPtr = json[kPayloadEncryptedBufferPtrKey].asString();
            ChipLogDetail(DataManagement, "Encrypted Payload (%u bytes) = ", size);
            ChipLogDetail(DataManagement, "{");
            ChipLogDetail(DataManagement, "    data       = %s", payload.c_str());
            ChipLogDetail(DataManagement, "    buffer_ptr = %s", bufferPtr.c_str());
            ChipLogDetail(DataManagement, "}");
            ChipLogDetail(DataManagement, "");
        }
    }
...
CHIP_ERROR TraceDecoder::MaybeLogAndConsumeOthers(Json::Value & json)
{
    ...
    if (keys.size())
    {
        ChipLogDetail(DataManagement, "Additional Fields =");
        ChipLogDetail(DataManagement, "{");
        for (std::vector<std::string>::const_iterator it = keys.begin(); it != keys.end(); ++it)
        {
            const char * key        = (*it).c_str();
            const Json::Value value = json[key];
            ChipLogDetail(DataManagement, "    %s = %s", key, value.asString().c_str());
        }
        ChipLogDetail(DataManagement, "}");
        ChipLogDetail(DataManagement, "");
    }
...
CHIP_ERROR TraceDecoder::MaybeLogAndConsumeMessageFlags(Json::Value & json)
{
    ...
    if (flags)
    {
        ...
        if (flags & to_underlying(chip::Header::MsgFlagValues::kDestinationNodeIdPresent))
        {
            ...
            ChipLogDetail(DataManagement, "        DestinationNodeId = 0x%16llx", id);
        }
        ...
        if (flags & to_underlying(chip::Header::MsgFlagValues::kDestinationGroupIdPresent))
        {
            ...
            ChipLogDetail(DataManagement, "        DestinationGroupIdPresent = 0x%04x", id);
        }
...
CHIP_ERROR TraceDecoder::MaybeLogAndConsumeExchangeFlags(Json::Value & json)
{
    ...
    if (flags)
    {
        ...
        ChipLogDetail(DataManagement, "    Exchange (0x%02x) = ", flags);
        ChipLogDetail(DataManagement, "    {");
