...
CHIP_ERROR LogCertificationDeclaration(const ByteSpan & cd)
{
    ...
    while (CHIP_NO_ERROR == (err = reader.Next()))
    {
        ...
        case kTag_AuthorizedPAAList: {
            TLV::TLVType arrayType = TLV::kTLVType_Array;
            ReturnErrorOnFailure(reader.EnterContainer(arrayType));
            IncreaseLogIndent("Authorized PAAs");
            while (CHIP_NO_ERROR == (err = reader.Next()))
            {
                ByteSpan str;
                ReturnErrorOnFailure(reader.Get(str));
...
CHIP_ERROR LogAttestationResponse(TLV::TLVReader & reader)
{
    ...
    IncreaseLogIndent("Attestation Elements");
    IncreaseLogIndent("Certification Declaration");
...
CHIP_ERROR LogCSRResponse(TLV::TLVReader & reader)
{
    ...
    IncreaseLogIndent("NOCSR Elements");
    Log("CSR", csr);
    ...
    while (CHIP_NO_ERROR == (err = reader.Next()))
    {
        if (IsTag(reader, app::CommandDataIB::Tag::kPath))
        {
            ...
            while (CHIP_NO_ERROR == (err = reader.Next()))
            {
                ...
                else if (to_underlying(app::CommandPathIB::kCsTag_CommandId) == TLV::TagNumFromTag(reader.GetTag()))
                {
        ...
        else if (IsTag(reader, app::CommandDataIB::Tag::kData))
        {
            switch (clusterId)
            {
            case app::Clusters::OperationalCredentials::Id:
