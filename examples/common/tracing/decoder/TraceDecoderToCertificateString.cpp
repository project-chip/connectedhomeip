...
bool IsChipCertificate(const chip::ByteSpan & data)
{
    return (data.size() > sizeof(kChipRawPrefix)) && (memcmp(data.data(), kChipRawPrefix, sizeof(kChipRawPrefix)) == 0);
}

const char * ToCertificate(const chip::ByteSpan & data, char * buffer, size_t len, const char * header = "",
                           const char * footer = "")
{
    // Reset the buffer
    memset(buffer, '\0', len);
    ...
    size_t base64DataLen = BASE64_ENCODED_LEN(data.size()) + 1;
    if (base64DataLen + strlen(header) + strlen(footer) > len)
    {
    ...
    auto encodedLen       = chip::Base64Encode(data.data(), data.size(), str.Get());
    str.Get()[encodedLen] = '\0';

    if (IsChipCertificate(data))
    {
    ...
    else
    {
        ...
        {
            outIndex += snprintf(&buffer[outIndex], len - outIndex, "%.64s\n", &str[inIndex]);
        }
        snprintf(&buffer[outIndex], len - outIndex, "%s", footer);
    }
...
const char * ToCertificateString(const ByteSpan & data, char * buffer, size_t len)
{
    constexpr const char * kCertificateHeader = "-----BEGIN CERTIFICATE-----";
    constexpr const char * kCertificateFooter = "-----END CERTIFICATE-----";

    return ToCertificate(data, buffer, len, kCertificateHeader, kCertificateFooter);
}

const char * ToCertificateRequestString(const ByteSpan & data, char * buffer, size_t len)
{
