...
#include "TraceDecoderToCertificateString.h"
...
void ENFORCE_FORMAT(1, 2) Log(const char * format, ...)
{
...
void Log(const char * name, chip::ByteSpan & data)
{
    ...
    chip::CanCastTo<uint16_t>(data.size())
        ? Log("%s (%u) = %s", name, static_cast<uint16_t>(data.size()), chip::trace::ToHexString(data, buffer, sizeof(buffer)))
        : Log("%s (>UINT16_MAX) = %s", name, chip::trace::ToHexString(data, buffer, sizeof(buffer)));
}
...
void Log(const char * name, chip::CharSpan & data)
{
    ...
    chip::CanCastTo<uint16_t>(data.size())
        ? Log("%s (%u) = %.*s", name, static_cast<uint16_t>(data.size()), static_cast<int>(data.size()), data.data())
        : Log("%s (>UINT16_MAX) = %.*s", name, static_cast<int>(data.size()), data.data());
}
...
void LogCertificate(const char * name, const chip::ByteSpan & data)
{
    ...
    chip::CanCastTo<uint16_t>(data.size()) ? Log("%s (%u) =", name, static_cast<uint16_t>(data.size()))
                                           : Log("%s (>UINT16_MAX) =", name);
...
CHIP_ERROR MaybeDecodeNestedResponse(const uint8_t * data, size_t dataLen)
{
    TLV::TLVReader reader;
...
CHIP_ERROR MaybeDecodeNestedRequest(const uint8_t * data, size_t dataLen)
{
    TLV::TLVReader reader;
    reader.Init(data, dataLen);
