                                                     ...
                                                     bool & outDeferredOffer)
{
    ...
    {
        outSession.videoStreamID = MakeOptional(DataModel::MakeNullable(videoStreams[0]));
    }
    else
    {
    ...
    {
        outSession.audioStreamID = MakeOptional(DataModel::MakeNullable(audioStreams[0]));
    }
    else
    {
        outSession.audioStreamID = MakeOptional(DataModel::NullNullable);
    }
...
CHIP_ERROR WebRTCProviderManager::HandleProvideOffer(const ProvideOfferRequestArgs & args, WebRTCSessionStruct & outSession)
{
    ...
    // An Answer can only carry the media sections present in the Offer.
    if (!videoMid.empty())
    {
