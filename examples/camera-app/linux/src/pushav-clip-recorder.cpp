...
void PushAVClipRecorder::SetupOutput(const std::string & outputPrefix, const std::string & initSegPattern,
                                     const std::string & mediaSegPattern, int usevideo)
{
...
bool PushAVClipRecorder::ProcessBuffersAndWrite()
{
    ...
    ChipLogProgress(Camera, "PushAVClipRecoder currentFragmentId: [%d], mLastFragmentId: [%d], mFragmentDuration: [%ld]",
                    currentFragmentId, mLastFragmentId, mFragmentDuration);

    if (clipLengthInPTS >= MAX_CLIP_DURATION)
    {
        ChipLogProgress(Camera,
                        "PushAVClipRecoder Clip length reached max duration. currentPts: [%ld], mCurrentClipStartPts: [%ld], "
    ...
    if (currentFragmentId > mLastFragmentId /* && check is frag exists*/)
    {
        ...
        ChipLogProgress(Camera, "PushAVClipRecoder Invoke Uploader for fragment id: [%d]", mLastFragmentId);
