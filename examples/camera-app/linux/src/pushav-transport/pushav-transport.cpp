                                                      ...
                                                      AudioStreamStruct & audioStreamParams, VideoStreamStruct & videoStreamParams)
{
    ...
    UpdateSendFlags();
    ChipLogProgress(Camera, "Transport[%u] Session[%" PRIu64 "] ConfigureRecorderSettings success - Track=%s HasVideo=%s HasAudio=%s",
...
void PushAVTransport::InitializeRecorder()
{
    ...
    if (mRecorder.get() == nullptr)
    {
        ...
        ChipLogProgress(Camera, "PushAVTransport, Initialize Recorder done !!! FabricIdx: %u Session Id: %" PRId64 "", mFabricIndex,
...
void PushAVTransport::CheckAndUpdateSession()
{
    ...
    if (mSessionStartedTimestamp == std::chrono::system_clock::time_point())
    {
        ...
        mSessionNumber++;
        ChipLogProgress(Camera, "Transport[%u] Session[%" PRIu64 "] SESSION_STARTED: First session initialized for Track=%s", mConnectionID,
    ...
    if (elapsed >= kMaxSessionDurationMinutes)
    {
        ...
        ChipLogProgress(Camera,
                        "Transport[%u] Session[%" PRIu64 "] SESSION_INCREMENTED: Session duration limit reached (%d min). New session "
                        "started. Track=%s",
