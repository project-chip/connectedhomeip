...
#include <WaterHeaterModeDelegate.h>
...
static WaterHeaterModeDelegate * gWaterHeaterModeDelegate = nullptr;
static ModeBase::Instance * gWaterHeaterModeInstance               = nullptr;

CHIP_ERROR WaterHeaterModeDelegate::Init()
{
    return CHIP_NO_ERROR;
}

// todo refactor code by making a parent class for all ModeInstance classes to reduce flash usage.
void WaterHeaterModeDelegate::HandleChangeToMode(uint8_t NewMode,
                                                          ModeBase::Commands::ChangeToModeResponse::Type & response)
{
    response.status = to_underlying(ModeBase::StatusCode::kGenericFailure);
}

CHIP_ERROR WaterHeaterModeDelegate::GetModeLabelByIndex(uint8_t modeIndex, chip::MutableCharSpan & label)
{
...
CHIP_ERROR WaterHeaterModeDelegate::GetModeValueByIndex(uint8_t modeIndex, uint8_t & value)
{
    if (modeIndex >= ArraySize(kModeOptions))
    {
        return CHIP_ERROR_PROVIDER_LIST_EXHAUSTED;
...
CHIP_ERROR WaterHeaterModeDelegate::GetModeTagsByIndex(uint8_t modeIndex, List<ModeTagStructType> & tags)
{
    if (modeIndex >= ArraySize(kModeOptions))
    {
        return CHIP_ERROR_PROVIDER_LIST_EXHAUSTED;
...
void emberAfWaterHeaterModeClusterInitCallback(chip::EndpointId endpointId)
{
    ...
    gWaterHeaterModeDelegate = new WaterHeaterMode::WaterHeaterModeDelegate;
