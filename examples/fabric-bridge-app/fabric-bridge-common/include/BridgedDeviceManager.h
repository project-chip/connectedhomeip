...
public:
    ...
    /**
     * @brief Gets a device from its HandleId.
     *
     * This function iterates through the available devices and returns the device that matches the
     * specified HandleId. If no device matches the HandleId, it returns nullptr.
     *
     * @param handleId The HandleId of the device to be retrieved. The HandleId is generated by Fabric
     *     Admin when it adds a device and is used to uniquely identify a particular Matter device.
     * @return BridgedDevice* A pointer to the device if found, nullptr otherwise.
     */
    BridgedDevice * GetDeviceByHandleId(uint64_t handleId) const;

    /**
     * @brief Removes a device from a dynamic endpoint by its HandleId.
     *
     * This function attempts to remove a device from a dynamic endpoint by iterating through the
     ...
     */
    std::optional<unsigned> RemoveDeviceByHandleId(uint64_t handleId);
