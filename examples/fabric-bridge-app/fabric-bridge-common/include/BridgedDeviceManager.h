...
public:
    ...
    /**
     * @brief Gets a device from its Handle.
     *
     * This function iterates through the available devices and returns the device that matches the
     * specified Handle. If no device matches the Handle, it returns nullptr.
     *
     * @param handle The Handle of the device to be retrieved. The Handle is generated by Fabric
     *     Admin when it adds a device and is used to uniquely identify a particular Matter device.
     * @return BridgedDevice* A pointer to the device if found, nullptr otherwise.
     */
    BridgedDevice * GetDeviceByHandle(uint64_t handle) const;

    /**
     * @brief Removes a device from a dynamic endpoint by its Handle.
     *
     * This function attempts to remove a device from a dynamic endpoint by iterating through the
     * available endpoints and checking if the device matches the specified Handle. If the device is
     * found, it clears the dynamic endpoint, logs the removal,
     *
     * @param handle The Handle of the device to be removed.
     * @return unsigned of the index of the removed dynamic endpoint if successful, nullopt otherwise.
     */
    std::optional<unsigned> RemoveDeviceByHandle(uint64_t handle);
