...
CHIP_ERROR LightingManager::Init()
{
    ...
    if (sLightTimer == NULL)
    {
        SCM1612S_LOG("sLightTimer timer create failed");
...
bool LightingManager::InitiateAction(int32_t aActor, Action_t aAction, uint8_t * aValue)
{
    ...
    case COLOR_ACTION_HSV: {
        ...
        rgb            = HsvToRgb(hsv);
        SCM1612S_LOG("LightingManager HSV(%d,%d) to RGB(%d,%d,%d)", hsv.h, hsv.s, rgb.r, rgb.g, rgb.b);
    }
    break;
    ...
    if (action_initiated)
    {
        ...
        {
            SCM1612S_LOG("LightingManager: set level %d", *aValue);
            sDimLED.SetLevel(*aValue);
        }
        else if (mState == kState_ColorInitiated)
        {
            SCM1612S_LOG("LightingManager: set color(%d,%d,%d)", rgb.r, rgb.g, rgb.b);
            sDimLED.Color(rgb);
        }
    }
...
void LightingManager::StartTimer(uint32_t aTimeoutMs)
{
    ...
    {
        SCM1612S_LOG("app timer already started!");
        CancelTimer();
    }
    ...
    {
        SCM1612S_LOG("sLightTimer timer start() failed");
...
void LightingManager::CancelTimer(void)
{
    ...
    {
        SCM1612S_LOG("sLightTimer stop() failed");
...
void LightingManager::AutoTurnOffTimerEventHandler(AppEvent * aEvent)
{
    ...
    SCM1612S_LOG("Auto Turn Off has been triggered!");
...
void LightingManager::ActuatorMovementTimerEventHandler(AppEvent * aEvent)
{
    ...
    if (actionCompleted != INVALID_ACTION)
    {
            ...
            (actionCompleted == ON_ACTION || actionCompleted == LEVEL_ACTION || actionCompleted == COLOR_ACTION))
        {
            ...
            SCM1612S_LOG("Auto Turn off enabled. Will be triggered in %u seconds", light->mAutoTurnOffDuration);
        }
    }
}
