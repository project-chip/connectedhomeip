...
CHIP_ERROR ColorLightCommandHandler(int argc, char ** argv)
{
    ...
    SCM1612S_LOG("ColorLightCommandHandler: %x", rgb);
...
CHIP_ERROR LevelLightCommandHandler(int argc, char ** argv)
{
    ...
    level = atoi(argv[0]);
    SCM1612S_LOG("LevelLightCommandHandler: %d", level);
...
CHIP_ERROR AppTask::StartAppTask()
{
    ...
    {
        SCM1612S_LOG("Failed to allocate app event queue");
...
CHIP_ERROR AppTask::Init()
{
    ...
    {
        SCM1612S_LOG("funct timer create failed");
        appError(APP_ERROR_CREATE_TIMER_FAILED);
    }

    SCM1612S_LOG("Current Software Version: %s", CHIP_DEVICE_CONFIG_DEVICE_SOFTWARE_VERSION_STRING);
    error = LightMgr().Init();
    if (error != CHIP_NO_ERROR)
    {
        SCM1612S_LOG("LightMgr Init failed");
        appError(error);
    }
...
void AppTask::AppTaskMain(void * pvParameter)
{
    ...
    if (error != CHIP_NO_ERROR)
    {
        SCM1612S_LOG("AppTask.Init() failed");
        appError(error);
    }

    SCM1612S_LOG("AppTask started");
...
void AppTask::LightActionEventHandler(AppEvent * aEvent)
{
    ...
    if (err == CHIP_NO_ERROR)
    {
        ...
        {
            SCM1612S_LOG("Lighting level: %d", level);
            initiated = LightMgr().InitiateAction(AppEvent::kEventType_Light, action, &level);
        }

        if (!initiated)
        {
            SCM1612S_LOG("Action is already in progress or active.");
        }
    }
}
...
void AppTask::ButtonTimerEventHandler(AppEvent * aEvent)
{
    ...
    case kFunction_LightSwitch:
        ...
        SCM1612S_LOG("Factory Reset Triggered.");
        SCM1612S_LOG("Release button within %ums to cancel.", FACTORY_RESET_CANCEL_WINDOW_TIMEOUT);
    ...
    case kFunction_FactoryReset:
        SCM1612S_LOG("Factory Reset Start.");
...
void AppTask::OccupancyEventHandler(AppEvent * aEvent)
{
    ...
    {
        SCM1612S_LOG("A Non Occupancy received %d", aEvent->Type);
        return;
    }

    uint8_t attributeValue = aEvent->OccupancytEvent.Present ? 1 : 0;

    SCM1612S_LOG("Lighting occupancy: %u", attributeValue);
...
void AppTask::SingleButtonEventHandler(AppEvent * aEvent)
{
    ...
    {
        SCM1612S_LOG("A Non ButtonEvent received %d", aEvent->Type);
        return;
    }
    ...
    if (aEvent->ButtonEvent.Pressed)
    {
        if (sAppTask.mFunctionTimerActive == false)
        {
            ...
            SCM1612S_LOG("AppTask status LED on");
            sStatusLED.Set(true);
        ...
        {
            SCM1612S_LOG("AppTask function timer already started");
        }
    }
    else
    {
        ...
        {
        case kFunction_LightSwitch:
            SCM1612S_LOG("AppTask light switch");
            AppEvent event;
        ...
        case kFunction_FactoryReset:
            ...
            SCM1612S_LOG("AppTask factory reset cancelled");
            break;
        default:
            SCM1612S_LOG("not handled key release event, mFunction = %x", sAppTask.mFunction);
            break;
        }
...
void AppTask::CancelTimer()
{
    ...
    {
        SCM1612S_LOG("app timer stop() failed");
...
void AppTask::StartTimer(uint32_t aTimeoutInMs)
{
    ...
    {
        SCM1612S_LOG("app timer start() failed");
...
void AppTask::ActionInitiated(LightingManager::Action_t aAction, int32_t aActor)
{
    ...
    {
        SCM1612S_LOG("Turning light ON")
    }
    else if (aAction == LightingManager::OFF_ACTION)
    {
        SCM1612S_LOG("Turning light OFF")
    }
    else if (aAction == LightingManager::LEVEL_ACTION)
    {
        SCM1612S_LOG("Changing light level")
    }
    else if (aAction == LightingManager::COLOR_ACTION)
    {
        SCM1612S_LOG("Changing light color")
    }
}
...
void AppTask::ActionCompleted(LightingManager::Action_t aAction)
{
    ...
    if (aAction == LightingManager::ON_ACTION)
    {
        SCM1612S_LOG("Light ON")
    }
    else if (aAction == LightingManager::OFF_ACTION)
    {
        SCM1612S_LOG("Light OFF")
    }
    else if (aAction == LightingManager::LEVEL_ACTION)
    {
        SCM1612S_LOG("Light level changed")
    }
    else if (aAction == LightingManager::COLOR_ACTION)
    {
        SCM1612S_LOG("Light color changed")
    }
}
...
void AppTask::PostEvent(const AppEvent * aEvent)
{
    if (sAppEventQueue != NULL)
    {
        ...
        if (!status)
            SCM1612S_LOG("Failed to post event to app task event queue");
    }
    else
    {
        SCM1612S_LOG("Event Queue is NULL should never happen");
    }
}
...
void AppTask::DispatchEvent(AppEvent * aEvent)
{
    ...
    {
        SCM1612S_LOG("Event received with no handler. Dropping event.");
    }
}
