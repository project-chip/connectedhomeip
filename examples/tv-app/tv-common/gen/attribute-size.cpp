    ...
    case 0x050B: // Audio Output Cluster
    {
        ...
        switch (am->attributeId)
        {
            case 0x0000: // audio output list
            {
                ...
                copyListMember(write ? dest : (uint8_t *)&entry->index, write ? (uint8_t *)&entry->index : src, write, &entryOffset, sizeof(entry->index)); // INT8U
                copyListMember(write ? dest : (uint8_t *)&entry->outputType, write ? (uint8_t *)&entry->outputType : src, write, &entryOffset, sizeof(entry->outputType)); // AudioOutputType
    ...
    case 0x050A: // Content Launcher Cluster
    {
        ...
        switch (am->attributeId)
        {
            case 0x0000: // accepts header list
            {
                ...
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, acceptsHeaderListRemainingSpace, acceptsHeaderListSpan) : ReadByteSpan(src + entryOffset, acceptsHeaderListRemainingSpace, acceptsHeaderListSpan)))
                {
                    ChipLogError(Zcl, "Index %" PRId32 " is invalid. Not enough remaining space", index);
                    return 0;
                }
    ...
    case 0x0507: // Media Input Cluster
    {
        ...
        switch (am->attributeId)
        {
            case 0x0000: // media input list
            {
                ...
                copyListMember(write ? dest : (uint8_t *)&entry->index, write ? (uint8_t *)&entry->index : src, write, &entryOffset, sizeof(entry->index)); // INT8U
                copyListMember(write ? dest : (uint8_t *)&entry->inputType, write ? (uint8_t *)&entry->inputType : src, write, &entryOffset, sizeof(entry->inputType)); // MediaInputType
                chip::ByteSpan * nameSpan = &entry->name; // OCTET_STRING
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, 34, nameSpan) : ReadByteSpan(src + entryOffset, 34, nameSpan)))
                {
                    ChipLogError(Zcl, "Index %" PRId32 " is invalid. Not enough remaining space", index);
                ...
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, 34, descriptionSpan) : ReadByteSpan(src + entryOffset, 34, descriptionSpan)))
                {
                    ChipLogError(Zcl, "Index %" PRId32 " is invalid. Not enough remaining space", index);
                    return 0;
                }
    ...
    case 0x0504: // TV Channel Cluster
    {
        ...
        switch (am->attributeId)
        {
            case 0x0000: // tv channel list
            {
                ...
                copyListMember(write ? dest : (uint8_t *)&entry->majorNumber, write ? (uint8_t *)&entry->majorNumber : src, write, &entryOffset, sizeof(entry->majorNumber)); // INT16U
                copyListMember(write ? dest : (uint8_t *)&entry->minorNumber, write ? (uint8_t *)&entry->minorNumber : src, write, &entryOffset, sizeof(entry->minorNumber)); // INT16U
                chip::ByteSpan * nameSpan = &entry->name; // OCTET_STRING
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, 34, nameSpan) : ReadByteSpan(src + entryOffset, 34, nameSpan)))
                {
                    ChipLogError(Zcl, "Index %" PRId32 " is invalid. Not enough remaining space", index);
                ...
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, 34, callSignSpan) : ReadByteSpan(src + entryOffset, 34, callSignSpan)))
                {
                    ChipLogError(Zcl, "Index %" PRId32 " is invalid. Not enough remaining space", index);
                    return 0;
                }
                ...
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, 34, affiliateCallSignSpan) : ReadByteSpan(src + entryOffset, 34, affiliateCallSignSpan)))
                {
                    ChipLogError(Zcl, "Index %" PRId32 " is invalid. Not enough remaining space", index);
                    return 0;
                }
    ...
    case 0x0505: // Target Navigator Cluster
    {
        ...
        switch (am->attributeId)
        {
            case 0x0000: // target navigator list
            {
                ...
                copyListMember(write ? dest : (uint8_t *)&entry->identifier, write ? (uint8_t *)&entry->identifier : src, write, &entryOffset, sizeof(entry->identifier)); // INT8U
                chip::ByteSpan * nameSpan = &entry->name; // OCTET_STRING
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, 34, nameSpan) : ReadByteSpan(src + entryOffset, 34, nameSpan)))
                {
                    ChipLogError(Zcl, "Index %" PRId32 " is invalid. Not enough remaining space", index);
