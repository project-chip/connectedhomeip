    ...
    switch (intentAction) {
      case MatterIntentConstants.ACTION_MATTER_COMMAND:
        ...
        if (commandId != -1) {
          ...
          String attributeAction = intent.getStringExtra(MatterIntentConstants.EXTRA_ATTRIBUTE_ACTION);
          if (attributeAction.equals(MatterIntentConstants.ATTRIBUTE_ACTION_READ)) {
            String response;
            if (attributeId == ACCEPT_HEADER) {
              response = "{\"0\": [\"video/mp4\", \"application/x-mpegURL\", \"application/dash+xml\"] }";
  ...
  private void sendResponseViaPendingIntent(Context context, Intent intent, String response) {
    ...
    if (pendingIntent != null) {
      final Intent responseIntent =
              new Intent()
                      .putExtra(
                              MatterIntentConstants.EXTRA_RESPONSE_PAYLOAD, response.getBytes());
      try {
