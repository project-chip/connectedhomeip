...
public class MatterAgentClient {
    ...
    private synchronized boolean bindService(Context context) {
        ...
        if (intent.getComponent() == null) {
            final ResolveInfo resolveInfo = resolveBindIntent(context,
                    intent,
            ...
            final ComponentName component = new ComponentName(resolveInfo.serviceInfo.packageName,
     ...
     */
    private ResolveInfo resolveBindIntent(final Context context,
        ...
        if (bindPermission == null || permissionHeldByService == null) {
            Log.w(TAG, "Must specify the permission protecting the service, as well as " +
        ...
        if (infos.size() > 1) {
            Log.w(TAG, "More than one permission-enforced system" +
                    " service can handle intent " + bindIntent + " and permission " + bindPermission);
        }
    ...
    private class MyServiceConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder binder) {
            Log.d(TAG, String.format("onServiceConnected for API with intent action %s", MatterIntentConstants.ACTION_MATTER_AGENT));
