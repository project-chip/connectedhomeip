...
public class EndpointsDataStore {
    ...
    private EndpointsDataStore(Context context) {
        ...
        for (Map.Entry<String, String> contentAppEntry: appMetadata.entrySet()) {
            persistedContentApps.put(contentAppEntry.getKey(), deserializeContentApp(contentAppEntry.getKey(), contentAppEntry.getValue()));
        }
    }
    ...
    private String serializeContentApp(ContentApp app) {
        ...
        try {
            jsonWriter.beginObject().name(KEY_VENDORID).value(app.getVendorId())
                                    .name(KEY_VENDORNAME).value(app.getVendorName())
                                    .name(KEY_PRODUCTID).value(app.getProductId())
                                    .name(KEY_VERSION).value(app.getVersion())
                                    .name(KEY_ENDPOINTID).value(app.getEndpointId())
                                    .endObject();
            jsonWriter.flush();
            jsonWriter.close();
        } catch (IOException e) {
