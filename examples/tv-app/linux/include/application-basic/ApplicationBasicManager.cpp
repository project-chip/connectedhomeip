...
{
    if (strcmp(application->vendorName, ""))
    {
        ...
        {
            std::cerr << "Failed to store vendor name attribute.";
        }
    }
    ...
    if (application->vendorId != 0)
    {
        ...
        {
            std::cerr << "Failed to store vendor id attribute.";
        }
    }
    ...
    if (strcmp(application->name, ""))
    {
        ...
        {
            std::cerr << "Failed to store name attribute.";
        }
    }
    ...
    if (application->productId != 0)
    {
        ...
        {
            std::cerr << "Failed to store product id attribute.";
        }
    }
    ...
    if (!strcmp(application->id, ""))
    {
        ...
        {
            std::cerr << "Failed to store id attribute.";
        }
    }
    ...
    if (application->catalogVendorId != 0)
    {
        ...
        {
            std::cerr << "Failed to store catalog vendor id attribute.";
        }
    }
    ...
    {
        std::cerr << "Failed to store status attribute.";
    }
}
...
Application ApplicationBasicManager::getApplicationForEndpoint(chip::EndpointId endpoint)
{
    ...
    {
        std::cerr << "Failed to get app name. ERR:" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "vendorName", app.vendorName, size);
    if (err != CHIP_NO_ERROR)
    {
        std::cerr << "Failed to get app vendor name. ERR:" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "id", app.id, size);
    if (err != CHIP_NO_ERROR)
    {
        std::cerr << "Failed to get app id. ERR:" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "catalogVendorId", app.catalogVendorId);
    if (err != CHIP_NO_ERROR)
    {
        std::cerr << "Failed to get app catalog vendor id. ERR:" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "productId", app.productId);
    if (err != CHIP_NO_ERROR)
    {
        std::cerr << "Failed to get app product id. ERR:" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "vendorId", app.vendorId);
    if (err != CHIP_NO_ERROR)
    {
        std::cerr << "Failed to get app vendor id. ERR:" << chip::ErrorStr(err) << std::endl;
    }
