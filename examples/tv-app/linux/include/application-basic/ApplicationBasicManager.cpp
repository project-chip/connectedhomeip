...
Application ApplicationBasicManager::getApplicationForEndpoint(chip::EndpointId endpoint)
{
    ...
    {
        std::cerr << "Failed to get app name" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "vendorName", app.vendorName, size);
    {
        std::cerr << "Failed to get app vendor name" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "id", app.id, size);
    {
        std::cerr << "Failed to get app id" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "catalogVendorId", app.catalogVendorId);
    {
        std::cerr << "Failed to get app catalog vendor id" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "productId", app.productId);
    {
        std::cerr << "Failed to get app product id" << chip::ErrorStr(err) << std::endl;
    }

    err = es->get(section, "vendorId", app.vendorId);
    {
        std::cerr << "Failed to get app vendor id" << chip::ErrorStr(err) << std::endl;
    }
