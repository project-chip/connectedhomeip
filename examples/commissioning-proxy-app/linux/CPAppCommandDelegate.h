...
class CPAppCommandHandler
{
public:
    static CPAppCommandHandler * FromJSON(const char * json);

    static void HandleCommand(intptr_t context);

    CPAppCommandHandler(Json::Value && jasonValue) : mJsonValue(std::move(jasonValue)) {}
...
class CPAppCommandDelegate : public NamedPipeCommandDelegate
{
public:
