...
{
public:
    static CommissioningProxyCommandHandler * FromJSON(const char * json);

    static void HandleCommand(intptr_t context);

    CommissioningProxyCommandHandler(Json::Value && jasonValue) : mJsonValue(std::move(jasonValue)) {}
...
class CommissioningProxyAppCommandDelegate : public NamedPipeCommandDelegate
{
public:
