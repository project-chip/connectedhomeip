...
You can use the `,bridged` option modifier on a device definition to automatically wrap it in a parent `bridged-node` endpoint. This is the typical topology used for representing bridged devices in Matter (exposing bridging metadata via the `BridgedDeviceBasicInformation` cluster on a parent node).
