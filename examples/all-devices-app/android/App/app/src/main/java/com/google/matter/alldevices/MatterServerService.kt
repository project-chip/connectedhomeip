...
/**
 * MatterServerService runs the native C++ Matter event loop and dynamic device simulator engine 
 * as an Android Foreground Service.
 *
 * This is necessary because modern Android OS aggressively restricts background processes 
 * (via the Cached Apps Freezer). Without a foreground service, the emulator/device OS halts 
 * the Matter UDP/TCP event loop threads shortly after the application loses screen focus, 
 * causing commissioning and cluster command handshakes to time out or fail.
 */
    ...
    private fun startServerInstance(discriminator: Int, configurationJson: String) {
        try {
            ...
            androidChipPlatform?.updateCommissionableDataProviderData(
                null, null, 0, 20202021L, discriminator
            )
