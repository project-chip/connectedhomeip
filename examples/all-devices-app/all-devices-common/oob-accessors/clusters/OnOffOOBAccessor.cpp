...
std::optional<CHIP_ERROR> OnOffOOBAccessor::HandleAction(CharSpan action, ByteSpan tlvData)
{
    if (!action.data_equal("SetOnOff"_span))
    {
        return std::nullopt;
    }
    ...
    while ((err = reader.Next()) == CHIP_NO_ERROR)
    {
        ...
        case 1:
            ...
            break;
        case 2:
            ReturnErrorOnFailure(reader.Get(onOff));
            hasOnOff = true;
            break;
        default:
            break;
    ...
    return mCluster.SetOnOff(onOff);
}
