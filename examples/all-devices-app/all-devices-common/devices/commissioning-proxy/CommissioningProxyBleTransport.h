...
// by chip_config_network_layer_ble).  Exposes a Ble:: API the dispatcher in
// CommissioningProxyCommandDelegate.cpp calls without knowing transport-specific
// internals.
namespace Ble {
...
// ProxyBackgroundScanStartRequest / Stop paths for BLE.
// Note: transport and wiFiBands are accepted for interface symmetry across
// transports but are unused — BLE scanning has no band concept.
...
// for BLE — kept for cross-transport interface symmetry (see the implementation
// comment for details).
