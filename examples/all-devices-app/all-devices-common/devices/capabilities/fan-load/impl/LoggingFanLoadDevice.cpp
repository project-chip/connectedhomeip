...
Protocols::InteractionModel::Status LoggingFanLoadDevice::HandleStep(FanControl::StepDirectionEnum aDirection, bool aWrap,
                                                                     bool aLowestOff)
{
    ChipLogProgress(DeviceLayer, "LoggingFanLoadDevice::HandleStep() -> direction=%u wrap=%d lowestOff=%d",
    ...
    {
        ChipLogProgress(DeviceLayer, "LoggingFanLoadDevice::HandleStep() -> Speed changed from %u to %u", speedSetting.Value(),
                        newSpeedSetting);
    }
    else if (speedSetting.IsNull())
    {
        ChipLogProgress(DeviceLayer, "LoggingFanLoadDevice::HandleStep() -> Speed changed from NULL to %u", newSpeedSetting);
    }

    return cluster.SetSpeedSetting(DataModel::MakeNullable(newSpeedSetting));
}

void LoggingFanLoadDevice::OnFanDriveStateChanged(const FanControl::FanDriveState & newState)
{
    ...
    else if (!percentSettingIsNull)
    {
        ChipLogProgress(
            DeviceLayer,
            "LoggingFanLoadDevice::OnFanDriveStateChanged() -> mode=%u percentSetting=%u percentCurrent=%u speedSetting=NULL "
            "speedCurrent=%u",
    ...
    else if (!speedSettingIsNull)
    {
        ChipLogProgress(
            DeviceLayer,
            "LoggingFanLoadDevice::OnFanDriveStateChanged() -> mode=%u percentSetting=NULL percentCurrent=%u speedSetting=%u "
            "speedCurrent=%u",
    ...
    else
    {
        ChipLogProgress(
            DeviceLayer,
            "LoggingFanLoadDevice::OnFanDriveStateChanged() -> mode=%u percentSetting=NULL percentCurrent=%u speedSetting=NULL "
            "speedCurrent=%u",
...
void LoggingFanLoadDevice::OnRockSettingChanged(BitMask<FanControl::RockBitmap> newValue)
{
    ChipLogProgress(DeviceLayer, "LoggingFanLoadDevice::OnRockSettingChanged() -> 0x%02X", newValue.Raw());
}

void LoggingFanLoadDevice::OnWindSettingChanged(BitMask<FanControl::WindBitmap> newValue)
{
    ChipLogProgress(DeviceLayer, "LoggingFanLoadDevice::OnWindSettingChanged() -> 0x%02X", newValue.Raw());
}

void LoggingFanLoadDevice::OnAirflowDirectionChanged(FanControl::AirflowDirectionEnum newValue)
{
    ChipLogProgress(DeviceLayer, "LoggingFanLoadDevice::OnAirflowDirectionChanged() -> %u",
                    static_cast<unsigned>(to_underlying(newValue)));
}

// OnOffDelegate

void LoggingFanLoadDevice::OnOffStartup(bool on)
{
    ApplyOnOffToFan(FanControlCluster(), on);
    ChipLogProgress(DeviceLayer, "LoggingFanLoadDevice::OnOffStartup() -> %s", on ? "ON" : "OFF");
}

void LoggingFanLoadDevice::OnOnOffChanged(bool on)
{
    ApplyOnOffToFan(FanControlCluster(), on);
    ChipLogProgress(DeviceLayer, "LoggingFanLoadDevice::OnOffChanged() -> %s", on ? "ON" : "OFF");
}
