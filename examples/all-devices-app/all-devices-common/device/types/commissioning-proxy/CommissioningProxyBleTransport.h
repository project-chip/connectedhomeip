...
// BLE central transport driver for the Linux commissioning-proxy example. Only
// compiled when CONFIG_NETWORK_LAYER_BLE is on (the .cpp is gated at GN level by
// chip_config_network_layer_ble). Implements CommissioningProxyTransport; the
// cluster owns all transport-agnostic session/scan/message bookkeeping and this
// driver reports results back through its host cluster's subsystem accessors.
//
// A single instance exists per device; the underlying BleLayer central callbacks
// are C-style singletons, so the driver's state is process-wide.
