...
CHIP_ERROR NetworkInfrastructureManager::Init(AttributeChangeCallback * attributeChangeCallback)
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::Init called");
    mAttributeChangeCallback = attributeChangeCallback;
    return CHIP_NO_ERROR;
}

bool NetworkInfrastructureManager::GetPanChangeSupported()
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::GetPanChangeSupported called");
    return true;
}

void NetworkInfrastructureManager::GetBorderRouterName(MutableCharSpan & borderRouterName)
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::GetBorderRouterName called");
    CopyCharSpanToMutableCharSpanWithTruncation("all-devices-br"_span, borderRouterName);
}

CHIP_ERROR NetworkInfrastructureManager::GetBorderAgentId(MutableByteSpan & borderAgentId)
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::GetBorderAgentId called");
...
uint16_t NetworkInfrastructureManager::GetThreadVersion()
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::GetThreadVersion called");
    return kThreadVersionForThread_1_3_1;
}

bool NetworkInfrastructureManager::GetInterfaceEnabled()
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::GetInterfaceEnabled called");
    return !mActiveDataset.IsEmpty();
}

CHIP_ERROR NetworkInfrastructureManager::GetDataset(Thread::OperationalDataset & dataset, DatasetType type)
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::GetDataset called (type: %d)", static_cast<int>(type));
...
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::CommitActiveDataset called");
    return CHIP_NO_ERROR;
}

CHIP_ERROR NetworkInfrastructureManager::RevertActiveDataset()
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::RevertActiveDataset called");
    return CHIP_ERROR_NOT_IMPLEMENTED;
}

CHIP_ERROR NetworkInfrastructureManager::SetPendingDataset(const Thread::OperationalDataset & pendingDataset)
{
    ChipLogProgress(AppServer, "NetworkInfrastructureManager::SetPendingDataset called");
