...
{
public:
    static OtaRequestorAppCommandHandler * FromJSON(const char * json, OtaRequestorAppCommandDelegate * delegate);
    ...
    {
    private:
        friend class OtaRequestorAppCommandHandler;
        PrivateCtorTag() = default;
    };

    OtaRequestorAppCommandHandler(PrivateCtorTag, Json::Value && v, OtaRequestorAppCommandDelegate * d) :
        mCommandPayload(std::move(v)), mDelegate(d)
    {}

private:
    Json::Value mCommandPayload;
    OtaRequestorAppCommandDelegate * mDelegate = nullptr;
};
