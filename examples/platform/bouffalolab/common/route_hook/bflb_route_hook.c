...
typedef struct bflb_route_hook_t
{
    struct netif * netif;
    struct raw_pcb * pcb;
    struct bflb_route_hook_t * next;
} bflb_route_hook_t;
...
static bflb_route_hook_t * s_hooks;
    ...
    while (payload_len >= 2)
    {
            ...
            payload_len >= opt_len)
        {
            ...
            if (rio_data_len >= prefix_len_bytes)
            {
                ...
                if (bflb_route_table_add_route_entry(&route) == NULL)
                {
...
static uint8_t icmp6_raw_recv_handler(void * arg, struct raw_pcb * pcb, struct pbuf * p, const ip_addr_t * addr)
{
    ...
    ip6_addr_t src;
    ip6_addr_t dest;
    bflb_route_hook_t * hook = (bflb_route_hook_t *) arg;
...
int8_t bflb_route_hook_init(void)
{
    ...
    for (bflb_route_hook_t * iter = s_hooks; iter != NULL; iter = iter->next)
    {
    ...
    hook = (bflb_route_hook_t *) malloc(sizeof(bflb_route_hook_t));
    if (hook == NULL)
    {
