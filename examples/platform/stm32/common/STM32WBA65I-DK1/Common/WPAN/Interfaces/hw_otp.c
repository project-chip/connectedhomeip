...
int HW_OTP_Read(uint8_t index, HW_OTP_data_t ** otp_ptr)
{
    ...
    *otp_ptr = (HW_OTP_data_t *) (FLASH_OTP_BASE + FLASH_OTP_SIZE - 16);
    ...
    while (((*otp_ptr)->index != index) && ((*otp_ptr) != (HW_OTP_data_t *) FLASH_OTP_BASE))
    {
        (*otp_ptr) -= 1;
    }
...
int HW_OTP_Write(uint8_t * additional_data, uint8_t * bd_address, uint8_t hsetune, uint8_t index)
{
    ...
    while ((*(uint64_t *) (free_address) != 0xFFFFFFFFFFFFFFFFUL) && (*(uint64_t *) (free_address + 8u) != 0xFFFFFFFFFFFFFFFFUL) &&
           ((free_address + 16u) != FLASH_OTP_BASE + FLASH_OTP_SIZE))
    {
        free_address += 16;
    }

    if ((*(uint64_t *) (free_address) != 0xFFFFFFFFFFFFFFFFUL) && (*(uint64_t *) (free_address + 8u) != 0xFFFFFFFFFFFFFFFFUL))
    {
        return 1; /* error */
    }
    ...
    err |= HAL_FLASH_Program(FLASH_NSCR1_PG, free_address, (uint32_t) &otp);
    err |= HAL_FLASH_Lock();
