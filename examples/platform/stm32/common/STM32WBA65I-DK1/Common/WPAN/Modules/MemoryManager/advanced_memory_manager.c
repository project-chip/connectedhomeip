...
uint32_t GetCurrentOccupation(const uint8_t VirtualMemoryId)
{
    ...
    if (AmmInitialized == INITIALIZED)
    {
        ...
        else
        {
            ...
            for (uint32_t memIdx = 0x00; (memIdx < AmmVirtualMemoryNumber) && (currentOccupation == 0x00); memIdx++)
            {
...
uint32_t GetPeakOccupation(const uint8_t VirtualMemoryId)
{
    ...
    if (AmmInitialized == INITIALIZED)
    {
        ...
        {
            /* Check virtual memory info */
            for (uint32_t memIdx = 0x00; (memIdx < AmmVirtualMemoryNumber) && (peakOccupation == 0x00); memIdx++)
            {
                /* Check if ID is known */
                if (VirtualMemoryId == p_AmmVirtualMemoryList[memIdx].Id)
                {
...
AMM_Function_Error_t AMM_Init(const AMM_InitParameters_t * const p_InitParams)
{
    ...
    else if ((p_InitParams->VirtualMemoryNumber != 0x00) && (p_InitParams->p_VirtualMemoryConfigList == NULL))
    {
    ...
    else
    {
        ...
        for (uint32_t memIdx = 0x00; (memIdx < p_InitParams->VirtualMemoryNumber) && (error == AMM_ERROR_NOK); memIdx++)
        {
        ...
        if (error == AMM_ERROR_NOK)
        {
            ...
            if ((AmmBmmFunctionsHandler.Init == NULL) || (AmmBmmFunctionsHandler.Allocate == NULL) ||
                (AmmBmmFunctionsHandler.Free == NULL))
            {
            ...
            else
            {
                ...
                AmmBmmFunctionsHandler.Init(p_AmmPoolAddress, AmmPoolSize);

                /* Allocate memory for the virtual memories info */
                p_AmmVirtualMemoryList = (VirtualMemoryInfo_t *) (AmmBmmFunctionsHandler.Allocate(
                    AMM_VIRTUAL_INFO_ELEMENT_SIZE * p_InitParams->VirtualMemoryNumber));
                ...
                else
                {
                    ...
                    AmmOccupiedSharedPoolSize =
                        AmmOccupiedSharedPoolSize + (AMM_VIRTUAL_INFO_ELEMENT_SIZE * AmmVirtualMemoryNumber);

#if (AMM_USE_MEMORY_STATISTICS == 1)
                    AmmPeakSharedPoolSize = AmmOccupiedSharedPoolSize;
#endif /* AMM_USE_MEMORY_STATISTICS */

                    for (uint32_t memIdx = 0x00; memIdx < AmmVirtualMemoryNumber; memIdx++)
                    {
                               ...
                               AMM_VirtualMemoryCallbackFunction_t * const p_CallBackFunction)
{
    ...
    *pp_AllocBuffer = NULL;
    ...
    else if (VirtualMemoryId == AMM_NO_VIRTUAL_ID)
    {
        ...
        if (BufferSize < (AmmPoolSize - AmmOccupiedSharedPoolSize - AmmRequiredVirtualMemorySize))
        {
            ...
            if (p_TmpAllocAddr != NULL)
            {
                /* Fulfill the header */
                *p_TmpAllocAddr =
                    (uint32_t) (((uint32_t) VirtualMemoryId << VIRTUAL_MEMORY_HEADER_ID_POS) & VIRTUAL_MEMORY_HEADER_ID_MASK) |
                    ((BufferSize << VIRTUAL_MEMORY_HEADER_BUFFER_SIZE_POS) & VIRTUAL_MEMORY_HEADER_BUFFER_SIZE_MASK);
    ...
    else
    {
        ...
        /* Check virtual memory info */
        for (uint32_t memIdx = 0x00; (memIdx < AmmVirtualMemoryNumber) && (error == AMM_ERROR_UNKNOWN_ID); memIdx++)
        {
            /* Check if ID is known */
            if (VirtualMemoryId == p_AmmVirtualMemoryList[memIdx].Id)
            {
                ...
                if (p_AmmVirtualMemoryList[memIdx].OccupiedSize < p_AmmVirtualMemoryList[memIdx].RequiredSize)
                {
                    ...
                    selfAvailable = p_AmmVirtualMemoryList[memIdx].RequiredSize - p_AmmVirtualMemoryList[memIdx].OccupiedSize;
                }
                    ...
                    if (p_TmpAllocAddr != NULL)
                    {
                        ...
                        *pp_AllocBuffer = (uint32_t *) (p_TmpAllocAddr + VIRTUAL_MEMORY_HEADER_SIZE);
                        ...
                        p_AmmVirtualMemoryList[memIdx].OccupiedSize =
                            p_AmmVirtualMemoryList[memIdx].OccupiedSize + BufferSize + VIRTUAL_MEMORY_HEADER_SIZE;
                        ...
                        if (p_AmmVirtualMemoryList[memIdx].RequiredSize < p_AmmVirtualMemoryList[memIdx].OccupiedSize)
                        {
                            ...
                            AmmOccupiedSharedPoolSize =
                                AmmOccupiedSharedPoolSize + BufferSize - selfAvailable + VIRTUAL_MEMORY_HEADER_SIZE;
#if (AMM_USE_MEMORY_STATISTICS == 1)
                            /* Check if peak occupation has to be updated */
                            if (AmmOccupiedSharedPoolSize > AmmPeakSharedPoolSize)
                            {
...
AMM_Function_Error_t AMM_Free(uint32_t * const p_BufferAddr)
{
    ...
    else if ((p_BufferAddr > (p_AmmPoolAddress + AmmPoolSize)) || (p_BufferAddr < p_AmmPoolAddress))
    {
    ...
    else
    {
        ...
        else
        {
            ...
            for (uint32_t memIdx = 0x00; (memIdx < AmmVirtualMemoryNumber) && (error == AMM_ERROR_UNKNOWN_ID); memIdx++)
            {
                ...
                {
                    occupiedOverRequired =
                        (p_AmmVirtualMemoryList[memIdx].OccupiedSize - p_AmmVirtualMemoryList[memIdx].RequiredSize);
                    ...
                    /* Update the occupation size */
                    p_AmmVirtualMemoryList[memIdx].OccupiedSize = p_AmmVirtualMemoryList[memIdx].OccupiedSize - allocatedSize;
