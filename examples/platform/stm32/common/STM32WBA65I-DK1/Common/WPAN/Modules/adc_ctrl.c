...
static ADCCTRL_Handle_t VRefIntRequest_Handle = {
    ...
    .ChannelConf = { .Channel      = LL_ADC_CHANNEL_VREFINT, /* Internal reference voltage channel */
...
__WEAK ADCCTRL_Cmd_Status_t ADCCTRL_RequestTemperature(const ADCCTRL_Handle_t * const p_Handle, int16_t * const p_ReadValue)
{
    ADCCTRL_Cmd_Status_t error = ADCCTRL_UNKNOWN;
    ...
    else
    {
        ...
        if (ADCCTRL_OK == error)
        {
            ...
            if (ADCCTRL_OK == error)
            {
                ...
                vddValue =
                    __LL_ADC_CALC_VREFANALOG_VOLTAGE(uhADCxConvertedData, VRefIntRequest_Handle.InitConf.ConvParams.Resolution);
            }
            else
            {
                error = ADCCTRL_ERROR_CONFIG;
            ...
            if (ADCCTRL_OK == error)
            {
                ...
                if (*TEMPSENSOR_CAL1_ADDR == *TEMPSENSOR_CAL2_ADDR)
                {
                    ...
                    *p_ReadValue = __LL_ADC_CALC_TEMPERATURE_TYP_PARAMS(TEMPSENSOR_TYP_AVGSLOPE, TEMPSENSOR_TYP_CAL1_V,
                                                                        TEMPSENSOR_CAL1_TEMP, vddValue, uhADCxConvertedData,
                ...
                else
                {
                    ...
                    *p_ReadValue =
                        __LL_ADC_CALC_TEMPERATURE(vddValue, uhADCxConvertedData, p_Handle->InitConf.ConvParams.Resolution);
                }
            }
#else
            /* Is the current config IS NOT the same as the one requested ? */
    ...
    else
    {
        ...
        if (ADCCTRL_OK == error)
        {
            ...
            if (ADCCTRL_OK == error)
            {
                ...
                /* using LL ADC driver helper macro.                                      */
                *p_ReadValue =
                    __LL_ADC_CALC_DATA_TO_VOLTAGE(vddValue, uhADCxConvertedData, p_Handle->InitConf.ConvParams.Resolution);
            }
#else
            /* Is the current config IS NOT the same as the one requested ? */
            if (CurrentConfig != p_Handle->Uid)
