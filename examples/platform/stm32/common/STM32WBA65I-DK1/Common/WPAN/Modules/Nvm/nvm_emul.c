...
#if CFG_NVM_ALIGN != 0
void NVM_Init(uint64_t * buffer, uint16_t size, uint16_t max_size)
#else
...
int NVM_Add(uint8_t type, const uint8_t * data, uint16_t size, const uint8_t * extra_data, uint16_t extra_size)
{
...
int NVM_Get(uint8_t mode, uint8_t type, uint16_t offset, uint8_t * data, uint16_t size)
{
    uint32_t *ptr, next;
...
int NVM_Compare(uint16_t offset, const uint8_t * data, uint16_t size)
{
    ...
    if ((size > remain_size) || (memcmp(data, ((uint8_t *) ptr) + 4 + offset, size) != 0))
    {
        return (int) size;
    }
