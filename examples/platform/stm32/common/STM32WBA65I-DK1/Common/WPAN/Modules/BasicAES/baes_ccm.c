...
#define SET_U16_BE(b, i, v)                                                                                                        \
    M_BEGIN uint16_t v_ = (uint16_t) (v);                                                                                          \
    uint8_t * b_        = &((uint8_t *) (b))[i];                                                                                   \
    b_[1]               = BYTE0(v_);                                                                                               \
    b_[0]               = BYTE1(v_);                                                                                               \
    M_END

int BAES_CcmCrypt(uint8_t mode, const uint8_t * key, uint8_t iv_length, const uint8_t * iv, uint16_t add_length,
                  const uint8_t * add, uint16_t input_length, const uint8_t * input, uint8_t tag_length, uint8_t * tag,
                  uint8_t * output)
{
#if CFG_BAES_SW == 0
