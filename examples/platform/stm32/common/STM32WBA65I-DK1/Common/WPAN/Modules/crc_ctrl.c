...
__WEAK CRCCTRL_Cmd_Status_t CRCCTRL_Calculate(CRCCTRL_Handle_t * const p_Handle, uint32_t a_Payload[], const uint32_t PayloadSize,
                                              uint32_t * const p_ConmputedValue)
{
    ...
    else if ((MaxRegisteredId < p_Handle->Uid) || (CRCCTRL_NO_CONFIG >= p_Handle->Uid))
    {
...
__WEAK CRCCTRL_Cmd_Status_t CRCCTRL_Accumulate(CRCCTRL_Handle_t * const p_Handle, uint32_t a_Payload[], const uint32_t PayloadSize,
                                               uint32_t * const p_ConmputedValue)
{
    CRCCTRL_Cmd_Status_t error = CRCCTRL_UNKNOWN;
    ...
    else
    {
        ...
        if (CRCCTRL_OK == error)
        {
            ...
            if (CurrentConfig == p_Handle->Uid)
            {
                *p_ConmputedValue = HAL_CRC_Accumulate(&CRCHandle, a_Payload, PayloadSize);
...
HAL_StatusTypeDef CrcConfigure(CRCCTRL_Handle_t * const p_Handle)
{
    ...
    if (HAL_OK == error)
    {
        ...
        else
        {
            ...
            memset((void *) (&CRCHandle.Init), 0x00, sizeof(CRC_InitTypeDef));
