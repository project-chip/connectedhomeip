...
#if CFG_NVM_ALIGN != 0
extern void NVM_Init(uint64_t * buffer, uint16_t size, uint16_t max_size);
#else
extern void NVM_Init(uint32_t * buffer);
#endif

extern int NVM_Add(uint8_t type, const uint8_t * data, uint16_t size, const uint8_t * extra_data, uint16_t extra_size);

extern int NVM_Get(uint8_t mode, uint8_t type, uint16_t offset, uint8_t * data, uint16_t size);
