...
#define BAES_OR_BYTE_BE(w, n, b) (w)[(n) / 4] |= ((uint32_t) (b)) << (8 * (3 - ((n) % 4)))
...
extern void BAES_RawEncrypt(const uint32_t * p_in, uint32_t * p_out, const uint32_t * p_exp_key);

extern void BAES_RawDecrypt(const uint32_t * p_in, uint32_t * p_out, const uint32_t * p_exp_key);
