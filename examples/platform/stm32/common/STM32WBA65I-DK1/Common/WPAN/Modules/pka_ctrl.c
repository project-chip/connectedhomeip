...
static const uint32_t PKA_P256_n[8] = { 0XFC632551, /* LSB */
...
static void PKA_P256_StartPointCheck(const uint32_t * x, const uint32_t * y);
static uint32_t PKA_P256_IsPointCheckOk(void);
static void PKA_P256_StartEccScalarMul(const uint32_t * k, const uint32_t * p_x, const uint32_t * p_y);
static void PKA_P256_ReadEccScalarMul(uint32_t * p_x, uint32_t * p_y);
...
static int PKACTRL_Process(void)
{
    ...
    case PKACTRL_RANGE_Y_CHECK:
        ...
        PKA_P256_StartPointCheck(PKACTRL_buffer + 8, PKACTRL_buffer + 16);
    ...
    case PKACTRL_POINT_CHECK:
        ...
        PKA_P256_StartEccScalarMul(PKACTRL_buffer, PKACTRL_buffer + 8, PKACTRL_buffer + 16);
...
static void PKA_P256_StartPointCheck(const uint32_t * x, const uint32_t * y)
{
    ...
    PKA_WriteOperand(PKA_POINT_CHECK_IN_MONTGOMERY_PARAM, 8, PKA_P256_r2);
...
static void PKA_P256_StartEccScalarMul(const uint32_t * k, const uint32_t * p_x, const uint32_t * p_y)
{
    ...
    PKA_WriteOperand(PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_X, 8, p_x ? p_x : PKA_P256_p_x);

    /* Set the point P coordinate y */
    PKA_WriteOperand(PKA_ECC_SCALAR_MUL_IN_INITIAL_POINT_Y, 8, p_y ? p_y : PKA_P256_p_y);

    /* Set the prime order n */
    PKA_WriteOperand(PKA_ECC_SCALAR_MUL_IN_N_PRIME_ORDER, 8, PKA_P256_n);
