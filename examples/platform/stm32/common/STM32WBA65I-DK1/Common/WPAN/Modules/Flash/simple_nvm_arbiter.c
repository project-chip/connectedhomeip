...
#define SNVMA_ALIGN_128(p_Addr) ((p_Addr + SNVMA_MASK_ALIGNMENT_128) & ~SNVMA_MASK_ALIGNMENT_128)
 ...
 */
static inline SNVMA_BankElt_t * GetNewestBank(SNVMA_BankElt_t * p_FirstBank, SNVMA_BankElt_t * p_SecondBank);
...
SNVMA_Cmd_Status_t SNVMA_Init(const uint32_t * p_NvmStartAddress)
{
    ...
    /* Check that configuration will not overlap flash */
             ...
             ((uint32_t) p_NvmStartAddress + (SNVMA_NUMBER_OF_SECTOR_NEEDED * FLASH_PAGE_SIZE) > (FLASH_BASE_NS + FLASH_SIZE)))
    {
    ...
    else
    {
        ...
        if ((CRCCTRL_OK != crcCtrlStatus) && (CRCCTRL_HANDLE_ALREADY_REGISTERED != crcCtrlStatus))
        {
        ...
        else
        {
            ...
            memset((void *) SNVMA_BankConfiguration, 0x00, (sizeof(SNVMA_BankElt_t) * SNVMA_NUMBER_OF_BANKS));

            /* For each NVM */
            for (uint8_t nvmIdx = 0x00; (nvmIdx < SNVMA_NVM_NUMBER) && (error == SNVMA_ERROR_NOK); nvmIdx++)
            {
                ...
                if ((SNVMA_NvmConfiguration[nvmIdx].BankNumber < SNVMA_MIN_NUMBER_BANK) || (bankNbr > SNVMA_NUMBER_OF_BANKS))
                {
                    error = SNVMA_ERROR_BANK_NUMBER;
                }
                /* Is bank size ok ? */
                else if ((SNVMA_NvmConfiguration[nvmIdx].BankSize == 0x00) || (bankPrint > SNVMA_NUMBER_OF_SECTOR_NEEDED))
                {
                ...
                else
                {
                    ...
                    /* For each bank ... */
                    for (uint8_t bankIdx = 0x00; bankIdx < SNVMA_NvmConfiguration[nvmIdx].BankNumber; bankIdx++)
                    {
                        /* ... compute bank addresses */
                        SNVMA_BankConfiguration[bankConfIdx].p_StartAddr =
                            (uint32_t *) ((uint32_t) p_NvmStartAddress + addressOffset);

                        /* Shall this bank be the one in use */
                        if (IsHeaderOk(SNVMA_BankConfiguration[bankConfIdx].p_StartAddr, nvmIdx) == FALSE)
                        {
                            /* Erase the bank */
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                SNVMA_NvmConfiguration[nvmIdx].p_BankForRestore =
                                    GetNewestBank(p_currentRestoreBank, &SNVMA_BankConfiguration[bankConfIdx]);
                                ...
                                else
                                {
                                    ...
                                    while (EraseSector(
                                               (((uint32_t) p_currentRestoreBank->p_StartAddr - FLASH_BASE_NS) / FLASH_PAGE_SIZE),
                                               SNVMA_NvmConfiguration[nvmIdx].BankSize) == FALSE)
                                        ;
                                }
                            }
...
SNVMA_Cmd_Status_t SNVMA_Register(const SNVMA_BufferId_t BufferId, const uint32_t * p_BufferAddress, const uint32_t BufferSize)
{
    ...
    else
    {
        ...
        for (uint8_t cnt = 0x00; (cnt < SNVMA_MAX_NUMBER_BUFFER) && (error == SNVMA_ERROR_NOK); cnt++)
        {
            /* Compute occupied space */
            if (SNVMA_NvmConfiguration[nvmId].a_Buffers[cnt].p_Addr != NULL)
            {
                neededSpace = neededSpace + SNVMA_ALIGN_128((SNVMA_NvmConfiguration[nvmId].a_Buffers[cnt].Size * sizeof(uint32_t)));
        ...
        if (error == SNVMA_ERROR_NOK)
        {
            ...
            for (uint8_t cnt = 0x00; cnt < SNVMA_MAX_NUMBER_BUFFER; cnt++)
            {
                if (SNVMA_NvmConfiguration[nvmId].a_Buffers[cnt].p_Addr != NULL)
                {
                    ...
                    /* Update offset value */
                    offSet = offSet + SNVMA_ALIGN_128((SNVMA_NvmConfiguration[nvmId].a_Buffers[cnt].Size * sizeof(uint32_t)));
                }
            }
...
SNVMA_Cmd_Status_t SNVMA_Restore(const SNVMA_BufferId_t BufferId)
{
    ...
    else
    {
        ...
        if (IsHeaderOk(SNVMA_NvmConfiguration[nvmId].p_BankForRestore->p_StartAddr, nvmId) == FALSE)
        {
        ...
        else
        {
            ...
            {
                paddingOffset = sizeof(SNVMA_BankHeader_t) + SNVMA_ALIGN_128((p_bankHeader->SizeId1 * sizeof(uint32_t)));
            }
            ...
            {
                p_bufferFlashAddr =
                    (uint32_t *) ((uint32_t) SNVMA_NvmConfiguration[nvmId].p_BankForRestore->p_StartAddr + paddingOffset);

                /* Restore the data in RAM */
                memcpy((void *) SNVMA_NvmConfiguration[nvmId].a_Buffers[idxBuf].p_Addr, (void *) p_bufferFlashAddr,
    ...
    else
    {
        ...
        if (SNVMA_CommandPending == FALSE)
        {
            ...
            if (StartFlashWrite(nvmId) == FM_ERROR)
            {
                ...
                memset((void *) &SNVMA_FlashInfo, 0x00, sizeof(SNVMA_FlashOpInfo_t));
            ...
            {
                LOG_INFO_SYSTEM("\r\nSNVMA_Write - Flash operation started (Header write request) : %d",
                                (uint8_t) SNVMA_NvmConfiguration[nvmId].PendingBufferWriteOp);
...
void SNVMA_FlashManagerCallback(FM_FlashOp_Status_t Status)
{
    ...
    case SNVMA_HEADER_WRITE: {
        ...
        if (Status == FM_OPERATION_COMPLETE)
        {
                              ...
                              (sizeof(SNVMA_BankHeader_t) / sizeof(uint32_t))) == FALSE)
            {
                ...
                flashFunRet = FM_Erase(
                    (((uint32_t) SNVMA_NvmConfiguration[SNVMA_FlashInfo.NvmId].p_BankForWrite->p_StartAddr - FLASH_BASE_NS) /
                     FLASH_PAGE_SIZE),
                    SNVMA_NvmConfiguration[SNVMA_FlashInfo.NvmId].BankSize, &SNVMA_FlashCallback);
            ...
            else
            {
                ...
                flashFunRet = FM_Write(
                    ...
                    SNVMA_NvmConfiguration[SNVMA_FlashInfo.NvmId].a_Buffers[SNVMA_FlashInfo.BufferId].Size, &SNVMA_FlashCallback);
        ...
        else
        {
            ...
            flashFunRet = FM_Write((uint32_t *) &SNVMA_WriteBankHeader,
                                   ...
                                   (sizeof(SNVMA_BankHeader_t) / sizeof(uint32_t)), &SNVMA_FlashCallback);
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (SNVMA_IdBitmask != 0x00000000)
                        {
                            ...
                            for (uint8_t cnt = 0x00; cnt < SNVMA_MAX_NUMBER_NVM; cnt++)
                            {
                                if ((SNVMA_IdBitmask & (1u << cnt)) != 0x00)
                                {
                                    ...
                                    for (uint8_t idx = 0x00; idx < SNVMA_MAX_NUMBER_BUFFER; idx++)
                                    {
                                        if ((SNVMA_NvmConfiguration[SNVMA_FlashInfo.NvmId].PendingBufferWriteOp & (1u << idx)) !=
                                            0x00)
                                        {
                            ...
                            LOG_INFO_SYSTEM("\r\nSNVMA_FlashManagerCallback - Flash operation state : SNVMA_BUFFER_WRITE - Start "
    ...
    case SNVMA_ERASE_BANK: {
        ...
        if (Status == FM_OPERATION_COMPLETE)
        {
            ...
            if (SNVMA_IdBitmask != 0x00000000)
            {
                ...
                LOG_INFO_SYSTEM("\r\nSNVMA_FlashManagerCallback - Flash operation state : SNVMA_ERASE_BANK - Start the pending "
                                "write operation");
    ...
    case SNVMA_RETRY_WRITE: {
        ...
        else
        {
            ...
            flashFunRet =
                FM_Erase((((uint32_t) SNVMA_NvmConfiguration[SNVMA_FlashInfo.NvmId].p_BankForWrite->p_StartAddr - FLASH_PAGE_SIZE) /
                          FLASH_PAGE_SIZE),
                         SNVMA_NvmConfiguration[SNVMA_FlashInfo.NvmId].BankSize, &SNVMA_FlashCallback);
...
uint8_t IsHeaderOk(const uint32_t * const p_BankStartAddress, const uint8_t NvmId)
{
    ...
    else if ((((SNVMA_BankHeader_t *) p_BankStartAddress)->SizeId1 + ((SNVMA_BankHeader_t *) p_BankStartAddress)->SizeId2 +
              ((SNVMA_BankHeader_t *) p_BankStartAddress)->SizeId3 + ((SNVMA_BankHeader_t *) p_BankStartAddress)->SizeId4) >
...
uint8_t IsCrcOk(const uint32_t * const p_BankStartAddress)
{
    ...
    while (cnt < SNVMA_MAX_NUMBER_BUFFER)
    {
        ...
        case 0: {
            ...
            {
                eReturn = CRCCTRL_Calculate(&SNVMA_Handle, payloadAddr, ((SNVMA_BankHeader_t *) p_BankStartAddress)->SizeId1,
                                            &crcComputedValue);
        ...
        case 1: {
            ...
            {
                eReturn = CRCCTRL_Accumulate(&SNVMA_Handle, (uint32_t *) ((uint32_t) payloadAddr + offSet),
                                             ((SNVMA_BankHeader_t *) p_BankStartAddress)->SizeId2, &crcComputedValue);
        ...
        case 2: {
            ...
            {
                eReturn = CRCCTRL_Accumulate(&SNVMA_Handle, (uint32_t *) ((uint32_t) payloadAddr + offSet),
                                             ((SNVMA_BankHeader_t *) p_BankStartAddress)->SizeId3, &crcComputedValue);
        ...
        case 3: {
            ...
            {
                eReturn = CRCCTRL_Accumulate(&SNVMA_Handle, (uint32_t *) ((uint32_t) payloadAddr + offSet),
                                             ((SNVMA_BankHeader_t *) p_BankStartAddress)->SizeId4, &crcComputedValue);
...
uint8_t IsSameContent(uint32_t * p_Source, uint32_t * p_Destination, uint32_t Size)
{
    ...
    for (uint32_t cnt = 0x00; (cnt < Size) && (error == TRUE); cnt++)
    {
...
SNVMA_BankElt_t * GetNewestBank(SNVMA_BankElt_t * p_FirstBank, SNVMA_BankElt_t * p_SecondBank)
{
    ...
    if (p_FirstBankHeader->Counter < p_SecondBankHeader->Counter)
    {
        if ((p_SecondBankHeader->Counter - p_FirstBankHeader->Counter) != 0xFF)
        {
            error = p_SecondBank;
        }
    }
...
uint8_t EraseSector(const uint32_t SectorId, const uint32_t SectorNumber)
{
    ...
    if (0u != SectorNumber)
    {
        ...
        if ((SectorNumber <= sectorIdx) && (FD_FLASHOP_SUCCESS == funcError))
        {
            error = TRUE;
        }
    }
...
FM_Cmd_Status_t StartFlashWrite(const uint8_t NvmId)
{
    ...
    memset((void *) &SNVMA_WriteBankHeader, 0x00, sizeof(SNVMA_BankHeader_t));
    ...
    while (cnt < SNVMA_MAX_NUMBER_BUFFER)
    {
        if (SNVMA_NvmConfiguration[NvmId].a_Buffers[cnt].p_Addr != NULL)
        {
            ...
            if (crcValue == 0x00)
            {
                eReturn = CRCCTRL_Calculate(&SNVMA_Handle, SNVMA_NvmConfiguration[NvmId].a_Buffers[cnt].p_Addr,
                                            SNVMA_NvmConfiguration[NvmId].a_Buffers[cnt].Size, &crcValue);
            ...
            {
                eReturn = CRCCTRL_Accumulate(&SNVMA_Handle, SNVMA_NvmConfiguration[NvmId].a_Buffers[cnt].p_Addr,
                                             SNVMA_NvmConfiguration[NvmId].a_Buffers[cnt].Size, &crcValue);
    ...
    {
        SNVMA_WriteBankHeader.Counter =
            (((SNVMA_BankHeader_t *) (SNVMA_NvmConfiguration[NvmId].p_BankForRestore->p_StartAddr))->Counter) + 1;
    }
    ...
    /* Write the header */
    error = FM_Write((uint32_t *) &SNVMA_WriteBankHeader, SNVMA_NvmConfiguration[NvmId].p_BankForWrite->p_StartAddr,
                     (sizeof(SNVMA_BankHeader_t) / sizeof(uint32_t)), &SNVMA_FlashCallback);
...
void InvokeBufferCallback(const uint8_t NvmId, const SNVMA_Callback_Status_t CallbackStatus)
{
    ...
    for (pendingShift = 0x00;
         ((SNVMA_NvmConfiguration[NvmId].PendingBufferWriteOp != 0x00) && (pendingShift < SNVMA_MAX_NUMBER_BUFFER)); pendingShift++)
    {
        tmpMask = SNVMA_NvmConfiguration[NvmId].PendingBufferWriteOp & (1u << (pendingShift + SNVMA_MAX_NUMBER_BUFFER));
