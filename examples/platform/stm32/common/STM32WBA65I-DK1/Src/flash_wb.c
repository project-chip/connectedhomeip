...
{
    /* Update status */
    FM_Write((uint32_t *) ram_nvm, NVM_MATTER_ADDR_INIT_PTR, NVM_SIZE_32_BITS, &FM_WriteStatusCallback);
}
...
NVM_StatusTypeDef NM_Dump(void)
{
    ...
    LockFMThread();
    FM_Erase(START_SECTOR_FLASH_NVM, NB_SECTOR_FLASH_NVM, &FM_EraseBeforeWriteStatusCallback);
...
{
    LockFMThread();
    FM_Erase(START_SECTOR_FLASH_NVM, NB_SECTOR_FLASH_NVM, &FM_EraseStatusCallback);
}
