...
static const ARM_DRIVER_VERSION DriverVersion = { ARM_FLASH_API_VERSION, /* Defined in the CMSIS Flash Driver header file */
...
/* Driver Capabilities */
static const ARM_FLASH_CAPABILITIES DriverCapabilities = { EVENT_READY_NOT_AVAILABLE, DATA_WIDTH_16BIT, /* 16bits for DTR mode */
...
static bool is_range_valid(struct arm_spi_flash_dev_t * flash_dev, uint32_t offset)
{
 ...
 *               otherwise.
 */
static bool is_erase_allow(struct arm_spi_flash_dev_t * flash_dev, uint32_t param)
{
...
static bool is_write_allow(struct arm_spi_flash_dev_t * flash_dev, uint32_t start, uint32_t len)
{
...
static bool is_write_aligned(struct arm_spi_flash_dev_t * flash_dev, uint32_t param)
{
 ...
 *               otherwise.
 */
static bool is_erase_aligned(struct arm_spi_flash_dev_t * flash_dev, uint32_t param)
{
    return ((param % (flash_dev->data->sector_size)) != 0) ? (false) : (true);
}

static ARM_FLASH_INFO ARM_SPI_FLASH0_DEV_DATA = { .sector_info  = NULL, /* Uniform sector layout */
...
static struct arm_spi_flash_dev_t ARM_SPI_FLASH0_DEV = { .dev = &(SPI_FLASH0_DEV), .data = &(ARM_SPI_FLASH0_DEV_DATA) };
...
static int32_t Spi_Flash_ProgramData(uint32_t addr, const void * data, uint32_t cnt)
{
    int32_t err = BSP_ERROR_NONE;
     ...
     */
    if ((!is_range_valid(&ARM_SPI_FLASH0_DEV, addr + cnt - 1)) || (!is_write_aligned(&ARM_SPI_FLASH0_DEV, addr)) ||
        (!is_write_aligned(&ARM_SPI_FLASH0_DEV, cnt)) || (!is_write_allow(&ARM_SPI_FLASH0_DEV, addr, cnt)))
    {
#ifdef DEBUG_SPI_FLASH_ACCESS
    ...
    if (err != BSP_ERROR_NONE)
    {
        printf("->>> Failed to write 0x%" PRIx32 " bytes at 0x%" PRIx32 " (Error 0x%" PRIx32 ")", cnt,
               (uint32_t) (SPI_FLASH_BASE_ADDRESS + addr), err);
#ifdef DEBUG_SPI_FLASH_ACCESS
...
#endif /* DEBUG_SPI_FLASH_ACCESS */
    if (!(is_range_valid(&ARM_SPI_FLASH0_DEV, addr)) || !(is_erase_aligned(&ARM_SPI_FLASH0_DEV, addr)) ||
...
ARM_DRIVER_FLASH Driver_SPI_FLASH0 = { Spi_Flash_GetVersion,   Spi_Flash_GetCapabilities, Spi_Flash_Initialize,
                                       Spi_Flash_Uninitialize, Spi_Flash_PowerControl,    Spi_Flash_ReadData,
                                       Spi_Flash_ProgramData,  Spi_Flash_EraseSector,     Spi_Flash_EraseChip,
                                       Spi_Flash_GetStatus,    Spi_Flash_GetInfo };
