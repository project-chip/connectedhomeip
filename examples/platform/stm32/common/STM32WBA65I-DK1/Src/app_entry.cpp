...
static void APPE_SetBootReason(void) {
	...
	/* The kSoftwareReset is triggered by NVIC_SystemReset() function */
		...
		if(error_nvm == 0){
			if(memcmp(SoftwareVersion,X_CUBE_MATTER_VERSION,sizeof(SoftwareVersion)) == 0){
				...
				NVM_SetKeyValue((char *)X_CUBE_MATTER_VERSION, (char *)SOFTWARE_VERSION_KEY, sizeof(X_CUBE_MATTER_VERSION),SECTOR_MATTER);
				m_Boot_Reason = 0x05;
			}
		}else{
			m_Boot_Reason = 0x06;
	...
	else if (__HAL_RCC_GET_FLAG(
			RCC_FLAG_PINRST) || __HAL_RCC_GET_FLAG(RCC_FLAG_OBLRST)) {
		m_Boot_Reason = 0x01;
	}
...
#endif
	LOG_INFO_APP(
			"SOFTWARE : X-CUBE-MATTER release revision " X_CUBE_MATTER_VERSION "\n");
...
static void runDACKeyProvisioning(void)
{
  ...
  else 
  {
	 ...
	 retval = psa_import_key(&key_attributes, chip::DevelopmentCerts::kDacPrivateKey.data(), chip::DevelopmentCerts::kDacPrivateKey.size(), &key_handle_private);
     if (retval != PSA_SUCCESS)
     {
