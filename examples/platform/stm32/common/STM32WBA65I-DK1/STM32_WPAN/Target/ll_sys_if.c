...
{
    /* Tasks creation */
    const osSemaphoreAttr_t LinkLayerSemaphore_attributes = { .name = "LinkLayerSemaphore" };
    LinkLayerSemaphore                                    = osSemaphoreNew(1U, 0U, &LinkLayerSemaphore_attributes);
    if (LinkLayerSemaphore == NULL)
    {
        Error_Handler();
    }
    const osMutexAttr_t LinkLayerMutex_attributes = { .name = "LinkLayer Mutex" };
    LinkLayerMutex                                = osMutexNew(&LinkLayerMutex_attributes);
    if (LinkLayerMutex == NULL)
    {
        Error_Handler();
    }
    const osThreadAttr_t LinkLayerTask_attributes = { .name       = "Link Layer Task",
...
static void LinkLayerTempMeasure_Task_Entry(void * argument)
{
    ...
    for (;;)
    {
        ...
        {
            APP_DBG("\x1b[34m"
                    "LinkLayerTempMeasureTaskEntry_stack_HighWaterMark %lu \n"
                    "\x1b[0m",
                    uxHighWaterMark);
            lastHighWaterMark = uxHighWaterMark;
        }
...
{
    /* Tasks creation */
    const osSemaphoreAttr_t LinkLayerMeasSemaphore_attributes = { .name = "LinkLayerMeasSemaphore" };
    ...
    const osThreadAttr_t LinkLayerMeasTask_attributes = { .name       = "LinkLayer Temp Measurement Task",
