...
static const uint8_t a_MBdAddr[BD_ADDR_SIZE] = {
    (uint8_t) ((CFG_BD_ADDRESS & 0x0000000000FF)),       (uint8_t) ((CFG_BD_ADDRESS & 0x00000000FF00) >> 8),
    (uint8_t) ((CFG_BD_ADDRESS & 0x000000FF0000) >> 16), (uint8_t) ((CFG_BD_ADDRESS & 0x0000FF000000) >> 24),
    (uint8_t) ((CFG_BD_ADDRESS & 0x00FF00000000) >> 32), (uint8_t) ((CFG_BD_ADDRESS & 0xFF0000000000) >> 40)
};
...
const osThreadAttr_t stHCIasyncEvtTaskAttributes = { .name       = "HCI asynch event Task",
...
void APP_BLE_Init(void)
{
    ...
    const osSemaphoreAttr_t BleStackSemaphore_attributes = { .name = "BLE stack Semaphore" };
    ...
    const osSemaphoreAttr_t HCIasyncEvtSemaphore_attributes = { .name = "HCI asynch EVT Semaphore" };
    ...
    if (HOST_BLE_Init() == 0u)
    {
        ...
        const osSemaphoreAttr_t ProcGapCompleteSemaphore_attributes = { .name = "ProcGapCompleteSemaphore" };
        ...
        SwitchOffLedTimerId = osTimerNew(Switch_OFF_Led, osTimerOnce, NULL, NULL);
#endif
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void * p_Pckt)
{
    ...
    case HCI_LE_META_EVT_CODE: {
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE: {
            ...
            hci_le_set_data_length(bleAppContext.BleApplicationContext_legacy.connectionHandle, 251, 2120);
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE: {
        ...
        case ACI_GAP_NUMERIC_COMPARISON_VALUE_VSEVT_CODE: {
            ...
            ret = aci_gap_numeric_comparison_value_confirm_yesno(bleAppContext.BleApplicationContext_legacy.connectionHandle,
                                                                 confirm_value);
            if (ret != BLE_STATUS_SUCCESS)
            {
...
void APP_BLE_Procedure_Gap_General(ProcGapGeneralId_t ProcGapGeneralId)
{
    ...
    case PROC_GAP_GEN_PHY_TOGGLE: {
        ...
        else
        {
            ...
            if ((phy_tx == HCI_TX_PHY_LE_2M) && (phy_rx == HCI_RX_PHY_LE_2M))
            {
                ...
                status = hci_le_set_phy(bleAppContext.BleApplicationContext_legacy.connectionHandle, 0, HCI_TX_PHYS_LE_1M_PREF,
            ...
            else
            {
                ...
                status = hci_le_set_phy(bleAppContext.BleApplicationContext_legacy.connectionHandle, 0, HCI_TX_PHYS_LE_2M_PREF,
    ...
    case PROC_GAP_GEN_CONN_TERMINATE: {
        status = aci_gap_terminate(bleAppContext.BleApplicationContext_legacy.connectionHandle,
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
    ...
    case PROC_GAP_PERIPH_ADVERTISE_START_LP: {
        ...
        status = aci_gap_set_discoverable(ADV_TYPE, paramA, paramB, CFG_BD_ADDRESS_TYPE, ADV_FILTER, 0, 0, 0, 0, 0, 0);
        if (status != BLE_STATUS_SUCCESS)
        {
    ...
    case PROC_GAP_PERIPH_CONN_PARAM_UPDATE: {
        status = aci_l2cap_connection_parameter_update_req(bleAppContext.BleApplicationContext_legacy.connectionHandle, paramA,
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
    ...
    ret = aci_gap_set_authentication_requirement(
        ...
        bleAppContext.BleApplicationContext_legacy.bleSecurityParam.mitm_mode, CFG_SC_SUPPORT, CFG_KEYPRESS_NOTIFICATION_SUPPORT,
        ...
        bleAppContext.BleApplicationContext_legacy.bleSecurityParam.Fixed_Pin, CFG_BD_ADDRESS_TYPE);
    if (ret != BLE_STATUS_SUCCESS)
    {
...
tBleStatus BLECB_Indication(const uint8_t * data, uint16_t length, const uint8_t * ext_data, uint16_t ext_length)
{
    ...
    if (data[0] == HCI_EVENT_PKT_TYPE)
    {
        ...
        if (AMM_Alloc(CFG_AMM_VIRTUAL_APP_BLE, DIVC((sizeof(BleEvtPacketHeader_t) + total_length), sizeof(uint32_t)),
                      (uint32_t **) &phcievt, &APP_BLE_ResumeFlowProcessCb) != AMM_ERROR_OK)
        {
...
static void BleUserEvtRx_Task_Entry(void * argument)
{
    ...
    while (1)
    {
        ...
        {
            APP_DBG("\x1b[34m"
                    "BleUserEvtRxTaskEntry_stack_HighWaterMark %lu \n"
                    "\x1b[0m",
                    uxHighWaterMark);
...
static void BleStack_Task_Entry(void * argument)
{
    ...
    while (1)
    {
        ...
        {
            APP_DBG("\x1b[34m"
                    "BleStackTaskEntry_stack_HighWaterMark %lu \n"
                    "\x1b[0m",
                    uxHighWaterMark);
            lastHighWaterMark = uxHighWaterMark;
        }
