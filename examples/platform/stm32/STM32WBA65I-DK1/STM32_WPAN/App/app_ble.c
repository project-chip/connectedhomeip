...
#define BLE_GATT_BUF_SIZE                                                                                                          \
    BLE_TOTAL_BUFFER_SIZE_GATT(CFG_BLE_NUM_GATT_ATTRIBUTES, CFG_BLE_NUM_GATT_SERVICES, CFG_BLE_ATT_VALUE_ARRAY_SIZE)

#define BLE_HOST_EVENT_BUF_SIZE CFG_BLE_HOST_EVENT_BUF_SIZE

#define MBLOCK_COUNT (BLE_MBLOCKS_CALC(PREP_WRITE_LIST_SIZE, CFG_BLE_ATT_MTU_MAX, CFG_BLE_NUM_LINK) + CFG_BLE_MBLOCK_COUNT_MARGIN)
...
static const uint8_t a_GapDeviceName[] = { 'S', 'T', '_', 'W', 'B', 'A', '_', 'M', 'A', 'T', 'T', 'E', 'R' }; /* Gap Device Name */
...
uint8_t a_AdvData[15] = { 0x02, AD_TYPE_FLAGS, 0x06, 0x0B, AD_TYPE_SERVICE_DATA, 0xF6, 0xFF, 0x00, 0x00, 0x00, 0x00,
                          0x00, 0x00,          0x00, 0x00 };
...
const osSemaphoreAttr_t BleHostSemaphore_attributes = { .name      = "BLE Host Semaphore",
                                                        .attr_bits = TASK_DEFAULT_ATTR_BITS,
                                                        .cb_mem    = TASK_DEFAULT_CB_MEM,
                                                        .cb_size   = TASK_DEFAULT_CB_SIZE };

const osSemaphoreAttr_t GapProcCompleteSemaphore_attributes = { .name      = "GAP procedure complete Semaphore",
                                                                .attr_bits = TASK_DEFAULT_ATTR_BITS,
                                                                .cb_mem    = TASK_DEFAULT_CB_MEM,
                                                                .cb_size   = TASK_DEFAULT_CB_SIZE };
...
void APP_BLE_Init(void)
{
    ...
    /* Check allocation */
    if (((host_buffer == NULL) && (BLE_DYN_ALLOC_SIZE != 0)) || ((gatt_buffer == NULL) && (BLE_GATT_BUF_SIZE != 0)) ||
    ...
    SNVMA_Register(APP_BLE_NvmBuffer, (uint32_t *) host_nvm_buffer, (CFG_BLE_NVM_SIZE_MAX * 2));
...
SVCCTL_UserEvtFlowStatus_t SVCCTL_App_Notification(void * p_Pckt)
{
    ...
    case HCI_DISCONNECTION_COMPLETE_EVT_CODE: {
        ...
        if (p_disconnection_complete_event->Connection_Handle == bleAppContext.connectionHandle)
        {
            ...
            LOG_INFO_APP("     - Connection Handle:   0x%04X\n     - Reason:    0x%02X\n",
                         p_disconnection_complete_event->Connection_Handle, p_disconnection_complete_event->Reason);
        ...
        handleNotification.ConnectionHandle = p_disconnection_complete_event->Connection_Handle;
    ...
    case HCI_LE_META_EVT_CODE: {
        ...
        case HCI_LE_CONNECTION_UPDATE_COMPLETE_SUBEVT_CODE: {
            ...
            LOG_INFO_APP("     - Connection Interval:   %d.%02d ms\n     - Connection latency:    %d\n     - Supervision Timeout:  "
                         " %d ms\n     - Status:                0x%02X\n",
                         conn_interval_us / 1000, (conn_interval_us % 1000) / 10, p_conn_update_complete->Conn_Latency,
                         p_conn_update_complete->Supervision_Timeout * 10, p_conn_update_complete->Status);
        ...
        case HCI_LE_ENHANCED_CONNECTION_COMPLETE_SUBEVT_CODE: {
            ...
            LOG_INFO_APP("     - Connection established with @:%02x:%02x:%02x:%02x:%02x:%02x\n",
                         p_enhanced_conn_complete->Peer_Address[5], p_enhanced_conn_complete->Peer_Address[4],
                         p_enhanced_conn_complete->Peer_Address[3], p_enhanced_conn_complete->Peer_Address[2],
                         p_enhanced_conn_complete->Peer_Address[1], p_enhanced_conn_complete->Peer_Address[0]);
            LOG_INFO_APP("     - Connection Interval:   %d.%02d ms\n     - Connection latency:    %d\n     - Supervision Timeout:  "
                         " %d ms\n     - Status:               0x%02X\n",
                         conn_interval_us / 1000, (conn_interval_us % 1000) / 10, p_enhanced_conn_complete->Conn_Latency,
                         p_enhanced_conn_complete->Supervision_Timeout * 10, p_enhanced_conn_complete->Status);
            UNUSED(conn_interval_us);
        ...
        case HCI_LE_CONNECTION_COMPLETE_SUBEVT_CODE: {
            ...
            LOG_INFO_APP("     - Connection established with @:%02x:%02x:%02x:%02x:%02x:%02x\n", p_conn_complete->Peer_Address[5],
                         p_conn_complete->Peer_Address[4], p_conn_complete->Peer_Address[3], p_conn_complete->Peer_Address[2],
                         p_conn_complete->Peer_Address[1], p_conn_complete->Peer_Address[0]);
            LOG_INFO_APP("     - Connection Interval:   %d.%02d ms\n     - Connection latency:    %d\n     - Supervision Timeout:  "
                         " %d ms\n     - Status:               0x%02X\n",
                         conn_interval_us / 1000, (conn_interval_us % 1000) / 10, p_conn_complete->Conn_Latency,
                         p_conn_complete->Supervision_Timeout * 10, p_conn_complete->Status);
            UNUSED(conn_interval_us);
            ...
            hci_le_set_data_length(bleAppContext.connectionHandle, 251, 2120);
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE: {
        ...
        case ACI_GAP_PAIRING_COMPLETE_VSEVT_CODE: {
            ...
            {
                LOG_INFO_APP("     - Pairing KO\n     - Status: 0x%02X\n     - Reason: 0x%02X\n", p_pairing_complete->Status,
                             p_pairing_complete->Reason);
            }
            else
            {
...
void APP_BLE_Procedure_Gap_Peripheral(ProcGapPeripheralId_t ProcGapPeripheralId)
{
    ...
    case PROC_GAP_PERIPH_ADVERTISE_NON_CONN_START_LP: {
        /* Start Advertising */
        status = aci_gap_set_discoverable(paramD, paramA, paramB, CFG_BD_ADDRESS_TYPE, ADV_FILTER, 0, 0, 0, 0, 0, 0);
    ...
    case PROC_GAP_PERIPH_CONN_PARAM_UPDATE: {
        status = aci_l2cap_connection_parameter_update_req(bleAppContext.connectionHandle, paramA, paramB, paramC, paramD);
        if (status != BLE_STATUS_SUCCESS)
        {
...
static void Ble_Hci_Gap_Gatt_Init(void)
{
    ...
    ret = aci_hal_write_config_data(CONFIG_DATA_GAP_ADD_REC_NBR_OFFSET, CONFIG_DATA_GAP_ADD_REC_NBR_LEN, &additional_svc_record);
    if (ret != BLE_STATUS_SUCCESS)
    {
    ...
    else
    {
        ...
        LOG_INFO_APP("  Random Bluetooth Address: %02x:%02x:%02x:%02x:%02x:%02x\n", (uint8_t) (p_bd_addr[1] >> 8),
                     (uint8_t) (p_bd_addr[1]), (uint8_t) (p_bd_addr[0] >> 24), (uint8_t) (p_bd_addr[0] >> 16),
                     (uint8_t) (p_bd_addr[0] >> 8), (uint8_t) (p_bd_addr[0]));
    }
    ...
    if (role > 0)
    {
        ret = aci_gap_init(role, CFG_PRIVACY, sizeof(a_GapDeviceName), &gap_service_handle, &gap_dev_name_char_handle,
        ...
        {
            ret = aci_gatt_update_char_value(gap_service_handle, gap_dev_name_char_handle, 0, sizeof(a_GapDeviceName),
            ...
            ret = aci_gatt_update_char_value(gap_service_handle, gap_appearance_char_handle, 0, sizeof(a_appearance),
    ...
    ret = aci_gap_set_authentication_requirement(
        bleAppContext.bleSecurityParam.bonding_mode, bleAppContext.bleSecurityParam.mitm_mode, CFG_SC_SUPPORT,
        CFG_KEYPRESS_NOTIFICATION_SUPPORT, bleAppContext.bleSecurityParam.encryptionKeySizeMin,
        bleAppContext.bleSecurityParam.encryptionKeySizeMax, USE_FIXED_PIN_FOR_PAIRING_FORBIDDEN, /* deprecated feature */
...
static const uint8_t * BleGenerateIRValue(void)
{
    ...
    uint8_t a_BLE_CfgIrValueDefault[16] = { 0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0,
...
static const uint8_t * BleGenerateERValue(void)
{
    ...
    uint8_t a_BLE_CfgErValueDefault[16] = { 0xFE, 0xDC, 0xBA, 0x09, 0x87, 0x65, 0x43, 0x21,
...
tBleStatus BLECB_Indication(const uint8_t * data, uint16_t length, const uint8_t * ext_data, uint16_t ext_length)
{
    ...
    if (data[0] == HCI_EVENT_PKT_TYPE)
    {
        ...
        if (AMM_Alloc(CFG_AMM_VIRTUAL_BLE_EVENTS, DIVC((sizeof(BleEvtPacketHeader_t) + total_length), sizeof(uint32_t)),
                      (uint32_t **) &phcievt, &BLE_EVENTS_ResumeFlowProcessCb) != AMM_ERROR_OK)
        {
