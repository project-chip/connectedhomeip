 ...
 */
static NVM_StatusTypeDef Cache_Read(uint8_t * pKeyValue, const uint8_t * pKeyAddr, size_t keySize, size_t * pReadSize);
 ...
 * @retval NVM_OK on success or an error code.
 */
static NVM_StatusTypeDef Cache_Update(const NVM_Sector_Struct selectSector, const uint8_t * pKeyName, const uint8_t * pKeyValue,
                                      size_t keySize);
 ...
 * @retval NVM_OK on success or an error code.
 */
static NVM_StatusTypeDef Cache_Replace(const NVM_Sector_Struct selectSector, uint8_t * pKeyFound, const uint8_t * pKeyName,
                                       const uint8_t * pKeyValue, size_t keySize);
 ...
 * @retval NVM_OK on success or an error code.
 */
static NVM_StatusTypeDef Cache_Write(uint8_t * pKeyFree, const uint8_t * pKey, const uint8_t * pValue, size_t valueSize);
 ...
 */
static uint8_t * Cache_Search_Key(uint8_t * pPage, const uint8_t * pKeyName, size_t nvmSize);
 ...
 */
static NVM_StatusTypeDef Cache_Delete_Key(const NVM_Sector_Struct selectSector, uint8_t * pKeyFound);
...
static const NVM_Sector_Struct sector_ot = { .id_sector   = SECTOR_OT,
                                             .ram_ptr     = ram_nvm + SECTOR_MATTER_SIZE,
                                             .sector_size = SECTOR_OT_SIZE };

static const NVM_Sector_Struct sector_matter = { .id_sector   = SECTOR_MATTER,
                                                 .ram_ptr     = ram_nvm,
...
static void FM_EraseBeforeWriteCallback(FM_FlashOp_Status_t Status)
{
    ...
    FM_Write((uint32_t *) ram_nvm, NVM_MATTER_ADDR_INIT_PTR, NVM_32_BITS_SIZE, &FM_WriteStatusCallback);
}
...
NVM_StatusTypeDef NVM_GetKeyExists(const char * pKeyName, NVM_Sector sector)
{
    ...
    pKeySearch = Cache_Search_Key(selectNvm.ram_ptr, (const uint8_t *) pKeyName, selectNvm.sector_size);
...
NVM_StatusTypeDef NVM_GetKeyValue(void * pKeyValue, const char * pKeyName, uint32_t keySize, size_t * pReadSize, NVM_Sector sector)
{
    ...
    if (pKeySearch != NULL)
    {
        ...
        return Cache_Read((uint8_t *) pKeyValue, pKeySearch, (size_t) keySize, pReadSize);
    }
...
NVM_StatusTypeDef NVM_SetKeyValue(const char * pKeyValue, const char * pKeyName, uint32_t keySize, NVM_Sector sector)
{
    ...
    /* Search existing key */
    pKeyFound = Cache_Search_Key(selectNvm.ram_ptr, (const uint8_t *) pKeyName, selectNvm.sector_size);

    if (pKeyFound == NULL)
    {
        /* New key */
        return Cache_Update(selectNvm, (const uint8_t *) pKeyName, (const uint8_t *) pKeyValue, (size_t) keySize);
    }
    else
    {
        /* Replace existing key */
        if (Cache_Replace(selectNvm, pKeyFound, (const uint8_t *) pKeyName, (const uint8_t *) pKeyValue, (size_t) keySize) ==
            NVM_OK)
        {
            return NVM_OK;
        }
...
static uint8_t * Cache_Search_Key(uint8_t * pPage, const uint8_t * pKeyName, size_t nvmSize)
{
    ...
    while (p < (pPage + nvmSize))
    {
        if (*p != DEFAULT_VALUE)
        {
            ...
            if ((readSize + sizeof(size_t) + MATTER_KEY_NAME_MAX_LENGTH) > (nvmSize - (size_t) (p - pPage)))
            {
                /* Corrupted structure */
                return NULL;
            }
...
static NVM_StatusTypeDef Cache_Read(uint8_t * pKeyValue, const uint8_t * pKeyAddr, size_t keySize, size_t * pReadSize)
{
    ...
    storedSize = *(const size_t *) (const void *) (pKeyAddr + MATTER_KEY_NAME_MAX_LENGTH);

    if (keySize >= storedSize)
    {
        memcpy(pKeyValue, pKeyAddr + MATTER_KEY_NAME_MAX_LENGTH + sizeof(size_t), storedSize);
...
static NVM_StatusTypeDef Cache_Write(uint8_t * pKeyFree, const uint8_t * pKey, const uint8_t * pValue, size_t valueSize)
{
    ...
    /* Store value size */
    memcpy(pKeyFree + MATTER_KEY_NAME_MAX_LENGTH, &valueSize, sizeof(size_t));

    /* Store value */
    memcpy(pKeyFree + MATTER_KEY_NAME_MAX_LENGTH + sizeof(size_t), pValue, valueSize);
...
static NVM_StatusTypeDef Cache_Delete_Key(const NVM_Sector_Struct selectSector, uint8_t * pKeyFound)
{
    ...
    {
        sizeKey = *(const size_t *) (void *) (pKeyNext + MATTER_KEY_NAME_MAX_LENGTH);

        memcpy(pKeyCopy, pKeyNext, sizeKey + sizeof(size_t) + MATTER_KEY_NAME_MAX_LENGTH);
    ...
    memset(pKeyCopy, DEFAULT_VALUE, (size_t) ((selectSector.ram_ptr + selectSector.sector_size) - pKeyCopy));
