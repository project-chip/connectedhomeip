                                       ...
                                       uint8_t * sl_host_pub_key)
{
    ...
    if (mbedtls_ecdh_compute_shared(&mbedtls_host_context.grp, &mbedtls_host_context.z, &mbedtls_host_context.Qp,
                                    &mbedtls_host_context.d, mbedtls_ctr_drbg_random, &host_drbg_context) != 0)
    {
        status = SL_STATUS_WIFI_SECURE_LINK_EXCHANGE_FAILED;
        goto error_handler;
    }
    ...
    status = mbedtls_md_hmac(mbedtls_md_info_from_type(MBEDTLS_MD_SHA256), sl_mac_key, SL_WFX_HOST_PUB_KEY_SIZE,
                             (uint8_t *) hmac_input, 92, shared_key_digest);
#endif
...
sl_status_t sl_wfx_host_decode_secure_link_data(uint8_t * buffer, uint32_t length, uint8_t * session_key)
{
    ...
    // Decrypt the data
    if (!mbedtls_ccm_auth_decrypt(&ccm_context, length, (uint8_t *) &nonce, SL_WFX_SECURE_LINK_NONCE_SIZE_BYTES, NULL, 0,
                                  (uint8_t *) buffer, (uint8_t *) buffer, (uint8_t *) buffer + length,
                                                ...
                                                uint8_t * nonce)
{
    ...
    {
        mbedtls_ccm_encrypt_and_tag(&ccm_context, data_length, nonce, SL_WFX_SECURE_LINK_NONCE_SIZE_BYTES, NULL, 0,
