...
CHIP_ERROR InitConfigurationManager(ConfigurationManagerImpl & configManager, LinuxDeviceOptions & options)
{
    ...
    {
        ReturnErrorOnFailure(configManager.StoreSoftwareVersionString(options.softwareVersionString.Value().c_str(), options.softwareVersionString.Value().length()));
    }

    if (options.hardwareVersionString.HasValue())
    {
        ReturnErrorOnFailure(configManager.StoreHardwareVersionString(options.hardwareVersionString.Value().c_str(), options.hardwareVersionString.Value().length()));
    }

    if (options.vendorName.HasValue())
    {
        ReturnErrorOnFailure(configManager.StoreVendorName(options.vendorName.Value().c_str(), options.vendorName.Value().length()));
    }

    if (options.productName.HasValue())
    {
        ReturnErrorOnFailure(configManager.StoreProductName(options.productName.Value().c_str(), options.productName.Value().length()));
    }

    if (options.serialNumber.HasValue())
    {
        ReturnErrorOnFailure(configManager.StoreSerialNumber(options.serialNumber.Value().c_str(), options.serialNumber.Value().length()));
    }
