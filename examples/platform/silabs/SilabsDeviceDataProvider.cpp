...
CHIP_ERROR SilabsDeviceDataProvider::generatePayload(uint8_t * outBuf)
{
    ...
    err = SilabsDeviceDataProvider::GetSetupDiscriminator(setupDiscriminator32);
    ReturnErrorOnFailure(err);
    err = SilabsDeviceDataProvider::GetVendorId(vendorId32);
    ReturnErrorOnFailure(err);
    err = SilabsDeviceDataProvider::GetProductId(productId32);
    ReturnErrorOnFailure(err);

    payload.version = 0;
    payload.discriminator.SetLongValue(setupDiscriminator32);
    ...
    payload.vendorID          = vendorId32;
    payload.productID         = productId32;
