...
constexpr chip::System::Clock::Timeout kBdxTimeout = chip::System::Clock::Seconds16(10 * 60); // OTA Spec mandates >= 5 minutes
constexpr uint32_t kBdxServerPollIntervalMillis    = 50;                                      // poll every 50ms by default

void RTGetUpdateTokenString(const chip::ByteSpan & token, char * buf, size_t bufSize)
{
...
void RTGenerateUpdateToken(uint8_t * buf, size_t bufSize)
{
...
chip::NXP::App::RTOTAProviderExample::RTOTAProviderExample()
{
...
void chip::NXP::App::RTOTAProviderExample::SetOTAFilePath(const char * path)
{
    if (path != nullptr)
    {
...
void chip::NXP::App::RTOTAProviderExample::SetImageUri(const char * imageUri)
{
    if (imageUri != nullptr)
    {
...
void chip::NXP::App::RTOTAProviderExample::SetOTACandidates(
    std::vector<RTOTAProviderExample::DeviceSoftwareVersionModel> candidates)
{
...
static bool CompareSoftwareVersions(const RTOTAProviderExample::DeviceSoftwareVersionModel & a,
                                    const RTOTAProviderExample::DeviceSoftwareVersionModel & b)
{
    return (a.softwareVersion < b.softwareVersion);
}

bool chip::NXP::App::RTOTAProviderExample::SelectOTACandidate(const uint16_t requestorVendorID, const uint16_t requestorProductID,
                                                              const uint32_t requestorSoftwareVersion,
                                                              RTOTAProviderExample::DeviceSoftwareVersionModel & finalCandidate)
{
...
UserConsentSubject chip::NXP::App::RTOTAProviderExample::GetUserConsentSubject(const app::CommandHandler * commandObj,
...
bool chip::NXP::App::RTOTAProviderExample::ParseOTAHeader(OTAImageHeaderParser & parser, const char * otaFilePath,
                                                          OTAImageHeader & header)
{
    uint8_t otaFileContent[kOtaHeaderMaxSize];
    ByteSpan buffer(otaFileContent);

    FS_ReadBufferFromFile(otaFilePath, reinterpret_cast<uint8_t *>(otaFileContent), kOtaHeaderMaxSize);
...
void chip::NXP::App::RTOTAProviderExample::SendQueryImageResponse(app::CommandHandler * commandObj,
                                                                  const app::ConcreteCommandPath & commandPath,
                                                                  const QueryImage::DecodableType & commandData)
{
    ...
    {
        RTGenerateUpdateToken(updateToken, kUpdateTokenLen);
        RTGetUpdateTokenString(ByteSpan(updateToken), strBuf, kUpdateTokenStrLen);
...
void chip::NXP::App::RTOTAProviderExample::HandleQueryImage(app::CommandHandler * commandObj,
                                                            const app::ConcreteCommandPath & commandPath,
                                                            const QueryImage::DecodableType & commandData)
{
    bool requestorCanConsent = commandData.requestorCanConsent.ValueOr(false);
    ...
    if (mQueryImageStatus == OTAQueryStatus::kUpdateAvailable)
    {
        ...
        {
            RTOTAProviderExample::DeviceSoftwareVersionModel candidate;
...
void chip::NXP::App::RTOTAProviderExample::HandleApplyUpdateRequest(app::CommandHandler * commandObj,
                                                                    const app::ConcreteCommandPath & commandPath,
                                                                    const ApplyUpdateRequest::DecodableType & commandData)
{
    ...
    RTGetUpdateTokenString(commandData.updateToken, tokenBuf, kUpdateTokenStrLen);
...
void chip::NXP::App::RTOTAProviderExample::HandleNotifyUpdateApplied(app::CommandHandler * commandObj,
                                                                     const app::ConcreteCommandPath & commandPath,
