                                           ...
                                           Optional<uint64_t> & outTimeSinceBoot)
{
    ...
    size_t diagSize = GetSizeForIntent(intent);
    VerifyOrReturnError(diagSize && (diagSize <= 0xFFFF), CHIP_ERROR_NOT_FOUND);

    uint8_t * diagData = (uint8_t *) calloc(1, diagSize);
    VerifyOrReturnError(diagData, CHIP_ERROR_NO_MEMORY);

    uint16_t diagSize16 = (uint16_t) diagSize;
    err                 = chip::Server::GetInstance().GetPersistentStorage().SyncGetKeyValue(key.KeyName(), diagData, diagSize16);
...
size_t LogProvider::GetSizeForIntent(IntentEnum intent)
{
    ...
    err = Server::GetInstance().GetPersistentStorage().SyncGetKeyValue(key.KeyName(), buffer.Get(), bufferLen);
    VerifyOrReturnValue(err == CHIP_NO_ERROR, 0);
    sizeForIntent = bufferLen;
