...
public:
	...
	{
		if(managerInstance == nullptr) managerInstance = new MatterManagerService();
...
private:
	...
	int makeAndSendCommand(std::string clusterName, std::string commandName, std::vector<std::string> parameters, std::string attributeName, std::string attributeValue);
