    ...
    public:
        ...
        static CHIP_ERROR ParseFromJson(const Json::Value& json, StructType& output) {
            static_assert(sizeof(StructType) == 0, 
                "No ParseFromJson specialization provided for this type");
        ...
        static CHIP_ERROR LoadFromJson(const Json::Value& json, DataClass& mgmtObj, 
                                     size_t maxLength) {
            if (!json.isArray()) return CHIP_ERROR_INVALID_ARGUMENT;
            if (json.size() > maxLength) return CHIP_ERROR_BUFFER_TOO_SMALL;
            ...
            for (const auto& entryJson : json) {
                StructType entry;
                if ((err = ParseFromJson(entryJson, entry)) != CHIP_NO_ERROR) break;
            ...
            if ((err = mgmtObj.CreateNewValue(tempList.size())) == CHIP_NO_ERROR) {
                ...
                for (auto& entry : tempList) mgmtObj.CleanupExtEntry(entry);
            }
        ...
        template <bool T = IsList, typename = std::enable_if_t<!T>>
        static CHIP_ERROR LoadFromJson(const Json::Value& json, DataClass& mgmtObj) {
            if (!json.isObject()) return CHIP_ERROR_INVALID_ARGUMENT;
