...
storage_value_type StaticSupportedTemperatureLevelsManager::temperatureLevelOptions[] = {
    buildTemperatureLevelStruct("wash Temperature", 1),
    buildTemperatureLevelStruct("rinse Temperature", 2),
...
const StaticSupportedTemperatureLevelsManager::EndpointSpanPair
    StaticSupportedTemperatureLevelsManager::supportedOptionsByEndpoints[EMBER_AF_MODE_SELECT_CLUSTER_SERVER_ENDPOINT_COUNT] = {
        EndpointSpanPair(1, Span<storage_value_type>(StaticSupportedTemperatureLevelsManager::temperatureLevelOptions)) // Options for Endpoint 1
    };

const StaticSupportedTemperatureLevelsManager StaticSupportedTemperatureLevelsManager::instance = StaticSupportedTemperatureLevelsManager();

StaticSupportedTemperatureLevelsManager::TemperatureLevelOptionsProvider StaticSupportedTemperatureLevelsManager::getTemperatureLevelOptionsProvider(EndpointId endpointId) const
{
...
Status StaticSupportedTemperatureLevelsManager::getTemperatureLevelOptionByTemperatureLevel(unsigned short endpointId, unsigned char temperatureLevel,
                                                        const TemperatureLevelStructType ** dataPtr) const
{
    ...
    auto * begin = this->getTemperatureLevelOptionsProvider(endpointId).begin();
    auto * end   = this->getTemperatureLevelOptionsProvider(endpointId).end();
