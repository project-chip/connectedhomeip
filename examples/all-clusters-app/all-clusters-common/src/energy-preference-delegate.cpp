...
EPrefDelegate::GetEnergyBalanceAtIndex(chip::EndpointId aEndpoint, size_t aIndex, BalanceStruct::Type &balance)
{
	...
	static BalanceStruct::Type energyBalances[] = 
	{
		{
			.step = 0,
			.label = Optional<chip::CharSpan>(chip::CharSpan(kEfficientLabel, strlen(kEfficientLabel)))
		},
		{
			.step = 50,
			.label = Optional<chip::CharSpan>()
		},
		{
			.step = 100,
			.label = Optional<chip::CharSpan>(chip::CharSpan(kComfortLabel, strlen(kComfortLabel)))
		},
	};

    if (aIndex < (sizeof(energyBalances) / sizeof (energyBalances[0])))
    {
        balance = energyBalances[aIndex];
...
EPrefDelegate::GetEnergyPriorityAtIndex(chip::EndpointId aEndpoint, size_t aIndex, EnergyPriorityEnum &priority)
{
	...
	{
		EnergyPriorityEnum::kEfficiency,
		EnergyPriorityEnum::kComfort
	};
...
EPrefDelegate::GetLowPowerModeSensitivityAtIndex(chip::EndpointId aEndpoint, size_t aIndex, BalanceStruct::Type &balance)
{
	...
	static BalanceStruct::Type powerBalances[] = 
	{
		{
			.step = 0,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k1MinuteLabel, strlen(k1MinuteLabel)))
		},
		{
			.step = 12,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k5MinutesLabel, strlen(k5MinutesLabel)))
		},
		{
			.step = 24,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k10MinutesLabel, strlen(k10MinutesLabel)))
		},
		{
			.step = 36,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k15MinutesLabel, strlen(k15MinutesLabel)))
		},
		{
			.step = 48,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k20MinutesLabel, strlen(k20MinutesLabel)))
		},
		{
			.step = 60,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k25MinutesLabel, strlen(k25MinutesLabel)))
		},
		{
			.step = 70,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k30MinutesLabel, strlen(k30MinutesLabel)))
		},
		{
			.step = 80,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k60MinutesLabel, strlen(k60MinutesLabel)))
		},
		{
			.step = 90,
			.label = Optional<chip::CharSpan>(chip::CharSpan(k120MinutesLabel, strlen(k120MinutesLabel)))
		},
		{
			.step = 100,
			.label = Optional<chip::CharSpan>(chip::CharSpan(kNeverLabel, strlen(kNeverLabel)))
		},
	};

    if (aIndex < (sizeof(powerBalances) / sizeof (powerBalances[0])))
    {
        balance = powerBalances[aIndex];
        return CHIP_NO_ERROR;
    }
