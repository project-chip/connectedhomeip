...
CHIP_ERROR MeterIdentification::LoadJson(const Json::Value & root)
{
        ...
        if (value.isUInt())
        {
            if (1 >= value.asUInt())
            {
                ...
                gMeterIdentificationCluster =
                    std::make_unique<Instance>(1, chip::BitMask<Feature, uint32_t>(Feature::kPowerThreshold));
                gMeterIdentificationCluster->Init();
                return ret;
            }
        }
    ...
    if (root.isMember("MeterType"))
    {
        ...
        if (value.isUInt())
        {
            ret = CHIP_NO_ERROR == ret ? gMeterIdentificationCluster->SetMeterType(
                                             chip::app::DataModel::MakeNullable(static_cast<MeterTypeEnum>(value.asUInt())))
                                       : ret;
        }
        else
        {
    ...
    if (root.isMember("PointOfDelivery"))
    {
        ...
        if (value.isString())
        {
            ret = CHIP_NO_ERROR == ret
                ? gMeterIdentificationCluster->SetPointOfDelivery(chip::CharSpan::fromCharString(value.asCString()))
                : ret;
        }
        else
        {
    ...
    if (root.isMember("MeterSerialNumber"))
    {
        ...
        {
            ret = CHIP_NO_ERROR == ret
                ? gMeterIdentificationCluster->SetMeterSerialNumber(chip::CharSpan::fromCharString(value.asCString()))
                : ret;
        }
        else
        {
    ...
    if (root.isMember("ProtocolVersion"))
    {
        ...
        {
            ret = CHIP_NO_ERROR == ret
                ? gMeterIdentificationCluster->SetProtocolVersion(chip::CharSpan::fromCharString(value.asCString()))
                : ret;
        }
        else
        {
    ...
    if (root.isMember("PowerThreshold"))
    {
        ...
        if (!value.empty() && value.isObject())
        {
            ...
            {
                ret = CHIP_NO_ERROR == ret
                    ? gMeterIdentificationCluster->SetPowerThreshold(chip::app::DataModel::MakeNullable(powerThreshopld))
                    : ret;
            }
            else
            {
