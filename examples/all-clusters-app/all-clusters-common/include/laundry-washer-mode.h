...
private:
    ...
    const ModeOptionStructType modeOptions[4] = {
        BuildModeOptionStruct("Normal", Clusters::LaundryWasherMode::ModeNormal, List<const ModeTagStructType>(modeTagsNormal)),
        BuildModeOptionStruct("Delicate", Clusters::LaundryWasherMode::ModeDelicate, List<const ModeTagStructType>(modeTagsDelicate)),
    ...
    uint8_t NumberOfModes() override {return 4;};
...
public:
    LaundryWasherModeInstance(EndpointId aEndpointId, ClusterId aClusterId, uint32_t aFeature) : Instance(aEndpointId, aClusterId, aFeature) {};
