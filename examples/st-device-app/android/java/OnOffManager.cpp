...
{
    ChipLogProgress(Zcl, "ST Device App::OnOff::PostClusterInit");
    StDeviceAppJNIMgr().PostClusterInit(chip::app::Clusters::OnOff::Id, endpoint);
}

void OnOffManager::NewManager(jint endpoint, jobject manager)
{
    ChipLogProgress(Zcl, "ST Device App: OnOffManager::NewManager");
    ...
    VerifyOrReturn(ep < kOnffManagerTableSize,
                   ChipLogError(Zcl, "ST Device App::OnOff::NewManager: endpoint %d not found", endpoint));

    VerifyOrReturn(gOnOffManagerTable[ep] == nullptr,
                   ChipLogError(Zcl, "ST Device App::OnOff::NewManager: endpoint %d already has a manager", endpoint));
    OnOffManager * mgr = new OnOffManager();
    CHIP_ERROR err     = mgr->InitializeWithObjects(manager);
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(Zcl, "ST Device App::OnOff::NewManager: failed to initialize manager for endpoint %d", endpoint);
        delete mgr;
    }
    else
    {
...
{
    ChipLogProgress(Zcl, "ST Device App: OnOffManager::PostOnOffChange");
