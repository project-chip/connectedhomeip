...
1. Pull docker image from repository:
    ...
    ```bash
    $ docker pull ghcr.io/project-chip/chip-build-telink:3
    ```

1. Run docker container:

    ```bash
    $ docker run -it --rm -v ${CHIP_BASE}:/root/chip -v /dev/bus/usb:/dev/bus/usb --device-cgroup-rule "c 189:* rmw" ghcr.io/project-chip/chip-build-telink:3
    ```
