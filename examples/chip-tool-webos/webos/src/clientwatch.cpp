...
ClientWatch::ClientWatch(LSHandle * handle, LSMessage * message, ClientWatchStatusCallback callback) :
    mHandle(handle), mMessage(message), mCookie(0), mCallback(callback), mNotificationTimeout(0)
{
    if (!mMessage)
        return;
...
void ClientWatch::startWatching()
{
    ...
    LS::Error error;
    if (!LSRegisterServerStatusEx(mHandle, serviceName, &ClientWatch::serverStatusCallback, this, &mCookie, error.get()))
        throw error;
