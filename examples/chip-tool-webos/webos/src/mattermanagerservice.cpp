...
MatterManagerService::MatterManagerService(void) : LS::Handle("com.webos.service.matter"), mMainLoop(0), mSendCommandWatch(0)
{
...
bool MatterManagerService::pair(LSMessage & msg)
{
    ...
    const std::string schema =
        STRICT_SCHEMA(PROPS_5(PROP(pairingType, string), PROP(setupPinCode, integer), PROP(discriminator, integer),
                              PROP(deviceRemoteIp, string), PROP(deviceRemotePort, integer)));
...
bool MatterManagerService::openCommissioningWindow(LSMessage & msg)
{
    ...
    const std::string schema = STRICT_SCHEMA(PROPS_5(PROP(timeout, integer), PROP(option, integer), PROP(discriminator, integer),
                                                     PROP(iteration, integer), PROP(endpointId, integer)));
...
bool MatterManagerService::revokeCommissioningWindow(LSMessage & msg)
{
    ...
    const std::string schema = STRICT_SCHEMA(PROPS_1(PROP(endpointId, integer)));
...
bool MatterManagerService::sendCommand(LSMessage & msg)
{
    ...
    const std::string schema =
        STRICT_SCHEMA(PROPS_5(PROP(clusterName, string), PROP(commandName, string), ARRAY(parameters, string),
                              PROP(attributeName, string), PROP(attributeValue, string)));
    ...
    mSendCommandWatch = new LSUtils::ClientWatch(get(), &msg, [this]() { notifySendCommandDropped(); });
...
int MatterManagerService::sendOpenCommissioningWindowCommand(int timeout, int option, int discriminator, int iteration,
                                                             int endpointId)
{
    MATTER_INFO("MANAGER_SERVICE", 0, "Chip Command is called : [%s : %d]", __FUNCTION__, __LINE__);
    ...
    else if (option == 2)
    {
        ...
        std::string PAKEVerifierStr("hex:"
                                    "aba60c30416b8f4177f5e16ad514cfd9577513f02fd60506b1049d0f2c7310010e5e40bfd86b4ef681a88b71e9e2a8"
                                    "53985a7def916ea30e01b8722fbf7d0e38856c12cd64c225bb24ef21417e0e44e5");
...
int MatterManagerService::makeAndSendCommand(std::string clusterName, std::string commandName, std::vector<std::string> parameters,
