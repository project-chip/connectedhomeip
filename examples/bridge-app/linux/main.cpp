...
void HandleDeviceStatusChanged(Device * dev, Device::Changed_t itemChangedMask)
{
    if (itemChangedMask & Device::kChanged_Reachable)
    {
        ...
        emberAfReportingAttributeChangeCallback(dev->GetEndpointId(),
    ...
    if (itemChangedMask & Device::kChanged_Location)
    {
        ...
        EmberAfAttributeMetadata am = { .attributeId = ZCL_LABEL_LIST_ATTRIBUTE_ID, .size = kFixedLabelAttributeArraySize, .defaultValue = nullptr };

        EncodeFixedLabel("room", dev->GetLocation(), buffer, sizeof(buffer), &am);

        emberAfReportingAttributeChangeCallback(dev->GetEndpointId(),
                                                ZCL_FIXED_LABEL_CLUSTER_ID,
...
EmberAfStatus HandleReadDescriptorAttribute(uint8_t endpointIndex, chip::AttributeId attributeId, uint8_t *buffer, uint16_t maxReadLength)
{
...
EmberAfStatus HandleWriteDescriptorAttribute(uint8_t endpointIndex, EmberAfAttributeMetadata * am, uint8_t *buffer, uint16_t length, int32_t index)
{
    chip::AttributeId attributeId = am->attributeId;

    if (emberAfCopyList(ZCL_DESCRIPTOR_CLUSTER_ID, am, true, &gDescriptorAttrStorage[endpointIndex][attributeId][0], buffer, index) > 0)
    {
        return EMBER_ZCL_STATUS_SUCCESS;
    }
...
EmberAfStatus emberAfExternalAttributeReadCallback(EndpointId endpoint, ClusterId clusterId, EmberAfAttributeMetadata * attributeMetadata,
...
EmberAfStatus emberAfExternalAttributeWriteCallback(EndpointId endpoint, ClusterId clusterId, EmberAfAttributeMetadata * attributeMetadata,
...
int main(int argc, char * argv[])
{
    ...
    gFirstDynamicEndpointId = static_cast<chip::EndpointId>(static_cast<int>(emberAfEndpointFromIndex(
                                                 static_cast<uint8_t>(emberAfFixedEndpointCount() - 1))) + 1);
    gCurrentEndpointId = gFirstDynamicEndpointId;
