    ...
    case 0x0040: // Fixed Label Cluster
    {
        ...
        switch (am->attributeId)
        {
            case 0x0000: // label list
            {
                ...
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, 18, labelSpan) : ReadByteSpan(src + entryOffset, 18, labelSpan)))
                {
                ...
                if (CHIP_NO_ERROR != (write ? WriteByteSpan(dest + entryOffset, 18, valueSpan) : ReadByteSpan(src + entryOffset, 18, valueSpan)))
                {
                    ChipLogError(Zcl, "Index %l is invalid. Not enough remaining space", index);
                    return 0;
                }
    ...
    case 0x0033: // General Diagnostics Cluster
    {
        ...
        switch (am->attributeId)
        {
            case 0x0000: // NetworkInterfaces
            {
                ...
                copyListMember(write ? dest : (uint8_t *)&entry->HardwareAddress, write ? (uint8_t *)&entry->HardwareAddress : src, write, &entryOffset, sizeof(entry->HardwareAddress)); // IEEE_ADDRESS
