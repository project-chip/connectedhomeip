...
#define GENERATED_ATTRIBUTES { \
  ...
  { 0x0003, ZAP_TYPE(INT16U), 2, 0, { (uint8_t *) 0 } }, /* Temperature Measurement (server): tolerance */  \
}
...
#define GENERATED_CLUSTERS { \
  ...
  { 0x0402, ZAP_ATTRIBUTE_INDEX(3), 5, 10, ZAP_CLUSTER_MASK(SERVER), NULL }, /* Endpoint: 1, Cluster: Temperature Measurement (server) */ \
}
...
#define FIXED_PROFILE_IDS { 0x0105 }
...
#define EMBER_AF_GENERATED_REPORTING_CONFIG_DEFAULTS { \
  ...
  { ZAP_REPORT_DIRECTION(REPORTED), 0x0001, 0x0402, 0x0003, ZAP_CLUSTER_MASK(SERVER), 0x0000, {{ 0, 65344, 0 }} }, /* Reporting for cluster: "Temperature Measurement", attribute: "tolerance". side: server */ \
}



// CLUSTER defines to replace PLUGIN logic where needed.
#define CHIP_BASIC_CLUSTER_SERVER
#define CHIP_TEMP_MEASUREMENT_CLUSTER_SERVER
