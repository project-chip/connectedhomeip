...
class TestUtil : public TestCommand
{
public:
    TestUtil() : TestCommand("TestUtil"), mTestIndex(0) {}
    ...
    void NextTest() override
    {
        ...
        if (mTestCount == mTestIndex)
        {
            ChipLogProgress(chipTool, "TestUtil: Test complete");
            SetCommandExitStatus(CHIP_NO_ERROR);
        }
        ...
        {
        case 0:
            err = TestSendClusterTestUtilCommandWaitForMs_0();
            break;
        }

        if (CHIP_NO_ERROR != err)
        {
            ChipLogProgress(chipTool, "TestUtil: %s", chip::ErrorStr(err));
            SetCommandExitStatus(err);
        }
    }
...
private:
    ...
    CHIP_ERROR TestSendClusterTestUtilCommandWaitForMs_0()
    {
        ChipLogProgress(chipTool, "TestUtil - Wait 100ms");
