...
public:
    ...
    void OnMessage(ChipDeviceController * dc, System::PacketBuffer * buffer) override;

    virtual size_t EncodeCommand(System::PacketBuffer * buffer, size_t bufferSize, uint16_t endPointId) = 0;

private:
    void SendCommand(ChipDeviceController * dc);
    void ReceiveCommandResponse(ChipDeviceController * dc, System::PacketBuffer * buffer);
