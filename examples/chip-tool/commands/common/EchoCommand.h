...
public:
    ...
    void OnMessage(ChipDeviceController * dc, System::PacketBuffer * buffer) override { ReceiveEcho(dc, buffer); }
    void OnError(ChipDeviceController * dc, CHIP_ERROR err) override {}

private:
    void SendEcho(ChipDeviceController * dc);
    void ReceiveEcho(ChipDeviceController * dc, System::PacketBuffer * buffer);
};
