...
public:
    ...
    static void CallbackFn(void * context, uint32_t delayedActionTime, uint8_t * imageURI, uint32_t softwareVersion,
        uint8_t * updateToken, uint8_t userConsentNeeded, uint8_t * metadataForClient)
    {
        ...
        if (callback && callback->mQueue) {
            dispatch_async(callback->mQueue, ^{
                callback->mHandler(nil, @ {
                    ...
                    @"updateToken" : [NSString stringWithFormat:@"%s", updateToken],
                    @"userConsentNeeded" : [NSNumber numberWithUnsignedChar:userConsentNeeded],
                    @"metadataForClient" : [NSString stringWithFormat:@"%s", metadataForClient],
                });
                callback->Cancel();
                delete callback;
            });
