...
public:
    CHIPFabricClusterGetFabricIdResponseCallbackBridge(ResponseHandler handler, dispatch_queue_t queue)
        : Callback::Callback<FabricClusterGetFabricIdResponseCallback>(CallbackFn, this)
        , mHandler(handler)
        , mQueue(queue)
    {
    }
    ...
    {
        CHIPFabricClusterGetFabricIdResponseCallbackBridge * callback
            = reinterpret_cast<CHIPFabricClusterGetFabricIdResponseCallbackBridge *>(context);
        if (callback && callback->mQueue) {
            dispatch_async(callback->mQueue, ^{
                callback->mHandler(nil, @ {
...
public:
    CHIPFabricFabricsListAttributeCallbackBridge(ResponseHandler handler, dispatch_queue_t queue)
        : Callback::Callback<FabricFabricsListListAttributeCallback>(CallbackFn, this)
        , mHandler(handler)
        , mQueue(queue)
    {
    }
    ...
    {
        CHIPFabricFabricsListAttributeCallbackBridge * callback
            = reinterpret_cast<CHIPFabricFabricsListAttributeCallbackBridge *>(context);
        if (callback && callback->mQueue) {
            id values[count];
            for (uint16_t i = 0; i < count; i++) {
                values[i] =
...
{
    CHIPFabricClusterGetFabricIdResponseCallbackBridge * onSuccess
        = new CHIPFabricClusterGetFabricIdResponseCallbackBridge(completionHandler, [self callbackQueue]);
    if (!onSuccess) {
        completionHandler([CHIPError errorForCHIPErrorCode:CHIP_ERROR_INCORRECT_STATE], nil);
        return;
    }
...
- (void)updateFabricLabel:(NSData *)fabricLabel completionHandler:(ResponseHandler)completionHandler
{
    ...
    CHIP_ERROR err = self.cppCluster.UpdateFabricLabel(
        onSuccess->Cancel(), onFailure->Cancel(), chip::ByteSpan((const uint8_t *) fabricLabel.bytes, fabricLabel.length));
    if (err != CHIP_NO_ERROR) {
        delete onSuccess;
        delete onFailure;
        completionHandler([CHIPError errorForCHIPErrorCode:err], nil);
...
{
    CHIPFabricFabricsListAttributeCallbackBridge * onSuccess
        = new CHIPFabricFabricsListAttributeCallbackBridge(completionHandler, [self callbackQueue]);
    if (!onSuccess) {
        completionHandler([CHIPError errorForCHIPErrorCode:CHIP_ERROR_INCORRECT_STATE], nil);
        return;
    }
