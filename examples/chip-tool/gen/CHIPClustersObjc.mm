...
class CHIPFabricClusterGetFabricIdResponseCallbackBridge : public Callback::Callback<FabricClusterGetFabricIdResponseCallback>
{
public:
    CHIPFabricClusterGetFabricIdResponseCallbackBridge(ResponseHandler handler, dispatch_queue_t queue): Callback::Callback<FabricClusterGetFabricIdResponseCallback>(CallbackFn, this), mHandler(handler), mQueue(queue) {}

    ~CHIPFabricClusterGetFabricIdResponseCallbackBridge() {};

    static void CallbackFn(void * context, chip::FabricId fabricId)
    {
        CHIPFabricClusterGetFabricIdResponseCallbackBridge * callback = reinterpret_cast<CHIPFabricClusterGetFabricIdResponseCallbackBridge *>(context);
        if (callback && callback->mQueue)
        {
            dispatch_async(callback->mQueue, ^{
                callback->mHandler(nil, @{
...
class CHIPFabricFabricsListAttributeCallbackBridge : public Callback::Callback<FabricFabricsListListAttributeCallback>
{
public:
    CHIPFabricFabricsListAttributeCallbackBridge(ResponseHandler handler, dispatch_queue_t queue): Callback::Callback<FabricFabricsListListAttributeCallback>(CallbackFn, this), mHandler(handler), mQueue(queue) {}

    ~CHIPFabricFabricsListAttributeCallbackBridge() {};

    static void CallbackFn(void * context, uint16_t count, _FabricDescriptor * entries)
    {
        CHIPFabricFabricsListAttributeCallbackBridge * callback = reinterpret_cast<CHIPFabricFabricsListAttributeCallbackBridge *>(context);
        if (callback && callback->mQueue)
        {
            id values[count];
            for (uint16_t i = 0; i < count; i++)
...
@interface CHIPFabric ()
@property (readonly) Controller::FabricCluster cppCluster;
@end

@implementation CHIPFabric
...
{
    CHIPFabricClusterGetFabricIdResponseCallbackBridge * onSuccess = new CHIPFabricClusterGetFabricIdResponseCallbackBridge(completionHandler, [self callbackQueue]);
    if (!onSuccess) {
        completionHandler([CHIPError errorForCHIPErrorCode:CHIP_ERROR_INCORRECT_STATE], nil);
        return;
    }
...
{
    CHIPFabricFabricsListAttributeCallbackBridge * onSuccess = new CHIPFabricFabricsListAttributeCallbackBridge(completionHandler, [self callbackQueue]);
    if (!onSuccess) {
        completionHandler([CHIPError errorForCHIPErrorCode:CHIP_ERROR_INCORRECT_STATE], nil);
        return;
    }
