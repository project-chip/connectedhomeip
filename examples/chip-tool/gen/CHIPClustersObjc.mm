...
public:
    ...
    static void CallbackFn(void * context, uint8_t keypadInputStatus)
    {
        ...
        if (callback && callback->mQueue)
        {
            dispatch_async(callback->mQueue, ^{
                callback->mHandler(nil, @{
                  @"keypadInputStatus": [NSNumber numberWithUnsignedChar:keypadInputStatus],
                                  });
                callback->Cancel();
                delete callback;
            });
...
{
public:
    CHIPMediaPlaybackClusterPlaybackCallbackBridge(ResponseHandler handler, dispatch_queue_t queue): Callback::Callback<MediaPlaybackClusterPlaybackCallback>(CallbackFn, this), mHandler(handler), mQueue(queue) {}

    ~CHIPMediaPlaybackClusterPlaybackCallbackBridge() {};

    static void CallbackFn(void * context)
    {
        CHIPMediaPlaybackClusterPlaybackCallbackBridge * callback = reinterpret_cast<CHIPMediaPlaybackClusterPlaybackCallbackBridge *>(context);
        if (callback && callback->mQueue)
        {
            dispatch_async(callback->mQueue, ^{
                callback->mHandler(nil, @{
...
- (void)launchApp:(uint16_t)catalogVendorId applicationId:(NSString *)applicationId data:(NSString *)data completionHandler:(ResponseHandler)completionHandler
{
    ...
    CHIP_ERROR err = self.cppCluster.LaunchApp(onSuccess->Cancel(), onFailure->Cancel(), catalogVendorId, chip::ByteSpan((const uint8_t*)[applicationId dataUsingEncoding:NSUTF8StringEncoding].bytes, [applicationId lengthOfBytesUsingEncoding:NSUTF8StringEncoding]), chip::ByteSpan((const uint8_t*)[data dataUsingEncoding:NSUTF8StringEncoding].bytes, [data lengthOfBytesUsingEncoding:NSUTF8StringEncoding]));
    if (err != CHIP_NO_ERROR) {
        delete onSuccess;
        delete onFailure;
        completionHandler([CHIPError errorForCHIPErrorCode:err], nil);
...
- (void)sendKey:(uint8_t)keyAction keyCode:(uint8_t)keyCode completionHandler:(ResponseHandler)completionHandler
{
    ...
    CHIP_ERROR err = self.cppCluster.SendKey(onSuccess->Cancel(), onFailure->Cancel(), keyAction, keyCode);
    if (err != CHIP_NO_ERROR) {
        delete onSuccess;
        delete onFailure;
        completionHandler([CHIPError errorForCHIPErrorCode:err], nil);
