...
-   [Matter Light Switch Example Application](#matter-light-switch-example-application)
...
The CC13XX_26XX light-switch example application provides a working
demonstration of a connected light-switch device. This uses the open-source
...
| ------------------------------------------------ | ---------------------------------- |
| Left Button (`BTN-1`) Press (less than 1000 ms)  | Turns connected bulb off           |
...
The steps below should be followed to commission the light-switch device onto
the network and control it once it has been commissioned.
...
First, you will need to have a lighting app device setup and commissioned into
the Matter network. Look through the README.md file for lighting app for
building and commissioning instructions for the default lighting app.

Commission the light-switch device onto the Matter network. Run the following
command on the CHIP tool:
...
You can check if the light-switch is able to receive commands from CHIP-TOOL by
using this command:
...
Tell the lighting device to give the light-switch device access to it's onoff
cluster (replace <> with lighting/light-switch node IDs):
...
Tell the light-switch device to bind to the onoff cluster on the lighting device
(replace <> with lighting/light-switch node IDs):
...
Now you can press the Right BTN on the light-switch device and it will turn ON
