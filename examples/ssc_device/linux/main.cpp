...
CHIP_ERROR PrintQRCodeContent()
{
    ...
    ChipLogProgress(DeviceLayer, "SetupPINCode: [%d]", setUpPINCode);
    // There might be whitespace in setup QRCode, add brackets to make it clearer.
    ChipLogProgress(DeviceLayer, "SetupQRCode:  [%s]", result.c_str());

exit:
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "Failed to generate QR Code: %s", ErrorStr(err));
    }
    return err;
}
} // namespace

class DeviceDelegate : public AppDelegate
{
public:
    virtual void OnReceiveError() {}
    virtual void OnRendezvousStarted()
    {
        // TODO: Need to request operational credentials here
        ChipLogError(DeviceLayer, "OnRendezvousStarted(): !!!! Commissioning has been finished!!! ");
    }
    virtual void OnRendezvousStopped()
    {
        ChipLogError(DeviceLayer, "OnRendezvousStopped() !!!! Commissioning has been finished!!! ");
    }
...
int main(int argc, char * argv[])
{
    ...
    chip::Logging::SetLogFilter(kDefaultLoggingLevel);
    ...
    if (LinuxDeviceOptions::GetInstance().mThread)
    {
        ...
        ChipLogError(DeviceLayer, "Thread initialized.");
    }
...
exit:
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "Failed to run Linux Lighting App: %s", ErrorStr(err));
