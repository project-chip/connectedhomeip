...
def _extract_case_block(content: str, cluster_name: str) -> str:
    ...
    Finds `case app::Clusters::<cluster_name>::Id: {
    ` and then uses a brace - depth counter to locate the matching closing `
}
...
# Skip multi - line comments
    ...
    raise ValueError(f "Unbalanced braces for cluster '{cluster_name}' — " f "reached end of file without finding matching '}}'")
        ...
        def _normalize_whitespace(text : str)
            ->str : ""
                    "
                    Collapse all runs of whitespace(spaces, tabs, newlines)
into a single space and strip leading /
    trailing whitespace
        .""
         "
...
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
            ...
            _slim_content = None _full_content = None
                                                     ...
                                                     def _get_slim_content() -> str : global _slim_content if _slim_content is None
    : _slim_content = _read_file(SLIM_ATTR_DECODER) return _slim_content

                          def _get_full_content()
                              ->str : global _full_content if _full_content is None
    : _full_content = _read_file(FULL_ATTR_DECODER) return _full_content

    _slim_event_content = None _full_event_content = None

                                                         def _get_slim_event_content()
                                                             ->str : global _slim_event_content if _slim_event_content is None
    : _slim_event_content = _read_file(SLIM_EVENT_DECODER) return _slim_event_content

                                def _get_full_event_content()
...
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
    ...
    *
    ...
    Property 4 : For each cluster in the Casting_Cluster_Set,
            the switch -
    case body in the slim attribute decoder SHALL be identical(modulo whitespace normalization) to the corresponding switch -
    case body in the full zap -
    generated CHIPAttributeTLVValueDecoder.cpp.""
                                               "
    slim_content = _get_slim_content() full_content = _get_full_content()

                slim_body = _extract_case_block(slim_content, cluster) full_body = _extract_case_block(full_content, cluster)

                    slim_normalized = _normalize_whitespace(slim_body) full_normalized = _normalize_whitespace(full_body)

                                                                                             assert slim_normalized
    == full_normalized,
...
# -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -
                ...
                @given(cluster = st.sampled_from(CASTING_CLUSTERS))
                    @settings(max_examples = 100, suppress_health_check = [HealthCheck.function_scoped_fixture],
                              deadline = None, ) def test_event_decode_logic_equivalence(cluster):
""
    "
    ...
    *
    ...
    Property 4 : For each cluster in the Casting_Cluster_Set,
    the switch -
    case body in the slim event decoder SHALL be identical(modulo whitespace normalization) to the corresponding switch -
    case body in the full zap -
    generated CHIPEventTLVValueDecoder.cpp.

    If a cluster is not present in the full event decoder(no events defined),
    it must also be absent from the slim event decoder — or
    both must have the same empty -
        event pattern.""
                      "
        slim_content = _get_slim_event_content() full_content = _get_full_event_content()
