...
CHIP_ERROR CastingServer::OpenBasicCommissioningWindow(std::function<CHIP_ERROR(bool)> commissioningCompleteCallback)
{
...
{
    CastingServer::GetInstance()->mLaunchURLResponseCallback(true);
}

void CastingServer::OnContentLauncherFailureResponse(void * context, CHIP_ERROR error)
{
    ChipLogError(AppServer, "OnContentLauncherFailureResponse %" CHIP_ERROR_FORMAT, error.Format());
    CastingServer::GetInstance()->mLaunchURLResponseCallback(false);
}
...
void CastingServer::DeviceEventCallback(const DeviceLayer::ChipDeviceEvent * event, intptr_t arg)
{
    ...
    else if (event->Type == DeviceLayer::DeviceEventType::kCommissioningComplete)
    {
        ...
        CastingServer::GetInstance()->mCommissioningCompleteCallback(err == CHIP_NO_ERROR);
    }
}
