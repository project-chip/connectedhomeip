                                                     ...
                                                     const SessionHandle & sessionHandle)
{
    ...
    VerifyOrReturn(self->mInitialized,
                   ChipLogProgress(BDX, "MediaFileManagementBdxClient: session established after abort, ignoring"));
...
void MediaFileManagementBdxClient::HandleTransferSessionOutput(TransferSession::OutputEvent & event)
{
    ...
    case TransferSession::OutputEventType::kMsgToSend: {
        ...
        else if (isBlockAckEof)
        {
            ...
            // size mismatch against the metadata GetSharedFile advertised.
            if (!mOutFile.good())
            {
                ChipLogError(BDX, "MediaFileManagementBdxClient: write error on %s", mTargetPath.c_str());
            }
            else if (mExpectedSize != 0 && mBytesWritten != mExpectedSize)
            {
                ChipLogError(BDX, "MediaFileManagementBdxClient: size mismatch, got %llu expected %llu",
