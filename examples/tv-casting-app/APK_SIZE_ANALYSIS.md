...
| Disable host unit-test hooks     | Default `CONFIG_BUILD_FOR_HOST_UNIT_TEST=0` in `CHIPProjectAppConfig.h` — excludes test-only code paths in CASESession, ExchangeContext, FabricTable, and MRP configuration. Set in the shared project-config header (not a per-target define) so every TU agrees on the layout of these types. | ~50–100 KB        |
...
| `CONFIG_BUILD_FOR_HOST_UNIT_TEST` | `0`      | `CHIPProjectAppConfig.h` (global project-config include; never a per-target define) |
