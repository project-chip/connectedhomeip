                                                   ...
                                                   const QueryImage::DecodableType & commandData)
{
    ...
    if (otaAvailable)
    {
        ...
        chip::bdx::MakeURI(nodeId, CharSpan(otaFilePath, strlen(otaFilePath)), uri);
    ...
    if (queryStatus == OTAQueryStatus::kUpdateAvailable)
    {
        ...
        response.softwareVersionString.Emplace(chip::CharSpan(newSoftwareVersionString, strlen(newSoftwareVersionString)));
