...
public:
    RelativeHumiditySensorManager(EndpointId aEndpointId) : mEndpointId(aEndpointId)
    {
        ...
        VerifyOrReturn(EMBER_ZCL_STATUS_SUCCESS == status,
                       ChipLogError(NotSpecified,
                                    "AirQualitySensor App: Failed to set RelativeHumidityMeasurement MinMeasuredValue attribute %x",
                                    (unsigned int) status));

        status = RelativeHumidityMeasurement::Attributes::MaxMeasuredValue::Set(mEndpointId, 100);
        VerifyOrReturn(EMBER_ZCL_STATUS_SUCCESS == status,
                       ChipLogError(NotSpecified,
                                    "AirQualitySensor App: Failed to set RelativeHumidityMeasurement MaxMeasuredValue attribute %x",
                                    (unsigned int) status));
    };
    ...
    void OnHumidityChangeHandler(uint16_t newValue)
    {
        ...
        VerifyOrReturn(EMBER_ZCL_STATUS_SUCCESS == status,
                       ChipLogError(NotSpecified,
                                    "AirQualitySensor App: Failed to set RelativeHumidityMeasurement MeasuredValue attribute %x",
                                    (unsigned int) status));
    }
