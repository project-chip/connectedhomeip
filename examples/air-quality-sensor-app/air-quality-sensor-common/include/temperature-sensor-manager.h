...
public:
    TemperatureSensorManager(EndpointId aEndpointId) : mEndpointId(aEndpointId)
    {
        ...
        VerifyOrReturn(EMBER_ZCL_STATUS_SUCCESS == status,
                       ChipLogError(NotSpecified,
                                    "AirQualitySensor App: Failed to set TemperatureMeasurement MinMeasuredValue attribute %x",
                                    (unsigned int) status));

        status = TemperatureMeasurement::Attributes::MaxMeasuredValue::Set(mEndpointId, 60);
        VerifyOrReturn(EMBER_ZCL_STATUS_SUCCESS == status,
                       ChipLogError(NotSpecified,
                                    "AirQualitySensor App: Failed to set TemperatureMeasurement MaxMeasuredValue attribute %x",
                                    (unsigned int) status));
    };
    ...
    void OnTemperatureChangeHandler(int16_t newValue)
    {
        ...
        VerifyOrReturn(EMBER_ZCL_STATUS_SUCCESS == status,
                       ChipLogError(NotSpecified,
                                    "AirQualitySensor App: Failed to set TemperatureMeasurement MeasuredValue attribute %x",
                                    (unsigned int) status));
    }
