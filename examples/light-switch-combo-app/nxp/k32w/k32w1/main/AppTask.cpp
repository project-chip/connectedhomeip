...
{
    if ((pin_no != RESET_BUTTON) && (pin_no != LIGHT_BUTTON) && (pin_no != OTA_BUTTON) && (pin_no != BLE_BUTTON) && (pin_no != SW_BUTTON))
    {
        return;
    }
    ...
    }else if (pin_no == SW_BUTTON)
    {
        button_event.Handler = SWHandler;
    }
        ...
        case kBUTTON_EventShortPress:
            switch (pinNb)
            {
                ...
                case LIGHT_BUTTON:
                    ...
                    ButtonEventHandler(SW_BUTTON, SW_BUTTON_PUSH);
                    break;
            }
            break;
...
void AppTask::SWHandler(AppEvent * aEvent)
{

    if (aEvent->Type == AppEvent::kEventType_Button)
    {
        PlatformMgr().ScheduleWork(AppTask::SwitchActionEventHandler, 0);
    }
}

void AppTask::SwitchActionEventHandler(intptr_t arg)
{
    BindingCommandData * data = Platform::New<BindingCommandData>();
    K32W_LOG("SW Button pressed");
    if(data != NULL)
    {
