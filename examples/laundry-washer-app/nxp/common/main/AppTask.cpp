...
static CHIP_ERROR cliOpState(int argc, char * argv[])
{
    ...
    if (map_cmd_opstate.find(argv[0]) != map_cmd_opstate.end())
    {
        ...
        if ((error == CHIP_NO_ERROR)&&(map_cmd_opstate.at(argv[0]) == to_underlying(chip::app::Clusters::OperationalState::OperationalStateEnum::kRunning))) {
            chip::app::Clusters::OperationalState::GenericOperationalError err(to_underlying(chip::app::Clusters::OperationalState::ErrorStateEnum::kNoError));
