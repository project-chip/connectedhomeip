...
class TestPkgConfigPath(unittest.TestCase):
    """Verify that SDK OpenSSL selection is isolated from unrelated queries."""

    def setUp(self) -> None:
        """Create system and private OpenSSL pkg-config fixtures."""
    ...
    def query(self, *args: str) -> list[list[str]]:
        """Query the OpenSSL fixture through the SDK pkg-config wrapper."""
        output = subprocess.check_output(
            [sys.executable, str(Path(__file__).with_name("pkg-config.py")), *args, "openssl"],
            env=self.env, text=True)
        return json.loads(output)

    def test_sdk_environment_does_not_change_unrelated_queries(self) -> None:
        """Verify that SDK environment variables leave system queries unchanged."""
    ...
    def test_private_path_is_scoped_to_one_query(self) -> None:
        """Verify that an explicit private path affects only its own query."""
        self.assertEqual(self.query("--pkg-config-path", str(self.private))[3], ["sdk_crypto"])
        self.assertEqual(self.query()[3], ["system_crypto"])

    def test_private_static_query_includes_private_dependencies(self) -> None:
        """Verify that static queries include the private library dependencies."""
