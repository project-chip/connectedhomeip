...
[
    ...
    if test "$1" = 1; then
        ...
        AC_ARG_WITH(lwip-target,
            AS_HELP_STRING([--with-lwip-target=target],
                [Specify the target environment for which LwIP will be built.  Choose one of: standalone, nrf5, efr32, k32w, qpg6100, none @<:@default=standalone@:>@.]),
            [
                if test "${nl_with_lwip}" != "internal"; then
                    AC_MSG_ERROR([--with-lwip-target can only be used when --with-lwip=internal is selected])
                else
                    case "${withval}" in
                    standalone|nrf5|efr32|k32w|qpg6100|none)
        ...
        if test "${nl_with_lwip}" = "internal"; then
            ...
            k32w)
                ...
                ;;
            qpg6100)
                LWIP_CPPFLAGS="${LWIP_CPPFLAGS} -I${ac_abs_confdir}/src/lwip/qpg6100 -I${ac_abs_confdir}/src/lwip/freertos"
                ;;
            *)
                ;;
            esac
    ...
    AM_CONDITIONAL([CHIP_LWIP_TARGET_QPG6100], [test "${nl_with_lwip}" = "internal" -a "${nl_with_lwip_target}" = "qpg6100" ])
])
