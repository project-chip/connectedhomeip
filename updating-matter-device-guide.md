...
1.  **Start `chip-tool`:** If you don't have a persistent Matter network
    running, you will need to start the `chip-tool` in interactive mode to
    create one. For this guide, we will assume a clean setup.

2.  **Commission the Device:** Use `chip-tool` to pair the device. This example
    ...
    ./out/chiptool/chip-tool pairing onnetwork ${DEVICE_NODE_ID} ${MANUAL_PAIRING_CODE}

    # Example:
    ./out/chiptool/chip-tool pairing onnetwork 1000 20202021
    ```
...
# Example:
./out/debug/chip-ota-provider-app --filepath image.ota -q updateAvailable --KVS /tmp/chip_kvs_provider --secured-device-port 5541 --discriminator 1111 --passcode 20202022
```
...
1.  **Pair OTA Provider using chip-tool on the same fabric:**
    ...
    ./out/chiptool/chip-tool pairing onnetwork ${PROVIDER_NODE_ID} ${OTA_PROVIDER_MANUAL_PAIRING_CODE}

    # Example:
    ./out/chiptool/chip-tool pairing onnetwork 2000 20202022
    ```
...
3.  **Trigger the Update (Announce OTA Provider):** This is the most effective
    ...
    -   **Announcement Reason:** `0` for `kSimpleAnnouncement` (advisory).
    -   **Target Device Node ID:** e.g., `2`

    ```bash
    # Announce OTA Provider
    ./out/chiptool/chip-tool otasoftwareupdatemanagement announce-ota-provider ${PROVIDER_NODE_ID} 0 0 0 ${DEVICE_NODE_ID} 0

    # Example:
    ./out/chiptool/chip-tool otasoftwareupdaterequestor announce-otaprovider 2000 0 0 0 1000 0
    ```
