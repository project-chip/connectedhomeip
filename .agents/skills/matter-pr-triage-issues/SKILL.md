...
description:
    For one pull request the triage recommended closing, the issues that should
    be closed with it, linked on GitHub or not.
---
...
For one pull request the triage recommended closing, whether a maintainer has
closed it yet or not: the issues that should be closed along with it, because
the work that made the pull request redundant resolves them too, whether or not
GitHub linked them to it. That list is the whole output; the pull request's own
state only appears in the header. The backend script gathers the evidence; you
supply the judgment. You never post, comment, close, label or otherwise modify
anything on GitHub.
...
```
For one pull request the triage recommended closing, closed yet or not: the issues that should be closed along with it, because the work that made the pull request redundant resolves them too, linked on GitHub or not. A script gathers the evidence; a language model reads it and writes the verdict. It never posts, comments, closes, labels or changes anything.

/matter-pr-triage-issues #40191                 the issues tied to that pull request, one report
...
/matter-pr-triage-issues sync                   build or refresh the local issue corpus; once per repository, about a minute
...
The report has these sections.
  SAFE TO CLOSE              the landed work resolves what these ask for; each carries the fact that shows it
                             and names any duplicate; a magnifier marks one found by content, not by a GitHub link
  LINKED BUT NOT RESOLVED BY THE PR   linked to the pull request on GitHub, but what they ask for isn't resolved by it
  ALREADY CLOSED                      linked, and closed already
  COULD NOT DETERMINE                 the check a person would do next
Only the first is always present.
...
triaged on this machine. Say so in one line: the linked and inferred tiers still
work, but no issue can be marked as closing with its pull request, because that
needs the pull request's own verdict. Do not run the sibling to get one; that is
a separate, larger decision the user makes.
...
The inferred tier works from a local corpus of the repository's issues, every
open one and the recently closed ones, scored on several signals at once: shared
test identifiers, file names, error codes and symbols; wording, with title words
weighted; an issue that names the pull request; a file in the same folder; the
issue graph around the linked issues; and, as boosters only, same author, same
weeks and shared labels. Without a corpus, `gather` falls back to a few API
searches and says so in `note`. Then offer `sync` in one line, about forty API
calls and a minute, and run it on a yes; after that, re-run `gather`. The corpus
refreshes incrementally, so a later `sync` costs a call or two.
...
For each issue, answer one question: **should this issue be closed along with
the pull request, because the work that made the pull request redundant resolves
it?**

1. **Is it already closed?** Then the verdict is `already-closed`. Its
   `state_reason` says how: completed, not planned, or duplicate.
2. **Is it the same ask?** Read the issue's own words, not the pull request's
   title. For `linked`, `referenced_by` and `mentioned` issues the tie is
   established, but a mention can be incidental, "similar to", "see also",
   "unlike"; that is `unrelated`. For `inferred` issues nothing on GitHub ties
   them to the pull request, so this question comes first and most candidates
   fail it: `unrelated`, and say what the issue asks for instead. An inferred
   issue that passes gets at most `medium` confidence, with a falsifier: the
   check that would show it is a different ask after all.
3. **Does the landed work resolve it?** The pull request's coverage proof is
   ...
   closable as covered unless `disagreement_reason` says why.
    ...
    branch for the issue's own ask before settling, with read-only commands from
    a clone, `git ls-tree` or `git grep` at `pr.triage.master_sha` when the
    dossier carries it, never at the branch name and never fetching. Found:
    `close`. Absent: `leave`. No clone to read: `unclear`, naming that check.
...
and the issue numbers as `to_close`. `close` verdicts fill Safe to close,
`leave` fills Linked but not resolved by the PR, `already-closed` and `unclear`
their own sections; `unrelated` leaves no trace.
