...
#
set -e
find "$(git rev-parse --show-toplevel)"/integrations/docker/images/base -name Dockerfile | while read -r dockerfile; do
...
find "$(git rev-parse --show-toplevel)"/integrations/docker/images/stage-1 -name Dockerfile | while read -r dockerfile; do
    pushd "$(dirname "$dockerfile")" >/dev/null
    ./build.sh "$@"
    popd >/dev/null
done

find "$(git rev-parse --show-toplevel)"/integrations/docker/images/stage-2 -name Dockerfile | while read -r dockerfile; do
    pushd "$(dirname "$dockerfile")" >/dev/null
    ./build.sh "$@"
    popd >/dev/null
done
