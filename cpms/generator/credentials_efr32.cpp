...
Credentials::Credentials(uint32_t kid)
{
...
int Credentials::generateKey()
{
    ...
    psa_key_id_t id = 0;
    int err = psa_generate_key(&attr, &id);
...
int asn1_callback(void *ctx, int tag, unsigned char *value, size_t size)
{
    if(MBEDTLS_ASN1_OCTET_STRING == tag)
    {
        ...
        return creds->importKey(value, size);
    }
    return 0;
}


int Credentials::importDerKey(const uint8_t * asn1, size_t size)
{
...
int Credentials::importKey(const uint8_t * key, size_t size)
{
    int err = 0;
    ...
    psa_key_id_t id = 0;
    err = psa_import_key(&attr, key, size, &id);
