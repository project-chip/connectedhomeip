...
JNI_METHOD(jlong, MyClusterCluster, initWithDevice)(JNIEnv * env, jobject self, jlong devicePtr, jint endpointId)
{
    ...
    MyClusterCluster * cppCluster =
        new MyClusterCluster(*device->GetExchangeManager(), device->GetSecureSession().Value(), endpointId);
    return reinterpret_cast<jlong>(cppCluster);
}

JNI_METHOD(void, MyClusterCluster, foo)
(JNIEnv * env, jobject self, jlong clusterPtr, jobject callback, jobject argument, jobject timedInvokeTimeoutMs)
{
    chip::DeviceLayer::StackLock lock;
    ...
    std::vector<Platform::UniquePtr<JniUtfString>> cleanupStrings;
    {
        ...
        if (optionalValue_0)
        {
            ...
            cleanupByteArrays.push_back(
                chip::Platform::MakeUnique<chip::JniByteArray>(env, static_cast<jbyteArray>(optionalValue_0)));
