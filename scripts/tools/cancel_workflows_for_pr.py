...
class Canceller:
    ...
    def check_all_pending_prs(self, max_age, required_runs):
        ...
        log.info("Searching PRs updated after '%s'", cutoff)
    ...
    def check_pr(self, pr: PullRequest, required_runs):
        ...
        if last_commit is None:
            log.error("Could not find any commit in the pull request.")
            return
        ...
        for check_suite in last_commit.get_check_suites():
            ...
            for run in check_suite.get_check_runs():
                ...
                if run.conclusion is None:
                    ...
                    log.warning("    Workflow '%s' failed", workflow.name)
        ...
        for id in in_progress_workflows:
            ...
            if self.dry_run:
                ...
                workflow.cancel()
                log.warning("Stopping workflow '%s'", workflow.name)
