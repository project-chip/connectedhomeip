...
TRIGGER_PATH_KEYS = ("paths", "paths-ignore")
...
def triggers(workflow: dict) -> str:
    ...
    for event, config in on.items():
        ...
        text = f"{event}[{','.join(branches)}]" if branches else str(event)
        paths = [f"{key}: {','.join(config[key])}" for key in TRIGGER_PATH_KEYS if config.get(key)]
        parts.append(text + (f"({'; '.join(paths)})" if paths else ""))
    return ", ".join(parts)


def path_filters(job: dict, condition) -> list[str]:
    """The dorny/paths-filter path lists that a step condition reads."""
    steps = {s["id"]: s for s in job.get("steps", []) if s.get("id")}
    lines = []
    for step_id, name in sorted(set(OUTPUT_REF.findall(str(condition or "")))):
        source = steps.get(step_id, {})
        if not str(source.get("uses", "")).startswith("dorny/paths-filter"):
            continue
        filters = yaml.safe_load(source.get("with", {}).get("filters", "")) or {}
        lines.append(f"  filter: {step_id}.{name} = {', '.join(flatten(filters.get(name)))}")
    return lines
