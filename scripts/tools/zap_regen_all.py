...
def getGlobalTemplatesTargets():
    ...
    targets.extend([[str(filepath)] for filepath in Path('./src/controller/test_data_model').rglob('*.zap')])
    return targets

def getSpecificTemplatesTargets():
    targets = []
    targets.append(['src/controller/test_data_model/clusters.zap', '-t', 'examples/chip-tool/templates/templates.json'])
    targets.append(['src/controller/test_data_model/clusters.zap', '-t', 'src/controller/python/templates/templates.json'])
