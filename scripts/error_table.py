    ...
    descriptors = {
        "src/lib/core/CHIPError.h":
            ErrorDescriptor(
                section="SDK Core",
                code_range=0x000,
                macro_regex=r"^#define\s+(?P<name>[_A-Z0-9]+)\s+CHIP(_CORE)?_ERROR[(](?P<code>(0x[a-fA-F0-9]+)|\d+)[)]"),
        ...
        "src/protocols/interaction_model/StatusCodeList.h":
            ErrorDescriptor(
                ...
                code_range=0x500,
                macro_regex=(
                    r"^CHIP_IM_STATUS_CODE[(][A-Za-z0-9_]+\s*,\s*(?P<name>[A-Z0-9_]+)\s*,\s*(?P<code>(0x[a-fA-F0-9]+)|\d+)[)]")),
    }
