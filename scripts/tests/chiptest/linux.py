...
class IsolatedNetworkNamespace:
    ...
    COMMANDS_SETUP = [
        ...
        "ip link add {app_link_name}-{index} type veth peer name eth-app-sw-{index}",
        "ip link add {tool_link_name}-{index} type veth peer name eth-tool-sw-{index}",
        ...
        "ip link set eth-app-sw-{index} master br1-{index}",
        "ip link set eth-tool-sw-{index} master br1-{index}",
    ...
    COMMANDS_APP_LINK_UP = [
        ...
        "ip link set dev eth-app-sw-{index} up",
        # Force IPv6 to use ULAs that we control.
        "ip netns exec app-{index} ip -6 addr flush {app_link_name}-{index}",
        "ip netns exec app-{index} ip -6 a add fd00:0:1:1::3/64 dev {app_link_name}-{index}",
    ...
    COMMANDS_TOOL_LINK_UP = [
        ...
        "ip link set dev eth-tool-sw-{index} up",
        # Force IPv6 to use ULAs that we control.
    ...
    COMMANDS_TERMINATE = [
        ...
        "ip link delete eth-tool-sw-{index}",
        "ip link delete eth-app-sw-{index}",
