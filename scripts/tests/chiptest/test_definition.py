...
class TestDefinition:
    ...
    def _RunImpl(self, target: TestTarget, runner: Runner, apps_register: AppsRegister, config: TestJobConfig,
        ...
        try:
            ...
            if config.test_runtime == TestRunTime.MATTER_REPL_PYTHON:
                assert 'matter-repl-yaml-tester' in config.subproc_info_repo, \
                    "Matter REPL YAML tester should have been set for selected test runtime"
                python_cmd = config.subproc_info_repo['matter-repl-yaml-tester'].with_args(
                    '--setup-code', setupCode, '--yaml-path', self.run_name, "--pics-file", str(config.pics_file))

                if config.dry_run:
                    log.info(shlex.join(python_cmd.to_cmd()))
                else:
                    runner.RunSubprocess(python_cmd, name='MATTER_REPL_YAML_TESTER',
                                         dependencies=[apps_register], timeout_seconds=config.test_timeout_seconds)
