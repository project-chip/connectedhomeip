...
class ApiHandler:
    ...
    def fetchCompletedBuilds(self, branch='master'):
        ...
        for build in self.api.get_project_build_summary(self.user, self.project, branch=branch, status_filter='successful'):
            logging.debug('Fetched summary: %r' % build)
            yield CompletedBuild(job_name=build['workflows']['job_name'], build_num=build['build_num'])
    ...
    def getArtifacts(self, build_num):
        ...
        for artifact in self.api.get_artifacts(self.user, self.project, build_num, self.vcs_type):
...
parser.add_argument('--download-dir', type=str, default='.', help='Where to download the artifacts')
