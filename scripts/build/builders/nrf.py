...
class NrfConnectBuilder(Builder):
        ...
        if not os.path.exists(self.output_dir):
            if not self._runner.dry_run:
                ...
                if not os.access(nrfconnect_sdk, os.W_OK):
                    raise Exception(
                        "Directory {} not writable. NRFConnect builds require updates to this directory.".format(nrfconnect_sdk))
    ...
    def build_outputs(self):
        yield BuilderOutput(
            os.path.join(self.output_dir, 'nrfconnect', 'zephyr', 'zephyr.elf'), '{}.elf'.format(self.app.AppNamePrefix()))
        if self.options.enable_link_map_file:
            yield BuilderOutput(
                os.path.join(self.output_dir, 'nrfconnect', 'zephyr', 'zephyr.map'), '{}.map'.format(self.app.AppNamePrefix()))
