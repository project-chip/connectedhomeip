...
def HostTargets():
    ...
    builder.AppendVariant(
        name="no-interactive",
        validator=AcceptNameWithSubstrings(["-chip-tool"]),
    ...
    yield target_native.Extend(
        "address-resolve-tool-platform-mdns-ipv6only",
        app=HostApp.ADDRESS_RESOLVE,
        use_platform_mdns=True,
        enable_ipv4=False,
    ).GlobBlacklist("Reduce default build variants")
    ...
    for board in [HostBoard.NATIVE, HostBoard.FAKE]:
        yield test_target.Extend(
            board.BoardName() + "-tests", board=board, app=HostApp.TESTS
        )
        yield test_target.Extend(
            board.BoardName() + "-tests-clang",
            board=board,
            app=HostApp.TESTS,
            use_clang=True,
        )
...
def Esp32Targets():
    ...
    yield esp32_target.Extend(
        "m5stack-all-clusters-rpc",
        board=Esp32Board.M5Stack,
        app=Esp32App.ALL_CLUSTERS,
        enable_rpcs=True,
    )
    yield esp32_target.Extend(
        "m5stack-all-clusters-rpc-ipv6only",
        board=Esp32Board.M5Stack,
        app=Esp32App.ALL_CLUSTERS,
        enable_rpcs=True,
        enable_ipv4=False,
    ...
    yield esp32_target.Extend(
        "m5stack-all-clusters-minimal-ipv6only",
        board=Esp32Board.M5Stack,
        app=Esp32App.ALL_CLUSTERS_MINIMAL,
        enable_ipv4=False,
    )
    yield esp32_target.Extend(
        "m5stack-all-clusters-minimal-rpc",
        board=Esp32Board.M5Stack,
        app=Esp32App.ALL_CLUSTERS_MINIMAL,
        enable_rpcs=True,
    )
    yield esp32_target.Extend(
        "m5stack-all-clusters-minimal-rpc-ipv6only",
        board=Esp32Board.M5Stack,
        app=Esp32App.ALL_CLUSTERS_MINIMAL,
        enable_rpcs=True,
        enable_ipv4=False,
...
def Efr32Targets():
    ...
    builder.AppendVariant(
        name="rpc",
        validator=AcceptNameWithSubstrings(["-light", "-lock"]),
        enable_rpcs=True,
    )
...
def NrfTargets():
    ...
    yield target.Extend(
        "nrf52840dongle-all-clusters",
        board=NrfBoard.NRF52840DONGLE,
        app=NrfApp.ALL_CLUSTERS,
    )
    yield target.Extend(
        "nrf52840dongle-all-clusters-minimal",
        board=NrfBoard.NRF52840DONGLE,
...
def AndroidTargets():
    ...
    yield target.Extend(
        "androidstudio-arm64-chip-tool",
        board=AndroidBoard.AndroidStudio_ARM64,
        app=AndroidApp.CHIP_TOOL,
    )
    yield target.Extend(
        "androidstudio-x86-chip-tool",
        board=AndroidBoard.AndroidStudio_X86,
        app=AndroidApp.CHIP_TOOL,
    )
    yield target.Extend(
        "androidstudio-x64-chip-tool",
    ...
    yield target.Extend(
        ...
        app=AndroidApp.CHIP_TV_CASTING_APP,
    )
    yield target.Extend(
        "arm-chip-tv-casting-app",
...
def Cyw30739Targets():
    yield Target(
        ...
        Cyw30739Builder,
        board=Cyw30739Board.CYW930739M2EVB_01,
        app=Cyw30739App.LIGHT,
    )
    yield Target(
        "cyw30739-cyw930739m2evb_01-lock",
        Cyw30739Builder,
        board=Cyw30739Board.CYW930739M2EVB_01,
        app=Cyw30739App.LOCK,
    )
    yield Target(
        "cyw30739-cyw930739m2evb_01-ota-requestor",
        Cyw30739Builder,
        board=Cyw30739Board.CYW930739M2EVB_01,
        app=Cyw30739App.OTA_REQUESTOR,
    ).GlobBlacklist("Running out of XIP flash space")
    yield Target(
