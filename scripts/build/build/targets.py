...
def BuildHostFakeTarget():
    ...
    target.AppendModifier(
        "mbedtls", crypto_library=HostCryptoLibrary.MBEDTLS
    ).ExceptIfRe("-boringssl")
    target.AppendModifier(
        "boringssl", crypto_library=HostCryptoLibrary.BORINGSSL
    ).ExceptIfRe("-mbedtls")
    ...
    target.AppendModifier(
        "libfuzzer", fuzzing_type=HostFuzzingType.LIB_FUZZER
    ).OnlyIfRe("-clang").ExceptIfRe("-ossfuzz")
    target.AppendModifier("ossfuzz", fuzzing_type=HostFuzzingType.OSS_FUZZ).OnlyIfRe(
        "-clang"
    ).ExceptIfRe("-libfuzzer")
    target.AppendModifier(
        "pw-fuzztest", fuzzing_type=HostFuzzingType.PW_FUZZTEST
    ).OnlyIfRe("-clang").ExceptIfRe("-(libfuzzer|ossfuzz|asan)")
...
def BuildHostTarget():
    ...
    if cross_compile:
        board_parts.append(
            TargetPart("arm64", board=HostBoard.ARM64).OnlyIfRe("-(clang|nodeps)")
        )
    ...
    app_parts = [
        TargetPart("rpc-console", app=HostApp.RPC_CONSOLE).OnlyIfRe(
            f"{native_board_name}-"
        ),
        ...
        TargetPart(
            "light-data-model-no-unique-id", app=HostApp.LIGHT_DATA_MODEL_NO_UNIQUE_ID
        ),
    ...
    if HostBoard.NATIVE.PlatformName() == "darwin":
        app_parts.append(
            TargetPart("darwin-framework-tool", app=HostApp.CHIP_TOOL_DARWIN)
        )
    ...
    target.AppendModifier("same-event-loop", separate_event_loop=False).OnlyIfRe(
        "-(chip-tool|darwin-framework-tool)"
    )
    target.AppendModifier("no-interactive", interactive_mode=False).OnlyIfRe(
        "-chip-tool"
    )
    ...
    target.AppendModifier(
        "evse-test-event", enable_test_event_triggers=["EVSE"]
    ).OnlyIfRe("-energy-management")
    target.AppendModifier("enable-dnssd-tests", enable_dnssd_tests=True).OnlyIfRe(
        "-tests"
    )
    target.AppendModifier("disable-dnssd-tests", enable_dnssd_tests=False).OnlyIfRe(
        "-tests"
    )
    target.AppendModifier(
        "chip-casting-simplified", chip_casting_simplified=True
    ).OnlyIfRe("-tv-casting-app")
    target.AppendModifier("data-model-check", data_model_interface="check").ExceptIfRe(
        "-data-model-(enabled|disabled)"
    )
    target.AppendModifier(
        "data-model-disabled", data_model_interface="disabled"
    ).ExceptIfRe("-data-model-(check|enabled)")
    target.AppendModifier(
        "data-model-enabled", data_model_interface="enabled"
    ).ExceptIfRe("-data-model-(check|disabled)")
    target.AppendModifier(
        "check-failure-die", chip_data_model_check_die_on_failure=True
    ).OnlyIfRe("-data-model-check")
...
def BuildEfr32Target():
    ...
    target.AppendFixedTargets(
        [
            ...
            TargetPart(
                "brd4338a",
                board=Efr32Board.BRD4338A,
                enable_wifi=True,
                enable_917_soc=True,
            ),
            TargetPart(
                "brd2605a",
                board=Efr32Board.BRD2605A,
                enable_wifi=True,
                enable_917_soc=True,
            ),
        ]
    ...
    target.AppendModifier(
        "additional-data-advertising", enable_additional_data_advertising=True
    )
...
def BuildNrfTarget():
    ...
    target.AppendModifier("rpc", enable_rpcs=True)
    target.AppendModifier(
        "data-model-disabled", use_data_model_interface=False
    ).ExceptIfRe("-data-model-enabled")
    target.AppendModifier(
        "data-model-enabled", use_data_model_interface=True
    ).ExceptIfRe("-data-model-disabled")
...
def BuildAndroidTarget():
    ...
    target.AppendFixedTargets(
        [
            ...
            TargetPart(
                "androidstudio-arm", board=AndroidBoard.AndroidStudio_ARM
            ).OnlyIfRe("chip-tool"),
            TargetPart(
                "androidstudio-arm64", board=AndroidBoard.AndroidStudio_ARM64
            ).OnlyIfRe("chip-tool"),
            TargetPart(
                "androidstudio-x86", board=AndroidBoard.AndroidStudio_X86
            ).OnlyIfRe("chip-tool"),
            TargetPart(
                "androidstudio-x64", board=AndroidBoard.AndroidStudio_X64
            ).OnlyIfRe("chip-tool"),
        ]
    )
    ...
    target.AppendFixedTargets(
        [
            ...
            TargetPart(
                "kotlin-matter-controller", app=AndroidApp.KOTLIN_MATTER_CONTROLLER
            ),
...
def BuildNxpTarget():
    ...
    target.AppendFixedTargets(
        [
            TargetPart("lighting", app=NxpApp.LIGHTING).OnlyIfRe(
                "(k32w0|k32w1|mcxw71)"
            ),
            TargetPart("contact-sensor", app=NxpApp.CONTACT).OnlyIfRe(
                "(k32w0|k32w1|mcxw71)"
            ),
            TargetPart("lock-app", app=NxpApp.LOCK_APP).OnlyIfRe("(k32w1|mcxw71)"),
            TargetPart("all-clusters", app=NxpApp.ALLCLUSTERS).OnlyIfRe(
                "rt1060|rt1170|rw61x"
            ),
            TargetPart("laundry-washer", app=NxpApp.LAUNDRYWASHER).OnlyIfRe(
                "rt1060|rt1170|rw61x"
            ),
            TargetPart("thermostat", app=NxpApp.THERMOSTAT).OnlyIfRe(
                "rt1060|rt1170|rw61x"
            ),
        ]
    )
    ...
    target.AppendModifier(name="smu2", smu2=True).OnlyIfRe(
        "(k32w1|mcxw71)-freertos-lighting"
    )
    target.AppendModifier(name="dac-conversion", convert_dac_pk=True).OnlyIfRe(
        "factory"
    ).ExceptIfRe("(k32w0|rw61x)")
    target.AppendModifier(name="rotating-id", enable_rotating_id=True).ExceptIfRe(
        "rw61x"
    )
    ...
    target.AppendModifier(name="matter-shell", enable_shell=True).ExceptIfRe(
        "k32w0|k32w1"
    )
    target.AppendModifier(
        "data-model-disabled", data_model_interface="disabled"
    ...
    target.AppendModifier(
        name="factory-build", enable_factory_data_build=True
    ).OnlyIfRe("rt1060|rt1170|rw61x")
    target.AppendModifier(name="frdm", board_variant=NxpBoardVariant.FRDM).OnlyIfRe(
        "rw61x"
    )
    target.AppendModifier(name="cmake", build_system=NxpBuildSystem.CMAKE).OnlyIfRe(
        "rw61x"
    )
    ...
    target.AppendModifier(name="iwx12", iwx12_transceiver=True).OnlyIfRe(
        "rt1060|rt1170"
    )
    target.AppendModifier(name="log-all", log_level=NxpLogLevel.ALL).ExceptIfRe(
        "-log-(progress|error|none)"
    )
    target.AppendModifier(
        name="log-progress", log_level=NxpLogLevel.PROGRESS
    ).ExceptIfRe("-log-(all|error|none)")
    target.AppendModifier(name="log-error", log_level=NxpLogLevel.ERROR).ExceptIfRe(
        "-log-(progress|all|none)"
    )
    target.AppendModifier(name="log-none", log_level=NxpLogLevel.NONE).ExceptIfRe(
        "-log-(progress|error|all)"
    )
...
def BuildCC13x4Target():
    ...
    target.AppendFixedTargets(
        [
            ...
            TargetPart(
                "pump-controller", app=TIApp.PUMP_CONTROLLER, openthread_ftd=False
            ),
        ]
    )
...
def BuildCyw30739Target():
    ...
    target.AppendFixedTargets(
        [
            TargetPart(
                "cyw30739b2_p5_evk_01", board=Cyw30739Board.CYW30739B2_P5_EVK_01
            ),
            TargetPart(
                "cyw30739b2_p5_evk_02", board=Cyw30739Board.CYW30739B2_P5_EVK_02
            ),
            TargetPart(
                "cyw30739b2_p5_evk_03", board=Cyw30739Board.CYW30739B2_P5_EVK_03
            ),
...
def BuildBouffalolabTarget():
    ...
    target.AppendFixedTargets(
        [
            TargetPart(
                "BL602DK",
                board=BouffalolabBoard.BL602_IoT_Matter_V1,
                module_type="BL602",
            ),
            TargetPart(
                "BL704LDK", board=BouffalolabBoard.BL704LDK, module_type="BL704L"
            ),
            TargetPart(
                "BL706DK", board=BouffalolabBoard.BL706DK, module_type="BL706C-22"
            ),
            TargetPart(
                "BL602-NIGHT-LIGHT",
                board=BouffalolabBoard.BL602_NIGHT_LIGHT,
                module_type="BL602",
            ),
            TargetPart(
                "BL706-NIGHT-LIGHT",
                board=BouffalolabBoard.BL706_NIGHT_LIGHT,
            ...
            TargetPart(
                "XT-ZB6-DevKit",
                board=BouffalolabBoard.XT_ZB6_DevKit,
                module_type="BL706C-22",
            ),
        ]
    )
...
def BuildTelinkTarget():
    ...
    target.AppendFixedTargets(
        [
            ...
            TargetPart(
                "temperature-measurement", app=TelinkApp.TEMPERATURE_MEASUREMENT
            ),
...
def BuildOpenIotSdkTargets():
    ...
    # Modifiers
    target.AppendModifier("mbedtls", crypto=OpenIotSdkCryptoBackend.MBEDTLS).ExceptIfRe(
        "-(psa)"
    )
    target.AppendModifier("psa", crypto=OpenIotSdkCryptoBackend.PSA).ExceptIfRe(
        "-(mbedtls)"
    )
