...
class IDFExecutor:
    ...
    def __init__(self):
        ...
        self.chip_root = os.path.realpath(
            os.path.join(script_path, '..', '..'))

        logging.info("CHIP Root directory: %s" % self.chip_root)

        self.run_cmd = os.path.join(
            self.chip_root, "scripts", "run_in_build_env.sh")
        logging.info("Executing via: %s" % self.run_cmd)

    def execute(self, command):
        os.chdir(self.chip_root)
        subprocess.call(
            [self.run_cmd, 'source "%s/export.sh"; cd %s; idf.py %s' % (idf_path, ROOT, command)])


def main():
    """Main task if executed standalone."""
    parser = argparse.ArgumentParser(
        description='Build all-clusters-app example')
    parser.add_argument(
        '--log-level',
    ...
    if args.clear_config:
        ...
        if args.clear_config == "m5stack" or args.clear_config == "devkit":
            ...
            old_default_sdkconfig = os.path.join(
                ROOT, 'sdkconfig.defaults.old')
