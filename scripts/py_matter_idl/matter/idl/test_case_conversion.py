...
class TestSnakeCase(unittest.TestCase):
    def test_snake_case(self):
        ...
        for s in inputs:
            converted = case_convert.to_snake_case(s)
            self.assertEqual(converted, expected, "they are not equal")

    def test_constant_case(self):
        expected = "FOO_BAR_BAZ"
        for s in inputs:
            converted = case_convert.to_constant_case(s)
            self.assertEqual(converted, expected, "they are not equal")

    def test_spinal_case(self):
        expected = "foo-bar-baz"
        for s in inputs:
            converted = case_convert.to_spinal_case(s)
            self.assertEqual(converted, expected, "they are not equal")

    def test_pascal_case(self):
        expected = "FooBarBaz"
        for s in inputs:
            converted = case_convert.to_pascal_case(s)
            self.assertEqual(converted, expected, "they are not equal")

    def test_camel_case(self):
        expected = "fooBarBaz"
        for s in inputs:
            converted = case_convert.to_camel_case(s)
            self.assertEqual(converted, expected, "they are not equal")
