...
The Matter IDL is evolving towards representing the full Matter Specification, including details defined in the spec (such as optional commands/events, constraints, conformance, and metadata). This will eventually include features that legacy ZAP/ZCL XML configurations might not support, but that are defined in newer tools like Alchemy and the Data Model (DM) XMLs.

There are currently several sources for parsing and generating IDL files, including:
- Files generated by ZAP (which omit some advanced specification details).
- ZAP XML files (which have similar limitations).
- Data Model XML files.

The goal is for the Matter IDL format to be generic enough to accommodate and unifiedly represent all these sources. This ensures we can trust the serialized `.matter` files as a source of truth for the codebase.
