...
def dump_ids_from_prebuilt_dirs():
    ...
    for dir in PrebuiltDataModelDirectory:
        ...
        def cluster_support_str(c):
            return "P" if c.is_provisional else "Y"
        ...
        def device_type_support_str(d):
            ...
            if provisional:
                print(f"Found provisional mandatory clusters {provisional} in dt {d.name} for revision {dir.dirname}")
                return "P"
            return "Y"
    ...
    def print_out_ids(filename, id_to_name: IdToNameDict, supported: dict[PrebuiltDataModelDirectory, IdToSupportedDict]):
    ...
    print_out_ids(get_cluster_documentation_file_path(), all_clusters, version_clusters)
    print_out_ids(get_device_types_documentation_file_path(), all_device_types, version_device_types)
