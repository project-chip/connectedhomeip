                         ...
                         int32_t index)
{
    ...
    case 0x003F: // Group Key Management Cluster
    {
        ...
        case 0x0000: // groups
        {
            ...
            copyListMember(write ? dest : (uint8_t *) &entry->GroupKeySetID, write ? (uint8_t *) &entry->GroupKeySetID : src, write,
                           &entryOffset, sizeof(entry->GroupKeySetID)); // INT16U
            copyListMember(write ? dest : (uint8_t *) &entry->Endpoints, write ? (uint8_t *) &entry->Endpoints : src, write,
                           &entryOffset, sizeof(entry->Endpoints));                                                  // INT16U
            ByteSpan GroupNameSpanStorage(Uint8::from_const_char(entry->GroupName.data()), entry->GroupName.size()); // CHAR_STRING
            ByteSpan * GroupNameSpan = &GroupNameSpanStorage;
            if (CHIP_NO_ERROR !=
                (write ? WriteByteSpan(dest + entryOffset, 18, GroupNameSpan) : ReadByteSpan(src + entryOffset, 18, GroupNameSpan)))
            {
                ChipLogError(Zcl, "Index %" PRId32 " is invalid. Not enough remaining space", index);
                return 0;
            }
