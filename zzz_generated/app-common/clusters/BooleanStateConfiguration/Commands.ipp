...
{
    constexpr uint32_t kRequiredFieldsBitmap = 0 | (1 << to_underlying(Fields::kAlarmsToSuppress));
    static_assert(to_underlying(Fields::kAlarmsToSuppress) < sizeof(kRequiredFieldsBitmap) * 8);
...
CHIP_ERROR DecodableType::Decode(TLV::TLVReader & reader)
{
    constexpr uint32_t kRequiredFieldsBitmap = 0 | (1 << to_underlying(Fields::kAlarmsToEnableDisable));
    static_assert(to_underlying(Fields::kAlarmsToEnableDisable) < sizeof(kRequiredFieldsBitmap) * 8);
    detail::StructDecodeIterator __iterator(reader, kRequiredFieldsBitmap);
    while (true)
    {
        uint8_t __context_tag = 0;
