/*
 *
 *    Copyright (c) 2022 Project CHIP Authors
 *
 *    Licensed under the Apache License, Version 2.0 (the "License");
 *    you may not use this file except in compliance with the License.
 *    You may obtain a copy of the License at
 *
 *        http://www.apache.org/licenses/LICENSE-2.0
 *
 *    Unless required by applicable law or agreed to in writing, software
 *    distributed under the License is distributed on an "AS IS" BASIS,
 *    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *    See the License for the specific language governing permissions and
 *    limitations under the License.
 */

// THIS FILE IS GENERATED BY ZAP
// This file is generated from clusters-Structs.h.zapt

#pragma once

#include <app/data-model/DecodableList.h>
#include <app/data-model/List.h>
#include <app/data-model/Nullable.h>
#include <lib/core/DataModelTypes.h>
#include <lib/core/Optional.h>
#include <lib/core/TLV.h>
#include <lib/support/BitMask.h>

#include <clusters/shared/Structs.h>

#include <cstdint>

namespace chip {
namespace app {
namespace Clusters {
namespace AmbientContextSensing {
namespace Structs {
namespace AmbientContextTypeStruct {
enum class Fields : uint8_t
{
    kAmbientContextSensed = 0,
    kDetectionStartTime   = 1,
};

struct Type
{
public:
    DataModel::List<const Globals::Structs::SemanticTagStruct::Type> ambientContextSensed;
    Optional<uint32_t> detectionStartTime;

    static constexpr bool kIsFabricScoped = false;

    CHIP_ERROR Encode(TLV::TLVWriter & aWriter, TLV::Tag aTag) const;
};

struct DecodableType
{
public:
    DataModel::DecodableList<Globals::Structs::SemanticTagStruct::DecodableType> ambientContextSensed;
    Optional<uint32_t> detectionStartTime;

    CHIP_ERROR Decode(TLV::TLVReader & reader);

    static constexpr bool kIsFabricScoped = false;
};

} // namespace AmbientContextTypeStruct
namespace HoldTimeLimitsStruct {
enum class Fields : uint8_t
{
    kHoldTimeMin     = 0,
    kHoldTimeMax     = 1,
    kHoldTimeDefault = 2,
};

struct Type
{
public:
    uint16_t holdTimeMin     = static_cast<uint16_t>(0);
    uint16_t holdTimeMax     = static_cast<uint16_t>(0);
    uint16_t holdTimeDefault = static_cast<uint16_t>(0);

    CHIP_ERROR Decode(TLV::TLVReader & reader);

    static constexpr bool kIsFabricScoped = false;

    CHIP_ERROR Encode(TLV::TLVWriter & aWriter, TLV::Tag aTag) const;
};

using DecodableType = Type;

} // namespace HoldTimeLimitsStruct
namespace PredictedActivityStruct {
enum class Fields : uint8_t
{
    kStartTimestamp     = 0,
    kEndTimestamp       = 1,
    kAmbientContextType = 2,
    kCrowdDetected      = 3,
    kCrowdCount         = 4,
    kConfidence         = 5,
};

struct Type
{
public:
    uint32_t startTimestamp = static_cast<uint32_t>(0);
    uint32_t endTimestamp   = static_cast<uint32_t>(0);
    Optional<DataModel::List<const Globals::Structs::SemanticTagStruct::Type>> ambientContextType;
    Optional<bool> crowdDetected;
    Optional<uint8_t> crowdCount;
    chip::Percent confidence = static_cast<chip::Percent>(0);

    static constexpr bool kIsFabricScoped = false;

    CHIP_ERROR Encode(TLV::TLVWriter & aWriter, TLV::Tag aTag) const;
};

struct DecodableType
{
public:
    uint32_t startTimestamp = static_cast<uint32_t>(0);
    uint32_t endTimestamp   = static_cast<uint32_t>(0);
    Optional<DataModel::DecodableList<Globals::Structs::SemanticTagStruct::DecodableType>> ambientContextType;
    Optional<bool> crowdDetected;
    Optional<uint8_t> crowdCount;
    chip::Percent confidence = static_cast<chip::Percent>(0);

    CHIP_ERROR Decode(TLV::TLVReader & reader);

    static constexpr bool kIsFabricScoped = false;
};

} // namespace PredictedActivityStruct
} // namespace Structs
} // namespace AmbientContextSensing
} // namespace Clusters
} // namespace app
} // namespace chip
