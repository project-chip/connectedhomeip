...
static void DbgConsole_SerialManagerTxCallback(void * callbackParam, serial_manager_callback_message_t * message,
                                               serial_manager_status_t status)
{
    ...
    if (kStatus_SerialManager_Success == status)
    {
        if (ioState->writeRingBuffer.ringTail != ioState->writeRingBuffer.ringHead)
        {
            ...
            (void) SerialManager_WriteNonBlocking(((serial_write_handle_t) &ioState->serialWriteHandleBuffer[0]),
...
static void DbgConsole_SerialManagerRxCallback(void * callbackParam, serial_manager_callback_message_t * message,
                                               serial_manager_status_t status)
{
    if ((NULL == callbackParam) || (NULL == message))
    {
...
#if defined(DEBUG_CONSOLE_TRANSFER_NON_BLOCKING) && (DEBUG_CONSOLE_SYNCHRONIZATION_MODE == DEBUG_CONSOLE_SYNCHRONIZATION_BM) &&    \
    defined(OSA_USED)
    return kStatus_Fail;
#else
...
#if defined(DEBUG_CONSOLE_TRANSFER_NON_BLOCKING)
    status =
        (status_t) SerialManager_ReadNonBlocking(((serial_read_handle_t) &s_debugConsoleState.serialReadHandleBuffer[0]), ch, 1);
#else
    status = (status_t) SerialManager_ReadBlocking(((serial_read_handle_t) &s_debugConsoleState.serialReadHandleBuffer[0]), ch, 1);
#endif
...
int DbgConsole_SendData(uint8_t * ch, size_t size)
{
    ...
    {
        txBusy         = 1;
        sendDataLength = (s_debugConsoleState.writeRingBuffer.ringHead + s_debugConsoleState.writeRingBuffer.ringBufferSize -
    ...
    if (txBusy == 0)
    {
        if (s_debugConsoleState.writeRingBuffer.ringHead > s_debugConsoleState.writeRingBuffer.ringTail)
        {
            sendDataLength = s_debugConsoleState.writeRingBuffer.ringHead - s_debugConsoleState.writeRingBuffer.ringTail;
        }
        else
        {
            sendDataLength = s_debugConsoleState.writeRingBuffer.ringBufferSize - s_debugConsoleState.writeRingBuffer.ringTail;
        }

        status = (status_t) SerialManager_WriteNonBlocking(
            ((serial_write_handle_t) &s_debugConsoleState.serialWriteHandleBuffer[0]),
            &s_debugConsoleState.writeRingBuffer.ringBuffer[s_debugConsoleState.writeRingBuffer.ringTail], sendDataLength);
    }
    EnableGlobalIRQ(regPrimask);
#else
    status =
        (status_t) SerialManager_WriteBlocking(((serial_write_handle_t) &s_debugConsoleState.serialWriteHandleBuffer[0]), ch, size);
#endif
...
#if (defined(DEBUG_CONSOLE_RX_ENABLE) && (DEBUG_CONSOLE_RX_ENABLE > 0U))
    {
        status = (status_t) SerialManager_OpenReadHandle(s_debugConsoleState.serialHandle,
...
{
    {
        ...
        {
            (void) SerialManager_CloseWriteHandle(((serial_write_handle_t) &s_debugConsoleState.serialWriteHandleBuffer[0]));
        }
    }
#if (defined(DEBUG_CONSOLE_RX_ENABLE) && (DEBUG_CONSOLE_RX_ENABLE > 0U))
    {
...
int DbgConsole_Printf(const char * formatString, ...)
{
    ...
    K32WWriteBlocking((uint8_t *) printBuf, (size_t) logLength);
...
status_t DbgConsole_TryGetchar(char * ch)
{
    ...
    if (kStatus_SerialManager_Success ==
        SerialManager_TryRead(((serial_read_handle_t) &s_debugConsoleState.serialReadHandleBuffer[0]), (uint8_t *) ch, 1, &length))
    {
        if (length != 0U)
        {
#if DEBUG_CONSOLE_ENABLE_ECHO_FUNCTION
