...
//
inline std::pair<std::string::iterator, std::string::iterator> mutable_part(std::string & str, detail::uri_part part)
{
...
inline string_view to_string_view(const std::string & uri, detail::uri_part part)
{
    if (!part.empty())
    {
...
void uri::initialize(optional<string_type> scheme, optional<string_type> user_info, optional<string_type> host,
                     optional<string_type> port, optional<string_type> path, optional<string_type> query,
...
{
    initialize(builder.scheme_, builder.user_info_, builder.host_, builder.port_, builder.path_, builder.query_, builder.fragment_);
}
...
uri::const_iterator uri::begin() const noexcept
{
    return uri_view_.begin();
}

uri::const_iterator uri::end() const noexcept
{
    return uri_view_.end();
}
...
{
    return has_scheme() ? to_string_view(uri_, *uri_parts_.scheme) : string_view{};
}
...
{
    return has_user_info() ? to_string_view(uri_, *uri_parts_.hier_part.user_info) : string_view{};
}
...
{
    return has_host() ? to_string_view(uri_, *uri_parts_.hier_part.host) : string_view{};
}
...
{
    return has_port() ? to_string_view(uri_, *uri_parts_.hier_part.port) : string_view{};
}
...
{
    return has_path() ? to_string_view(uri_, *uri_parts_.hier_part.path) : string_view{};
}
...
uri::query_iterator::query_iterator(optional<detail::uri_part> query) : query_(query), kvp_{}
{
...
uri::query_iterator::query_iterator(const query_iterator & other) : query_(other.query_), kvp_(other.kvp_) {}
...
void uri::query_iterator::advance_to_next_kvp() noexcept
{
    ...
    auto sep_it = std::find_if(first, last, [](char c) -> bool { return c == '&' || c == ';'; });
...
void uri::query_iterator::assign_kvp() noexcept
{
    ...
    auto sep_it = std::find_if(first, last, [](char c) -> bool { return c == '&' || c == ';'; });
    auto eq_it  = std::find_if(first, sep_it, [](char c) -> bool { return c == '='; });
...
{
    return has_fragment() ? to_string_view(uri_, *uri_parts_.fragment) : string_view{};
}
...
uri uri::normalize(uri_comparison_level level) const
{
    ...
    if (uri_comparison_level::syntax_based == level)
    {
        ...
        normalized.erase(detail::decode_encoded_unreserved_chars(std::begin(normalized), std::end(normalized)),
                         std::end(normalized));
        ...
        {
            uri::string_type path = detail::normalize_path_segments(to_string_view(normalized, *parts.hier_part.path));
...
uri uri::make_relative(const uri & other) const
{
    ...
    if ((!has_scheme() || !other.has_scheme()) || !detail::equal(scheme(), other.scheme()))
    {
        return other;
    }

    if ((!has_authority() || !other.has_authority()) || !detail::equal(authority(), other.authority()))
    {
        return other;
    }
    ...
    auto other_path = detail::normalize_path(other.path(), uri_comparison_level::syntax_based);
    ...
    network::uri result;
    result.initialize(optional<string_type>(), optional<string_type>(), optional<string_type>(), optional<string_type>(),
...
uri uri::resolve(const uri & base) const
{
    ...
    network::uri result;
    result.initialize(make_arg(base.scheme()), std::move(user_info), std::move(host), std::move(port), std::move(path),
