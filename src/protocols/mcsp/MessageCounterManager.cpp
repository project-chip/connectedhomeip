...
CHIP_ERROR MessageCounterManager::VerifyPacket(Transport::PeerConnectionState * state, const PacketHeader & packetHeader)
{
...
CHIP_ERROR MessageCounterManager::SendMsgCounterSyncResp(Messaging::ExchangeContext * exchangeContext, SecureSessionHandle session, std::array<uint8_t, kChallengeSize> challenge)
{
    ...
    // Allocate new buffer.
    msgBuf = System::PacketBufferHandle::New(kSyncRespMsgSize);
    VerifyOrExit(!msgBuf.IsNull(), err = CHIP_ERROR_NO_MEMORY);
                                                     ...
                                                     const PacketHeader & packetHeader, System::PacketBufferHandle msgBuf)
{
    ...
    VerifyOrExit(state->GetSessionMessageCounter().GetPeerMessageCounter().VerifyChallenge(syncCounter, challenge),
