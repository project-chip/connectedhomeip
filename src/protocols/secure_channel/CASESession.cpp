...
// Step 4
    ...
    ReturnErrorOnFailure(tlvWriter.Put(CASETLVTag::kSessionID, GetLocalKeyId(), true));
    // Step 2/3
    ...
    ReturnErrorOnFailure(ComputeIPK(GetLocalKeyId(), mIPK, sizeof(mIPK)));
...
CHIP_ERROR CASESession::SendSigmaR2()
{
    ...
    err = ComputeIPK(GetLocalKeyId(), mIPK, sizeof(mIPK));
    SuccessOrExit(err);
    ...
    // now construct sigmaR2
    {
        ...
        SuccessOrExit(err = tlvWriter.Put(CASETLVTag::kSessionID, GetLocalKeyId(), true));
...
CHIP_ERROR CASESession::HandleSigmaR2(System::PacketBufferHandle & msg)
{
    ...
    err = ComputeIPK(GetPeerKeyId(), mRemoteIPK, sizeof(mRemoteIPK));
    SuccessOrExit(err);
