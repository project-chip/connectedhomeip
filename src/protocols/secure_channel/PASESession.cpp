...
void PASESession::HandleErrorMsg(const System::PacketBufferHandle & msg)
{
    ...
    VerifyOrExit(buf != nullptr, ChipLogError(SecureChannel, "Null error msg received during pairing"));
    VerifyOrExit(buflen == sizeof(Spake2pErrorMsg),
                 ChipLogError(SecureChannel, "Error msg with incorrect length received during pairing"));

    pMsg = reinterpret_cast<Spake2pErrorMsg *>(msg->Start());
    ChipLogError(SecureChannel, "Received error (%d) during pairing process", pMsg->error);
