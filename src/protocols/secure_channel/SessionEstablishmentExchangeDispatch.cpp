...
CHIP_ERROR SessionEstablishmentExchangeDispatch::OnMessageReceived(uint32_t messageId, const PayloadHeader & payloadHeader,
                                                                   const Transport::PeerAddress & peerAddress,
                                                                   Messaging::MessageFlags msgFlags,
                                                                   ReliableMessageContext * reliableMessageContext)
{
    mPeerAddress = peerAddress;
    return ExchangeMessageDispatch::OnMessageReceived(messageId, payloadHeader, peerAddress, msgFlags, reliableMessageContext);
}
