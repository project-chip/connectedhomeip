 ...
 * TransferSession API to proceed with the BDX transfer. For e.g., If this class received a ReceiveInit message and passed it to the
 * HandleTransferSessionOutput method, the subclass would call the AcceptTransfer on the TransferSession object to negotiate params
 * and prepare a ReceiveAccept message.
 *
...
public:
     ...
     * If the OutputEvent was handled successfully by the subclass that means the TransferSession API call to handle the output
     * event was successful and the TransferSession has the next message prepared. In that case, the "status" is set to
     * CHIP_NO_ERROR and the AsyncResponder should call ProcessOutputEvents to get and process the next output events from the
     * TransferSession to proceed with the BDX transfer. Otherwise if the output event indicates the end of BDX transfer (like EOF
     * receieved or transfer timeout), it will clean up and destroy itself and the subclass. If the "status" is set to an error
     * code, the AsyncResponder should abort the transfer and clean up once the status report is sent over the exchange.
     *
