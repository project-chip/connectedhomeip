...
bool ChannelContext::MatchNodeId(NodeId nodeId)
{
    ...
    case ChannelState::kReady: {
        ...
        if (state == nullptr)
            return false;
        return nodeId == state->GetPeerCache()->GetPeer().GetNodeId();
    }
    default:
        return false;
    }
...
bool ChannelContext::MatchTransport(Transport::Type transport)
{
    ...
    case ChannelState::kReady: {
        ...
        if (state == nullptr)
            return false;
        return transport == state->GetPeerCache()->GetPeerAddress().GetTransportType();
    }
    default:
        return false;
    }
...
bool ChannelContext::MatchesSession(SecureSessionHandle session, SecureSessionMgr * ssm)
{
    ...
    case ChannelState::kPreparing: {
        ...
        case PrepareState::kCasePairing: {
            ...
            return (state->GetPeerCache()->GetPeer().GetNodeId() == GetPrepareVars().mBuilder.GetPeerNodeId() &&
