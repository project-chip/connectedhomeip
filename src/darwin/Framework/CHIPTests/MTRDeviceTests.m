...
- (void)test045b_MTRDeviceInvokeGroupExceedingMaxPathsPerInvoke
{
    ...
    // The endpoint 2 command is deliberately first so the first chunk spans two endpoints, exercising a
    // batched message that mixes endpoints rather than one that happens to target a single endpoint.
    __auto_type * device = [MTRDevice deviceWithNodeID:kDeviceId1 deviceController:sController];
    dispatch_queue_t queue = dispatch_get_main_queue();
...
- (void)test045c_MTRDeviceInvokeGroupIsActuallyBatchedOnTheWire
{
    // test045b asserts the responses a chunked group produces, but a serial implementation returns the
    // same array, so it cannot tell the two apart.  This test pins the batching itself by relying on a
    // behavior that only exists when the commands share one InvokeRequestMessage: the server rejects a
    // message containing duplicate concrete command paths with INVALID_ACTION (see
    // CommandHandlerImpl::ValidateInvokeRequestMessageAndBuildRegistry, which fails to register a
    // non-unique path). Sent one-per-message, as the pre-change implementation did, both On commands
    // would simply succeed. So an INVALID_ACTION here is proof a single batched message went out.
    __auto_type * device = [MTRDevice deviceWithNodeID:kDeviceId1 deviceController:sController];
    dispatch_queue_t queue = dispatch_get_main_queue();
    ...
    __auto_type * onPath = [MTRCommandPath commandPathWithEndpointID:@(1)
                                                           clusterID:@(MTRClusterIDTypeOnOffID)
                                                           commandID:@(MTRCommandIDTypeClusterOnOffCommandOnID)];
    __auto_type * duplicateGroup = @[
        [[MTRCommandWithRequiredResponse alloc] initWithPath:onPath commandFields:nil requiredResponse:nil],
        [[MTRCommandWithRequiredResponse alloc] initWithPath:onPath commandFields:nil requiredResponse:nil],
    ];

    XCTestExpectation * duplicateInvokeDone = [self expectationWithDescription:@"Invoke of a group with duplicate paths done"];
    [device invokeCommands:@[ duplicateGroup ]
                     queue:queue
                completion:^(NSArray<NSDictionary<NSString *, id> *> * _Nullable values, NSError * _Nullable error) {
                    XCTAssertNil(error);
                    XCTAssertNotNil(values);
                    ...
                    // The whole batched message is rejected, so every command in it reports the failure.
                    XCTAssertEqual(values.count, duplicateGroup.count);
                    for (NSDictionary<NSString *, id> * value in values) {
                        XCTAssertEqualObjects(value[MTRCommandPathKey], onPath);
                        NSError * commandError = value[MTRErrorKey];
                        XCTAssertNotNil(commandError);
                        XCTAssertEqualObjects(commandError.domain, MTRInteractionErrorDomain);
                        XCTAssertEqual(commandError.code, MTRInteractionErrorCodeInvalidAction);
                    }

                    [duplicateInvokeDone fulfill];
                }];

    [self waitForExpectations:@[ duplicateInvokeDone ] timeout:(2 * kTimeoutInSeconds)];
}
