...
- (void)subscribeWithMinInterval:(uint16_t)minInterval maxInterval:(uint16_t)maxInterval
{
    ...
    [_deviceController getBaseDevice:_deviceID
                               queue:_queue
                   completionHandler:^(MTRBaseDevice * _Nullable baseDevice, NSError * _Nullable error) {
                       dispatch_async(DeviceLayer::PlatformMgrImpl().GetWorkQueue(), ^{
                           DeviceProxy * device = [baseDevice internalDevice];
                           if (!device) {
                               dispatch_async(self->_queue, ^{
                                   [self _handleSubscriptionError:[MTRError errorForCHIPErrorCode:CHIP_ERROR_INCORRECT_STATE]];
                               });
                               return;
                           }
                           ...
                           readClient
                               = std::make_unique<ReadClient>(InteractionModelEngine::GetInstance(), device->GetExchangeManager(),
                       ...
                       params:(MTRSubscribeParams *)params
{
    ...
    [self subscribeWithMinInterval:minInterval maxInterval:maxInterval];
}
