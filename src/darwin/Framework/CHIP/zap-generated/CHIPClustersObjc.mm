...
@property (readonly) Controller::OtaSoftwareUpdateRequestorCluster cppCluster;
@end
            ...
            responseHandler:(ResponseHandler)responseHandler
{
    ...
    __block CHIP_ERROR err;
    dispatch_sync([self chipWorkQueue], ^{
        err = self.cppCluster.AnnounceOtaProvider(onSuccess->Cancel(), onFailure->Cancel(),
            chip::ByteSpan((const uint8_t *) serverLocation.bytes, serverLocation.length), vendorId, announcementReason,
            chip::ByteSpan((const uint8_t *) metadataForNode.bytes, metadataForNode.length));
    });
...
- (void)readAttributeDefaultOtaProviderWithResponseHandler:(ResponseHandler)responseHandler
{
    ...
    __block CHIP_ERROR err;
    dispatch_sync([self chipWorkQueue], ^{
        err = self.cppCluster.ReadAttributeDefaultOtaProvider(onSuccess->Cancel(), onFailure->Cancel());
    });
...
- (void)writeAttributeDefaultOtaProviderWithValue:(NSData *)value responseHandler:(ResponseHandler)responseHandler
{
    ...
    __block CHIP_ERROR err;
    dispatch_sync([self chipWorkQueue], ^{
        err = self.cppCluster.WriteAttributeDefaultOtaProvider(
...
- (void)readAttributeUpdatePossibleWithResponseHandler:(ResponseHandler)responseHandler
{
    ...
    __block CHIP_ERROR err;
    dispatch_sync([self chipWorkQueue], ^{
        err = self.cppCluster.ReadAttributeUpdatePossible(onSuccess->Cancel(), onFailure->Cancel());
    });
