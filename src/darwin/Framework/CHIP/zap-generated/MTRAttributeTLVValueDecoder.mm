...
id MTRDecodeAttributeValue(const ConcreteAttributePath & aPath, TLV::TLVReader & aReader, CHIP_ERROR * aError)
{
            ...
            NSString * _Nullable value;
              if (cppValue.IsNull()) {
                value = nil;
                        ...
                        value = [[NSString alloc] initWithBytes:cppValue.Value().data() length:cppValue.Value().size() encoding:NSUTF8StringEncoding];
  }
            return value;
        }
    ...
    case Clusters::GroupKeyManagement::Id: {
        ...
        if (entry_0.groupName.HasValue()) {
                  newElement_0.groupName = [[NSString alloc] initWithBytes:entry_0.groupName.Value().data() length:entry_0.groupName.Value().size() encoding:NSUTF8StringEncoding];
  } else {
    ...
    case Clusters::Channel::Id: {
        ...
        if (entry_0.callSign.HasValue()) {
                  newElement_0.callSign = [[NSString alloc] initWithBytes:entry_0.callSign.Value().data() length:entry_0.callSign.Value().size() encoding:NSUTF8StringEncoding];
  } else {
          newElement_0.callSign = nil;
        }
        if (entry_0.affiliateCallSign.HasValue()) {
                  newElement_0.affiliateCallSign = [[NSString alloc] initWithBytes:entry_0.affiliateCallSign.Value().data() length:entry_0.affiliateCallSign.Value().size() encoding:NSUTF8StringEncoding];
  } else {
        ...
        case Attributes::Lineup::Id: {
            ...
            *aError = DataModel::Decode(aReader, cppValue);
            ...
            MTRChannelClusterLineupInfoStruct * _Nullable value;
              ...
              value.operatorName = [[NSString alloc] initWithBytes:cppValue.Value().operatorName.data() length:cppValue.Value().operatorName.size() encoding:NSUTF8StringEncoding];
        if (cppValue.Value().lineupName.HasValue()) {
                  value.lineupName = [[NSString alloc] initWithBytes:cppValue.Value().lineupName.Value().data() length:cppValue.Value().lineupName.Value().size() encoding:NSUTF8StringEncoding];
  } else {
          value.lineupName = nil;
        }
        if (cppValue.Value().postalCode.HasValue()) {
                  value.postalCode = [[NSString alloc] initWithBytes:cppValue.Value().postalCode.Value().data() length:cppValue.Value().postalCode.Value().size() encoding:NSUTF8StringEncoding];
  } else {
            ...
            *aError = DataModel::Decode(aReader, cppValue);
            ...
            MTRChannelClusterChannelInfoStruct * _Nullable value;
              ...
              if (cppValue.Value().name.HasValue()) {
                  value.name = [[NSString alloc] initWithBytes:cppValue.Value().name.Value().data() length:cppValue.Value().name.Value().size() encoding:NSUTF8StringEncoding];
  } else {
          value.name = nil;
        }
        if (cppValue.Value().callSign.HasValue()) {
                  value.callSign = [[NSString alloc] initWithBytes:cppValue.Value().callSign.Value().data() length:cppValue.Value().callSign.Value().size() encoding:NSUTF8StringEncoding];
  } else {
          value.callSign = nil;
        }
        if (cppValue.Value().affiliateCallSign.HasValue()) {
                  value.affiliateCallSign = [[NSString alloc] initWithBytes:cppValue.Value().affiliateCallSign.Value().data() length:cppValue.Value().affiliateCallSign.Value().size() encoding:NSUTF8StringEncoding];
  } else {
        ...
        case Attributes::InputList::Id: {
            ...
            *aError = DataModel::Decode(aReader, cppValue);
            ...
            NSArray * _Nonnull value;
              ...
              newElement_0.name = [[NSString alloc] initWithBytes:entry_0.name.data() length:entry_0.name.size() encoding:NSUTF8StringEncoding];
              newElement_0.descriptionString = [[NSString alloc] initWithBytes:entry_0.description.data() length:entry_0.description.size() encoding:NSUTF8StringEncoding];
      [array_0 addObject:newElement_0];
                }
                CHIP_ERROR err = iter_0.GetStatus();
                if (err != CHIP_NO_ERROR) {
        ...
        case Attributes::CurrentApp::Id: {
          ...
          value.application = [MTRApplicationLauncherClusterApplicationStruct new];
              value.application.catalogVendorID = [NSNumber numberWithUnsignedShort:cppValue.Value().application.catalogVendorID];
                    value.application.applicationID = [[NSString alloc] initWithBytes:cppValue.Value().application.applicationID.data() length:cppValue.Value().application.applicationID.size() encoding:NSUTF8StringEncoding];
            ...
            *aError = DataModel::Decode(aReader, cppValue);
            ...
            MTRApplicationBasicClusterApplicationStruct * _Nonnull value;
              ...
              value.catalogVendorID = [NSNumber numberWithUnsignedShort:cppValue.catalogVendorID];
                    value.applicationID = [[NSString alloc] initWithBytes:cppValue.applicationID.data() length:cppValue.applicationID.size() encoding:NSUTF8StringEncoding];
            return value;
        }
        case Attributes::Status::Id: {
            using TypeInfo = Attributes::Status::TypeInfo;
        ...
        case Attributes::NullableStruct::Id: {
            ...
            *aError = DataModel::Decode(aReader, cppValue);
            ...
            MTRUnitTestingClusterSimpleStruct * _Nullable value;
              ...
              value.e = [[NSString alloc] initWithBytes:cppValue.Value().e.data() length:cppValue.Value().e.size() encoding:NSUTF8StringEncoding];
