...
function asObjectiveCNumberType(label, type, asLowerCased)
{
  ...
  {
    const options
        = { hash : {} } return zclHelper.asUnderlyingZclType.call(this, type, options)
              .then(zclType => {
                ...
                case 'bool':
                  return 'Bool'
                  case 'uint8_t': return 'UnsignedChar'
                  case 'uint16_t': return 'UnsignedShort'
                  case 'uint32_t': return 'UnsignedLong'
                  case 'uint64_t': return 'UnsignedLongLong'
                  case 'int8_t': return 'Char'
                  case 'int16_t': return 'Short'
                  case 'int32_t': return 'Long'
                  case 'int64_t': return 'LongLong' default: error
                      = label + ': Unhandled underlying type ' + zclType + ' for original type ' + type
                  throw error
                }
              })
              .then(
                  typeName => asLowerCased ? typeName[0].toLowerCase() + typeName.substring(1) : typeName,
              )
  }
