...
class KeyValueStore {
  public:
    ...
    *   CHIP_ERROR_BUFFER_TOO_SMALL with the number of bytes read. The remainder of the
    *   value can be read by calling get with an offset.
    *
    * @param[in]     key               The name of the key in which to get.
    * @param[in,out] buffer            A buffer to read the value into.
    * @param[in]     buffer_size       The size of the buffer in bytes.
    * @param[in,out] read_bytes_size   The number of bytes which were
    *                                  copied into the buffer. Optionally can
    *                                  provide nullptr if not needed.
    * @param[in]     offset_bytes      The offset byte index in which to start
    *                                  the read.
    *
    ...
    *                                     too large
    */
    virtual CHIP_ERROR Get(const char* key, void* value, size_t value_size,
                           size_t* read_bytes_size = nullptr, size_t offset = 0) const = 0;
    ...
    *
    * @param[in]      key       The name of the key in which to get.
    * @param[in,out]  pointer   Pointer to a trivially copyable object.
    *
    * @return CHIP_NO_ERROR the entry was successfully read
    *         CHIP_ERROR_PERSISTED_STORAGE_VALUE_NOT_FOUND the key is not
    *                                                      present in the KVS
    ...
    *
    * @param[in]      key       The name of the key in which to delete.
    *
