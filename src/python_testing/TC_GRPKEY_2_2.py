...
class TC_GRPKEY_2_2(MatterBaseTest):
    ...
    async def test_TC_GRPKEY_2_2(self):
        ...
        asserts.assert_equal(
            ...
            group_key_set_id,
            f"KeySetRead response should contain the requested GroupKeySetID {group_key_set_id:04x}"
        )
        asserts.assert_equal(
        ...
        current_group_key_set_ids = await self.read_single_attribute_check_success(
            cluster=Clusters.GroupKeyManagement,
            attribute=Clusters.GroupKeyManagement.Attributes.GroupKeyMap,
            endpoint=endpoint,
        )
        for i in range(len(current_group_key_set_ids)+1, max_grp_key):
            group_key_set = Clusters.GroupKeyManagement.Structs.GroupKeySetStruct(
                groupKeySetID=i,
                groupKeySecurityPolicy=self.GROUP_KEY_SECURITY_POLICY,
                epochKey0=self.EPOCH_KEY_0,
