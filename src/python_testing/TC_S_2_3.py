...
from matter.testing.spec_parsing import XmlAttribute, build_xml_clusters, dm_from_spec_version
from mobly import asserts
from TC_GC_common import is_groupcast_on_root_node
from TC_S_common import _ScenableAttribute, _scene_value_field, _value_bounds, _value_other_than
...
class TC_S_2_3(MatterBaseTest):
    ...
    def steps_TC_S_2_3(self) -> list[TestStep]:
        return [
            ...
            TestStep("0a", "TH sends KeySetWrite command in the GroupKeyManagement cluster to DUT with GroupKeySetID 0x01a1.",
                     "DUT sends a SUCCESS response."),
            TestStep("0b", "TH sends KeySetWrite command in the GroupKeyManagement cluster to DUT with GroupKeySetID 0x01a2.",
                     "DUT sends a SUCCESS response."),
            TestStep("0c", "If the Groupcast cluster is enabled on the RootNode endpoint, skip this step. Otherwise, TH writes the GroupKeyMap attribute on the GroupKeyManagement cluster binding G1 to GroupKeySetID 0x01a1 and G2 to GroupKeySetID 0x01a2.",
                     "DUT sends a SUCCESS response."),
            TestStep("1a", "If the Groupcast cluster is enabled on the RootNode endpoint, TH sends Groupcast JoinGroup command for G1 with KeySetID 0x01a1. Otherwise, TH sends a Groups AddGroup command with GroupID G1.",
                     "DUT sends SUCCESS (JoinGroup) or AddGroupResponse with Status 0x00 (SUCCESS) and GroupID G1."),
            TestStep("1b", "If the Groupcast cluster is enabled on the RootNode endpoint, TH sends Groupcast JoinGroup command for G2 with KeySetID 0x01a2. Otherwise, TH sends a Groups AddGroup command with GroupID G2.",
                     "DUT sends SUCCESS (JoinGroup) or AddGroupResponse with Status 0x00 (SUCCESS) and GroupID G2."),
            TestStep("1c", "TH installs access control on the device to allow group commands to take actions.",
                     "DUT sends a WriteResponseMessage with Status 0x00 (SUCCESS)."),
            TestStep("1d", "TH sends a RemoveAllScenes command to DUT with GroupID G1.",
                     "DUT sends a RemoveAllScenesResponse with Status 0x00 (SUCCESS) and GroupID G1."),
            TestStep("1e", "TH sends a RemoveAllScenes command to DUT with GroupID G2.",
                     "DUT sends a RemoveAllScenesResponse with Status 0x00 (SUCCESS) and GroupID G2."),
            TestStep("1f", "TH sends a GetSceneMembership command to DUT with GroupID G1.",
            ...
            TestStep(2, "TH sends an AddScene command to DUT with GroupID G1, SceneID 0x01, TransitionTime 1000 and a set of extension fields producing AC1.",
                     "DUT sends an AddSceneResponse with Status 0x00 (SUCCESS), GroupID G1 and SceneID 0x01."),
            TestStep(3, "TH sends a ViewScene command to DUT with GroupID G1 and SceneID 0x01.",
            ...
            TestStep(4, "TH sends a GetSceneMembership command to DUT with GroupID G1.",
            ...
            TestStep("5a", "TH configures AC2 on DUT for all implemented application clusters supporting scenes.",
                     "DUT is configured with AC2 which is different from AC1."),
            TestStep("5b", "TH sends a RecallScene command to group G1 with GroupID G1 and SceneID 0x01.",
                     "There is no status response since this was a groupcast. DUT returns to AC1."),
            TestStep("6a", "TH sends an AddScene command to group G1 with GroupID G1, SceneID 0x03, TransitionTime 1000 and extension fields appropriate to AC1.",
                     "There is no status response since this was a groupcast."),
            TestStep("6b", "TH sends a ViewScene command to DUT with GroupID G1 and SceneID 0x03.",
            ...
            TestStep("6c", "TH sends a ViewScene command to group G1 with GroupID G1 and SceneID 0x03.",
                     "There is no status response since this was a groupcast."),
            TestStep("6d", "TH sends a RemoveScene command to group G1 with GroupID G1 and SceneID 0x03.",
                     "There is no status response since this was a groupcast."),
            TestStep("6e", "TH sends a ViewScene command to DUT with GroupID G1 and SceneID 0x03.",
            ...
            TestStep("6f", "TH sends a StoreScene command to group G1 with GroupID G1 and SceneID 0x03.",
                     "There is no status response since this was a groupcast."),
            TestStep("6g", "TH sends a ViewScene command to DUT with GroupID G1 and SceneID 0x03.",
                     "DUT sends a ViewSceneResponse with Status 0x00 (SUCCESS), GroupID G1 and SceneID 0x03."),
            TestStep("6h", "TH sends a RemoveScene command to group G1 with GroupID G1 and SceneID 0x03.",
                     "There is no status response since this was a groupcast."),
            TestStep("6i", "TH sends a ViewScene command to DUT with GroupID G1 and SceneID 0x03.",
                     "DUT sends a ViewSceneResponse with Status 0x8b (NOT_FOUND), GroupID G1, SceneID 0x03, and TransitionTime/extension fields omitted."),
            TestStep("7a", "TH sends a CopyScene command to DUT with mode 0x00, group from G1, scene from 0x01, group to G1 and scene to 0x02.",
                     "DUT sends a CopySceneResponse with Status 0x00 (SUCCESS), group from G1 and scene from 0x01."),
            TestStep("7b", "TH sends a CopyScene command to DUT with mode 0x00, group from GI (absent from group table), scene from 0x01, group to GI and scene to 0x02.",
                     "DUT sends a CopySceneResponse with Status 0x85 (INVALID_COMMAND), group from GI and scene from 0x01."),
            TestStep("7c", "TH sends a CopyScene command to DUT with mode 0x00, group from G1, scene from 0xfe, group to G1 and scene to 0x02.",
                     "DUT sends a CopySceneResponse with Status 0x8b (NOT_FOUND), group from G1 and scene from 0xfe."),
            TestStep("7d", "TH sends a GetSceneMembership command to DUT with GroupID G1.",
            ...
            TestStep("8a", "TH sends a CopyScene command to DUT with mode 0x01, group from G1, scene from 0x01, group to G2 and scene to 0x02.",
                     "DUT sends a CopySceneResponse with Status 0x00 (SUCCESS), group from G1 and scene from 0x01."),
            TestStep("8b", "TH sends a GetSceneMembership command to DUT with GroupID G2.",
            ...
            TestStep("8c", "TH sends a CopyScene command to group G2 with mode 0x00, group from G2, scene from 0x02, group to G2 and scene to 0x03.",
                     "There is no status response since this was a groupcast."),
            TestStep("8d", "TH sends a GetSceneMembership command to DUT with GroupID G2.",
            ...
            TestStep("9a", "TH sends a RemoveAllScenes command to group G1 with GroupID G1.",
                     "There is no status response since this was a groupcast."),
            TestStep("9b", "TH sends a GetSceneMembership command to DUT with GroupID G1.",
            ...
            TestStep("10a", "TH sends a RemoveAllScenes command to group G2 with GroupID G2.",
                     "There is no status response since this was a groupcast."),
            TestStep("10b", "TH sends a GetSceneMembership command to DUT with GroupID G2.",
            ...
            TestStep(11, "TH sends a KeySetRemove command to the GroupKeyManagement cluster with GroupKeySetID 0x01a1.",
                     "DUT sends a SUCCESS response."),
            TestStep(12, "TH sends a KeySetRemove command to the GroupKeyManagement cluster with GroupKeySetID 0x01a2.",
                     "DUT sends a SUCCESS response."),
        ]

    async def _select_scenable_attribute(self) -> _ScenableAttribute:
        ...
        with the data model 'scene' (S) quality, so that the AC1/AC2 application state
        is built from a real scenable attribute rather than being hardcoded. Boolean
        attributes (e.g. OnOff) are preferred because two distinct valid values are
        ...
        endpoint_data = attributes.get(self._scene_endpoint)
        asserts.assert_true(endpoint_data is not None, f"Scene endpoint {self._scene_endpoint} not found on the DUT")
        ...
        asserts.assert_true(fallback is not None,
                            f"No scenable attribute found on endpoint {self._scene_endpoint}")
        return fallback

    async def _read_scenable_value(self) -> int:
        """Reads the current value of the selected scenable attribute as an int."""
        value = await self.read_single_attribute_check_success(
            endpoint=self._scene_endpoint, cluster=self._scenable.cluster, attribute=self._scenable.attribute)
        return int(value)

    def _build_extension_fields(self, value: int) -> list:
        """Builds the ExtensionFieldSetStructs setting the selected scenable attribute to value."""
        pair = Clusters.ScenesManagement.Structs.AttributeValuePairStruct(
            attributeID=self._scenable.attribute.attribute_id)
        setattr(pair, self._scenable.value_field, int(value))
        return [Clusters.ScenesManagement.Structs.ExtensionFieldSetStruct(
            clusterID=self._scenable.cluster.id, attributeValueList=[pair])]
    ...
    def _assert_extension_fields_match(self, extension_field_sets, expected_value: int, context: str) -> None:
        ...
        asserts.assert_true(extension_field_sets is not None and extension_field_sets is not NullValue,
        ...
        asserts.assert_equal(len(cluster_sets), 1,
                             f"{context}: expected exactly one extension field set for the scenable cluster")
        pairs = [p for p in cluster_sets[0].attributeValueList
                 if p.attributeID == self._scenable.attribute.attribute_id]
        asserts.assert_equal(len(pairs), 1, f"{context}: expected the scenable attribute in the extension field set")
        asserts.assert_equal(getattr(pairs[0], self._scenable.value_field), expected_value,
                             f"{context}: scenable attribute value should match AC1")
    ...
    async def _view_scene(self, group_id: int, scene_id: int):
        ...
        return await self.send_single_cmd(
            Clusters.ScenesManagement.Commands.ViewScene(groupID=group_id, sceneID=scene_id),
    ...
    async def test_TC_S_2_3(self):
        ...
        self._scenable = await self._select_scenable_attribute()
        scenable_lo, scenable_hi = _value_bounds(self._scenable)
        current_value = await self._read_scenable_value()
        ac1_value = _value_other_than(current_value, scenable_lo, scenable_hi)
        writable = self._scenable.xml_attribute.write_access != \
            Clusters.AccessControl.Enums.AccessControlEntryPrivilegeEnum.kUnknownEnumValue
        log.info("Using scenable attribute %s::%s (%s, writable=%s): AC1=%s, current(AC2)=%s",
                 self._scenable.cluster.__name__, self._scenable.attribute.__name__,
                 self._scenable.xml_attribute.datatype, writable, ac1_value, current_value)
        ...
        await self.send_single_cmd(endpoint=0, cmd=Clusters.GroupKeyManagement.Commands.KeySetWrite(
            groupKeySet=Clusters.GroupKeyManagement.Structs.GroupKeySetStruct(
                ...
                epochKey0=keyset1_key0, epochStartTime0=1110000,
                epochKey1=keyset1_key1, epochStartTime1=1110001,
        ...
        dev_ctrl.SetGroupKeySet(
            ...
            num_keys=2,
            epoch_key0=keyset1_key0, epoch_start_time0=1110000,
            epoch_key1=keyset1_key1, epoch_start_time1=1110001)
        ...
        await self.send_single_cmd(endpoint=0, cmd=Clusters.GroupKeyManagement.Commands.KeySetWrite(
            groupKeySet=Clusters.GroupKeyManagement.Structs.GroupKeySetStruct(
                ...
                groupKeySecurityPolicy=Clusters.GroupKeyManagement.Enums.GroupKeySecurityPolicyEnum.kTrustFirst,
                epochKey0=keyset2_key0, epochStartTime0=2220000,
                epochKey1=keyset2_key1, epochStartTime1=2220001,
                epochKey2=NullValue, epochStartTime2=NullValue)))
        dev_ctrl.SetGroupKeySet(
            keyset_id=keyset2,
            policy=Clusters.GroupKeyManagement.Enums.GroupKeySecurityPolicyEnum.kTrustFirst,
            num_keys=2,
            epoch_key0=keyset2_key0, epoch_start_time0=2220000,
            epoch_key1=keyset2_key1, epoch_start_time1=2220001)
        ...
        if groupcast_enabled:
            ...
            self.step("0c")
            result = await dev_ctrl.WriteAttribute(self.dut_node_id, [(0, Clusters.GroupKeyManagement.Attributes.GroupKeyMap([
        ...
        if groupcast_enabled:
            await self.send_single_cmd(endpoint=0, cmd=Clusters.Groupcast.Commands.JoinGroup(
                groupID=group_g2, endpoints=[self._scene_endpoint], keySetID=keyset2))
        else:
            resp = await self.send_single_cmd(
        ...
        result = await dev_ctrl.WriteAttribute(self.dut_node_id, [(0, Clusters.AccessControl.Attributes.Acl([acl_admin, acl_group]))])
        asserts.assert_equal(result[0].Status, Status.Success, "Step 1c: ACL write failed")

        # Step 1d: RemoveAllScenes for G1.
        self.step("1d")
        resp = await self.send_single_cmd(Clusters.ScenesManagement.Commands.RemoveAllScenes(groupID=group_g1), endpoint=self._scene_endpoint)
        ...
        self.step("1e")
        resp = await self.send_single_cmd(Clusters.ScenesManagement.Commands.RemoveAllScenes(groupID=group_g2), endpoint=self._scene_endpoint)
        ...
        self.step(2)
        resp = await self.send_single_cmd(Clusters.ScenesManagement.Commands.AddScene(
            groupID=group_g1, sceneID=0x01, transitionTime=transition_time, sceneName="",
            extensionFieldSetStructs=self._build_extension_fields(ac1_value)), endpoint=self._scene_endpoint)
        ...
        if writable:
            ...
            result = await dev_ctrl.WriteAttribute(
                self.dut_node_id, [(self._scene_endpoint, self._scenable.attribute(ac2_value))])
        ...
        ac2_reading = await self._read_scenable_value()
        asserts.assert_not_equal(ac2_reading, ac1_value,
                                 "Step 5a: DUT should be in AC2 (a state different from AC1) before recall")
        ...
        recalled = await self._read_scenable_value()
        ...
        self.step("6a")
        dev_ctrl.SendGroupCommand(group_g1, Clusters.ScenesManagement.Commands.AddScene(
            groupID=group_g1, sceneID=0x03, transitionTime=transition_time, sceneName="",
            extensionFieldSetStructs=self._build_extension_fields(ac1_value)))
        await asyncio.sleep(3)
        ...
        self.step("7a")
        resp = await self.send_single_cmd(Clusters.ScenesManagement.Commands.CopyScene(
            mode=0x00, groupIdentifierFrom=group_g1, sceneIdentifierFrom=0x01,
            groupIdentifierTo=group_g1, sceneIdentifierTo=0x02), endpoint=self._scene_endpoint)
        ...
        self.step("7b")
        resp = await self.send_single_cmd(Clusters.ScenesManagement.Commands.CopyScene(
            mode=0x00, groupIdentifierFrom=group_gi, sceneIdentifierFrom=0x01,
            groupIdentifierTo=group_gi, sceneIdentifierTo=0x02), endpoint=self._scene_endpoint)
        ...
        self.step("7c")
        resp = await self.send_single_cmd(Clusters.ScenesManagement.Commands.CopyScene(
            mode=0x00, groupIdentifierFrom=group_g1, sceneIdentifierFrom=0xFE,
            groupIdentifierTo=group_g1, sceneIdentifierTo=0x02), endpoint=self._scene_endpoint)
        ...
        self.step("8a")
        resp = await self.send_single_cmd(Clusters.ScenesManagement.Commands.CopyScene(
            mode=0x01, groupIdentifierFrom=group_g1, sceneIdentifierFrom=0x01,
            groupIdentifierTo=group_g2, sceneIdentifierTo=0x02), endpoint=self._scene_endpoint)
        ...
        self.step("8c")
        dev_ctrl.SendGroupCommand(group_g2, Clusters.ScenesManagement.Commands.CopyScene(
            mode=0x00, groupIdentifierFrom=group_g2, sceneIdentifierFrom=0x02,
            groupIdentifierTo=group_g2, sceneIdentifierTo=0x03))
        await asyncio.sleep(3)
        ...
        asserts.assert_equal(sorted(resp.sceneList or []), [0x01, 0x02, 0x03],
                             "Step 8d: SceneList should contain 0x01, 0x02 and 0x03")
