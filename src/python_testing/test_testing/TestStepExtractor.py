...
def test_skip_step():
    source = '''
    def test_foo(self):
        self.step(1, "Do something")
        self.skip_step(2)
        self.step(3, "Do something else")
    '''
    steps = extract_steps_from_source(source)
    assert len(steps) == 3
...
def test_dynamic_step_number_raises():
    ...
    try:
        ...
        assert "dynamic step number" in str(e).lower()
...
def test_sc_5_1_pattern():
    ...
    def test_TC_SC_5_1(self):
        ...
        if groupcast_enabled:
            ...
            self.step("3", "Bind GroupId")
            self.step("4", "RemoveAllGroups")
