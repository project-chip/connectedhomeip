...
class TC_EVSE_2_2(MatterBaseTest):
    ...
    async def write_user_max_charge(self, endpoint, user_max_charge):
        result = await self.default_controller.WriteAttribute(self.dut_node_id, [(endpoint, Clusters.EnergyEvse.Attributes.UserMaximumChargeCurrent(user_max_charge))])
        asserts.assert_equal(result[0].Status, Status.Success, "UserMaximumChargeCurrent write failed")
   
    async def send_enable_charge_command(self, endpoint: int = 0, charge_until: int = None, timedRequestTimeoutMs: int = 60000, min_charge: int = None, max_charge: int = None,  expected_status: Status=Status.Success):
        try:
            await self.send_single_cmd(cmd=Clusters.EnergyEvse.Commands.EnableCharging(chargingEnabledUntil=charge_until, minimumChargeCurrent=6000, maximumChargeCurrent=60000), endpoint = 1, timedRequestTimeoutMs=timedRequestTimeoutMs)
    ...
    async def test_TC_EVSE_2_2(self):
       ...
       # asserts.assert_equal(charge_until_time, epoch_time, f'ChargingEnabledUntil should be {epoch_time}, but is actually {charge_until_time}')         
        ...
        self.print_step('6d', f'TH reads from the DUT the MinimumChargeCurrent attribute and checks it is the commanded value (6000)')
        ...
        self.print_step('6e', f'TH reads from the DUT the MaximumChargeCurrent attribute and checks it is the MIN(command value (60000), CircuitCapacity)')
       ...
       # Part 7
        ...
        self.print_step('8d', f'TH reads from the DUT the MinimumChargeCurrent attribute and checks it is the commanded value (6000)')
        minimum_charge_value = await self.read_evse_attribute_exepct_success(endpoint=1, attribute='MinimumChargeCurrent')
        asserts.assert_equal(minimum_charge_value, 6000, f'MinimumChargeValue should be 6000, but is actually {minimum_charge_value}')                 
        
        self.print_step('8e', f'TH reads from the DUT the MaximumChargeCurrent attribute and checks it is the MIN(command value (60000), CircuitCapacity)')
