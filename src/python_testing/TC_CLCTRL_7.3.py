...
class TC_CLCTRL_7_3(MatterBaseTest):
    ...
    def desc_TC_CLCTRL_7_3(self) -> str:
        return "[TC-CLCTRL-7.3] GroupedMoveTo Command Field Sanity Check with Server as DUT"

    def steps_TC_CLCTRL_7_3(self) -> list[TestStep]:
        return [
            ...
            TestStep("4d", "Send GroupedMoveTo command with Position = MoveToFullyClosed", "Receive SUCCESS response from the DUT"),
            ...
            TestStep("4f", "Send GroupedMoveTo command with Latch = True", "Receive SUCCESS response from the DUT"),
            TestStep("4g", "Send GroupedMoveTo command with Latch = True and Position = MoveToFullyOpen",
            ...
            TestStep("5e", "Send GroupedMoveTo command with Latch = False", "Receive SUCCESS response from the DUT"),
            ...
            TestStep("5i", "Send GroupedMoveTo command with Position = MoveToFullyOpen", "Receive SUCCESS response from the DUT"),
            TestStep("5j", "If LatchControlModes Bit 0 = 0, skip step 5k and 5l"),
            TestStep("5k", "Send GroupedMoveTo command with Positon = MoveToFullyOpen and Latch = True",
                     "Receive SUCCESS response from the DUT"),
            ...
            TestStep("5p", "Send GroupedMoveTo command with Latch = False",
                     "Receive SUCCESS response from the DUT"),
            ...
            TestStep("5u", "Send GroupedMoveTo command with Position = MoveToFullyOpen and Latch = True",
            ...
            TestStep("6d", "Send GroupedMoveTo command with Speed = High", "Receive SUCCESS response from the DUT"),
            ...
            TestStep("6f", "Send GroupedMoveTo command with Latch = True", "Receive SUCCESS response from the DUT"),
            TestStep("6g", "Send GroupedMoveTo command with Latch = True and Speed = Low", "Receive SUCCESS response from the DUT"),
            ...
            TestStep("7a", "If the PS feature is supported, send GroupedMoveTo command with Position = 6",
                     "Receive CONSTRAINT_ERROR response from the DUT"),
            TestStep("7b", "If the SP feature is supported, send GroupedMoveTo command with Speed = 4",
                     "Receive CONSTRAINT_ERROR response from the DUT"),
            TestStep("7c", "If the SP feature is supported, send GroupedMoveTo command with Position = 6 and Speed = High",
                     "Receive CONSTRAINT_ERROR response from the DUT"),
            TestStep("7d", "If the SP feature is supported, send GroupedMoveTo command with Position = MoveToFullyClosed and Speed = 4",
                     "Receive CONSTRAINT_ERROR response from the DUT"),
            ...
            TestStep("8b", "Read the OverallCurrentState attribute",
                     "OverallCurrentState of the ClosureControl cluster is returned by the DUT; Latching field is saved as CurrentLatch"),
            TestStep("8c", "Send GroupedMoveTo command with Latch = CurrentLatch, when CurrentLatch is True and LatchControlModes Bit 0 = 1",
                     "Receive SUCCESS response from the DUT"),
            TestStep("8d", "Send GroupedMoveTo command with Latch = CurrentLatch, when CurrentLatch is True and LatchControlModes Bit 0 = 0",
                     "Receive INVALID_IN_STATE response from the DUT"),
            TestStep("8e", "Send GroupedMoveTo command with Latch = CurrentLatch, when CurrentLatch is False and LatchControlModes Bit 1 = 1",
                     "Receive SUCCESS response from the DUT"),
            TestStep("8f", "Send GroupedMoveTo command with Latch = CurrentLatch, when CurrentLatch is False and LatchControlModes Bit 1 = 0",
                     "Receive INVALID_IN_STATE response from the DUT"),
            ...
            TestStep("9e", "Send GroupedMoveTo command with Latch = False",
                     "Receive SUCCESS response from the DUT"),
            ...
            TestStep("9i", "Send GroupedMoveTo command with Position = CurrentPosition", "Receive SUCCESS response from the DUT"),
            ...
            TestStep("10e", "Send GroupedMoveTo command with Latch = False",
                     "Receive SUCCESS response from the DUT"),
            ...
            TestStep("10h", "Wait until a subscription report with OverallCurrentState.Latch is received",
                     "OverallCurrentState.Latch has to be False"),
            TestStep("10i", "Send GroupedMoveTo command with Speed = CurrentSpeed", "Receive SUCCESS response from the DUT"),
        ]

    def pics_TC_CLCTRL_7_3(self) -> list[str]:
        return [
            "CLCTRL.S", "CLCTRL.S.C03.Rsp"
        ]
    ...
    @async_test_body
    async def test_TC_CLCTRL_7_3(self):
        ...
        try:
            ...
            log.info(f"GroupedMoveTo command with no fields failed: {e}")
            asserts.assert_equal(e.status, Status.InvalidCommand,
                                 f"Expected INVALID_COMMAND status for GroupedMoveTo with no fields, but got: {e}")
        ...
        if not is_position_supported or is_latching_supported:
            ...
            if current_position == Clusters.ClosureControl.Enums.CurrentPositionEnum.kFullyClosed:
                ...
                try:
                    ...
                    asserts.assert_equal(e.status, Status.Success,
                                         f"Expected Success status for GroupedMoveTo with Position = MoveToFullyClosed, but got: {e}")
            ...
            try:
                ...
                asserts.assert_equal(e.status, Status.Success,
                                     f"Expected Success status for GroupedMoveTo with Latch = True and Position = MoveToFullyOpen, but got: {e}")
            self.step("4h")
            sub_handler.await_all_expected_report_matches(expected_matchers=[current_position_matcher(
        ...
        if not is_latching_supported or is_position_supported:
            ...
            try:
                ...
                asserts.assert_equal(e.status, Status.Success,
                                     f"Expected Success status for GroupedMoveTo with Position = MoveToFullyOpen, but got: {e}")
            ...
            if not latch_control_modes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteLatching:
                ...
                try:
                    ...
                    asserts.assert_equal(e.status, Status.Success,
                                         f"Expected Success status for GroupedMoveTo with Position = MoveToFullyOpen and Latch = True, but got: {e}")
            ...
            if not current_latch and current_latch is not NullValue:
                ...
                if not latch_control_modes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteUnlatching:
                    ...
                    try:
                        ...
                        asserts.assert_equal(e.status, Status.Success,
                                             f"Expected Success status for GroupedMoveTo with Latch = False, but got: {e}")
            ...
            if latch_control_modes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteLatching:
                ...
                try:
                    ...
                    log.info(f"Exception caught for GroupedMoveTo with Position = MoveToFullyOpen and Latch = True: {e}")
                    asserts.assert_equal(e.status, Status.InvalidInState,
                                         f"Expected INVALID_IN_STATE for GroupedMoveTo with Position = MoveToFullyOpen and Latch = True but got: {e}")
            sub_handler.reset()
        ...
        if not is_speed_supported or is_latching_supported:
            ...
            if current_speed == Clusters.Globals.Enums.ThreeLevelAutoEnum.kHigh:
                ...
                try:
                    ...
                    asserts.assert_equal(e.status, Status.Success,
                                         f"Expected Success status for GroupedMoveTo with Speed = High, but got: {e}")
            ...
            try:
                ...
                asserts.assert_equal(e.status, Status.Success,
                                     f"Expected Success status for GroupedMoveTo with Latch = True and Speed = Low, but got: {e}")
            self.step("6h")
            sub_handler.await_all_expected_report_matches(expected_matchers=[current_speed_matcher(
        ...
        if is_position_supported:
            ...
            try:
                ...
                log.info(f"Exception caught for GroupedMoveTo with Position = 6: {e}")
                asserts.assert_equal(e.status, Status.ConstraintError,
                                     f"Expected CONSTRAINT_ERROR for GroupedMoveTo with Position = 6 but got: {e}")
        else:
        ...
        if is_speed_supported:
            ...
            try:
                ...
                log.info(f"Exception caught for GroupedMoveTo with Speed = 4: {e}")
                asserts.assert_equal(e.status, Status.ConstraintError,
            ...
            try:
                ...
                log.info(f"Exception caught for GroupedMoveTo with Position = 6 and Speed = High: {e}")
                asserts.assert_equal(e.status, Status.ConstraintError,
            ...
            try:
                ...
                log.info(f"Exception caught for GroupedMoveTo with Position = MoveToFullyClosed and Speed = 4: {e}")
                asserts.assert_equal(e.status, Status.ConstraintError,
                                     f"Expected CONSTRAINT_ERROR for GroupedMoveTo with Position = MoveToFullyClosed and Speed = 4 but got: {e}")
        else:
        ...
        if is_latching_supported:
            ...
            if current_latch is True and latch_control_modes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteLatching:
                ...
                try:
                    ...
                    asserts.assert_equal(e.status, Status.Success,
                                         f"Expected Success status for GroupedMoveTo with Latch = CurrentLatch, but got: {e}")
            else:
            ...
            if current_latch is True and not (latch_control_modes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteLatching):
                ...
                try:
                    ...
                    asserts.assert_equal(e.status, Status.InvalidInState,
                                         f"Expected INVALID_IN_STATE for GroupedMoveTo with Latch = CurrentLatch but got: {e}")
            else:
        ...
        if is_position_supported:
            ...
            try:
                ...
                asserts.assert_equal(e.status, Status.Success,
                                     f"Expected Success status for GroupedMoveTo with Position = CurrentPosition, but got: {e}")
            sub_handler.reset()
        else:
        ...
        if is_speed_supported:
            ...
            try:
                ...
                asserts.assert_equal(e.status, Status.Success,
                                     f"Expected Success status for GroupedMoveTo with Speed = CurrentSpeed, but got: {e}")
            sub_handler.reset()
