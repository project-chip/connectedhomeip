...
class TC_CLCTRL_7_3(MatterBaseTest):
    ...
    def steps_TC_CLCTRL_7_3(self) -> list[TestStep]:
        return [
            ...
            TestStep("5k", "Send GroupedMoveTo command with Position = MoveToFullyOpen and Latch = True",
                     "Receive SUCCESS response from the DUT"),
            ...
            TestStep("8f", "Send GroupedMoveTo command with Latch = CurrentLatch, when CurrentLatch is False and LatchControlModes Bit 1 = 0",
                     "Receive INVALID_IN_STATE response from the DUT"),
            TestStep("9a", "Check PS feature support", "Skip steps 9b to 9i if the PS feature is not supported"),
            ...
            TestStep("10a", "Check SP feature support", "Skip steps 10b to 10i if the PS feature is not supported"),
    ...
    async def test_TC_CLCTRL_7_3(self):
        ...
        try:
            ...
            log.info("GroupedMoveTo command with no fields failed: %s", e)
        ...
        if not is_latching_supported or is_position_supported:
            ...
            if latch_control_modes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteLatching:
                ...
                try:
                    ...
                    log.info("Exception caught for GroupedMoveTo with Position = MoveToFullyOpen and Latch = True: %s", e)
        ...
        if is_position_supported:
            ...
            try:
                ...
                log.info("Exception caught for GroupedMoveTo with Position = 6: %s", e)
        ...
        if is_speed_supported:
            ...
            try:
                ...
                log.info("Exception caught for GroupedMoveTo with Speed = 4: %s", e)
                asserts.assert_equal(e.status, Status.ConstraintError,
            ...
            try:
                ...
                log.info("Exception caught for GroupedMoveTo with Position = 6 and Speed = High: %s", e)
                asserts.assert_equal(e.status, Status.ConstraintError,
            ...
            try:
                ...
                log.info("Exception caught for GroupedMoveTo with Position = MoveToFullyClosed and Speed = 4: %s", e)
                asserts.assert_equal(e.status, Status.ConstraintError,
        ...
        if is_latching_supported:
            ...
            if current_latch is True and not (latch_control_modes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteLatching):
                ...
                try:
                    ...
                    log.info("Exception caught for GroupedMoveTo with Latch = CurrentLatch: %s", e)
                    asserts.assert_equal(e.status, Status.InvalidInState,
