...
class TC_DD_3_26(MatterTestCommissioner):
    disable_wildcard_subscription = True

    def desc_TC_DD_3_26(self) -> str:
        return "[TC-DD-3.26] NFC-based commissioning - Re-Armed Fail-Safe Timer Behavior (Unpowered) [DUT as Commissionee]"

    def steps_TC_DD_3_26(self) -> list[TestStep]:
        return [
            ...
            TestStep(3, "After PASE session establishment, (re)arm the Fail-Safe with ExpiryLengthSeconds "
                     "field set to 2 seconds and send Read Request commands for a duration shorter than "
                     "2 seconds, repeated three times in a loop"),
            TestStep(4, "After the loop ends, send Read Request command(s) until the fail-safe timer "
                     "expires on the Commissionee and the next Read Request fails with a TIMEOUT error"),
    ...
    @async_test_body
    async def test_TC_DD_3_26(self):
        ...
        # Step 3: (Re)arm the Fail-Safe three times in a loop, reading repeatedly for a duration
        # shorter than ExpiryLengthSeconds after each arm, to verify that re-arming resets the
        # timer and the DUT never expires the Fail-Safe while it keeps being refreshed.
        self.step(3)

        expiry_length_seconds = 2
        safe_read_duration_seconds = 1.0  # well under expiry_length_seconds, with margin for jitter
        poll_interval_seconds = 0.3
        rearm_iterations = 3

        breadcrumb_attr = Clusters.GeneralCommissioning.Attributes.Breadcrumb
        fail_safe_armed_at = None

        for iteration in range(1, rearm_iterations + 1):
            arm_cmd = Clusters.GeneralCommissioning.Commands.ArmFailSafe(expiryLengthSeconds=expiry_length_seconds)
            ...
            asserts.assert_equal(
                ...
                f"ArmFailSafe (re-arm #{iteration}) failed with errorCode={resp.errorCode}"
            )
            fail_safe_armed_at = time.monotonic()
            log.info("Fail-Safe (re-)armed for %ss (iteration %d/%d)", expiry_length_seconds, iteration, rearm_iterations)

            while time.monotonic() - fail_safe_armed_at < safe_read_duration_seconds:
                ...
                try:
                    ...
                    asserts.fail(
                        ...
                        f"could have expired: {e}"
                    )
                await asyncio.sleep(poll_interval_seconds)

        # Step 4: The loop has ended without re-arming again. Keep polling from the last arm
        # onward; the Fail-Safe is now expected to actually expire and the DUT to tear down the
        # PASE session, at which point the next Read Request fails with a TIMEOUT error.
        self.step(4)
        ...
        asserts.assert_true(
            failure_seen,
            f"Read Request never failed within {max_wait_seconds}s of the last Fail-Safe arm "
            f"({expiry_length_seconds}s expiry)"
        )
        asserts.assert_true(
