...
class TC_DEM_2_10(MatterBaseTest, DEMTestBase):
    ...
    async def test_TC_DEM_2_10(self):
        ...
        async def accumulate_reports(wait_time):
            logger.info(f"Test will now wait {wait_time} seconds to accumulate reports")
        ...
        if not has_fa:
            ...
            logger.info(f"Received {count} Forecast updates in {wait} seconds")
        ...
        if not has_pa:
            ...
            logger.info(powerAdjustCapabilityStruct)
            ...
            logger.info(f"Received {count} PowerAdjustmentCapability updates in {wait} seconds")
