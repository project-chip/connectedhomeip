...
class TC_S_2_5(MatterBaseTest):
    ...
    def desc_TC_S_2_5(self) -> str:
        return "[TC-S-2.5] RemainingCapacity functionality with DUT as Server"

    def pics_TC_S_2_5(self):
        return ["S"]
    ...
    def steps_TC_S_2_5(self) -> list[TestStep]:
        return [
            ...
            TestStep(
                "0b",
                "If the Groupcast cluster is enabled on the RootNode endpoint, skip this step. Otherwise, TH writes the GroupKeyMap attribute on the GroupKeyManagement cluster binding G1 to GroupKeySetID 0x01a1.",
                "DUT sends a SUCCESS response.",
            ),
            TestStep(
                "1a",
                "If the Groupcast cluster is enabled on the RootNode endpoint, TH reads the Groupcast Membership attribute on the DUT. Otherwise, skip this step.",
                "DUT sends the Membership attribute; its contents decide whether Step 1b needs to leave any group.",
            ),
            TestStep(
                "1b",
                "If the Groupcast cluster is enabled on the RootNode endpoint and Membership was not empty in Step 1a, TH sends the Groupcast LeaveGroup command with GroupID 0. Otherwise, TH sends a Groups RemoveAllGroups command.",
                "DUT sends a LeaveGroupResponse (groupcast) or a SUCCESS response (legacy Groups).",
            ),
            TestStep(
                2,
                "If the Groupcast cluster is enabled on the RootNode endpoint, TH sends a Groupcast JoinGroup command for G1 with KeySetID 0x01a1. Otherwise, TH sends a Groups AddGroup command with GroupID G1.",
                "DUT sends SUCCESS (JoinGroup) or an AddGroupResponse with Status 0x00 (SUCCESS) and GroupID G1.",
            ),
            TestStep(
                3,
            ...
            TestStep(
                ...
                "DUT sends SceneTableSize, which is at least 16. MaxRemainingCapacity is (SceneTableSize - 1) / 2.",
            ),
            TestStep(
                "4b",
                "TH sends a subscription request action for FabricSceneInfo to the DUT with MinIntervalFloor 5 and MaxIntervalCeiling 100.",
                "The subscription is activated and the DUT reports FabricSceneInfo with RemainingCapacity equal to MaxRemainingCapacity for this fabric's entry.",
            ),
            TestStep(
                "5a",
                "TH sends an AddScene command to DUT with GroupID G1, SceneID 0x01, TransitionTime 20000 and no extension field sets.",
            ...
            TestStep(
                "6a",
                "If RemainingCapacity is greater than 0, TH sends a StoreScene command to DUT with GroupID G1 and SceneID 0x02. Otherwise, skip to Step 8a.",
                "DUT sends a StoreSceneResponse with Status 0x00 (SUCCESS), GroupID G1 and SceneID 0x02.",
            ),
            TestStep(
                "6b",
                "TH waits for a FabricSceneInfo report and records RemainingCapacity.",
            ...
            TestStep(
                "7a",
                "If RemainingCapacity is greater than 0, TH sends an AddScene command to DUT with GroupID G1, SceneID 0x03, TransitionTime 20000 and no extension field sets. Otherwise, skip to Step 8a.",
                "DUT sends an AddSceneResponse with Status 0x00 (SUCCESS), GroupID G1 and SceneID 0x03.",
            ),
            TestStep(
                "7b",
                "TH waits for a FabricSceneInfo report and records RemainingCapacity.",
            ...
            TestStep(
                ...
                "DUT sends a RemoveSceneResponse with Status 0x00 (SUCCESS), GroupID G1 and SceneID 0x01.",
            ),
            TestStep(
                "8b",
                "TH waits for a FabricSceneInfo report and records RemainingCapacity.",
    ...
    async def test_TC_S_2_5(self):
        ...
        # Step 0b: bind G1 to the key set via GroupKeyMap (legacy path only).
        ...
        # Step 1a: on a groupcast DUT, find out whether any group must be left first.
        membership = []
        if groupcast_enabled:
            self.step("1a")
        ...
        # Step 1b: clear existing group membership so G1 starts from a known state.
        self.step("1b")
        if groupcast_enabled:
            if membership:
        ...
        # Step 2: add G1 (groupcast JoinGroup or legacy AddGroup).
        self.step(2)
        if groupcast_enabled:
            await self.send_single_cmd(
                endpoint=0,
        ...
        if max_remaining_capacity - scene_count > 0:
            ...
            self._scene_info_cb.reset()
            resp = await self.send_single_cmd(
                Clusters.ScenesManagement.Commands.StoreScene(groupID=group_g1, sceneID=0x02), endpoint=self._scene_endpoint
            )
        ...
        self._scene_info_cb.reset()
        resp = await self.send_single_cmd(
            Clusters.ScenesManagement.Commands.RemoveScene(groupID=group_g1, sceneID=0x01), endpoint=self._scene_endpoint
        )
