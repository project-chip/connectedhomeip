...
ALARM = cluster.Bitmaps.AlarmBitmap.kShortCircuitFault
ALARM_MASK = int(ALARM)


class TC_EPALM_3_1(ElectricalProtectionAlarmTestBaseHelper):
    ...
    async def test_TC_EPALM_3_1(self):
        """[TC-EPALM-3.1] Notify event for ShortCircuitFault with Server as DUT

        This test case verifies Notify event reporting and the corresponding State
        transitions for the ShortCircuitFault alarm of the Electrical Protection Alarm
        Cluster server.
        """
        ...
        self.step("1c", "TH reads the Supported attribute",
                  expectation="Bit 0 is set, so the DUT supports ShortCircuitFault.")
        ...
        self.step("1e", "TH sends the TestEventTrigger clearing ShortCircuitFault, returning the DUT to its "
                        "no-fault baseline",
                  expectation="DUT returns SUCCESS and ShortCircuitFault is clear.")
        ...
        try:
            self.step("2a", "TH sends the TestEventTrigger setting ShortCircuitFault",
                      expectation="DUT returns SUCCESS and reports a Notify event with bit 0 "
            ...
            self.step("2b", "TH reads the State attribute", expectation="Bit 0 is 1.")
            asserts.assert_true(await self.read_state(endpoint) & ALARM_MASK,
                                "State bit 0 must be 1 after the set trigger")
            ...
            self.step("3b", "TH reads the State attribute", expectation="Bit 0 is 0.")
            asserts.assert_false(await self.read_state(endpoint) & ALARM_MASK,
                                 "State bit 0 must be 0 after the clear trigger")
        finally:
