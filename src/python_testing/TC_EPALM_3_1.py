...
SET_TRIGGER = 0x00A3_0000_0000_0001
CLEAR_TRIGGER = 0x00A3_0000_0000_0011

EVENT_TIMEOUT_SEC = 10.0
SETTLE_SEC = 2.0


class TC_EPALM_3_1(MatterBaseTest):

    def desc_TC_EPALM_3_1(self) -> str:
        return "[TC-EPALM-3.1] Notify event for ShortCircuitFault with DUT as Server"

    def pics_TC_EPALM_3_1(self) -> list[str]:
        return ["EPALM.S", "EPALM.S.F20", "EPALM.S.E00"]

    def steps_TC_EPALM_3_1(self) -> list[TestStep]:
        return [
            ...
            TestStep("1c", "TH reads the Supported attribute",
                     "Bit 0 is set, so the DUT supports ShortCircuitFault."),
            TestStep("1d", "TH reads TestEventTriggersEnabled from General Diagnostics on endpoint 0",
                     "Value is 1 (True)."),
            TestStep("1e", "TH sends the TestEventTrigger clearing ShortCircuitFault, returning the DUT to its "
                           "no-fault baseline", "DUT returns SUCCESS."),
            TestStep("2a", "TH sends the TestEventTrigger setting ShortCircuitFault",
                     "DUT returns SUCCESS and reports a Notify event with bit 0 set in Active and "
                     "set in State."),
            TestStep("2b", "TH reads the State attribute", "Bit 0 is 1."),
            TestStep("3a", "TH sends the TestEventTrigger clearing ShortCircuitFault",
                     "DUT returns SUCCESS and reports a Notify event with bit 0 set in Inactive and "
                     "clear in State."),
            TestStep("3b", "TH reads the State attribute", "Bit 0 is 0."),
        ]
    ...
    @run_if_endpoint_matches(has_feature(cluster, cluster.Bitmaps.Feature.kShortCircuit))
    async def test_TC_EPALM_3_1(self):
        ...
        asserts.assert_true(int(supported) & ALARM_MASK,
                            "Supported must set bit 0 for ShortCircuitFault")
        ...
        try:
            ...
            asserts.assert_true(int(report.active) & ALARM_MASK,
                                "Notify.Active must set bit 0 when ShortCircuitFault is raised")
            asserts.assert_true(int(report.state) & ALARM_MASK,
                                "Notify.State must set bit 0 when ShortCircuitFault is raised")

            self.step("2b")
            state = await self._read_state(endpoint)
            asserts.assert_true(int(state) & ALARM_MASK, "State bit 0 must be 1 after the set trigger")
            ...
            log.info("Notify on clear: active=0x%02X inactive=0x%02X state=0x%02X",
                     report.active, report.inactive, report.state)
            asserts.assert_true(int(report.inactive) & ALARM_MASK,
                                "Notify.Inactive must set bit 0 when ShortCircuitFault is cleared")
            asserts.assert_false(int(report.state) & ALARM_MASK,
                                 "Notify.State must clear bit 0 when ShortCircuitFault is cleared")

            self.step("3b")
            state = await self._read_state(endpoint)
            asserts.assert_false(int(state) & ALARM_MASK, "State bit 0 must be 0 after the clear trigger")
        finally:
            ...
            try:
                ...
                log.exception('Could not clear ShortCircuitFault during cleanup')
            sub.cancel()
