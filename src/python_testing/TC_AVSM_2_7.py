...
class TC_AVSM_2_7(MatterBaseTest):
    ...
    def steps_TC_AVSM_2_7(self) -> list[TestStep]:
        return [
            ...
            TestStep(3, "TH reads RankedVideoStreamPrioritiesList attribute from CameraAVStreamManagement Cluster on TH_SERVER.",
                     "Store this value in aRankedStreamPriorities."),
            TestStep(4, "TH reads RateDistortionTradeOffPoints attribute from CameraAVStreamManagement Cluster on TH_SERVER.",
                     "Store this value in aRateDistortionTradeOffPoints."),
            TestStep(5, "TH reads MinViewport attribute from CameraAVStreamManagement Cluster on TH_SERVER.",
                     "Store this value in aMinViewport."),
            TestStep(6, "TH reads VideoSensorParams attribute from CameraAVStreamManagement Cluster on TH_SERVER.",
                     "Store this value in aVideoSensorParams."),
            TestStep(7, "TH reads MaxEncodedPixelRate attribute from CameraAVStreamManagement Cluster on TH_SERVER.",
                     "Store this value in aMaxEncodedPixelRate."),
            TestStep(8, "TH sets StreamUsage from aRankedStreamPriorities. TH sets VideoCodec, MinResolution, MaxResolution, MinBitRate, MaxBitRate conforming with aRateDistortionTradeOffPoints. TH sets MinFrameRate, MaxFrameRate conforming with aVideoSensorParams. TH sets the MinFragmentLen and MaxFragmentLen = 4000. TH sends the VideoStreamAllocate command with these arguments.",
                     "DUT responds with VideoStreamAllocateResponse command with a valid VideoStreamID."),
            TestStep(9, "TH reads AllocatedVideoStreams attribute from CameraAVStreamManagement Cluster on TH_SERVER",
                     "Verify the number of allocated video streams in the list is 1."),
            TestStep(10, "TH sends the VideoStreamAllocate command with the same arguments from step 7 except StreamUsage set to a value not in aRankedStreamPriorities.",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
            TestStep(11, "TH sends the VideoStreamAllocate command with the same arguments from step 7 except MinFrameRate set to 0(outside of valid range).",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
            TestStep(12, "TH sends the VideoStreamAllocate command with the same arguments from step 7 except MinFrameRate > MaxFrameRate.",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
            TestStep(13, "TH sends the VideoStreamAllocate command with the same arguments from step 7 except MinBitRate set to 0(outside of valid range).",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
            TestStep(14, "TH sends the VideoStreamAllocate command with the same arguments from step 7 except MinBitRate > MaxBitRate.",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
            TestStep(15, "TH sends the VideoStreamAllocate command with the same arguments from step 7 except MinFragmentLen > MaxFragmentLen.",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
        ]
    ...
    async def test_TC_AVSM_2_7(self):
        ...
        self.step(10)
        try:
            notSupportedStreamUsage = next((e for e in cluster.Enums.StreamUsageEnum if e not in aRankedStreamPriorities),
