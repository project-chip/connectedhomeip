...
class TC_SwitchTests(MatterBaseTest):
    ...
    def _await_sequence_of_events(self, event_queue: queue.Queue, endpoint_id: int, sequence: list[ClusterObjects.ClusterEvent], timeout_sec: float):
        ...
        while time_remaining > 0:
            logger.info(f"Expecting event {sequence[sequence_idx]} on endpoint {endpoint_id}")
            try:
                ...
                if item.Header.EndpointId == endpoint_id and item.Header.ClusterId == event_data.cluster_id:
                    ...
                    if event_data == expected_event:
                        logger.info(f"Got expected Event {sequence_idx+1}/{len(sequence)}: {event_data}")
                        sequence_idx += 1
                    else:
                    ...
                    if sequence_idx == len(sequence):
                        logger.info("Got all expected events, done waiting.")
                        return
            except queue.Empty:
    ...
    def _expect_no_events_for_cluster(self, event_queue: queue.Queue, endpoint_id: int, expected_cluster: ClusterObjects.Cluster, timeout_sec: float):
        ...
        logger.info(f"Successfully waited for no further events on {expected_cluster} for {elapsed:.1f} seconds")
    ...
    async def test_TC_SWTCH_2_2(self):
        ...
        for expected_switch_position in range(1, num_positions):
            ...
            logger.info(f"-> SwitchLatched event last received: {data}")
            ...
            logger.info(f"Checking to see if a report for {expected_switch_position} is received")
            ...
            if num_positions > 2:
                logger.info("Looping for the other positions")
        ...
        logger.info(f"-> SwitchLatched event last received: {last_event_data}")
    ...
    async def test_TC_SWTCH_2_4(self):
        ...
        if not has_ms_feature:
            logger.info("Skipping rest of test: SWTCH.S.F01(MS) feature not present")
        ...
        logger.info(f"Starting to wait for {post_prompt_settle_delay_seconds:.1f} seconds for InitialPress event.")
        ...
        if not has_msl_feature:
            logger.info("Since MSL feature unsupported, skipping check for LongPress/LongRelease")
        else:
            # - TH expects report of LongPress, LongRelease in that order.
            logger.info(f"Starting to wait for {post_prompt_settle_delay_seconds:.1f} seconds for LongPress then LongRelease.")
            expected_events = []
