...
class TC_G_2_2(MatterBaseTest):
    ...
    def steps_TC_G_2_2(self):
        return [
            ...
            TestStep(
                "25",
                "If the cluster revision obtained at step 0 is less than 5, skip the remaining steps. Otherwise, TH removes any existing Group and KeySetID on the DUT.",
            ),
            TestStep(
                "26",
                "TH sends AddGroup command to DUT on PIXIT.G.ENDPOINT as unicast with the following fields: GroupID as 0x0001, GroupName as "
                " (empty string)",
            ),
            TestStep(
                "27",
            ...
            TestStep(
                "28",
                "TH sends Groupcast JoinGroup command to DUT on endpoint 0 with the following fields: GroupID as 0x0001, endpoints as [PIXIT.G.ENDPOINT], KeySetID as 0x01, Key as a random 16-byte value",
            ),
            TestStep(
                "29",
                "TH sends a second Groupcast JoinGroup command to DUT on endpoint 0 with the following fields: GroupID as 0x0002, endpoints as [PIXIT.G.ENDPOINT], KeySetID as 0x01, omit Key field",
            ),
            TestStep(
                "30",
    ...
    async def test_TC_G_2_2(self):
        ...
        if cluster_revision < 5:
            maxgroups: int = await self.read_single_attribute_check_success(
                cluster=Clusters.GroupKeyManagement, attribute=Clusters.GroupKeyManagement.Attributes.MaxGroupsPerFabric, endpoint=0
            )
            ...
            groupTableList: list[
                Clusters.GroupKeyManagement.Attributes.GroupTable
            ...
            asserts.assert_equal(
                result.status, Status.ConstraintError, "Returned status is different from expected ConstraintError"
            )
            ...
            if group_name_supported:
                result = await th1.SendCommand(
                    self.dut_node_id,
                    self.matter_test_config.endpoint,
                    Clusters.Groups.Commands.AddGroup(kGroupId, kGroupNameOverflow),
                )
                asserts.assert_equal(
                    result.status,
        ...
        # Cluster revision >= 5
            ...
            resp: Clusters.GroupKeyManagement.Commands.KeySetReadAllIndicesResponse = await self.send_single_cmd(
                endpoint=0, cmd=Clusters.GroupKeyManagement.Commands.KeySetReadAllIndices()
            )
            ...
            await self.send_single_cmd(
                ...
                cmd=Clusters.Groupcast.Commands.JoinGroup(
                    groupID=kGroupId1, endpoints=[self.matter_test_config.endpoint], keySetID=kKeySetID, key=secrets.token_bytes(16)
                ),
            )
            ...
            await self.send_single_cmd(
                ...
                node_id=self.dut_node_id,
                cmd=Clusters.Groupcast.Commands.JoinGroup(
                    groupID=kGroupId2, endpoints=[self.matter_test_config.endpoint], keySetID=kKeySetID
                ),
            )
            ...
            asserts.assert_equal(
                membership[0].groupID, kGroupId2, "Groupcast membership should contain only 1 entry with the GroupID 0x0002"
            )
            asserts.assert_equal(
                membership[0].endpoints,
                [self.matter_test_config.endpoint],
