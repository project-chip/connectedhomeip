...
class TC_TSTAT_4_3(MatterBaseTest):
    ...
    def pics_TC_TSTAT_4_3(self):
        ...
        return ["TSTAT.S", "TSTAT.S.F08", "TSTAT.S.F0a"]
    ...
    def steps_TC_TSTAT_4_3(self) -> list[TestStep]:
        return [
            ...
            TestStep("5", "TH picks a random preset handle that does not match any entry in the Presets attribute and calls the AddThermostatSuggestion command with the preset handle, the EffectiveTime set to the current UTC timestamp and ExpirationInMinutes is set to 30 minutes.",
            ...
            TestStep("8c", "TH calls the RemoveThermostatSuggestion command with the UniqueID field set to the UniqueID field of the CurrentThermostatSuggestion attribute.",
                     "Verify that that RemoveThermostatSuggestion command returns SUCCESS that the entry with the UniqueID matching the UniqueID field in the CurrentThermostatSuggestion attribute is removed from the ThermostatSuggestions attribute."),
            ...
            TestStep("9b", "TH calls the RemoveThermostatSuggestion command with the UniqueID field set to the UniqueID fields of the two entries in the ThermostatSuggestions attribute.",
                     "Verify that both entries are removed from the ThermostatSuggestions attribute."),
    ...
    async def test_TC_TSTAT_4_3(self):
        ...
        log.info("Supported Presets: %s", supported_presets)
        ...
        has_valid_time = True
        self.step("3")
        if self.pics_guard(self.check_pics("TIMESYNC.S.A0000")):
        ...
        if not has_valid_time:
            ...
            log.info("Active Preset Handle: %s", activePresetHandle)
        ...
        # TH picks a random preset handle that does not match any entry in the Presets attribute and calls the AddThermostatSuggestion command with the preset handle, the EffectiveTime set to the current UTC timestamp and ExpirationInMinutes set to 30 minutes.
        ...
        activePresetHandle = await self.read_single_attribute_check_success(endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.ActivePresetHandle)
        log.info("Active Preset Handlers: %s", activePresetHandle)
        possiblePresetHandles = [
            preset.presetHandle for preset in supported_presets if preset.presetHandle != activePresetHandle]
        ...
        log.info("UniqueID from AddThermostatSuggestionResponse: %s", addThermostatSuggestionResponse_uniqueID)
        ...
        self.step("7a")
        if self.pics_guard(self.check_pics("TSTAT.S.A0023") and self.check_pics("TSTAT.S.A0024")):
        ...
        if self.pics_guard(self.check_pics("TSTAT.S.A0023") and self.check_pics("TSTAT.S.A0024")):
            ...
            thermostatSuggestions = await self.read_single_attribute_check_success(endpoint=endpoint, cluster=cluster, attribute=cluster.Attributes.ThermostatSuggestions)
            asserts.assert_equal(len(thermostatSuggestions), 0,
                                 "ThermostatSuggestions should not have any entries after the RemoveThermostatSuggestion command removed the matching entry.")
        ...
        log.info("UniqueID from First AddThermostatSuggestionResponse: %s", firstAddThermostatSuggestionResponse_uniqueID)
        ...
        log.info("UniqueID from Second AddThermostatSuggestionResponse: %s", secondAddThermostatSuggestionResponse_uniqueID)
        ...
        log.info("MaxThermostatSuggestions: %s", maxThermostatSuggestions)
        # TH picks a preset handle from an entry in the SupportedPresets that does not match the ActivePresetHandle and calls the AddThermostatSuggestion command with the preset handle, the EffectiveTime set to the current UTC timestamp and ExpirationInMinutes is set to 30 minutes for the number of times specified in the value of MaxThermostatSuggestions + 1.
        possiblePresetHandles = [
            preset.presetHandle for preset in supported_presets if preset.presetHandle != activePresetHandle]
        asserts.assert_greater_equal(
