...
ALARM = cluster.Bitmaps.AlarmBitmap.kResidualCurrentFault
...
class TC_EPALM_3_5(ElectricalProtectionAlarmTestBaseHelper):
    ...
    async def test_TC_EPALM_3_5(self):
        ...
        self.step("1c", "TH reads the Supported attribute",
                  expectation="Bit 4 is set, so the DUT supports ResidualCurrentFault.")
        ...
        self.step("1e", "TH sends the TestEventTrigger clearing ResidualCurrentFault, returning the DUT to its "
                        "no-fault baseline",
                  expectation="DUT returns SUCCESS and ResidualCurrentFault is clear.")
        ...
        try:
            self.step("2a", "TH sends the TestEventTrigger setting ResidualCurrentFault",
                      expectation="DUT returns SUCCESS and reports a Notify event with bit 4 "
            ...
            self.step("2b", "TH reads the State attribute", expectation="Bit 4 is 1.")
            asserts.assert_true(await self.read_state(endpoint) & ALARM_MASK,
                                "State bit 4 must be 1 after the set trigger")

            self.step("3a", "TH sends the TestEventTrigger clearing ResidualCurrentFault",
                      expectation="DUT returns SUCCESS and reports a Notify event with bit 4 "
            ...
            self.step("3b", "TH reads the State attribute", expectation="Bit 4 is 0.")
            asserts.assert_false(await self.read_state(endpoint) & ALARM_MASK,
                                 "State bit 4 must be 0 after the clear trigger")
        finally:
