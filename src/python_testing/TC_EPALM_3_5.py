...
SET_TRIGGER = 0x00A3_0000_0000_0005
CLEAR_TRIGGER = 0x00A3_0000_0000_0015

EVENT_TIMEOUT_SEC = 10.0
SETTLE_SEC = 2.0


class TC_EPALM_3_5(MatterBaseTest):
    ...
    def steps_TC_EPALM_3_5(self) -> list[TestStep]:
        return [
            ...
            TestStep("1c", "TH reads the Supported attribute",
                     "Bit 4 is set, so the DUT supports ResidualCurrentFault."),
            TestStep("1d", "TH reads TestEventTriggersEnabled from General Diagnostics on endpoint 0",
                     "Value is 1 (True)."),
            TestStep("1e", "TH sends the TestEventTrigger clearing ResidualCurrentFault, returning the DUT to its "
                           "no-fault baseline", "DUT returns SUCCESS."),
            TestStep("2a", "TH sends the TestEventTrigger setting ResidualCurrentFault",
                     "DUT returns SUCCESS and reports a Notify event with bit 4 set in Active and "
                     "set in State."),
            TestStep("2b", "TH reads the State attribute", "Bit 4 is 1."),
            TestStep("3a", "TH sends the TestEventTrigger clearing ResidualCurrentFault",
                     "DUT returns SUCCESS and reports a Notify event with bit 4 set in Inactive and "
                     "clear in State."),
            TestStep("3b", "TH reads the State attribute", "Bit 4 is 0."),
        ]
    ...
    async def test_TC_EPALM_3_5(self):
        ...
        asserts.assert_true(int(supported) & ALARM_MASK,
                            "Supported must set bit 4 for ResidualCurrentFault")
        ...
        if state_before & ALARM_MASK:
            ...
            asserts.assert_true(int(report.inactive) & ALARM_MASK,
                                "Baseline clear must report ResidualCurrentFault in Inactive")
        asserts.assert_false(int(await self._read_state(endpoint)) & ALARM_MASK,
                             "ResidualCurrentFault must be clear before the set trigger")
        sub.flush_events()
        ...
        try:
            ...
            asserts.assert_true(int(report.active) & ALARM_MASK,
                                "Notify.Active must set bit 4 when ResidualCurrentFault is raised")
            asserts.assert_true(int(report.state) & ALARM_MASK,
                                "Notify.State must set bit 4 when ResidualCurrentFault is raised")

            self.step("2b")
            state = await self._read_state(endpoint)
            asserts.assert_true(int(state) & ALARM_MASK, "State bit 4 must be 1 after the set trigger")
            ...
            log.info("Notify on clear: active=0x%02X inactive=0x%02X state=0x%02X",
                     report.active, report.inactive, report.state)
            asserts.assert_true(int(report.inactive) & ALARM_MASK,
                                "Notify.Inactive must set bit 4 when ResidualCurrentFault is cleared")
            asserts.assert_false(int(report.state) & ALARM_MASK,
                                 "Notify.State must clear bit 4 when ResidualCurrentFault is cleared")

            self.step("3b")
            state = await self._read_state(endpoint)
            asserts.assert_false(int(state) & ALARM_MASK, "State bit 4 must be 0 after the clear trigger")
        finally:
            ...
            try:
                ...
                log.exception('Could not clear ResidualCurrentFault during cleanup')
            sub.cancel()
