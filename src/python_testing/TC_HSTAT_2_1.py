    ...
    def steps_TC_HSTAT_2_1(self):
        ...
        return [TestStep(next(step), "Commissioning already done.", is_commissioning=True),
                ...
                TestStep(next(step), "TH reads from the DUT the Step attribute.",
                         ...
                         "StepValue is such that (MaxSetpointValue - MinSetpointValue) % StepValue == 0."),
                TestStep(next(step), "TH reads from the DUT the UserSetpoint attribute.",
                         ...
                         "such that (SetpointValue - MinSetpointValue) % StepValue == 0."),
                TestStep(next(step), "TH reads from the DUT the TargetSetpoint attribute.",
                         "Verify that the DUT response contains a value between MinSetpointValue and MaxSetpointValue inclusive "),=
                ...
                TestStep(next(step), "TH reads from the DUT the Optimal attribute.",
                         "Verify that the DUT response contains a value or True or False"),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Mode field set to Off",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Mode attribute.",
                         "Verify that the DUT response contains Off."),
                TestStep(next(step), "TH reads from the DUT the SystemState attribute.",
                         "Verify that the DUT response contains Off."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Mode field set to Humidifier"
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Mode attribute.",
                         "Verify that the DUT response contains Humidifier."),
                TestStep(next(step), "TH reads from the DUT the SystemState attribute.",
                         "Verify that the DUT response contains Idle or Humidifying."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Mode field set to Dehumidifier",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Mode attribute.",
                         "Verify that the DUT response contains Dehumidifier."),
                TestStep(next(step), "TH reads from the DUT the SystemState attribute.",
                         "Verify that the DUT response contains Idle or Dehumidifying."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Mode field set to Auto",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Mode attribute.",
                         "Verify that the DUT response is Auto."),
                TestStep(next(step), "TH reads from the DUT the SystemState attribute.",
                         "Verify that the DUT response contains Idle, Humidifying, or Dehumidifying."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Mode field set to FanOnly",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Mode attribute.",
                         "Verify that the DUT response is FanOnly."),
                TestStep(next(step), "TH reads from the DUT the SystemState attribute.",
                         "Verify that the DUT response contains Fan."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Mode field set to Humidifier or Dehumidifier",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the UserSetpoint field set to MinSetpointValue."
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the UserSetpoint attribute.",
                         "Verify that the DUT response contains MinSetpointValue."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the UserSetpoint field set to MaxSetpointValue."
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the UserSetpoint attribute.",
                         "Verify that the DUT response contains MaxSetpointValue."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the UserSetpoint field set to MinSetpointValue + StepValue.",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the UserSetpoint attribute.",
                         "Verify that the DUT response contains MinSetpointValue + StepValue."),
                TestStep(next(step), "TH reads from the DUT the Continuous attribute."
                         "Store the value as ContState"),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Continuous field set to !ContState.",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Continuous attribute.",
                         "Verify that the DUT response contains !ContState."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Continuous field set to False.",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Sleep attribute.",
                         "Store the value as SleepState"),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Sleep field set to !SleepState.",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Sleep attribute.",
                         "Verify that the DUT response contains !SleepState."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Sleep field set to False.",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Optimal attribute.",
                         "Store the value as OptState"),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Optimal field set to !OptState.",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the Optimal attribute.",
                         "Store the value as !OptState"),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Optimal field set to False.",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the Mode field set to Humidifier",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the MistType field set to MistWarm",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the MistType attribute.",
                         "Verify that the DUT response contains MistWarm."),
                TestStep(next(step), "TH sends the SetSettings command to the DUT with the MistType field set to MistCold",
                         "Verify DUT responds w/ status SUCCESS(0x00)"),
                TestStep(next(step), "TH reads from the DUT the MistType attribute.",
