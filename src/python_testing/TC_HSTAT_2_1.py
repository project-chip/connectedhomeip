...
class TC_HSTAT_2_1(MatterBaseTest, HSTATBase):
    ...
    async def test_TC_HSTAT_2_1(self):
        ...
        dut_Mode = await self.read_attribute_expect_success(endpoint=endpoint, attribute=self.attributes.Mode)
        ...
        if self.sensorFeatureSupported:
            dut_UserSetpoint = await self.read_attribute_expect_success(endpoint=endpoint, attribute=self.attributes.UserSetpoint)
        ...
        if self.optimalFeatureSupported or (self.sensorFeatureSupported and self.attributes.TargetSetpoint.attribute_id in self.supported_attributes):
            dut_TargetSetpoint = await self.read_attribute_expect_success(endpoint=endpoint, attribute=self.attributes.TargetSetpoint)
        ...
        if self.condPumpFeatureSupported:
            dut_CondPumpEnabled = await self.read_attribute_expect_success(endpoint=endpoint, attribute=self.attributes.CondPumpEnabled)
        ...
        # Verify that the DUT response contains an unsigned integer value.
        if self.condPumpFeatureSupported:
            dut_CondRunCount = await self.read_attribute_expect_success(endpoint=endpoint, attribute=self.attributes.CondRunCount)
