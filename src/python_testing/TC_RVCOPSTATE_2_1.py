    ...
    async def test_TC_RVCOPSTATE_2_1(self):
        ...
        if self.check_pics("RVCOPSTATE.S.A0004"):
            ...
            if self.check_pics("RVCOPSTATE.S.M.ST_CHARGING"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ChargerFound"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
        ...
        if self.check_pics("RVCOPSTATE.S.A0005"):
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_UNABLE_TO_COMPLETE_OPERATION"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "UnableToCompleteOperation"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_COMMAND_INVALID_IN_STATE"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "CommandInvalidInState"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_FAILED_TO_FIND_CHARGING_DOCK"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "FailedToFindChargingDock"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_STUCK"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "Stuck"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_DUST_BIN_MISSING"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "DustBinMissing"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_DUST_BIN_FULL"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "DustBinFull"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_WATER_TANK_EMPTY"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "WaterTankEmpty"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_WATER_TANK_MISSING"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "WaterTankMissing"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_WATER_TANK_LID_OPEN"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "WaterTankLidOpen"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
            ...
            if self.check_pics("RVCOPSTATE.S.M.ERR_MOP_CLEANING_PAD_MISSING"):
                ...
                if self.is_ci:
                    self.write_to_app_pipe(self.app_pipe, {"Name": "ErrorEvent", "Error": "MopCleaningPadMissing"})
                else:
                    self.wait_for_user_input(prompt_msg=f"{test_step}, and press Enter when done.\n")
