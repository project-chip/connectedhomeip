...
class TC_TSTATM_2_1(MatterBaseTest):
    ...
    async def test_TC_TSTATM_2_1(self):
        ...
        if can_manually_control:
            asserts.assert_true('PIXIT.TSTATM.MODE_CHANGE_OK' in self.matter_test_config.global_test_params,
                                "PIXIT.TSTATM.MODE_CHANGE_OK must be included ...")
            self.mode_ok = self.matter_test_config.global_test_params['PIXIT.TSTATM.MODE_CHANGE_OK']

        if failure_and_manual:
            asserts.assert_true('PIXIT.TSTATM.MODE_CHANGE_FAIL' in self.matter_test_config.global_test_params,
                                "PIXIT.TSTATM.MODE_CHANGE_FAIL must be included ...")
        ...
        # Manually put the device in a state from which it will FAIL to transition to PIXIT.TSTATM.MODE_CHANGE_FAIL
        ...
        # TH sends a ChangeToMode command to the DUT with NewMode set to PIXIT.TSTATM.MODE_CHANGE_FAIL
        self.step(7)
        if self.pics_guard(failure_and_manual):
       ...
       # TH reads from the DUT the CurrentMode attribute
        ...
        # Manually put the device in a state from which it will SUCCESSFULLY transition to PIXIT.TSTATM.MODE_CHANGE_OK
        self.step(9)
        ...
        # TH sends a ChangeToMode command to the DUT with NewMode set to PIXIT.TSTATM.MODE_CHANGE_OK
        self.step(11)
        if self.pics_guard(can_manually_control):
