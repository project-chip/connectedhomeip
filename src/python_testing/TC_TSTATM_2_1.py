...
class TC_TSTATM_2_1(MatterTestCommissionedDevice):
    ...
    def desc_TC_TSTATM_2_1(self) -> str:
        return "[TC-TSTATM-2.1] Change to Mode functionality with DUT as Server"

    def steps_TC_TSTATM_2_1(self) -> list[TestStep]:
        return [
            ...
            TestStep(5, "Manually put the device in a state from which it will FAIL to transition to PIXIT.TSTATM.MODE_CHANGE_FAIL"),
            TestStep(6, "TH reads from the DUT the CurrentMode attribute", "CurrentMode attribute value is an integer value"),
            TestStep(7, "TH sends a ChangeToMode command to the DUT with NewMode set to PIXIT.TSTATM.MODE_CHANGE_FAIL",
                     "DUT responds contains a ChangeToModeResponse command with Status field set to GenericFailure(0x02), "
                     "InvalidInMode(0x03), or in the MfgCodes (0x80 to 0xBF) range and StatusText field has a length between 1 and 64"),
            ...
            TestStep(9, "Manually put the device in a state from which it will SUCCESSFULLY transition to PIXIT.TSTATM.MODE_CHANGE_OK"),
            TestStep(10, "TH reads from the DUT the CurrentMode attribute", "CurrentMode attribute value is an integer value"),
            TestStep(11, "TH sends a ChangeToMode command to the DUT with NewMode set to PIXIT.TSTATM.MODE_CHANGE_OK",
                     "DUT responds contains a ChangeToModeResponse command with a SUCCESS (value 0x00) status response"),
    ...
    def pics_TC_TSTATM_2_1(self) -> list[str]:
        return [
            "TSTATM.S"
        ]
    ...
    async def test_TC_TSTATM_2_1(self):
        ...
        if test_manual:
            ...
            logger.info("Skipping manual mode change steps (CAN_MANUALLY_CONTROLLED not enabled or PIXIT.TSTATM.MODE_CHANGE_OK omitted)")
        ...
        # Step 2: TH reads from the DUT the SupportedModes attribute
        self.step(2)
        ...
        # Step 3: TH reads from the DUT the CurrentMode attribute
        self.step(3)
        ...
        # Step 4: TH sends a ChangeToMode command to the DUT with NewMode set to old_current_mode_dut
        self.step(4)
        ...
        # Step 5: Manually put the device in a state from which it will FAIL to transition to PIXIT.TSTATM.MODE_CHANGE_FAIL
        self.step(5)
        ...
        # Step 6: TH reads from the DUT the CurrentMode attribute
        self.step(6)
        if self.pics_guard(test_failure):
            old_current_mode_dut = await self.read_single_attribute_check_success(
                endpoint=endpoint, cluster=cluster, attribute=current_mode_attribute
        ...
        # Step 7: TH sends a ChangeToMode command to the DUT with NewMode set to PIXIT.TSTATM.MODE_CHANGE_FAIL
        self.step(7)
        if self.pics_guard(test_failure):
        ...
        # Step 8: TH reads from the DUT the CurrentMode attribute
        self.step(8)
        if self.pics_guard(test_failure):
        ...
        # Step 9: Manually put the device in a state from which it will SUCCESSFULLY transition to PIXIT.TSTATM.MODE_CHANGE_OK
        self.step(9)
        ...
        # Step 10: TH reads from the DUT the CurrentMode attribute
        self.step(10)
        if self.pics_guard(test_manual):
            old_current_mode_dut = await self.read_single_attribute_check_success(
                endpoint=endpoint, cluster=cluster, attribute=current_mode_attribute
        ...
        # Step 11: TH sends a ChangeToMode command to the DUT with NewMode set to PIXIT.TSTATM.MODE_CHANGE_OK
        self.step(11)
        if self.pics_guard(test_manual):
        ...
        # Step 12: TH reads from the DUT the CurrentMode attribute
        self.step(12)
        if self.pics_guard(test_manual):
