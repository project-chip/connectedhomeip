...
class TC_HSTAT_2_3(MatterBaseTest):
    ...
    def steps_TC_HSTAT_2_3(self):
        return [
            ...
            TestStep(6, "TH sends command SetSettings with the Mode field set to Humidifier or Dehumidifier", "Verify DUT responds w/ status SUCCESS(0x00)"),
            ...
            TestStep(8, "TH sends command SetSettings with the UserSetpoint field set to MinSetpointValue.", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep(9, "TH reads from the DUT the UserSetpoint attribute.", "Verify that the DUT response contains a value of MinSetpointValue"),
            TestStep(10, "Individually subscribe to the UserSetpoint attribute", "This will receive updates when this attribute changes value."),
            TestStep(11, "TH sends command SetSettings with the UserSetpoint field set to MaxSetpointValue.", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep(12, "TH reads from the DUT the UserSetpoint attribute.", "Verify that the DUT response contains a value of MaxSetpointValue"),
            TestStep(13, "TH writes to the DUT the UserSetpoint attribute with MinSetpointValue + StepValue.", "Verify DUT responds w/ status SUCCESS(0x00)"),
            ...
            TestStep(15, "TH sends command SetSettings with the UserSetpoint field set to MinSetpointValue-1", "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)"),
            TestStep(16, "TH sends command SetSettings with the UserSetpoint field set to MaxSetpointValue+1", "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)"),
            TestStep(17, "TH sends command SetSettings with the UserSetpoint field set to MinSetpointValue+1", "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)"),
        ]
