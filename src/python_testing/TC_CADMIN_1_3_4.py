...
class TC_CADMIN(MatterBaseTest):
    ...
    async def combined_commission_val_steps(self, commission_type: str):
        ...
        if commission_type == "ECM":
            ...
            except ChipStackError as e:  # chipstack-ok: This disables ChipStackError linter check. Error occurs if a NOC for the fabric already exists, which may depend on device state. Can not use assert_raises
