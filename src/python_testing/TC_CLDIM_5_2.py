...
class TC_CLDIM_5_2(MatterBaseTest):
    ...
    def desc_TC_CLDIM_5_2(self) -> str:
        return "[TC-CLDIM-5.2] GroupedSetTarget Command Latching Functionality with DUT as Server"

    def steps_TC_CLDIM_5_2(self) -> list[TestStep]:
        return [
            TestStep(1, "Commissioning, already done", is_commissioning=True),
            TestStep("2a", "Read FeatureMap attribute"),
            TestStep("2b", "If MotionLatching feature is not supported OR Access feature is supported, skip remaining steps"),
    ...
    def pics_TC_CLDIM_5_2(self) -> list[str]:
        return [
            "CLDIM.S", "CLDIM.S.F01", "CLDIM.S.C02.Rsp"
        ]
    ...
    async def test_TC_CLDIM_5_2(self):
        ...
        # STEP 2b: If MotionLatching Feature is not supported OR Access feature is supported, skip remaining steps
        self.step("2b")
        if (not is_latching_supported) or is_access_supported:
            log.info("MotionLatching Feature is not supported or Access feature is supported. Skipping remaining steps.")
        ...
        # STEP 4a: Send GroupedStep command while device is latched
        self.step("4a")
        ...
        # STEP 4b: Send GroupedSetTarget command while device is latched
        self.step("4b")
        if is_positioning_supported:
            try:
                await self.send_single_cmd(
