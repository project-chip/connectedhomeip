...
class TC_BRBINFO_4_1(MatterBaseTest):
    ...
    def _wait_for_active_changed_event(self, cb: EventSubscriptionHandler, event, timeout_s: int) -> int:
        ...
        logger.info(f"PromisedActiveDurationEvent.Data: {promised_active_duration_event_data}")
    ...
    async def _send_keep_active_command(self, stay_active_duration_ms, timeout_ms, endpoint_id) -> int:
        logger.info("Sending keep active command")
    ...
    async def setup_class(self):
        ...
        logger.info("Commissioning of ICD to fabric one (TH)")
        ...
        logger.info("Commissioning of ICD to fabric two (DUT)")
    ...
    async def test_TC_BRBINFO_4_1(self):
        ...
        logger.info(f"Dynamic endpoint is {dynamic_endpoint_id}")

        self.step("0")
        logger.info("Ensuring DUT is commissioned to TH")
        ...
        logger.info("Ensuring ICD is commissioned to TH")
        ...
        logger.info(f"IdleModeDurationS: {idle_mode_duration_s}")
        ...
        logger.info(f"ActiveModeDurationMs: {active_mode_duration_ms}")
        ...
        logger.info(f"ActiveModeThresholdMs: {active_mode_threshold_ms}")
        ...
        logger.info(f"Sending first KeepActiveCommand({stay_active_duration_ms})")
        await self._send_keep_active_command(stay_active_duration_ms, keep_active_timeout_ms, dynamic_endpoint_id)
        logger.info(f"Sending second KeepActiveCommand({stay_active_duration_ms})")
        await self._send_keep_active_command(stay_active_duration_ms, keep_active_timeout_ms, dynamic_endpoint_id)
        logger.info(f"Sending third KeepActiveCommand({stay_active_duration_ms})")
        await self._send_keep_active_command(stay_active_duration_ms, keep_active_timeout_ms, dynamic_endpoint_id)
