...
class CommodityTariffTestBaseHelper(MatterBaseTest):
    ...
    async def checkDayEntryStruct(self,
        ...
        if await self.attribute_guard(cluster=cluster, endpoint=endpoint, feature_int=cluster.Bitmaps.Feature.kRandomization):
    ...
    async def checkTariffComponentStruct(self,
        ...
        if await self.attribute_guard(cluster=cluster, endpoint=endpoint, feature_int=cluster.Bitmaps.Feature.kPricing):
        ...
        if await self.attribute_guard(cluster=cluster, endpoint=endpoint, feature_int=cluster.Bitmaps.Feature.kFriendlyCredit):
        ...
        if await self.attribute_guard(cluster=cluster, endpoint=endpoint, feature_int=cluster.Bitmaps.Feature.kAuxiliaryLoad):
        ...
        if await self.attribute_guard(cluster=cluster, endpoint=endpoint, feature_int=cluster.Bitmaps.Feature.kPeakPeriod):
        ...
        if await self.attribute_guard(cluster=cluster, endpoint=endpoint, feature_int=cluster.Bitmaps.Feature.kPowerThreshold):
            if struct.powerThreshold is not None:
    ...
    async def check_default_randomization_offset_attribute(self, endpoint: int, attribute_value: int | None = None) -> None:
        ...
        if (await self.attribute_guard(endpoint=endpoint, attribute=cluster.Attributes.DefaultRandomizationOffset) and
                await self.attribute_guard(endpoint=endpoint, cluster=cluster, feature_int=cluster.Bitmaps.Feature.kRandomization)):
