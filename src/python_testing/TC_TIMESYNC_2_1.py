...
class TC_TIMESYNC_2_1(MatterBaseTest):
    ...
    async def test_TC_TIMESYNC_2_1(self):
        ...
        features = await self.read_single_attribute_check_success(attribute=attributes.FeatureMap)
        ...
        attribute_list = await self.read_single_attribute_check_success(attribute=timesync_attr_list)
        timesource_attr_id = attributes.TimeSource.attribute_id

        self.print_step(1, "Commissioning, already done")

        self.print_step(2, "Read Granularity attribute")
        granularity_dut = await self.read_single_attribute_check_success(attribute=attributes.Granularity)
        ...
        if timesource_attr_id in attribute_list:
            time_source = await self.read_single_attribute_check_success(attribute=attributes.TimeSource)
        ...
        if self.supports_trusted_time_source:
            trusted_time_source = await self.read_single_attribute_check_success(attribute=attributes.TrustedTimeSource)
        ...
        if self.supports_ntpc:
            default_ntp = await self.read_single_attribute_check_success(attribute=attributes.DefaultNTP)
        ...
        if self.supports_time_zone:
            tz_dut = await self.read_single_attribute_check_success(attribute=attributes.TimeZone)
        ...
        if self.supports_time_zone:
            dst_dut = await self.read_single_attribute_check_success(attribute=attributes.DSTOffset)
            last_valid_until = -1
        ...
        utc_dut = await self.read_single_attribute_check_success(attribute=attributes.UTCTime)
        ...
        if self.supports_time_zone:
            utc_dut = await self.read_single_attribute_check_success(attribute=attributes.UTCTime)
            local_dut = await self.read_single_attribute_check_success(attribute=attributes.LocalTime)
            if utc_dut is NullValue:
        ...
        if self.supports_time_zone:
            tz_db_dut = await self.read_single_attribute_check_success(attribute=attributes.TimeZoneDatabase)
        ...
        if self.supports_ntps:
            ...
            await self.read_single_attribute_check_success(attribute=attributes.NTPServerAvailable)

        self.print_step(12, "Read TimeZoneListMaxSize")
        if self.supports_time_zone:
            size = await self.read_single_attribute_check_success(attribute=attributes.TimeZoneListMaxSize)
        ...
        if self.supports_time_zone:
            size = await self.read_single_attribute_check_success(attribute=attributes.DSTOffsetListMaxSize)
            asserts.assert_greater_equal(size, 1, "DSTOffsetListMaxSize must be at least 1")

        self.print_step(14, "Read SupportsDNSResolve")
        # bool typechecking happens in the test read functions, so all we need to do here is do the read
        if self.supports_ntpc:
            await self.read_single_attribute_check_success(attribute=attributes.SupportsDNSResolve)
