...
class TC_LUNIT_3_1(MatterBaseTest):
    ...
    async def write_lunit_temp_unit(self,
        ...
        asserts.assert_equal(status, expected_status, f"TemperatureUnit write returned {status.name}; expected {expected_status.name}")
        return status
    ...
    async def test_TC_LUNIT_3_1(self):
        ...
        if self.supports_TEMP:
            ...
            asserts.assert_greater_equal(temperature_unit, Clusters.Objects.UnitLocalization.Enums.TempUnitEnum.kFahrenheit, "TemperatureUnit has to be Fahrenheit, Celsius or Kelvin")
            asserts.assert_less_equal(temperature_unit, Clusters.Objects.UnitLocalization.Enums.TempUnitEnum.kKelvin, "TemperatureUnit has to be Fahrenheit, Celsius or Kelvin")
        else:
        ...
        if self.supports_TEMP:
            ...
            asserts.assert_greater_equal(len(supported_temperature_units), 2, "SupportedTemperatureUnits must have at least two entry in the list")
            asserts.assert_less_equal(len(supported_temperature_units), 3, "SupportedTemperatureUnits may have a maximum of three entries in the list")
        else:
            logging.info("Test step skipped")
        ...
        if self.supports_TEMP:
            for unit in [Clusters.Objects.UnitLocalization.Enums.TempUnitEnum.kFahrenheit, Clusters.Objects.UnitLocalization.Enums.TempUnitEnum.kCelsius, Clusters.Objects.UnitLocalization.Enums.TempUnitEnum.kKelvin]:
