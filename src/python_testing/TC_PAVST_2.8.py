...
class TC_PAVST_2_8(MatterBaseTest):
    ...
    async def test_TC_PAVST_2_8(self):
        ...
        self.step(3)
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            status = await self.send_single_cmd(cmd=pvcluster.Commands.FindTransport (
                {"connectionID": 10,
                }), endpoint=endpoint)
       ...
       self.step(5)
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            transport_configs = await self.send_single_cmd(cmd=pvcluster.Commands.FindTransport(
                {"connectionID": aConnectionID,
                }), endpoint=endpoint)
            asserts.assert_true(transport_configs.connectionID, aConnectionID, "DUT must responds with TransportConfiguration with the given connectionID.")
       ...
       self.step(6)
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            ...
            asserts.assert_true(transport_configs.connectionID, aConnectionID, "DUT must responds with TransportConfiguration with the existing connectionID.")
