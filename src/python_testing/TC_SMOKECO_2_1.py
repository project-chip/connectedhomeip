...
class TC_SMOKECO_2_1(SmokeCoBaseTest):
    ...
    async def test_TC_SMOKECO_2_1(self):
        ...
        self.step(3)
        if await self.attribute_guard(endpoint=self.get_endpoint(), cluster=Clusters.SmokeCoAlarm, feature_int=Clusters.SmokeCoAlarm.Bitmaps.Feature.kSmokeAlarm):
            await self.read_attribute_check_range(self.smokeco_cluster.Attributes.SmokeState, enum=self.smokeco_enums.AlarmStateEnum)
        else:
            self.skip_step(3)

        self.step(4)
        if await self.attribute_guard(endpoint=self.get_endpoint(), cluster=Clusters.SmokeCoAlarm, feature_int=Clusters.SmokeCoAlarm.Bitmaps.Feature.kCoAlarm):
