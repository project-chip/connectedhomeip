...
def main(filename, classname, test):
    ...
    try:
        ...
        log.error('Unable to load python module from %s. Please ensure this is a valid python file path', filename)
        return -1

    try:
        test_class = getattr(module, classname)
    except AttributeError:
        log.error('Unable to load the test class %s. Please ensure this class is implemented in %s', classname, filename)
        return -1
    ...
    if not steps:
        log.error('Unable to find steps for test %s. Please ensure the steps_ function is implemented', test)
        return -1
