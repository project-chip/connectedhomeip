...
class TC_WebRTCP_2_5(MatterBaseTest, WEBRTCPTestBase):
    ...
    def steps_TC_WebRTCP_2_5(self) -> list[TestStep]:
        ...
        return [
            ...
            TestStep(3, "Allocate Video Stream"),
            TestStep(4, "Send SolicitOffer with valid parameters, audio stream ID is Null, video stream ID is for the allocated stream => expect INVALID_IN_STATE"),
            TestStep(5, "Allocate Audio Stream"),
            TestStep(6, "Send SolicitOffer with valid parameters, audio stream ID is the allocated stream ID, video stream ID is invalid => expect DYNAMIC_CONSTRAINT_ERROR"),
            TestStep(7, "Send SolicitOffer with a supported stream usage that doesn't match the allocated streams, audio stream ID is the allocated stream ID, video stream ID is the allocated stream => expect SolictOfferResponse"),
            TestStep(8, "Send SolicitOffer with an unsupported stream usage, audio stream ID is the allocated stream ID, video stream ID is the allocated stream => expect DYNAMIC_CONSTRAINT_ERROR"),
            TestStep(9, "Send SolicitOffer with a supported stream usage that doesn't match the allocated streams, audio stream ID is null, video stream ID is null => expect SolicitOfferResponse and Ids that match the allocated streams"),
            TestStep(10, "Send SolicitOffer with valid parameters and stream IDs => expect SolicitOfferResponse"),
            TestStep(11, "Read CurrentSessions attribute => expect 3"),
            TestStep(12, "Send EndSession with invalid WebRTCSessionID => expect NOT_FOUND"),
            TestStep(13, "Send EndSession with valid WebRTCSessionID => expect SUCCESS"),
        ]
