...
from support_modules.TC_GC_common import get_feature_map, get_operate_only_commands, is_groupcast_on_root_node
...
class TC_SC_5_2(MatterBaseTest):
    ...
    async def test_TC_SC_5_2(self):
        ...
        if raw_delay is not None:
            ...
            logger.info("Explicit PIXIT.GROUPCAST_SETTLE_DELAY_SECONDS argument provided (%.1f s)",
                        self.groupcast_settle_delay_seconds)
    ...
    async def run_test_against_endpoint(self, groups_endpoint: int):
        ...
        if not is_groupcast_listener:
            ...
            # Step 16c-16k: Update GroupKeyMap and test Group Session ID collisions
            # Direct GroupKeyMap updates are disallowed when Groupcast cluster is present per Matter spec.
            if groupcast_enabled:
                logger.warning(
                    "Skipping steps 16c-16k: GroupKeyMap direct write is disallowed when Groupcast cluster is present. Tracked for test plan update."
                )
                self.mark_step_range_skipped("16c", "16k")
            else:
