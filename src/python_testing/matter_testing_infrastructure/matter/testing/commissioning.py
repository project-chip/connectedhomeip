...
async def commission_device(
    ...
    if commissioning_info.commissioning_method == "on-network":
        ...
        try:
            ...
            LOGGER.error("Commissioning failed: %s" % e)
            return PairingStatus(exception=e)
    elif commissioning_info.commissioning_method == "thread-only":
        try:
            asserts.assert_is_not_none(commissioning_info.thread_operational_dataset,
                                       "Thread dataset must be provided for thread-only commissioning")
            # Type assertion to help mypy understand this is not None after the assert
            assert commissioning_info.thread_operational_dataset is not None
            asserts.assert_is_not_none(commissioning_info.border_agent_ip_addr,
                                       "border_agent_ip_addr must be provided for thread-only commissioning")
            # Type assertion to help mypy understand this is not None after the assert
            ...
            asserts.assert_is_not_none(commissioning_info.border_agent_port,
                                       "border_agent_port must be provided for thread-only commissioning")
            # Type assertion to help mypy understand this is not None after the assert
            assert commissioning_info.border_agent_port is not None

            await dev_ctrl.CommissionThreadOnly(
                node_id,
                info.passcode,
                info.filter_value,
