...
def load_errata_file(errata_path: str | Path | Traversable) -> dict[str, Any]:
    """Loads and returns the YAML errata rules from a given path or Traversable resource."""
    try:
        if isinstance(errata_path, (str, Path)):
...
def _apply_attribute_errata(target_attribute: Any, overrides: dict[str, Any], context: str, problems: list[str]) -> None:
    ...
    if 'read_access' in overrides:
        try:
            ...
            problems.append(f"Invalid read_access on '{context}': {e}")

    if 'write_access' in overrides:
        try:
            target_attribute.write_access = parse_errata_access(str(overrides['write_access']))
        except ValueError as e:
            problems.append(f"Invalid write_access on '{context}': {e}")


def _apply_command_errata(target_cmd: Any, overrides: dict[str, Any], context: str, problems: list[str]) -> None:
    ...
    if 'invoke_access' in overrides or 'privilege' in overrides:
        ...
        try:
            ...
            problems.append(f"Invalid privilege/invoke_access on '{context}': {e}")


def _apply_element_errata(target_cluster: Any, element_name: str, overrides: Any, cluster_name: str,
    ...
    if attr_id is not None:
        ...
        if not target_attribute:
            problems.append(f"Attribute ID {attr_id} for '{context}' missing in AST.")
            return
        _apply_attribute_errata(target_attribute, overrides, context, problems)
        return
    ...
    if cmd_id is not None:
        ...
        if not target_cmd:
            problems.append(f"Command ID {cmd_id} for '{context}' missing in accepted/generated AST lists.")
            return
        _apply_command_errata(target_cmd, overrides, context, problems)
        return

    problems.append(f"Errata referenced unknown Element '{element_name}' in Cluster '{cluster_name}'.")


def apply_errata(clusters: Mapping[uint, Any], errata_data: dict[str, Any],
                 active_spec_revision: Optional[str] = None) -> list[str]:
    """Applies a dictionary of errata override rules directly to the built in-memory AST.

    Returns a list of warning/error notice strings for any orphaned, unrecognized, or incompatible rules.
    """
    problems = _validate_spec_revision(errata_data, active_spec_revision)
    if problems:
        return problems
    ...
    for cluster_name, elements in errata_data.items():
        ...
        target_cluster = next((c for c in clusters.values() if _sanitize_name(c.name) == cluster_name.lower()), None)
        if not target_cluster:
            problems.append(f"Errata referenced unknown Cluster name: '{cluster_name}'")
            continue

        if not isinstance(elements, dict):
            problems.append(f"Errata definitions for cluster '{cluster_name}' must be a dictionary.")
            continue
        ...
        for element_name, overrides in elements.items():
            _apply_element_errata(target_cluster, element_name, overrides, cluster_name,
