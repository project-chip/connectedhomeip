            ...
            try:
                ...
                if len(map_id) == 0:
                    if is_disallowed(conformance_override):
                        LOGGER.info(
                            "Ignoring unknown %s %s in cluster %04X because the conformance is disallowed", override_element_type, element_name, c_id)
                        continue
                    problems.append(ProblemNotice("Parse Device Type XML", location=location,
        ...
        for c in clusters:
            try:
                ...
                if c_id not in cluster_definition_xml:
                    LOGGER.info("Skipping unknown cluster %04X", c_id)
                    continue
