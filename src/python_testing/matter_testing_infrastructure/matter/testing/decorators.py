...
EndpointCheckFunction = Callable[[ReadResponse, int], bool]


def _has_cluster(wildcard: ReadResponse, endpoint: int, cluster: ClusterObjects.ClusterObjectDescriptor) -> bool:
...
def _has_attribute(wildcard: ReadResponse, endpoint: int, attribute: ClusterObjects.ClusterAttributeDescriptor) -> bool:
...
def _has_command(wildcard: ReadResponse, endpoint: int, command: ClusterObjects.ClusterCommand) -> bool:
...
def _has_feature(wildcard: ReadResponse, endpoint: int, cluster: ClusterObjects.ClusterObjectDescriptor, feature: IntFlag) -> bool:
    if endpoint not in wildcard.attributes:
        return False
...
def _read_response_from_cached_endpoints(test_instance: "MatterBaseTest") -> ReadResponse:
    """Reuse composition data from setup_class_helper as a ReadResponse."""
    return ReadResponse(
        attributes=test_instance.endpoints,
        events=[],
        tlvAttributes=getattr(test_instance, "endpoints_tlv", {}),
    )
...
async def _get_all_matching_endpoints(test_instance, accept_function: EndpointCheckFunction) -> list[int]:
    ...
    if cached_endpoints:
        wildcard = _read_response_from_cached_endpoints(test_instance)
    else:
    ...
    def run_on_singleton_matching_endpoint_internal(body):
        ...
        def matching_runner(self: "MatterBaseTest", *args, **kwargs):
            ...
            try:
                ...
                LOGGER.info("Running test on endpoint %s", self.matter_test_config.endpoint)
    ...
    def run_if_endpoint_matches_internal(body):
        ...
        def per_endpoint_runner(test_instance, *args, **kwargs):
            ...
            if not should_run_test:
                ...
                return
            LOGGER.info("Running test on endpoint %s", test_instance.matter_test_config.endpoint)
            timeout = getattr(test_instance.matter_test_config, "timeout", None) or test_instance.default_timeout
