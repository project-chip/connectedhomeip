...
class TC_AVSM_2_5(MatterBaseTest):
    ...
    def steps_TC_AVSM_2_5(self) -> list[TestStep]:
        return [
            ...
            TestStep(5, "TH sends the AudioStreamAllocate command with valid values of AudioCodec, ChannelCount, SampleRate and BitDepth from aMicrophoneCapabilities and StreamUsage from aRankedStreamPriorities.",
                     "DUT responds with AudioStreamAllocateResponse command with a valid AudioStreamID."),
            TestStep(6, "TH reads AllocatedAudioStreams attribute from CameraAVStreamManagement Cluster on TH_SERVER",
                     "Verify the number of allocated audio streams in the list is 1."),
            TestStep(7, "TH sends the AudioStreamAllocate command with values from step 4 except with ChannelCount set to 16(outside of valid range)",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
            TestStep(8, "TH sends the AudioStreamAllocate command with values from step 4 except with BitDepth set to 48(outside of valid range)",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
            TestStep(9, "TH sends the AudioStreamAllocate command with values from step 4 except with Samplerate set to 0(outside of valid range)",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
            TestStep(10, "TH sends the AudioStreamAllocate command with values from step 4 except with BitRate set to 0(outside of valid range)",
                     "DUT responds with a CONSTRAINT_ERROR status code."),
        ]
