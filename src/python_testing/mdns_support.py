...
class TxtRecordUtil:
    ...
    def _get_property_value(self, key: str):
        # Convert the key from string to bytes, as the dictionary uses bytes
        byte_key = key.encode('utf-8')

        # Check if the key exists in the dictionary
        asserts.assert_in(byte_key, self._properties, f"Property '{key}' not found")
        value = self._properties[byte_key]
    ...
    async def get(self, key: str = None, refresh = False):
        ...
        if self._properties is None or refresh:
            ...
            self._properties = service_info.properties
