...
class TC_EPALM_3_6(ElectricalProtectionAlarmTestBaseHelper):
    ...
    async def test_TC_EPALM_3_6(self):
        ...
        self.step("1c", "TH reads the Supported attribute",
                  expectation="Bit 5 is set, so the DUT supports ArcFault.")
        ...
        self.step("1e", "TH sends the TestEventTrigger clearing ArcFault, returning the DUT to its "
                        "no-fault baseline",
                  expectation="DUT returns SUCCESS and ArcFault is clear.")
        ...
        try:
            self.step("2a", "TH sends the TestEventTrigger setting ArcFault",
                      expectation="DUT returns SUCCESS and reports a Notify event with bit 5 "
            ...
            self.step("2b", "TH reads the State attribute", expectation="Bit 5 is 1.")
            asserts.assert_true(await self.read_state(endpoint) & ALARM_MASK,
                                "State bit 5 must be 1 after the set trigger")

            self.step("3a", "TH sends the TestEventTrigger clearing ArcFault",
                      expectation="DUT returns SUCCESS and reports a Notify event with bit 5 "
            ...
            self.step("3b", "TH reads the State attribute", expectation="Bit 5 is 0.")
            asserts.assert_false(await self.read_state(endpoint) & ALARM_MASK,
                                 "State bit 5 must be 0 after the clear trigger")
        finally:
