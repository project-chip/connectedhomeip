    ...
    async def test_TC_PWRTL_2_1(self):
        ...
        if avail_endpoints_attr_id in attribute_list:
            ...
            if available_endpoints == []:
                ...
                log.info(f"AvailableEndpoints: {available_endpoints}")
                asserts.assert_less_equal(len(available_endpoints), 20,
                                          f"AvailableEndpoints length {len(available_endpoints)} must be less than 21!")
        ...
        if act_endpoints_attr_id in attribute_list:
            ...
            log.info(f"ActiveEndpoints: {active_endpoints}")
            asserts.assert_less_equal(len(active_endpoints), 20,
                                      f"ActiveEndpoints length {len(active_endpoints)} must be less than 21!")
