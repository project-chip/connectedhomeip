...
class TC_TLSCERT(MatterBaseTest):
    ...
    class TargetedFabric:
        ...
        async def send_remove_root_command(
            ...
            try:
                ...
                asserts.assert_true(type_matches(result, Status),  "Unexpected return type for RemoveRootCertificate")
                asserts.assert_equal(result, expected_status, "Unexpected error returned")
                return result
            except InteractionModelError as e:
                asserts.assert_equal(e.status, expected_status, "Unexpected error returned")
                return e
    ...
    async def test_TC_TLSCERT_3_1(self):
        ...
        resp = await self.send_single_cmd(cmd=opcreds.Commands.RemoveFabric(fabric_index_cr2))
