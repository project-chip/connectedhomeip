...
class TC_PAVST_2_4(MatterBaseTest):
    ...
    async def test_TC_PAVST_2_4(self):
        ...
        self.step(3)
        try:
            await self.send_single_cmd(cmd=cluster.Commands.ModifyPushTransport(
        ...
        try:
            await self.send_single_cmd(cmd=cluster.Commands.ModifyPushTransport(
                ConnectionID=aConnectionID,
                {"streamUsage":aTransportOptions.streamUsage,
                 "videoStreamID":aTransportOptions.videoStreamID,
                 "audioStreamID":aTransportOptions.audioStreamID,
                 "endpointID":aTransportOptions.endpointID,
                 "url":aTransportOptions.url,
                 "triggerOptions":aTransportOptions.triggerOptions,
                 "ingestMethod":aTransportOptions.ingestMethod,
                 "containerFormat"::aTransportOptions.containerFormat,
                 "containerOptions":aTransportOptions.containerOptions,
                 "expiryTime":aTransportOptions.expiryTime+120,
                 }),
                                       endpoint=endpoint)
        except InteractionModelError as e:
        ...
        asserts.assert_equal(transport_configs.ConnectionID, aConnectionID, "TransportConfigurations expecting same connectionID")
        asserts.assert_equal(transport_configs.TransportOptions.expiryTime, aTransportOptions.expiryTime+120, "TransportConfigurations expecting same expiryTime")
