...
SET_TRIGGER = 0x00A3_0000_0000_0003
CLEAR_TRIGGER = 0x00A3_0000_0000_0013

EVENT_TIMEOUT_SEC = 10.0
SETTLE_SEC = 2.0


class TC_EPALM_3_3(MatterBaseTest):

    def desc_TC_EPALM_3_3(self) -> str:
        return "[TC-EPALM-3.3] Notify event for OverVoltageFault with DUT as Server"
    ...
    def steps_TC_EPALM_3_3(self) -> list[TestStep]:
        return [
            ...
            TestStep("1c", "TH reads the Supported attribute",
                     "Bit 2 is set, so the DUT supports OverVoltageFault."),
            TestStep("1d", "TH reads TestEventTriggersEnabled from General Diagnostics on endpoint 0",
                     "Value is 1 (True)."),
            TestStep("1e", "TH sends the TestEventTrigger clearing OverVoltageFault, returning the DUT to its "
                           "no-fault baseline", "DUT returns SUCCESS."),
            TestStep("2a", "TH sends the TestEventTrigger setting OverVoltageFault",
                     "DUT returns SUCCESS and reports a Notify event with bit 2 set in Active and "
                     "set in State."),
            TestStep("2b", "TH reads the State attribute", "Bit 2 is 1."),
            TestStep("3a", "TH sends the TestEventTrigger clearing OverVoltageFault",
                     "DUT returns SUCCESS and reports a Notify event with bit 2 set in Inactive and "
                     "clear in State."),
            TestStep("3b", "TH reads the State attribute", "Bit 2 is 0."),
        ]
    ...
    async def test_TC_EPALM_3_3(self):
        ...
        asserts.assert_true(int(state) & ALARM_MASK, "State bit 2 must be 1 after the set trigger")
        ...
        self.step("3b")
        state = await self._read_state(endpoint)
        asserts.assert_false(int(state) & ALARM_MASK, "State bit 2 must be 0 after the clear trigger")
        sub.cancel()
