...
class TC_CC_2_1(MatterBaseTest):
    ...
    async def _guard_and_read_attribute(self, attribute: str) -> Union[bool, Any]:
        ...
        if self.attribute_guard(self.endpoint, tmp_attr):
    ...
    async def _verify_attribute(self, attribute: str, data_type: str, enum_range: Optional[list] = None, min_len: Optional[int] = None, max_len: Optional[int] = None, skip_verification: bool = False):
        ...
        elif data_type == "string" and isinstance(attr_val, str):
            if max_len > 0:
    ...
    async def test_TC_CC_2_1(self):
        ...
        self.step(2)
        await self._verify_attribute("CurrentHue", "uint8")

        self.step(3)
        await self._verify_attribute("CurrentSaturation", "uint8")

        self.step(4)
        await self._verify_attribute("RemainingTime", "uint16")

        self.step(5)
        await self._verify_attribute("CurrentX", "uint16", max_len=0xfeff)

        self.step(6)
        await self._verify_attribute("CurrentY", "uint16", max_len=0xfeff)

        self.step(7)
        await self._verify_attribute("DriftCompensation", "enum", enum_range=range(0, 5))

        self.step(8)
        await self._verify_attribute("CompensationText", "string", max_len=254)

        self.step(9)
        await self._verify_attribute("ColorTemperatureMireds", "uint16", max_len=0xfeff)
        ...
        self.step(13)
        await self._verify_attribute("EnhancedColorMode", "enum", enum_range=range(0, 4))

        self.step(14)
        await self._verify_attribute("ColorLoopActive", "uint8")

        self.step(15)
        await self._verify_attribute("ColorLoopDirection", "uint8")

        self.step(16)
        await self._verify_attribute("ColorLoopTime", "uint16")

        self.step(17)
        await self._verify_attribute("ColorLoopStartEnhancedHue", "uint16")
        ...
        self.step(25)
        await self._verify_attribute("Primary1X", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp1)
        self.step(26)
        await self._verify_attribute("Primary1Y", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp1)
        self.step(27)
        await self._verify_attribute("Primary1Intensity", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp1)

        skip_steps_verifyp2 = self._verify_for_numberofprimaries_value(numberofprimaries_value, 2)
        self.step(28)
        await self._verify_attribute("Primary2X", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp2)
        self.step(29)
        await self._verify_attribute("Primary2Y", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp2)
        self.step(30)
        await self._verify_attribute("Primary2Intensity", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp2)

        skip_steps_verifyp3 = self._verify_for_numberofprimaries_value(numberofprimaries_value, 3)
        self.step(31)
        await self._verify_attribute("Primary3X", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp3)
        self.step(32)
        await self._verify_attribute("Primary3Y", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp3)
        self.step(33)
        await self._verify_attribute("Primary3Intensity", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp3)

        skip_steps_verifyp4 = self._verify_for_numberofprimaries_value(numberofprimaries_value, 4)
        self.step(34)
        await self._verify_attribute("Primary4X", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp4)
        self.step(35)
        await self._verify_attribute("Primary4Y", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp4)
        self.step(36)
        await self._verify_attribute("Primary4Intensity", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp4)

        skip_steps_verifyp5 = self._verify_for_numberofprimaries_value(numberofprimaries_value, 5)
        self.step(37)
        await self._verify_attribute("Primary5X", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp5)
        self.step(38)
        await self._verify_attribute("Primary5Y", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp5)
        self.step(39)
        await self._verify_attribute("Primary5Intensity", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp5)

        skip_steps_verifyp6 = self._verify_for_numberofprimaries_value(numberofprimaries_value, 6)
        self.step(40)
        await self._verify_attribute("Primary6X", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp6)
        self.step(41)
        await self._verify_attribute("Primary6Y", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp6)
        self.step(42)
        await self._verify_attribute("Primary6Intensity", "uint16", max_len=0xfeff, skip_verification=skip_steps_verifyp6)
        ...
        self.step(44)
        await self._verify_attribute("WhitePointY", "uint16", max_len=0xfeff)

        self.step(45)
        await self._verify_attribute("ColorPointRX", "uint16", max_len=0xfeff)

        self.step(46)
        await self._verify_attribute("ColorPointRY", "uint16", max_len=0xfeff)

        self.step(47)
        await self._verify_attribute("ColorPointRIntensity", "uint8")

        self.step(48)
        await self._verify_attribute("ColorPointGX", "uint16", max_len=0xfeff)

        self.step(49)
        await self._verify_attribute("ColorPointGY", "uint16", max_len=0xfeff)

        self.step(50)
        await self._verify_attribute("ColorPointGIntensity", "uint8")

        self.step(51)
        await self._verify_attribute("ColorPointBX", "uint16", max_len=0xfeff)

        self.step(52)
        await self._verify_attribute("ColorPointBY", "uint16", max_len=0xfeff)

        self.step(53)
        await self._verify_attribute("ColorPointBIntensity", "uint8")
