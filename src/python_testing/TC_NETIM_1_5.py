...
from support_modules.network_identity import (NETWORK_ADMINISTRATOR_RAW_SECRET_LENGTH, NETWORK_IDENTITY_IDENTIFIER_LENGTH,
                                              derive_ecdsa_network_identity, encode_network_administrator_secret, matter_epoch_now,
                                              network_identity_identifier)
...
class TC_NETIM_1_5(NetworkIdentityTHServerTest):
    ...
    @pics('NETIM.C', 'NETIM.C.C00.Tx', 'NETIM.C.C02.Tx', 'NETIM.C.C03.Tx')
    @async_test_body
    async def test_TC_NETIM_1_5(self):
        ...
        asserts.assert_equal(len(clients), 1, "AddClient with a valid client identity should have added exactly one client.")
        ...
        self.ask_dut_to_send(
            "Using the DUT's interface, send AddClient to the TH Network Identity Management server again with the exact "
            f"same client identity used in the previous step (its Identifier is {client_identifier_a.hex()}).",
        ...
        self.ask_dut_to_send(
            "Using the DUT's interface, send AddClient to the TH Network Identity Management server with a client identity "
            f"that already exists: one whose Identifier is {client_identifier_a.hex()} but whose encoding differs from the "
            "identity added in step 2 (for example, the same public key carrying a freshly generated signature).",
        ...
        self.ask_dut_to_send(
            "Using the DUT's interface, send QueryIdentity to the TH Network Identity Management server with exactly one "
            f"selector: NetworkIdentityIndex={network_identity_index} (or, equivalently, "
        ...
        self.ask_dut_to_send(
            "Using the DUT's interface, send QueryIdentity to the TH Network Identity Management server with none of the "
        ...
        self.step(10, "DUT sends QueryIdentity with multiple selector fields present.",
                  expectation="TH responds INVALID_COMMAND.")
        self.ask_dut_to_send(
            "Using the DUT's interface, send QueryIdentity to the TH Network Identity Management server with more than one "
            f"selector field present, such as NetworkIdentityIndex={network_identity_index} together with "
            f"Identifier={network_identity_identifier_value.hex()}.",
            "the TH responded with INVALID_COMMAND.",
            expected_log=self.expect_command_status(commands.QueryIdentity.command_id, Status.InvalidCommand))
        ...
        self.ask_dut_to_send(
            "Using the DUT's interface, send QueryIdentity to the TH Network Identity Management server with an unknown "
            f"selector value, such as Identifier={unknown_identifier.hex()}.",
            "the TH responded with NOT_FOUND.",
