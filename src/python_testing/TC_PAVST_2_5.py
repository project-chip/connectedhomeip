...
class TC_PAVST_2_5(MatterBaseTest):
    ...
    async def test_TC_PAVST_2_5(self):
        ...
        self.step(3)
        try:
            await self.send_single_cmd(cmd=cluster.Commands.DeallocatePushTransport(ConnectionID=aConnectionID+1),
                                       endpoint=endpoint)
        except InteractionModelError as e:
        ...
        self.step(5)
        try:
            await self.send_single_cmd(cmd=cluster.Commands.DeallocatePushTransport(ConnectionID=aConnectionID),
                                       endpoint=endpoint)
        except InteractionModelError as e:
