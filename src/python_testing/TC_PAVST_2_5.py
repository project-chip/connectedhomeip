    ...
    async def test_TC_PAVST_2_5(self):
        ...
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            status = await self.send_single_cmd(
                cmd=pvcluster.Commands.DeallocatePushTransport({"connectionID": 10}),
                endpoint=endpoint,
            )
            asserts.assert_true(
                status,
                pvattr.Status.NOT_FOUND,
                "DUT responds with NOT_FOUND status code.",
            )
        ...
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            ...
            asserts.assert_true(
                status, pvattr.Status.Success, "DUT responds with SUCCESS status code."
            )
        ...
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            ...
            asserts.assert_greater_equal(
                len(transport_configs), 0, "TransportConfigurations must be empty!"
            )
