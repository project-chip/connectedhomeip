...
class TC_ACL_2_5(MatterBaseTest):
    ...
    def steps_TC_ACL_2_5(self) -> list[TestStep]:
        steps = [
            ...
            TestStep(2, "TH1 reads DUT Endpoint 0 OperationalCredentials cluster CurrentFabricIndex attribute", "Result is SUCCESS, value is a valid index"),
            TestStep(3, "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged events", "Result is SUCCESS, value is empty list"),
            ...
            TestStep(5, "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged events", "Result is SUCCESS, value is list of AccessControlExtensionChanged containing 1 element"),
            ...
            TestStep(7, "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged events", "Result is SUCCESS, value is list of AccessControlExtensionChanged containing at least 2 new elements"),
            TestStep(8, "TH1 writes DUT Endpoint 0 AccessControl cluster Extension attribute, value is list of AccessControlExtensionStruct containing 1 element", "Result is 0x87 (CONSTRAINT_ERROR)-Data value exceeds maximum length."),
            TestStep(9, "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged event", "Result is SUCCESS, value is list of AccessControlExtensionChanged containing at least 1 new element, and MUST NOT contain an added event for the extension with data that is too large"),
            TestStep(10, "TH1 writes DUT Endpoint 0 AccessControl cluster Extension attribute, value is list of AccessControlExtensionStruct containing 2 elements", "Result is 0x87 (CONSTRAINT_ERROR)-as there are more than 1 entry associated with the given accessing fabric index in the extension list"),
            TestStep(11, "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged event", "Result is SUCCESS, value is list of AccessControlExtensionChanged containing at least 1 new element, and MUST NOT contain an added event for the second extension"),
            TestStep(12, "TH1 writes DUT Endpoint 0 AccessControl cluster Extension attribute, value is an empty list", "Result is SUCCESS"),
            TestStep(13, "TH1 reads DUT Endpoint 0 AccessControl cluster AccessControlExtensionChanged event", "Result is SUCCESS, value is list of AccessControlExtensionChanged containing at least 1 new element"),
        ]
        return steps
    ...
    async def test_TC_ACL_2_5(self):
        ...
        extension = Clusters.AccessControl.Structs.AccessControlExtensionStruct(data=extension_data)
        ...
        try:
            ...
            result = await self.default_controller.WriteAttribute(
                self.dut_node_id, 
                [(0, extension_attr(value=extensions_list))]  # Use named parameter 'value'
            )
        ...
        self.print_step(f"Reading AccessControlExtensionChanged events after write...","")
            ...
            try:
                ...
                if len(events) > 0:
                    ...
                    for event_data in events:
                        ...
                        if (hasattr(event_data, 'Data') and 
                            ...
                            asserts.assert_in('chip.clusters.Types.Nullable', str(type(event_data.Data.adminPasscodeID)), "AdminPasscodeID should be Null")
                            asserts.assert_equal(event_data.Data.adminNodeID, self.default_controller.nodeId, "AdminNodeID should be the controller node ID")
                            asserts.assert_equal(event_data.Data.latestValue.data, b'\x17\x18', "LatestValue.Data should be 1718")
                            asserts.assert_equal(event_data.Data.latestValue.fabricIndex, f1, "LatestValue.FabricIndex should be the current fabric index")
                            asserts.assert_equal(event_data.Data.fabricIndex, f1, "FabricIndex should be the current fabric index")
                            found_match = True
                            break
                ...
                # If no events, wait and try again (up to 5 times)
                    ...
                    if attempt < max_attempts:
                        ...
                        asserts.fail("Did not receive AccessControlExtensionChanged event")
                ...
                if attempt < max_attempts:
                    ...
                    self.print_step("ERROR", "All attempts failed")
                    asserts.fail(f"Failed to read AccessControlExtensionChanged events: {e}")

        # After all attempts, check if we found a match
        if not found_match:
            asserts.fail("Did not receive AccessControlExtensionChanged event after multiple attempts")
        ...
        new_extension = Clusters.AccessControl.Structs.AccessControlExtensionStruct(data=new_extension_data)

        # Write the new extension
        self.print_step(f"Writing new extension with data", new_extension_data.hex())
        try:
        ...
        try:
            self.print_step(f"Attempting to write extension that exceeds max length (should fail)","")
        ...
        self.print_step(f"Reading events after failed write (too long extension)...","")
            ...
            for event_data in events:
                ...
                if hasattr(event_data, 'Data') and hasattr(event_data.Data, 'changeType'):
                    ...
                    if event_data.Data.changeType == Clusters.AccessControl.Enums.ChangeTypeEnum.kAdded:
                        ...
                        latest_value = getattr(event_data.Data, 'latestValue', None)
                        if latest_value and hasattr(latest_value, 'data'):
                            ...
                            if hasattr(latest_value.data, 'hex'):
                                ...
                                self.print_step(f"Checking latest data (hex)", latest_data_hex)
                            else:
                                self.print_step(f"Checking latest data", str(latest_value.data))
                                
                            too_long_data_hex = ""
                            if hasattr(too_long_data, 'hex'):
                                too_long_data_hex = too_long_data.hex()
                                self.print_step(f"Doesn't match too long data (hex)", too_long_data_hex)
                            else:
                                self.print_step(f"Doesn't match too long data", str(too_long_data))
        ...
        try:
            self.print_step(f"Attempting to write multiple extensions (should fail)","")
            # Create the Extension attribute
            extension_attr = Clusters.AccessControl.Attributes.Extension
            ...
            self.print_step(f"Error writing multiple extensions other than constraint error", str(e))
            asserts.fail(f"Failed to write multiple extensions due to other error: {e}")

        self.step(11)
        # Verify no event was generated for the second extension
        self.print_step(f"Reading events after failed write (multiple extensions)...","")
        ...
        try:
            ...
            asserts.assert_true(found_valid_event, "Found the expected ADD event with specified fields")
        ...
        self.print_step(f"Writing empty extension list to clear all extensions","")
        try:
            # Create the Extension attribute
            extension_attr = Clusters.AccessControl.Attributes.Extension
        ...
        for attempt in range(1, max_attempts + 1):
            try:
                ...
                if found_valid_event:
                    self.print_step(f"Found valid REMOVE event, proceeding","")
                    break
        ...
        asserts.assert_true(found_valid_event, "Did not find the expected REMOVE event with specified fields")
