...
class TC_PAVST_2_6(MatterBaseTest):
    ...
    def steps_TC_PAVST_2_6(self) -> list[TestStep]:
        return [
            ...
            TestStep(3, "TH1 sends the SetTransportStatus command with ConnectionID != aConnectionID.", "DUT responds with NOT_FOUND status code."),
            TestStep(4, "TH2 sends the SetTransportStatus command with ConnectionID = aConnectionID.", "DUT responds with NOT_FOUND status code."),
            TestStep(5, "TH1 sends the SetTransportStatus command with ConnectionID = aConnectionID.", "DUT responds with SUCCESS status code."),
            TestStep(6, "TH1 Reads CurrentConnections attribute from PushAV Stream Transport Cluster on DUT over a large-payload session", "Verify that the TransportStatus is set to !aTransportStatus in the TransportConfiguration corresponding to aConnectionID."),
        ]
