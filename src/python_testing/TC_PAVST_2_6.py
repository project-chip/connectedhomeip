...
class TC_PAVST_2_5(MatterBaseTest):
    ...
    async def test_TC_PAVST_2_5(self):
        ...
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            status = await self.send_single_cmd(
                cmd=pvcluster.Commands.SetTransportStatus(
                    {
                        "connectionID": aConnectionID,
                        "transportStatus": not aTransportOptions.transportStatus,
                    }
                ),
                endpoint=endpoint,
            )
        ...
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            ...
            asserts.assert_greater_equal(
                len(transport_configs), 1, "TransportConfigurations must be 1!"
            )
            asserts.assert_greater_equal(
                transport_configs.transportStatus
                == (not aTransportOptions.transportStatus),
