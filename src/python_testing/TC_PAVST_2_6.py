    ...
    async def test_TC_PAVST_2_5(self):
       ...
       self.step(6)
        if self.pics_guard(self.check_pics("PAVST.S.A0001")):
            ...
            asserts.assert_greater_equal(len(transport_configs), 1, "TransportConfigurations must be 1!")
            asserts.assert_greater_equal(transport_configs.transportStatus == !aTransportOptions.transportStatus , "Transport Status must be same the modified one")
