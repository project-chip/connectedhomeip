...
_TRIGGER_SET = 0x00A100000000000B
_TRIGGER_CLEAR = 0x00A100000000000C
_TRIGGER_ALL_CLEAR = 0x00A1000000000000

_ALARM_BIT = int(cluster.Bitmaps.AlarmBitmap.kUnderPower)


class TC_ESALM_3_6(MatterBaseTest):

    @run_if_endpoint_matches(has_feature(cluster, _F.kUnderPower))
    async def test_TC_ESALM_3_6(self):
        ...
        self.step(3, "TH reads Supported, Mask, Latch, and State",
                  expectation="SUCCESS. The UnderPower bit is set in Supported and enabled in Mask.")
        ...
        asserts.assert_true(int(supported) & _ALARM_BIT,
                            "UnderPower is not set in Supported, so this alarm cannot be tested")
        asserts.assert_true(int(mask) & _ALARM_BIT,
                            "UnderPower is not enabled in Mask, so the alarm would be suppressed")
        ...
        asserts.assert_false(int(current_state) & _ALARM_BIT,
                             "UnderPower is already active before the test starts")
        ...
        self.step(5, "TH sends TestEventTrigger to simulate the UnderPower alarm condition",
                  expectation="SUCCESS. Subscription report received with the UnderPower bit set in State.")
        state_sub.reset()
        event_sub.reset()
        ...
        state_sub.await_all_expected_report_matches(
            [AttributeMatcher.from_callable("State has the UnderPower bit set", state_has_alarm)],
            timeout_sec=30)

        self.step(6, "TH waits up to 30 seconds for Notify event",
                  expectation="Notify received. Active has the UnderPower bit, Inactive does not, State does, Mask matches.")
        ...
        asserts.assert_true(notify_event.active & _ALARM_BIT,
                            "Notify event: UnderPower not set in Active")
        asserts.assert_false(notify_event.inactive & _ALARM_BIT,
                             "Notify event: UnderPower unexpectedly set in Inactive")
        asserts.assert_true(notify_event.state & _ALARM_BIT,
                            "Notify event: UnderPower not set in State")
        ...
        self.step(7, "TH reads State",
                  expectation="DUT returns an AlarmBitmap with the UnderPower bit set.")
        state_val = await self.read_single_attribute_check_success(
            endpoint=endpoint, cluster=cluster, attribute=attrs.State)
        asserts.assert_true(int(state_val) & _ALARM_BIT, "State does not have the UnderPower bit set")

        self.step(8, "TH sends TestEventTrigger to clear the UnderPower alarm condition",
                  expectation="SUCCESS.")
        state_sub.reset()
        event_sub.reset()
        ...
        if not is_latched:
            self.step(9, "If non-latched: TH awaits subscription report with the UnderPower bit cleared",
                      expectation="Report received with the UnderPower bit at 0.")

            def state_cleared(report):
                return not bool(report.value & _ALARM_BIT)

            state_sub.await_all_expected_report_matches(
                [AttributeMatcher.from_callable("State has the UnderPower bit cleared", state_cleared)],
                timeout_sec=30)

            self.step(10, "If non-latched: TH waits up to 30 seconds for Notify event",
                      expectation="Notify received. Inactive has the UnderPower bit, Active does not, State does not.")
            ...
            asserts.assert_true(clear_event.inactive & _ALARM_BIT,
                                "Notify event: UnderPower not set in Inactive on clear")
            asserts.assert_false(clear_event.active & _ALARM_BIT,
                                 "Notify event: UnderPower unexpectedly set in Active on clear")
            asserts.assert_false(clear_event.state & _ALARM_BIT,
                                 "Notify event: UnderPower still set in State on clear")

            self.step(11, "If latched: TH reads State",
                      expectation="The UnderPower bit remains set until Reset.")
            self.mark_current_step_skipped()

            self.step(12, "If latched and Reset supported: TH sends Reset for the UnderPower alarm",
                      expectation="SUCCESS. Subscription report received with the UnderPower bit cleared.")
            self.mark_current_step_skipped()
        else:
            self.step(9, "If non-latched: TH awaits subscription report with the UnderPower bit cleared",
                      expectation="Report received with the UnderPower bit at 0.")
            ...
            asserts.assert_true(int(state_val) & _ALARM_BIT,
                                "Latched alarm: UnderPower should still be set in State after the condition clears")
