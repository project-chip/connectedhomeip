...
class TC_HSTAT_2_5(MatterBaseTest):
    ...
    def steps_TC_HSTAT_2_5(self):
        return [
            ...
            TestStep(3, "TH sends command SetSettings with the Mode field set to Humidifier", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep(4, "TH sends command SetSettings with the Mode field set to Humidifier", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep(5, "TH sends command SetSettings with only the MistWarm bit of the MistType field set", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep(6, "TH reads from the DUT the MistType attribute.", "Verify that the DUT response contains a value with the MistWarm bit set. Store the value as MistSetting"),
            TestStep(7, "TH writes to the DUT the MistType attribute with only the MistCold bit set", "Verify DUT responds w/ status SUCCESS(0x00)"),
            TestStep(8, "TH reads from the DUT the MistType attribute.", "Verify that the DUT response contains a value with the MistCold bit set."),
            TestStep(9, "Individually subscribe to the MistType attribute", "This will receive updates when these attributes change value."),
            TestStep(10, "TH writes to the DUT the MistType attribute with only the MistWarm bit of the MistType field set", "Verify that the DUT response contains a value with only the MistWarm bit set. Verify that an attribute report was received for MistType and that the value received has only the MistWarm bit set."),
            TestStep(11, "TH sends command SetSettings with the MistType field set to MistWarm", "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)"),
            TestStep(12, "TH sends command SetSettings with the MistType field set to MistCold", "Verify DUT responds w/ status CONSTRAINT_ERROR(0x87)"),
            TestStep(13, "TH sends command Off to the On/Off cluster on the same endpoint as this cluster.", "Verify DUT responds w/ status SUCCESS(0x00)"),
