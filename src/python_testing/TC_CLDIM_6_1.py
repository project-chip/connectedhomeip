...
class TC_CLDIM_6_1(MatterBaseTest):
    ...
    def desc_TC_CLDIM_6_1(self) -> str:
        return "[TC-CLDIM-6.1] GroupedStep Command Primary Functionality with DUT as Server"
    ...
    def steps_TC_CLDIM_6_1(self) -> list[TestStep]:
        return [
            ...
            TestStep("4d", "Send GroupedStep command to increase position by 2 steps"),
            ...
            TestStep("5b", "Send GroupedStep command to decrease position by 1 step with Speed=High"),
            ...
            TestStep("6b", "Send GroupedStep command to increase position by 1 step with Speed=Auto"),
            ...
            TestStep("7b", "Send GroupedStep command to decrease position by 65535"),
            TestStep("7c", "Verify TargetState attribute is at MinPosition"),
            TestStep("7d", "Wait for CurrentState to be updated"),
            TestStep("7e", "Send GroupedStep command to increase position by 65535"),
    ...
    async def test_TC_CLDIM_6_1(self):
        ...
        sub_handler.reset()
        if self.groupcast_enabled:
            log.info("Sending GroupedStep command by groupcast")
            ...
            log.info("Sending GroupedStep command by unicast")
            await self.send_single_cmd(
