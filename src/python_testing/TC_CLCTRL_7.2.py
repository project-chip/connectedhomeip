...
class TC_CLCTRL_7_2(MatterBaseTest):
    ...
    def desc_TC_CLCTRL_7_2(self) -> str:
        return "[TC-CLCTRL-7.2] GroupedMoveTo Command Latching Functionality with DUT as Server"

    def steps_TC_CLCTRL_7_2(self) -> list[TestStep]:
        return [
            ...
            TestStep("2h", "Send GroupedMoveTo command with Latch = False", "Receive SUCCESS response from the DUT"),
            ...
            TestStep("3d", "Send GroupedMoveTo command with Latch = False", "Receive INVALID_IN_STATE error from the DUT"),
            ...
            TestStep("4g", "Send GroupedMoveTo command with Latch = False", "Receive INVALID_IN_STATE error from the DUT"),
    ...
    def pics_TC_CLCTRL_7_2(self) -> list[str]:
        return [
            "CLCTRL.S", "CLCTRL.S.F01", "CLCTRL.S.C03.Rsp"
        ]
        ...
        if latch_control_modes != 0:
            ...
            try:
                ...
                asserts.assert_equal(e.status, Status.InvalidInState,
                                     f"GroupedMoveTo command with Latch = True failed with unexpected status: {e.status}")
            ...
            try:
                ...
                asserts.assert_equal(e.status, Status.InvalidInState,
                                     f"GroupedMoveTo command with Latch = False failed with unexpected status: {e.status}")
                log.info("Received INVALID_IN_STATE error as expected")
            self.step("3e")
