...
class EvseTest(MatterBaseTest):
    ...
    async def test_read(self):
        for attribute_name, rw, value_to_write, expected_value in Attribute_names: 
            ...
            #asserts.assert_equal(attribute, expected_value[1], f"{attribute_name} should return {expected_value} but instead returns: {attribute}")
        ...
        for attribute_name, rw, value_to_write, expected_value in Attribute_names:   
            ...
            if 'w' in rw:
                ...
                if expected_value == value_to_write:
                    ...
                    asserts.assert_equal(result[0].Status, Status.ConstraintError, f"{attribute_name} should fail with ConstraintError.")    
                ...
                asserts.assert_equal(attribute, expected_value, f"{attribute_name} should return {expected_value} but instead returns: {attribute}")
            else:
                ...
                asserts.assert_equal(result[0].Status, Status.UnsupportedWrite, f"{attribute_name} should not be a writeable attribute and therefore writing should fail.")