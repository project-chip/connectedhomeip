...
SET_TRIGGER = 0x00A3_0000_0000_0002
CLEAR_TRIGGER = 0x00A3_0000_0000_0012

EVENT_TIMEOUT_SEC = 10.0
SETTLE_SEC = 2.0


class TC_EPALM_3_2(MatterBaseTest):
    ...
    def steps_TC_EPALM_3_2(self) -> list[TestStep]:
        return [
            ...
            TestStep("1c", "TH reads the Supported attribute",
                     "Bit 1 is set, so the DUT supports OverLoadFault."),
            TestStep("1d", "TH reads TestEventTriggersEnabled from General Diagnostics on endpoint 0",
                     "Value is 1 (True)."),
            TestStep("1e", "TH sends the TestEventTrigger clearing OverLoadFault, returning the DUT to its "
                           "no-fault baseline", "DUT returns SUCCESS."),
            TestStep("2a", "TH sends the TestEventTrigger setting OverLoadFault",
                     "DUT returns SUCCESS and reports a Notify event with bit 1 set in Active and "
                     "set in State."),
            TestStep("2b", "TH reads the State attribute", "Bit 1 is 1."),
            TestStep("3a", "TH sends the TestEventTrigger clearing OverLoadFault",
                     "DUT returns SUCCESS and reports a Notify event with bit 1 set in Inactive and "
                     "clear in State."),
            TestStep("3b", "TH reads the State attribute", "Bit 1 is 0."),
        ]
