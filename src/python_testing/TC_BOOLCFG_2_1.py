...
class TC_BOOLCFG_2_1(MatterBaseTest):
    ...
    async def test_TC_BOOLCFG_2_1(self):
        ...
        if attributes.SupportedSensitivityLevels.attribute_id in attribute_list:
            number_of_supported_levels = await self.read_ts_attribute_expect_success(endpoint=endpoint, attribute=attributes.SupportedSensitivityLevels)
        ...
        if attributes.CurrentSensitivityLevel.attribute_id in attribute_list:
            current_sensitivity_level_dut = await self.read_ts_attribute_expect_success(endpoint=endpoint, attribute=attributes.CurrentSensitivityLevel)
        ...
        if attributes.DefaultSensitivityLevel.attribute_id in attribute_list:
            default_sensitivity_level_dut = await self.read_ts_attribute_expect_success(endpoint=endpoint, attribute=attributes.DefaultSensitivityLevel)
        ...
        if attributes.AlarmsSupported.attribute_id in attribute_list:
            alarms_supported_dut = await self.read_ts_attribute_expect_success(endpoint=endpoint, attribute=attributes.AlarmsSupported)
        ...
        if attributes.SensorFault.attribute_id in attribute_list:
            sensor_fault_dut = await self.read_ts_attribute_expect_success(endpoint=endpoint, attribute=attributes.SensorFault)
