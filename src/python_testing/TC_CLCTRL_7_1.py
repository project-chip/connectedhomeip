...
class TC_CLCTRL_7_1(MatterBaseTest):
    ...
    def desc_TC_CLCTRL_7_1(self) -> str:
        return "[TC-CLCTRL-7.1] GroupedMoveTo Command Position Functionality with DUT as Server"
    ...
    def steps_TC_CLCTRL_7_1(self) -> list[TestStep]:
        return [
            ...
            TestStep("3j", "TH sends command GroupedMoveTo with Latch = True."),
            ...
            TestStep("4a", "TH sends command GroupedMoveTo with Position = MoveToFullyOpen."),
            TestStep("5a", "If LatchControlModes Bit 1 = 0 (RemoteUnlatching = False), skip step 5b."),
            TestStep("5b", "TH sends command GroupedMoveTo with Latch = False."),
            ...
            TestStep("6d", "TH sends command GroupedMoveTo with Speed = Low."),
            TestStep("6e", "Wait until TH receives a subscription report with OverallCurrentState.Speed = Low."),
            TestStep("7a", "TH sends command GroupedMoveTo with Position = MoveToSignaturePosition."),
            ...
            TestStep("8d", "TH sends command GroupedMoveTo with Position = MoveToFullyClosed."),
            TestStep("8e", "Wait until TH receives a subscription report with OverallCurrentState.Position = FullyClosed."),
            TestStep("8f", "TH sends command GroupedMoveTo with Position = MoveToVentilationPosition."),
            ...
            TestStep("9d", "TH sends command GroupedMoveTo with Position = MoveToFullyClosed."),
            TestStep("9e", "Wait until TH receives a subscription report with OverallCurrentState.Position = FullyClosed."),
            TestStep("9f", "TH sends command GroupedMoveTo with Position = MoveToPedestrianPosition."),
            ...
            TestStep("10c", "TH sends command GroupedMoveTo with Position = MoveToFullyClosed."),
            TestStep("10d", "Wait until TH receives a subscription report with OverallCurrentState.Position = FullyClosed."),
            TestStep("10e", "TH sends command GroupedMoveTo with Position = MoveToFullyOpen."),
            ...
            TestStep("11a", "TH sends command GroupedMoveTo with Position = MoveToFullyClosed."),
            ...
            TestStep("12b", "TH sends command GroupedMoveTo with Speed = High."),
            ...
            TestStep("12e", "TH sends command GroupedMoveTo with Speed = Low."),
            ...
            TestStep("13d", "TH sends command GroupedMoveTo with Position = MoveToFullyOpen."),
            ...
            TestStep("14d", "TH sends command GroupedMoveTo with Position = MoveToFullyOpen."),
            ...
            TestStep("15b", "TH sends command GroupedMoveTo with Position = MoveToFullyOpen."),
            ...
            TestStep("15f", "TH sends command GroupedMoveTo with Position = MoveToFullyClosed."),
    ...
    def pics_TC_CLCTRL_7_1(self) -> list[str]:
        return [
            "CLCTRL.S", "CLCTRL.S.F00", "CLCTRL.S.C03.Rsp"
        ]
    ...
    async def test_TC_CLCTRL_7_1(self):
        ...
        if CurrentPosition == Clusters.ClosureControl.Enums.CurrentPositionEnum.kFullyClosed:
            log.info("CurrentPosition is FullyClosed, skipping steps 3c to 3d")

            # Skipping steps 3c and 3d
            self.skip_step("3c")
            self.skip_step("3d")
        else:
            # STEP 3c: TH sends command GroupedMoveTo with Position = MoveToFullyClosed
            self.step("3c")
        ...
        if is_lt_feature_supported:
            ...
            if CurrentLatch:
                log.info("CurrentLatch is True, skipping steps 3i to 3m")

                # Skipping steps 3i to 3m
                self.mark_step_range_skipped("3i", "3m")
            else:
                log.info("CurrentLatch is False, proceeding to steps 3i to 3m")
                ...
                if not LatchControlModes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteLatching:
                    log.info("LatchControlModes Bit 0 is 0 (RemoteLatching = False), skipping step 3j")
                    self.skip_step("3j")

                    # STEP 3k: If LatchControlModes Bit 0 = 1 (RemoteLatching = True), skip step 3l
                    self.step("3k")

                    log.info("RemoteLatching is False, proceeding to step 3l")

                    # STEP 3l: Latch the DUT manually to set OverallCurrentState.Latch to True
                    self.step("3l")

                    log.info("Latch the DUT manually to set OverallCurrentState.Latch to True")
                    ...
                    log.info("Manual latching completed.")
                else:
                    log.info("LatchControlModes Bit 0 is 1 (RemoteLatching = True), proceeding to step 3j")
                ...
                log.info("Waiting for OverallCurrentState.Latch to be True")
            ...
            # STEP 4a: TH sends command GroupedMoveTo with Position = MoveToFullyOpen
            self.step("4a")
            ...
            try:
                ...
                asserts.assert_equal(
                    e.status, Status.InvalidInState, f"The GroupedMoveTo command sends an incorrect state: {e.status}")
                pass
            ...
            if not LatchControlModes & Clusters.ClosureControl.Bitmaps.LatchControlModesBitmap.kRemoteUnlatching:
                log.info("LatchControlModes Bit 1 is 0 (RemoteUnlatching = False), skipping step 5b")
                self.skip_step("5b")

                # STEP 5c: If LatchControlModes Bit 1 = 1 (RemoteUnlatching = True), skip step 5d
                self.step("5c")

                log.info("RemoteUnlatching is False, proceeding to step 5d")
                ...
                log.info("LatchControlModes Bit 1 is 1 (RemoteUnlatching = True), proceeding to step 5b")
            ...
            log.info("Motion Latching feature not supported, skipping steps 3f to 5e")
        ...
        if not is_sp_feature_supported:
            log.info("Speed feature not supported, skipping steps 6b to 6e")

            # Skipping steps 6b to 6e
            self.mark_step_range_skipped("6b", "6e")
        else:
            log.info("Speed feature supported.")
            ...
            if CurrentSpeed == Clusters.Globals.Enums.ThreeLevelAutoEnum.kLow:
                log.info("CurrentSpeed is Low, skipping steps 6d to 6e")

                # Skipping steps 6d and 6e
                self.skip_step("6d")
                self.skip_step("6e")
            else:
                log.info("CurrentSpeed is not Low, proceeding to steps 6d and 6e")

                # STEP 6d: TH sends command GroupedMoveTo with Speed = Low
                self.step("6d")
                ...
                log.info("Waiting for OverallCurrentState.Speed to be Low")
        ...
        # STEP 7a: TH sends command GroupedMoveTo with Position = MoveToSignaturePosition
        self.step("7a")
        ...
        log.info("Waiting for OverallCurrentState.Position to be OpenedAtSignature")
        ...
        if not is_vt_feature_supported:
            log.info("Ventilation feature not supported, skipping steps 8b to 8i")
            ...
            if CurrentPosition == Clusters.ClosureControl.Enums.CurrentPositionEnum.kFullyClosed:
                log.info("CurrentPosition is FullyClosed, skipping steps 8d to 8e")

                # Skipping steps 8d and 8e
                self.mark_step_range_skipped("8d", "8e")
            else:
                log.info("CurrentPosition is not FullyClosed, proceeding to steps 8d and 8e")

                # STEP 8d: TH sends command GroupedMoveTo with Position = MoveToFullyClosed
                self.step("8d")
            ...
            # STEP 8f: TH sends command GroupedMoveTo with Position = MoveToVentilationPosition
            self.step("8f")
            ...
            log.info("Waiting for OverallCurrentState.Position to be OpenedForVentilation")
        ...
        if not is_pd_feature_supported:
            log.info("Pedestrian feature not supported, skipping steps 9b to 9i")

            # Skipping steps 9b to 9i
            self.mark_step_range_skipped("9b", "9i")
        else:
            log.info("Pedestrian feature supported.")
            ...
            if CurrentPosition == Clusters.ClosureControl.Enums.CurrentPositionEnum.kFullyClosed:
                log.info("CurrentPosition is FullyClosed, skipping steps 9d to 9e")

                # Skipping steps 9d and 9e
                self.mark_step_range_skipped("9d", "9e")
            else:
                log.info("CurrentPosition is not FullyClosed, proceeding to steps 9d and 9e")

                # STEP 9d: TH sends command GroupedMoveTo with Position = MoveToFullyClosed
                self.step("9d")
            ...
            # STEP 9f: TH sends command GroupedMoveTo with Position = MoveToPedestrianPosition
            self.step("9f")
            ...
            log.info("Waiting for OverallCurrentState.Position to be OpenedForPedestrian")
        ...
        if CurrentPosition == Clusters.ClosureControl.Enums.CurrentPositionEnum.kFullyClosed:
            log.info("CurrentPosition is FullyClosed, skipping steps 10c to 10d")
            self.mark_step_range_skipped("10c", "10d")
        else:
            log.info("CurrentPosition is not FullyClosed, proceeding to steps 10c and 10d")

            # STEP 10c: TH sends command GroupedMoveTo with Position = MoveToFullyClosed
            self.step("10c")
        ...
        # STEP 10e: TH sends command GroupedMoveTo with Position = MoveToFullyOpen
        self.step("10e")
        ...
        log.info("Waiting for OverallCurrentState.Position to be FullyOpened")
        ...
        # STEP 11a: TH sends command GroupedMoveTo with Position = MoveToFullyClosed
        self.step("11a")
        ...
        if not is_sp_feature_supported:
            log.info("Speed feature not supported, skipping steps 12b to 12g")

            # Skipping steps 12b to 12g
            self.mark_step_range_skipped("12b", "12g")
        else:
            # STEP 12b: TH sends command GroupedMoveTo with Speed = High
            self.step("12b")
            ...
            log.info("Waiting for OverallCurrentState.Speed to be High")
            ...
            # STEP 12e: TH sends command GroupedMoveTo with Speed = Low
            self.step("12e")
        ...
        if is_pt_feature_supported:
            ...
            # STEP 13d: TH sends command GroupedMoveTo with Position = MoveToFullyOpen
            self.step("13d")
        ...
        if is_mo_feature_supported:
            ...
            # STEP 14d: TH sends command GroupedMoveTo with Position = MoveToFullyOpen
            self.step("14d")
            ...
            log.info("ManuallyOperable feature not supported, skipping steps 14b to 14f")
        ...
        if is_is_feature_supported:
            # STEP 15b: TH sends command GroupedMoveTo with Position = MoveToFullyOpen
            self.step("15b")
            ...
            # STEP 15f: TH sends command GroupedMoveTo with Position = MoveToFullyClosed
            self.step("15f")
            ...
            log.info("Instantaneous feature not supported, skipping steps 15b to 15i")
