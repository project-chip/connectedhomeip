...
bool HandleOption(const char * progName, OptionSet * optSet, int id, const char * name, const char * arg)
{
    ...
    case 'i':
        if (!ParseInt(arg, gIterationCount) ||
            !(gIterationCount >= chip::kMin_PBKDF_Iterations && gIterationCount <= chip::kMax_PBKDF_Iterations))
        {
    ...
    case 'l':
        if (!ParseInt(arg, gSaltLen) || !(gSaltLen >= chip::kMin_PBKDF_Salt_Length && gSaltLen <= chip::kMax_PBKDF_Salt_Length))
        {
    ...
    case 's':
        gSalt = arg;
        if (!(strlen(gSalt) >= chip::kMin_PBKDF_Salt_Length && strlen(gSalt) <= chip::kMax_PBKDF_Salt_Length))
        {
...
bool Cmd_GenVerifier(int argc, char * argv[])
{
    ...
    {
        uint8_t salt[chip::kMax_PBKDF_Salt_Length];
        if (gSalt == nullptr)
        {
        ...
        char saltB64[BASE64_ENCODED_LEN(chip::kMax_PBKDF_Salt_Length) + 1];
