...
1. The expected install location on the RPi is the home directory of the user
   specified in `idt/scripts/vars.sh`, which will be generated by running a script in the next section.
...
1. Ensure the RPi is connected to your network, either via ethernet or with Wi-Fi preconfigured in the disk image.
1. Boot the RPi.
...
1. `ssh` to the RPi, e.g. with `MobaXterm`
    - **_NOTE:_** You may need to wait a few minutes after boot for the `ssh`
      server to be available on the RPi. Retry if needed!
    - Use `$PIUSER@$PIHOST` or `$PIUSER@$ip` where `$ip` is the RPi's IP
