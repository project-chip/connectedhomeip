...
class CaptureImplsLoader:
    ...
    def __init__(self, root_dir: str, root_package: str, search_type: type, logger=_LOGGER):
        self.logger = logger
    ...
    def is_package(potential_package: str) -> bool:
        init_path = os.path.join(potential_package, "__init__.py")
        ...
        for item in dir(self.search_type):
            ...
            if inspect.iscoroutinefunction(item_attr):
                if not hasattr(subclass, item):
                    self.logger.error(f"Missing coroutine attr {subclass}.{item}")
                    return False
                if not inspect.iscoroutinefunction(getattr(subclass, item)):
                    self.logger.error(f"{subclass}.{item} is not a coroutine, but it should be")
                    return False
        for item in dir(subclass):
            ...
            if inspect.iscoroutinefunction(item_attr) and hasattr(self.search_type, item):
                if not inspect.iscoroutinefunction(getattr(self.search_type, item)):
                    self.logger.error(f"{subclass}.{item} is a coroutine, when it should not be")
                    return False
        return True

    def verify_arguments(self, subclass) -> bool:
        arguments_match = True
        for item in dir(self.search_type):
            item_attr = getattr(self.search_type, item)
            if inspect.isfunction(item_attr):
                if not hasattr(subclass, item):
                    self.logger.error(f"Missing attr {subclass}.{item}()")
                    arguments_match = False
                if not inspect.isfunction(getattr(subclass, item)):
                    self.logger.error(f"Missing function {subclass}.{item}()")
                expected_signature = inspect.signature(item_attr)
    ...
    def is_type_match(self, potential_class_match: Any) -> bool:
        if inspect.isclass(potential_class_match):
            ...
            if issubclass(potential_class_match, self.search_type):
                ...
                if self.verify_coroutines(potential_class_match) and self.verify_arguments(potential_class_match):
                    return True
        return False
    ...
    def load_module(self, to_load):
        ...
        if saw_one_impl and not saw_more_than_one_impl:
            ...
            self.logger.error(f"more than one impl in {module_item}")
        if not saw_one_impl:
            self.logger.error(f"No impl found in {module_item}")
        ...
        for item in os.listdir(self.root_dir):
            ...
            if self.is_package(dir_content):
                ...
                try:
                    ...
                    self.logger.error(f"No module matching package name for {item}\n{traceback.format_exc()}")
