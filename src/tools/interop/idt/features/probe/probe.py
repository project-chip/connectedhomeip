...
class GenericMatterProber(ABC):
    ...
    def run_command(self, cmd: str, capture_output=False) -> Bash:
        Bash(f"echo {cmd} >> {self.output}", sync=True, capture_output=True).start_command()
        cmd = f" {cmd} 2>&1 | tee -a {self.output}"
    ...
    def discover_targets_by_browsing(self) -> None:
        browser = MatterDnssdListener(self.dnssd_artifact_dir, None, None)
        self.targets = asyncio.run(browser.browse_once(config.DNSSD_BROWSING_TIME_SECONDS))
    ...
    def probe_single_target(self, target: ProbeTarget) -> None:
        ...
        elif is_ipv6_link_local(target.ip):
