...
class MatterBleScanner:
    ...
    def __init__(self, artifact_dir: str, vendor_id: str | None, product_id: str | None):
        ...
        self.vendor_id = None if not vendor_id else vendor_id.lstrip("0x")
        self.product_id = None if not product_id else product_id.lstrip("0x")
    ...
    def parse_vid_pid_for_filter(self, loggable_data: str) -> tuple[str, str]:
        try:
            vid = loggable_data[8:10] + loggable_data[6:8]
            pid = loggable_data[12:14] + loggable_data[10:12]
        except IndexError:
            self.logger.warning("Error parsing vid / pid from BLE ad data")
            return "", ""
        return vid.lstrip("0"), pid.lstrip("0")
    ...
    def passing_filter(self, hex_service_data) -> bool:
        vid, pid = self.parse_vid_pid_for_filter(hex_service_data)
        if self.vendor_id:
            if vid != self.vendor_id:
                self.logger.debug(f"Filtered ad for vid {vid} not matching vid {self.vendor_id}")
                return False
            if self.product_id and pid != self.product_id:
                self.logger.debug(f"Filtered ad for pid {pid} not matching pid {self.product_id}")
                return False
        return True
    ...
    def log_ble_discovery(
        ...
        if self.is_matter_device(name):
            if not self.passing_filter(hex_service_data):
                return
