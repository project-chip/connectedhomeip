...
class Android(PlatformLogStreamer):
    ...
    def __init__(self, artifact_dir: str) -> None:
        ...
        self.adb_devices: typing.Dict[str, bool] = {}  # Key: Device id, Value: Is authorized
        self.capabilities: None | Capabilities = None
        self.streams = {}
        self.connected = False
        self.hci_out_path = os.path.join(self.artifact_dir, "btsnoop_hci.log")
    ...
    def _get_adb_devices(self) -> typing.Dict[str, bool]:
    ...
    def _selected_device_connected_and_authd(self) -> bool:
        current_devices = self._get_adb_devices()
        return self.device_id in current_devices and current_devices[self.device_id]
    ...
    def _authorize_adb(self) -> None:
        ...
        while not self._selected_device_connected_and_authd():
    ...
    async def _handle_stream_action(self, action: str) -> None:
        had_error = False
    ...
    async def start_streaming(self) -> None:
        await self._handle_stream_action("start")
    ...
    def _pull_hci_logs(self) -> None:
        for possible_hci_path in config.HCI_LOG_POSSIBLE_PATHS:
            pull_command = self.run_adb_command(f"pull {possible_hci_path} {self.hci_out_path}")
            if pull_command.finished_success():
                self.logger.info("Successfully pulled hci logs!")
                return
        self.logger.error("Could not locate hci logs!")

    async def stop_streaming(self) -> None:
        await self._handle_stream_action("stop")
        if config.ENABLE_BUG_REPORT:
            self.logger.info("Taking bugreport")
            self.run_adb_command("bugreport", cwd=self.artifact_dir)
        else:
            self.logger.warning("bugreport disabled in settings!")
        self.logger.info("Attempting to pull hci logs!")
        self._pull_hci_logs()
