...
enum KeyFormat
{
    ...
    kKeyFormat_X509_DER,
    kKeyFormat_X509_PEM,
    kKeyFormat_X509_DER_PKCS8,
    kKeyFormat_X509_PEM_PKCS8,
    kKeyFormat_Chip_Raw,
...
extern bool ReadPrivateKey(const char * fileName, EVP_PKEY *& key);
extern bool WritePrivateKey(const char * fileName, EVP_PKEY * key, KeyFormat keyFmt);

extern bool X509ToChipCert(X509 * cert, uint8_t *& chipCert, uint32_t & chipCertLen);

extern bool InitOpenSSL();
extern char * Base64Encode(const uint8_t * inData, uint32_t inDataLen);
extern uint8_t * Base64Encode(const uint8_t * inData, uint32_t inDataLen, uint8_t * outBuf, uint32_t outBufSize,
                              uint32_t & outDataLen);
extern uint8_t * Base64Decode(const uint8_t * inData, uint32_t inDataLen, uint8_t * outBuf, uint32_t outBufSize,
                              uint32_t & outDataLen);
...
extern bool OpenFileToWrite(const char * fileName, FILE *& file);
extern void CloseFile(const char * fileName, FILE *& file);
