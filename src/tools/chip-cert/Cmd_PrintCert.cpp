...
static void Indent(FILE * file, int count)
{
    while (count--)
    {
        fputc(' ', file);
...
static void PrintHexField(FILE * file, const char * name, int indent, uint16_t count, const uint8_t * data, int countPerRow = 16)
{
    Indent(file, indent);
...
static void PrintEpochTime(FILE * file, const char * name, int indent, uint32_t epochTime)
{
...
static void PrintDN(FILE * file, const char * name, int indent, const ChipDN * dn)
{
...
static bool PrintCert(const char * fileName, X509 * cert)
{
    bool res       = true;
...
bool Cmd_PrintCert(int argc, char * argv[])
{
    ...
    res = PrintCert(gOutFileName, cert);
    VerifyTrueOrExit(res);
