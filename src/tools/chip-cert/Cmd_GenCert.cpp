...
static ToolChipDN gSubjectDN;
static uint8_t gCertType             = kCertType_NotSpecified;
static bool gSelfSign                = false;
static const char * gCACertFileName  = nullptr;
static const char * gCAKeyFileName   = nullptr;
static const char * gInKeyFileName   = nullptr;
static const char * gOutCertFileName = nullptr;
static const char * gOutKeyFileName  = nullptr;
static CertFormat gOutCertFormat     = kCertFormat_Chip_Base64;
static KeyFormat gOutKeyFormat       = kKeyFormat_Chip_Base64;
static uint32_t gValidDays           = 0;
static struct tm gValidFrom;
static const char * gFutureExtensionSub  = nullptr;
static const char * gFutureExtensionInfo = nullptr;
...
bool Cmd_GenCert(int argc, char * argv[])
{
    ...
    {
        res = ReadPrivateKey(gInKeyFileName, newKey);
        VerifyTrueOrExit(res);

        // Verify a supported curve is used in the specified private key.
        if (EC_GROUP_get_curve_name(EC_KEY_get0_group(EVP_PKEY_get1_EC_KEY(newKey))) != OBJ_sn2nid("prime256v1"))
        {
    ...
    res = MakeCert(gCertType, &gSubjectDN, caCert, caKey, gValidFrom, gValidDays, newCert, newKey, gFutureExtensionSub,
                   gFutureExtensionInfo);
    VerifyTrueOrExit(res);

    res = WriteCert(gOutCertFileName, newCert, gOutCertFormat);
    VerifyTrueOrExit(res);

    if (gOutKeyFileName != nullptr)
    {
        res = WritePrivateKey(gOutKeyFileName, newKey, gOutKeyFormat);
        VerifyTrueOrExit(res);
    }
...
bool HandleOption(const char * progName, OptionSet * optSet, int id, const char * name, const char * arg)
{
    ...
    case 'c':
        ...
        break;
    case 'x':
        gFutureExtensionSub = arg;
        break;
    case '2':
        gFutureExtensionInfo = arg;
        break;
    case 'k':
        gInKeyFileName = arg;
        break;
