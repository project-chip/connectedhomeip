...
public:
    ...
    /**
     *   Called when the commissioning process is complete (with success or error)
     *   Should be override by PairingDelegates which are JF compatible
     *   - trustedIcacPublicKeyB has a value only when Joint Commissioning Method is executed
     */
    virtual void OnCommissioningComplete(NodeId deviceId, Optional<Crypto::P256PublicKey> trustedIcacPublicKeyB, CHIP_ERROR error)
    {
