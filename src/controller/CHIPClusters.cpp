...
CHIP_ERROR ApplicationLauncherCluster::LaunchApp(Callback::Cancelable * onSuccessCallback, Callback::Cancelable * onFailureCallback, uint16_t catalogVendorId, chip::ByteSpan applicationId, chip::ByteSpan data)
{
...
#else
    uint8_t seqNum = mDevice->GetNextSequenceNumber();
    System::PacketBufferHandle encodedCommand = encodeApplicationLauncherClusterLaunchAppCommand(seqNum, mEndpoint, catalogVendorId, applicationId, data);
    return SendCommand(seqNum, std::move(encodedCommand), onSuccessCallback, onFailureCallback);
#endif
}
...
CHIP_ERROR KeypadInputCluster::SendKey(Callback::Cancelable * onSuccessCallback, Callback::Cancelable * onFailureCallback, uint8_t keyAction, uint8_t keyCode)
{
#ifdef CHIP_APP_USE_INTERACTION_MODEL
    VerifyOrReturnError(mDevice != nullptr, CHIP_ERROR_INCORRECT_STATE);
    (void) onSuccessCallback;
(void) onFailureCallback;
    ...
    uint8_t argSeqNumber = 0;
    // keyAction: keypadInputKeyAction
