...
class ChipStack(object):
    def __init__(self, installDefaultLogHandler=True, bluetoothAdapter=0):
        ...
        self._ChipStackLib.pychip_DeviceController_SetChipThreadRunnerCallback(self.cbHandleChipThreadRun)
    ...
    def PostTaskOnChipThread(self, callFunct) -> CHIPAsyncCallableHandle:
        '''Run a Python function on CHIP stack, and wait for the response.
        ...
        '''
        callObj = CHIPAsyncCallableHandle(callFunct)
        pythonapi.Py_IncRef(py_object(callObj))
        res = self._ChipStackLib.pychip_DeviceController_PostTaskOnChipThread(py_object(callObj))
        if res != 0:
    ...
    def _loadLib(self):
        if self._ChipStackLib is None:
            ...
            self._ChipStackLib.pychip_DeviceController_SetChipThreadRunnerCallback.argtypes = [_ChipThreadTaskRunnerFunct]
            self._ChipStackLib.pychip_DeviceController_SetChipThreadRunnerCallback.restype = None

            self._ChipStackLib.pychip_DeviceController_PostTaskOnChipThread.argtypes = [py_object]
