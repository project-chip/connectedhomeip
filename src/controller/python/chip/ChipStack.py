...
__all__ = [ 'DeviceStatusStruct', 'ChipStackException', 'DeviceError', 'ChipStackError', 'ChipStack']
...
class ChipStack(object):
    ...
    def Shutdown(self):
        self._ChipStack.Call(
            lambda: self._dmLib.nl_Chip_Stack_Shutdown()
        )
    ...
    def ErrorToException(self, err, devStatusPtr=None):
        if (err == 4044 and devStatusPtr):
            ...
            msg = ChipUtility.CStringToString((self._ChipStackLib.nl_Chip_Stack_StatusReportToString(devStatus.ProfileId, devStatus.StatusCode)))
            ...
            return ChipStackError(err, ChipUtility.CStringToString((self._ChipStackLib.nl_Chip_Stack_ErrorToString(err))))
    ...
    def LocateChipDLL(self):
        ...
        relDMDLLPathGlob = os.path.join('build', buildMachineGlob, 'src/controller/python/.libs', ChipStackDLLBaseName)
        ...
        raise Exception("Unable to locate Chip Device Manager DLL (%s); expected location: %s" % (ChipStackDLLBaseName, scriptDir))
    ...
    def _loadLib(self):
        if (self._ChipStackLib == None):
            ...
            self._ChipStackLib.nl_Chip_Stack_StatusReportToString.argtypes = [ c_uint32, c_uint16 ]
