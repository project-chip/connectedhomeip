...
def on_ice(candidates, peer):
    logging.info(f"on_ice received ICE candidates: {candidates}")
    if future_ice is not None:
        future_ice.set_result(candidates)


def on_offer(offer, peer):
    logging.info(f"on_offer received SDP offer: {offer}")
...
def on_answer(answer, peer):
    logging.info(f"on_answer received SDP answer: {answer}")


def on_error(error, peer):
    logging.info(f"on_error received error: {error}")


def on_stats(stats, peer):
    logging.info(f"on_stats received stats: {stats}")
