        ...
        for endpoint in tlvCache:
            ...
            for cluster in tlvCache[endpoint]:
                ...
                if (self.returnClusterObject):
                    try:
                        # Since the TLV data is already organized by attribute tags,
                        # we can trivially convert to a cluster object representation.
...
def _OnReadEventDataCallback(closure, endpoint: int, cluster: int, event: c_uint64,
    ...
    closure.handleEventData(EventHeader(
        EndpointId=endpoint, ClusterId=cluster, EventId=event,
        EventNumber=number, Priority=EventPriority(priority),
        Timestamp=timestamp, TimestampType=EventTimestampType(timestampType)), path, dataBytes[:], status)
