        ...
        for v in events:
            ...
            if v == ('*') or v == ():
                ...
                if issubclass(v[1], ClusterObjects.Cluster):
                    ...
                    raise ValueError("Unsupported Attribute Path")
            attrs.append(ClusterAttribute.EventPath(
                EndpointId=endpoint, Cluster=cluster, Attribute=attribute))
        res = self._ChipStack.Call(
            lambda: ClusterAttribute.ReadEvents(future, eventLoop, device, self, attrs, ClusterAttribute.SubscriptionParameters(reportInterval[0], reportInterval[1]) if reportInterval else None))
        if res != 0:
            raise self._ChipStack.ErrorToException(res)
        return await future
