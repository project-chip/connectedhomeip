...
class ClusterObjectTests:
    ...
    async def TestCommandWithResponse(cls, devCtrl):
        req = Clusters.Test.Commands.TestAddArguments(arg1=2, arg2=3)
        res = await devCtrl.SendCommand(nodeid=NODE_ID, endpoint=LIGHTING_ENDPOINT_ID, payload=req)
        if not isinstance(res, Clusters.Test.Commands.TestAddArgumentsResponse):
    ...
    async def TestReadWriteOnlyAttribute(cls, devCtrl):
        ...
        res = await devCtrl.ReadAttribute(nodeid=NODE_ID, attributes=[(Clusters.Test)])
        if Clusters.Test.Attributes.WriteOnlyInt8u in res[1][Clusters.Test]:
    ...
    async def TestWriteRequest(cls, devCtrl):
        ...
        res = await devCtrl.WriteAttribute(nodeid=NODE_ID,
                                           attributes=[(1, Clusters.Test.Attributes.ListLongOctetString([b"0123456789abcdef" * 32] * 5))])
        expectedRes = [
            AttributeStatus(Path=AttributePath(
                EndpointId=1, Attribute=Clusters.Test.Attributes.ListLongOctetString), Status=chip.interaction_model.Status.Success),
        ]
    ...
    async def TestReadAttributeRequests(cls, devCtrl):
        ...
        # logger.info(
        #    f"Test Cluster - Struct: {res[1][Clusters.Test].structAttr}")
        logger.info(f"Test Cluster: {res[1][Clusters.Test]}")

        logger.info("7: Reading Chunked List")
        res = await devCtrl.ReadAttribute(nodeid=NODE_ID, attributes=[(1, Clusters.Test.Attributes.ListLongOctetString)])
        if res[1][Clusters.Test][Clusters.Test.Attributes.ListLongOctetString] != [b'0123456789abcdef' * 32] * 4:
        ...
        # res = await devCtrl.ReadAttribute(nodeid=NODE_ID, attributes=[(1, Clusters.Test.Attributes.ListFabricScoped)], fabricFiltered=False)
        # if len(res[1][Clusters.Test][Clusters.Test.Attributes.ListFabricScoped]) == 1:
        #     raise AssertionError("Expect more elements in the response")
        # logger.info("9: Read with fabric filter")
        # res = await devCtrl.ReadAttribute(nodeid=NODE_ID, attributes=[(1, Clusters.Test.Attributes.ListFabricScoped)], fabricFiltered=True)
        # if len(res[1][Clusters.Test][Clusters.Test.Attributes.ListFabricScoped]) != 1:
        #     raise AssertionError("Expect exact one element in the response")
        # if res[1][Clusters.Test][Clusters.Test.Attributes.ListFabricScoped][0].fabricIndex != fabricIndex:
        #     raise AssertionError(
    ...
    async def _TriggerEvent(cls, devCtrl):
        ...
        await devCtrl.SendCommand(nodeid=NODE_ID, endpoint=1, payload=Clusters.Test.Commands.TestEmitTestEventRequest())
        await devCtrl.SendCommand(nodeid=NODE_ID, endpoint=1, payload=Clusters.Test.Commands.TestEmitTestEventRequest())
        await devCtrl.SendCommand(nodeid=NODE_ID, endpoint=1, payload=Clusters.Test.Commands.TestEmitTestEventRequest())
    ...
    async def TestGenerateUndefinedFabricScopedEventRequests(cls, devCtrl):
        ...
        try:
            res = await devCtrl.SendCommand(nodeid=NODE_ID, endpoint=1, payload=Clusters.Test.Commands.TestEmitTestFabricScopedEventRequest(arg1=0))
            raise ValueError(f"Unexpected Failure")
        except chip.interaction_model.InteractionModelError as ex:
            logger.info(f"Recevied {ex} from server.")
        res = await devCtrl.ReadEvent(nodeid=NODE_ID, events=[
            (1, Clusters.Test.Events.TestFabricScopedEvent, 0),
        ])
    ...
    async def TestReadEventRequests(cls, devCtrl, expectEventsNum):
        ...
        req = [
            (1, Clusters.Test.Events.TestEvent, 0),
        ]
    ...
    async def TestTimedRequest(cls, devCtrl):
        ...
        req = Clusters.Test.Commands.TimedInvokeRequest()
        ...
        await devCtrl.WriteAttribute(nodeid=NODE_ID,
                                     attributes=[
                                         (1, Clusters.Test.Attributes.TimedWriteBoolean(
                                             True)),
                                     ],
    ...
    async def TestMixedReadAttributeAndEvents(cls, devCtrl):
        ...
        def eventPathPossibilities():
            yield ('Ex Cx Ex', [(1, Clusters.Test.Events.TestEvent, 0)])
            yield ('Ex Cx E*', [(1, Clusters.Test, 0)])
            yield ('Ex C* E*', [1])
            yield ('E* C* E*', ['*'])
            yield ('Ex Cx E* Urgent', [(1, Clusters.Test, 1)])
