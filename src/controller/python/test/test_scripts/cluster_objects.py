...
class ClusterObjectTests:
    ...
    async def RoundTripTest(cls, devCtrl):
        ...
        res = await devCtrl.SendCommand(nodeId=NODE_ID, endpointId=LIGHTING_ENDPOINT_ID, payload=req)
        if res is not None:
            logger.error(
    ...
    async def RoundTripTestWithBadEndpoint(cls, devCtrl):
        req = Clusters.OnOff.Commands.On()
        try:
            await devCtrl.SendCommand(nodeId=NODE_ID, endpointId=233, payload=req)
    ...
    async def SendCommandWithResponse(cls, devCtrl):
        ...
        res = await devCtrl.SendCommand(nodeId=NODE_ID, endpointId=LIGHTING_ENDPOINT_ID, payload=req, responseType=Clusters.TestCluster.Commands.TestAddArgumentsResponse)
