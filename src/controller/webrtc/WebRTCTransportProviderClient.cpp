...
void WebRTCTransportProviderClient::HandleProvideOfferResponse(TLV::TLVReader data)
{
    ...
    session.videoStreamID =
        value.videoStreamID.HasValue() ? value.videoStreamID.Value() : MakeOptional(DataModel::MakeNullable<uint16_t>());
    session.audioStreamID =
        value.audioStreamID.HasValue() ? value.audioStreamID.Value() : MakeOptional(DataModel::MakeNullable<uint16_t>());
