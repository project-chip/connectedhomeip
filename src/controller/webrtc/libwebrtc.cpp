...
void AddTracks(void * Client)
{
    ...
    rtc::scoped_refptr<webrtc::AudioTrackInterface> audio_track(client->peer_connection_factory->CreateAudioTrack(
        kAudioLabel, client->peer_connection_factory->CreateAudioSource(cricket::AudioOptions()).get()));
    auto result_or_error = client->webrtc_observer->peer_connection->AddTrack(audio_track, { kStreamId });
    if (!result_or_error.ok())
    {
        RTC_LOG(LS_ERROR) << "[LIBWEBRTC] Failed to add audio track to PeerConnection: " << result_or_error.error().message();
    }

    rtc::scoped_refptr<FakeVideoTrackSource> video_device = FakeVideoTrackSource::Create();
    if (video_device)
    {
        ...
        if (!result_or_error.ok())
        {
            RTC_LOG(LS_ERROR) << "[LIBWEBRTC] Failed to add video track to PeerConnection: " << result_or_error.error().message();
        }
    }
    else
    {
...
void CreateOffer(void * Client)
{
    ...
    client->webrtc_observer->peer_connection->CreateOffer(client->webrtc_observer.get() /*CreateSDPObserver.get()*/,
...
void initialise_peer_connection(void * Client)
{
    ...
    client->peer_connection_factory = webrtc::CreatePeerConnectionFactory(
        nullptr, nullptr, client->signaling_thread.get(), nullptr /* default_adm */, webrtc::CreateBuiltinAudioEncoderFactory(),
        webrtc::CreateBuiltinAudioDecoderFactory(),
        std::make_unique<
            webrtc::VideoEncoderFactoryTemplate<webrtc::LibvpxVp8EncoderTemplateAdapter, webrtc::LibvpxVp9EncoderTemplateAdapter,
                                                webrtc::OpenH264EncoderTemplateAdapter, webrtc::LibaomAv1EncoderTemplateAdapter>>(),
        std::make_unique<
            webrtc::VideoDecoderFactoryTemplate<webrtc::LibvpxVp8DecoderTemplateAdapter, webrtc::LibvpxVp9DecoderTemplateAdapter,
    ...
    client->webrtc_observer->peer_connection = client->peer_connection_factory->CreatePeerConnection(
