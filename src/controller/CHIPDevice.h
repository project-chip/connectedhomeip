...
public:
    ...
    CHIP_ERROR SetCSRNonce(Optional<ByteSpan> csrNonce)
    {
        VerifyOrReturnError(csrNonce.Value().size() == sizeof(mCSRNonce), CHIP_ERROR_INVALID_ARGUMENT);
        memcpy(mCSRNonce, csrNonce.Value().data(), csrNonce.Value().size());
