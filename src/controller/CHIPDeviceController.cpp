                                                ...
                                                Callback::Callback<OnDeviceConnectionFailure> * onFailure)
{
    ...
    CHIP_ERROR err = device->Connect(onConnection, onFailure);
                                               ...
                                               System::PacketBufferHandle && msgBuf)
{
    ...
    device = FindOperationalDevice(ec->GetSecureSession());
...
CHIP_ERROR DeviceController::OpenCommissioningWindow(NodeId deviceId, uint16_t timeout, uint16_t iteration, uint16_t discriminator,
                                                     uint8_t option, SetupPayload & payload)
{
    ...
    payload.version               = 0;
    payload.rendezvousInformation = RendezvousInformationFlags(RendezvousInformationFlag::kBLE);
...
void DeviceController::OnNodeIdResolved(const chip::Dnssd::ResolvedNodeData & nodeData)
{
    ...
    CHIP_ERROR err = device->UpdateAddress(Transport::PeerAddress::UDP(nodeData.mAddress, nodeData.mPort, interfaceId));
...
CHIP_ERROR DeviceCommissioner::OperationalDiscoveryComplete(NodeId remoteDeviceId)
{
    ...
    CommissioneeDeviceProxy * device = nullptr;
    ReturnErrorOnFailure(GetDeviceBeingCommissioned(remoteDeviceId, &device));
    device->CloseSession();
    mDeviceBeingCommissioned = nullptr;

    return GetOperationalDevice(remoteDeviceId, &mOnDeviceConnectedCallback, &mOnDeviceConnectionFailureCallback);
}
...
void DeviceCommissioner::RendezvousCleanup(CHIP_ERROR status)
{
    ...
    if (mDeviceBeingCommissioned != nullptr && mStorageDelegate != nullptr)
    {
