...
void TestReadInteraction::TestReadSubscribeAttributeResponseWithCache(nlTestSuite * apSuite, void * apContext)
{
    ...
    // Eleventh subscribe of C1A* and C2A* and C3A*, it would use C1 cached version to construct DataVersionFilter, expect no
    // attributes in report Then issue another read with same paths, no attribute report, when it closes, it would clear up
    // corresponding read paths in cache, but read paths for previous subscribe would be kept, generate wildcard setDirty in server,
    // expect to receive C1, C2, C3 attributes in report, and cache their latest versions. Then bump data version in server, issue
    // another subscribe with same paths, receive C1, C2, C3, attribute report, and cache their latest versions
    {
        testId++;
        ChipLogProgress(DataManagement, "\t -- Running Read with AttributeCache Test ID %d", testId);
        app::ReadClient readClient(chip::app::InteractionModelEngine::GetInstance(), &ctx.GetExchangeManager(),
        ...
        Test::BumpVersion();
        chip::app::AttributePathParams info;
        engine->GetReportingEngine().SetDirty(info); // Wilcard SetDirty
        ctx.DrainAndServiceIO();

        NL_TEST_ASSERT(apSuite, delegate.mNumAttributeResponse == 12);
        version1.ClearValue();
        NL_TEST_ASSERT(apSuite, cache.GetVersion(Test::kMockEndpoint2, Test::MockClusterId(3), version1) == CHIP_NO_ERROR);
        NL_TEST_ASSERT(apSuite, version1.HasValue() && (version1.Value() == 3));
        version2.ClearValue();
        NL_TEST_ASSERT(apSuite, cache.GetVersion(Test::kMockEndpoint2, Test::MockClusterId(2), version2) == CHIP_NO_ERROR);
        NL_TEST_ASSERT(apSuite, version2.HasValue() && (version2.Value() == 3));
        version3.ClearValue();
        NL_TEST_ASSERT(apSuite, cache.GetVersion(Test::kMockEndpoint1, Test::MockClusterId(2), version3) == CHIP_NO_ERROR);
        NL_TEST_ASSERT(apSuite, version3.HasValue() && (version3.Value() == 3));
        delegate.mNumAttributeResponse = 0;

        Test::BumpVersion();

        app::ReadClient readClient3(chip::app::InteractionModelEngine::GetInstance(), &ctx.GetExchangeManager(),
                                    cache.GetBufferedCallback(), chip::app::ReadClient::InteractionType::Subscribe);
        readPrepareParams.mpAttributePathParamsList    = attributePathParams3;
        readPrepareParams.mAttributePathParamsListSize = 3;
        err                                            = readClient3.SendRequest(readPrepareParams);
        NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);
        ctx.DrainAndServiceIO();
        version1.ClearValue();
        NL_TEST_ASSERT(apSuite, cache.GetVersion(Test::kMockEndpoint2, Test::MockClusterId(3), version1) == CHIP_NO_ERROR);
        NL_TEST_ASSERT(apSuite, version1.HasValue() && (version1.Value() == 4));
        version2.ClearValue();
        NL_TEST_ASSERT(apSuite, cache.GetVersion(Test::kMockEndpoint2, Test::MockClusterId(2), version2) == CHIP_NO_ERROR);
        NL_TEST_ASSERT(apSuite, version2.HasValue() && (version2.Value() == 4));
        version3.ClearValue();
        NL_TEST_ASSERT(apSuite, cache.GetVersion(Test::kMockEndpoint1, Test::MockClusterId(2), version3) == CHIP_NO_ERROR);
        NL_TEST_ASSERT(apSuite, version3.HasValue() && (version3.Value() == 4));
        NL_TEST_ASSERT(apSuite, delegate.mNumAttributeResponse == 12);
        delegate.mNumAttributeResponse = 0;
