...
static CHIP_ERROR N2J_AttestationInfo(JNIEnv * env, jbyteArray challenge, jbyteArray nonce, jbyteArray elements,
                                      ...
                                      jbyteArray firmwareInfo, chip::VendorId vendorId, uint16_t productId, jobject & outAttestationInfo);
                                                                         ...
                                                                         Callback::Callback<OnNOCChainGeneration> * onCompletion)
{
    ...
    chip::VendorId vendorId = mAutoCommissioner->GetCommissioningParameters().GetRemoteVendorId().ValueOr(chip::VendorId::Unspecified);
    uint16_t productId = mAutoCommissioner->GetCommissioningParameters().GetRemoteProductId().ValueOr(0x0000); // 0x0000 is invalid product ID value.

    jobject attestationInfo;
    err = N2J_AttestationInfo(env, javaAttestationChallenge, javaAttestationNonce, javaAttestationElements,
                              javaAttestationElementsSignature, javaDAC, javaPAI, javaCD, javaFirmwareInfo, vendorId, productId, attestationInfo);
    if (err != CHIP_NO_ERROR)
    {
                               ...
                               chip::VendorId vendorId, uint16_t productId, jobject & outAttestationInfo)
{
    ...
    outAttestationInfo =
        (jobject) env->NewObject(infoClass, constructor, challenge, nonce, elements, elementsSignature, dac, pai, cd, firmwareInfo, static_cast<jint>(vendorId), static_cast<jint>(productId));
