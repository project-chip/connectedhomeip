...
class OnboardingPayload(
    ...
    /**
     * The CHIP device supported rendezvous flags: At least one DiscoveryCapability must be
     * included.
     */
    ...
    /**
     * If hasShortDiscriminator is true, the discriminator value contains just the high 4 bits of
     * the full discriminator. For example, if hasShortDiscriminator is true and discriminator is
     * 0xA, then the full discriminator can be anything in the range 0xA00 to 0xAFF.
     */
    var hasShortDiscriminator: Boolean = false,

    /**
     * The CHIP device setup PIN code: setupPINCode SHALL be greater than 0. Also invalid
     * setupPINCode is {000000000, 11111111, 22222222, 33333333, 44444444, 55555555, 66666666,
    ...
    fun isValidQRCodePayload(): Boolean {
        ...
        if (
            commissioningFlow.toUInt() > ((1 shl kCommissioningFlowFieldLengthInBits) - 1).toUInt()
        ) {
            return false
        }
        ...
        // means
        // is needed before commissioning.
        // 3: Reserved
        if (
