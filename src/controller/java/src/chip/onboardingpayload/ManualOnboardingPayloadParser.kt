...
class ManualOnboardingPayloadParser(decimalRepresentation: String) {
  ...
  fun populatePayload(outPayload: OnboardingPayload): Unit {
    ...
    // First digit of '8' or '9' would be invalid for v1 and would indicate new format (e.g. version
    // 2)
    ...
    if (isLongCode) {
      ...
      // Need to do dynamic checks, because we are reading 5 chars, so could have 99,999 here or
      // something.
  ...
  companion object {
    ...
    // Populate numberOfChars into dest from decimalString starting at startIndex (least significant
