 ...
 *
 * <p>If DeviceAttestationFailureCallback is implemented, then it will always be called when device
...
public interface DeviceAttestationDelegate {
  ...
  public interface DeviceAttestationCompletionCallback extends DeviceAttestationDelegate {
     ...
     * additional verification. If this callback is implemented, continueCommissioningDevice on
     * {@link ChipDeviceController} is expected to be called if commissioning should continue.
     *
     * <p>This allows the callback to stop commissioning after examining the device info (DAC, PAI,
     * CD).
     *
     * @param deviceControllerPtr Controller corresponding to the commissioning process
