...
public class ChipDeviceController {
   ...
   */
  public static byte[] createRootCertificate(KeypairDelegate keypair, long issuerId, @Nullable Long fabricId) {
  ...
  public static native byte[] createRootCertificate(KeypairDelegate keypair, long issuerId, @Nullable Long fabricId, Calendar validityStart, Calendar validityEnd);

  /**
   * Create an intermediate X.509 DER encoded certificate
   */
  public static byte[] createIntermediateCertificate(KeypairDelegate rootKeypair, byte[] rootCertificate, byte[] intermediatePublicKey, long issuerId, @Nullable Long fabricId) {
    Calendar start = Calendar.getInstance();
    Calendar end = Calendar.getInstance();
    end.add(Calendar.YEAR, 10);
    return createIntermediateCertificate(rootKeypair, rootCertificate, intermediatePublicKey, issuerId, fabricId, start, end);
  }

  public static native byte[] createIntermediateCertificate(KeypairDelegate rootKeypair, byte[] rootCertificate, byte[] intermediatePublicKey, long issuerId, @Nullable Long fabricId, Calendar validityStart, Calendar validityEnd);
   ...
   *
   * caseAuthenticatedTags may be null to indicate no CASE Authenticated Tags
   * should be used. If caseAuthenticatedTags is not null, it must contain at most
   * 3 numbers, which are expected to be 32-bit unsigned Case Authenticated Tag
   * values.
   */
  public static byte[] createOperationalCertificate(KeypairDelegate signingKeypair, byte[] signingCertificate, byte[] operationalPublicKey, long fabricId, long nodeId, List<Integer> caseAuthenticatedTags) {
    Calendar start = Calendar.getInstance();
    Calendar end = Calendar.getInstance();
    end.add(Calendar.YEAR, 10);
    return createOperationalCertificate(signingKeypair, signingCertificate, operationalPublicKey, fabricId, nodeId, caseAuthenticatedTags, start, end);
  }

  public static native byte[] createOperationalCertificate(KeypairDelegate signingKeypair, byte[] signingCertificate, byte[] operationalPublicKey, long fabricId, long nodeId, List<Integer>caseAuthenticatedTags, Calendar validityStart, Calendar validityEnd);
