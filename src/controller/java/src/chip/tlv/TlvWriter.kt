...
class TlvWriter(initialCapacity: Int = 32) {
    ...
    private fun put(element: Element): TlvWriter {
        ...
        if (containerDepth == 0) {
            ...
            containerType[containerDepth - 1] is StructureType && type !is EndOfContainerType
        ) {
            require(tag !is AnonymousTag) {
     ...
     *
     * Note that this method can only be used when all elements of an array are of Octet String
     * type.
     *
     * @throws TlvEncodingException if the data was invalid
     */
     ...
     * the source TLV element. The newly encoded element will have the same type, tag and contents
     * as the input container. If the supplied element is a TLV container (structure, array or
     * list), the entire contents of the container will be copied.
     *
     * @param reader a TlvReader object positioned at a Tlv element whose tag, type and value should
     *   be copied. If this method is executed successfully, the reader will be positioned at the
     ...
     * source TLV element. The newly encoded element will have the same type and contents as the
     * input container, however the tag will be set to the specified argument. If the supplied
     * element is a TLV container (structure, array or list), the entire contents of the container
     * will be copied.
     *
     ...
     *   copied. If this method is executed successfully, the reader will be positioned at the end
    ...
    fun validateTlv(): TlvWriter {
        if (containerDepth > 0) {
            throw TlvEncodingException(
                "Invalid Tlv data: $containerDepth containers are not closed"
            )
        }
        return this
    }
