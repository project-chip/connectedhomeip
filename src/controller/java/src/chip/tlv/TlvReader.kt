...
class TlvReader(bytes: ByteArray) : Iterable<Element> {
    ...
    fun nextElement(): Element {
            ...
            runCatching { Type.from(controlByte) }
                .onFailure {
                    throw TlvParsingException(
                        "Type error at $index for ${controlByte.toBinary()}",
                        it
                    )
                }
                .getOrThrow()
        ...
        val tag =
            runCatching { Tag.from(controlByte, index, bytes) }
                .onFailure {
                    throw TlvParsingException(
                        "Tag error at $index for ${controlByte.toBinary()}",
                        it
                    )
                }
                .getOrThrow()
        ...
        if (lengthSize > 0) {
            ...
            valueSize =
                bytes.sliceArray(index until index + lengthSize).fromLittleEndianToLong().toInt()
            index += lengthSize
        } else {
        ...
        val value: Value =
            when (elementType) {
                ...
                is FloatType ->
                    FloatValue(intBitsToFloat(valueBytes.fromLittleEndianToLong().toInt()))
    ...
    fun getUInt(tag: Tag): UInt {
        return checkRange(getULong(tag), UInt.MIN_VALUE.toULong()..UInt.MAX_VALUE.toULong())
            .toUInt()
    }
    ...
    fun getShort(tag: Tag): Short {
        return checkRange(getLong(tag), Short.MIN_VALUE.toLong()..Short.MAX_VALUE.toLong())
            .toShort()
    }
    ...
    fun getUByte(tag: Tag): UByte {
        return checkRange(getULong(tag), UByte.MIN_VALUE.toULong()..UByte.MAX_VALUE.toULong())
            .toUByte()
    }
    ...
    fun getFloat(tag: Tag): Float {
        ...
        require(value is FloatValue) {
            "Unexpected value $value at index $index (expected FloatValue)"
        }
        return value.value
    }
    ...
    fun getNull(tag: Tag) {
        val value = nextElement().verifyTagAndGetValue(tag)
        require(value is NullValue) {
            "Unexpected value $value at index $index (expected NullValue)"
        }
    }
    ...
    fun enterArray(tag: Tag) {
        ...
        require(value is ArrayValue) {
            "Unexpected value $value at index $index (expected ArrayValue)"
        }
    }
    ...
    fun enterList(tag: Tag) {
        ...
        require(value is ListValue) {
            "Unexpected value $value at index $index (expected ListValue)"
        }
    }
