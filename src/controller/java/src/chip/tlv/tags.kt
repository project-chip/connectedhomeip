...
sealed class Tag {
    ...
    internal companion object {
        /**
         * Parses a [Tag] from the given byte array using the control byte to determine the size of
         * the tag.
         *
         ...
         * @throws IllegalStateException if the byte array is too short to include the required tag
         *   data
         */
            ...
            return when (controlByte and TAG_MASK) {
                ...
                COMMON_PROFILE_2 -> {
                    CommonProfileTag(
                        size = 2,
                        tagNumber =
                            checkBytes(startIndex, 2, bytes).fromLittleEndianToLong().toUInt()
                    )
                }
                COMMON_PROFILE_4 -> {
                    CommonProfileTag(
                        size = 4,
                        tagNumber =
                            checkBytes(startIndex, 4, bytes).fromLittleEndianToLong().toUInt()
                    )
                }
                IMPLICIT_PROFILE_2 -> {
                    ImplicitProfileTag(
                ...
                FULLY_QUALIFIED_6 -> {
                    FullyQualifiedTag(
                        size = 6,
                        vendorId =
                            checkBytes(startIndex, 2, bytes).fromLittleEndianToLong().toUShort(),
                        profileNumber =
                            checkBytes(startIndex + 2, 2, bytes)
                                .fromLittleEndianToLong()
                                .toUShort(),
                        tagNumber =
                            checkBytes(startIndex + 4, 2, bytes).fromLittleEndianToLong().toUInt()
                    )
                }
                FULLY_QUALIFIED_8 -> {
                    FullyQualifiedTag(
                        ...
                        profileNumber =
                            checkBytes(startIndex + 2, 2, bytes)
                                ...
                                .toUShort(),
                        tagNumber =
                            checkBytes(startIndex + 4, 4, bytes).fromLittleEndianToLong().toUInt()
                    )
                }
            ...
            val controlByte =
                encodedType or
                    when (tag) {
                        ...
                        is CommonProfileTag ->
                            if (tag.size == 2) COMMON_PROFILE_2 else COMMON_PROFILE_4
                        is ImplicitProfileTag ->
                            if (tag.size == 2) IMPLICIT_PROFILE_2 else IMPLICIT_PROFILE_4
                        is FullyQualifiedTag ->
                            if (tag.size == 6) FULLY_QUALIFIED_6 else FULLY_QUALIFIED_8
                    }
