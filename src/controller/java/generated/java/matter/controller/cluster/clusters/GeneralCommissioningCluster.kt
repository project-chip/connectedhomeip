...
class AttributeListAttribute(
  ...
  suspend fun armFailSafe(expiryLengthSeconds: UShort
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ERROR_CODE)) {errorCode_decoded = tlvReader.getUByte(tag)} else
      
      if (tag == ContextSpecificTag(TAG_DEBUG_TEXT)) {debugText_decoded = tlvReader.getString(tag)} else
      {
...
suspend fun readIsCommissioningWithoutPowerAttribute(): Boolean? {val ATTRIBUTE_ID: UInt = 12u
    ...
    requireNotNull(attributeData) { 
      "Iscommissioningwithoutpower attribute not found in response" 
    }
  ...
  suspend fun subscribeIsCommissioningWithoutPowerAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<BooleanSubscriptionState> {
