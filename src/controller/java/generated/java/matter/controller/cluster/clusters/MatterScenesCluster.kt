...
class MatterScenesCluster(
  ...
  class ViewSceneResponse(
    ...
    val extensionFieldSets: List<MatterScenesClusterExtensionFieldSet>?
  )
  ...
  class FabricSceneInfoAttribute(val value: List<MatterScenesClusterSceneInfoStruct>)

  sealed class FabricSceneInfoAttributeSubscriptionState {
    data class Success(val value: List<MatterScenesClusterSceneInfoStruct>) :
  ...
  suspend fun addScene(
    ...
    sceneName: String,
    extensionFieldSets: List<MatterScenesClusterExtensionFieldSet>,
  ...
  suspend fun viewScene(
    ...
    var extensionFieldSets_decoded: List<MatterScenesClusterExtensionFieldSet>? = null
      ...
      if (tag == ContextSpecificTag(TAG_EXTENSION_FIELD_SETS)) {
        extensionFieldSets_decoded =
          if (tlvReader.isNull()) {
            ...
            if (tlvReader.isNextTag(tag)) {
              buildList<MatterScenesClusterExtensionFieldSet> {
                tlvReader.enterArray(tag)
                while (!tlvReader.isEndOfContainer()) {
                  add(MatterScenesClusterExtensionFieldSet.fromTlv(AnonymousTag, tlvReader))
                }
                tlvReader.exitContainer()
              }
            } else {
  ...
  suspend fun readFabricSceneInfoAttribute(): FabricSceneInfoAttribute {
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<MatterScenesClusterSceneInfoStruct> =
      buildList<MatterScenesClusterSceneInfoStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(MatterScenesClusterSceneInfoStruct.fromTlv(AnonymousTag, tlvReader))
        }
        tlvReader.exitContainer()
      }
  ...
  companion object {
    private val logger = Logger.getLogger(MatterScenesCluster::class.java.name)
