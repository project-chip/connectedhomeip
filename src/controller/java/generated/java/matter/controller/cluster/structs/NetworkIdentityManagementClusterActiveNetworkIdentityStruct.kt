...
class NetworkIdentityManagementClusterActiveNetworkIdentityStruct(
  ...
  val createdTimestamp: UInt,
  val current: Boolean,
  val remainingClients: UShort?,
) {
  ...
  companion object {
    ...
    fun fromTlv(
      ...
      val index = tlvReader.getUShort(ContextSpecificTag(TAG_INDEX))
      ...
      val createdTimestamp = tlvReader.getUInt(ContextSpecificTag(TAG_CREATED_TIMESTAMP))
      val current = tlvReader.getBoolean(ContextSpecificTag(TAG_CURRENT))
      val remainingClients =
        if (!tlvReader.isNull()) {
          tlvReader.getUShort(ContextSpecificTag(TAG_REMAINING_CLIENTS))
        } else {
