...
class SoilMeasurementCluster(private val controller: MatterController, private val endpointId: UShort) {class SoilMoistureMeasurementLimitsAttribute(
  ...
  sealed class SoilMoistureMeasurementLimitsAttributeSubscriptionState {
    ...
    data class Error(val exception: Exception) : SoilMoistureMeasurementLimitsAttributeSubscriptionState()

    object SubscriptionEstablished : SoilMoistureMeasurementLimitsAttributeSubscriptionState()    
  }  
class SoilMoistureMeasuredValueAttribute(
    val value: UByte?
  )
...
suspend fun readSoilMoistureMeasurementLimitsAttribute(): SoilMoistureMeasurementLimitsAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    val decodedValue: SoilMeasurementClusterMeasurementAccuracyStruct = SoilMeasurementClusterMeasurementAccuracyStruct.fromTlv(AnonymousTag, tlvReader)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SoilMoistureMeasurementLimitsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
...
suspend fun readSoilMoistureMeasuredValueAttribute(): SoilMoistureMeasuredValueAttribute {val ATTRIBUTE_ID: UInt = 1u
