...
class ZoneManagementCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  class GetTwoDCartesianZoneResponse(
    val zones: List<ZoneManagementClusterTwoDCartesianZoneStruct>
  )
class SupportedZoneSourcesAttribute(
  ...
  sealed class SupportedZoneSourcesAttributeSubscriptionState {
    data class Success(
    val value: List<UByte>
    ) : SupportedZoneSourcesAttributeSubscriptionState()
...
class AttributeListAttribute(
  ...
  suspend fun getTwoDCartesianZone(zoneID: UShort?
      ,timedInvokeTimeout: Duration? = null): GetTwoDCartesianZoneResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ZONES)) {zones_decoded = buildList<ZoneManagementClusterTwoDCartesianZoneStruct> {
...
suspend fun readSupportedZoneSourcesAttribute(): SupportedZoneSourcesAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Supportedzonesources attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SupportedZoneSourcesAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
...
suspend fun readZonesAttribute(): ZonesAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<ZoneManagementClusterZoneInformationStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
  ...
  suspend fun subscribeZonesAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      ...
      null
          ...
          decodedValue?.let {
            emit(ZonesAttributeSubscriptionState.Success(it))
          }
...
suspend fun readTriggersAttribute(): TriggersAttribute {val ATTRIBUTE_ID: UInt = 2u
...
suspend fun readSensitivityAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 3u
