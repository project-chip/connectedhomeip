...
class ThreadBorderRouterManagementCluster(private val controller: MatterController, private val endpointId: UShort) {
  class DatasetResponse(
    val dataset: ByteArray
  )
class ActiveDatasetTimestampAttribute(
    val value: ULong?
  )
  ...
  sealed class ActiveDatasetTimestampAttributeSubscriptionState {
    data class Success(
    val value: ULong?
    ) : ActiveDatasetTimestampAttributeSubscriptionState()
...
class PendingDatasetTimestampAttribute(
    val value: ULong?
  )
  ...
  sealed class PendingDatasetTimestampAttributeSubscriptionState {
    ...
    data class Error(val exception: Exception) : PendingDatasetTimestampAttributeSubscriptionState()
...
class AttributeListAttribute(
  ...
  suspend fun getActiveDatasetRequest(timedInvokeTimeout: Duration? = null): DatasetResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_DATASET)) {dataset_decoded = tlvReader.getByteArray(tag)} else
      {
  ...
  suspend fun setActiveDatasetRequest(activeDataset: ByteArray
      ,breadcrumb: ULong?
  ...
  suspend fun setPendingDatasetRequest(pendingDataset: ByteArray
      ,timedInvokeTimeout: Duration) {
...
suspend fun readBorderRouterNameAttribute(): String {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Borderroutername attribute not found in response" 
    }
...
suspend fun readBorderAgentIDAttribute(): ByteArray {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Borderagentid attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Borderagentid attribute not found in Node State update" 
          }
...
suspend fun readThreadVersionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 2u
...
suspend fun readInterfaceEnabledAttribute(): Boolean {val ATTRIBUTE_ID: UInt = 3u
...
suspend fun readActiveDatasetTimestampAttribute(): ActiveDatasetTimestampAttribute {val ATTRIBUTE_ID: UInt = 4u
    ...
    requireNotNull(attributeData) { 
      "Activedatasettimestamp attribute not found in response" 
    }
  ...
  suspend fun subscribeActiveDatasetTimestampAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(ActiveDatasetTimestampAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(ActiveDatasetTimestampAttributeSubscriptionState.Success(it))
          }
...
suspend fun readPendingDatasetTimestampAttribute(): PendingDatasetTimestampAttribute {val ATTRIBUTE_ID: UInt = 5u
    ...
    requireNotNull(attributeData) { 
      "Pendingdatasettimestamp attribute not found in response" 
    }
  ...
  suspend fun subscribePendingDatasetTimestampAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(PendingDatasetTimestampAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(PendingDatasetTimestampAttributeSubscriptionState.Success(it))
          }
