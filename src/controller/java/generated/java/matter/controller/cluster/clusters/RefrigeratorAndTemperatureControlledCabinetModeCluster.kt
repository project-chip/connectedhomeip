...
class RefrigeratorAndTemperatureControlledCabinetModeCluster(private val controller: MatterController, private val endpointId: UShort) {
...
class StartUpModeAttribute(
  ...
  sealed class StartUpModeAttributeSubscriptionState {
    data class Success(
    val value: UByte?
    ) : StartUpModeAttributeSubscriptionState()
...
class OnModeAttribute(
  ...
  sealed class OnModeAttributeSubscriptionState {
    data class Success(
    val value: UByte?
    ) : OnModeAttributeSubscriptionState()
...
class AttributeListAttribute(
  ...
  suspend fun changeToMode(newMode: UByte
      ,timedInvokeTimeout: Duration? = null): ChangeToModeResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_STATUS_TEXT)) {statusText_decoded = 
          if (tlvReader.isNull()) {
...
suspend fun readSupportedModesAttribute(): SupportedModesAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    val decodedValue: List<RefrigeratorAndTemperatureControlledCabinetModeClusterModeOptionStruct> = buildList<RefrigeratorAndTemperatureControlledCabinetModeClusterModeOptionStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(RefrigeratorAndTemperatureControlledCabinetModeClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
      }
      tlvReader.exitContainer()
    }
...
suspend fun readStartUpModeAttribute(): StartUpModeAttribute {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Startupmode attribute not found in response" 
    }
  ...
  suspend fun subscribeStartUpModeAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(StartUpModeAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Startupmode attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(StartUpModeAttributeSubscriptionState.Success(it))
          }
...
suspend fun readOnModeAttribute(): OnModeAttribute {val ATTRIBUTE_ID: UInt = 3u
    ...
    requireNotNull(attributeData) { 
      "Onmode attribute not found in response" 
    }
  ...
  suspend fun subscribeOnModeAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(OnModeAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Onmode attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(OnModeAttributeSubscriptionState.Success(it))
          }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(RefrigeratorAndTemperatureControlledCabinetModeCluster::class.java.name)
