...
class ElectricalProtectionAlarmClusterResidualCurrentFaultRatingsStruct(
  ...
  val groundFaultClass: Optional<UByte>,
  val waveform: Optional<UByte>,
  val trippingCharacteristic: Optional<UShort>,
    ...
    fun fromTlv(
      ...
      val groundFaultClass =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_GROUND_FAULT_CLASS))) {
          Optional.of(tlvReader.getUByte(ContextSpecificTag(TAG_GROUND_FAULT_CLASS)))
        } else {
          Optional.empty()
        }
      val waveform =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_WAVEFORM))) {
          Optional.of(tlvReader.getUByte(ContextSpecificTag(TAG_WAVEFORM)))
        } else {
          Optional.empty()
        }
      val trippingCharacteristic =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_TRIPPING_CHARACTERISTIC))) {
          Optional.of(tlvReader.getUShort(ContextSpecificTag(TAG_TRIPPING_CHARACTERISTIC)))
        } else {
          Optional.empty()
        }
