...
class IlluminanceMeasurementCluster(private val controller: MatterController, private val endpointId: UShort) {class MeasuredValueAttribute(
    val value: UShort?
  )
...
class LightSensorTypeAttribute(
  ...
  sealed class LightSensorTypeAttributeSubscriptionState {
    data class Success(
    val value: UByte?
    ) : LightSensorTypeAttributeSubscriptionState()
...
suspend fun readLightSensorTypeAttribute(): LightSensorTypeAttribute {val ATTRIBUTE_ID: UInt = 4u
    ...
    requireNotNull(attributeData) { 
      "Lightsensortype attribute not found in response" 
    }
  ...
  suspend fun subscribeLightSensorTypeAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(LightSensorTypeAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(LightSensorTypeAttributeSubscriptionState.Success(it))
          }
