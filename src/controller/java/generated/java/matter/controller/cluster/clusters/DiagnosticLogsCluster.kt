...
class AttributeListAttribute(
  ...
  suspend fun retrieveLogsRequest(intent: UByte
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_LOG_CONTENT)) {logContent_decoded = tlvReader.getByteArray(tag)}  
      
      if (tag == ContextSpecificTag(TAG_U_T_C_TIME_STAMP)) {UTCTimeStamp_decoded = 
          if (tlvReader.isNull()) {
