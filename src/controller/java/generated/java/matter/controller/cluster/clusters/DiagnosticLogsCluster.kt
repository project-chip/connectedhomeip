...
class AttributeListAttribute(
  ...
  suspend fun retrieveLogsRequest(intent: UByte
      ,requestedProtocol: UByte
      ,transferFileDesignator: String?
      ,timedInvokeTimeout: Duration? = null): RetrieveLogsResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_LOG_CONTENT)) {logContent_decoded = tlvReader.getByteArray(tag)} else
      
      if (tag == ContextSpecificTag(TAG_UTC_TIME_STAMP)) {UTCTimeStamp_decoded = 
          if (tlvReader.isNull()) {
      ...
      if (tag == ContextSpecificTag(TAG_TIME_SINCE_BOOT)) {timeSinceBoot_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
