...
class WiFiNetworkManagementCluster(private val controller: MatterController, private val endpointId: UShort) {
  class NetworkPassphraseResponse(
...
class SsidAttribute(
  ...
  sealed class SsidAttributeSubscriptionState {
    data class Success(
    val value: ByteArray?
    ) : SsidAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : SsidAttributeSubscriptionState()

    object SubscriptionEstablished : SsidAttributeSubscriptionState()    
  }  
class PassphraseSurrogateAttribute(
    val value: ULong?
  )

  sealed class PassphraseSurrogateAttributeSubscriptionState {
    data class Success(
    val value: ULong?
    ) : PassphraseSurrogateAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : PassphraseSurrogateAttributeSubscriptionState()

    object SubscriptionEstablished : PassphraseSurrogateAttributeSubscriptionState()    
  }  
...
class AttributeListAttribute(
  ...
  suspend fun networkPassphraseRequest(timedInvokeTimeout: Duration? = null): NetworkPassphraseResponse {
    ...
    val TAG_PASSPHRASE: Int = 0
    var passphrase_decoded: ByteArray? = null
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_PASSPHRASE)) {passphrase_decoded = tlvReader.getByteArray(tag)} else
      {
    ...
    if (passphrase_decoded == null) {
      throw IllegalStateException("passphrase not found in TLV")  
    }
...
suspend fun readSsidAttribute(): SsidAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Ssid attribute not found in response" 
    }
    ...
    return SsidAttribute(decodedValue)
  }

  suspend fun subscribeSsidAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<SsidAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SsidAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Ssid attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(SsidAttributeSubscriptionState.Success(it))
          }
          
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(SsidAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
suspend fun readPassphraseSurrogateAttribute(): PassphraseSurrogateAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Passphrasesurrogate attribute not found in response" 
    }
    ...
    return PassphraseSurrogateAttribute(decodedValue)
  }

  suspend fun subscribePassphraseSurrogateAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<PassphraseSurrogateAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(PassphraseSurrogateAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Passphrasesurrogate attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(PassphraseSurrogateAttributeSubscriptionState.Success(it))
          }
          
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(PassphraseSurrogateAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
