...
class ServiceAreaCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  class SkipAreaResponse(
    val status: UByte, 
    val statusText: String
  )
class SupportedAreasAttribute(
    val value: List<ServiceAreaClusterAreaStruct>
  )
...
class SelectedAreasAttribute(
  ...
  sealed class SelectedAreasAttributeSubscriptionState {
    data class Success(
    val value: List<UInt>
    ) : SelectedAreasAttributeSubscriptionState()
...
class CurrentAreaAttribute(
  ...
  sealed class CurrentAreaAttributeSubscriptionState {
    data class Success(
    val value: UInt?
    ) : CurrentAreaAttributeSubscriptionState()
...
class AttributeListAttribute(
  ...
  suspend fun selectAreas(newAreas: List<UInt>
      ,timedInvokeTimeout: Duration? = null): SelectAreasResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_STATUS_TEXT)) {statusText_decoded = tlvReader.getString(tag)} else
      {
  ...
  suspend fun skipArea(skippedArea: UInt
      ,timedInvokeTimeout: Duration? = null): SkipAreaResponse {
...
suspend fun readSupportedAreasAttribute(): SupportedAreasAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Supportedareas attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<ServiceAreaClusterAreaStruct> = buildList<ServiceAreaClusterAreaStruct> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SupportedAreasAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
...
suspend fun readSupportedMapsAttribute(): SupportedMapsAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<ServiceAreaClusterMapStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
...
suspend fun readSelectedAreasAttribute(): SelectedAreasAttribute {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Selectedareas attribute not found in response" 
    }
  ...
  suspend fun subscribeSelectedAreasAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SelectedAreasAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Selectedareas attribute not found in Node State update" 
          }
...
suspend fun readCurrentAreaAttribute(): CurrentAreaAttribute {val ATTRIBUTE_ID: UInt = 3u
    ...
    requireNotNull(attributeData) { 
      "Currentarea attribute not found in response" 
    }
  ...
  suspend fun subscribeCurrentAreaAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(CurrentAreaAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Currentarea attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(CurrentAreaAttributeSubscriptionState.Success(it))
          }
...
suspend fun readProgressAttribute(): ProgressAttribute {val ATTRIBUTE_ID: UInt = 5u
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<ServiceAreaClusterProgressStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
