...
class MeasurementTypeAttribute(
    val value: UShort?
  )
  ...
  sealed class MeasurementTypeAttributeSubscriptionState {
    data class Success(
    val value: UShort?
    ) : MeasurementTypeAttributeSubscriptionState()
...
suspend fun readMeasurementTypeAttribute(): MeasurementTypeAttribute {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Measurementtype attribute not found in response" 
    }
  ...
  suspend fun subscribeMeasurementTypeAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(MeasurementTypeAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(MeasurementTypeAttributeSubscriptionState.Success(it))
          }
