...
class ScenesManagementCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  class ViewSceneResponse(
    ...
    val extensionFieldSetStructs: List<ScenesManagementClusterExtensionFieldSetStruct>?
  )
...
class AttributeListAttribute(
  ...
  suspend fun addScene(groupID: UShort
      ...
      ,transitionTime: UInt
      ,sceneName: String
      ,extensionFieldSetStructs: List<ScenesManagementClusterExtensionFieldSetStruct>
    ...
    val TAG_EXTENSION_FIELD_SET_STRUCTS_REQ: Int = 4
    tlvWriter.startArray(ContextSpecificTag(TAG_EXTENSION_FIELD_SET_STRUCTS_REQ))
      for (item in extensionFieldSetStructs.iterator()) {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_STATUS)) {status_decoded = tlvReader.getUByte(tag)} else
      
      if (tag == ContextSpecificTag(TAG_GROUP_ID)) {groupID_decoded = tlvReader.getUShort(tag)} else
      
      if (tag == ContextSpecificTag(TAG_SCENE_ID)) {sceneID_decoded = tlvReader.getUByte(tag)} else
      {
  ...
  suspend fun viewScene(groupID: UShort
    ...
    val TAG_EXTENSION_FIELD_SET_STRUCTS: Int = 5
    var extensionFieldSetStructs_decoded: List<ScenesManagementClusterExtensionFieldSetStruct>? = null
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_EXTENSION_FIELD_SET_STRUCTS)) {extensionFieldSetStructs_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
            if (tlvReader.isNextTag(tag)) {
      buildList<ScenesManagementClusterExtensionFieldSetStruct> {
      tlvReader.enterArray(tag)
      while(!tlvReader.isEndOfContainer()) {
        add(ScenesManagementClusterExtensionFieldSetStruct.fromTlv(AnonymousTag, tlvReader))
      }
      tlvReader.exitContainer()
    }
    } else {
  ...
  suspend fun copyScene(mode: UByte
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_GROUP_IDENTIFIER_FROM)) {groupIdentifierFrom_decoded = tlvReader.getUShort(tag)} else
      
      if (tag == ContextSpecificTag(TAG_SCENE_IDENTIFIER_FROM)) {sceneIdentifierFrom_decoded = tlvReader.getUByte(tag)} else
      {
        tlvReader.skipElement()
      }
    }
...
suspend fun readSceneTableSizeAttribute(): UShort {val ATTRIBUTE_ID: UInt = 1u
...
suspend fun readFabricSceneInfoAttribute(): FabricSceneInfoAttribute {val ATTRIBUTE_ID: UInt = 2u
