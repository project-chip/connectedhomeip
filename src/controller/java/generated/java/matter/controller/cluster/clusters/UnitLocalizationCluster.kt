...
class UnitLocalizationCluster(private val controller: MatterController, private val endpointId: UShort) {class SupportedTemperatureUnitsAttribute(
    val value: List<UByte>?
  )

  sealed class SupportedTemperatureUnitsAttributeSubscriptionState {
    data class Success(
    val value: List<UByte>?
    ) : SupportedTemperatureUnitsAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : SupportedTemperatureUnitsAttributeSubscriptionState()

    object SubscriptionEstablished : SupportedTemperatureUnitsAttributeSubscriptionState()    
  }  
...
suspend fun readTemperatureUnitAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Temperatureunit attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Temperatureunit attribute not found in Node State update" 
          }
...
suspend fun readSupportedTemperatureUnitsAttribute(): SupportedTemperatureUnitsAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Supportedtemperatureunits attribute not found in response" 
    }
    ...
    return SupportedTemperatureUnitsAttribute(decodedValue)
  }

  suspend fun subscribeSupportedTemperatureUnitsAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<SupportedTemperatureUnitsAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SupportedTemperatureUnitsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Supportedtemperatureunits attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(SupportedTemperatureUnitsAttributeSubscriptionState.Success(it))
          }
          
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(SupportedTemperatureUnitsAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(UnitLocalizationCluster::class.java.name)
