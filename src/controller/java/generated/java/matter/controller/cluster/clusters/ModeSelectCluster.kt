...
class ModeSelectCluster(private val controller: MatterController, private val endpointId: UShort) {class StandardNamespaceAttribute(
    val value: UShort?
  )
  ...
  sealed class StandardNamespaceAttributeSubscriptionState {
    data class Success(
    val value: UShort?
    ) : StandardNamespaceAttributeSubscriptionState()
...
suspend fun readDescriptionAttribute(): String {val ATTRIBUTE_ID: UInt = 0u
...
suspend fun readStandardNamespaceAttribute(): StandardNamespaceAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Standardnamespace attribute not found in response" 
    }
  ...
  suspend fun subscribeStandardNamespaceAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(StandardNamespaceAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(StandardNamespaceAttributeSubscriptionState.Success(it))
          }
...
suspend fun readSupportedModesAttribute(): SupportedModesAttribute {val ATTRIBUTE_ID: UInt = 2u
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<ModeSelectClusterModeOptionStruct> = buildList<ModeSelectClusterModeOptionStruct> {
...
suspend fun readCurrentModeAttribute(): UByte {val ATTRIBUTE_ID: UInt = 3u
...
suspend fun readStartUpModeAttribute(): StartUpModeAttribute {val ATTRIBUTE_ID: UInt = 4u
...
suspend fun readOnModeAttribute(): OnModeAttribute {val ATTRIBUTE_ID: UInt = 5u
