...
class ThermostatModeCluster(
  private val controller: MatterController,
  private val endpointId: UShort,
) {
  class ChangeToModeResponse(val status: UByte, val statusText: String?)

  class SupportedModesAttribute(val value: List<ThermostatModeClusterModeOptionStruct>)

  sealed class SupportedModesAttributeSubscriptionState {
    data class Success(val value: List<ThermostatModeClusterModeOptionStruct>) :
  ...
  suspend fun readSupportedModesAttribute(): SupportedModesAttribute {
    ...
    val decodedValue: List<ThermostatModeClusterModeOptionStruct> =
      buildList<ThermostatModeClusterModeOptionStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(ThermostatModeClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
        }
        tlvReader.exitContainer()
      }
  ...
  companion object {
    private val logger = Logger.getLogger(ThermostatModeCluster::class.java.name)
    const val CLUSTER_ID: UInt = 99u
  }
}
