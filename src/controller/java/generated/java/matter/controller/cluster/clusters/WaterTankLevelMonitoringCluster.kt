...
class WaterTankLevelMonitoringCluster(private val controller: MatterController, private val endpointId: UShort) {class LastChangedTimeAttribute(
    val value: UInt?
  )
...
class ReplacementProductListAttribute(
    val value: List<WaterTankLevelMonitoringClusterReplacementProductStruct>?
  )
...
suspend fun readReplacementProductListAttribute(): ReplacementProductListAttribute {val ATTRIBUTE_ID: UInt = 5u
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<WaterTankLevelMonitoringClusterReplacementProductStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
      buildList<WaterTankLevelMonitoringClusterReplacementProductStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(WaterTankLevelMonitoringClusterReplacementProductStruct.fromTlv(AnonymousTag, tlvReader))
      }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(WaterTankLevelMonitoringCluster::class.java.name)
    const val CLUSTER_ID: UInt = 121u
  }
}
