...
class WaterTankLevelMonitoringCluster(
  ...
  sealed class ReplacementProductListAttributeSubscriptionState {
    data class Success(val value: List<WaterTankLevelMonitoringClusterReplacementProductStruct>?) :
  ...
  suspend fun readReplacementProductListAttribute(): ReplacementProductListAttribute {
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<WaterTankLevelMonitoringClusterReplacementProductStruct>? =
      if (tlvReader.isNextTag(AnonymousTag)) {
        buildList<WaterTankLevelMonitoringClusterReplacementProductStruct> {
          tlvReader.enterArray(AnonymousTag)
          while (!tlvReader.isEndOfContainer()) {
            add(
              WaterTankLevelMonitoringClusterReplacementProductStruct.fromTlv(
                AnonymousTag,
                tlvReader,
              )
            )
