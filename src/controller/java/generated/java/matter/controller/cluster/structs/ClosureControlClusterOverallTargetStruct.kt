...
class ClosureControlClusterOverallTargetStruct(
  val tagPosition: Optional<UByte>,
  val tagLatch: Optional<UByte>,
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): ClosureControlClusterOverallTargetStruct {
      ...
      val tagPosition =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_TAG_POSITION))) {
          Optional.of(tlvReader.getUByte(ContextSpecificTag(TAG_TAG_POSITION)))
        } else {
          Optional.empty()
        }
      val tagLatch =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_TAG_LATCH))) {
          Optional.of(tlvReader.getUByte(ContextSpecificTag(TAG_TAG_LATCH)))
        } else {
          Optional.empty()
        }
      val speed =
