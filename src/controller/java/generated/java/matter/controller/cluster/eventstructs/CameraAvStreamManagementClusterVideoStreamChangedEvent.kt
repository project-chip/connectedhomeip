...
class CameraAvStreamManagementClusterVideoStreamChangedEvent(
  ...
  val maxResolution:
    Optional<
      matter.controller.cluster.structs.CameraAvStreamManagementClusterVideoResolutionStruct
    >,
  val minBitRate: Optional<UShort>,
  val maxBitRate: Optional<UShort>,
    ...
    fun fromTlv(
      ...
      val minBitRate =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MIN_BIT_RATE))) {
          Optional.of(tlvReader.getUShort(ContextSpecificTag(TAG_MIN_BIT_RATE)))
        } else {
          Optional.empty()
        }
      val maxBitRate =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_MAX_BIT_RATE))) {
          Optional.of(tlvReader.getUShort(ContextSpecificTag(TAG_MAX_BIT_RATE)))
        } else {
          Optional.empty()
        }
      val minFragmentLen =
