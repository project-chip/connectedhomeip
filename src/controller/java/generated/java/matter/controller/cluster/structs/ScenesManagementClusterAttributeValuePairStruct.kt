...
class ScenesManagementClusterAttributeValuePairStruct(
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): ScenesManagementClusterAttributeValuePairStruct {
      tlvReader.enterStructure(tlvTag)
      val attributeID = tlvReader.getUInt(ContextSpecificTag(TAG_ATTRIBUTE_ID))
      val valueUnsigned8 = if (tlvReader.isNextTag(ContextSpecificTag(TAG_VALUE_UNSIGNED8))) {
      Optional.of(tlvReader.getUByte(ContextSpecificTag(TAG_VALUE_UNSIGNED8)))
    } else {
      Optional.empty()
    }
      val valueSigned8 = if (tlvReader.isNextTag(ContextSpecificTag(TAG_VALUE_SIGNED8))) {
      Optional.of(tlvReader.getByte(ContextSpecificTag(TAG_VALUE_SIGNED8)))
    } else {
      Optional.empty()
    }
      val valueUnsigned16 = if (tlvReader.isNextTag(ContextSpecificTag(TAG_VALUE_UNSIGNED16))) {
      Optional.of(tlvReader.getUShort(ContextSpecificTag(TAG_VALUE_UNSIGNED16)))
    } else {
      Optional.empty()
    }
      val valueSigned16 = if (tlvReader.isNextTag(ContextSpecificTag(TAG_VALUE_SIGNED16))) {
      Optional.of(tlvReader.getShort(ContextSpecificTag(TAG_VALUE_SIGNED16)))
    } else {
      Optional.empty()
    }
      val valueUnsigned32 = if (tlvReader.isNextTag(ContextSpecificTag(TAG_VALUE_UNSIGNED32))) {
      Optional.of(tlvReader.getUInt(ContextSpecificTag(TAG_VALUE_UNSIGNED32)))
    } else {
      Optional.empty()
    }
      val valueSigned32 = if (tlvReader.isNextTag(ContextSpecificTag(TAG_VALUE_SIGNED32))) {
      Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_VALUE_SIGNED32)))
    } else {
      Optional.empty()
    }
      val valueUnsigned64 = if (tlvReader.isNextTag(ContextSpecificTag(TAG_VALUE_UNSIGNED64))) {
      Optional.of(tlvReader.getULong(ContextSpecificTag(TAG_VALUE_UNSIGNED64)))
    } else {
      Optional.empty()
    }
      val valueSigned64 = if (tlvReader.isNextTag(ContextSpecificTag(TAG_VALUE_SIGNED64))) {
      ...
      return ScenesManagementClusterAttributeValuePairStruct(attributeID, valueUnsigned8, valueSigned8, valueUnsigned16, valueSigned16, valueUnsigned32, valueSigned32, valueUnsigned64, valueSigned64)
    }
  }
}
