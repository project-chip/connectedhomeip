...
class AttributeListAttribute(
  ...
  suspend fun queryImage(vendorID: UShort
      ,productID: UShort
      ...
      ,timedInvokeTimeout: Duration? = null): QueryImageResponse {
    ...
    location?.let {
      tlvWriter.put(ContextSpecificTag(TAG_LOCATION_REQ), location)
    }
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_DELAYED_ACTION_TIME)) {delayedActionTime_decoded = 
          if (tlvReader.isNull()) {
      ...
      if (tag == ContextSpecificTag(TAG_IMAGE_URI)) {imageURI_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
      ...
      if (tag == ContextSpecificTag(TAG_SOFTWARE_VERSION)) {softwareVersion_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
      ...
      if (tag == ContextSpecificTag(TAG_SOFTWARE_VERSION_STRING)) {softwareVersionString_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
      ...
      if (tag == ContextSpecificTag(TAG_UPDATE_TOKEN)) {updateToken_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
      ...
      if (tag == ContextSpecificTag(TAG_USER_CONSENT_NEEDED)) {userConsentNeeded_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
      ...
      if (tag == ContextSpecificTag(TAG_METADATA_FOR_REQUESTOR)) {metadataForRequestor_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
  ...
  suspend fun applyUpdateRequest(updateToken: ByteArray
      ,newVersion: UInt
      ,timedInvokeTimeout: Duration? = null): ApplyUpdateResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ACTION)) {action_decoded = tlvReader.getUByte(tag)} else
      
      if (tag == ContextSpecificTag(TAG_DELAYED_ACTION_TIME)) {delayedActionTime_decoded = tlvReader.getUInt(tag)} else
      {
        tlvReader.skipElement()
      }
    }
  ...
  suspend fun notifyUpdateApplied(updateToken: ByteArray
      ,softwareVersion: UInt
