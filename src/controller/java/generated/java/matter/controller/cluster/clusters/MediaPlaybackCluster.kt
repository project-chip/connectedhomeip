...
class MediaPlaybackCluster(
  ...
  class AvailableCommandsAttribute(val value: List<UInt>?)

  sealed class AvailableCommandsAttributeSubscriptionState {
    data class Success(val value: List<UInt>?) : AvailableCommandsAttributeSubscriptionState()

    data class Error(val exception: Exception) : AvailableCommandsAttributeSubscriptionState()

    object SubscriptionEstablished : AvailableCommandsAttributeSubscriptionState()
  }

  class ContentInfoAttribute(val value: MediaPlaybackClusterContentInfoStruct?)

  sealed class ContentInfoAttributeSubscriptionState {
    data class Success(val value: MediaPlaybackClusterContentInfoStruct?) :
      ContentInfoAttributeSubscriptionState()

    data class Error(val exception: Exception) : ContentInfoAttributeSubscriptionState()

    object SubscriptionEstablished : ContentInfoAttributeSubscriptionState()
  }
  ...
  suspend fun readAvailableCommandsAttribute(): AvailableCommandsAttribute {
    ...
    requireNotNull(attributeData) { "Availablecommands attribute not found in response" }
    ...
    return AvailableCommandsAttribute(decodedValue)
  }

  suspend fun subscribeAvailableCommandsAttribute(
    minInterval: Int,
    maxInterval: Int,
  ): Flow<AvailableCommandsAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 11u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            AvailableCommandsAttributeSubscriptionState.Error(
              Exception(
                "Subscription terminated with error code: ${subscriptionState.terminationCause}"
              )
            )
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) {
            "Availablecommands attribute not found in Node State update"
          }
          ...
          decodedValue?.let { emit(AvailableCommandsAttributeSubscriptionState.Success(it)) }
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(AvailableCommandsAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }

  suspend fun readContentInfoAttribute(): ContentInfoAttribute {
    ...
    requireNotNull(attributeData) { "Contentinfo attribute not found in response" }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: MediaPlaybackClusterContentInfoStruct? =
      if (!tlvReader.isNull()) {
        MediaPlaybackClusterContentInfoStruct.fromTlv(AnonymousTag, tlvReader)
      } else {
        tlvReader.getNull(AnonymousTag)
        null
      }

    return ContentInfoAttribute(decodedValue)
  }

  suspend fun subscribeContentInfoAttribute(
    minInterval: Int,
    maxInterval: Int,
  ): Flow<ContentInfoAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 12u
    val attributePaths =
      listOf(
        AttributePath(endpointId = endpointId, clusterId = CLUSTER_ID, attributeId = ATTRIBUTE_ID)
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            ContentInfoAttributeSubscriptionState.Error(
              Exception(
                "Subscription terminated with error code: ${subscriptionState.terminationCause}"
              )
            )
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Contentinfo attribute not found in Node State update" }
          ...
          decodedValue?.let { emit(ContentInfoAttributeSubscriptionState.Success(it)) }
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(ContentInfoAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }
  }
