...
class TargetNavigatorCluster(private val controller: MatterController, private val endpointId: UShort) {
  class NavigateTargetResponse(
    val status: UByte, 
    val data: String?
  )
class TargetListAttribute(
    val value: List<TargetNavigatorClusterTargetInfoStruct>
  )

  sealed class TargetListAttributeSubscriptionState {
    data class Success(
    val value: List<TargetNavigatorClusterTargetInfoStruct>
    ) : TargetListAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : TargetListAttributeSubscriptionState()

    object SubscriptionEstablished : TargetListAttributeSubscriptionState()    
  }  
...
class AttributeListAttribute(
  ...
  suspend fun navigateTarget(target: UByte
      ,data: String?
      ,timedInvokeTimeout: Duration? = null): NavigateTargetResponse {
    val commandId: UInt = 0u

    val tlvWriter = TlvWriter()
    tlvWriter.startStructure(AnonymousTag)

    val TAG_TARGET_REQ: Int = 0
    tlvWriter.put(ContextSpecificTag(TAG_TARGET_REQ), target)
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_DATA)) {data_decoded = 
          if (tlvReader.isNull()) {
...
suspend fun readTargetListAttribute(): TargetListAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Targetlist attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<TargetNavigatorClusterTargetInfoStruct> = buildList<TargetNavigatorClusterTargetInfoStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(TargetNavigatorClusterTargetInfoStruct.fromTlv(AnonymousTag, tlvReader))
      }
      tlvReader.exitContainer()
    }


    return TargetListAttribute(decodedValue)
  }

  suspend fun subscribeTargetListAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<TargetListAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(TargetListAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Targetlist attribute not found in Node State update" 
          }
      ...
      tlvReader.exitContainer()
          ...
          emit(TargetListAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(TargetListAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
suspend fun readCurrentTargetAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Currenttarget attribute not found in response" 
    }
  ...
  suspend fun subscribeCurrentTargetAttribute(
    minInterval: Int,
    maxInterval: Int
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Currenttarget attribute not found in Node State update" 
          }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(TargetNavigatorCluster::class.java.name)
