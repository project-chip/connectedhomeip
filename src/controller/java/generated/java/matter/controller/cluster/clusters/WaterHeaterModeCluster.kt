...
class WaterHeaterModeCluster(private val controller: MatterController, private val endpointId: UShort) {
  class ChangeToModeResponse(
    val status: UByte, 
    val statusText: String?
  )
class SupportedModesAttribute(
    val value: List<WaterHeaterModeClusterModeOptionStruct>
  )
...
suspend fun readSupportedModesAttribute(): SupportedModesAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    val decodedValue: List<WaterHeaterModeClusterModeOptionStruct> = buildList<WaterHeaterModeClusterModeOptionStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(WaterHeaterModeClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
      }
      tlvReader.exitContainer()
    }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(WaterHeaterModeCluster::class.java.name)
    const val CLUSTER_ID: UInt = 158u
  }
}
