...
class EcosystemInformationCluster(private val controller: MatterController, private val endpointId: UShort) {class RemovedOnAttribute(
    val value: ULong?
  )
  ...
  sealed class RemovedOnAttributeSubscriptionState {
    data class Success(
    val value: ULong?
    ) : RemovedOnAttributeSubscriptionState()
...
suspend fun readRemovedOnAttribute(): RemovedOnAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Removedon attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(RemovedOnAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Removedon attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(RemovedOnAttributeSubscriptionState.Success(it))
          }
...
suspend fun readDeviceDirectoryAttribute(): DeviceDirectoryAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Devicedirectory attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<EcosystemInformationClusterEcosystemDeviceStruct> = buildList<EcosystemInformationClusterEcosystemDeviceStruct> {
  ...
  suspend fun subscribeDeviceDirectoryAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(DeviceDirectoryAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
...
suspend fun readLocationDirectoryAttribute(): LocationDirectoryAttribute {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Locationdirectory attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<EcosystemInformationClusterEcosystemLocationStruct> = buildList<EcosystemInformationClusterEcosystemLocationStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
  ...
  suspend fun subscribeLocationDirectoryAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(LocationDirectoryAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
