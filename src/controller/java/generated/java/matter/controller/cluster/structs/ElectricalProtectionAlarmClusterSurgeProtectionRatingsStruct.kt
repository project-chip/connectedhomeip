...
class ElectricalProtectionAlarmClusterSurgeProtectionRatingsStruct(
  ...
  val protectionClass: Optional<UShort>,
  val protectionType: Optional<UShort>,
    ...
    fun fromTlv(
      ...
      val protectionClass =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_PROTECTION_CLASS))) {
          Optional.of(tlvReader.getUShort(ContextSpecificTag(TAG_PROTECTION_CLASS)))
        } else {
          Optional.empty()
        }
      val protectionType =
        if (tlvReader.isNextTag(ContextSpecificTag(TAG_PROTECTION_TYPE))) {
          Optional.of(tlvReader.getUShort(ContextSpecificTag(TAG_PROTECTION_TYPE)))
        } else {
          Optional.empty()
        }
