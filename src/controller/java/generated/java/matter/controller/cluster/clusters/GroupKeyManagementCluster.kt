...
class GroupcastAdoptionAttribute(
    val value: List<GroupKeyManagementClusterGroupcastAdoptionStruct>?
  )

  sealed class GroupcastAdoptionAttributeSubscriptionState {
    data class Success(
    val value: List<GroupKeyManagementClusterGroupcastAdoptionStruct>?
    ) : GroupcastAdoptionAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : GroupcastAdoptionAttributeSubscriptionState()

    object SubscriptionEstablished : GroupcastAdoptionAttributeSubscriptionState()    
  }  
...
class AttributeListAttribute(
  ...
  suspend fun keySetWrite(groupKeySet: GroupKeyManagementClusterGroupKeySetStruct
  ...
  suspend fun keySetRead(groupKeySetID: UShort
      ,timedInvokeTimeout: Duration? = null): KeySetReadResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_GROUP_KEY_SET)) {groupKeySet_decoded = GroupKeyManagementClusterGroupKeySetStruct.fromTlv(tag, tlvReader)} else
      {
  ...
  suspend fun keySetReadAllIndices(timedInvokeTimeout: Duration? = null): KeySetReadAllIndicesResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_GROUP_KEY_SET_I_DS)) {groupKeySetIDs_decoded = buildList<UShort> {
...
suspend fun readGroupKeyMapAttribute(): GroupKeyMapAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Groupkeymap attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<GroupKeyManagementClusterGroupKeyMapStruct> = buildList<GroupKeyManagementClusterGroupKeyMapStruct> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(GroupKeyMapAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Groupkeymap attribute not found in Node State update" 
          }
...
suspend fun readGroupTableAttribute(): GroupTableAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Grouptable attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<GroupKeyManagementClusterGroupInfoMapStruct> = buildList<GroupKeyManagementClusterGroupInfoMapStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
  ...
  suspend fun subscribeGroupTableAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(GroupTableAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Grouptable attribute not found in Node State update" 
          }
...
suspend fun readMaxGroupsPerFabricAttribute(): UShort {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Maxgroupsperfabric attribute not found in response" 
    }
...
suspend fun readMaxGroupKeysPerFabricAttribute(): UShort {val ATTRIBUTE_ID: UInt = 3u
    ...
    requireNotNull(attributeData) { 
      "Maxgroupkeysperfabric attribute not found in response" 
    }
...
suspend fun readGroupcastAdoptionAttribute(): GroupcastAdoptionAttribute {val ATTRIBUTE_ID: UInt = 4u
    ...
    requireNotNull(attributeData) { 
      "Groupcastadoption attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<GroupKeyManagementClusterGroupcastAdoptionStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
      buildList<GroupKeyManagementClusterGroupcastAdoptionStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(GroupKeyManagementClusterGroupcastAdoptionStruct.fromTlv(AnonymousTag, tlvReader))
      }
      tlvReader.exitContainer()
    }
    } else {
    ...
    return GroupcastAdoptionAttribute(decodedValue)
  }

  suspend fun writeGroupcastAdoptionAttribute(
    value: List<GroupKeyManagementClusterGroupcastAdoptionStruct>,
    timedWriteTimeout: Duration? = null) {
  ...
  suspend fun subscribeGroupcastAdoptionAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<GroupcastAdoptionAttributeSubscriptionState> {
    val ATTRIBUTE_ID: UInt = 4u
    val attributePaths = listOf(
      AttributePath(
          endpointId = endpointId,
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(GroupcastAdoptionAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Groupcastadoption attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(GroupcastAdoptionAttributeSubscriptionState.Success(it))
          }
          
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(GroupcastAdoptionAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
