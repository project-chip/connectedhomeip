...
class TlsClientManagementCluster(
  ...
  class FindEndpointResponse(val endpoint: TlsClientManagementClusterTLSEndpointStruct)

  class ProvisionedEndpointsAttribute(val value: List<TlsClientManagementClusterTLSEndpointStruct>)

  sealed class ProvisionedEndpointsAttributeSubscriptionState {
    data class Success(val value: List<TlsClientManagementClusterTLSEndpointStruct>) :
  ...
  suspend fun findEndpoint(
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ENDPOINT)) {
        endpoint_decoded = TlsClientManagementClusterTLSEndpointStruct.fromTlv(tag, tlvReader)
      } else {
        tlvReader.skipElement()
      }
    }
    ...
    return FindEndpointResponse(endpoint_decoded)
  }
  ...
  suspend fun readProvisionedEndpointsAttribute(): ProvisionedEndpointsAttribute {
    ...
    requireNotNull(attributeData) { "Provisionedendpoints attribute not found in response" }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<TlsClientManagementClusterTLSEndpointStruct> =
      buildList<TlsClientManagementClusterTLSEndpointStruct> {
  ...
  suspend fun subscribeProvisionedEndpointsAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            ProvisionedEndpointsAttributeSubscriptionState.Error(
              Exception(
                "Subscription terminated with error code: ${subscriptionState.terminationCause}"
              )
            )
