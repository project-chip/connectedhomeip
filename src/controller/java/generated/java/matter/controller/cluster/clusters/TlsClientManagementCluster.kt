...
class TlsClientManagementCluster(private val controller: MatterController, private val endpointId: UShort) {
  class ProvisionEndpointResponse(
    val endpointID: UShort
  )

  class FindEndpointResponse(
    val endpoint: TlsClientManagementClusterTLSEndpointStruct
  )
class ProvisionedEndpointsAttribute(
    val value: List<TlsClientManagementClusterTLSEndpointStruct>
  )

  sealed class ProvisionedEndpointsAttributeSubscriptionState {
    data class Success(
    val value: List<TlsClientManagementClusterTLSEndpointStruct>
    ) : ProvisionedEndpointsAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : ProvisionedEndpointsAttributeSubscriptionState()

    object SubscriptionEstablished : ProvisionedEndpointsAttributeSubscriptionState()    
  }  
...
class AttributeListAttribute(
  ...
  suspend fun provisionEndpoint(hostname: ByteArray
      ,port: UShort
      ,caid: UShort
      ,ccdid: UShort?
      ,endpointID: UShort?
      ,timedInvokeTimeout: Duration? = null): ProvisionEndpointResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ENDPOINT_ID)) {endpointID_decoded = tlvReader.getUShort(tag)} else
      {
  ...
  suspend fun findEndpoint(endpointID: UShort
      ,timedInvokeTimeout: Duration? = null): FindEndpointResponse {
    ...
    val tlvReader = TlvReader(response.payload)
    tlvReader.enterStructure(AnonymousTag)
    val TAG_ENDPOINT: Int = 0
    var endpoint_decoded: TlsClientManagementClusterTLSEndpointStruct? = null
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ENDPOINT)) {endpoint_decoded = TlsClientManagementClusterTLSEndpointStruct.fromTlv(tag, tlvReader)} else
      {
        tlvReader.skipElement()
      }
    }
    ...
    if (endpoint_decoded == null) {
      throw IllegalStateException("endpoint not found in TLV")  
    }
  ...
  suspend fun removeEndpoint(endpointID: UShort
...
suspend fun readMaxProvisionedAttribute(): UByte {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Maxprovisioned attribute not found in response" 
    }
...
suspend fun readProvisionedEndpointsAttribute(): ProvisionedEndpointsAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Provisionedendpoints attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<TlsClientManagementClusterTLSEndpointStruct> = buildList<TlsClientManagementClusterTLSEndpointStruct> {
    ...
    return ProvisionedEndpointsAttribute(decodedValue)
  }

  suspend fun subscribeProvisionedEndpointsAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<ProvisionedEndpointsAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(ProvisionedEndpointsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Provisionedendpoints attribute not found in Node State update" 
          }
      ...
      tlvReader.exitContainer()
          ...
          emit(ProvisionedEndpointsAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(ProvisionedEndpointsAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
