...
class ObjectCountConfigAttribute(
    val value: AmbientContextSensingClusterObjectCountConfigStruct?
  )
...
suspend fun readObjectCountReachedAttribute(): Boolean? {val ATTRIBUTE_ID: UInt = 5u
    ...
    requireNotNull(attributeData) { 
      "Objectcountreached attribute not found in response" 
    }
...
suspend fun readObjectCountConfigAttribute(): ObjectCountConfigAttribute {val ATTRIBUTE_ID: UInt = 6u
    ...
    requireNotNull(attributeData) { 
      "Objectcountconfig attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: AmbientContextSensingClusterObjectCountConfigStruct? = if (tlvReader.isNextTag(AnonymousTag)) {
  ...
  suspend fun subscribeObjectCountConfigAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(ObjectCountConfigAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(ObjectCountConfigAttributeSubscriptionState.Success(it))
          }
...
suspend fun readObjectCountAttribute(): UShort? {val ATTRIBUTE_ID: UInt = 7u
...
suspend fun readSimultaneousDetectionLimitAttribute(): UByte {val ATTRIBUTE_ID: UInt = 8u
...
suspend fun readHoldTimeLimitsAttribute(): HoldTimeLimitsAttribute {val ATTRIBUTE_ID: UInt = 10u
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: AmbientContextSensingClusterHoldTimeLimitsStruct = AmbientContextSensingClusterHoldTimeLimitsStruct.fromTlv(AnonymousTag, tlvReader)
