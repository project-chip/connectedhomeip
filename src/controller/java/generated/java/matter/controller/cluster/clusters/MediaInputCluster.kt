...
class MediaInputCluster(private val controller: MatterController, private val endpointId: UShort) {class InputListAttribute(
    val value: List<MediaInputClusterInputInfoStruct>
  )

  sealed class InputListAttributeSubscriptionState {
    data class Success(
    val value: List<MediaInputClusterInputInfoStruct>
    ) : InputListAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : InputListAttributeSubscriptionState()

    object SubscriptionEstablished : InputListAttributeSubscriptionState()    
  }  
...
class AttributeListAttribute(
  ...
  suspend fun showInputStatus(timedInvokeTimeout: Duration? = null) {
  ...
  suspend fun hideInputStatus(timedInvokeTimeout: Duration? = null) {
  ...
  suspend fun renameInput(index: UByte
      ,name: String
      ,timedInvokeTimeout: Duration? = null) {
...
suspend fun readInputListAttribute(): InputListAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Inputlist attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<MediaInputClusterInputInfoStruct> = buildList<MediaInputClusterInputInfoStruct> {
  ...
  suspend fun subscribeInputListAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<InputListAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(InputListAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Inputlist attribute not found in Node State update" 
          }
      ...
      tlvReader.exitContainer()
          ...
          emit(InputListAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(InputListAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
suspend fun readCurrentInputAttribute(): UByte {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Currentinput attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Currentinput attribute not found in Node State update" 
          }
