...
class TemperatureControlledCabinetTopologyCluster(private val controller: MatterController, private val endpointId: UShort) {class DisabledCabinetsAttribute(
    val value: List<UShort>
  )

  sealed class DisabledCabinetsAttributeSubscriptionState {
    data class Success(
    val value: List<UShort>
    ) : DisabledCabinetsAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : DisabledCabinetsAttributeSubscriptionState()

    object SubscriptionEstablished : DisabledCabinetsAttributeSubscriptionState()    
  }  
...
suspend fun readDisabledCabinetsAttribute(): DisabledCabinetsAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Disabledcabinets attribute not found in response" 
    }
    ...
    return DisabledCabinetsAttribute(decodedValue)
  }

  suspend fun subscribeDisabledCabinetsAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<DisabledCabinetsAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(DisabledCabinetsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Disabledcabinets attribute not found in Node State update" 
          }
      ...
      tlvReader.exitContainer()
          ...
          emit(DisabledCabinetsAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(DisabledCabinetsAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
suspend fun readTopologyAttribute(): UByte {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Topology attribute not found in response" 
    }
  ...
  suspend fun subscribeTopologyAttribute(
    minInterval: Int,
    maxInterval: Int
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Topology attribute not found in Node State update" 
          }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(TemperatureControlledCabinetTopologyCluster::class.java.name)
    const val CLUSTER_ID: UInt = 75u
  }
}
