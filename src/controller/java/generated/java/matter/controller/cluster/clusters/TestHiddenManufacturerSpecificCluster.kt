...
class TestHiddenManufacturerSpecificCluster(private val controller: MatterController, private val endpointId: UShort) {class GeneratedCommandListAttribute(
    val value: List<UInt>
  )
...
suspend fun readTestAttributeAttribute(): Boolean {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Testattribute attribute not found in response" 
    }
  ...
  suspend fun subscribeTestAttributeAttribute(
    minInterval: Int,
    maxInterval: Int
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Testattribute attribute not found in Node State update" 
          }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(TestHiddenManufacturerSpecificCluster::class.java.name)
    const val CLUSTER_ID: UInt = 4294048801u
  }
}
