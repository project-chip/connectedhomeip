...
class LocalizationConfigurationCluster(private val controller: MatterController, private val endpointId: UShort) {class SupportedLocalesAttribute(
    val value: List<String>
  )

  sealed class SupportedLocalesAttributeSubscriptionState {
    data class Success(
    val value: List<String>
    ) : SupportedLocalesAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : SupportedLocalesAttributeSubscriptionState()

    object SubscriptionEstablished : SupportedLocalesAttributeSubscriptionState()    
  }  
...
suspend fun readActiveLocaleAttribute(): String {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Activelocale attribute not found in response" 
    }
  ...
  suspend fun writeActiveLocaleAttribute(
    value: String,
  ...
  suspend fun subscribeActiveLocaleAttribute(
    minInterval: Int,
    maxInterval: Int
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Activelocale attribute not found in Node State update" 
          }
...
suspend fun readSupportedLocalesAttribute(): SupportedLocalesAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Supportedlocales attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<String> = buildList<String> {
    ...
    return SupportedLocalesAttribute(decodedValue)
  }

  suspend fun subscribeSupportedLocalesAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<SupportedLocalesAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SupportedLocalesAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Supportedlocales attribute not found in Node State update" 
          }
      ...
      tlvReader.exitContainer()
          ...
          emit(SupportedLocalesAttributeSubscriptionState.Success(decodedValue))
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(SupportedLocalesAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(LocalizationConfigurationCluster::class.java.name)
