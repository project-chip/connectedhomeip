...
class AttributeListAttribute(
  ...
  suspend fun addGroup(groupID: UShort
      ,groupName: String
      ,timedInvokeTimeout: Duration? = null): AddGroupResponse {
  ...
  suspend fun viewGroup(groupID: UShort
      ,timedInvokeTimeout: Duration? = null): ViewGroupResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_GROUP_NAME)) {groupName_decoded = tlvReader.getString(tag)} else
      {
        tlvReader.skipElement()
      }
    }
  ...
  suspend fun getGroupMembership(groupList: List<UShort>
      ,timedInvokeTimeout: Duration? = null): GetGroupMembershipResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_GROUP_LIST)) {groupList_decoded = buildList<UShort> {
  ...
  suspend fun removeGroup(groupID: UShort
      ,timedInvokeTimeout: Duration? = null): RemoveGroupResponse {
  ...
  suspend fun addGroupIfIdentifying(groupID: UShort
      ,groupName: String
...
suspend fun readNameSupportAttribute(): UByte {val ATTRIBUTE_ID: UInt = 0u
