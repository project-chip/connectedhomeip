...
class SupportedSFrameCipherSuitesAttribute(
  ...
  sealed class SupportedSFrameCipherSuitesAttributeSubscriptionState {
    ...
    data class Error(val exception: Exception) : SupportedSFrameCipherSuitesAttributeSubscriptionState()
...
class AttributeListAttribute(
  ...
  suspend fun solicitOffer(streamUsage: UByte
    ...
    SFrameConfig?.let {
      SFrameConfig.toTlv(ContextSpecificTag(TAG_S_FRAME_CONFIG_REQ), tlvWriter)
    }
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_WEB_RTC_SESSION_ID)) {webRTCSessionID_decoded = tlvReader.getUShort(tag)} else
      
      if (tag == ContextSpecificTag(TAG_DEFERRED_OFFER)) {deferredOffer_decoded = tlvReader.getBoolean(tag)} else
  ...
  suspend fun updateSession(webRTCSessionID: UShort
...
suspend fun readSupportedSFrameCipherSuitesAttribute(): SupportedSFrameCipherSuitesAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Supportedsframeciphersuites attribute not found in response" 
    }
  ...
  suspend fun subscribeSupportedSFrameCipherSuitesAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(SupportedSFrameCipherSuitesAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
