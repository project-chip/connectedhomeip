...
class AttributeListAttribute(
  ...
  suspend fun audioStreamAllocate(streamUsage: UByte
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_AUDIO_STREAM_ID)) {audioStreamID_decoded = tlvReader.getUShort(tag)} else
      {
  ...
  suspend fun videoStreamAllocate(streamUsage: UByte
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_VIDEO_STREAM_ID)) {videoStreamID_decoded = tlvReader.getUShort(tag)} else
      {
        tlvReader.skipElement()
      }
    }
  ...
  suspend fun snapshotStreamAllocate(imageCodec: UByte
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_SNAPSHOT_STREAM_ID)) {snapshotStreamID_decoded = tlvReader.getUShort(tag)} else
      {
        tlvReader.skipElement()
      }
    }
  ...
  suspend fun captureSnapshot(snapshotStreamID: UShort?
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_DATA)) {data_decoded = tlvReader.getByteArray(tag)} else
      
      if (tag == ContextSpecificTag(TAG_IMAGE_CODEC)) {imageCodec_decoded = tlvReader.getUByte(tag)} else
      
      if (tag == ContextSpecificTag(TAG_RESOLUTION)) {resolution_decoded = CameraAvStreamManagementClusterVideoResolutionStruct.fromTlv(tag, tlvReader)} else
      {
        tlvReader.skipElement()
      }
    }
...
suspend fun readMinViewportResolutionAttribute(): MinViewportResolutionAttribute {val ATTRIBUTE_ID: UInt = 4u
    ...
    requireNotNull(attributeData) { 
      "Minviewportresolution attribute not found in response" 
    }
  ...
  suspend fun subscribeMinViewportResolutionAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(MinViewportResolutionAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(MinViewportResolutionAttributeSubscriptionState.Success(it))
          }
