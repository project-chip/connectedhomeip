...
class JointFabricDatastoreCluster(
  ...
  class GroupKeySetListAttribute(val value: List<JointFabricDatastoreClusterGroupKeySetStruct>)
  ...
  class StatusEntryAttribute(
    val value: List<JointFabricDatastoreClusterDatastoreStatusEntryStruct>
  )

  sealed class StatusEntryAttributeSubscriptionState {
    data class Success(val value: List<JointFabricDatastoreClusterDatastoreStatusEntryStruct>) :
  ...
  class NodeKeySetListAttribute(
    val value: List<JointFabricDatastoreClusterDatastoreNodeKeyEntryStruct>
  )

  sealed class NodeKeySetListAttributeSubscriptionState {
    data class Success(val value: List<JointFabricDatastoreClusterDatastoreNodeKeyEntryStruct>) :
  ...
  suspend fun addAdmin(
    adminInformationEntry: JointFabricDatastoreClusterDatastoreAdministratorInformationEntryStruct?,
    timedInvokeTimeout: Duration? = null,
  ) {
  ...
  suspend fun readStatusEntryAttribute(): StatusEntryAttribute {
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<JointFabricDatastoreClusterDatastoreStatusEntryStruct> =
      buildList<JointFabricDatastoreClusterDatastoreStatusEntryStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(
  ...
  suspend fun readNodeKeySetListAttribute(): NodeKeySetListAttribute {
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<JointFabricDatastoreClusterDatastoreNodeKeyEntryStruct> =
      buildList<JointFabricDatastoreClusterDatastoreNodeKeyEntryStruct> {
        tlvReader.enterArray(AnonymousTag)
        while (!tlvReader.isEndOfContainer()) {
          add(
            JointFabricDatastoreClusterDatastoreNodeKeyEntryStruct.fromTlv(AnonymousTag, tlvReader)
          )
        }
        tlvReader.exitContainer()
      }
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          val decodedValue: List<JointFabricDatastoreClusterDatastoreNodeKeyEntryStruct> =
            buildList<JointFabricDatastoreClusterDatastoreNodeKeyEntryStruct> {
              ...
              while (!tlvReader.isEndOfContainer()) {
                add(
                  JointFabricDatastoreClusterDatastoreNodeKeyEntryStruct.fromTlv(
                    AnonymousTag,
                    tlvReader,
                  )
                )
