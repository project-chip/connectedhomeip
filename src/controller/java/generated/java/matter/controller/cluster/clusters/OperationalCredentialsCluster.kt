...
class NOCsAttribute(
    val value: List<OperationalCredentialsClusterNOCStruct>
  )
...
class FabricsAttribute(
    val value: List<OperationalCredentialsClusterFabricDescriptorStruct>
  )
...
class TrustedRootCertificatesAttribute(
...
class AttributeListAttribute(
  ...
  suspend fun attestationRequest(attestationNonce: ByteArray
      ,timedInvokeTimeout: Duration? = null): AttestationResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ATTESTATION_ELEMENTS)) {attestationElements_decoded = tlvReader.getByteArray(tag)} else
      
      if (tag == ContextSpecificTag(TAG_ATTESTATION_SIGNATURE)) {attestationSignature_decoded = tlvReader.getByteArray(tag)} else
      {
    ...
    return AttestationResponse(
      attestationElements_decoded,
      attestationSignature_decoded
    )
  }

  suspend fun certificateChainRequest(certificateType: UByte
      ,timedInvokeTimeout: Duration? = null): CertificateChainResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_CERTIFICATE)) {certificate_decoded = tlvReader.getByteArray(tag)} else
      {
        tlvReader.skipElement()
      }
    }
  ...
  suspend fun CSRRequest(CSRNonce: ByteArray
      ,isForUpdateNOC: Boolean?
      ,timedInvokeTimeout: Duration? = null): CSRResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_NOCSR_ELEMENTS)) {NOCSRElements_decoded = tlvReader.getByteArray(tag)} else
    ...
    return CSRResponse(
      NOCSRElements_decoded,
      attestationSignature_decoded
    )
  }

  suspend fun addNOC(NOCValue: ByteArray
      ,ICACValue: ByteArray?
      ,IPKValue: ByteArray
      ,caseAdminSubject: ULong
      ,adminVendorId: UShort
      ,timedInvokeTimeout: Duration? = null): NOCResponse {
    ...
    ICACValue?.let {
      tlvWriter.put(ContextSpecificTag(TAG_ICAC_VALUE_REQ), ICACValue)
    }
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_STATUS_CODE)) {statusCode_decoded = tlvReader.getUByte(tag)} else
      
      if (tag == ContextSpecificTag(TAG_FABRIC_INDEX)) {fabricIndex_decoded = 
          if (tlvReader.isNull()) {
  ...
  suspend fun updateNOC(NOCValue: ByteArray
      ,ICACValue: ByteArray?
      ,timedInvokeTimeout: Duration? = null): NOCResponse {
  ...
  suspend fun updateFabricLabel(label: String
      ,timedInvokeTimeout: Duration? = null): NOCResponse {
  ...
  suspend fun removeFabric(fabricIndex: UByte
      ,timedInvokeTimeout: Duration? = null): NOCResponse {
  ...
  suspend fun setVIDVerificationStatement(vendorID: UShort?
    ...
    VIDVerificationStatement?.let {
      tlvWriter.put(ContextSpecificTag(TAG_VID_VERIFICATION_STATEMENT_REQ), VIDVerificationStatement)
    }

    val TAG_VVSC_REQ: Int = 2
    vvsc?.let {
      tlvWriter.put(ContextSpecificTag(TAG_VVSC_REQ), vvsc)
    }    
    tlvWriter.endStructure()
  ...
  suspend fun signVIDVerificationRequest(fabricIndex: UByte
      ,clientChallenge: ByteArray
      ,timedInvokeTimeout: Duration? = null): SignVIDVerificationResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_FABRIC_INDEX)) {fabricIndex_decoded = tlvReader.getUByte(tag)} else
      
      if (tag == ContextSpecificTag(TAG_FABRIC_BINDING_VERSION)) {fabricBindingVersion_decoded = tlvReader.getUByte(tag)} else
      
      if (tag == ContextSpecificTag(TAG_SIGNATURE)) {signature_decoded = tlvReader.getByteArray(tag)} else
      {
        tlvReader.skipElement()
      }
    }
...
suspend fun readNOCsAttribute(): NOCsAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Nocs attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<OperationalCredentialsClusterNOCStruct> = buildList<OperationalCredentialsClusterNOCStruct> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(NOCsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Nocs attribute not found in Node State update" 
          }
...
suspend fun readFabricsAttribute(): FabricsAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Fabrics attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<OperationalCredentialsClusterFabricDescriptorStruct> = buildList<OperationalCredentialsClusterFabricDescriptorStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
  ...
  suspend fun subscribeFabricsAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(FabricsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Fabrics attribute not found in Node State update" 
          }
...
suspend fun readSupportedFabricsAttribute(): UByte {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Supportedfabrics attribute not found in response" 
    }
...
suspend fun readCommissionedFabricsAttribute(): UByte {val ATTRIBUTE_ID: UInt = 3u
    ...
    requireNotNull(attributeData) { 
      "Commissionedfabrics attribute not found in response" 
    }
...
suspend fun readTrustedRootCertificatesAttribute(): TrustedRootCertificatesAttribute {val ATTRIBUTE_ID: UInt = 4u
    ...
    requireNotNull(attributeData) { 
      "Trustedrootcertificates attribute not found in response" 
    }
  ...
  suspend fun subscribeTrustedRootCertificatesAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(TrustedRootCertificatesAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
...
suspend fun readCurrentFabricIndexAttribute(): UByte {val ATTRIBUTE_ID: UInt = 5u
    ...
    requireNotNull(attributeData) { 
      "Currentfabricindex attribute not found in response" 
    }
