...
class AttributeListAttribute(
  ...
  suspend fun attestationRequest(attestationNonce: ByteArray
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ATTESTATION_ELEMENTS)) {attestationElements_decoded = tlvReader.getByteArray(tag)}  
      
      if (tag == ContextSpecificTag(TAG_ATTESTATION_SIGNATURE)) {attestationSignature_decoded = tlvReader.getByteArray(tag)}  
  ...
  suspend fun certificateChainRequest(certificateType: UByte
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_CERTIFICATE)) {certificate_decoded = tlvReader.getByteArray(tag)}  
  ...
  suspend fun CSRRequest(CSRNonce: ByteArray
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_NOCSR_ELEMENTS)) {NOCSRElements_decoded = tlvReader.getByteArray(tag)}  
  ...
  suspend fun addNOC(NOCValue: ByteArray
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_STATUS_CODE)) {statusCode_decoded = tlvReader.getUByte(tag)}  
  ...
  suspend fun signVIDVerificationRequest(fabricIndex: UByte
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_FABRIC_INDEX)) {fabricIndex_decoded = tlvReader.getUByte(tag)}  
      
      if (tag == ContextSpecificTag(TAG_FABRIC_BINDING_VERSION)) {fabricBindingVersion_decoded = tlvReader.getUByte(tag)}  
      
      if (tag == ContextSpecificTag(TAG_SIGNATURE)) {signature_decoded = tlvReader.getByteArray(tag)}  
