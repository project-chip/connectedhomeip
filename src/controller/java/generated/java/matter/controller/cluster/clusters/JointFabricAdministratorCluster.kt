...
class JointFabricAdministratorCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  class TransferAnchorResponse(
    val statusCode: UByte
  )
class AdministratorFabricIndexAttribute(
    val value: UByte?
  )

  sealed class AdministratorFabricIndexAttributeSubscriptionState {
    data class Success(
    val value: UByte?
    ) : AdministratorFabricIndexAttributeSubscriptionState()
    
    data class Error(val exception: Exception) : AdministratorFabricIndexAttributeSubscriptionState()

    object SubscriptionEstablished : AdministratorFabricIndexAttributeSubscriptionState()    
  }  
...
class AttributeListAttribute(
  ...
  suspend fun ICACCSRRequest(timedInvokeTimeout: Duration? = null): ICACCSRResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ICACCSR)) {icaccsr_decoded = tlvReader.getByteArray(tag)} else
      {
  ...
  suspend fun announceJointFabricAdministrator(endpointID: UShort
      ,timedInvokeTimeout: Duration? = null) {
...
suspend fun readAdministratorFabricIndexAttribute(): AdministratorFabricIndexAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    return AdministratorFabricIndexAttribute(decodedValue)
  }

  suspend fun subscribeAdministratorFabricIndexAttribute(
    minInterval: Int,
    maxInterval: Int
  ): Flow<AdministratorFabricIndexAttributeSubscriptionState> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(AdministratorFabricIndexAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
      ...
      null
          ...
          decodedValue?.let {
            emit(AdministratorFabricIndexAttributeSubscriptionState.Success(it))
          }
          
        }
        SubscriptionState.SubscriptionEstablished -> {
          emit(AdministratorFabricIndexAttributeSubscriptionState.SubscriptionEstablished)
        }
      }
    }    
  }
