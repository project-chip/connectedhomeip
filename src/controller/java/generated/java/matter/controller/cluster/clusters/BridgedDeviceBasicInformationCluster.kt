...
suspend fun readDeviceLocationAttribute(): DeviceLocationAttribute {val ATTRIBUTE_ID: UInt = 23u
    ...
    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: BridgedDeviceBasicInformationClusterLocationDescriptorStruct? = if (tlvReader.isNextTag(AnonymousTag)) {
      BridgedDeviceBasicInformationClusterLocationDescriptorStruct.fromTlv(AnonymousTag, tlvReader)
    } else {
      null
    }
