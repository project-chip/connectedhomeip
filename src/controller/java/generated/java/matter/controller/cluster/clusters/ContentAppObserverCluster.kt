...
class ContentAppObserverCluster(private val controller: MatterController, private val endpointId: UShort) {
  class ContentAppMessageResponse(
    val status: UByte, 
    val data: String?, 
    val encodingHint: String?
  )
...
class AttributeListAttribute(
  ...
  suspend fun contentAppMessage(data: String?
      ,encodingHint: String
      ,timedInvokeTimeout: Duration? = null): ContentAppMessageResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ENCODING_HINT)) {encodingHint_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(ContentAppObserverCluster::class.java.name)
    const val CLUSTER_ID: UInt = 1296u
  }
}
