...
class AttributeListAttribute(
  ...
  suspend fun registerClient(checkInNodeID: ULong
      ,monitoredSubject: ULong
      ,key: ByteArray
      ,verificationKey: ByteArray?
      ,clientType: UByte
      ,timedInvokeTimeout: Duration? = null): RegisterClientResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ICD_COUNTER)) {ICDCounter_decoded = tlvReader.getUInt(tag)} else
      {
  ...
  suspend fun unregisterClient(checkInNodeID: ULong
      ,verificationKey: ByteArray?
  ...
  suspend fun stayActiveRequest(stayActiveDuration: UInt
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_PROMISED_ACTIVE_DURATION)) {promisedActiveDuration_decoded = tlvReader.getUInt(tag)} else
      {
        tlvReader.skipElement()
      }
    }
    ...
    return StayActiveResponse(
      promisedActiveDuration_decoded
    )
  }
suspend fun readIdleModeDurationAttribute(): UInt {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Idlemodeduration attribute not found in response" 
    }
...
suspend fun readActiveModeDurationAttribute(): UInt {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Activemodeduration attribute not found in response" 
    }
...
suspend fun readActiveModeThresholdAttribute(): UShort {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Activemodethreshold attribute not found in response" 
    }
...
suspend fun readRegisteredClientsAttribute(): RegisteredClientsAttribute {val ATTRIBUTE_ID: UInt = 3u
    ...
    requireNotNull(attributeData) { 
      "Registeredclients attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<IcdManagementClusterMonitoringRegistrationStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
  ...
  suspend fun subscribeRegisteredClientsAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(RegisteredClientsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(RegisteredClientsAttributeSubscriptionState.Success(it))
          }
...
suspend fun readICDCounterAttribute(): UInt? {val ATTRIBUTE_ID: UInt = 4u
    ...
    requireNotNull(attributeData) { 
      "Icdcounter attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Icdcounter attribute not found in Node State update" 
          }
...
suspend fun readClientsSupportedPerFabricAttribute(): UShort? {val ATTRIBUTE_ID: UInt = 5u
    ...
    requireNotNull(attributeData) { 
      "Clientssupportedperfabric attribute not found in response" 
    }
...
suspend fun readUserActiveModeTriggerHintAttribute(): UInt? {val ATTRIBUTE_ID: UInt = 6u
    ...
    requireNotNull(attributeData) { 
      "Useractivemodetriggerhint attribute not found in response" 
    }
...
suspend fun readUserActiveModeTriggerInstructionAttribute(): String? {val ATTRIBUTE_ID: UInt = 7u
...
suspend fun readOperatingModeAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 8u
...
suspend fun readMaximumCheckInBackOffAttribute(): UInt? {val ATTRIBUTE_ID: UInt = 9u
    ...
    requireNotNull(attributeData) { 
      "Maximumcheckinbackoff attribute not found in response" 
    }
