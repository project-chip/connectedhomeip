...
class AttributeListAttribute(
  ...
  suspend fun setActivePresetRequest(presetHandle: ByteArray
    ...
    delayMinutes?.let {
      tlvWriter.put(ContextSpecificTag(TAG_DELAY_MINUTES_REQ), delayMinutes)
    }    
    tlvWriter.endStructure()
  ...
  suspend fun startPresetsSchedulesEditRequest(timeoutSeconds: UShort
      ,timedInvokeTimeout: Duration? = null) {
  ...
  suspend fun setTemperatureSetpointHoldPolicy(temperatureSetpointHoldPolicy: UByte
      ,timedInvokeTimeout: Duration? = null) {
    ...
    tlvWriter.put(ContextSpecificTag(TAG_TEMPERATURE_SETPOINT_HOLD_POLICY_REQ), temperatureSetpointHoldPolicy)    
    tlvWriter.endStructure()
...
suspend fun readPresetsSchedulesEditableAttribute(): Boolean? {val ATTRIBUTE_ID: UInt = 82u
    ...
    requireNotNull(attributeData) { 
      "Presetsscheduleseditable attribute not found in response" 
    }
...
suspend fun readTemperatureSetpointHoldPolicyAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 83u
...
suspend fun readSetpointHoldExpiryTimestampAttribute(): SetpointHoldExpiryTimestampAttribute {val ATTRIBUTE_ID: UInt = 84u
...
suspend fun readQueuedPresetAttribute(): QueuedPresetAttribute {val ATTRIBUTE_ID: UInt = 85u
    ...
    requireNotNull(attributeData) { 
      "Queuedpreset attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: ThermostatClusterQueuedPresetStruct? = if (!tlvReader.isNull()) {
      if (tlvReader.isNextTag(AnonymousTag)) {
  ...
  suspend fun subscribeQueuedPresetAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(QueuedPresetAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Queuedpreset attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(QueuedPresetAttributeSubscriptionState.Success(it))
          }
