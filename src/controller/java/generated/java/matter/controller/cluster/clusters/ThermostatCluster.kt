...
class AttributeListAttribute(
  ...
  suspend fun getWeeklySchedule(daysToReturn: UByte
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_NUMBER_OF_TRANSITIONS_FOR_SEQUENCE)) {numberOfTransitionsForSequence_decoded = tlvReader.getUByte(tag)}  
      
      if (tag == ContextSpecificTag(TAG_DAY_OF_WEEK_FOR_SEQUENCE)) {dayOfWeekForSequence_decoded = tlvReader.getUByte(tag)}  
      
      if (tag == ContextSpecificTag(TAG_MODE_FOR_SEQUENCE)) {modeForSequence_decoded = tlvReader.getUByte(tag)}  
  ...
  suspend fun addThermostatSuggestion(presetHandle: ByteArray
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_UNIQUE_ID)) {uniqueID_decoded = tlvReader.getUByte(tag)}  
