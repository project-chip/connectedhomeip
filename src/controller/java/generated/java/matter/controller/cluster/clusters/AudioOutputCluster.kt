...
class AudioOutputCluster(private val controller: MatterController, private val endpointId: UShort) {class OutputListAttribute(
...
class AttributeListAttribute(
  ...
  suspend fun renameOutput(index: UByte
      ,name: String
      ,timedInvokeTimeout: Duration? = null) {
...
suspend fun readOutputListAttribute(): OutputListAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Outputlist attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<AudioOutputClusterOutputInfoStruct> = buildList<AudioOutputClusterOutputInfoStruct> {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(OutputListAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Outputlist attribute not found in Node State update" 
          }
...
suspend fun readCurrentOutputAttribute(): UByte {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Currentoutput attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Currentoutput attribute not found in Node State update" 
          }
