...
class AttributeListAttribute(
  ...
  suspend fun getSetupPIN(tempAccountIdentifier: String
      ,timedInvokeTimeout: Duration): GetSetupPINResponse {
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_SETUP_PIN)) {setupPIN_decoded = tlvReader.getString(tag)} else
      {
  ...
  suspend fun login(tempAccountIdentifier: String
    ...
    node?.let {
      tlvWriter.put(ContextSpecificTag(TAG_NODE_REQ), node)
    }    
    tlvWriter.endStructure()
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(AccountLoginCluster::class.java.name)
