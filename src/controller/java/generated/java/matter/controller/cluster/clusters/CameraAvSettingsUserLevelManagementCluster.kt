...
class CameraAvSettingsUserLevelManagementCluster(private val controller: MatterController, private val endpointId: UShort) {class MPTZPositionAttribute(
    val value: CameraAvSettingsUserLevelManagementClusterMPTZStruct?
  )
...
class AttributeListAttribute(
  ...
  suspend fun MPTZSetPosition(pan: Short?
      ,tilt: Short?
      ,zoom: UByte?
    ...
    pan?.let {
      tlvWriter.put(ContextSpecificTag(TAG_PAN_REQ), pan)
    }

    val TAG_TILT_REQ: Int = 1
    tilt?.let {
      tlvWriter.put(ContextSpecificTag(TAG_TILT_REQ), tilt)
    }

    val TAG_ZOOM_REQ: Int = 2
    zoom?.let {
      tlvWriter.put(ContextSpecificTag(TAG_ZOOM_REQ), zoom)
    }    
  ...
  suspend fun MPTZRelativeMove(panDelta: Short?
      ,tiltDelta: Short?
      ,zoomDelta: Byte?
      ,timedInvokeTimeout: Duration? = null) {
    ...
    panDelta?.let {
      tlvWriter.put(ContextSpecificTag(TAG_PAN_DELTA_REQ), panDelta)
    }

    val TAG_TILT_DELTA_REQ: Int = 1
    tiltDelta?.let {
      tlvWriter.put(ContextSpecificTag(TAG_TILT_DELTA_REQ), tiltDelta)
    }

    val TAG_ZOOM_DELTA_REQ: Int = 2
    zoomDelta?.let {
      tlvWriter.put(ContextSpecificTag(TAG_ZOOM_DELTA_REQ), zoomDelta)
    }    
    tlvWriter.endStructure()
  ...
  suspend fun MPTZMoveToPreset(presetID: UByte
      ,timedInvokeTimeout: Duration? = null) {
  ...
  suspend fun MPTZSavePreset(presetID: UByte?
      ,name: String
      ,timedInvokeTimeout: Duration? = null) {
  ...
  suspend fun MPTZRemovePreset(presetID: UByte
      ,timedInvokeTimeout: Duration? = null) {
  ...
  suspend fun DPTZSetViewport(videoStreamID: UShort
  ...
  suspend fun DPTZRelativeMove(videoStreamID: UShort
      ,deltaX: Short?
      ,deltaY: Short?
      ,zoomDelta: Byte?
      ,timedInvokeTimeout: Duration? = null) {
    ...
    deltaX?.let {
      tlvWriter.put(ContextSpecificTag(TAG_DELTA_X_REQ), deltaX)
    }

    val TAG_DELTA_Y_REQ: Int = 2
    deltaY?.let {
      tlvWriter.put(ContextSpecificTag(TAG_DELTA_Y_REQ), deltaY)
    }
...
suspend fun readMPTZPositionAttribute(): MPTZPositionAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Mptzposition attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: CameraAvSettingsUserLevelManagementClusterMPTZStruct? = if (tlvReader.isNextTag(AnonymousTag)) {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(MPTZPositionAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Mptzposition attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(MPTZPositionAttributeSubscriptionState.Success(it))
          }
...
suspend fun readMaxPresetsAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Maxpresets attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Maxpresets attribute not found in Node State update" 
          }
...
suspend fun readMPTZPresetsAttribute(): MPTZPresetsAttribute {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Mptzpresets attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<CameraAvSettingsUserLevelManagementClusterMPTZPresetStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
      buildList<CameraAvSettingsUserLevelManagementClusterMPTZPresetStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(CameraAvSettingsUserLevelManagementClusterMPTZPresetStruct.fromTlv(AnonymousTag, tlvReader))
      }
  ...
  suspend fun subscribeMPTZPresetsAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(MPTZPresetsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Mptzpresets attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(MPTZPresetsAttributeSubscriptionState.Success(it))
          }
...
suspend fun readDPTZStreamsAttribute(): DPTZStreamsAttribute {val ATTRIBUTE_ID: UInt = 3u
    ...
    requireNotNull(attributeData) { 
      "Dptzstreams attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<CameraAvSettingsUserLevelManagementClusterDPTZStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
      buildList<CameraAvSettingsUserLevelManagementClusterDPTZStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(CameraAvSettingsUserLevelManagementClusterDPTZStruct.fromTlv(AnonymousTag, tlvReader))
      }
      tlvReader.exitContainer()
    }
    } else {
  ...
  suspend fun subscribeDPTZStreamsAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(DPTZStreamsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          ...
          requireNotNull(attributeData) { 
            "Dptzstreams attribute not found in Node State update" 
          }
      ...
      null
          ...
          decodedValue?.let {
            emit(DPTZStreamsAttributeSubscriptionState.Success(it))
          }
...
suspend fun readZoomMaxAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 4u
    ...
    requireNotNull(attributeData) { 
      "Zoommax attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Zoommax attribute not found in Node State update" 
          }
...
suspend fun readTiltMinAttribute(): Short? {val ATTRIBUTE_ID: UInt = 5u
    ...
    requireNotNull(attributeData) { 
      "Tiltmin attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Tiltmin attribute not found in Node State update" 
          }
...
suspend fun readTiltMaxAttribute(): Short? {val ATTRIBUTE_ID: UInt = 6u
    ...
    requireNotNull(attributeData) { 
      "Tiltmax attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Tiltmax attribute not found in Node State update" 
          }
...
suspend fun readPanMinAttribute(): Short? {val ATTRIBUTE_ID: UInt = 7u
    ...
    requireNotNull(attributeData) { 
      "Panmin attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Panmin attribute not found in Node State update" 
          }
...
suspend fun readPanMaxAttribute(): Short? {val ATTRIBUTE_ID: UInt = 8u
    ...
    requireNotNull(attributeData) { 
      "Panmax attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Panmax attribute not found in Node State update" 
          }
...
suspend fun readMovementStateAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 9u
    ...
    requireNotNull(attributeData) { 
      "Movementstate attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Movementstate attribute not found in Node State update" 
          }
...
suspend fun readClusterRevisionAttribute(): UShort {val ATTRIBUTE_ID: UInt = 65533u
  ...
  companion object {
    private val logger = Logger.getLogger(CameraAvSettingsUserLevelManagementCluster::class.java.name)
