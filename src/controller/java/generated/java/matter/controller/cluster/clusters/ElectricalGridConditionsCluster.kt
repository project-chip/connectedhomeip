...
class ElectricalGridConditionsCluster(private val controller: MatterController, private val endpointId: UShort) {class LocalGenerationAvailableAttribute(
    val value: Boolean?
  )
  ...
  sealed class LocalGenerationAvailableAttributeSubscriptionState {
    ...
    data class Error(val exception: Exception) : LocalGenerationAvailableAttributeSubscriptionState()
...
suspend fun readLocalGenerationAvailableAttribute(): LocalGenerationAvailableAttribute {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Localgenerationavailable attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(LocalGenerationAvailableAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
      ...
      null
          ...
          decodedValue?.let {
            emit(LocalGenerationAvailableAttributeSubscriptionState.Success(it))
          }
...
suspend fun readCurrentConditionsAttribute(): CurrentConditionsAttribute {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Currentconditions attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: ElectricalGridConditionsClusterElectricalGridConditionsStruct? = if (!tlvReader.isNull()) {
      ElectricalGridConditionsClusterElectricalGridConditionsStruct.fromTlv(AnonymousTag, tlvReader)
    } else {
      tlvReader.getNull(AnonymousTag)
      null
    }
  ...
  suspend fun subscribeCurrentConditionsAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(CurrentConditionsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(CurrentConditionsAttributeSubscriptionState.Success(it))
          }
...
suspend fun readForecastConditionsAttribute(): ForecastConditionsAttribute {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Forecastconditions attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<ElectricalGridConditionsClusterElectricalGridConditionsStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
      buildList<ElectricalGridConditionsClusterElectricalGridConditionsStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(ElectricalGridConditionsClusterElectricalGridConditionsStruct.fromTlv(AnonymousTag, tlvReader))
      }
  ...
  suspend fun subscribeForecastConditionsAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(ForecastConditionsAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(ForecastConditionsAttributeSubscriptionState.Success(it))
          }
