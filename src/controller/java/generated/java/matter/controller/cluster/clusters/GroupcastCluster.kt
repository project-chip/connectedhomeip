...
class AttributeListAttribute(
  ...
  suspend fun leaveGroup(groupID: UShort
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_ENDPOINTS)) {endpoints_decoded = 
          if (tlvReader.isNull()) {
      ...
      if (tag == ContextSpecificTag(TAG_LIST_TOO_LARGE)) {listTooLarge_decoded = 
          if (tlvReader.isNull()) {
            tlvReader.getNull(tag)
            null
          } else {
  ...
  suspend fun expireGracePeriod(groupID: UShort
      ,timedInvokeTimeout: Duration? = null) {
