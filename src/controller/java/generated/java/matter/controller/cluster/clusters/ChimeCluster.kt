...
class AttributeListAttribute(
  ...
  suspend fun playChimeSound(chimeID: UByte?
    ...
    chimeID?.let {
      tlvWriter.put(ContextSpecificTag(TAG_CHIME_ID_REQ), chimeID)
    }    
...
suspend fun readSelectedChimeAttribute(): UByte {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Selectedchime attribute not found in response" 
    }
  ...
  suspend fun writeSelectedChimeAttribute(
    value: UByte,
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Selectedchime attribute not found in Node State update" 
          }
