...
class ChimeCluster(private val controller: MatterController, private val endpointId: UShort) {
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            InstalledChimeSoundsAttributeSubscriptionState.Error(
              Exception(
  ...
  suspend fun writeEnabledAttribute(value: Boolean, timedWriteTimeout: Duration? = null) {
    val ATTRIBUTE_ID: UInt = 2u
