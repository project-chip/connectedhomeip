...
class JointFabricPkiCluster(
  private val controller: MatterController,
  private val endpointId: UShort,
) {
  class SignNOCIssuerRequest(val NOCIssuerCSR: ByteArray)

  class JointFabricResponse(val statusCode: UByte, val fabricIndex: UByte?, val debugText: String?)
  ...
  suspend fun jointFabricRequest(
    fabricIndex: ULong,
    timedInvokeTimeout: Duration? = null,
  ): SignNOCIssuerRequest {
    ...
    val TAG_N_O_C_ISSUER_C_S_R: Int = 0
    var NOCIssuerCSR_decoded: ByteArray? = null

    while (!tlvReader.isEndOfContainer()) {
      val tag = tlvReader.peekElement().tag

      if (tag == ContextSpecificTag(TAG_N_O_C_ISSUER_C_S_R)) {
        NOCIssuerCSR_decoded = tlvReader.getByteArray(tag)
      } else {
        tlvReader.skipElement()
      }
    }

    if (NOCIssuerCSR_decoded == null) {
      throw IllegalStateException("NOCIssuerCSR not found in TLV")
    }

    tlvReader.exitContainer()

    return SignNOCIssuerRequest(NOCIssuerCSR_decoded)
  }

  suspend fun signNOCIssuerResponse(
    statusCode: UByte,
    NOCIssuerCert: ByteArray,
    nodeId: ULong,
    fabricId: ULong,
    ...
    val TAG_STATUS_CODE_REQ: Int = 0
    tlvWriter.put(ContextSpecificTag(TAG_STATUS_CODE_REQ), statusCode)

    val TAG_N_O_C_ISSUER_CERT_REQ: Int = 1
    tlvWriter.put(ContextSpecificTag(TAG_N_O_C_ISSUER_CERT_REQ), NOCIssuerCert)

    val TAG_NODE_ID_REQ: Int = 2
    tlvWriter.put(ContextSpecificTag(TAG_NODE_ID_REQ), nodeId)

    val TAG_FABRIC_ID_REQ: Int = 3
    tlvWriter.put(ContextSpecificTag(TAG_FABRIC_ID_REQ), fabricId)

    val TAG_ADMIN_VENDOR_ID_REQ: Int = 4
    tlvWriter.put(ContextSpecificTag(TAG_ADMIN_VENDOR_ID_REQ), adminVendorId)

    val TAG_CASE_ADMIN_SUBJECT_REQ: Int = 5
    ...
    return JointFabricResponse(statusCode_decoded, fabricIndex_decoded, debugText_decoded)
  }
  ...
  companion object {
    private val logger = Logger.getLogger(JointFabricPkiCluster::class.java.name)
    const val CLUSTER_ID: UInt = 61u
  }
}
