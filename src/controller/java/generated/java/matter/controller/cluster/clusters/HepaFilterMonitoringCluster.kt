...
class HepaFilterMonitoringCluster(private val controller: MatterController, private val endpointId: UShort) {class LastChangedTimeAttribute(
    val value: UInt?
  )
  ...
  sealed class LastChangedTimeAttributeSubscriptionState {
    data class Success(
    val value: UInt?
    ) : LastChangedTimeAttributeSubscriptionState()
...
suspend fun readConditionAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Condition attribute not found in response" 
    }
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { 
            "Condition attribute not found in Node State update" 
          }
...
suspend fun readDegradationDirectionAttribute(): UByte? {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Degradationdirection attribute not found in response" 
    }
...
suspend fun readChangeIndicationAttribute(): UByte {val ATTRIBUTE_ID: UInt = 2u
    ...
    requireNotNull(attributeData) { 
      "Changeindication attribute not found in response" 
    }
...
suspend fun readInPlaceIndicatorAttribute(): Boolean? {val ATTRIBUTE_ID: UInt = 3u
    ...
    requireNotNull(attributeData) { 
      "Inplaceindicator attribute not found in response" 
    }
...
suspend fun readLastChangedTimeAttribute(): LastChangedTimeAttribute {val ATTRIBUTE_ID: UInt = 4u
    ...
    requireNotNull(attributeData) { 
      "Lastchangedtime attribute not found in response" 
    }
  ...
  suspend fun subscribeLastChangedTimeAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(LastChangedTimeAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(LastChangedTimeAttributeSubscriptionState.Success(it))
          }
...
suspend fun readReplacementProductListAttribute(): ReplacementProductListAttribute {val ATTRIBUTE_ID: UInt = 5u
    ...
    requireNotNull(attributeData) { 
      "Replacementproductlist attribute not found in response" 
    }

    // Decode the TLV data into the appropriate type
    val tlvReader = TlvReader(attributeData.data)
    val decodedValue: List<HepaFilterMonitoringClusterReplacementProductStruct>? = if (tlvReader.isNextTag(AnonymousTag)) {
      buildList<HepaFilterMonitoringClusterReplacementProductStruct> {
      tlvReader.enterArray(AnonymousTag)
      while(!tlvReader.isEndOfContainer()) {
        add(HepaFilterMonitoringClusterReplacementProductStruct.fromTlv(AnonymousTag, tlvReader))
      }
  ...
  suspend fun subscribeReplacementProductListAttribute(
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(ReplacementProductListAttributeSubscriptionState.Error(Exception("Subscription terminated with error code: ${subscriptionState.terminationCause}")))
        }
        is SubscriptionState.NodeStateUpdate -> {
          val attributeData =
            subscriptionState.updateState.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
      ...
      null
          ...
          decodedValue?.let {
            emit(ReplacementProductListAttributeSubscriptionState.Success(it))
          }
