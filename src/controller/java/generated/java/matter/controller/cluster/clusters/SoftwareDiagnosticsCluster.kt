...
class SoftwareDiagnosticsCluster(
  ...
  sealed class ThreadMetricsAttributeSubscriptionState {
    data class Success(val value: List<SoftwareDiagnosticsClusterThreadMetricsStruct>?) :
    ...
    return controller.subscribe(subscribeRequest).transform { subscriptionState ->
      when (subscriptionState) {
        is SubscriptionState.SubscriptionErrorNotification -> {
          emit(
            ThreadMetricsAttributeSubscriptionState.Error(
              Exception(
        ...
        is SubscriptionState.NodeStateUpdate -> {
          ...
          requireNotNull(attributeData) { "Threadmetrics attribute not found in Node State update" }
          ...
          val decodedValue: List<SoftwareDiagnosticsClusterThreadMetricsStruct>? =
            if (tlvReader.isNextTag(AnonymousTag)) {
              buildList<SoftwareDiagnosticsClusterThreadMetricsStruct> {
                ...
                while (!tlvReader.isEndOfContainer()) {
                  add(
                    SoftwareDiagnosticsClusterThreadMetricsStruct.fromTlv(AnonymousTag, tlvReader)
                  )
                }
                tlvReader.exitContainer()
              }
          ...
          decodedValue?.let { emit(ThreadMetricsAttributeSubscriptionState.Success(it)) }
        }
