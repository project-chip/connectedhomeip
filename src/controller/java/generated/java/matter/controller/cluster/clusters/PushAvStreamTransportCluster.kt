...
class AttributeListAttribute(
  ...
  suspend fun allocatePushTransport(transportOptions: PushAvStreamTransportClusterTransportOptionsStruct
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_CONNECTION_ID)) {connectionID_decoded = tlvReader.getUShort(tag)}  
      
      if (tag == ContextSpecificTag(TAG_TRANSPORT_OPTIONS)) {transportOptions_decoded = PushAvStreamTransportClusterTransportOptionsStruct.fromTlv(tag, tlvReader)}  
      
      if (tag == ContextSpecificTag(TAG_TRANSPORT_STATUS)) {transportStatus_decoded = tlvReader.getUByte(tag)}  
  ...
  suspend fun findTransport(connectionID: UShort?
    ...
    var streamConfigurations_decoded: List<PushAvStreamTransportClusterTransportConfigurationStruct>? = null
    ...
    while (!tlvReader.isEndOfContainer()) {
      ...
      if (tag == ContextSpecificTag(TAG_STREAM_CONFIGURATIONS)) {streamConfigurations_decoded = buildList<PushAvStreamTransportClusterTransportConfigurationStruct> {
...
suspend fun readSupportedContainerFormatsAttribute(): UByte {val ATTRIBUTE_ID: UInt = 0u
    ...
    requireNotNull(attributeData) { 
      "Supportedcontainerformats attribute not found in response" 
    }
...
suspend fun readSupportedIngestMethodsAttribute(): UByte {val ATTRIBUTE_ID: UInt = 1u
    ...
    requireNotNull(attributeData) { 
      "Supportedingestmethods attribute not found in response" 
    }
...
suspend fun readCurrentConnectionsAttribute(): CurrentConnectionsAttribute {val ATTRIBUTE_ID: UInt = 2u
