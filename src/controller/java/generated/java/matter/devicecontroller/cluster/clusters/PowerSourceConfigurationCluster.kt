...
class PowerSourceConfigurationCluster(
  ...
  suspend fun readSourcesAttribute(): SourcesAttribute {
    val ATTRIBUTE_ID_SOURCES: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SOURCES
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SOURCES
        }

      requireNotNull(attributeData) { "Sources attribute not found in response" }
      ...
      return SourcesAttribute(decodedValue)
    } else {
  ...
  companion object {
    private val logger = Logger.getLogger(PowerSourceConfigurationCluster::class.java.name)
