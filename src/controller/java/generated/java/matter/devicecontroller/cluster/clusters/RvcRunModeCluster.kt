...
class RvcRunModeCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  suspend fun readSupportedModesAttribute(): SupportedModesAttribute {
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val decodedValue: List<RvcRunModeClusterModeOptionStruct> =
        buildList<RvcRunModeClusterModeOptionStruct> {
          tlvReader.enterArray(AnonymousTag)
          while (!tlvReader.isEndOfContainer()) {
            add(RvcRunModeClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
          }
          tlvReader.exitContainer()
        }
  ...
  companion object {
    private val logger = Logger.getLogger(RvcRunModeCluster::class.java.name)
