...
class TargetNavigatorCluster(
  ...
  suspend fun readTargetListAttribute(): TargetListAttribute {
    val ATTRIBUTE_ID_TARGETLIST: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_TARGETLIST
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_TARGETLIST
        }
  ...
  suspend fun readCurrentTargetAttribute(): UByte? {
    val ATTRIBUTE_ID_CURRENTTARGET: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_CURRENTTARGET
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_CURRENTTARGET
        }
