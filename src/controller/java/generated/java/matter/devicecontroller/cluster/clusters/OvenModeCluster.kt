...
class OvenModeCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  class SupportedModesAttribute(val value: List<OvenModeClusterModeOptionStruct>)
  ...
  suspend fun changeToMode(
    ...
    val statusText_decoded =
      if (tlvReader.isNextTag(ContextSpecificTag(TAG_STATUS_TEXT))) {
        tlvReader.getString(ContextSpecificTag(TAG_STATUS_TEXT))
      } else {
        null
      }
  ...
  suspend fun readSupportedModesAttribute(): SupportedModesAttribute {
    val ATTRIBUTE_ID_SUPPORTEDMODES: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SUPPORTEDMODES
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SUPPORTEDMODES
        }
      ...
      val decodedValue: List<OvenModeClusterModeOptionStruct> =
        buildList<OvenModeClusterModeOptionStruct> {
          tlvReader.enterArray(AnonymousTag)
          while (!tlvReader.isEndOfContainer()) {
            add(OvenModeClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
          }
          tlvReader.exitContainer()
        }
  ...
  suspend fun readCurrentModeAttribute(): UByte {
    val ATTRIBUTE_ID_CURRENTMODE: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_CURRENTMODE
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_CURRENTMODE
        }
  ...
  suspend fun readStartUpModeAttribute(): StartUpModeAttribute {
    val ATTRIBUTE_ID_STARTUPMODE: UInt = 2u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_STARTUPMODE
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_STARTUPMODE
        }
  ...
  suspend fun readOnModeAttribute(): OnModeAttribute {
    val ATTRIBUTE_ID_ONMODE: UInt = 3u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_ONMODE
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_ONMODE
        }
  ...
  companion object {
    private val logger = Logger.getLogger(OvenModeCluster::class.java.name)
