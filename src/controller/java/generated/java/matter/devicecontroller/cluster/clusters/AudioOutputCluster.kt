...
class AudioOutputCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  suspend fun readOutputListAttribute(): OutputListAttribute {
    val ATTRIBUTE_ID_OUTPUTLIST: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_OUTPUTLIST
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_OUTPUTLIST
        }
  ...
  suspend fun readCurrentOutputAttribute(): UByte {
    val ATTRIBUTE_ID_CURRENTOUTPUT: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_CURRENTOUTPUT
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_CURRENTOUTPUT
        }
