...
class ApplicationLauncherCluster(
  ...
  suspend fun launchApp(
    ...
    val data_decoded =
      if (tlvReader.isNextTag(ContextSpecificTag(TAG_DATA))) {
        tlvReader.getByteArray(ContextSpecificTag(TAG_DATA))
      } else {
        null
      }
  ...
  suspend fun readCatalogListAttribute(): CatalogListAttribute {
    val ATTRIBUTE_ID_CATALOGLIST: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_CATALOGLIST
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_CATALOGLIST
        }
  ...
  suspend fun readCurrentAppAttribute(): CurrentAppAttribute {
    val ATTRIBUTE_ID_CURRENTAPP: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_CURRENTAPP
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_CURRENTAPP
        }
