...
class TimeFormatLocalizationCluster(
  ...
  suspend fun readHourFormatAttribute(): UByte {
    val ATTRIBUTE_ID_HOURFORMAT: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_HOURFORMAT
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_HOURFORMAT
        }
  ...
  suspend fun readActiveCalendarTypeAttribute(): UByte? {
    val ATTRIBUTE_ID_ACTIVECALENDARTYPE: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_ACTIVECALENDARTYPE
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_ACTIVECALENDARTYPE
        }
  ...
  suspend fun readSupportedCalendarTypesAttribute(): SupportedCalendarTypesAttribute {
    val ATTRIBUTE_ID_SUPPORTEDCALENDARTYPES: UInt = 2u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SUPPORTEDCALENDARTYPES
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SUPPORTEDCALENDARTYPES
        }
