...
class DishwasherAlarmCluster(
  ...
  suspend fun reset(alarms: UInt, timedInvokeTimeoutMs: Int? = null) {
  ...
  suspend fun modifyEnabledAlarms(mask: UInt, timedInvokeTimeoutMs: Int? = null) {
  ...
  suspend fun readMaskAttribute(): UInt {
    val ATTRIBUTE_ID_MASK: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_MASK
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_MASK
        }
  ...
  suspend fun readLatchAttribute(): UInt? {
    val ATTRIBUTE_ID_LATCH: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_LATCH
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_LATCH
        }
  ...
  suspend fun readStateAttribute(): UInt {
    val ATTRIBUTE_ID_STATE: UInt = 2u
  ...
  suspend fun readSupportedAttribute(): UInt {
    val ATTRIBUTE_ID_SUPPORTED: UInt = 3u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SUPPORTED
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SUPPORTED
        }
  ...
  companion object {
    private val logger = Logger.getLogger(DishwasherAlarmCluster::class.java.name)
