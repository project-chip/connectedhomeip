...
class AdministratorCommissioningCluster(
  ...
  suspend fun readWindowStatusAttribute(): UByte {
    val ATTRIBUTE_ID_WINDOWSTATUS: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_WINDOWSTATUS
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_WINDOWSTATUS
        }
  ...
  suspend fun readAdminFabricIndexAttribute(): AdminFabricIndexAttribute {
    val ATTRIBUTE_ID_ADMINFABRICINDEX: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_ADMINFABRICINDEX
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_ADMINFABRICINDEX
        }
  ...
  suspend fun readAdminVendorIdAttribute(): AdminVendorIdAttribute {
    val ATTRIBUTE_ID_ADMINVENDORID: UInt = 2u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_ADMINVENDORID
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_ADMINVENDORID
        }
