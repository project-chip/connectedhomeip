...
class UnitTestingClusterTestEventEvent(
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader) : UnitTestingClusterTestEventEvent {
      ...
      val arg4 = matter.devicecontroller.cluster.structs.UnitTestingClusterSimpleStruct.fromTlv(ContextSpecificTag(TAG_ARG4), tlvReader)
      val arg5 = buildList <matter.devicecontroller.cluster.structs.UnitTestingClusterSimpleStruct> {
        tlvReader.enterArray(ContextSpecificTag(TAG_ARG5))
        while(!tlvReader.isEndOfContainer()) {
          this.add(matter.devicecontroller.cluster.structs.UnitTestingClusterSimpleStruct.fromTlv(AnonymousTag, tlvReader))
        }
        tlvReader.exitContainer()
      }
      val arg6 = buildList <UInt> {
