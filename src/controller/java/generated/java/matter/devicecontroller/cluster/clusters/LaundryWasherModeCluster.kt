...
class LaundryWasherModeCluster(
  ...
  suspend fun readSupportedModesAttribute(): SupportedModesAttribute {
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val decodedValue: List<LaundryWasherModeClusterModeOptionStruct> =
        buildList<LaundryWasherModeClusterModeOptionStruct> {
          tlvReader.enterArray(AnonymousTag)
          while (!tlvReader.isEndOfContainer()) {
            add(LaundryWasherModeClusterModeOptionStruct.fromTlv(AnonymousTag, tlvReader))
          }
          tlvReader.exitContainer()
        }
  ...
  companion object {
    private val logger = Logger.getLogger(LaundryWasherModeCluster::class.java.name)
