...
class AirQualityCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  suspend fun readAirQualityAttribute(): UByte {
    val ATTRIBUTE_ID_AIRQUALITY: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_AIRQUALITY
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_AIRQUALITY
        }

      requireNotNull(attributeData) { "Airquality attribute not found in response" }
  ...
  companion object {
    private val logger = Logger.getLogger(AirQualityCluster::class.java.name)
