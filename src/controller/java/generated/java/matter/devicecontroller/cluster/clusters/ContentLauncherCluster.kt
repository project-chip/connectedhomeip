...
class ContentLauncherCluster(
  ...
  suspend fun readAcceptHeaderAttribute(): AcceptHeaderAttribute {
    val ATTRIBUTE_ID_ACCEPTHEADER: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_ACCEPTHEADER
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_ACCEPTHEADER
        }
  ...
  suspend fun readSupportedStreamingProtocolsAttribute(): UInt? {
    val ATTRIBUTE_ID_SUPPORTEDSTREAMINGPROTOCOLS: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SUPPORTEDSTREAMINGPROTOCOLS
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SUPPORTEDSTREAMINGPROTOCOLS
        }
