...
class TimerCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  suspend fun readSetTimeAttribute(): UInt {
    val ATTRIBUTE_ID_SETTIME: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SETTIME
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SETTIME
        }

      requireNotNull(attributeData) { "Settime attribute not found in response" }
  ...
  suspend fun readTimeRemainingAttribute(): UInt {
    val ATTRIBUTE_ID_TIMEREMAINING: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_TIMEREMAINING
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_TIMEREMAINING
        }

      requireNotNull(attributeData) { "Timeremaining attribute not found in response" }
  ...
  suspend fun readTimerStateAttribute(): UByte {
    val ATTRIBUTE_ID_TIMERSTATE: UInt = 2u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_TIMERSTATE
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_TIMERSTATE
        }

      requireNotNull(attributeData) { "Timerstate attribute not found in response" }
