...
class TemperatureMeasurementCluster(
  ...
  suspend fun readToleranceAttribute(): UShort? {
    val ATTRIBUTE_ID_TOLERANCE: UInt = 3u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_TOLERANCE
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_TOLERANCE
        }
  ...
  companion object {
    private val logger = Logger.getLogger(TemperatureMeasurementCluster::class.java.name)
