...
class LocalizationConfigurationCluster(
  ...
  suspend fun readActiveLocaleAttribute(): String {
    val ATTRIBUTE_ID_ACTIVELOCALE: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_ACTIVELOCALE
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_ACTIVELOCALE
        }
  ...
  suspend fun readSupportedLocalesAttribute(): SupportedLocalesAttribute {
    val ATTRIBUTE_ID_SUPPORTEDLOCALES: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SUPPORTEDLOCALES
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SUPPORTEDLOCALES
        }
