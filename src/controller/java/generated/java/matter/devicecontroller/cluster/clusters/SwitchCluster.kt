...
class SwitchCluster(private val controller: MatterController, private val endpointId: UShort) {
  ...
  suspend fun readNumberOfPositionsAttribute(): UByte {
    val ATTRIBUTE_ID_NUMBEROFPOSITIONS: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_NUMBEROFPOSITIONS
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_NUMBEROFPOSITIONS
        }

      requireNotNull(attributeData) { "Numberofpositions attribute not found in response" }
  ...
  suspend fun readCurrentPositionAttribute(): UByte {
    val ATTRIBUTE_ID_CURRENTPOSITION: UInt = 1u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_CURRENTPOSITION
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_CURRENTPOSITION
        }

      requireNotNull(attributeData) { "Currentposition attribute not found in response" }
  ...
  suspend fun readMultiPressMaxAttribute(): UByte? {
    val ATTRIBUTE_ID_MULTIPRESSMAX: UInt = 2u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_MULTIPRESSMAX
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_MULTIPRESSMAX
        }

      requireNotNull(attributeData) { "Multipressmax attribute not found in response" }
  ...
  companion object {
    private val logger = Logger.getLogger(SwitchCluster::class.java.name)
