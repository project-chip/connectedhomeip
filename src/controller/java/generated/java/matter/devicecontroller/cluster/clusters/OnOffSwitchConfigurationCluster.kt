...
class OnOffSwitchConfigurationCluster(
  ...
  suspend fun readSwitchTypeAttribute(): UByte {
    val ATTRIBUTE_ID_SWITCHTYPE: UInt = 0u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SWITCHTYPE
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SWITCHTYPE
        }

      requireNotNull(attributeData) { "Switchtype attribute not found in response" }
  ...
  suspend fun readSwitchActionsAttribute(): UByte {
    val ATTRIBUTE_ID_SWITCHACTIONS: UInt = 16u

    val attributePath =
      AttributePath(
        endpointId = endpointId,
        clusterId = CLUSTER_ID,
        attributeId = ATTRIBUTE_ID_SWITCHACTIONS
      )
    ...
    if (response.successes.isNotEmpty()) {
      ...
      val attributeData =
        response.successes.filterIsInstance<ReadData.Attribute>().firstOrNull {
          it.path.attributeId == ATTRIBUTE_ID_SWITCHACTIONS
        }

      requireNotNull(attributeData) { "Switchactions attribute not found in response" }
