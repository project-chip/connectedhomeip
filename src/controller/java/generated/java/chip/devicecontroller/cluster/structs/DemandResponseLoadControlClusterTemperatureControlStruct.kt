...
class DemandResponseLoadControlClusterTemperatureControlStruct(
  val coolingTempOffset: Optional<Int>?,
  val heatingTempOffset: Optional<Int>?,
      ...
      val coolingTempOffset =
        if (!tlvReader.isNull()) {
          if (tlvReader.isNextTag(ContextSpecificTag(TAG_COOLING_TEMP_OFFSET))) {
            Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_COOLING_TEMP_OFFSET)))
          } else {
            Optional.empty()
          }
        } else {
      ...
      val heatingTempOffset =
        if (!tlvReader.isNull()) {
          if (tlvReader.isNextTag(ContextSpecificTag(TAG_HEATING_TEMP_OFFSET))) {
            Optional.of(tlvReader.getInt(ContextSpecificTag(TAG_HEATING_TEMP_OFFSET)))
          } else {
            Optional.empty()
          }
        } else {
