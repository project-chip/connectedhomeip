...
class DynamicLightingClusterEffectColorStruct(
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): DynamicLightingClusterEffectColorStruct {
      ...
      val level =
        if (!tlvReader.isNull()) {
          tlvReader.getUInt(ContextSpecificTag(TAG_LEVEL))
        } else {
      ...
      val x =
        if (!tlvReader.isNull()) {
          tlvReader.getUInt(ContextSpecificTag(TAG_X))
        } else {
      ...
      val y =
        if (!tlvReader.isNull()) {
          tlvReader.getUInt(ContextSpecificTag(TAG_Y))
        } else {
      ...
      val hue =
        if (!tlvReader.isNull()) {
          tlvReader.getUInt(ContextSpecificTag(TAG_HUE))
        } else {
      ...
      val enhancedHue =
        if (!tlvReader.isNull()) {
          tlvReader.getUInt(ContextSpecificTag(TAG_ENHANCED_HUE))
        } else {
      ...
      val saturation =
        if (!tlvReader.isNull()) {
          tlvReader.getUInt(ContextSpecificTag(TAG_SATURATION))
        } else {
