...
class ContentLauncherClusterLaunchUrlInfo(
  ...
  val offsetMillisecs: Optional<ULong>?,
  val queueType: Optional<UInt>?,
      ...
      val offsetMillisecs =
        if (!tlvReader.isNull()) {
          if (tlvReader.isNextTag(ContextSpecificTag(TAG_OFFSET_MILLISECS))) {
            Optional.of(tlvReader.getULong(ContextSpecificTag(TAG_OFFSET_MILLISECS)))
          } else {
            Optional.empty()
          }
        } else {
      ...
      val queueType =
        if (!tlvReader.isNull()) {
          if (tlvReader.isNextTag(ContextSpecificTag(TAG_QUEUE_TYPE))) {
            Optional.of(tlvReader.getUInt(ContextSpecificTag(TAG_QUEUE_TYPE)))
          } else {
            Optional.empty()
          }
        } else {
