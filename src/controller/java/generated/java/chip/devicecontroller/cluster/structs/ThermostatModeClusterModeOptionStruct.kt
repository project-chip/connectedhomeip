...
class ThermostatModeClusterModeOptionStruct(
  val label: String,
  val mode: UInt,
  val modeTags: List<ThermostatModeClusterModeTagStruct>,
) {
  override fun toString(): String = buildString {
    append("ThermostatModeClusterModeOptionStruct {\n")
    append("\tlabel : $label\n")
    append("\tmode : $mode\n")
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): ThermostatModeClusterModeOptionStruct {
      tlvReader.enterStructure(tlvTag)
      val label = tlvReader.getString(ContextSpecificTag(TAG_LABEL))
      val mode = tlvReader.getUInt(ContextSpecificTag(TAG_MODE))
      val modeTags =
        buildList<ThermostatModeClusterModeTagStruct> {
          tlvReader.enterArray(ContextSpecificTag(TAG_MODE_TAGS))
          while (!tlvReader.isEndOfContainer()) {
            add(ThermostatModeClusterModeTagStruct.fromTlv(AnonymousTag, tlvReader))
          }
      ...
      return ThermostatModeClusterModeOptionStruct(label, mode, modeTags)
    }
  }
}
