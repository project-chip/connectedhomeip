...
class DynamicLightingClusterEffectStruct(
  ...
  companion object {
    ...
    fun fromTlv(tlvTag: Tag, tlvReader: TlvReader): DynamicLightingClusterEffectStruct {
      ...
      val effectID = tlvReader.getUInt(ContextSpecificTag(TAG_EFFECT_ID))
      val source = tlvReader.getUInt(ContextSpecificTag(TAG_SOURCE))
      val label = tlvReader.getString(ContextSpecificTag(TAG_LABEL))
      val maxSpeed = tlvReader.getUInt(ContextSpecificTag(TAG_MAX_SPEED))
      val defaultSpeed = tlvReader.getUInt(ContextSpecificTag(TAG_DEFAULT_SPEED))
