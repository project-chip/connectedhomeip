...
function convertBasicCTypeToJavaType(cType)
{
  ...
  case 'int16_t':
    return 'int'
    case 'uint32_t': case 'int32_t':
        // TODO(#8074): replace with BigInteger, or mark as unsigned.
        case 'uint64_t': case 'int64_t': return 'long'
    case 'bool': return 'boolean' default: error = 'Unhandled type ' + cType
    throw error
  }
}
...
function convertBasicCTypeToJniType(cType)
{
  ...
  case 'int':
    ...
    case 'boolean': return 'jboolean' default: error = 'Unhandled type ' + cType
    throw error
  }
}
...
function convertBasicCTypeToJavaBoxedType(cType)
{
  ...
  case 'int':
    ...
    case 'boolean': return 'Boolean' default: error = 'Unhandled type ' + cType
    throw error
  }
}
...
function asJavaBasicType(type)
{
  if (StringHelper.isOctetString(type)) {
    ...
    return convertBasicCTypeToJavaType(
  ...
  function fn(pkgId)
  {
    const options
        = { hash : {} } return zclHelper.asUnderlyingZclType.call(this, type, options)
              .then(zclType => { return convertBasicCTypeToJniType(
                        ChipTypesHelper.asBasicType(zclType),
                        ) })
  }
...
function asJniSignature(type)
{
  function fn(pkgId)
  {
    const options
        = { hash : {} } return zclHelper.asUnderlyingZclType.call(this, type, options)
              .then(zclType => { return convertCTypeToJniSignature(
                        ChipTypesHelper.asBasicType(zclType),
                        ) })
  }
...
function convertCTypeToJniSignature(cType)
{
  ...
  case 'int':
    return 'I'
    case 'long': return 'J'
    case 'boolean': return 'Z' default: error = 'Unhandled Java type ' + javaType + ' for C type ' + cType
    throw error
  }
}
...
function omitCommaForFirstNonStatusCommand(id, index)
{
    ...
    let firstNonStatusCommandIndex = result.findIndex(
        command => !command.isArray,
    )
