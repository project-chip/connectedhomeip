...
JNI_METHOD(jlong, newDeviceController)(JNIEnv * env, jobject self)
{
    ...
    wrapper =
        AndroidDeviceControllerWrapper::AllocateNew(sJVM, self, GetStackLock(), kLocalDeviceId, &sSystemLayer, &sInetLayer, &err);
    SuccessOrExit(err);
...
void * IOThreadMain(void * arg)
{
    ...
    pthread_mutex_lock(GetStackLock());
    ...
    while (!quit.load(std::memory_order_relaxed))
    {
        ...
        pthread_mutex_unlock(GetStackLock());
