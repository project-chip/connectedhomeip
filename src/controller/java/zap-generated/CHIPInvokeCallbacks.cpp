...
CHIPClientMonitoringClusterRegisterClientMonitoringResponseCallback::
    ...
    Callback::Callback<CHIPClientMonitoringClusterRegisterClientMonitoringResponseCallbackType>(CallbackFn, this)
{
    JNIEnv * env = JniReferences::GetInstance().GetEnvForCurrentThread();
    if (env == nullptr)
    {
...
void CHIPClientMonitoringClusterRegisterClientMonitoringResponseCallback::CallbackFn(
    void * context,
    const chip::app::Clusters::ClientMonitoring::Commands::RegisterClientMonitoringResponse::DecodableType & dataResponse)
{
    chip::DeviceLayer::StackUnlock unlock;
    CHIP_ERROR err = CHIP_NO_ERROR;
    JNIEnv * env   = JniReferences::GetInstance().GetEnvForCurrentThread();
    ...
    std::unique_ptr<CHIPClientMonitoringClusterRegisterClientMonitoringResponseCallback,
                    void (*)(CHIPClientMonitoringClusterRegisterClientMonitoringResponseCallback *)>
        cppCallback(reinterpret_cast<CHIPClientMonitoringClusterRegisterClientMonitoringResponseCallback *>(context),
                    chip::Platform::Delete<CHIPClientMonitoringClusterRegisterClientMonitoringResponseCallback>);
    VerifyOrReturn(cppCallback != nullptr, ChipLogError(Zcl, "Error invoking Java callback: failed to cast native callback"));
    ...
    chip::JniReferences::GetInstance().CreateBoxedObject<uint8_t>(StatusClassName.c_str(), StatusCtorSignature.c_str(),
                                                                  dataResponse.status, Status);
    jobject ICDCounter;
    if (dataResponse.ICDCounter.IsNull())
    {
        ICDCounter = nullptr;
    }
    else
    {
        ...
        chip::JniReferences::GetInstance().CreateBoxedObject<uint32_t>(ICDCounterClassName.c_str(), ICDCounterCtorSignature.c_str(),
                                                                       dataResponse.ICDCounter.Value(), ICDCounter);
    }

    env->CallVoidMethod(javaCallbackRef, javaMethod, Status, ICDCounter);
}
