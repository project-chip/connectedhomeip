...
public:
    ...
    const uint8_t (&GetMacAddress() const)[6] { return macAddress; }
    BLEDeviceConnectionParameters & SetMacAddress(const uint8_t (&value)[6])
    {
...
public:
     ...
     * @return CHIP_ERROR               The connection status
     */
    CHIP_ERROR ConnectDevice(NodeId remoteDeviceId, uint8_t (&macAddress)[6], const uint32_t setupPINCode, void * appReqState,
                             NewConnectionHandler onConnected, MessageReceiveHandler onMessageReceived, ErrorHandler onError)
    {
        return ConnectDevice(BLEDeviceConnectionParameters()
                                 .SetRemoteDeviceId(remoteDeviceId)
