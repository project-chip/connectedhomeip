...
def test_commissioning(device, controller):
    ...
    assert discover_device(devCtrl, setupPayload)

    ipAddress = get_discovered_device_ip_address(devCtrl, 0)
    assert ipAddress != None
    log.info("Found device at {} address".format(ipAddress))

    nodeId = connect_device(devCtrl, setupPayload, ipAddress)
    assert nodeId != None
    log.info("Device {} connected".format(ipAddress))
