...
class CHIPVirtualHome:
    ...
    def execute_device_cmd(self, device_id, cmd, stream=False):
        ...
        ret = requests.get(
            self._build_request_url("device_cmd", [self.home_id, device_id, cmd]),
        ...
        self.logger.info(
            "command return code: {}".format(ret_struct.get("return_code", "Unknown"))
        )
    ...
    def reset_thread_devices(self, devices: Union[List[str], str]):
        ...
        for device_id in devices:
            ...
            self.assertTrue(
                self.wait_for_device_output(device_id, "[SVR] Server Listening...", 15)
            )
            # Clear default Thread network commissioning data
            self.logger.info(
                "Resetting thread network on {}".format(
    ...
    def form_thread_network(
        self,
        device_id: str,
        expected_role: Union[str, List[str]],
        timeout: int = 15,
        dataset: str = "",
    ):
        """
    ...
    def connect_to_thread_network(self):
        ...
        for i in range(30):
            ...
            threadNetworkFormed = (roles.count("leader") == 1) and (
                roles.count("leader") + roles.count("router") + roles.count("child")
                == len(self.thread_devices)
            )
    ...
    def get_device_thread_ip(self, device_id):
        ...
        for ipstr in ipaddr_list:
            try:
                self.logger.info(
                    "device: {} thread ip: {}".format(
                        self.get_device_pretty_id(device_id), ipstr
                    )
                )
                ...
                if (
                    re.match(
                        (
                            "fd[0-9a-f]{2}:[0-9a-f]{4}:[0-9a-f]"
                            "{4}:[0-9a-f]{4}:0000:00ff:fe00:[0-9a-f]{4}"
                        ),
                        ipaddr.exploded,
                    )
                    is not None
    ...
    def wait_for_device_output(self, device_id, pattern, timeout=1):
        ...
        while True:
            if self.sequenceMatch(
                self.get_device_log(device_id).decode(),
                [
                    pattern,
                ],
            ):
    ...
    def _build_request_url(self, end_point, args=[]):
        ...
        return urljoin(
            self.cirque_url,
            "{}/{}".format(end_point, "/".join([str(argv) for argv in args])),
        )
    ...
    def initialize_home(self):
        home_id = requests.post(
            self._build_request_url("create_home"), json=self.device_config
        ).json()

        self.logger.info("home id: {} created!".format(home_id))

        self.assertTrue(
            home_id in list(self.query_api("get_homes")),
    ...
    def start_wpa_supplicant(self, device_id):
        self.logger.info(
            "device: {}: starting wpa_supplicant on device".format(
                self.get_device_pretty_id(device_id)
            )
        )
    ...
    def write_psk_to_wpa_supplicant_config(self, device_id, ssid, psk):
        self.logger.info(
            "device: {}: writing ssid, psk to wpa_supplicant config".format(
                self.get_device_pretty_id(device_id)
            )
        )
    ...
    def kill_existing_wpa_supplicant(self, device_id):
        self.logger.info(
            "device: {}: kill existing wpa_supplicant".format(
                self.get_device_pretty_id(device_id)
            )
        )
