...
#define JNI_METHOD(RETURN, CLASS_NAME, METHOD_NAME) \
  extern "C" JNIEXPORT RETURN JNICALL               \
      Java_chip_testing_##CLASS_NAME##_##METHOD_NAME
...
(JNIEnv *env, jobject self, jboolean initializeNodes) {
  LoopbackMessagingContext *messagingContext =
      chip::Platform::New<LoopbackMessagingContext>();
  messagingContext->ConfigInitializeNodes(initializeNodes != JNI_FALSE);
  CHIP_ERROR err = messagingContext->Init();
  if (err != CHIP_NO_ERROR) {
    jclass exceptionCls =
        env->FindClass("chip/platform/AndroidChipPlatformException");
...
(JNIEnv *env, jobject self, jlong contextHandle) {
  MessagingContext *messagingContext =
      reinterpret_cast<MessagingContext *>(contextHandle);
  VerifyOrReturn(messagingContext != nullptr,
                 ChipLogError(Test, "MessagingContext handle is nullptr"));
