...
CHIP_ERROR WiFiPAFEndPoint::GetPktSn(Encoding::LittleEndian::Reader & reader, uint8_t * pHead, SequenceNumber_t & seqNum)
{
    ...
    VerifyOrReturnError(SnOffset < reader.OctetsRead() + reader.Remaining(), CHIP_ERROR_MESSAGE_INCOMPLETE);
    pSn    = pHead + SnOffset;
    seqNum = *pSn;
...
CHIP_ERROR WiFiPAFEndPoint::DebugPktAckSn(const PktDirect_t PktDirect, Encoding::LittleEndian::Reader & reader, uint8_t * pHead)
{
    ...
    VerifyOrExit(SnOffset < reader.OctetsRead() + reader.Remaining(), err = CHIP_ERROR_MESSAGE_INCOMPLETE);
    pSn = pHead + SnOffset;
    if (pAct == nullptr)
    {
        strcpy(AckBuff, "  ");
