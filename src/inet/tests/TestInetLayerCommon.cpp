    ...
    for (size_t i = aPatternStartOffset; i < aLength; i++)
    {
        ...
        if (lValue != static_cast<uint8_t>(aFirstValue))
        {
            printf("Bad data value at offset %u (0x%04x): "
                   ...
                   static_cast<unsigned>(i), static_cast<unsigned>(i), static_cast<unsigned>(aFirstValue), lValue);
...
void HandleRawMessageReceived(const UDPEndPointHandle & aEndPoint, const PacketBufferHandle & aBuffer,
...
void HandleRawReceiveError(const UDPEndPointHandle & aEndPoint, const CHIP_ERROR & aError, const IPPacketInfo * aPacketInfo)
{
