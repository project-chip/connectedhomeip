...
static void TestAKID_x509Extraction(nlTestSuite * inSuite, void * inContext)
{
    ...
    for (size_t i = 0; i < gNumTestCerts; i++)
    {
        ...
        NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);
        NL_TEST_ASSERT(inSuite, akidSpan.size() == akidOut.size() && memcmp(akidOut.data(), akidSpan.data(), akidSpan.size()) == 0);
    }
}
...
static void TestAttestationElements_Construction(nlTestSuite * inSuite, void * inContext)
{
...

    {
        ...
        err = ConstructAttestationElements(ByteSpan(certificationDeclaration), ByteSpan(attestationNonce), timestamp, ByteSpan(),
                                           ByteSpan(vendorReserved1), ByteSpan(), ByteSpan(vendorReserved3), ByteSpan(),
    ...
    ByteSpan vendorReserved4Span;
    err = DeconstructAttestationElements(ByteSpan(attestationElementsTestVector), certificationDeclarationSpan,
                                         attestationNonceSpan, timestampDeconstructed, firmwareInfoSpan, vendorReserved1Span,
                                         vendorReserved2Span, vendorReserved3Span, vendorReserved4Span);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);
    ...
    NL_TEST_ASSERT(inSuite, sizeof(vendorReserved1) == vendorReserved1Span.size());
    NL_TEST_ASSERT(inSuite, memcmp(vendorReserved1Span.data(), vendorReserved1, sizeof(vendorReserved1)) == 0);

    NL_TEST_ASSERT(inSuite, sizeof(vendorReserved3) == vendorReserved3Span.size());
    NL_TEST_ASSERT(inSuite, memcmp(vendorReserved3Span.data(), vendorReserved3, sizeof(vendorReserved3)) == 0);

    NL_TEST_ASSERT(inSuite, firmwareInfoSpan.empty());
    NL_TEST_ASSERT(inSuite, vendorReserved2Span.empty());
    NL_TEST_ASSERT(inSuite, vendorReserved4Span.empty());
}

static void TestAttestationElements_SigVer(nlTestSuite * inSuite, void * inContext)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    uint8_t deviceAttestationPrivkey[] = { 0x38, 0xf3, 0xe0, 0xa1, 0xf1, 0x45, 0xba, 0x1b, 0xf3, 0xe4, 0x4b,
                                           0x55, 0x2d, 0xef, 0x65, 0x27, 0x3d, 0x1d, 0x8e, 0x27, 0x6a, 0xa3,
                                           0x14, 0xac, 0x74, 0x2e, 0xb1, 0x28, 0x93, 0x3b, 0xa6, 0x4b };
    uint8_t deviceAttestationPubkey[]  = { 0x04, 0xce, 0x5c, 0xf8, 0xef, 0xb0, 0x5d, 0x4e, 0xee, 0x79, 0x0d, 0x0a, 0x71,
    ...
    uint8_t attestationSignatureTestVector[] = { 0x79, 0x82, 0x53, 0x5d, 0x24, 0xcf, 0xe1, 0x4a, 0x71, 0xab, 0x04, 0x24, 0xcf,
                                                 ...
                                                 0x9e, 0xcc, 0xfb, 0x39, 0x08, 0x1e, 0x9f, 0x1d, 0x35, 0x6f, 0xc0, 0x19, 0x0b,
                                                 0x08, 0x86, 0x65, 0xb1, 0xce, 0xf1, 0xf2, 0xeb, 0xb8, 0x89, 0xb6, 0x86, 0x34,
                                                 0x79, 0xef, 0x50, 0x8e, 0x2b, 0x19, 0x8a, 0xe4, 0x7b, 0x8a, 0xb5, 0xd6 };

    P256SerializedKeypair serialized;
    P256Keypair keypair;
    P256ECDSASignature signature;

    err = serialized.SetLength(sizeof(deviceAttestationPrivkey) + sizeof(deviceAttestationPubkey));
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);
    memcpy(serialized, deviceAttestationPubkey, sizeof(deviceAttestationPubkey));
    memcpy(serialized + sizeof(deviceAttestationPubkey), deviceAttestationPrivkey, sizeof(deviceAttestationPrivkey));
    err = keypair.Deserialize(serialized);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);

    err = keypair.ECDSA_sign_attestation_data(ByteSpan(attestationElementsTestVector), ByteSpan(attestationChallengeTestVector),
                                              signature);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);

    NL_TEST_ASSERT(inSuite, sizeof(attestationSignatureTestVector) == signature.Length());

    err = keypair.Pubkey().ECDSA_validate_attestation_data(ByteSpan(attestationElementsTestVector),
                                                           ByteSpan(attestationChallengeTestVector), signature);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);
}
...
#endif // CHIP_CRYPTO_OPENSSL
    NL_TEST_DEF("Test AttestationElements Construction", TestAttestationElements_Construction),
    NL_TEST_DEF("Test AttestationElements signing and validation message using SHA256", TestAttestationElements_SigVer),
    NL_TEST_SENTINEL()
};
