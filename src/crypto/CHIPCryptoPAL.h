...
inline constexpr size_t kVendorIdVerificationStatementV1MaxSize = sizeof(uint8_t) + kSubjectKeyIdentifierLength + kP256_ECDSA_Signature_Length_Raw;
static_assert(kVendorIdVerificationStatementV1MaxSize == 85, "Expected size of VendorIdVerificationStatement version 1 was computed incorrectly due to changes of fundamental constants");

// vendor_fabric_binding_message := fabric_binding_version (1 byte) || root_public_key || fabric_id || vendor_id
inline constexpr size_t kVendorFabricBindingMessageV1MaxSize = sizeof(uint8_t) + CHIP_CRYPTO_PUBLIC_KEY_SIZE_BYTES + sizeof(uint64_t) + sizeof(uint16_t);
static_assert(kVendorFabricBindingMessageV1MaxSize == 76, "Expected size of VendorFabricBindingMessage version 1 was computed incorrectly due to changes of fundamental constants");
...
inline constexpr size_t kVendorIdVerificationTbsV1MaxSize = sizeof(uint8_t) + kVendorIdVerificationClientChallengeSize + CHIP_CRYPTO_SYMMETRIC_KEY_LENGTH_BYTES + sizeof(uint8_t) + kVendorFabricBindingMessageV1MaxSize + kVendorIdVerificationStatementV1MaxSize;
