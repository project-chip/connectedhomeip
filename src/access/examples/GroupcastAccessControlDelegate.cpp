...
public:
    ...
    CHIP_ERROR Init()
    {
        ...
        // sList.Init(1, mStorage);
        // sList.Clear();
        // sList.Save();
        // FabricIndex fabric = 0;
        // size_t index = 0;
        // Entry e;
        // PrepareEntry(e);
        // e.SetFabricIndex(1);
        // e.SetPrivilege(Privilege::kOperate);
        // e.SetAuthMode(AuthMode::kGroup);
        // e.AddSubject(&index, 257);
        // e.AddSubject(&index, 33025);

        // // Create
        // CHIP_ERROR err = CreateEntry(&index, e, &fabric);
        // volatile size_t size1 = (unsigned)sizeof(EntryStorage);
        // volatile size_t size2 = (unsigned)sizeof(EntryList);
        
        // ChipLogProgress(DeviceLayer, "~~~ GROUPCAST ENTRY: #%04x, SS:%u, LS:%u, TS:%u\n", (unsigned)err.AsInteger(), (unsigned)size1, (unsigned)size2, (unsigned)sizeof(Target));
        // // DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG DEBUG
        return CHIP_NO_ERROR;
    }
    virtual void Finish() {}
    ...
    {
        FabricIndex fabric_index = fabricIndex ? *fabricIndex : 1; // FIXME FIXME FIXME FIXME FIXME FIXME FIXME FIXME FIXME FIXME FIXME
