...
class DeviceMgrCmd(Cmd):
    ...
    def do_setuppayload(self, line):
        ...
        try:
            ...
            if args[0] == "parse-manual":
                setup_payload = self.devCtrl.ParseManualSetupCode(args[1])

            if args[0] == "parse-qr":
                setup_payload = self.devCtrl.ParseQRCode(args[1])

            if setup_payload:
                self.devCtrl.PrintSetupPayload(setup_payload)
    ...
    def ConnectFromSetupPayload(self, setupPayload, nodeid):
        ...
        if setupPayload.rendezvousInformation.Raw() & onnetwork:
            print("Attempting to find device on Network")
            longDiscriminator = setupPayload.discriminator
    ...
    def do_connect(self, line):
        ...
        try:
            ...
            if args[0] == "-ip" and len(args) >= 3:
                ...
                setupPayload = self.devCtrl.ParseQrCode(args[1])
