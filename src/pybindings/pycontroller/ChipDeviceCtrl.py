...
class ChipDeviceController(object):
    ...
    def ResolveNode(self, fabricid, nodeid):
        return Mdns.Resolver.Instance().ResolveNodeId(PeerId().SetNodeId(nodeid).SetFabricId(fabricid), Inet.IPAddressType.kIPAddressType_Any)

    def GetAddressAndPort(self, nodeid):
        address = Inet.IPAddress()
        device = Controller.Device()
        self.dev_com.GetDevice(nodeid, device)
        err, port = device.GetAddress(address, 0)
        ChipExceptions.CHIPErrorToException(err)
        err_str, result_str = address.ToString("")

        return port, result_str
