...
public:
    ...
    /**
     * @brief
     *    Set the delegate for handling incoming messages. There can be only one message delegate (probably the
     *    ExchangeManager)
     */
    void SetMessageDelegate(SessionMessageDelegate * cb) { mCB = cb; }

    /// @brief Set the delegate for handling incoming messages.
...
private:
    ...
    BitMapObjectPool<std::reference_wrapper<SessionCreationDelegate>, 4> mSessionCreationDelegates;
