...
void TestBasicFunctionality(nlTestSuite * inSuite, void * inContext)
{
    ...
    err = connections.CreateNewPeerConnectionState(peers.FindOrAllocateEntry(kPeer1, kUndefinedAdminId), 1, 2, statePtr);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);
    NL_TEST_ASSERT(inSuite, statePtr != nullptr);
    NL_TEST_ASSERT(inSuite, statePtr->GetPeerCache()->GetPeer() == kPeer1);
    ...
    err = connections.CreateNewPeerConnectionState(peers.FindOrAllocateEntry(kPeer2, kUndefinedAdminId), 3, 4, statePtr);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);
    NL_TEST_ASSERT(inSuite, statePtr != nullptr);
    NL_TEST_ASSERT(inSuite, statePtr->GetPeerCache()->GetPeer() == kPeer2);
    ...
    err = connections.CreateNewPeerConnectionState(peers.FindOrAllocateEntry(kPeer3, kUndefinedAdminId), 5, 6, statePtr);
    NL_TEST_ASSERT(inSuite, err != CHIP_NO_ERROR);
}
...
void TestFindByPeer(nlTestSuite * inSuite, void * inContext)
{
    ...
    err = connections.CreateNewPeerConnectionState(peers.FindOrAllocateEntry(kPeer1, kUndefinedAdminId), 5, 6, statePtr);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);

    NL_TEST_ASSERT(inSuite, statePtr = connections.FindPeerConnectionState(kPeer1, kUndefinedAdminId, 1));
    NL_TEST_ASSERT(inSuite, statePtr->GetLocalKeyID() == 2);

    NL_TEST_ASSERT(inSuite, statePtr = connections.FindPeerConnectionState(kPeer1, kUndefinedAdminId, 5));
    NL_TEST_ASSERT(inSuite, statePtr->GetLocalKeyID() == 6);

    NL_TEST_ASSERT(inSuite, statePtr = connections.FindPeerConnectionState(kPeer2, kUndefinedAdminId, 3));
    NL_TEST_ASSERT(inSuite, statePtr->GetLocalKeyID() == 4);
}
...
void TestExpireConnections(nlTestSuite * inSuite, void * inContext)
{
    ...
    connections.ExpireInactiveConnections(150, [&callInfo](const PeerConnectionState & state) {
        callInfo.callCount++;
        callInfo.lastCallPeer = state.GetPeerCache()->GetPeer();
    });
    NL_TEST_ASSERT(inSuite, callInfo.callCount == 1);
    NL_TEST_ASSERT(inSuite, callInfo.lastCallPeer == kPeer1);
    NL_TEST_ASSERT(inSuite, !connections.FindPeerConnectionState(kPeer1, kUndefinedAdminId, 1));

    // now that the connections were expired, we can add peer3
    connections.GetTimeSource().SetCurrentMonotonicTimeMs(300);
    err = connections.CreateNewPeerConnectionState(peers.FindOrAllocateEntry(kPeer3, kUndefinedAdminId), 7, 8, statePtr);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);
    ...
    NL_TEST_ASSERT(inSuite, !connections.FindPeerConnectionState(kPeer1, kUndefinedAdminId, 1));
    NL_TEST_ASSERT(inSuite, connections.FindPeerConnectionState(kPeer2, kUndefinedAdminId, 3));
    NL_TEST_ASSERT(inSuite, !connections.FindPeerConnectionState(kPeer3, kUndefinedAdminId, 5));
    NL_TEST_ASSERT(inSuite, !connections.FindPeerConnectionState(kPeer3, kUndefinedAdminId, 7));

    err = connections.CreateNewPeerConnectionState(peers.FindOrAllocateEntry(kPeer1, kUndefinedAdminId), 9, 10, statePtr);
    NL_TEST_ASSERT(inSuite, err == CHIP_NO_ERROR);

    NL_TEST_ASSERT(inSuite, !connections.FindPeerConnectionState(kPeer1, kUndefinedAdminId, 1));
    NL_TEST_ASSERT(inSuite, connections.FindPeerConnectionState(kPeer1, kUndefinedAdminId, 9));
    NL_TEST_ASSERT(inSuite, connections.FindPeerConnectionState(kPeer2, kUndefinedAdminId, 3));
    NL_TEST_ASSERT(inSuite, !connections.FindPeerConnectionState(kPeer3, kUndefinedAdminId, 5));
    NL_TEST_ASSERT(inSuite, !connections.FindPeerConnectionState(kPeer3, kUndefinedAdminId, 7));
