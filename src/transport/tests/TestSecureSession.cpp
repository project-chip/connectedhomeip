...
static uint8_t secure_channel_test_public_key1[] = { 0x04, 0xe2, 0x07, 0x64, 0xff, 0x6f, 0x6a, 0x91, 0xd9, 0xc2, 0xc3,
...
static uint8_t secure_channel_test_public_key2[] = { 0x04, 0x30, 0x77, 0x2c, 0xe7, 0xd4, 0x0a, 0xf2, 0xf3, 0x19, 0xbd,
...
void SecureChannelInitTest(nlTestSuite * inSuite, void * inContext)
{
    ...
    TestECPKey pubk(secure_channel_test_public_key1, sizeof(secure_channel_test_public_key1), Crypto::SupportedECPKeyTypes::ECP256R1);
    TestECPKey privk(secure_channel_test_private_key2, sizeof(secure_channel_test_private_key2), Crypto::SupportedECPKeyTypes::ECP256R1);

    // Test all combinations of invalid parameters
    NL_TEST_ASSERT(inSuite,
                   channel.Init(pubk, privk, NULL, 0, NULL,
                                0) == CHIP_ERROR_INVALID_ARGUMENT);
    NL_TEST_ASSERT(inSuite,
                   channel.Init(pubk, privk, NULL, 10, NULL,
                                0) == CHIP_ERROR_INVALID_ARGUMENT);

    // Test the channel is successfully created with valid parameters
    const char * info = "Test Info";
    NL_TEST_ASSERT(inSuite,
                   channel.Init(pubk, privk,  NULL, 0,
    ...
    NL_TEST_ASSERT(inSuite,
                   channel2.Init(pubk, privk,  (const uint8_t *) salt,
...
void SecureChannelEncryptTest(nlTestSuite * inSuite, void * inContext)
{
    ...
    const char * salt = "Test Salt";
    NL_TEST_ASSERT(inSuite,
                   channel.Init(pubk, privk, (const uint8_t *) salt,
                                sizeof(salt), (const uint8_t *) info, sizeof(info)) == CHIP_NO_ERROR);
...
void SecureChannelDecryptTest(nlTestSuite * inSuite, void * inContext)
{
    ...
    TestECPKey pubk2(secure_channel_test_public_key2, sizeof(secure_channel_test_public_key2), Crypto::SupportedECPKeyTypes::ECP256R1);
    TestECPKey privk1(secure_channel_test_private_key1, sizeof(secure_channel_test_private_key1), Crypto::SupportedECPKeyTypes::ECP256R1);
    NL_TEST_ASSERT(inSuite,
                   channel2.Decrypt(encrypted, sizeof(plain_text), output, header) == CHIP_ERROR_INVALID_USE_OF_SESSION_KEY);
    NL_TEST_ASSERT(inSuite,
                   channel2.Init(pubk2, privk1, (const uint8_t *) salt,
                                 sizeof(salt), (const uint8_t *) info, sizeof(info)) == CHIP_NO_ERROR);
