...
CHIP_ERROR TCPBase::SendMessage(const Transport::PeerAddress & address, System::PacketBufferHandle && msgBuf)
{
    ...
    if (connection != nullptr && connection->mConnectionState == TCPState::kConnected)
    {
                                 ...
                                 Transport::ActiveTCPConnectionState ** peerConnState)
{
    ...
    *peerConnState                              = nullptr;
    ...
    // connection.
    *peerConnState                     = activeConnection;
...
void TCPBase::HandleTCPEndPointConnectComplete(Inet::TCPEndPoint * endPoint, CHIP_ERROR conErr)
{
    ...
    if (conErr == CHIP_NO_ERROR)
    {
        ...
        ChipLogProgress(Inet, "Connection established successfully with [%s:%d]", ipAddrStr, port);
    ...
    {
        ChipLogError(Inet, "Connection establishment with [%s:%d] encountered an error: %" CHIP_ERROR_FORMAT, ipAddrStr, port,
                     err.Format());
        endPoint->Free();
...
{
    TCPBase * tcp                               = reinterpret_cast<TCPBase *>(endPoint->mAppState);
    ActiveTCPConnectionState * activeConnection = tcp->FindActiveConnection(endPoint);
    Inet::IPAddress ipAddress;
    uint16_t port;
    ...
    if (err == CHIP_NO_ERROR && activeConnection->mConnectionState == TCPState::kConnected)
    {
    ...
    char ipAddrStr[Inet::IPAddress::kMaxStringLength];
    ipAddress.ToString(ipAddrStr);
    ChipLogProgress(Inet, "Connection closed with [%s:%d]", ipAddrStr, port);
}
                                       ...
                                       const Inet::IPAddress & peerAddress, uint16_t peerPort)
{
    ...
    if (tcp->mUsedEndPointCount < tcp->mActiveConnectionsSize)
    {
        ...
        char ipAddrStr[Inet::IPAddress::kMaxStringLength];
        peerAddress.ToString(ipAddrStr);
        ChipLogProgress(Inet, "Incoming connection established with peer at [%s:%d]", ipAddrStr, port);
                               ...
                               Transport::ActiveTCPConnectionState ** peerConnState)
{
    ...
    ReturnErrorOnFailure(StartConnect(address, appState, peerConnState));
