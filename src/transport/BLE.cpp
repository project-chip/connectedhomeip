...
CHIP_ERROR BLE::Init(BleConnectionParameters & params)
{
    ...
    VerifyOrExit(params.HasConnectionObject() || params.HasDiscriminator() || params.HasMacAddr(),
    ...
    else if (params.HasMacAddr())
    {
        err = DelegateConnection(params.GetBleLayer(), params.GetMacAddr());
    }
    else
    {
        err = CHIP_ERROR_INVALID_ARGUMENT;
...
CHIP_ERROR BLE::DelegateConnection(Ble::BleLayer * bleLayer, const Ble::MacAddr & macAddr)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    err = bleLayer->NewBleConnection(reinterpret_cast<void *>(this), macAddr, OnBleConnectionComplete, OnBleConnectionError);
    SuccessOrExit(err);
