...
{
public:
    PeerCacheEntry(const PeerId & peer, Transport::AdminId admin) : mPeer(peer), mAdmin(admin) {}
    ...
    /// @brief Hold a cache entry to prevent it from being released.
    PeerHandle Hold() { return PeerHandle(this); }

private:
    uint64_t mLastActivityTimeMs = 0;

    const PeerId mPeer;
    const Transport::AdminId mAdmin;
...
public:
     ...
     */
    CHECK_RETURN_VALUE
    CHIP_ERROR AllocEntry(const PeerId & peer, Transport::AdminId admin, PeerCacheEntry *& entry)
    {
        entry = mEntries.CreateObject(peer, admin);
        if (entry != nullptr)
        ...
        mEntries.ResetObject(entry, peer, admin);
        return CHIP_NO_ERROR;
    }

    /**
     * Get a peer given the node id and admin id.
     *
     * @return the peer found, nullptr if not found
     */
    CHECK_RETURN_VALUE
    PeerCacheEntry * FindEntry(const PeerId & peer, Transport::AdminId admin)
    {
    ...
    /**
     * Get a peer given the node id and admin id. If the peer doesn't exist in the cache, allocate a new entry for it.
     *
     * @return the peer found or allocated, nullptr if not found and allocate failed.
     */
    CHECK_RETURN_VALUE
    PeerCacheEntry * FindOrAllocateEntry(const PeerId & peer, Transport::AdminId admin)
    {
        PeerCacheEntry * result = FindEntry(peer, admin);
        if (result != nullptr)
            return result;

        CHIP_ERROR err = AllocEntry(peer, admin, result);
    ...
    void MarkPeerActive(PeerCacheEntry * state) { state->SetLastActivityTimeMs(mTimeSource.GetCurrentMonotonicTimeMs()); }
