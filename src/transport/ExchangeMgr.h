...
public:
    ...
    CHIP_ERROR SendMessage(uint32_t profileId, uint8_t msgType, System::PacketBuffer * msgBuf, uint16_t sendFlags,
                           PacketHeader & packetHeader, void * msgCtxt = nullptr);
     ...
     */
    typedef void (*MessageReceiveFunct)(ExchangeContext * ec, const Inet::IPPacketInfo * pktInfo, const PacketHeader & packetHeader,
     ...
     *  @param[in]    ec            A pointer to the ExchangeContext object.
     *
     *  @param[in]    con           A pointer to the PeerConnectionState object.
     *
     *  @param[in]    conErr        The CHIP_ERROR type that was reported when the connection
     *                              was closed.
     *
     */
    typedef void (*ConnectionClosedFunct)(ExchangeContext * ec, PeerConnectionState * con, CHIP_ERROR conErr);
    ...
    enum
    {
        kGetPeerDescription_MaxLength = 100,
...
private:
    ...
    bool MatchExchange(PeerConnectionState * msgCon, const PacketHeader & packetHeader, const ChipExchangeHeader * exchangeHeader);
 ...
 *
 */
class DLL_EXPORT ExchangeManager
{
...
public:
    ...
    ExchangeContext * NewContext(PeerConnectionState * con, void * appState = nullptr);

    ExchangeContext * FindContext(uint64_t peerNodeId, PeerConnectionState * con, void * appState, bool isInitiator);
    ...
    CHIP_ERROR RegisterUnsolicitedMessageHandler(uint32_t profileId, uint8_t msgType, PeerConnectionState * con,
    ...
    CHIP_ERROR UnregisterUnsolicitedMessageHandler(uint32_t profileId, uint8_t msgType, PeerConnectionState * con);

    void AllowUnsolicitedMessages(PeerConnectionState * con);
...
private:
    ...
    void HandleConnectionReceived(PeerConnectionState * con);
    void HandleConnectionClosed(PeerConnectionState * con, CHIP_ERROR conErr);
    void DispatchMessage(const PacketHeader & packetHeader, System::PacketBuffer * msgBuf);
    CHIP_ERROR RegisterUMH(uint32_t profileId, int16_t msgType, PeerConnectionState * con, bool allowDups,
                           ExchangeContext::MessageReceiveFunct handler, void * appState);
