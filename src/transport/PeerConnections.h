...
public:
     ...
     */
    CHECK_RETURN_VALUE
    CHIP_ERROR CreateNewPeerConnectionState(PeerCacheEntry * peer, uint16_t peerKeyId, uint16_t localKeyId,
                                            PeerConnectionState *& state)
    {
        state = mStates.CreateObject(peer, peerKeyId, localKeyId);
        if (state == nullptr)
     ...
     */
    CHECK_RETURN_VALUE
    PeerConnectionState * FindPeerConnectionState(const PeerId & peer, Transport::AdminId admin, uint16_t peerKeyId)
    {
        PeerConnectionState * result = nullptr;
        mStates.ForEachActiveObject([&](PeerConnectionState * state) {
            auto entry = state->GetPeerCache();
            if (entry->GetPeer() == peer && entry->GetAdminId() == admin && state->GetPeerKeyID() == peerKeyId)
            {
                result = state;
                return false;
            }
    ...
    {
        mStates.ForEachActiveObject(function);
    }
