...
#define CHIPTAssertNil(__OBJECT__) \
        ({ \
            ...
            if((__in_object) != nil) { \
                CHIPPrintf(@"Assertion failed: '%s' is unexpectedly NOT nil (%s, %d) -> %@", #__OBJECT__, __FILE__, __LINE__, [__in_object description]); \
                exit(1); \
            } \
        })
