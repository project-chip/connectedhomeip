...
{
    chip::GroupId group_id = packetHeader.GetDestinationGroupId().Value();

    FabricList fabric_list;
    ReturnErrorOnFailure(fabric_list.Load(mStorage));

    // Search the matching index in the fabric list
    FabricData fabric;
    fabric.fabric_index = fabric_list.first_fabric;
    for (size_t i = 0; i < fabric_list.fabric_count; i++, fabric.fabric_index = fabric.next)
    {
        ReturnErrorOnFailure(fabric.Load(mStorage));
        ...
        {
            ReturnErrorOnFailure(mapping.Load(mStorage));
            if (mapping.group_id == group_id)
            {
                // Group found, get the keyset
                KeySet keyset;
                ReturnErrorOnFailure(GetKeySet(fabric.fabric_index, mapping.keyset_id, keyset));
                ...
                ReturnErrorOnFailure(chip::MessageSecurity::Groups::DeriveOperationalKey(epoch_key, operational_key));
                VerifyOrReturnError(CHIP_NO_ERROR !=
                                        chip::MessageSecurity::Decrypt(ByteSpan(operational_key, sizeof(operational_key)),
                                                                       payloadHeader, packetHeader, msg),
                                    CHIP_NO_ERROR);
            }
        }
    }
