...
struct PersistentData
{
    ...
    virtual CHIP_ERROR Save(PersistentStorageDelegate * storage)
    {
        ...
        return storage->SyncSetKeyValue(key.Value(), buffer, static_cast<uint16_t>(writer.GetLengthWritten()));
    }
    ...
    CHIP_ERROR Load(PersistentStorageDelegate * storage)
    {
        ...
        CHIP_ERROR err = storage->SyncGetKeyValue(key.Value(), buffer, size);
