...
enum
{
     ...
     *
     * TODO: Note that spec limits CHIP certificate size in DER form to 600 bytes.
     *       Although some of the test certificates exceed 600 byte limit in DER form
     *       their TLV encoded representation is still below the 400 byte limit.
     *       For example, the largest Node02_05 certificate is 633 byte in DER form
     *       and 355 bytes when encoded in TLV form.
     *       Is the DER cert size limit important???
     */
    kTestDERCertBufSize = 650,
};
