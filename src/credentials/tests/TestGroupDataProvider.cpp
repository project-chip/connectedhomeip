...
void SetupGroup(Groupcast::Data & g, size_t n, uint16_t endpoints)
{
    g.groupID       = GetGroupId(n);
    g.keySetID      = static_cast<KeysetId>(g.groupID * 10);
...
TEST_F(TestGroupDataProvider, TestGroupcastAdd)
{
...
TEST_F(TestGroupDataProvider, TestGroupcastIterator)
{
    FabricIndex fabric = 1;
    Groupcast::Data g, expected;
    size_t count = 0;
    ...
    auto result = sProvider.IterateGroups(fabric, [&](auto & iter) -> CHIP_ERROR {
        ...
        while (iter.Next(g))
        {
            ...
            EXPECT_EQ(g.keySetID, expected.keySetID);
...
TEST_F(TestGroupDataProvider, TestGroupcastRemove)
{
    // Remove groups
    FabricIndex fabric   = 1;
    size_t removed_count = 0;
    ...
    auto result           = sProvider.IterateGroups(fabric, [&](auto & iter) -> CHIP_ERROR {
        ...
        while (iter.Next(g))
        {
            ...
            EXPECT_EQ(g.groupID * 10, g.keySetID);
...
TEST_F(TestGroupDataProvider, TestGroupcastEditEndpoints)
{
    // Modify endpoints
    FabricIndex fabric = 1;
    GroupId target_id  = GetGroupId(3);
    Groupcast::Data g(target_id);
