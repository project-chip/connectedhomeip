...
struct TargetList : public PersistentArray<CHIP_CONFIG_MAX_FABRICS, kPersistentBufferMax, GroupEntry>
{
...
bool DataProvider::GroupIterator::Next(Group & out)
{
    ...
    GroupEntry entry;
    chip::Groupcast::TargetList list(mFabric, mProvider.mStorage);
    list.Load();
                                   ...
                                   Group & target, const ByteSpan & key, uint32_t period)
{
    ...
    // Insert entry
    {
        chip::Groupcast::TargetList list(fabric_idx, mStorage);
...
chip::Crypto::SymmetricKeyContext * DataProvider::CreateKeyContext(FabricIndex fabric, GroupId group_id)
{
    ...
    size_t index = 0;
    chip::Groupcast::TargetList list(fabric, mStorage);
