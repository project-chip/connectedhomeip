...
constexpr size_t kPersistentBufferMax = 4 + kEndpointsMax * kEndpointEntrySize; // GroupId(2) + KeyId(2) + kEndpointsMax(8) * kEndpointEntrySize(4);
...
DataProvider::KeyIterator::KeyIterator(DataProvider & group_data, FabricTable *fabrics, GroupId group_id, uint16_t session_id) :
    mProvider(group_data), mFabrics(fabrics), mGroupId(group_id), mSessionId(session_id)
{
}
