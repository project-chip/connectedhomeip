...
constexpr uint8_t sTestCert_CD_Cert_Array[] = {
...
static const ByteSpan kTestCDCerts[] = { ByteSpan(sTestCert_CD_Cert_Array) };

const ArrayAttestationTrustStore kTestAttestationTrustStore{ &kTestPaaRoots[0], ArraySize(kTestPaaRoots), &kTestCDCerts[0],
                                                             ArraySize(kTestCDCerts) };
                                                                                            ...
                                                                                            ByteSpan & certDeclBuffer)
{
    ...
    if (err == CHIP_ERROR_NOT_IMPLEMENTED)
    {
        VerifyOrReturnError(kTestAttestationTrustStore.GetCertificationDeclarationSigningKey(skid, pubKey) == CHIP_NO_ERROR,
                            AttestationVerificationResult::kCertificationDeclarationNoCertificateFound);
    }
