...
public:
    ...
    public:
         ...
         * no list.
         */
        Iterator(const TLV::TLVReader & reader, FabricIndex fabricIndex) : mFabricIndex(fabricIndex)
        {
        ...
        bool Next()
        {
            ...
            {
                mStatus = DataModel::Decode(mReader, mValue,
                                            mFabricIndex == kUndefinedFabricIndex ? Optional<FabricIndex>::Missing()
    ...
    private:
        ...
        const FabricIndex mFabricIndex = kUndefinedFabricIndex;
    };
    ...
    template <typename T0 = T, std::enable_if_t<DataModel::IsFabricScoped<T0>::value, bool> = true>
    void SetOverwriteFabricIndex(FabricIndex fabricIndex_)
    {
        mFabricIndex = fabricIndex_;
    }
    ...
    CHIP_ERROR Decode(TLV::TLVReader & reader, const Optional<FabricIndex> & overwriteFabricIndex)
    {
        ...
        ReturnErrorOnFailure(reader.EnterContainer(type));
        SetReader(reader);
        SetOverwriteFabricIndex(overwriteFabricIndex.ValueOr(kUndefinedFabricIndex));
        ReturnErrorOnFailure(reader.ExitContainer(type));
        return CHIP_NO_ERROR;
    }
