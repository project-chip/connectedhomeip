...
const char * ReadClient::GetStateStr() const
{
    ...
    case ClientState::Initialized:
        return "INIT";
    case ClientState::WaitingResponse:
...
CHIP_ERROR ReadClient::SendReadRequest(NodeId aNodeId, Transport::AdminId aAdminId)
{
    ...
    SuccessOrExit(err);
    MoveToState(ClientState::WaitingResponse);
...
CHIP_ERROR ReadClient::ProcessReportData(System::PacketBufferHandle aPayload)
{
...

    {
        ...
        {
            System::PacketBufferTLVReader eventListReader;
            eventList.GetReader(&eventListReader);
            err = mpDelegate->HandleEventStreamReceivedCallBack(mpExchangeCtx, &eventListReader);
            SuccessOrExit(err);
        }
    }
