...
Delegate * GetDelegate(EndpointId endpoint)
{
    ...
    if (app != nullptr)
    {
        ChipLogError(Zcl, "ApplicationBasic returning ContentApp delegate for endpoint:%u", endpoint);
        return app->GetApplicationBasicDelegate();
    }
#endif // CHIP_DEVICE_CONFIG_APP_PLATFORM_ENABLED
    ChipLogError(Zcl, "ApplicationBasic NOT returning ContentApp delegate for endpoint:%u", endpoint);
...
bool isDelegateNull(Delegate * delegate, EndpointId endpoint)
{
    if (delegate == nullptr)
    {
        ChipLogError(Zcl, "Application Basic has no delegate set for endpoint:%u", endpoint);
        return true;
    }
    return false;
}
