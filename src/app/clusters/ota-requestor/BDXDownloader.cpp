...
constexpr System::Clock::Timeout kBdxIdleTimeout = System::Clock::Seconds16(5 * 60);
	
// TODO: add the bdx total timeout of 5 mins here and call on timeout from the transfer session
...
void BDXDownloader::OnMessageReceived(const chip::PayloadHeader & payloadHeader, chip::System::PacketBufferHandle msg)
{
    ...
    CHIP_ERROR err =
        mBdxTransfer.HandleMessageReceived(payloadHeader, std::move(msg));
...
{
	ChipLogProgress(BDX, "OnBDXEventReceived %hu", event.EventType);
                                       ...
                                       System::Clock::Timeout timeout)
{
    ...
    ChipLogProgress(BDX, "Register for BDX events from the transfer session");
