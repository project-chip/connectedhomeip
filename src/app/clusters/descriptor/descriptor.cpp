...
EmberAfStatus writeClientServerAttribute(uint8_t endpoint, bool server)
{
    ...
    AttributeId attributeId = server ? ZCL_SERVER_ATTRIBUTE_ID : ZCL_CLIENT_ATTRIBUTE_ID;
...
{
    EmberAfStatus status    = EMBER_ZCL_STATUS_SUCCESS;
    AttributeId attributeId = ZCL_DEVICE_ATTRIBUTE_ID;

    uint16_t deviceTypeCount = 0;
    uint8_t startIndex       = endpoint;
    uint8_t stopIndex        = (endpoint == 0x00 ? emberAfEndpointCount() : endpoint + 1);

    EmberAfDeviceType deviceType;
    for (uint8_t endpointIndex = startIndex; endpointIndex < stopIndex; endpointIndex++)
    {
        DeviceId deviceId = emberAfDeviceIdFromIndex(endpointIndex);
        ...
        for (uint8_t entryIndex = 0; entryIndex < (endpointIndex - endpoint); entryIndex++)
        {
            ...
            if (deviceType.deviceTypeId == deviceId)
            {
                duplicate = true;
                break;
            }
        ...
        if (duplicate == false)
        {
            deviceType.deviceTypeId = deviceId;
...
{
    EmberAfStatus status    = EMBER_ZCL_STATUS_SUCCESS;
    AttributeId attributeId = ZCL_PARTS_ATTRIBUTE_ID;
...
void emberAfPluginDescriptorServerInitCallback(void)
{
    ...
    for (uint8_t endpoint = 0; endpoint < emberAfEndpointCount(); endpoint++)
    {
