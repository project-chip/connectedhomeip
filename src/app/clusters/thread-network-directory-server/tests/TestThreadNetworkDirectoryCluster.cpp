...
TEST_F(TestThreadNetworkDirectoryCluster, TestApplicationMutators)
{
    ...
    // An update replaces the stored dataset and notifies again. The
    // increasing Active Timestamp rule deliberately does not apply here: the
    // application is the authority on its own network, and a re-formed
    // network legitimately starts over, so the second update goes backwards
    // from timestamp 2 to 1 and is still accepted.
    for (const ByteSpan replacement : { ByteSpan(kDataset1Updated), ByteSpan(kDataset1) })
    {
        EXPECT_EQ(cluster.AddOrUpdateNetwork(MakeExPanId1(), replacement), CHIP_NO_ERROR);
        ...
        EXPECT_TRUE(readBack.data_equal(replacement));
