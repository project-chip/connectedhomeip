...
static EmberAfStatus GroupAdd(FabricIndex fabricIndex, EndpointId endpointId, GroupId groupId, const CharSpan & groupName)
{
    ...
    {
        emberAfGroupsClusterPrint("ERR: Failed to add mapping (end:0x%x, group:0x%x), err:0x%08" PRIX32, endpointId, groupId,
                                  err.AsInteger());
...
static EmberAfStatus GroupRemove(FabricIndex fabricIndex, EndpointId endpointId, GroupId groupId)
{
    ...
    if (GroupExists(fabricIndex, endpointId, groupId))
    {
        ...
        {
            emberAfGroupsClusterPrint("ERR: Failed to remove mapping (end:0x%x, group:0x%x), err:0x%08" PRIX32, endpointId, groupId,
                                      err.AsInteger());
...
static void GroupPrint(FabricIndex fabricIndex, EndpointId endpointId)
{
    GroupDataProvider * groups = GetGroupDataProvider();
    if (groups)
    {
        GroupDataProvider::GroupMappingIterator * groupIt = groups->IterateGroupMappings(fabricIndex, endpointId);
        size_t count                                      = groupIt ? groupIt->Count() : 0;
        size_t i                                          = 0;
        GroupDataProvider::GroupMapping mapping;
...
exit:
    GroupPrint(fabricIndex, endpointId);
    if (err != CHIP_NO_ERROR)
    {
        ChipLogError(Zcl, "Failed to encode response command.");
    }
