...
void emAfMessagingServerCliMsg(void)
{
    uint8_t length     = emberCopyStringArgument(0, message.message + 1, EMBER_AF_PLUGIN_MESSAGING_SERVER_MESSAGE_SIZE, false);
...
void emAfMessagingServerCliAppend(void)
{
    ...
    uint8_t length     = emberCopyStringArgument(0, message.message + oldLength + 1,
                                             (EMBER_AF_PLUGIN_MESSAGING_SERVER_MESSAGE_SIZE - oldLength), false);
...
void emAfMessagingServerCliRelativeTime(void)
{
    message.startTime         = (emberAfGetCurrentTime() + (emberSignedCommandArgument(0) * 60));
    message.durationInMinutes = (uint16_t) emberUnsignedCommandArgument(1);
}
...
void emAfMessagingServerCliValid(void)
{
    ...
    emberAfPluginMessagingServerSetMessage(endpoint, emberCurrentCommand->name[0] == 'v' ? &message : NULL);
}
...
void emAfMessagingServerCliDisplay(void)
{
    ...
    emberAfPluginMessagingServerDisplayMessage(nodeId, srcEndpoint, dstEndpoint);
}
...
void emAfMessagingServerCliCancel(void)
{
    ...
    uint8_t srcEndpoint = (uint8_t) emberUnsignedCommandArgument(1);
    uint8_t dstEndpoint = (uint8_t) emberUnsignedCommandArgument(2);
    emberAfPluginMessagingServerCancelMessage(nodeId, srcEndpoint, dstEndpoint);
}
