...
CHIP_ERROR CloseValve(EndpointId ep)
{
    ...
    VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == TargetState::Set(ep, ValveConfigurationAndControl::ValveStateEnum::kClosed),
                        attribute_error);
    VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS ==
                            CurrentState::Set(ep, ValveConfigurationAndControl::ValveStateEnum::kTransitioning),
                        attribute_error);
    VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == OpenDuration::SetNull(ep), attribute_error);
    if (HasFeature(ep, ValveConfigurationAndControl::Feature::kLevel))
    {
        VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == TargetLevel::Set(ep, 0), attribute_error);
    }
    if (HasFeature(ep, ValveConfigurationAndControl::Feature::kTimeSync))
    {
        VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == AutoCloseTime::SetNull(ep), attribute_error);
    }
    SetRemainingDurationNull(ep);
    RemainingDurationTable * item = GetRemainingDurationItem(ep);
    (void) DeviceLayer::SystemLayer().CancelTimer(onValveConfigurationAndControlTick, item);
...
CHIP_ERROR SetValveLevel(EndpointId ep, DataModel::Nullable<Percent> level, DataModel::Nullable<uint32_t> openDuration)
{
    ...
    if (HasFeature(ep, ValveConfigurationAndControl::Feature::kTimeSync))
    {
                ...
                TimeSynchronization::GranularityEnum::kNoTimeGranularity)
        {
            ...
            VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == AutoCloseTime::Set(ep, autoCloseTime), attribute_error);
        }
        else
        {
            VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == AutoCloseTime::SetNull(ep), attribute_error);
    ...
    {
        VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == TargetLevel::Set(ep, level), attribute_error);
    }

    VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == OpenDuration::Set(ep, openDuration), attribute_error);
    ...
    VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == TargetState::Set(ep, ValveConfigurationAndControl::ValveStateEnum::kOpen),
                        attribute_error);
    VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS ==
                            CurrentState::Set(ep, ValveConfigurationAndControl::ValveStateEnum::kTransitioning),
                        attribute_error);
    ...
    if (!isDelegateNull(delegate))
    {
        ...
        if (HasFeature(ep, ValveConfigurationAndControl::Feature::kLevel))
        {
            VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == CurrentLevel::Set(ep, cLevel), attribute_error);
        }
    }
    // start countdown
    startRemainingDurationTick(ep);
...
CHIP_ERROR UpdateCurrentLevel(EndpointId ep, Percent currentLevel)
{
    if (HasFeature(ep, ValveConfigurationAndControl::Feature::kLevel))
    {
        VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == CurrentLevel::Set(ep, currentLevel),
...
{
    VerifyOrReturnError(MATTER_CL_STATUS_SUCCESS == CurrentState::Set(ep, currentState), CHIP_IM_GLOBAL_STATUS(ConstraintError));
...
void UpdateAutoCloseTime(uint64_t time)
{
    for (auto & t : gRemainingDuration)
    {
        ...
        if (!d.IsNull() && d.Value() != 0)
        {
            ...
            if (MATTER_CL_STATUS_SUCCESS != AutoCloseTime::Set(t.endpoint, closingTime))
            {
    ...
    const ValveConfigurationAndControl::Commands::Open::DecodableType & commandData)
{
    ...
    if (MATTER_CL_STATUS_SUCCESS == ValveFault::Get(ep, &fault) && fault.HasAny())
    {
    ...
    {
        VerifyOrExit(MATTER_CL_STATUS_SUCCESS == DefaultOpenDuration::Get(ep, duration), status.Emplace(Status::Failure));
    }
    ...
    if (HasFeature(ep, ValveConfigurationAndControl::Feature::kLevel))
    {
        ...
        else if (MATTER_CL_STATUS_SUCCESS == DefaultOpenLevel::Get(ep, &defOpenLevel))
        {
