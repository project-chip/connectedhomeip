...
public:
                          ...
                          scenes::TransitionTimeMs timeMs) override
    {
        ...
        if (!(LevelControlWithOnOffFeaturePresent(endpoint) &&
              MatterScenes::ScenesServer::Instance().IsHandlerRegistered(endpoint, LevelControlServer::GetSceneHandler())))
#endif
        {
                                       ...
                                       const Commands::OffWithEffect::DecodableType & commandData)
{
    ...
    if (SupportsLightingApplications(endpoint))
    {
        ...
        if (globalSceneControl)
        {
            ...
            MatterScenes::ScenesServer::Instance().StoreCurrentScene(fabric, endpoint, groupId, ZCL_SCENES_GLOBAL_SCENE_SCENE_ID);
#endif // EMBER_AF_PLUGIN_MATTER_SCENES
...
bool OnOffServer::OnWithRecallGlobalSceneCommand(app::CommandHandler * commandObj, const app::ConcreteCommandPath & commandPath)
{
    ...
    MatterScenes::ScenesServer::Instance().RecallScene(fabric, endpoint, groupId, ZCL_SCENES_GLOBAL_SCENE_SCENE_ID);
#endif // EMBER_AF_PLUGIN_MATTER_SCENES
