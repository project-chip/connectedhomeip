...
{
    if (CHECK_BOUNDS_INVALID(0, liftPercent100ths, WC_PERCENT100THS_MAX))
        return EMBER_ZCL_STATUS_INVALID_VALUE;

    return wcWriteAttribute(ep, ZCL_WC_TARGET_POSITION_LIFT_PERCENT100_THS_ATTRIBUTE_ID, (uint8_t *) &liftPercent100ths,
                            ZCL_INT16U_ATTRIBUTE_TYPE);
}

EmberAfStatus wcSetTargetPositionTilt(EndpointId ep, posPercent100ths_t tiltPercent100ths)
{
    if (CHECK_BOUNDS_INVALID(0, tiltPercent100ths, WC_PERCENT100THS_MAX))
        return EMBER_ZCL_STATUS_INVALID_VALUE;

    return wcWriteAttribute(ep, ZCL_WC_TARGET_POSITION_TILT_PERCENT100_THS_ATTRIBUTE_ID, (uint8_t *) &tiltPercent100ths,
                            ZCL_INT16U_ATTRIBUTE_TYPE);
}

EmberAfStatus wcGetCurrentPositionLift(EndpointId ep, posPercent100ths_t * p_liftPercent100ths)
{
    return wcReadAttribute(ep, ZCL_WC_CURRENT_POSITION_LIFT_PERCENT100_THS_ATTRIBUTE_ID, (uint8_t *) p_liftPercent100ths,
                           sizeof(posPercent100ths_t));
}

EmberAfStatus wcGetCurrentPositionTilt(EndpointId ep, posPercent100ths_t * p_tiltPercent100ths)
{
    return wcReadAttribute(ep, ZCL_WC_CURRENT_POSITION_TILT_PERCENT100_THS_ATTRIBUTE_ID, (uint8_t *) p_tiltPercent100ths,
                           sizeof(posPercent100ths_t));
}
...
EmberAfStatus wcSetCurrentPositionLift(EndpointId ep, posPercent100ths_t liftPercent100ths, uint16_t liftValue)
{
    ...
    if (m_wcFeature.PA && m_wcFeature.LF)
    {
        if (EMBER_ZCL_STATUS_SUCCESS !=
            (status = wcWriteAttribute(ep, ZCL_WC_CURRENT_POSITION_LIFT_PERCENT100_THS_ATTRIBUTE_ID, (uint8_t *) &liftPercent100ths,
                                       ZCL_INT16U_ATTRIBUTE_TYPE)))
            return status;
        if (EMBER_ZCL_STATUS_SUCCESS !=
            (status = wcWriteAttribute(ep, ZCL_WC_CURRENT_POSITION_LIFT_PERCENTAGE_ATTRIBUTE_ID, (uint8_t *) &liftPercentage,
                                       ZCL_INT8U_ATTRIBUTE_TYPE)))
            return status;
        if (EMBER_ZCL_STATUS_SUCCESS !=
            (status = wcWriteAttribute(ep, ZCL_WC_CURRENT_POSITION_LIFT_ATTRIBUTE_ID, (uint8_t *) &liftValue,
                                       ZCL_INT16U_ATTRIBUTE_TYPE)))
            return status;
    }
    else
...
EmberAfStatus wcSetCurrentPositionTilt(EndpointId ep, posPercent100ths_t tiltPercent100ths, uint16_t tiltValue)
{
    ...
    {
        if (EMBER_ZCL_STATUS_SUCCESS !=
            (status = wcWriteAttribute(ep, ZCL_WC_CURRENT_POSITION_TILT_PERCENT100_THS_ATTRIBUTE_ID, (uint8_t *) &tiltPercent100ths,
                                       ZCL_INT16U_ATTRIBUTE_TYPE)))
            return status;
        if (EMBER_ZCL_STATUS_SUCCESS !=
            (status = wcWriteAttribute(ep, ZCL_WC_CURRENT_POSITION_TILT_PERCENTAGE_ATTRIBUTE_ID, (uint8_t *) &tiltPercentage,
                                       ZCL_INT8U_ATTRIBUTE_TYPE)))
            return status;
        if (EMBER_ZCL_STATUS_SUCCESS !=
            (status = wcWriteAttribute(ep, ZCL_WC_CURRENT_POSITION_TILT_ATTRIBUTE_ID, (uint8_t *) &tiltValue,
                                       ZCL_INT16U_ATTRIBUTE_TYPE)))
            return status;
    }
    else
