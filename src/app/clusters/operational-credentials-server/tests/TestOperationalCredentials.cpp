...
=======
    ...
    TEST_F(TestOperationalCredentials, TestCertificateChainRequestPQCModeRejectsUnsupportedRequests)
    {
        ...
        EXPECT_EQ(unsupportedProfileResult.status->GetStatusCode().GetStatus(),
        ...
        EXPECT_EQ(unsupportedPqcProfileResult.status->GetStatusCode().GetStatus(),
                  Protocols::InteractionModel::Status::InvalidCommand);
        ...
        EXPECT_EQ(undersizedSegmentSizeResult.status->GetStatusCode().GetStatus(),
                  Protocols::InteractionModel::Status::InvalidCommand);
        ...
        for (uint16_t maxSegmentSize :
             { static_cast<uint16_t>(kMaxCertificateSegmentSize + 1), std::numeric_limits<uint16_t>::max() })
        {
