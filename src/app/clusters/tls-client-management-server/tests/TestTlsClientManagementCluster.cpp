...
public:
    ...
    CHIP_ERROR ForEachEndpoint(EndpointId matterEndpoint, FabricIndex fabric, LoadedEndpointCallback callback) override
    {
        for (auto & endpoint : endpoints)
        {
            if (endpoint.matterEndpoint == matterEndpoint && endpoint.fabric == fabric)
            {
                ...
                decodable.hostname =
                    ByteSpan(reinterpret_cast<const uint8_t *>(endpoint.hostname.data()), endpoint.hostname.size());
                      ...
                      const Commands::ProvisionEndpoint::DecodableType & provisionReq, uint16_t & endpointID) override
    {
        ...
        memcpy(hostnameBuf, provisionReq.hostname.data(), provisionReq.hostname.size());

        endpointID = nextEndpointID++;
        endpoints.push_back({ matterEndpoint, fabric, endpointID, CharSpan(hostnameBuf, provisionReq.hostname.size()),
                              provisionReq.caid, provisionReq.ccdid });
...
TEST_F(TestTlsClientManagementCluster, TestProvisionedEndpointsList)
{
    // Provision multiple endpoints
    for (int i = 0; i < 3; i++)
    {
        Commands::ProvisionEndpoint::Type request;
        char hostname[32];
        snprintf(hostname, sizeof(hostname), "example%d.com", i);
        request.hostname = ByteSpan(reinterpret_cast<const uint8_t *>(hostname), strlen(hostname));
        request.caid     = 1;
        request.ccdid.SetNull();
    ...
    // Read the ProvisionedEndpoints attribute
    Attributes::ProvisionedEndpoints::TypeInfo::DecodableType list;
    EXPECT_EQ(mClusterTester.ReadAttribute(Attributes::ProvisionedEndpoints::Id, list), CHIP_NO_ERROR);
    ...
    {
        count++;
        EXPECT_GE(it.GetValue().endpointID, 1);
        EXPECT_LE(it.GetValue().endpointID, 3);
    }
    EXPECT_EQ(count, 3);
}

TEST_F(TestTlsClientManagementCluster, TestMaxProvisionedValidation)
{
    // The constructor validates that maxProvisioned is between 5 and 254
    // This is tested via VerifyOrDieWithMsg, so we just verify the getter works
    EXPECT_EQ(mCluster.GetMaxProvisioned(), kMaxProvisioned);
}
