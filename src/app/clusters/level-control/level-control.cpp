                                 ...
                                 chip::Optional<BitMask<OptionsBitmap>> optionsOverride, uint16_t storedLevel)
{
    ...
    if (emberAfContainsServer(endpoint, MatterScenes::Id))
    {
...
void emberAfLevelControlClusterServerInitCallback(EndpointId endpoint)
{
    ...
    app::Clusters::MatterScenes::ScenesServer::Instance().RegisterSceneHandler(endpoint, LevelControlServer::GetSceneHandler());
#endif // defined(EMBER_AF_PLUGIN_MATTER_SCENES) && CHIP_CONFIG_SCENES_USE_DEFAULT_HANDLERS
