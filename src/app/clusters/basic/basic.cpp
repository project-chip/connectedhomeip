...
void emberAfBasicClusterServerInitCallback(chip::EndpointId endpoint)
{
    ...
    char vendorName[chip::DeviceLayer::ConfigurationManager::kMaxVendorNameLength + 1];
    ...
    char productName[chip::DeviceLayer::ConfigurationManager::kMaxProductNameLength + 1];
    ...
    char hardwareVersionString[chip::DeviceLayer::ConfigurationManager::kMaxHardwareVersionStringLength + 1];
    if (ConfigurationMgr().GetHardwareVersionString(hardwareVersionString, sizeof(hardwareVersionString)) == CHIP_NO_ERROR)
    {
        status =
            Attributes::HardwareVersionString::Set(endpoint, chip::CharSpan(hardwareVersionString, strlen(hardwareVersionString)));
    ...
    char softwareVersionString[chip::DeviceLayer::ConfigurationManager::kMaxSoftwareVersionLength + 1];
    if (ConfigurationMgr().GetSoftwareVersionString(softwareVersionString, sizeof(softwareVersionString)) == CHIP_NO_ERROR)
    {
        status =
            Attributes::SoftwareVersionString::Set(endpoint, chip::CharSpan(softwareVersionString, strlen(softwareVersionString)));
        VerifyOrReturn(EMBER_ZCL_STATUS_SUCCESS == status,
    ...
    {
        status = Attributes::HardwareVersion::Set(endpoint, softwareVersion);
        VerifyOrReturn(EMBER_ZCL_STATUS_SUCCESS == status, ChipLogError(Zcl, "Error setting Software Version: 0x%02x", status));
    }

    char serialNumberString[chip::DeviceLayer::ConfigurationManager::kMaxSerialNumberLength + 1];
    ...
    char manufacturingDateString[chip::DeviceLayer::ConfigurationManager::kMaxManufacturingDateLength + 1];
