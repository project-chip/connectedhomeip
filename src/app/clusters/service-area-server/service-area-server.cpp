...
void Instance::HandleSelectLocationsCmd(HandlerContext & ctx, const Commands::SelectLocations::DecodableType & req)
{
    ...
    VerifyOrExit(
        (req.newLocations.IsNull() || (CHIP_NO_ERROR == req.newLocations.Value().ComputeSize(&numberOfLocations))),
        ...
        useLocationStatusText = true; ChipLogError(Zcl, "%s", locationStatusText));
    ...
    if (!req.newLocations.IsNull())
    {
        ...
        while (locationIter.Next())
        {
            ...
            VerifyOrExit(IsSupportedLocation(aSelectedLocation), locationStatus = SelectLocationsStatus::kInvalidSet;
        ...
        VerifyOrExit(CHIP_NO_ERROR == locationIter.GetStatus(), locationStatus = SelectLocationsStatus::kInvalidSet;
                     strncat(locationStatusText, "SelectLocations command - newLocations parameter value decoding failed",
                             kMaxSizeStatusText);
                     ChipLogError(Zcl, "%s", locationStatusText));
    }
...
void Instance::HandleSkipCurrentLocationCmd(HandlerContext & ctx)
{
    ...
    // description.
    VerifyOrExit(!mCurrentLocation.IsNull(), skipStatus = SkipCurrentLocationStatus::kInvalidInMode;
                 strncat(skipStatusText, "SkipCurrentLocation command - Current Location attribute is null", kMaxSizeStatusText);
                 ChipLogError(Zcl, "%s", skipStatusText); cmdStatus = Status::Failure);
    ...
    VerifyOrExit(mDelegate->HandleSkipCurrentLocation(skipStatusText), skipStatus = SkipCurrentLocationStatus::kInvalidInMode;
                 cmdStatus = Status::Failure);
...
bool Instance::IsValidSupportedLocation(const LocationStructureWrapper & aLocation)
{
    ...
    VerifyOrExit(
        (!aLocation.locationInfo.locationInfo.IsNull()) || (!aLocation.locationInfo.landmarkTag.IsNull()),
    ...
    if (!aLocation.locationInfo.locationInfo.IsNull())
    {
        ...
        VerifyOrExit(
            ...
            ChipLogError(Zcl,
                         "IsValidSupportedLocation %u - LocationName is empty string, Floornumber, AreaType, LandmarkTag are null",
    ...
    if (mDelegate->GetNumberOfSupportedMaps() == 0)
    {
        ...
        VerifyOrExit(aLocation.mapID.IsNull(),
                     ChipLogError(Zcl, "IsValidSupportedLocation %u - map Id %u is not in empty supported map list ",
                                  aLocation.locationID, aLocation.mapID.Value()));
    }
    else
    {
        ...
        VerifyOrExit((IsSupportedMap(aLocation.mapID.Value())),
                     ChipLogError(Zcl, "IsValidSupportedLocation %u - map Id %u is not in supported map list ",
                                  aLocation.locationID, aLocation.mapID.Value()));
    }
                                       ...
                                       const DataModel::Nullable<FloorSurfaceTag> & aSurfaceTag)
{
    ...
    VerifyOrExit(locationExists, ChipLogError(Zcl, "ModifySupportedLocation %u - not a supported locationID", aLocationId));

    {
        ...
        {
            // does device mode allow this attribute to be updated?
            VerifyOrExit(mDelegate->IsSupportedLocationsChangeAllowed(),
        ...
        VerifyOrExit(mDelegate->ModifySupportedLocation(listIndex, aNewLocation),
                     /* if false, should be logged as error in delegate function */);
    }
...
bool Instance::AddSelectedLocation(uint32_t & aSelectedLocation)
{
    ...
    VerifyOrExit(mDelegate->AddSelectedLocation(aSelectedLocation, dummyIndex),
                 /* if false, should be logged as error in delegate function */);
...
bool Instance::AddPendingProgressElement(uint32_t aLocationId)
{
    ...
    // check max# of list entries
    VerifyOrExit(
        (mDelegate->GetNumberOfProgressElements() <
         kMaxNumProgressElements), // Note: progress elements must uniquely map to supported locations, so max size is the same
...
bool Instance::SetProgressStatus(uint32_t aLocationId, OperationalStatusEnum opStatus)
{
    ...
    VerifyOrExit(mDelegate->ModifyProgressElement(listIndex, progressElement),
                 /* if false, should be logged as error in delegate function */);
...
bool Instance::SetProgressTotalOperationalTime(uint32_t aLocationId, const DataModel::Nullable<uint32_t> & aTotalOperationalTime)
{
    ...
    VerifyOrExit((aTotalOperationalTime.IsNull() || (progressElement.status == OperationalStatusEnum::kCompleted) ||
                 ...
                 ChipLogError(Zcl,
                              "SetProgressTotalOperationalTime - location %u opStatus value %u - can be non-null only if opStatus "
