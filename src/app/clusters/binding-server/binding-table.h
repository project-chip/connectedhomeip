...
{
    static BindingTableEntry ForNode(chip::FabricIndex fabric, chip::NodeId node, chip::EndpointId localEndpoint,
    ...
    static BindingTableEntry ForGroup(chip::FabricIndex fabric, chip::GroupId group, chip::EndpointId localEndpoint,
    ...
    /** The type of binding. */
    BindingType type = MATTER_UNUSED_BINDING;
...
public:
    ...
    public:
        BindingTableEntry & operator*() { return mTable->mBindingTable[mIndex]; }

        const BindingTableEntry & operator*() const { return mTable->mBindingTable[mIndex]; }
    ...
    CHIP_ERROR Add(const BindingTableEntry & entry);

    const BindingTableEntry & GetAt(uint8_t index);
...
private:
    ...
    BindingTableEntry mBindingTable[kMaxBindingEntries];
