...
using chip::app::Clusters::BindingTable;
...
using chip::app::Clusters::PendingNotificationEntry;
using chip::app::Clusters::PendingNotificationMap;
...
void CreateDefaultFullBindingTable(BindingTable & table)
{
    ...
    {
        table.Add(BindingTableEntry::ForNode(i / 10, i % 5, 0, 0, std::make_optional<ClusterId>(i)));
    }
}
...
TEST_F(TestPendingNotificationMap, TestAddRemove)
{
    ...
    {
        PendingNotificationEntry entry = *iter;
        BindingTableEntry bindingEntry = BindingTable::GetInstance().GetAt(entry.mBindingEntryId);
        pendingMapCount++;
    ...
    for (FabricIndex index = 1; index < BindingTable::kMaxBindingEntries / 10 + 1; index++)
    {
