...
static constexpr size_t kBindingFixedClusterCount =
    Binding::StaticApplicationConfig::kFixedClusterConfig.size();
...
{
    uint16_t arrayIndex =
        emberAfGetClusterServerEndpointIndex(endpointId, Binding::Id, kBindingFixedClusterCount);
...
void emberAfBindingClusterInitCallback(EndpointId endpointId)
{
...
void emberAfBindingClusterShutdownCallback(EndpointId endpointId)
{
    uint16_t arrayIndex = 0;
    if (!findEndpointWithLog(endpointId, arrayIndex))
    {
