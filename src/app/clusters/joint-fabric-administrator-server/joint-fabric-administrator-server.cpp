                                                           ...
                                                           const Commands::AddICAC::DecodableType & commandData)
{
    ...
    err = ConvertChipCertToX509Cert(ByteSpan{ anchorCAChipCert.data(), anchorCAChipCert.size() }, anchorCAX509);
    VerifyOrExit(err == CHIP_NO_ERROR, nonDefaultStatus = Status::Failure);

    // Convert the incoming ICAC to X.509
    err = ConvertChipCertToX509Cert(ByteSpan{ commandData.ICACValue.data(), commandData.ICACValue.size() }, crossSignedICACX509);
    VerifyOrExit(err == CHIP_NO_ERROR, nonDefaultStatus = Status::Failure);
    ...
    err = ConvertX509CertToChipCert(ByteSpan{ crossSignedICACX509.data(), crossSignedICACX509.size() }, crossSignedICACChipCert);
    VerifyOrExit(err == CHIP_NO_ERROR, nonDefaultStatus = Status::Failure);
