...
public:
    ...
    CHIP_ERROR SerializeSave(EndpointId endpoint, ClusterId cluster, MutableByteSpan & serializedBytes) override
    {
        ...
        {
            uint16_t xValue;
            if (MATTER_CL_STATUS_SUCCESS != Attributes::CurrentX::Get(endpoint, &xValue))
            {
            ...
            uint16_t yValue;
            if (MATTER_CL_STATUS_SUCCESS != Attributes::CurrentY::Get(endpoint, &yValue))
            {
        ...
        if (ColorControlServer::Instance().HasFeature(endpoint, ColorControlServer::Feature::kHueAndSaturation))
        {
            ...
            if (MATTER_CL_STATUS_SUCCESS != Attributes::CurrentSaturation::Get(endpoint, &saturationValue))
            {
        ...
        if (ColorControlServer::Instance().HasFeature(endpoint, ColorControlServer::Feature::kColorLoop))
        {
            ...
            if (MATTER_CL_STATUS_SUCCESS != Attributes::ColorLoopActive::Get(endpoint, &loopActiveValue))
            {
            ...
            if (MATTER_CL_STATUS_SUCCESS != Attributes::ColorLoopDirection::Get(endpoint, &loopDirectionValue))
            {
            ...
            if (MATTER_CL_STATUS_SUCCESS != Attributes::ColorLoopTime::Get(endpoint, &loopTimeValue))
            {
        ...
        if (ColorControlServer::Instance().HasFeature(endpoint, ColorControlServer::Feature::kColorTemperature))
        {
            ...
            if (MATTER_CL_STATUS_SUCCESS != Attributes::ColorTemperatureMireds::Get(endpoint, &temperatureValue))
            {
        ...
        uint8_t modeValue;
        if (MATTER_CL_STATUS_SUCCESS != Attributes::EnhancedColorMode::Get(endpoint, &modeValue))
        {
...
void ColorControlServer::startUpColorTempCommand(EndpointId endpoint)
{
    ...
    if (status == MATTER_CL_STATUS_SUCCESS && !startUpColorTemp.IsNull())
    {
...
void ColorControlServer::updateTempCommand(EndpointId endpoint)
{
    ...
    if (!isColorTempTransitionDone)
    {
        ...
        if (Attributes::ColorTemperatureMireds::Get(endpoint, &currentColorTemp) != MATTER_CL_STATUS_SUCCESS)
        {
