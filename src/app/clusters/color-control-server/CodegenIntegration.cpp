...
// Live attribute read accessors, re-provided in the generated-accessor shape (Attributes::<Name>::Get) so
// callers keep reading color state through the same fully-qualified names as before the migration. Each
// resolves the per-endpoint cluster and writes its live value, or returns UnsupportedEndpoint (leaving
// `value` untouched) when no Color Control server is registered on the endpoint. See CodegenIntegration.h.
...
namespace ColorMode {
Status Get(EndpointId endpoint, ColorModeEnum * value)
{
    auto * cluster = FindClusterOnEndpoint(endpoint);
    VerifyOrReturnValue(cluster != nullptr, Status::UnsupportedEndpoint);
