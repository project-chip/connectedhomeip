...
public:
    ...
    static CHIP_ERROR InitServerForFabricScopedTesting()
    {
        ...
        state.storage         = std::make_unique<chip::TestPersistentStorageDelegate>();
        state.timerDelegate   = std::make_unique<chip::app::DefaultTimerDelegate>();
        state.reportScheduler = std::make_unique<chip::app::reporting::ReportSchedulerImpl>(state.timerDelegate.get());
        ...
        initParams.persistentStorageDelegate = state.storage.get(); // Use test storage instead of KVS
        initParams.reportScheduler           = state.reportScheduler.get();
        ...
        initParams.dataModelProvider      = chip::app::CodegenDataModelProviderInstance(state.storage.get());
    ...
    static void ShutdownServerForFabricScopedTesting()
    {
        ...
        // Clean up static resources (always execute to prevent leaks if Init failed)
        // Note: reportScheduler must be reset before timerDelegate since it holds a reference to it
        state.reportScheduler.reset();
        state.timerDelegate.reset();
        state.storage.reset();
...
private:
    ...
    {
        std::unique_ptr<chip::TestPersistentStorageDelegate> storage;
        std::unique_ptr<chip::app::DefaultTimerDelegate> timerDelegate;
        std::unique_ptr<chip::app::reporting::ReportSchedulerImpl> reportScheduler;
