...
public:
    ...
    [[nodiscard]] std::optional<app::DataModel::ActionReturnStatus> Invoke(chip::CommandId commandId, const RequestType & request)
    {
        const auto & paths = mCluster.GetPaths();
        VerifyOrReturnValue(paths.size() == 1u, std::nullopt);
        ...
        return mCluster.InvokeCommand(invokeRequest, reader, &mHandler);
    }
