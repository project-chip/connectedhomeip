...
CameraAVStreamMgmtServer::CameraAVStreamMgmtServer(
    CameraAVStreamMgmtDelegate & aDelegate, EndpointId aEndpointId, BitFlags<Feature> aFeature,
    BitFlags<OptionalAttribute> aOptionalAttrs, PersistentStorageDelegate & aPersistentStorage, uint8_t aMaxConcurrentVideoEncoders,
                                                         ...
                                                         const Commands::VideoStreamAllocate::DecodableType & commandData)
{
    ...
    isWaterMarkEnabled = commandData.watermarkEnabled.Value();
    ...
    VerifyOrReturn((HasFeature(Feature::kOnScreenDisplay) && commandData.OSDEnabled.HasValue()) ||
                   ...
                   ctx.mCommandHandler.AddStatus(ctx.mRequestPath, Status::InvalidCommand));
    isOSDEnabled = commandData.OSDEnabled.Value();
