                                                                                        ...
                                                                                        CommandHandler * handler)
{
    ...
    case AllocatePushTransport::Id: {
        ...
        return HandleAllocatePushTransport(*handler, request.path, data);
    }
    case DeallocatePushTransport::Id: {
        DeallocatePushTransport::DecodableType data;
        ReturnErrorOnFailure(data.Decode(input_arguments, fabricIndex));
        return HandleDeallocatePushTransport(*handler, request.path, data);
    }
    case ModifyPushTransport::Id: {
        ModifyPushTransport::DecodableType data;
        ReturnErrorOnFailure(data.Decode(input_arguments, fabricIndex));
        return HandleModifyPushTransport(*handler, request.path, data);
    }
    case SetTransportStatus::Id: {
        SetTransportStatus::DecodableType data;
        ReturnErrorOnFailure(data.Decode(input_arguments, fabricIndex));
        return HandleSetTransportStatus(*handler, request.path, data);
    }
    case ManuallyTriggerTransport::Id: {
        ManuallyTriggerTransport::DecodableType data;
        ReturnErrorOnFailure(data.Decode(input_arguments, fabricIndex));
        return HandleManuallyTriggerTransport(*handler, request.path, data);
    }
    case FindTransport::Id: {
        FindTransport::DecodableType data;
        ReturnErrorOnFailure(data.Decode(input_arguments, fabricIndex));
        return HandleFindTransport(*handler, request.path, data);
    }
    }
...
bool PushAvStreamTransportServer::IsNullDelegateWithLogging(EndpointId endpointIdForLogging)
{
...
PushAvStreamTransportServer::UpsertStreamTransportConnection(const TransportConfigurationStorage & transportConfiguration)
{
    ...
    NotifyAttributeChanged(PushAvStreamTransport::Attributes::CurrentConnections::Id);
...
bool PushAvStreamTransportServer::ValidateUrl(const std::string & url)
{
    UriUriA uri;
    const char * errorPos;
...
CHIP_ERROR PushAvStreamTransportServer::ScheduleTransportDeallocate(uint16_t connectionID, uint32_t timeoutSec)
{
    ...
    const Structs::TransportOptionsStruct::DecodableType & transportOptions)
{
    ...
    VerifyOrReturnValue(
        ...
        ChipLogError(Zcl, "Transport Options verification from command data[ep=%d]: Invalid streamUsage ", mPath.mEndpointId));
    ...
    VerifyOrReturnValue(transportOptions.url.size() >= kMinUrlLength && transportOptions.url.size() <= kMaxUrlLength,
                        ...
                        ChipLogError(Zcl,
                                     ...
                                     mPath.mEndpointId, static_cast<uint32_t>(transportOptions.url.size())));
    ...
    VerifyOrReturnValue(
        ...
        ChipLogError(Zcl, "Transport Options verification from command data[ep=%d]: Invalid triggerType ", mPath.mEndpointId));

    if (triggerOptions.triggerType == TransportTriggerTypeEnum::kMotion)
    {
        VerifyOrReturnValue(
            triggerOptions.motionZones.HasValue(), Status::InvalidCommand,
            ChipLogError(Zcl, "Transport Options verification from command data[ep=%d]: Missing motion zones ", mPath.mEndpointId));
    ...
    if (triggerOptions.triggerType == TransportTriggerTypeEnum::kMotion && !mFeatures.Has(Feature::kPerZoneSensitivity))
    {
        VerifyOrReturnValue(triggerOptions.motionSensitivity.HasValue(), Status::InvalidCommand,
                            ChipLogError(Zcl,
                                         "Transport Options verification from command data[ep=%d]: Missing Motion Sensitivity ",
                                         mPath.mEndpointId));
    ...
    VerifyOrReturnValue(
        ...
        ChipLogError(Zcl, "Transport Options verification from command data[ep=%d]: Invalid Ingest Method ", mPath.mEndpointId));
    ...
    VerifyOrReturnValue(
        ...
        ChipLogError(Zcl, "Transport Options verification from command data[ep=%d]: Invalid Container Format ", mPath.mEndpointId));
...
std::optional<DataModel::ActionReturnStatus> PushAvStreamTransportServer::ValidateStreamParameters(
    ...
    const std::shared_ptr<PushAvStreamTransport::TransportOptionsStorage> transportOptionsPtr)
{
    ...
    VerifyOrReturnValue(
        !(hasNewStreamFields && hasOldStreamFields), Status::InvalidCommand,
        ChipLogError(Zcl, "Transport Options[ep=%d]: Both new and old stream fields are present", mPath.mEndpointId));

    VerifyOrReturnValue(hasNewStreamFields || hasOldStreamFields, Status::InvalidCommand,
                        ChipLogError(Zcl, "Transport Options[ep=%d]: Missing stream fields", mPath.mEndpointId));
    ...
    if (hasNewStreamFields)
    {
        ...
        if (transportOptions.videoStreams.HasValue())
        {
            ...
            if ((videoStreamNames.size() != videoStreamsCount) || (videoStreamIDs.size() != videoStreamsCount))
            {
                ...
                ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Duplicate videoStreamName or videoStreamID found",
                             mPath.mEndpointId);
                LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, status));
                return std::nullopt;
            }
            ...
            for (auto iter = transportOptions.videoStreams.Value().begin(); iter.Next();)
            {
                ...
                VerifyOrReturnValue(
                    status.IsSuccess(), Status::InvalidCommand,
                    ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Invalid Video Stream ID", mPath.mEndpointId));
            }
        }
        ...
        if (transportOptions.audioStreams.HasValue())
        {
            ...
            for (auto iter = transportOptions.audioStreams.Value().begin(); iter.Next();)
            {
                ...
                VerifyOrReturnValue(
                    status.IsSuccess(), Status::InvalidCommand,
                    ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Invalid Audio Stream ID", mPath.mEndpointId));
            }
        }
        ...
        {
            auto status = to_underlying(StatusCodeEnum::kInvalidStream);
            ChipLogError(Zcl, "Transport Options[ep=%d]: Both VideoStreams and AudioStreams empty", mPath.mEndpointId);
            LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, status));
            return std::nullopt;
        }
    }
    ...
    if (hasOldStreamFields)
    {
        ...
        if (transportOptions.videoStreamID.HasValue())
        {
            ...
            if (transportOptions.videoStreamID.Value().IsNull())
            {
                ...
                if (!delegateStatus.IsSuccess())
                {
                    ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: AllocatedVideoStreamsAttribute is empty",
                                 mPath.mEndpointId);
            ...
            else
            {
                ...
                if (!delegateStatus.IsSuccess())
                {
                    ...
                    ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Invalid Video Stream ", mPath.mEndpointId);
                    LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, cluster_status));
                    return std::nullopt;
                }
        ...
        if (transportOptions.audioStreamID.HasValue())
        {
            ...
            if (transportOptions.audioStreamID.Value().IsNull())
            {
                ...
                if (!delegateStatus.IsSuccess())
                {
                    ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: AllocatedAudioStreamsAttribute is empty",
                                 mPath.mEndpointId);
                    handler.AddStatus(commandPath, Status::InvalidInState);
                    return std::nullopt;
                }
            ...
            else
            {
                ...
                {
                    auto cluster_status = to_underlying(StatusCodeEnum::kInvalidStream);
                    ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Invalid Audio Stream ", mPath.mEndpointId);
                    LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, cluster_status));
                    return std::nullopt;
                }
...
PushAvStreamTransportServer::HandleAllocatePushTransport(CommandHandler & handler, const ConcreteCommandPath & commandPath,
                                                         const Commands::AllocatePushTransport::DecodableType & commandData)
{
    ...
    VerifyOrDo(transportOptionsValidityStatus == Status::Success, {
        ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: TransportOptions of command data is not Valid", mPath.mEndpointId);
    ...
    {
        ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: TLS Client Management Delegate is not set", mPath.mEndpointId);
        auto status = to_underlying(StatusCodeEnum::kInvalidTLSEndpoint);
        LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, status));
        return std::nullopt;
    }
    ...
    if (isFormatSupported == false)
    {
        ...
        ChipLogError(Zcl,
                     ...
                     mPath.mEndpointId, to_underlying(ingestMethod),
        ...
        transportOptions.containerOptions.CMAFContainerOptions.HasValue())
    {
        ...
        if (isValidUrl == false)
        {
            ...
            ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Invalid Url", mPath.mEndpointId);
            LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, status));
            return std::nullopt;
        }
    }
        ...
        (transportOptions.triggerOptions.motionZones.HasValue()) && (!transportOptions.triggerOptions.motionZones.Value().IsNull()))
    {
        ...
        LogErrorOnFailure(motionZonesList.Value().Value().ComputeSize(&zoneListSize));
        ...
        VerifyOrReturnValue(isValidZoneSize, Status::DynamicConstraintError,
                            ChipLogError(Zcl, "Transport Options verification from command data[ep=%d]: Invalid Motion Zone Size ",
                                         mPath.mEndpointId));
        ...
        while (iter.Next())
        {
            ...
            if (!transportZoneOption.zone.IsNull())
            {
                ...
                if (zoneIdStatus != Status::Success)
                {
                    ...
                    ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Invalid ZoneId", mPath.mEndpointId);
                    LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, status));
                    return std::nullopt;
                }
            }
    ...
    {
        ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Memory Allocation failed for transportOptions", mPath.mEndpointId);
        ...
        transportOptions.containerOptions.CMAFContainerOptions.HasValue())
    {
        ...
        if (transportOptionsPtr->videoStreamID.HasValue())
        {
            ...
            if (isValidSegmentDuration == false)
            {
                ...
                ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Segment Duration not within allowed range",
                             mPath.mEndpointId);
                LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, segmentDurationStatus));
                return std::nullopt;
            }
        }
    }
    ...
    {
        ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Resource Exhausted", mPath.mEndpointId);
    ...
    if (transportOptionsPtr->videoStreamID.HasValue() && transportOptions.triggerOptions.maxPreRollLen.HasValue())
    {
            ...
            !mDelegate->ValidateMaxPreRollLength(maxPreRollLength, transportOptionsPtr->videoStreamID.Value()))
        {
            ...
            ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: MaxPreRollLength (%u) validation failed", mPath.mEndpointId,
                         maxPreRollLength);
            LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, maxPreRollLengthStatus));
            return std::nullopt;
        }
    }
    ...
    {
        ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: Max Connections Exhausted", mPath.mEndpointId);
        handler.AddStatus(commandPath, Status::ResourceExhausted);
        return std::nullopt;
    }
    ...
    if (retStatus == Status::Success)
    {
        ...
        {
            LogErrorOnFailure(ScheduleTransportDeallocate(connectionID, transportOptions.expiryTime.Value()));
        }
...
std::optional<DataModel::ActionReturnStatus>
PushAvStreamTransportServer::HandleDeallocatePushTransport(CommandHandler & handler, const ConcreteCommandPath & commandPath,
                                                           const Commands::DeallocatePushTransport::DecodableType & commandData)
{
    ...
    {
        ChipLogError(Zcl, "HandleDeallocatePushTransport[ep=%d]: ConnectionID Not Found.", mPath.mEndpointId);
    ...
    {
        ChipLogError(Zcl, "HandleDeallocatePushTransport[ep=%d]: Connection is Busy", mPath.mEndpointId);
...
std::optional<DataModel::ActionReturnStatus>
PushAvStreamTransportServer::HandleModifyPushTransport(CommandHandler & handler, const ConcreteCommandPath & commandPath,
                                                       const Commands::ModifyPushTransport::DecodableType & commandData)
{
    ...
    VerifyOrDo(transportOptionsValidityStatus == Status::Success, {
        ChipLogError(Zcl, "HandleModifyPushTransport[ep=%d]: TransportOptions of command data is not Valid", mPath.mEndpointId);
        handler.AddStatus(commandPath, transportOptionsValidityStatus);
        return std::nullopt;
    });
    ...
    if (transportConfiguration == nullptr)
    {
        ChipLogError(Zcl, "HandleModifyPushTransport[ep=%d]: ConnectionID Not Found.", mPath.mEndpointId);
        handler.AddStatus(commandPath, Status::NotFound);
        return std::nullopt;
    }
    ...
    if (mDelegate->GetTransportBusyStatus(connectionID) == PushAvStreamTransportStatusEnum::kBusy)
    {
        ChipLogError(Zcl, "HandleModifyPushTransport[ep=%d]: Connection is Busy", mPath.mEndpointId);
        handler.AddStatus(commandPath, Status::Busy);
        return std::nullopt;
    }
    ...
    if (transportOptionsPtr == nullptr)
    {
        ChipLogError(Zcl, "HandleModifyPushTransport[ep=%d]: Memory Allocation failed for transportOptions", mPath.mEndpointId);
        handler.AddStatus(commandPath, Status::ResourceExhausted);
        return std::nullopt;
    }
...
std::optional<DataModel::ActionReturnStatus>
PushAvStreamTransportServer::HandleSetTransportStatus(CommandHandler & handler, const ConcreteCommandPath & commandPath,
                                                      const Commands::SetTransportStatus::DecodableType & commandData)
{
    ...
    VerifyOrDo(transportStatus != TransportStatusEnum::kUnknownEnumValue, {
        ChipLogError(Zcl, "HandleSetTransportStatus[ep=%d]: Invalid Transport Status in command data", mPath.mEndpointId);
    ...
    else
    {
        ...
        if (transportConfiguration == nullptr)
        {
            ChipLogError(Zcl, "HandleSetTransportStatus[ep=%d]: ConnectionID Not Found.", mPath.mEndpointId);
            handler.AddStatus(commandPath, Status::NotFound);
            return std::nullopt;
        }
...
std::optional<DataModel::ActionReturnStatus>
PushAvStreamTransportServer::HandleManuallyTriggerTransport(CommandHandler & handler, const ConcreteCommandPath & commandPath,
                                                            const Commands::ManuallyTriggerTransport::DecodableType & commandData)
{
    ...
    VerifyOrDo(activationReason != TriggerActivationReasonEnum::kUnknownEnumValue, {
        ChipLogError(Zcl, "HandleManuallyTriggerTransport[ep=%d]: Invalid Activation Reason ", mPath.mEndpointId);
        handler.AddStatus(commandPath, Status::ConstraintError);
        return std::nullopt;
    });
    ...
    if (transportConfiguration == nullptr)
    {
        ChipLogError(Zcl, "HandleManuallyTriggerTransport[ep=%d]: ConnectionID Not Found.", mPath.mEndpointId);
        handler.AddStatus(commandPath, Status::NotFound);
        return std::nullopt;
    }
    ...
    if (mDelegate->GetTransportBusyStatus(connectionID) == PushAvStreamTransportStatusEnum::kBusy)
    {
        ChipLogError(Zcl, "HandleManuallyTriggerTransport[ep=%d]: Connection is Busy", mPath.mEndpointId);
        handler.AddStatus(commandPath, Status::Busy);
        return std::nullopt;
    }
    ...
    if (transportConfiguration->transportStatus == TransportStatusEnum::kInactive)
    {
        ...
        ChipLogError(Zcl, "HandleManuallyTriggerTransport[ep=%d]: Invalid Transport status", mPath.mEndpointId);
        LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, clusterStatus));
        return std::nullopt;
    }
    ...
    if (transportConfiguration->transportOptions.Value().triggerOptions.triggerType == TransportTriggerTypeEnum::kContinuous)
    {
        ...
        ChipLogError(Zcl, "HandleManuallyTriggerTransport[ep=%d]: Invalid Trigger type", mPath.mEndpointId);
        LogErrorOnFailure(handler.AddClusterSpecificFailure(commandPath, clusterStatus));
        return std::nullopt;
    }
    ...
    {
        ChipLogError(Zcl, "HandleManuallyTriggerTransport[ep=%d]: Time control field not present", mPath.mEndpointId);
...
std::optional<DataModel::ActionReturnStatus>
PushAvStreamTransportServer::HandleFindTransport(CommandHandler & handler, const ConcreteCommandPath & commandPath,
                                                 const Commands::FindTransport::DecodableType & commandData)
{
    ...
    if (connectionID.IsNull())
    {
        ...
        {
            ChipLogError(Zcl, "HandleFindTransport[ep=%d]: ConnectionID not found", mPath.mEndpointId);
            handler.AddStatus(commandPath, Status::NotFound);
            return std::nullopt;
        }
...
Status PushAvStreamTransportServer::CheckPrivacyModes(StreamUsageEnum streamUsage)
{
...
Status PushAvStreamTransportServer::GeneratePushTransportBeginEvent(const uint16_t connectionID,
                                                                    ...
                                                                    const ContainerFormatEnum containerType,
...
Status PushAvStreamTransportServer::GeneratePushTransportEndEvent(const uint16_t connectionID)
{
    ...
    if (CHIP_NO_ERROR != err)
    {
        ChipLogError(AppServer, "Endpoint %d - Unable to generate PushAVTransportEnd event: %" CHIP_ERROR_FORMAT, mPath.mEndpointId,
                     err.Format());
        return Status::Failure;
    }
    return Status::Success;
...
Status PushAvStreamTransportServer::NotifyTransportStarted(uint16_t connectionID, TransportTriggerTypeEnum triggerType,
