    ...
    HandlerContext & ctx, const Commands::ManuallyTriggerTransport::DecodableType & commandData)
{
    ...
    /*if (transportConfiguration->transportOptions)
    {
        if (transportConfiguration->transportOptions.triggerOptions.triggerType == TransportTriggerTypeEnum::kContinuous)
        {
            {
                auto status = static_cast<uint8_t>(StatusCodeEnum::kInvalidTriggerType);
        ...
        else if (transportConfiguration->transportOptions.triggerOptions.triggerType ==
