    ...
    const Structs::TransportOptionsStruct::DecodableType & transportOptions)
{
    ...
    VerifyOrReturnValue(
        ...
        ChipLogError(Zcl, "HandleAllocatePushTransport[ep=%d]: URL length: %zu not in allowed length range of 13 to 2000",
    ...
    if (containerOptions.containerType == ContainerFormatEnum::kCmaf)
    {
        ...
        if (containerOptions.CMAFContainerOptions.Value().CENCKey.HasValue())
        {
            VerifyOrReturnValue(
                ...
                ChipLogError(
                    Zcl,
                    "HandleAllocatePushTransport[ep=%d]: CMAF Container Options CENC Key constraint Error, actual length: %zu not "
        ...
        if (containerOptions.CMAFContainerOptions.Value().CENCKey.HasValue())
        {
            ...
            VerifyOrReturnValue(
                ...
                ChipLogError(Zcl,
                             ...
                             "length: %zu not equal to expected length of 16",
                             AttributeAccessInterface::GetEndpointId().Value(),
