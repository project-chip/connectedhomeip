...
Status ThermostatCluster::SetActiveSchedule(DataModel::Nullable<ByteSpan> scheduleHandle)
{
    ...
    if (!scheduleHandle.IsNull() && !IsScheduleHandlePresentInSchedules(mDelegate, scheduleHandle.Value()))
    {
    ...
    bool havePreviousScheduleHandle   = (mDelegate->GetActiveScheduleHandle(previousScheduleHandleBuffer) == CHIP_NO_ERROR);
    ...
    CHIP_ERROR err = mDelegate->SetActiveScheduleHandle(scheduleHandle);
    ...
    GenerateActiveScheduleChangeEvent(mPath.mEndpointId, previousScheduleHandle, scheduleHandle);

    return Status::Success;
}

CHIP_ERROR ThermostatCluster::AppendPendingSchedule(const ScheduleStruct::DecodableType & newSchedule)
{
    if (mDelegate == nullptr)
    {
        ChipLogError(Zcl, "Delegate is null");
...
Status ThermostatCluster::PrecommitSchedules()
{
    if (mDelegate == nullptr)
    {
        ChipLogError(Zcl, "Delegate is null");
