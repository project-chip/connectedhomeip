...
Status ThermostatCluster::PrecommitPresets()
{
    ...
    for (uint8_t i = 0; true; i++)
    {
        ...
        {
            pendingPreset.SetCoolingSetpoint(MakeOptional(mSetpoints.EnforceCoolingSetpointLimits(coolingSetpointValue.Value())));
        }

        Optional<int16_t> heatingSetpointValue = pendingPreset.GetHeatingSetpoint();
        if (heatingSetpointValue.HasValue())
        {
            pendingPreset.SetHeatingSetpoint(MakeOptional(mSetpoints.EnforceHeatingSetpointLimits(heatingSetpointValue.Value())));
        }
    }
