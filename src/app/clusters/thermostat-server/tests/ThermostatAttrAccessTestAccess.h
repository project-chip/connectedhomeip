...
/**
 * @brief Accessor class that exposes private schedule-related methods of ThermostatAttrAccess for unit testing.
 *
 * ThermostatAttrAccess grants friend access to this class so that tests can call private methods
 * directly without resorting to #define private public.
 */
...
public:
    ...
    CHIP_ERROR AppendPendingSchedule(chip::app::Clusters::Thermostat::Delegate * delegate,
                                     const chip::app::Clusters::Thermostat::Structs::ScheduleStruct::DecodableType & schedule)
    {
        return mAccess->AppendPendingSchedule(delegate, schedule);
    }

    Protocols::InteractionModel::Status PrecommitSchedules(EndpointId endpoint) { return mAccess->PrecommitSchedules(endpoint); }
