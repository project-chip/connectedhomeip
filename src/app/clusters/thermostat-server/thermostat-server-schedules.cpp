...
bool IsScheduleHandlePresentInSchedules(Delegate * delegate, const ByteSpan & scheduleHandleToMatch)
{
    ...
    for (uint8_t i = 0; true; i++)
    {
        ...
        if (!matchingSchedule.GetScheduleHandle().IsNull() && matchingSchedule.GetScheduleHandle().Value().data_equal(scheduleHandleToMatch))
        {
            return true;
        }
    }
 ...
 * @param[out] matchingSchedule The schedule in the Schedules attribute list that has the same ScheduleHandle as the scheduleToMatch.
 *
                                ...
                                ScheduleStructWithOwnedMembers & matchingSchedule)
{
    ...
    for (uint8_t i = 0; true; i++)
    {
        ...
        if (err != CHIP_NO_ERROR)
        {
            ChipLogError(Zcl, "GetMatchingScheduleInSchedules: GetScheduleAtIndex failed with error %" CHIP_ERROR_FORMAT, err.Format());
            return false;
        }
...
/**
 * @brief Returns the length of the list of schedules if the pending schedules were to be applied. The size of the pending schedules list
 *        calculated, after all the constraint checks are done, is the new size of the updated Schedules attribute since the pending
 ...
 */
bool ScheduleHandlesExistAndMatch(const ScheduleStructWithOwnedMembers & schedule, const ScheduleStructWithOwnedMembers & scheduleToMatch)
{
...
CHIP_ERROR ThermostatAttrAccess::AppendPendingSchedule(Thermostat::Delegate * delegate, const ScheduleStruct::Type & newSchedule)
{
    ...
    if (schedule.GetName().HasValue() && !ScheduleTypeSupportsFeature(delegate, schedule.GetSystemMode(), ScheduleTypeFeaturesBitmap::kSupportsNames))
    {
        return CHIP_IM_GLOBAL_STATUS(ConstraintError);
    }
...
Status ThermostatAttrAccess::PrecommitSchedules(EndpointId endpoint)
{
    ...
    // If there is an ActiveScheduleHandle set, find the schedule in the pending schedules list that matches the ActiveScheduleHandle
    // attribute. If a schedule is not found with the same scheduleHandle, return INVALID_IN_STATE. If there is no ActiveScheduleHandle
