                                                                                     ...
                                                                                     const DataModel::InvokeRequest & request)
{
    ...
    response.proxyScanResult = List<const ScanResult>(Span<const ScanResult>(results.data(), results.size()));
...
Protocols::InteractionModel::Status CommissioningProxyMockDelegate::ProxyMessageRequest(uint16_t sessionId,
