                                               ...
                                               RemoteLockOpHandler opHandler, const Optional<ByteSpan> & pinCode)
{
    ...
    if (!success && reason == DlOperationError::kInvalidCredential)
    {
        ...
        auto status                 = Attributes::WrongCodeEntryLimit::Get(endpoint, &wrongCodeEntryLimit);
        if (EMBER_ZCL_STATUS_SUCCESS == status)
        {
            if (++endpointContext->wrongCodeEntryAttempts >= wrongCodeEntryLimit)
            {
                emberAfDoorLockClusterPrintln(
                    "Too many wrong code entry attempts, engaging lockout [endpoint=%d,wrongCodeAttempts=%d]", endpoint,
                    endpointContext->wrongCodeEntryAttempts);
                EngageLockout(endpoint);
            }
        }
