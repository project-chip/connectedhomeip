...
Delegate * GetDelegate(EndpointId endpoint)
{
    ...
    if (app != nullptr)
    {
        ChipLogError(Zcl, "KeypadInput returning ContentApp delegate for endpoint:%u", endpoint);
        return app->GetKeypadInputDelegate();
    }
#endif // CHIP_DEVICE_CONFIG_APP_PLATFORM_ENABLED
    ChipLogError(Zcl, "KeypadInput NOT returning ContentApp delegate for endpoint:%u", endpoint);
...
bool isDelegateNull(Delegate * delegate, EndpointId endpoint)
{
    if (delegate == nullptr)
    {
        ChipLogError(Zcl, "Keypad Input has no delegate set for endpoint:%u", endpoint);
        return true;
    }
    return false;
}
