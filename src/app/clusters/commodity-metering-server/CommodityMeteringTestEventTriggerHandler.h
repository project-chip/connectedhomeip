 ...
 */
enum class CommodityMeteringTrigger : uint64_t
{
    // Scenarios
    // Attributes Value Update Test Event | Increment all attributes values (strings are incremented lexicographically)
    kAttributesValueUpdate = 0x0b07000000000000,
    // Attributes Value Test Event Clear | Return the device to pre-test status
    kAttributesValueUpdateClear = 0x0b07000000000001,
};

class CommodityMeteringTestEventTriggerHandler : public TestEventTriggerHandler
{
public:
    ...
    CHIP_ERROR HandleEventTrigger(uint64_t eventTrigger) override
    {
        ...
        if (HandleCommodityMeteringTestEventTrigger(eventTrigger))
        {
