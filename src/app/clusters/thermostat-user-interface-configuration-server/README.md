...
`CodegenIntegration.h` also declares live attribute `Get()` and `Set()` functions.
Generated `GetDefault()` accessors read startup defaults from the ZAP/Ember
attribute store, rather than the live cluster state.
...
`Config`, attach the delegate with `SetDelegate()`, and register the instance with
the application's data model provider. No `CodegenIntegration` lookup is needed.
See the [cluster development guide](../../../../docs/guides/writing_clusters.md)
for the registration and lifecycle model.
