...
Delegate * GetDelegate(EndpointId endpoint)
{
    ...
    if (app != nullptr)
    {
        ChipLogError(Zcl, "AccountLogin returning ContentApp delegate for endpoint:%u", endpoint);
        return app->GetAccountLoginDelegate();
    }
#endif // CHIP_DEVICE_CONFIG_APP_PLATFORM_ENABLED
    ChipLogError(Zcl, "AccountLogin NOT returning ContentApp delegate for endpoint:%u", endpoint);
...
bool isDelegateNull(Delegate * delegate, EndpointId endpoint)
{
    if (delegate == nullptr)
    {
        ChipLogError(Zcl, "Account Login has no delegate set for endpoint:%u", endpoint);
        return true;
    }
    return false;
}
