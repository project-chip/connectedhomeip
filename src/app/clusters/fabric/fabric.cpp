...
bool emberAfFabricClusterRemoveFabricCallback(chip::FabricId fabricId, chip::NodeId nodeId, uint16_t vendorId)
{
    ...
    emberAfPrintln(EMBER_AF_PRINT_DEBUG, "Fabric: RemoveFabric"); // TODO: Generate emberAfFabricClusterPrintln
...
{
    emberAfPrintln(EMBER_AF_PRINT_DEBUG, "Fabric: GetFabricId");
    EmberAfStatus status = EMBER_ZCL_STATUS_FAILURE;
    emberAfSendImmediateDefaultResponse(status);
    return true;
}
...
{
    emberAfPrintln(EMBER_AF_PRINT_DEBUG, "Fabric: UpdateFabricLabel");
