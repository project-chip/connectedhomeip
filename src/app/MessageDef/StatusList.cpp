...
CHIP_ERROR StatusList::Parser::CheckSchemaValidity() const
{
    CHIP_ERROR err     = CHIP_NO_ERROR;
    size_t NumStatusElements = 0;
...
StatusElement::Builder & StatusList::Builder::CreateStatusElementBuilder()
{
    // skip if error has already been set
    VerifyOrExit(CHIP_NO_ERROR == mError, mStatusElementBuilder.ResetError(mError));

    mError = mStatusElementBuilder.Init(mpWriter);
