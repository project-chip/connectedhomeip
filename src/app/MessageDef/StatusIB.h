...
enum
{
    kCsTag_GeneralStatus = 0,
    kCsTag_ClusterStatus = 1,
};

struct Type
{
    Protocols::SecureChannel::GeneralStatusCode generalStatus;
    Protocols::InteractionModel::Status clusterStatus;
};
...
public:
    ...
    /**
    `* Read the GeneralStatus, ClusterStatus
     *
     * @param[out]   apGeneralStatus     Pointer to the storage for the GeneralStatus
     * @param[out]   apClusterStatus   Pointer to the storage for the ClusterStatus
     *
     ...
     *               type.
     */
    CHIP_ERROR DecodeStatusIB(Protocols::SecureChannel::GeneralStatusCode * apGeneralStatus,
                              Protocols::InteractionModel::Status * apClusterStatus) const;

    inline CHIP_ERROR DecodeStatusIB(Type & aStatusIB)
    {
        return DecodeStatusIB(&aStatusIB.generalStatus, &aStatusIB.clusterStatus);
    };
};
...
public:
    ...
    `* Read the GeneralStatus, ClusterStatus
     *
     * @param[in]   aGeneralStatus    General status code
     * @param[in]   aClusterStatus   16-bit protocol-specific error code
     *
     * @return       CHIP_ERROR codes returned by chip::TLV objects. CHIP_END_OF_TLV if either
     *               element is missing. CHIP_ERROR_WRONG_TLV_TYPE if the elements are of the wrong
     *               type.
     */
    StatusIB::Builder & EncodeStatusIB(const Protocols::SecureChannel::GeneralStatusCode aGeneralStatus,
                                       const Protocols::InteractionModel::Status aClusterStatus);

    inline StatusIB::Builder & EncodeStatusIB(const Type & aStatusIB)
    {
        return EncodeStatusIB(aStatusIB.generalStatus, aStatusIB.clusterStatus);
    }
