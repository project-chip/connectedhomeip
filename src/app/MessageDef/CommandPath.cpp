...
CHIP_ERROR Type::Decode(chip::TLV::TLVReader & reader)
{
    CHIP_ERROR err;
    TLV::TLVType outer;
    ...
    while ((err = reader.Next()) == CHIP_NO_ERROR)
    {
        ...
        if (tag == TLV::ContextTag(kCsTag_EndpointId))
        {
            ReturnErrorOnFailure(DataModel::Decode(reader, endpointId));
        }
        else if (tag == TLV::ContextTag(kCsTag_ClusterId))
        {
            ReturnErrorOnFailure(DataModel::Decode(reader, clusterId));
        }
        else if (tag == TLV::ContextTag(kCsTag_CommandId))
        {
