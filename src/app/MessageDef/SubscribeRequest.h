...
public:
     ...
     *          #CHIP_END_OF_TLV if there is no such element
     */
    CHIP_ERROR GetMinInterval(uint16_t * const apMinInterval) const;
...
public:
    /**
     *  @brief Initialize a SubscribeRequest::Builder for writing into a TLV stream
     *
    ...
    /**
     *  @brief The requested minimum interval boundary floor in seconds
     *  @return A reference to *this
     */
    SubscribeRequest::Builder & MinInterval(const uint16_t aMinInterval);

    /**
     *  @brief  the requested maximum interval boundary ceiling in seconds
     *  @return A reference to *this
     */
    SubscribeRequest::Builder & MaxInterval(const uint16_t aMinInterval);
