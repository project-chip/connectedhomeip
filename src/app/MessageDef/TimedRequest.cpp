...
CHIP_ERROR TimedRequest::Parser::CheckSchemaValidity() const
{
    ...
    while (CHIP_NO_ERROR == (err = reader.Next()))
    {
        ...
        {
        case kCsTag_Timeout:
            VerifyOrReturnLogError(!(TagPresenceMask & (1 << kCsTag_Timeout)), CHIP_ERROR_INVALID_TLV_TAG);
            TagPresenceMask |= (1 << kCsTag_Timeout);
...
#if CHIP_DETAIL_LOGGING
            {
                ...
                PRETTY_PRINT("\tTimeout = 0x%" PRIx8 ",", timeout);
            }
    ...
    {
        const uint16_t RequiredFields = (1 << kCsTag_Timeout);
...
CHIP_ERROR TimedRequest::Parser::GetTimeout(uint8_t * const apTimeout) const
{
    return GetUnsignedInteger(kCsTag_Timeout, apTimeout);
}
...
TimedRequest::Builder & TimedRequest::Builder::Timeout(const uint8_t aTimeout)
{
    // skip if error has already been set
    if (mError == CHIP_NO_ERROR)
    {
        mError = mpWriter->Put(chip::TLV::ContextTag(kCsTag_Timeout), aTimeout);
        ChipLogFunctError(mError);
    }
    return *this;
}

TimedRequest::Builder & TimedRequest::Builder::EndOfTimedRequest()
{
    EndOfContainer();
    return *this;
}
