...
CHIP_ERROR TimedRequest::Parser::CheckSchemaValidity() const
{
    ...
    while (CHIP_NO_ERROR == (err = reader.Next()))
    {
        ...
        {
        case kCsTag_TimeoutMilliseconds:
            VerifyOrReturnLogError(!(TagPresenceMask & (1 << kCsTag_TimeoutMilliseconds)), CHIP_ERROR_INVALID_TLV_TAG);
            TagPresenceMask |= (1 << kCsTag_TimeoutMilliseconds);
...
#if CHIP_DETAIL_LOGGING
            {
                ...
                PRETTY_PRINT("\tTimeoutMilliseconds = 0x%" PRIx8 ",", timeout);
            }
    ...
    {
        const uint16_t RequiredFields = (1 << kCsTag_TimeoutMilliseconds);
...
CHIP_ERROR TimedRequest::Parser::GetTimeoutMilliseconds(uint16_t * const apTimeoutMilliseconds) const
{
    return GetUnsignedInteger(kCsTag_TimeoutMilliseconds, apTimeoutMilliseconds);
}
...
TimedRequest::Builder & TimedRequest::Builder::TimeoutMilliseconds(const uint16_t aTimeoutMilliseconds)
{
    // skip if error has already been set
    if (mError == CHIP_NO_ERROR)
    {
        mError = mpWriter->Put(chip::TLV::ContextTag(kCsTag_TimeoutMilliseconds), aTimeoutMilliseconds);
