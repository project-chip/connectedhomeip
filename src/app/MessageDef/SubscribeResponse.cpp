...
CHIP_ERROR SubscribeResponse::Parser::CheckSchemaValidity() const
{
    ...
    while (CHIP_NO_ERROR == (err = reader.Next()))
    {
        ...
        case kCsTag_FinalSyncInterval:
            VerifyOrExit(!(TagPresenceMask & (1 << kCsTag_FinalSyncInterval)), err = CHIP_ERROR_INVALID_TLV_TAG);
            TagPresenceMask |= (1 << kCsTag_FinalSyncInterval);
            VerifyOrExit(chip::TLV::kTLVType_UnsignedInteger == reader.GetType(), err = CHIP_ERROR_WRONG_TLV_TYPE);
#if CHIP_DETAIL_LOGGING
            {
                uint16_t finalSyncInterval;
                err = reader.Get(finalSyncInterval);
                SuccessOrExit(err);
...
CHIP_ERROR SubscribeResponse::Parser::GetFinalSyncInterval(uint16_t * const apFinalSyncInterval) const
{
    return GetUnsignedInteger(kCsTag_FinalSyncInterval, apFinalSyncInterval);
}
...
SubscribeResponse::Builder & SubscribeResponse::Builder::FinalSyncInterval(const uint16_t aFinalSyncInterval)
{
    // skip if error has already been set
    if (mError == CHIP_NO_ERROR)
    {
        mError = mpWriter->Put(chip::TLV::ContextTag(kCsTag_FinalSyncInterval), aFinalSyncInterval);
        ChipLogFunctError(mError);
    }
    return *this;
}
