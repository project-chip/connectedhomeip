...
CHIP_ERROR JointFabricDatastore::RefreshNode(NodeId nodeId, ReadOnlyBuffer<DataModel::EndpointEntry> endpointsList)
{
    ...
    {
        auto it =
            std::find_if(mEndpointEntries.begin(), mEndpointEntries.end(),
                         [&endpoint](const Clusters::JointFabricDatastore::Structs::DatastoreEndpointEntryStruct::Type & entry) {
                             return entry.endpointID == endpoint.id;
                         });
    ...
    for (auto it = mEndpointGroupIDEntries.begin(); it != mEndpointGroupIDEntries.end();)
    {
        if (it->nodeID == nodeId)
        {
            ...
            {
                // TODO: update the endpoint group list on the actual device/node
                // If the node is pending, we need to update the status entry to Committed
            ...
            {
                // TODO: check if this matches the endpoint group list on the actual device
                // TODO: If removal from the device actually succeeds, then we need to remove the entry
            ...
            else if (it->statusEntry.state == Clusters::JointFabricDatastore::DatastoreStateEnum::kCommitFailed)
            {
                ...
                if (failureCode == CHIP_ERROR_CONSTRAINT || failureCode == CHIP_ERROR_RESOURCE_EXHAUSTED)
                {
                ...
                {
                    // in this case we are leaving for a subsequent NodeRefresh
                }
            }
        }
    ...
    for (auto it = mEndpointBindingEntries.begin(); it != mEndpointBindingEntries.end();)
    {
        if (it->nodeID == nodeId)
        {
            ...
            {
                // If the node is pending, we need to update the status entry to Committed
                // TODO: update the endpoint binding group list on the actual device/node
                it->statusEntry.state = Clusters::JointFabricDatastore::DatastoreStateEnum::kCommitted;
            }
            else if (it->statusEntry.state == Clusters::JointFabricDatastore::DatastoreStateEnum::kDeletePending)
            {
                // Remove the binding from the actual device. If successful, then remove the entry.
                // If the node is delete pending, we need to remove the entry
                // TODO: If removal from the device actually succeeds, then we need to remove the entry
                // otherwise update status to CommitFailed and return error
...
CHIP_ERROR JointFabricDatastore::RemoveKeySet(uint16_t groupKeySetId)
{
    for (auto it = mNodeKeySetEntries.begin(); it != mNodeKeySetEntries.end(); ++it)
    {
        if (it->groupKeySetID == groupKeySetId)
        {
            ...
            {
                return CHIP_ERROR_CONSTRAINT; // Cannot remove a key set that is not pending
            }
...
CHIP_ERROR JointFabricDatastore::AddGroup(const Clusters::JointFabricDatastore::Commands::AddGroup::DecodableType & commandData)
{
    ...
    VerifyOrReturnError(IsGroupIDInDatastore(commandData.groupID, index) == CHIP_ERROR_NOT_FOUND, CHIP_ERROR_CONSTRAINT);
...
JointFabricDatastore::UpdateGroup(const Clusters::JointFabricDatastore::Commands::UpdateGroup::DecodableType & commandData)
{
    ...
    VerifyOrReturnError(IsGroupIDInDatastore(commandData.groupID, index) == CHIP_NO_ERROR, CHIP_ERROR_CONSTRAINT);
    ...
    if (commandData.groupKeySetID.IsNull() == false)
    {
        if (mGroupInformationEntries[index].groupKeySetID != commandData.groupKeySetID.Value())
        {
            ...
            ReturnErrorOnFailure(AddOrUpdateNodeKeySetEntry(commandData.groupKeySetID.Value()));
            ReturnErrorOnFailure(
                RemoveNodeKeySetEntry(mGroupInformationEntries[index].groupKeySetID.Value())); // Remove the old key set
        }
...
CHIP_ERROR JointFabricDatastore::AddGroupIDToEndpointForNode(NodeId nodeId, chip::EndpointId endpointId, chip::GroupId groupId)
{
    ...
    VerifyOrReturnError(IsGroupIDInDatastore(groupId, index) == CHIP_ERROR_NOT_FOUND, CHIP_ERROR_CONSTRAINT);
...
CHIP_ERROR JointFabricDatastore::AddOrUpdateNodeKeySetEntry(uint16_t groupKeySetId)
{
    for (auto & entry : mNodeKeySetEntries)
    {
        if (entry.groupKeySetID == groupKeySetId)
        {
            // TODO: mark entry as pending, then actually update the key set entry on the node, then if successful, mark as
            // committed. If the key set already exists, just update the state to committed
            entry.statusEntry.state = Clusters::JointFabricDatastore::DatastoreStateEnum::kCommitted;
            return CHIP_NO_ERROR;
        }
    }
...
CHIP_ERROR JointFabricDatastore::RemoveNodeKeySetEntry(uint16_t groupKeySetId)
{
    // TODO: make sure there are no nodes using this keyset. If there are (and which are not marked deletePending), then command
    // should fail.
