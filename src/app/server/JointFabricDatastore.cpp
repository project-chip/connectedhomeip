...
CHIP_ERROR JointFabricDatastore::RemoveNodeKeySetEntry(GroupId groupId, uint16_t groupKeySetId)
{
    ...
    {
        // Using groupId as nodeId temporarily, will be updated when iterating through nodes
        if (it->nodeID == groupId && it->groupKeySetID == groupKeySetId)
        {
            any_node_removed = true;

            it->statusEntry.state = Clusters::JointFabricDatastore::DatastoreStateEnum::kDeletePending;

            // TODO: Update device: send command to node to remove this keyset
