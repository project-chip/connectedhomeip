...
public:
     ...
     *
     * The caller can optionally provide `onConnection`, `onFailure`, and `onExtendedConnectionFailure`
     ...
     *
     * The `onFailure` or `onExtendedConnectionFailure` callback may be called before the `FindOrEstablishSession`
     * call returns, for error cases that are detected synchronously.
     *
     ...
     * @param onExtendedConnectionFailure A callback to be called upon an extended device connection failure.
     *
     * @note Only one of `onFailure` and `onExtendedConnectionFailure` should be provided and not both.
     *
