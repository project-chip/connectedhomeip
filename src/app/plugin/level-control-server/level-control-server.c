...
#define retrieveSceneSubTableEntry(entry, i) \
  (entry = zapPluginLevelControlServerSceneSubTable[i])
#define saveSceneSubTableEntry(entry, i) \
  (zapPluginLevelControlServerSceneSubTable[i] = entry)
#endif
#endif

int abs(int I);
static void moveToLevelHandler(const ZapCommandContext_t *context,
                               const ZapClusterLevelControlServerCommandMoveToLevelRequest_t *request,
                               bool withOnOff);
static void moveHandler(const ZapCommandContext_t *context,
                        const ZapClusterLevelControlServerCommandMoveRequest_t *request,
                        bool withOnOff);
static void stepHandler(const ZapCommandContext_t *context,
                        const ZapClusterLevelControlServerCommandStepRequest_t *request,
                        bool withOnOff);
static void stopHandler(const ZapCommandContext_t *context,
...
static void setCurrentLevel(ZapEndpointId_t endpointId,
...
void zapLevelControlServerSetOnOff(ZapEndpointId_t endpointId,
                                        bool value)
{
  State state = {
...
void zapClusterLevelControlServerCommandMoveToLevelRequestHandler(const ZapCommandContext_t *context,
                                                                  const ZapClusterLevelControlServerCommandMoveToLevelRequest_t *request)
{
...
void zapClusterLevelControlServerCommandMoveToLevelWithOnOffRequestHandler(const ZapCommandContext_t *context,
                                                                           const ZapClusterLevelControlServerCommandMoveToLevelWithOnOffRequest_t *request)
{
...
void zapClusterLevelControlServerCommandMoveWithOnOffRequestHandler(const ZapCommandContext_t *context,
                                                                    const ZapClusterLevelControlServerCommandMoveWithOnOffRequest_t *request)
{
  moveHandler(context,
              (const ZapClusterLevelControlServerCommandMoveRequest_t *)request,
              true); // with on/off
...
void zapClusterLevelControlServerCommandStepWithOnOffRequestHandler(const ZapCommandContext_t *context,
                                                                    const ZapClusterLevelControlServerCommandStepWithOnOffRequest_t *request)
{
  stepHandler(context,
              (const ZapClusterLevelControlServerCommandStepRequest_t *)request,
              true); // with on/off
...
void zapClusterLevelControlServerCommandStopWithOnOffRequestHandler(const ZapCommandContext_t *context,
                                                                    const ZapClusterLevelControlServerCommandStopWithOnOffRequest_t *request)
{
  stopHandler(context,
              (const ZapClusterLevelControlServerCommandStopRequest_t *)request,
              true); // with on/off
...
{
  bool onOff;
  if (zapReadAttribute(endpointId,
                       &zapClusterOnOffServerSpec,
                       ZAP_CLUSTER_ON_OFF_SERVER_ATTRIBUTE_ON_OFF,
                       &onOff,
...
static uint8_t getCurrentLevel(ZapEndpointId_t endpointId)
{
  uint8_t currentLevel;
  if (zapReadAttribute(endpointId,
                       &zapClusterLevelControlServerSpec,
                       ZAP_CLUSTER_LEVEL_CONTROL_SERVER_ATTRIBUTE_CURRENT_LEVEL,
                       &currentLevel,
                       sizeof(currentLevel)) == ZAP_STATUS_SUCCESS)
  {
    return currentLevel;
  }
  else
...
static uint16_t getOnOffTransitionTime(ZapEndpointId_t endpointId)
{
  ...
  if (zapReadAttribute(endpointId,
                       &zapClusterLevelControlServerSpec,
                       ZAP_CLUSTER_LEVEL_CONTROL_SERVER_ATTRIBUTE_ON_OFF_TRANSITION_TIME,
                       &onOffTransitionTime,
...
static uint8_t getDefaultMoveRate(ZapEndpointId_t endpointId)
{
  ...
  if (zapReadAttribute(endpointId,
                       &zapClusterLevelControlServerSpec,
                       ZAP_CLUSTER_LEVEL_CONTROL_SERVER_ATTRIBUTE_DEFAULT_MOVE_RATE,
                       &defaultMoveRate,
                               ...
                               uint8_t optionOverride)
{
  ...
  uint8_t options;
  if (zapReadAttribute(endpoint,
                       &zapClusterLevelControlServerSpec,
                       ZAP_CLUSTER_LEVEL_CONTROL_SERVER_ATTRIBUTE_OPTIONS,
                       &options,
...
static uint8_t getOnLevelOrCurrentLevel(ZapEndpointId_t endpointId)
{
  ...
  if ((zapReadAttribute(endpointId,
                        &zapClusterLevelControlServerSpec,
                        ZAP_CLUSTER_LEVEL_CONTROL_SERVER_ATTRIBUTE_ON_LEVEL,
                        &onLevel,
                        sizeof(onLevel)) == ZAP_STATUS_SUCCESS) &&
      onLevel != 0xFF)
  {
    return onLevel;
  }
...
static LevelControlEvent *cancel(State *state)
{
  return (LevelControlEvent *)zapFindEvent(actions.queue,
                                           &actions,
...
bool zapLevelControlServerAddSceneHandler(ZapClusterId_t clusterId,
                                            uint8_t tableIdx,
...
void zapLevelControlServerRecallSceneHandler(ZapEndpointId_t endpointId,
                                               uint8_t tableIdx,
...
void zapLevelControlServerStoreSceneHandler(ZapEndpointId_t endpointId,
                                              uint8_t tableIdx)
{
  ZapLevelControlSceneSubTableEntry_t entry;

  entry.hasCurrentLevelValue =
      (zapReadAttribute(endpointId,
                        &zapClusterLevelControlServerSpec,
                        ZAP_CLUSTER_LEVEL_CONTROL_SERVER_ATTRIBUTE_CURRENT_LEVEL,
                        (uint8_t *)&entry.currentLevelValue,
                        sizeof(entry.currentLevelValue)) == ZAP_STATUS_SUCCESS);
...
void zapLevelControlServerViewSceneHandler(uint8_t tableIdx,
                                             uint8_t **ppExtFldData)
{
  ...
  if (entry.hasCurrentLevelValue)
  {
    zapPluginScenesServerPutUint16InBuffer(ppExtFldData,
                                                ZAP_CLUSTER_LEVEL_CONTROL);
    zapPluginScenesServerPutUint8InBuffer(ppExtFldData, 1); // length=1
    zapPluginScenesServerPutUint8InBuffer(ppExtFldData,
