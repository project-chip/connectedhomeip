...
#define retrieveSceneSubTableEntry(entry, i) \
  (entry = zapPluginOnOffServerSceneSubTable[i])
#define saveSceneSubTableEntry(entry, i) \
  (zapPluginOnOffServerSceneSubTable[i] = entry)
#endif
#endif
...
bool zapOnOffServerAddSceneHandler(ZapClusterId_t clusterId,
                                     uint8_t tableIdx,
...
void zapOnOffServerRecallSceneHandler(ZapEndpointId_t endpointId,
                                        uint8_t tableIdx,
...
void zapOnOffServerStoreSceneHandler(ZapEndpointId_t endpointId,
                                       uint8_t tableIdx)
{
  ZapOnOffSceneSubTableEntry_t entry;

  entry.hasOnOffValue = (zapReadAttribute(endpointId,
                                          &zapClusterOnOffServerSpec,
                                          ZAP_CLUSTER_ON_OFF_SERVER_ATTRIBUTE_ON_OFF,
                                          (uint8_t *)&entry.onOffValue,
                                          sizeof(entry.onOffValue)) == ZAP_STATUS_SUCCESS);
