...
ChipZclStatus_t emAfReadOrWriteAttribute(ChipZclAttributeSearchRecord *attRecord,
...
static ChipZclStatus_t typeSensitiveMemCopy(uint8_t* dest,
                                          uint8_t* src,
                                          ChipZclAttributeMetadata * am,
                                          bool write,
                                          uint16_t readLength)
{
...
// could be found.
static uint16_t getManufacturerCode(ChipZclManufacturerCodeEntry *codes,
...
{
  return getManufacturerCode((ChipZclManufacturerCodeEntry *)clusterManufacturerCodes,
 ...
 */
bool chipZclMatchCluster(ChipZclCluster *cluster,
                      ChipZclAttributeSearchRecord *attRecord)
{
  return (cluster->clusterId == attRecord->clusterId
          ...
          && (!chipZclClusterIsManufacturerSpecific(cluster)
              || (chipZclGetManufacturerCodeForCluster(cluster)
...
{
  return getManufacturerCode((ChipZclManufacturerCodeEntry *) attributeManufacturerCodes,
                             attributeManufacturerCodeCount,
                             (metadata - generatedAttributes));
}


uint16_t chipZclGetManufacturerCodeForAttribute(ChipZclCluster *cluster,
                                             ChipZclAttributeMetadata *attMetaData)
{
  return (chipZclClusterIsManufacturerSpecific(cluster)
          ? chipZclGetManufacturerCodeForCluster(cluster)
...
void chipZclSaveAttributeToToken(uint8_t *data,
                              uint8_t endpoint,
...
ChipZclStatus_t emAfWriteAttribute(uint8_t endpoint,
                                 ChipZclClusterId cluster,
                                 ...
                                 bool justTest)
{
  ...
  if (metadata == NULL) {
    chipZclCorePrintln("%pep %x clus %2x attr %2x not supported",
                             "WRITE ERR: ",
                             endpoint,
                             cluster,
                             attributeID);
  ...
  if (!justTest) {
    ...
    ChipZclStatus_t status
      = chipZclPreAttributeChangeCallback(endpoint,
                                          cluster,
                                          attributeID,
                                          mask,
                                          manufacturerCode,
                                          dataType,
                                          chipZclAttributeSize(metadata),
                                          data);
    ...
    // of cluster.
    chipZclPostAttributeChangeCallback(endpoint,
                                       cluster,
                                       attributeID,
                                       mask,
                                       manufacturerCode,
 ...
 */
bool chipZclMatchAttribute(ChipZclCluster *cluster,
                        ChipZclAttributeMetadata *am,
                        ChipZclAttributeSearchRecord *attRecord)
{
...
ChipZclStatus_t emAfReadAttribute(uint8_t endpoint,
                                ChipZclClusterId cluster,
                                ChipZclAttributeId attributeID,
                                uint8_t mask,
                                uint16_t manufacturerCode,
  ...
  for (i = 0; i < chipZclEndpointCount(); i++) {
    if (emAfEndpoints[i].endpoint == attRecord->endpoint) {
      ...
      for (clusterIndex = 0;
        ...
        if (chipZclMatchCluster(cluster, attRecord)) { // Got the cluster
          uint16_t attrIndex;
          for (attrIndex = 0;
               attrIndex < cluster->attributeCount;
               attrIndex++) {
            ChipZclAttributeMetadata *am = &(cluster->attributes[attrIndex]);
            if (chipZclMatchAttribute(cluster,
                                   am,
...

              {
                ...
                if (write) {
                  ...
                  if (!chipZclAttributeWriteAccessCallback(attRecord->endpoint,
                                                           attRecord->clusterId,
                                                           chipZclGetManufacturerCodeForAttribute(cluster, am),
                                                           am->attributeId)) {
                  ...
                  dst = buffer;
                  if (!chipZclAttributeReadAccessCallback(attRecord->endpoint,
                                                          attRecord->clusterId,
                                                          chipZclGetManufacturerCodeForAttribute(cluster, am),
                                                          am->attributeId)) {
                    return CHIP_ZCL_STATUS_NOT_AUTHORIZED;
                ...
                return (am->mask & ATTRIBUTE_MASK_EXTERNAL_STORAGE
                        ? (write)
                        ? chipZclExternalAttributeWriteCallback(attRecord->endpoint,
                                           attRecord->clusterId,
                                           am,
                                           chipZclGetManufacturerCodeForAttribute(cluster, am),
                                           buffer)
                        : chipZclExternalAttributeReadCallback(attRecord->endpoint,
                                          attRecord->clusterId,
                                          am,
                                          chipZclGetManufacturerCodeForAttribute(cluster, am),
                                          buffer,
