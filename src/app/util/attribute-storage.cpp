...
void emberAfEndpointConfigure(void)
{
    ...
    if (MAX_ENDPOINT_COUNT > FIXED_ENDPOINT_COUNT)
    {
        ...
        {
            emAfEndpoints[ep].endpoint = EMBER_AF_INVALID_ENDPOINT;
        }
    }
#endif
}
...
EndpointId emberAfClearDynamicEndpoint(uint16_t index)
{
    ...
    if ((index < MAX_ENDPOINT_COUNT) && (emAfEndpoints[index].endpoint != EMBER_AF_INVALID_ENDPOINT) &&
        (emberAfEndpointIndexIsEnabled(index)))
    {
        ...
        emAfEndpoints[index].endpoint = EMBER_AF_INVALID_ENDPOINT;
    }
