...
void emAfClusterAttributeChangedCallback(const ConcreteAttributePath & attributePath)
{
    ...
    if (cluster != nullptr)
    {
        EmberAfGenericClusterFunction f = emberAfFindClusterFunction(cluster, MATTER_CLUSTER_BITMAP_ATTRIBUTE_CHANGED_FUNCTION);
        if (f != nullptr)
        {
                                              ...
                                              uint16_t size, uint8_t * value)
{
    ...
    EmberAfClusterPreAttributeChangedCallback f = (EmberAfClusterPreAttributeChangedCallback) (emberAfFindClusterFunction(
        cluster, MATTER_CLUSTER_BITMAP_PRE_ATTRIBUTE_CHANGED_FUNCTION));
    if (f != nullptr)
    {
...
static void initializeEndpoint(EmberAfDefinedEndpoint * definedEndpoint)
{
    ...
    for (clusterIndex = 0; clusterIndex < epType->clusterCount; clusterIndex++)
    {
        ...
        f = emberAfFindClusterFunction(cluster, MATTER_CLUSTER_BITMAP_INIT_FUNCTION);
        if (f != nullptr)
        {
...
static void shutdownEndpoint(EmberAfDefinedEndpoint * definedEndpoint)
{
    ...
    {
        const EmberAfCluster * cluster  = &(epType->cluster[clusterIndex]);
        EmberAfGenericClusterFunction f = emberAfFindClusterFunction(cluster, MATTER_CLUSTER_BITMAP_SHUTDOWN_FUNCTION);
        if (f != nullptr)
        {
...
{
    return (cluster->clusterId == attRecord->clusterId && (cluster->mask & MATTER_CLUSTER_BITMAP_SERVER));
}
                ...
                { // Got the cluster
                    ...
                    for (attrIndex = 0; attrIndex < cluster->attributeCount; attrIndex++)
                    {
                        ...
                        { // Got the attribute
...

                            {
                                uint8_t * attributeLocation =
                                    (am->mask & MATTER_ATTRIBUTE_BITMAP_SINGLETON ? singletonAttributeLocation(am)
                                ...
                                if (am->mask & MATTER_ATTRIBUTE_BITMAP_EXTERNAL_STORAGE)
                                {
                        ...
                        { // Not the attribute we are looking for
                            ...
                            if (!(am->mask & MATTER_ATTRIBUTE_BITMAP_EXTERNAL_STORAGE) &&
                                !(am->mask & MATTER_ATTRIBUTE_BITMAP_SINGLETON))
                            {
...
bool emberAfContainsClient(EndpointId endpoint, ClusterId clusterId)
{
    ...
    return (emberAfFindClusterInType(emAfEndpoints[ep].endpointType, clusterId, MATTER_CLUSTER_BITMAP_CLIENT) != nullptr);
}
...
bool emberAfContainsServerFromIndex(uint16_t index, ClusterId clusterId)
{
    ...
    return emberAfFindClusterInType(emAfEndpoints[index].endpointType, clusterId, MATTER_CLUSTER_BITMAP_SERVER);
}
...
const EmberAfCluster * emberAfFindServerCluster(EndpointId endpoint, ClusterId clusterId)
{
    ...
    return emberAfFindClusterInType(emAfEndpoints[ep].endpointType, clusterId, MATTER_CLUSTER_BITMAP_SERVER);
}
...
uint16_t emberAfGetClusterServerEndpointIndex(EndpointId endpoint, ClusterId cluster, uint16_t fixedClusterServerEndpointCount)
{
    ...
    if (emberAfFindClusterInType(emAfEndpoints[epIndex].endpointType, cluster, MATTER_CLUSTER_BITMAP_SERVER) == nullptr)
    {
    ...
    if (epIndex < FIXED_ENDPOINT_COUNT)
    {
        ...
        for (uint16_t i = 0; i < epIndex; i++)
        {
            ...
            if (emAfEndpoints[i].endpoint != kInvalidEndpointId &&
                (emberAfFindClusterInType(emAfEndpoints[i].endpointType, cluster, MATTER_CLUSTER_BITMAP_SERVER) != nullptr))
            {
            ...
            for (attr = 0; attr < cluster->attributeCount; attr++)
            {
                ...
                if (!am->IsExternal())
                {
                    ...
                    if (ptr == nullptr)
                    {
                        ...
                        if ((am->mask & MATTER_ATTRIBUTE_BITMAP_MIN_MAX) != 0U)
                        {
...
DataVersion * emberAfDataVersionStorage(const ConcreteClusterPath & aConcreteClusterPath)
{
    ...
    auto clusterIndex =
        emberAfClusterIndex(aConcreteClusterPath.mEndpointId, aConcreteClusterPath.mClusterId, MATTER_CLUSTER_BITMAP_SERVER);
