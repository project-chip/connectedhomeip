...
EmberStatus emberAfSendMulticastToBindings(EmberApsFrame * apsFrame, uint16_t messageLength, uint8_t * message)
{
    ...
    for (i = 0; i < EMBER_BINDING_TABLE_SIZE; i++)
    {
        ...
        {
            groupDest                     = (binding.identifier[0] + (((uint16_t)(binding.identifier[1])) << 8));
...
EmberStatus emberAfPermitJoin(uint8_t duration, bool broadcastMgmtPermitJoin)
{
    ...
    return emAfPermitJoin(duration, broadcastMgmtPermitJoin);
}
...
bool emAfProcessZdo(EmberNodeId sender, EmberApsFrame * apsFrame, uint8_t * message, uint16_t length)
{
    ...
    emberAfZdoPrintln("RX: ZDO, command 0x%2x, status: 0x%X", apsFrame->clusterId,
                      (apsFrame->clusterId >= CLUSTER_ID_RESPONSE_MINIMUM ? message[1] : 0));
    ...
    case NODE_DESCRIPTOR_RESPONSE:
        ...
        emberAfZdoPrintln(", Matches: 0x%2x", (uint16_t)(message[1]) + (uint16_t)(message[2] << 8));
        break;
    default:
        break;
    }
                                ...
                                uint16_t messageLength, uint8_t * messageContents)
{
...
#ifdef EMBER_AF_PLUGIN_FRAGMENTATION
    if (emAfFragmentationIncomingMessage(type, apsFrame, sender, &messageContents, &messageLength))
    {
...
#ifdef EMBER_AF_PLUGIN_SUB_GHZ_SERVER
    if (emAfSubGhzServerIncomingMessage(type, apsFrame, sender, messageLength, messageContents))
    {
    ...
    emberAfDebugPrintln("Processing message: len=%d profile=%2x cluster=%2x", messageLength, apsFrame->profileId,
                        apsFrame->clusterId);
    emberAfDebugFlush();
    ...
    if (emAfServiceDiscoveryIncoming(sender, apsFrame, messageContents, messageLength))
    {
        return;
    }
    ...
    // Handle ZCL messages.
    if (emberAfProcessMessage(apsFrame, type, messageContents, messageLength, sender, NULL))
    { // no inter-pan header
        return;
    }
}
...
static void printMessage(EmberIncomingMessageType type, EmberApsFrame * apsFrame, uint16_t messageLength, uint8_t * messageContents)
{
    emberAfAppPrint("Profile: %p (0x%2X), Cluster: 0x%2X, %d bytes,",
                    (apsFrame->profileId == EMBER_ZDO_PROFILE_ID
                         ? "ZDO"
                         : (apsFrame->profileId == SE_PROFILE_ID ? "SE" : (apsFrame->profileId == 0x0104 ? "HA" : "??"))),
                    apsFrame->profileId, apsFrame->clusterId, messageLength);
    if (apsFrame->profileId != EMBER_ZDO_PROFILE_ID && messageLength >= 3)
    {
                            ...
                            EmberStatus status, uint16_t messageLength, uint8_t * messageContents, uint8_t messageTag)
{
    ...
    if (status == EMBER_SUCCESS && apsFrame->profileId == EMBER_ZDO_PROFILE_ID && apsFrame->clusterId < CLUSTER_ID_RESPONSE_MINIMUM)
    {
...
void emAfStackStatusHandler(EmberStatus status)
{
    ...
    case EMBER_TRUST_CENTER_EUI_HAS_CHANGED: // also means NETWORK_UP
    {
        ...
        if (data != NULL)
        {
            newExtended = (trustCenter ? data->tcExtendedBitmask : data->nodeExtendedBitmask);
        }
        ...
        if (emAfProIsCurrentNetwork())
        {
            ...
            if (emberAfGetNodeType(&nodeType) == EMBER_SUCCESS &&
                (nodeType != emAfCurrentZigbeeProNetwork->nodeType &&
                 (emAfCurrentZigbeeProNetwork->nodeType != EMBER_COORDINATOR || nodeType != EMBER_ROUTER)))
            {
                initialize = false;
            }
        }
...
EmberStatus emberAfFormNetwork(EmberNetworkParameters * parameters)
{
    ...
    if (emAfProIsCurrentNetwork() && emAfCurrentZigbeeProNetwork->nodeType == EMBER_COORDINATOR)
    {
        zaTrustCenterSecurityInit(true); // centralized network
        emberAfCorePrintln("%ping on ch %d, panId 0x%2X", "Form", parameters->radioChannel, parameters->panId);
        emberAfCoreFlush();
...
EmberStatus emberAfJoinNetwork(EmberNetworkParameters * parameters)
{
    ...
    if (emAfProIsCurrentNetwork())
    {
        ...
        emberAfCorePrintln("%ping on ch %d, panId 0x%2X", "Join", parameters->radioChannel, parameters->panId);
...
static const EmberReleaseTypeStruct releaseTypes[] = { EMBER_RELEASE_TYPE_TO_STRING_STRUCT_DATA };
...
void emAfParseAndPrintVersion(EmberVersion versionStruct)
{
    ...
    emberAfAppPrint("stack ver. [%d.%d.%d", versionStruct.major, versionStruct.minor, versionStruct.patch);
    if (versionStruct.special != 0)
    {
        emberAfAppPrint(".%d", versionStruct.special);
    }
    UNUSED_VAR(typeText);
    emberAfAppPrintln(" %p build %d]", (typeText == NULL ? "???" : typeText), versionStruct.build);
    emberAfAppFlush();
}
