...
constexpr uint32_t kImMesssageTimeoutMsec = 3;
constexpr uint32_t kMinEventBoundarySize  = 20;
...
public:
    ...
    /**
     *  zcl/app-layer code retrieve one idle command sender from interaction model engine, and manage lifecycle in upper layer
     *
     *  @param[out]    apCommandSender    A pointer to the ExchangeManager object.
     *
     *  @retval #CHIP_ERROR_INCORRECT_STATE If the command send fails to initialize
     *  @retval #CHIP_NO_ERROR On success.
     */
    CHIP_ERROR NewCommandSender(CommandSender ** const apCommandSender);

    /**
     *  zcl/app-layer code retrieve one read client from interaction model engine, and manage lifecycle in upper layer
     *
     *  @param[out]    apCommandSender    A pointer to the ExchangeManager object.
     *
     *  @retval #CHIP_ERROR_INCORRECT_STATE If the command send fails to initialize
    ...
    /**
     *  Get current read client index in mReadClients
     *
     *  @param[in]    apReadClient    A pointer to the current read client object.
     *
...
private:
    ...
    /**
     *  Called when Interaction Model receive one read request, and picked up one uninitialized read handle for read request
     * processing If error happens during read request processing inside read handler, read handle would handle error close reader
     * handler accordingly.
     */
