...
CHIP_ERROR DeviceProxy::SendWriteAttributeRequest(app::WriteClient * writeClient)
{
    VerifyOrReturnLogError(IsSecureConnected(), CHIP_ERROR_INCORRECT_STATE);
    VerifyOrReturnError(writeClient != nullptr, CHIP_ERROR_INVALID_ARGUMENT);
    return writeClient->SendWriteRequest(GetSecureSession().Value());
}
