                                                       ...
                                                       MutableByteSpan & buffer)
{
    ...
    for (auto attr : attributes)
    {
        ...
        {
            ChipLogError(NotSpecified, "Migration Error - Failed %s path %x/%x/%lx - %" CHIP_ERROR_FORMAT, "Reading",
        ...
        if (err != CHIP_NO_ERROR)
        {
            ChipLogError(NotSpecified, "Migration Error - Failed %s path %x/%x/%lx - %" CHIP_ERROR_FORMAT, "Writing",
                         cluster.mEndpointId, cluster.mClusterId, attr, err.Format());
            finalError = err; // make sure we report an error if any element fails
        }
        ...
        if (err != CHIP_NO_ERROR)
        {
            ChipLogError(NotSpecified, "Migration Error - Failed %s path %x/%x/%lx - %" CHIP_ERROR_FORMAT, "Deleting",
                         cluster.mEndpointId, cluster.mClusterId, attr, err.Format());
            finalError = err; // make sure we report an error if any element fails
        }
    }
