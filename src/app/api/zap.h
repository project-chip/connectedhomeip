 ...
 */
#define zapFindEvent(queue, actions, predicate, data) \
  (zapEventFind((queue), (actions), (predicate), (data), false))
...
#define zapFindAllEvents(queue, actions, predicate, data) \
  (zapEventFind((queue), (actions), (predicate), (data), true))
...
#define zapAllocateBuffer(dataSizeInBytes) \
  zapReallyAllocateBuffer(dataSizeInBytes, false)
