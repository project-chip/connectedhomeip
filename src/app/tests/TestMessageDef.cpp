...
void BuildSubscribeRequest(nlTestSuite * apSuite, chip::TLV::TLVWriter & aWriter)
{
    ...
    subscribeRequestBuilder.MinInterval(1);
    NL_TEST_ASSERT(apSuite, subscribeRequestBuilder.GetError() == CHIP_NO_ERROR);

    subscribeRequestBuilder.MaxInterval(1);
    NL_TEST_ASSERT(apSuite, subscribeRequestBuilder.GetError() == CHIP_NO_ERROR);
...
void ParseSubscribeRequest(nlTestSuite * apSuite, chip::TLV::TLVReader & aReader)
{
    ...
    err = subscribeRequestParser.GetMinInterval(&minInterval);
    NL_TEST_ASSERT(apSuite, minInterval == 1 && err == CHIP_NO_ERROR);

    err = subscribeRequestParser.GetMaxInterval(&maxInterval);
    NL_TEST_ASSERT(apSuite, maxInterval == 1 && err == CHIP_NO_ERROR);
...
void BuildSubscribeResponse(nlTestSuite * apSuite, chip::TLV::TLVWriter & aWriter)
{
    ...
    subscribeResponseBuilder.FinalSyncInterval(1);
    NL_TEST_ASSERT(apSuite, subscribeResponseBuilder.GetError() == CHIP_NO_ERROR);
...
void ParseSubscribeResponse(nlTestSuite * apSuite, chip::TLV::TLVReader & aReader)
{
    ...
    err = subscribeResponseParser.GetFinalSyncInterval(&finalSyncInterval);
    NL_TEST_ASSERT(apSuite, finalSyncInterval == 1 && err == CHIP_NO_ERROR);
}
