...
void ParseStatusElement(nlTestSuite * apSuite, StatusElement::Parser & aStatusElementParser)
{
    ...
    NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR && generalCode == 1 && protocolId == 2 && protocolCode == 3 && namespacedClusterId == 4);
}
...
void BuildAttributeDataElement(nlTestSuite * apSuite, AttributeDataElement::Builder &aAttributeDataElementBuilder)
{
    ...
    // Construct attribute data
    {
        ...
        chip::TLV::TLVType dummyType   = chip::TLV::kTLVType_NotSpecified;
        err =
                pWriter->StartContainer(chip::TLV::ContextTag(AttributeDataElement::kCsTag_Data), chip::TLV::kTLVType_Structure, dummyType);
        NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);
...
void BuildCommandDataElement(nlTestSuite * apSuite, CommandDataElement::Builder &aCommandDataElementBuilder)
{
    ...
    // Construct command data
    {
        ...
        chip::TLV::TLVType dummyType   = chip::TLV::kTLVType_NotSpecified;
        err =
                pWriter->StartContainer(chip::TLV::ContextTag(CommandDataElement::kCsTag_Data), chip::TLV::kTLVType_Structure, dummyType);
        NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);
...
void BuildInvokeCommandRequest(nlTestSuite * apSuite, chip::TLV::TLVWriter & aWriter)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    InvokeCommandRequest::Builder invokeCommandRequestBuilder;

    err = invokeCommandRequestBuilder.Init(&aWriter);
    NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);

    CommandList::Builder commandList= invokeCommandRequestBuilder.CreateCommandListBuilder();
    NL_TEST_ASSERT(apSuite, invokeCommandRequestBuilder.GetError() == CHIP_NO_ERROR);
    BuildCommandList(apSuite, commandList);

    invokeCommandRequestBuilder.EndOfInvokeCommandRequest();
    NL_TEST_ASSERT(apSuite, invokeCommandRequestBuilder.GetError() == CHIP_NO_ERROR);
}

void ParseInvokeCommandRequest(nlTestSuite * apSuite, chip::TLV::TLVReader & aReader)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    InvokeCommandRequest::Parser invokeCommandRequestParser;
    CommandList::Parser commandListParser;

    err = invokeCommandRequestParser.Init(aReader);
    NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);

    err = invokeCommandRequestParser.CheckSchemaValidity();
    NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);

    err = invokeCommandRequestParser.GetCommandList(&commandListParser);
    NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);
}
...
void InvokeCommandRequestTest(nlTestSuite * apSuite, void * apContext)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    chip::TLV::TLVWriter writer;
    chip::TLV::TLVReader reader;
    ...
    writer.Init(buf);
    BuildInvokeCommandRequest(apSuite, writer);
    err = writer.Finalize();
    NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);
    ...
    err = reader.Next();
    NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);
    ParseInvokeCommandRequest(apSuite, reader);
...
const nlTest sTests[] =
        {
                ...
                NL_TEST_DEF("InvokeCommandRequestTest", InvokeCommandRequestTest),
                NL_TEST_SENTINEL()
        };
// clang-format on
} // namespace
