...
void TestStructGenericOperationalStateCopyConstructor(nlTestSuite * inSuite, void * inContext)
{
    ...
    NL_TEST_ASSERT(inSuite, desOperationalState.operationalStateID == to_underlying(ManufactureOperationalStateEnum::kRebooting));
    NL_TEST_ASSERT(inSuite, desOperationalState.operationalStateLabel.HasValue() == true);
    NL_TEST_ASSERT(inSuite, desOperationalState.operationalStateLabel.Value().size() == strlen(buffer));
    NL_TEST_ASSERT(inSuite, memcmp(const_cast<char *>(desOperationalState.operationalStateLabel.Value().data()), buffer, strlen(buffer)) == 0);
}
...
void TestStructGenericOperationalStateFuncSet(nlTestSuite * inSuite, void * inContext)
{
    ...
    for(size_t i = 0; i < kOperationalStateLabelMaxSize; i++)
    {
        buffer[i] = 1;
    }
    ...
    NL_TEST_ASSERT(inSuite, operationalState.operationalStateID == to_underlying(ManufactureOperationalStateEnum::kRebooting));
    NL_TEST_ASSERT(inSuite, operationalState.operationalStateLabel.HasValue() == true);
    NL_TEST_ASSERT(inSuite, operationalState.operationalStateLabel.Value().size() == sizeof(buffer2) - 1);
    NL_TEST_ASSERT(inSuite, memcmp(const_cast<char *>(operationalState.operationalStateLabel.Value().data()), buffer2, sizeof(buffer2) - 1) == 0);
...
const nlTest sTests[] = { NL_TEST_DEF("Test struct GenericOperationalState: constructor with only StateID", TestStructGenericOperationalStateConstructorWithOnlyStateID),
                          ...
                          NL_TEST_DEF("Test struct GenericOperationalState: copy constructor",
                                      TestStructGenericOperationalStateCopyConstructor),
