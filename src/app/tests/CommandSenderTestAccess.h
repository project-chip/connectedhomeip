...
public:
    CommandSenderTestAccess(app::CommandSender * aCommandSender) : pCommandSender(aCommandSender) {}

    CHIP_ERROR ProcessInvokeResponse(System::PacketBufferHandle && payload, bool & moreChunkedMessages)
    {
        return pCommandSender->ProcessInvokeResponse(std::move(payload), moreChunkedMessages);
    }

    void FlushNoCommandResponse() { pCommandSender->FlushNoCommandResponse(); }
    ...
    void MoveToStateAddedCommand() { pCommandSender->MoveToState(app::CommandSender::State::AddedCommand); }

private:
    app::CommandSender * pCommandSender = nullptr;
};
