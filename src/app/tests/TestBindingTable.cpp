...
void TestPersistentStorage(nlTestSuite * aSuite, void * aContext)
{
    ...
    testStorage.AddPoisonKey(DefaultStorageKeyAllocator::BindingTableEntry(4));
    ...
    testStorage.AddPoisonKey(DefaultStorageKeyAllocator::BindingTable());
    auto iter = table.begin();
    NL_TEST_ASSERT(aSuite, table.RemoveAt(iter) != CHIP_NO_ERROR);
    ...
    testStorage.AddPoisonKey(DefaultStorageKeyAllocator::BindingTableEntry(0));
    iter = table.begin();
    ++iter;
    NL_TEST_ASSERT(aSuite, table.RemoveAt(iter) != CHIP_NO_ERROR);
    VerifyTableSame(aSuite, table, expected);
