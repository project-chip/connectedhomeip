...
void TestPersistentStorage(nlTestSuite * aSuite, void * aContext)
{
    ...
    testStorage.AddPoisonKey(chip::DefaultStorageKeyAllocator::BindingTableEntry(4));
    ...
    testStorage.AddPoisonKey(chip::DefaultStorageKeyAllocator::BindingTable());
    auto iter = table.begin();
    NL_TEST_ASSERT(aSuite, table.RemoveAt(iter) != CHIP_NO_ERROR);
    ...
    testStorage.AddPoisonKey(chip::DefaultStorageKeyAllocator::BindingTableEntry(0));
    iter = table.begin();
    ++iter;
    NL_TEST_ASSERT(aSuite, table.RemoveAt(iter) != CHIP_NO_ERROR);
    VerifyTableSame(aSuite, table, expected);
