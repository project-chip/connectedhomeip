...
public:
    ...
    void OnResponse(const WriteClient * apWriteClient, const chip::app::ConcreteAttributePath & path) override
    {
        mOnSuccessCalled++;
    }
    void OnError(const WriteClient * apWriteClient, Protocols::InteractionModel::Status imStatus, CHIP_ERROR chipError) override
    {
        mOnErrorCalled++;
    }
...
void TestWriteInteraction::TestWriteClient(nlTestSuite * apSuite, void * apContext)
{
    ...
    err = writeClient.Init(&ctx.GetExchangeManager(), &callback, 0);
    NL_TEST_ASSERT(apSuite, err == CHIP_NO_ERROR);
