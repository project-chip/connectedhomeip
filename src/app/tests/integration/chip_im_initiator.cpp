...
class MockInteractionModelApp : public chip::app::InteractionModelDelegate, public ::chip::app::CommandSender::Delegate
{
public:
    ...
    {
        printf(
            "CommandResponseStatus with GeneralCode %d, ProtocolId %d, ProtocolCode %d, EndpointId %d, ClusterId %d, CommandId %d",
            static_cast<uint16_t>(aStatus.generalCode), aStatus.protocolId, aStatus.protocolCode, aPath.endpointId, aPath.clusterId,
            aPath.commandId);
        gLastCommandResult =
            (aStatus.generalCode == chip::Protocols::SecureChannel::GeneralStatusCode::kSuccess && aStatus.protocolCode == 0)
