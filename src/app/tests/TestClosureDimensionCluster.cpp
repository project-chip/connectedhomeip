...
public:
    ...
    static void TearDownTestSuite() { chip::DeviceLayer::SetSystemLayerForTesting(nullptr); }
};
...
{
public:
    MockedMatterContext(EndpointId endpoint) : MatterContext(endpoint) {}
...
TEST_F(TestClosureDimensionClusterLogic, TestConformanceValid)
{
    ...
    conformance = { .featureMap = 0, .supportsOverflow = true };
    EXPECT_FALSE(conformance.Valid());

    // LVL on, no optional stuff, should be valid
    conformance = { .featureMap = to_underlying(Feature::kRotation), .supportsOverflow = true };
    EXPECT_TRUE(conformance.Valid());

    // LVL on, no optional stuff, should be valid
    conformance = { .featureMap = to_underlying(Feature::kRotation), .supportsOverflow = false };
    EXPECT_FALSE(conformance.Valid());

    // LVL on, no optional stuff, should be valid
    conformance = { .featureMap = to_underlying(Feature::kMotionLatching), .supportsOverflow = true };
    EXPECT_TRUE(conformance.Valid());

    // LVL on, no optional stuff, should be valid
    conformance = { .featureMap = to_underlying(Feature::kMotionLatching), .supportsOverflow = false };
    EXPECT_TRUE(conformance.Valid());

    // LVL on, no optional stuff, should be valid
    conformance = { .featureMap = 66, .supportsOverflow = true };
    EXPECT_TRUE(conformance.Valid());

    // LVL on, no optional stuff, should be valid
    conformance = { .featureMap = 66, .supportsOverflow = false };
    EXPECT_FALSE(conformance.Valid());
}
...
TEST_F(TestClosureDimensionClusterLogic, TestDelegates)
{
    ...
    ClusterConformance conformanceLevel = { .featureMap = 255, .supportsOverflow = true };
...
TEST_F(TestClosureDimensionClusterLogic, TestSetAttributesUninitialized)
{
    ...
    GenericCurrentStateStruct currentState{ Optional<Percent100ths>(0), Optional<LatchingEnum>(LatchingEnum::kNotLatched),
                                            Optional<Globals::ThreeLevelAutoEnum>(Globals::ThreeLevelAutoEnum::kAuto) };
    GenericTargetStruct target{ Optional<Percent100ths>(0), Optional<TargetLatchEnum>(TargetLatchEnum::kUnlatch),
                                Optional<Globals::ThreeLevelAutoEnum>(Globals::ThreeLevelAutoEnum::kAuto) };
