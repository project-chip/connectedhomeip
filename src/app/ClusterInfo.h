...
struct ClusterInfo
{
    ...
    bool IsAttributePathSupersetOf(const ClusterInfo & other) const
    {
        ...
        if (mFlags.Has(Flags::kFieldIdValid) && !mFlags.Has(Flags::kListIndexValid) && other.mFlags.Has(Flags::kFieldIdValid) && (mFieldId == other.mFieldId))
        {
            return true;
        }

        if (mFlags.Has(Flags::kFieldIdValid) && mFlags.Has(Flags::kListIndexValid)
        && other.mFlags.Has(Flags::kListIndexValid) && other.mFlags.Has(Flags::kListIndexValid)
        && (mFieldId == other.mFieldId) && (mListIndex == other.mListIndex))
        {
            return true;
        }
        return false;
