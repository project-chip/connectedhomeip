                                     ...
                                     const LogStorageResources * const apLogStorageResources)
{
    ...
    for (j = 0; j < aNumBuffers; j++)
    {
            ...
            (apLogStorageResources[i].mCounterEpoch != 0))
        {
            ...
            current->mpEventNumberCounter = apLogStorageResources[i].mCounterStorage;
        }
        else
        {
...
LoggingManagement::LoggingManagement(void) :
    mpEventBuffer(NULL), mpExchangeMgr(NULL), mState(kLoggingManagementState_Idle), mBytesWritten(0),
...
{
    const LoggingConfiguration & config = LoggingConfiguration::GetInstance();
    return (config.GetGlobalPriorityLevel() < mMaxPriorityBuffer ? config.GetGlobalPriorityLevel() : mMaxPriorityBuffer);
}
                                              ...
                                              const EventOptions * apOptions)
{
    ...
    event_number = LogEventPrivate(aSchema, aEventWriter, apAppData, apOptions);
...
exit:
    ...
    {
        event_number = GetPriorityBuffer(aSchema.mPriority)->VendEventNumber();
                                                 ...
                                                 size_t & aBytesOffloaded)
{
    ...
    while (eventBuffer != NULL && aNumPriorityLevels > 0)
    {
        ...
        {
            apEventEndpoints[eventBuffer->mPriority - kPriorityLevel_First] = eventBuffer->mLastEventNumber;
        }
        eventBuffer = eventBuffer->mpNext;
    }
