...
CHIP_ERROR LoggingManagement::BlitEvent(EventLoadOutContext * apContext, EventLoggingDelegate * apDelegate, const EventOptions * apOptions)
{
    ...
    VerifyOrExit(apOptions->mTimestampType != TimestampType::Invalid, err = CHIP_ERROR_INVALID_ARGUMENT);
    ...
    if (apContext->mFirst)
    {
        ...
        eventDataElementBuilder.SystemTimestamp(apOptions->mTimestamp.systemTimestamp);
    }
    // else record delta
    else
    {
        uint32_t deltatime = apOptions->mTimestamp.systemTimestamp - apContext->mCurrentTime;
...
exit:
    ...
    else
    {
        ...
        {
            apContext->mCurrentTime = apOptions->mTimestamp.systemTimestamp;
        }
    }
    return err;
}
                                              ...
                                              chip::EventNumber &aEventNumber)
{
    ...
    if ((apOptions != nullptr) && (apOptions->mTimestampType == TimestampType::System))
    {
        opts.mTimestamp.systemTimestamp = apOptions->mTimestamp.systemTimestamp;
    }

    if (GetPriorityBuffer(apOptions->mpEventSchema->mPriority)->mFirstEventTimestamp == 0)
    {
        GetPriorityBuffer(apOptions->mpEventSchema->mPriority)->AddEvent(opts.mTimestamp.systemTimestamp);
    }
    ...
    ctxt.mCurrentTime        = GetPriorityBuffer(opts.mpEventSchema->mPriority)->mLastEventTimestamp;
...
exit:
    ...
    else if (opts.mpEventSchema->mPriority <= GetCurrentPriority(opts.mpEventSchema->mClusterId))
    {
        ...
        currentBuffer->AddEvent(opts.mTimestamp.systemTimestamp);
