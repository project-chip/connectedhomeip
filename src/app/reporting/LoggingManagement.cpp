...
// Event priority element consumes 3 bytes: control byte, 1-byte tag, and 1 byte value
                                        ...
                                        void * apAppData, const EventOptions * apOptions)
{
    ...
    eventDataElementBuilder.PriorityLevel(apContext->mPriority);
 ...
 *
 * @param[in] inLogStorageResources  An array of LogStorageResources for each priority level.
 *
 ...
 *   if we have one.
 *
 * @return chip::EventId Event ID for this priority.
 */
...
/**
 * @brief
 *   Fetch the most recently vended ID for a particular priority level
 *
 * @param aPriority Priority level
 *
 * @return chip::EventId most recently vended event ID for that event priority
 */
...
/**
 * @brief
 *   Fetch the first event ID currently stored for a particular priority level
 *
 * @param aPriority Priority level
 *
 * @return chip::EventId First currently stored event ID for that event priority
 */
    ...
    else if (aSchema.mPriority <= GetCurrentPriority(aSchema.mClusterId))
    {
...

        {
            ...
            ChipLogDetail(EventLogging,
                          "LogEvent event number: %u priority: %u cluster id: 0x%x event id: 0x%x sys timestamp: 0x%" PRIx32,
...
CHIP_ERROR LoggingManagement::CopyEvent(const TLVReader & aReader, TLVWriter & aWriter, EventLoadOutContext * apContext)
{
    ...
    VerifyOrExit(kMIN_EVENT_BOUNDARY_SIZE <= aWriter.GetRemainingFreeLength(), err = CHIP_ERROR_BUFFER_TOO_SMALL);
exit:
    return err;
}
...
exit:
    aEventID = static_cast<chip::EventId>(context.mCurrentEventNumber);
    Platform::CriticalSectionExit();
...
EventEnvelopeContext::EventEnvelopeContext(void) :
    mNumFieldsToRead(2), // read out priority and either system or utc delta time. events do not store both deltas.
