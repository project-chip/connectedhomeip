...
CHIP_ERROR
Engine::RetrieveClusterData(FabricIndex aAccessingFabricIndex, AttributeReports::Builder & aAttributeReports,
...
CHIP_ERROR Engine::BuildSingleReportDataAttributeReports(ReportDataMessage::Builder & aReportDataBuilder,
                                                         ReadHandler * apReadHandler)
{
    ...
    AttributeReports::Builder AttributeReports = aReportDataBuilder.CreateAttributeReports();
    ...
    for (auto clusterInfo = apReadHandler->GetAttributeClusterInfolist(); clusterInfo != nullptr; clusterInfo = clusterInfo->mpNext)
    {
        if (apReadHandler->IsInitialReport())
        {
            ...
            err = RetrieveClusterData(apReadHandler->GetFabricIndex(), AttributeReports, *clusterInfo);
    ...
    AttributeReports.EndOfAttributeReports();
    err = AttributeReports.GetError();
...
CHIP_ERROR Engine::BuildAndSendSingleReportData(ReadHandler * apReadHandler)
{
    ...
    err = BuildSingleReportDataAttributeReports(reportDataBuilder, apReadHandler);
    SuccessOrExit(err);
