...
CHIP_ERROR ReportingEngine::BuildSingleReportDataEventList(ReportData::Builder & aReportDataBuilder, ReadHandler * aReadHandler)
{
    CHIP_ERROR err  = CHIP_NO_ERROR;
    int event_count = 0;
    bool dataClean;
    ...
    mEventList = aReportDataBuilder.CreateEventDataListBuilder();

    // events only enter the picture if the read handler is
    // subscribed to events.
    // TODO:: Add mechanism to get particular set of interested clusters instead of all events
    if (aReadHandler->mGetToAllEvents)
    {
        ...
        for (int i = 0; i < kPriorityLevel_Last - kPriorityLevel_First + 1; i++)
        {
            chip::EventId tmp_id = logger.GetFirstEventID(static_cast<PriorityLevel>(i));
        ...
        if (aReadHandler->mCurrentPriority == kPriorityLevel_Invalid)
        {
            // Upload is not underway.  Check for new events, and set a checkpoint
            dataClean = aReadHandler->CheckEventUpToDate(logger);
            if (!dataClean)
            {
            ...
            aReadHandler->mCurrentPriority = aReadHandler->FindNextPriorityForTransfer();
        }
        else
        {
            aReadHandler->mCurrentPriority = aReadHandler->FindNextPriorityForTransfer();
            dataClean                      = (aReadHandler->mCurrentPriority == kPriorityLevel_Invalid);
        }
        ...
        while (aReadHandler->mCurrentPriority != kPriorityLevel_Invalid)
        {
            size_t i = static_cast<size_t>(aReadHandler->mCurrentPriority - kPriorityLevel_First);
            err      = logger.FetchEventsSince(*aReportDataBuilder.GetWriter(), aReadHandler->mCurrentPriority,
            ...
            {
                for (int t = 0; t <= kPriorityLevel_Last - kPriorityLevel_First; t++)
                {
    ...
    mEventList.EndOfEventList();
    SuccessOrExit(mEventList.GetError());
...
CHIP_ERROR ReportingEngine::BuildAndSendSingleReportData(ReadHandler * aReadHandler)
{
    CHIP_ERROR err = CHIP_NO_ERROR;
    chip::System::PacketBufferTLVWriter reportDataWriter;
    ReportData::Builder reportDataBuilder;
    chip::System::PacketBufferHandle bufHandle = System::PacketBufferHandle::New(System::kMaxPacketBufferSize);
    ...
    err = BuildSingleReportDataEventList(reportDataBuilder, aReadHandler);
    SuccessOrExit(err);

    // TODO: Add mechanism to set mSuppressResponse to handle status reports for multiple reports
    if (aReadHandler->mSuppressResponse)
    {
        ChipLogDetail(DataManagement, "<RE:Run> suppress response..");
        reportDataBuilder.SuppressResponse(aReadHandler->mSuppressResponse);
    }

    if (mMoreChunkedMessages)
    {
        ChipLogDetail(DataManagement, "<RE:Run> mMoreChunkedMessages is set..");
    ...
    bufHandle->EnsureReservedSize(CHIP_SYSTEM_CONFIG_HEADER_RESERVE_SIZE);
    ...
    ChipLogDetail(DataManagement, "<RE:Run> Sending notify...");
    err = SendReport(aReadHandler, std::move(bufHandle));
    VerifyOrExit(err == CHIP_NO_ERROR, ChipLogError(DataManagement, "<RE:Run> Error sending out report data!"));

exit:
    if (err != CHIP_NO_ERROR)
    {
        aReadHandler->Shutdown();
        err = CHIP_NO_ERROR;
    }
    return err;
}
...
void ReportingEngine::Run()
{
    ...
    while ((mNumReportsInFlight < CHOP_MAX_REPORTS_IN_FLIGHT) && (numReadHandled < CHIP_MAX_NUM_READ_HANDLER))
    {
        if (readHandler->IsReportable())
        {
            ...
            ChipLogDetail(DataManagement, "<RE:Run> ReportsInFlight = %u with readHandler %u, RE has %s", mNumReportsInFlight,
                          mCurReadHandlerIdx, mMoreChunkedMessages ? "MoreChunkedMessages" : "Clean");
            if (!mMoreChunkedMessages)
            {
...
void ReportingEngine::OnReportConfirm()
{
    ...
    mNumReportsInFlight--;
    // TODO: Add more chunk message support
    // if (mMoreChunkedMessages)
    //    Run()
}
