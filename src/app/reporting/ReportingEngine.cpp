...
CHIP_ERROR ReportingEngine::BuildSingleReportDataEventList(ReportData::Builder & aReportDataBuilder, ReadHandler * apReadHandler)
{
    ...
    if (apReadHandler->mGetToAllEvents)
    {
        ...
        if (apReadHandler->mCurrentPriority == kPriorityLevel_Invalid)
        {
        ...
        while (apReadHandler->mCurrentPriority != kPriorityLevel_Invalid)
        {
            size_t i = static_cast<size_t>(apReadHandler->mCurrentPriority - kPriorityLevel_First);
    ...
    SuccessOrExit(eventList.GetError());
...
CHIP_ERROR ReportingEngine::BuildAndSendSingleReportData(ReadHandler * apReadHandler)
{
    ...
    chip::System::PacketBufferHandle bufHandle = System::PacketBufferHandle::New(chip::app::kMax_SecureSDU_Length);
    ...
    ChipLogDetail(DataManagement, "<RE:Run> Sending report...");
...
uint32_t ReportingEngine::CountEvents(ReadHandler * apReadHandler, chip::EventId * apInitialEvents)
{
    uint32_t event_count = 0;
