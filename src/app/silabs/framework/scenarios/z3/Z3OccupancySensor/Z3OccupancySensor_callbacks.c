...
void emberAfPluginOccupancySensorServerOccupancyStateChangedCallback(HalOccupancyState occupancyState)
{
    ...
    // of an identify
    emberAfReadServerAttribute(1, ZCL_IDENTIFY_CLUSTER_ID, ZCL_IDENTIFY_TIME_ATTRIBUTE_ID, (uint8_t *) (&identifyTime),
                               sizeof(uint16_t));
    if (identifyTime != 0)
    {
        return;
    ...
    if (!GPIO_PinInGet(BSP_PYD1698_INSTALLATION_JP_PORT, BSP_PYD1698_INSTALLATION_JP_PIN))
    {
...
EmberAfStatus emberAfPluginReportingConfiguredCallback(const EmberAfPluginReportingEntry * entry)
{
    ...
    if ((entry->clusterId == ZCL_TEMP_MEASUREMENT_CLUSTER_ID) && (entry->attributeId == ZCL_TEMP_MEASURED_VALUE_ATTRIBUTE_ID))
    {
        ...
        else
        {
            ...
            // in this API.
            emberAfCorePrintln("Temperature reporting interval set: %d seconds", entry->data.reported.maxInterval);
             ...
             (entry->attributeId == ZCL_RELATIVE_HUMIDITY_MEASURED_VALUE_ATTRIBUTE_ID))
    {
        ...
        else
        {
            ...
            // expects in this API.
            emberAfCorePrintln("Humidity reporting interval set: %d seconds", entry->data.reported.maxInterval);
             ...
             (entry->attributeId == ZCL_ILLUM_MEASURED_VALUE_ATTRIBUTE_ID))
    {
        ...
        else
        {
            ...
            // in this API.
            emberAfCorePrintln("Illuminance reporting interval set: %d seconds", entry->data.reported.maxInterval);
