...
EmberAfStatus emberAfPollControlClusterServerCommandParse(EmberAfClusterCommand * cmd)
{
    ...
    if (!cmd->mfgSpecific)
    {
        ...
        case ZCL_CHECK_IN_RESPONSE_COMMAND_ID: {
            ...
            wasHandled      = emberAfPollControlClusterCheckInResponseCallback(startFastPolling, fastPollTimeout);
            break;
        }
...
EmberAfStatus emberAfSlWwahClusterServerCommandParse(EmberAfClusterCommand * cmd)
{
    ...
    if (cmd->mfgSpecific)
    {
        if (cmd->mfgCode == 0x1217 && cmd->commandId == ZCL_ENABLE_APS_LINK_KEY_AUTHORIZATION_COMMAND_ID)
        {
            ...
            wasHandled = emberAfSlWwahClusterEnableApsLinkKeyAuthorizationCallback(numberExemptClusters, clusterId);
        }
        else if (cmd->mfgCode == 0x1217 && cmd->commandId == ZCL_DISABLE_APS_LINK_KEY_AUTHORIZATION_COMMAND_ID)
        {
            ...
            wasHandled = emberAfSlWwahClusterDisableApsLinkKeyAuthorizationCallback(numberExemptClusters, clusterId);
        }
        ...
        else if (cmd->mfgCode == 0x1217 && cmd->commandId == ZCL_ENABLE_WWAH_APP_EVENT_RETRY_ALGORITHM_COMMAND_ID)
        {
            ...
            wasHandled            = emberAfSlWwahClusterEnableWwahAppEventRetryAlgorithmCallback(
        ...
        else if (cmd->mfgCode == 0x1217 && cmd->commandId == ZCL_ENABLE_WWAH_REJOIN_ALGORITHM_COMMAND_ID)
        {
            ...
            wasHandled           = emberAfSlWwahClusterEnableWwahRejoinAlgorithmCallback(
                fastRejoinTimeoutSeconds, durationBetweenRejoinsSeconds, fastRejoinFirstBackoffSeconds, maxBackoffTimeSeconds,
        ...
        else if (cmd->mfgCode == 0x1217 && cmd->commandId == ZCL_SET_PENDING_NETWORK_UPDATE_COMMAND_ID)
        {
            ...
            wasHandled = emberAfSlWwahClusterSetPendingNetworkUpdateCallback(channel, panId);
        }
        else if (cmd->mfgCode == 0x1217 && cmd->commandId == ZCL_REQUIRE_APS_ACKS_ON_UNICASTS_COMMAND_ID)
        {
            ...
            wasHandled = emberAfSlWwahClusterRequireApsAcksOnUnicastsCallback(numberExemptClusters, clusterId);
        }
        ...
        else if (cmd->mfgCode == 0x1217 && cmd->commandId == ZCL_USE_TRUST_CENTER_FOR_CLUSTER_SERVER_COMMAND_ID)
        {
            ...
            wasHandled = emberAfSlWwahClusterUseTrustCenterForClusterServerCallback(numberOfClusters, clusterId);
        }
