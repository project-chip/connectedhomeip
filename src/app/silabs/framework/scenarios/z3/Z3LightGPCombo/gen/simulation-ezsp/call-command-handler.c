...
EmberAfStatus emberAfClusterSpecificCommandParse(EmberAfClusterCommand * cmd)
{
             ...
             emberAfContainsServerWithMfgCode(cmd->apsFrame->destinationEndpoint, cmd->apsFrame->clusterId, cmd->mfgCode))
    {
        ...
        case ZCL_LEVEL_CONTROL_CLUSTER_ID:
            ...
            break;
        case ZCL_GREEN_POWER_CLUSTER_ID:
            result = emberAfGreenPowerClusterServerCommandParse(cmd);
            break;
...
// Cluster: Green Power, server
EmberAfStatus emberAfGreenPowerClusterServerCommandParse(EmberAfClusterCommand * cmd)
{
    bool wasHandled = false;
    if (!cmd->mfgSpecific)
    {
        ...
        case ZCL_GP_NOTIFICATION_COMMAND_ID: {
            uint16_t payloadOffset = cmd->payloadStartIndex;
            uint16_t options;                 // Ver.: since gp-1.0-09-5499-24
            uint32_t gpdSrcId;                // Ver.: since gp-1.0-09-5499-24
            uint8_t * gpdIeee;                // Ver.: since gp-1.0-09-5499-24
            uint8_t gpdEndpoint;              // Ver.: since gp-1.0-09-5499-24
            uint32_t gpdSecurityFrameCounter; // Ver.: since gp-1.0-09-5499-24
            uint8_t gpdCommandId;             // Ver.: since gp-1.0-09-5499-24
            uint8_t * gpdCommandPayload;      // Ver.: since gp-1.0-09-5499-24
            uint16_t gppShortAddress;         // Ver.: since gp-1.0-09-5499-24
            uint8_t gppDistance;              // Ver.: since gp-1.0-09-5499-24
            // Command is not a fixed length
            if ((cmd->bufLen < payloadOffset + 2u))
            {
                // Argument is not always present:
            ...
            {
                options = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 2u;
            }
            if (!((options & 0x0007) == 0x0000) || (cmd->bufLen < payloadOffset + 4u))
            {
            ...
            {
                gpdEndpoint = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if ((cmd->bufLen < payloadOffset + 4u))
            {
            ...
            if (!((options & 0x4000) || (options & 0x0800)) || (cmd->bufLen < payloadOffset + 2u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x4000) || (options & 0x0800)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                gppShortAddress = 0xFFFF;
            }
            else
            {
                gppShortAddress = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 2u;
            }
            if (!((options & 0x4000) || (options & 0x0800)) || (cmd->bufLen < payloadOffset + 1u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x4000) || (options & 0x0800)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
            ...
            {
                gppDistance = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
            }
            wasHandled =
                emberAfGreenPowerClusterGpNotificationCallback(options, gpdSrcId, gpdIeee, gpdEndpoint, gpdSecurityFrameCounter,
        ...
        case ZCL_GP_COMMISSIONING_NOTIFICATION_COMMAND_ID: {
            ...
            uint8_t * gpdCommandPayload;      // Ver.: since gp-1.0-09-5499-24
            uint16_t gppShortAddress;         // Ver.: since gp-1.0-09-5499-24
            uint8_t gppLink;                  // Ver.: since gp-1.0-09-5499-24
            uint32_t mic;                     // Ver.: since gp-1.0-09-5499-24
            // Command is not a fixed length
            if ((cmd->bufLen < payloadOffset + 2u))
            {
                // Argument is not always present:
            ...
            if (!((options & 0x0800) || (options & 0x0008)) || (cmd->bufLen < payloadOffset + 2u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0800) || (options & 0x0008)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                gppShortAddress = 0xFFFF;
            }
            else
            ...
            if (!((options & 0x0800) || (options & 0x0008)) || (cmd->bufLen < payloadOffset + 1u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0800) || (options & 0x0008)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
            ...
            {
                gppLink = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if (!(options & 0x0200) || (cmd->bufLen < payloadOffset + 4u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: options & 0x0200
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                mic = 0xFFFFFFFF;
            }
            else
            {
                mic = emberAfGetInt32u(cmd->buffer, payloadOffset, cmd->bufLen);
            }
            wasHandled = emberAfGreenPowerClusterGpCommissioningNotificationCallback(
                options, gpdSrcId, gpdIeee, endpoint, gpdSecurityFrameCounter, gpdCommandId, gpdCommandPayload, gppShortAddress,
                gppLink, mic);
            break;
        }
        case ZCL_GP_SINK_COMMISSIONING_MODE_COMMAND_ID: {
            uint16_t payloadOffset = cmd->payloadStartIndex;
            uint8_t options;             // Ver.: always
            uint16_t gpmAddrForSecurity; // Ver.: always
            uint16_t gpmAddrForPairing;  // Ver.: always
            uint8_t sinkEndpoint;        // Ver.: always
            ...
            options = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
            payloadOffset += 1u;
            gpmAddrForSecurity = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
            payloadOffset += 2u;
            gpmAddrForPairing = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
            payloadOffset += 2u;
            sinkEndpoint = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
            wasHandled   = emberAfGreenPowerClusterGpSinkCommissioningModeCallback(options, gpmAddrForSecurity, gpmAddrForPairing,
                                                                                 sinkEndpoint);
            break;
        }
        case ZCL_GP_TRANSLATION_TABLE_UPDATE_COMMAND_ID: {
            ...
            uint8_t * gpdIeee;      // Ver.: since gp-1.0-09-5499-24
            uint8_t endpoint;       // Ver.: since gp-1.0-09-5499-24
            uint8_t * translations; // Ver.: since gp-1.0-09-5499-24
            // Command is not a fixed length
            if ((cmd->bufLen < payloadOffset + 2u))
            {
                // Argument is not always present:
            ...
            translations = cmd->buffer + payloadOffset;
            wasHandled =
                emberAfGreenPowerClusterGpTranslationTableUpdateCallback(options, gpdSrcId, gpdIeee, endpoint, translations);
            break;
        }
        case ZCL_GP_TRANSLATION_TABLE_REQUEST_COMMAND_ID: {
            uint16_t payloadOffset = cmd->payloadStartIndex;
            uint8_t startIndex; // Ver.: since gp-1.0-09-5499-24
            // Command is not a fixed length
            if ((cmd->bufLen < payloadOffset + 1u))
            {
                // Argument is not always present:
            ...
            wasHandled = emberAfGreenPowerClusterGpTranslationTableRequestCallback(startIndex);
            break;
        }
        case ZCL_GP_PAIRING_CONFIGURATION_COMMAND_ID: {
            uint16_t payloadOffset = cmd->payloadStartIndex;
            uint8_t actions;                  // Ver.: since gp-1.0-09-5499-24
            uint16_t options;                 // Ver.: since gp-1.0-09-5499-24
            uint32_t gpdSrcId;                // Ver.: since gp-1.0-09-5499-24
            uint8_t * gpdIeee;                // Ver.: since gp-1.0-09-5499-24
            uint8_t endpoint;                 // Ver.: since gp-1.0-09-5499-24
            uint8_t deviceId;                 // Ver.: since gp-1.0-09-5499-24
            uint8_t groupListCount;           // Ver.: since gp-1.0-09-5499-24
            uint8_t * groupList;              // Ver.: since gp-1.0-09-5499-24
            uint16_t gpdAssignedAlias;        // Ver.: since gp-1.0-09-5499-24
            uint8_t groupcastRadius;          // Ver.: since gp-1.0-15-2014-05-CCB2180
            uint8_t securityOptions;          // Ver.: since gp-1.0-09-5499-24
            uint32_t gpdSecurityFrameCounter; // Ver.: since gp-1.0-09-5499-24
            uint8_t * gpdSecurityKey;         // Ver.: since gp-1.0-09-5499-24
            uint8_t numberOfPairedEndpoints;  // Ver.: since gp-1.0-09-5499-24
            uint8_t * pairedEndpoints;        // Ver.: since gp-1.0-09-5499-24
            uint8_t applicationInformation;   // Ver.: always
            uint16_t manufacturerId;          // Ver.: always
            uint16_t modeId;                  // Ver.: always
            uint8_t numberOfGpdCommands;      // Ver.: always
            uint8_t * gpdCommandIdList;       // Ver.: always
            uint8_t clusterIdListCount;       // Ver.: always
            uint8_t * clusterListServer;      // Ver.: always
            uint8_t * clusterListClient;      // Ver.: always
            uint8_t switchInformationLength;  // Ver.: always
            uint8_t switchConfiguration;      // Ver.: always
            uint8_t currentContactStatus;     // Ver.: always
            uint8_t totalNumberOfReports;     // Ver.: always
            uint8_t numberOfReports;          // Ver.: always
            uint8_t * reportDescriptor;       // Ver.: always
            // Command is not a fixed length
            if ((cmd->bufLen < payloadOffset + 1u))
            {
                // Argument is not always present:
            ...
            {
                actions = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if ((cmd->bufLen < payloadOffset + 2u))
            {
            ...
            if (!((options & 0x0018) == 0x0010) || (cmd->bufLen < payloadOffset + 1u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0018) == 0x0010
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                groupListCount = 0xFF;
            }
            else
            {
                groupListCount = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if ((options & 0x0018) == 0x0010)
            {
                // Array is conditionally present based on expression: (options & 0x0018) == 0x0010
                groupList = cmd->buffer + payloadOffset;
                payloadOffset += groupListCount * sizeof(uint8_t *);
            }
            else
            {
                groupList = NULL;
            }
            if (!((options & 0x0100)) || (cmd->bufLen < payloadOffset + 2u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0100)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                gpdAssignedAlias = 0xFFFF;
            }
            else
            {
                gpdAssignedAlias = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 2u;
            }
            if ((cmd->bufLen < payloadOffset + 1u))
            {
                // Argument is not always present:
                // - it is present only in versions higher than: gp-1.0-15-2014-05-CCB2180
                groupcastRadius = 0xFF;
            }
            else
            {
            ...
            if (!((options & 0x0200)) || (cmd->bufLen < payloadOffset + 1u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0200)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                securityOptions = 0xFF;
            }
            else
            {
                securityOptions = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if (!((options & 0x0200) || (options & 0x0020)) || (cmd->bufLen < payloadOffset + 4u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0200) || (options & 0x0020)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                gpdSecurityFrameCounter = 0xFFFFFFFF;
            }
            else
            ...
            if (!((options & 0x0200)) || (cmd->bufLen < payloadOffset + 16u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0200)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
            ...
            {
                gpdSecurityKey = cmd->buffer + payloadOffset;
                payloadOffset += 16u;
            }
            if ((cmd->bufLen < payloadOffset + 1u))
            {
                // Argument is not always present:
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                numberOfPairedEndpoints = 0xFF;
            }
            else
            {
                numberOfPairedEndpoints = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if (0 < numberOfPairedEndpoints && numberOfPairedEndpoints < 0xFD)
            {
                // Array is conditionally present based on expression: 0 < numberOfPairedEndpoints && numberOfPairedEndpoints < 0xFD
                pairedEndpoints = cmd->buffer + payloadOffset;
                payloadOffset += numberOfPairedEndpoints * sizeof(uint8_t);
            }
            else
            {
                pairedEndpoints = NULL;
            }
            if (!((options & 0x0400)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400)
                applicationInformation = 0xFF;
            }
            else
            {
                applicationInformation = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x01)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x01)
                manufacturerId = 0xFFFF;
            }
            else
            {
                manufacturerId = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 2u;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x02)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x02)
                modeId = 0xFFFF;
            }
            else
            {
                modeId = emberAfGetInt16u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 2u;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x04)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x04)
                numberOfGpdCommands = 0xFF;
            }
            else
            {
                numberOfGpdCommands = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if ((options & 0x0400) && (applicationInformation & 0x04))
            {
                // Array is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x04)
                gpdCommandIdList = cmd->buffer + payloadOffset;
                payloadOffset += numberOfGpdCommands * sizeof(uint8_t);
            }
            else
            {
                gpdCommandIdList = NULL;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x08)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x08)
                clusterIdListCount = 0xFF;
            }
            else
            {
                clusterIdListCount = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if ((options & 0x0400) && (applicationInformation & 0x08) && ((clusterIdListCount & 0x0F) > 0))
            {
                // Array is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x08) &&
                // ((clusterIdListCount & 0x0F) > 0)
                clusterListServer = cmd->buffer + payloadOffset;
                payloadOffset += (clusterIdListCount & 0x0F) * sizeof(uint16_t);
            }
            else
            {
                clusterListServer = NULL;
            }
            if ((options & 0x0400) && (applicationInformation & 0x08) && ((clusterIdListCount >> 4) > 0))
            {
                // Array is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x08) &&
                // ((clusterIdListCount >> 4) > 0)
                clusterListClient = cmd->buffer + payloadOffset;
                payloadOffset += (clusterIdListCount >> 4) * sizeof(uint16_t);
            }
            else
            {
                clusterListClient = NULL;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x10)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x10)
                switchInformationLength = 0xFF;
            }
            else
            {
                switchInformationLength = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x10)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x10)
                switchConfiguration = 0xFF;
            }
            else
            {
                switchConfiguration = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x10)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x10)
                currentContactStatus = 0xFF;
            }
            else
            {
                currentContactStatus = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x20)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x20)
                totalNumberOfReports = 0xFF;
            }
            else
            {
                totalNumberOfReports = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if (!((options & 0x0400) && (applicationInformation & 0x20)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x20)
                numberOfReports = 0xFF;
            }
            else
            {
                numberOfReports = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
                payloadOffset += 1u;
            }
            if ((options & 0x0400) && (applicationInformation & 0x20))
            {
                // Array is conditionally present based on expression: (options & 0x0400) && (applicationInformation & 0x20)
                reportDescriptor = cmd->buffer + payloadOffset;
            }
            else
            {
                reportDescriptor = NULL;
            }
            wasHandled = emberAfGreenPowerClusterGpPairingConfigurationCallback(
                actions, options, gpdSrcId, gpdIeee, endpoint, deviceId, groupListCount, groupList, gpdAssignedAlias,
                groupcastRadius, securityOptions, gpdSecurityFrameCounter, gpdSecurityKey, numberOfPairedEndpoints, pairedEndpoints,
                applicationInformation, manufacturerId, modeId, numberOfGpdCommands, gpdCommandIdList, clusterIdListCount,
                clusterListServer, clusterListClient, switchInformationLength, switchConfiguration, currentContactStatus,
                totalNumberOfReports, numberOfReports, reportDescriptor);
            break;
        }
        case ZCL_GP_SINK_TABLE_REQUEST_COMMAND_ID: {
            ...
            options = emberAfGetInt8u(cmd->buffer, payloadOffset, cmd->bufLen);
            payloadOffset += 1u;
            if (!(((options & 0x07) == 0x00) && ((options & 0x18) == 0x00)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: ((options & 0x07) == 0x00) && ((options & 0x18) == 0x00)
                gpdSrcId = 0xFFFFFFFF;
            }
            else
            {
            ...
            if (!(((options & 0x07) == 0x02) && ((options & 0x18) == 0x00)))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: ((options & 0x07) == 0x02) && ((options & 0x18) == 0x00)
                gpdIeee = NULL;
            }
            else
            {
            ...
            wasHandled = emberAfGreenPowerClusterGpSinkTableRequestCallback(options, gpdSrcId, gpdIeee, endpoint, index);
            break;
        }
        default: {
            // Unrecognized command ID, error status will apply.
