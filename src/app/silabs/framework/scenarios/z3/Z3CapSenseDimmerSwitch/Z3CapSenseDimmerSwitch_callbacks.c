...
static void generateMatchDescriptorRequest(EmberAfClusterId clusterId)
{
    ...
    emberAfFindDevicesByProfileAndCluster(target, profile, clusterId, serverCluster, matchDescriptorCallback);
}
...
void sendMatchDescriptorEventHandler(void)
{
    ...
    if (lightingClientClusterIdx < COUNTOF(lightingClientClusters))
    {
        ...
        generateMatchDescriptorRequest(lightingClientClusters[lightingClientClusterIdx]);
    }
}
...
static EmberStatus addToBindingTable(EmberBindingTableEntry * entry)
{
    ...
    emberSetBindingRemoteNodeId(lightingClientClusterIdx, EMBER_ZIGBEE_COORDINATOR_ADDRESS);
...
void emberAfPluginSb1GestureSensorGestureReceivedCallback(int8u gesture, int8u ui8SwitchNum)
{
    int8u sendNeeded;
...
void formCommandFromHoldUp(void)
{
    ...
    case NORMAL_TAB:
        ...
        emberAfFillCommandLevelControlClusterStepWithOnOff(0, dimStepSize, dimTime);
        break;
    case TEMP_TAB:
        emberSerialPrintf(APP_SERIAL, "TEMP UP \r\n");

        emberAfFillCommandColorControlClusterStepColorTemperature(
            COLOR_TEMP_STEP_POSITIVE, COLOR_TEMP_STEP_AMOUNT,
            0,      // transitionTime = 0
    ...
    case HUE_TAB:
        ...
        emberAfFillCommandColorControlClusterStepHue(COLOR_HUE_STEP_POSITIVE, COLOR_HUE_STEP_AMOUNT,
                                                     0,     // Transition time = 0
                                                     0x00,  // optionsMask
                                                     0x00); // optionsOverride
        break;
...
void formCommandFromHoldDown(void)
{
    ...
    case NORMAL_TAB:
        ...
        emberAfFillCommandLevelControlClusterStepWithOnOff(1, dimStepSize, dimTime);

        break;
    case TEMP_TAB:
        emberSerialPrintf(APP_SERIAL, "TEMP DN \r\n");
        emberAfFillCommandColorControlClusterStepColorTemperature(
            COLOR_TEMP_STEP_NEGATIVE, COLOR_TEMP_STEP_AMOUNT,
            0,      // transitionTime = 0
            0,      // colorTemperatureMinimum = 0, as it will be handled by the gateway
    ...
    case HUE_TAB:
        ...
        emberAfFillCommandColorControlClusterStepHue(COLOR_HUE_STEP_NEGATIVE, COLOR_HUE_STEP_AMOUNT,
                                                     0,     // Transition time = 0
                                                     0x00,  // optionsMask
                                                     0x00); // optionsOverride
        break;
...
void localSendCommand(void)
{
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfCorePrintln("Error sending msg to binding entries: 0x%X", status);
    }
}
