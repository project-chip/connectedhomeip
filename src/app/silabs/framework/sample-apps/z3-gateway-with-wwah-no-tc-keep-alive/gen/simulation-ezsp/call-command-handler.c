...
EmberAfStatus emberAfGreenPowerClusterClientCommandParse(EmberAfClusterCommand * cmd)
{
    ...
    if (!cmd->mfgSpecific)
    {
        ...
        case ZCL_GP_NOTIFICATION_RESPONSE_COMMAND_ID: {
            ...
            wasHandled = emberAfGreenPowerClusterGpNotificationResponseCallback(options, gpdSrcId, gpdIeee, endpoint,
        ...
        case ZCL_GP_PAIRING_COMMAND_ID: {
            ...
            if (!((options & 0x000010) == 0x000000 && ((options & 0x000060) == 0x000000 || (options & 0x000060) == 0x000060)) ||
                (cmd->bufLen < payloadOffset + 8u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x000010) == 0x000000 && ((options & 0x000060) ==
                // 0x000000 || (options & 0x000060) == 0x000060)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
            ...
            if (!((options & 0x000010) == 0x000000 && ((options & 0x000060) == 0x000020 || (options & 0x000060) == 0x000040)) ||
                (cmd->bufLen < payloadOffset + 2u))
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x000010) == 0x000000 && ((options & 0x000060) ==
                // 0x000020 || (options & 0x000060) == 0x000040)
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                sinkGroupId = 0xFFFF;
            }
            else
            ...
            {
                // Argument is not always present:
                // - it is conditionally present based on expression: (options & 0x000008) == 0x000008 && (options & 0x000010) !=
                // 0x000010
                // - it is present only in versions higher than: gp-1.0-09-5499-24
                deviceId = 0xFF;
            }
            ...
            wasHandled = emberAfGreenPowerClusterGpPairingCallback(options, gpdSrcId, gpdIeee, endpoint, sinkIeeeAddress,
                                                                   sinkNwkAddress, sinkGroupId, deviceId, gpdSecurityFrameCounter,
        ...
        case ZCL_GP_PROXY_COMMISSIONING_MODE_COMMAND_ID: {
            ...
            wasHandled = emberAfGreenPowerClusterGpProxyCommissioningModeCallback(options, commissioningWindow, channel);
            break;
        }
        case ZCL_GP_RESPONSE_COMMAND_ID: {
            ...
            wasHandled = emberAfGreenPowerClusterGpResponseCallback(options, tempMasterShortAddress, tempMasterTxChannel, gpdSrcId,
                                                                    gpdIeee, endpoint, gpdCommandId, gpdCommandPayload);
            break;
        }
        case ZCL_GP_SINK_TABLE_RESPONSE_COMMAND_ID: {
            ...
            wasHandled       = emberAfGreenPowerClusterGpSinkTableResponseCallback(status, totalNumberofNonEmptySinkTableEntries,
                                                                             startIndex, sinkTableEntriesCount, sinkTableEntries);
            break;
        }
        case ZCL_GP_PROXY_TABLE_REQUEST_COMMAND_ID: {
            ...
            wasHandled = emberAfGreenPowerClusterGpProxyTableRequestCallback(options, gpdSrcId, gpdIeee, endpoint, index);
            break;
        }
        default: {
            // Unrecognized command ID, error status will apply.
...
EmberAfStatus emberAfSlWwahClusterClientCommandParse(EmberAfClusterCommand * cmd)
{
    ...
    if (cmd->mfgSpecific)
    {
        if (cmd->mfgCode == 0x1217 && cmd->commandId == ZCL_DEBUG_REPORT_QUERY_RESPONSE_COMMAND_ID)
        {
            ...
            wasHandled      = emberAfSlWwahClusterDebugReportQueryResponseCallback(debugReportId, debugReportData);
        }
    }
    return status(wasHandled, true, cmd->mfgSpecific);
}
