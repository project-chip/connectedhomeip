...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Implementations for network related CLI
                                                                               *commands.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
EmberCommandEntry networkCommands[] = { { "form", networkFormCommand, "usv" },
                                        ...
                                        emberCommandEntryAction("id", networkIdCommand, "", "Print the current network IDs"),
...
void networkRejoinCommand(void)
{
    ...
    EmberStatus status =
        emberFindAndRejoinNetworkWithReason(haveCurrentNetworkKey, channelMask, EMBER_AF_REJOIN_DUE_TO_CLI_COMMAND);
...
void networkRejoinDiffDeviceTypeCommand(void)
{
    ...
    EmberStatus status         = emberFindAndRejoinNetworkWithNodeType(haveCurrentNetworkKey, channelMask, emberNodeType);
...
void networkPermitJoinCommand(void)
{
    ...
    emAfPermitJoin(duration, ('b' == emberStringCommandArgument(-1,
...
void networkChangeChannelCommand(void)
{
    ...
    emberAfAppPrintln("Changing to channel %d: 0x%X", channel, status);
}
...
void networkMultiPhyStartCommand(void)
{
    ...
    {
        emberAfCorePrintln("Failed to %s %s 0x%X", "start", "multi-phy interface", status);
    }
}
...
void networkMultiPhyStopCommand(void)
{
    ...
    if (status == EMBER_SUCCESS)
    {
        emberAfCorePrintln("Terminated %s 0x%X", "multi-phy interface", status);
    }
    else
    {
        emberAfCorePrintln("Failed to %s %s 0x%X", "stop", "multi-phy interface", status);
    }
}
