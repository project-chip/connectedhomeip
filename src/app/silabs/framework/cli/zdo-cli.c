...
EmberStatus matchDescriptorsRequest(EmberNodeId target, uint16_t profile, uint8_t inCount, uint8_t outCount, uint16_t * inClusters,
                                    uint16_t * outClusters, EmberApsOption options);
...
static EmberCommandEntry zdoClusterCommands[] = {
    emberCommandEntryActionWithDetails("add", zdoAddClusterCommand, "v", "Add a ZCL cluster to the CLI's list.",
                                       addClusterArguments),
    emberCommandEntryAction("clear", zdoClearClusterCommand, "", "Remove all ZCL clusters from the CLI's list"),
...
static EmberCommandEntry zdoNetworkUpdateCommands[] = {
    emberCommandEntryActionWithDetails("chan", zdoNetworkUpdateChannelCommand, "uu*", "Send a channel change command.",
                                       channelChangeArguments),
    emberCommandEntryActionWithDetails("scan", zdoNetworkUpdateScanCommand, "vuvw*", "Tell a remote node to perform a channel scan",
                                       channelScanArguments),
    emberCommandEntryActionWithDetails("set", zdoNetworkUpdateSetCommand, "vw",
                                       "Broadcast a new NWK manager Node ID and channel list.", channelManagerArguments),
    emberCommandEntryTerminator(),
};
...
static const char * zdoBindCommandArguments[] = {
    "Dest node ID", "local EP", "remote EP", "cluster ID", "THEIR EUI", "binding dest EUI", NULL,
};
...
EmberCommandEntry zdoCommands[] = {
    emberCommandEntryAction("active", zdoActiveEpCommand, "v", "Send an Active EP request"),
    emberCommandEntryActionWithDetails("bind", zdoBindCommand, "vuuvbb", "Sends bind request", zdoBindCommandArguments),
    emberCommandEntryAction("ieee", zdoIeeeAddressRequestCommand, "v", "Unicast an IEEE address request to the specified node."),
    emberCommandEntrySubMenu("in-cl-list", zdoClusterCommands, "Modify input cluster list"),
    ...
    emberCommandEntryActionWithDetails("mgmt-lqi", zdoMgmtLqiCommand, "vu", "Send a MGMT-LQI (LQI Table) request to target",
                                       zdoMgmtLqiCommandArguments),
    emberCommandEntryActionWithDetails("mgmt-bind", zdoMgmtBindCommand, "vu", "Send a MGMT-Bind (Binding Table) request to target",
                                       zdoMgmtLqiCommandArguments), // Binding Table Req uses same args as LQI Table Req
    emberCommandEntryAction("node", zdoNodeCommand, "v", "Send a node descriptor request to target"),
    emberCommandEntryAction("nwk", zdoNwkAddressRequestCommand, "b", "Broadcast a NWK address request for the specified IEEE."),
    emberCommandEntrySubMenu("nwk-upd", zdoNetworkUpdateCommands, "Frequency Agility Commands"),
    emberCommandEntrySubMenu("out-cl-list", zdoClusterCommands, "Modify output cluster list"),
...
void zdoNwkAddressRequestCommand(void)
{
    ...
    emberAfFindNodeId(eui64, emAfCliServiceDiscoveryCallback);
}

void zdoIeeeAddressRequestCommand(void)
{
    EmberNodeId id = (EmberNodeId) emberUnsignedCommandArgument(0);
    emberAfFindIeeeAddress(id, emAfCliServiceDiscoveryCallback);
}
...
void zdoSimpleCommand(void)
{
    ...
    EmberStatus status     = emberSimpleDescriptorRequest(target, targetEndpoint, EMBER_AF_DEFAULT_APS_OPTIONS);
...
{
    EmberNodeId target = (EmberNodeId) emberUnsignedCommandArgument(0);
    EmberStatus status = emberNodeDescriptorRequest(target, EMBER_AF_DEFAULT_APS_OPTIONS);
...
void zdoMatchCommand(void)
{
    ...
    EmberStatus status = matchDescriptorsRequest(target, profile, emAfCliInClCount, emAfCliOutClCount, emAfCliZdoInClusters,
                                                 emAfCliZdoOutClusters, EMBER_AF_DEFAULT_APS_OPTIONS);
...
void zdoNetworkUpdateChannelCommand(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            status                      = emberEnergyScanRequest(EMBER_SLEEPY_BROADCAST_ADDRESS, pageChannelBitMask, 0xFE, 0);
        }
...
void zdoNetworkUpdateScanCommand(void)
{
    ...
    else
    {
        ...
        EmberStatus status = emberEnergyScanRequest(target, scanChannelsMask, scanDuration, scanCount);
...
void zdoNetworkUpdateSetCommand(void)
{
    ...
    EmberStatus status         = emberSetNetworkManagerRequest(networkManager, activeChannels);
...
{
    EmberNodeId target = (EmberNodeId) emberUnsignedCommandArgument(0);
    EmberStatus status = emberActiveEndpointsRequest(target, EMBER_APS_OPTION_RETRY);
    emberAfAppPrint("Active EP request status: 0x%X", status);
}
...
void zdoMgmtLqiCommand(void)
{
    ...
    EmberStatus status = emberLqiTableRequest(target, index, EMBER_APS_OPTION_RETRY);
...
void zdoMgmtBindCommand(void)
{
    EmberNodeId target = (EmberNodeId) emberUnsignedCommandArgument(0);
    uint8_t index      = emberUnsignedCommandArgument(1);
    EmberStatus status = emberBindingTableRequest(target, index, EMBER_APS_OPTION_RETRY);
...
void zdoLeaveRequestCommand(void)
{
    ...
    status = emberLeaveRequest(target, nullEui64, options, EMBER_APS_OPTION_RETRY);
...
{
    EmberNodeId target = (EmberNodeId) emberUnsignedCommandArgument(0);
    EmberStatus status = emberPowerDescriptorRequest(target, EMBER_APS_OPTION_RETRY);
...
static void unbindRequest(bool isGroupAddress, void * destination, uint8_t destinationEndpoint)
{
    ...
    status =
        emberUnbindRequest(target, sourceEui64, sourceEndpoint, clusterId, (isGroupAddress ? MULTICAST_BINDING : UNICAST_BINDING),
                           destinationEui64, groupAddress, destinationEndpoint, EMBER_APS_OPTION_RETRY);
    UNUSED_VAR(status);
    emberAfAppPrintln("Unbind %p %p0x%X", (isGroupAddress ? "Group" : "Unicast"), "Request: ", status);
}
...
void zdoUnbindUnicastCommand(void)
{
    ...
    unbindRequest(false, // group addressed binding?
                  destinationEui64, destinationEndpoint);
}
...
void zdoRouteRequestCommand(void)
{
    ...
    EmberStatus status = emberRoutingTableRequest(target, index, EMBER_APS_OPTION_RETRY);
...
EmberStatus matchDescriptorsRequest(EmberNodeId target, uint16_t profile, uint8_t inCount, uint8_t outCount, uint16_t * inClusters,
                                    uint16_t * outClusters, EmberApsOption options)
{
    return ezspMatchDescriptorsRequest(target, profile, inCount, outCount, inClusters, outClusters, options);
}
                                    ...
                                    uint16_t * outClusters, EmberApsOption options)
{
    ...
    for (output = 0; output < 2; output++)
    {
        ...
        *bufferPtr = emberAllocateStackBuffer();
        ...
        for (i = 0; i < count; i++)
        {
            ...
            status     = emberAppendToLinkedBuffers(*bufferPtr, cluster, 2);
    ...
    status = emberMatchDescriptorsRequest(target, profile, inClusterBuffer, outClusterBuffer, options);
cleanup:
