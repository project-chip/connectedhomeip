...
#define APP_ZCL_BUFFER_SIZE_CLI (APP_ZCL_BUFFER_SIZE > 255 ? 255 : APP_ZCL_BUFFER_SIZE)
...
{
    globalApsFrame.sourceEndpoint      = (srcEndpoint == 0 ? emberAfPrimaryEndpointForCurrentNetworkIndex() : srcEndpoint);
...
void cliBufferPrint(void)
{
    ...
    zclCmdIsBuilt    = true;
    emberAfGuaranteedPrintln("Msg: clus 0x%2x, cmd 0x%x, len %d", globalApsFrame.clusterId, appZclBuffer[cmdIndex],
                             appZclBufferLen);
...
void zclBufferSetup(uint8_t frameType, uint16_t clusterId, uint8_t commandId)
{
    ...
    appZclBuffer[index++] = (frameType | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER |
                             (mfgSpecificId != EMBER_AF_NULL_MANUFACTURER_CODE ? ZCL_MANUFACTURER_SPECIFIC_MASK : 0) |
                             (disableDefaultResponse ? ZCL_DISABLE_DEFAULT_RESPONSE_MASK : 0));
...
static void bufferAddLengthAndStringFromArgument(uint8_t argIndex, bool isLongStringType)
{
    ...
    uint8_t length = emberCopyStringArgument(argIndex, appZclBuffer + appZclBufferLen + prefixSize, maxLength, false);
...
void zclSimpleCommand(uint8_t frameControl, uint16_t clusterId, uint8_t commandId)
{
    ...
    for (argumentIndex = 0; argumentIndex < count; argumentIndex++)
    {
        ...
        if (typeIndex + 1 < (count + optionalDelimiters) && emberCurrentCommand->argumentTypes[typeIndex + 1] != '*')
        {
            typeIndex++;
        }
...
void zclGlobalSetup(uint8_t commandId)
{
    ...
    zclBufferSetup(ZCL_GLOBAL_COMMAND | zclGlobalDirection, clusterId, commandId);
...
{
    zclGlobalDirection =
        ((((uint8_t) emberUnsignedCommandArgument(0)) == ZCL_DIRECTION_CLIENT_TO_SERVER) ? ZCL_FRAME_CONTROL_CLIENT_TO_SERVER
...
void zclGlobalWriteCommand(void)
{
    ...
    uint8_t commandId =
        (commandChar == 'u' ? ZCL_WRITE_ATTRIBUTES_UNDIVIDED_COMMAND_ID
                            : (commandChar == 'n' ? ZCL_WRITE_ATTRIBUTES_NO_RESPONSE_COMMAND_ID : ZCL_WRITE_ATTRIBUTES_COMMAND_ID));
    ...
    else
    {
        ...
        emberCopyStringArgument(3, appZclBuffer + appZclBufferLen, length,
...
void zclGlobalReportCommand(void)
{
    ...
    status = emberAfReadAttribute((uint8_t) emberUnsignedCommandArgument(0), // endpoint
                                  clusterId, attributeId, (mask == 0 ? CLUSTER_MASK_CLIENT : CLUSTER_MASK_SERVER), data,
                                  sizeof(data), &type);
    ...
    zclBufferSetup(ZCL_GLOBAL_COMMAND | (mask == 0 ? ZCL_FRAME_CONTROL_CLIENT_TO_SERVER : ZCL_FRAME_CONTROL_SERVER_TO_CLIENT),
                   clusterId, ZCL_REPORT_ATTRIBUTES_COMMAND_ID);
...
void zclGlobalSendMeAReportCommand(void)
{
    ...
    if (emberAfGetAttributeAnalogOrDiscreteType(type) == EMBER_AF_DATA_TYPE_ANALOG)
    {
        uint8_t dataSize = emberAfGetDataSize(type);
        emberCopyStringArgument(5, appZclBuffer + appZclBufferLen, dataSize, false);
...
void zclGlobalCommandDiscoveryCommand(void)
{
    zclBufferSetup(ZCL_GLOBAL_COMMAND | zclGlobalDirection,
                   (EmberAfClusterId) emberUnsignedCommandArgument(0), // cluster id
                   (emberCurrentCommand->name[9] == 'g' ? ZCL_DISCOVER_COMMANDS_GENERATED_COMMAND_ID
...
void zclIdentifyOnOffCommand(void)
{
    ...
    afStatus = emberAfWriteServerAttribute((uint8_t) emberUnsignedCommandArgument(0), // endpoint
                                           ZCL_IDENTIFY_CLUSTER_ID, ZCL_IDENTIFY_TIME_ATTRIBUTE_ID, (uint8_t *) &timeS,
    ...
    if (EMBER_ZCL_STATUS_SUCCESS != afStatus)
    {
        emberAfDebugPrintln("Identify On/Off: failed to write value 0x%x  to cluster "
                            ...
                            timeS, ZCL_IDENTIFY_CLUSTER_ID, ZCL_IDENTIFY_TIME_ATTRIBUTE_ID, afStatus);
    }
}
#else
...
{
    uint8_t commandId = (emberCurrentCommand->name[2] == 'd' ? ZCL_ADD_GROUP_COMMAND_ID : ZCL_ADD_GROUP_IF_IDENTIFYING_COMMAND_ID);
...
void zclGroupsGetCommand(void)
{
    ...
    zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, ZCL_GROUPS_CLUSTER_ID,
...
{
    zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, ZCL_SCENES_CLUSTER_ID,
                   (emberCurrentCommand->name[0] != 'e' ? ZCL_ADD_SCENE_COMMAND_ID : ZCL_ENHANCED_ADD_SCENE_COMMAND_ID));
...
void zclScenesViewCommand(void)
{
    zclSimpleClientCommand(ZCL_SCENES_CLUSTER_ID,
                           (emberCurrentCommand->name[0] != 'e' ? ZCL_VIEW_SCENE_COMMAND_ID : ZCL_ENHANCED_VIEW_SCENE_COMMAND_ID));
}
...
void zclOnOffCommand(void)
{
    ...
    uint8_t commandId  = (secondChar == 'n' ? ZCL_ON_COMMAND_ID : (secondChar == 'f' ? ZCL_OFF_COMMAND_ID : ZCL_TOGGLE_COMMAND_ID));
...
{
    uint8_t commandId =
        (emberCurrentCommand->name[0] == 'o' ? ZCL_MOVE_TO_LEVEL_WITH_ON_OFF_COMMAND_ID : ZCL_MOVE_TO_LEVEL_COMMAND_ID);
...
{
    uint8_t commandId = (emberCurrentCommand->name[0] == 'o' ? ZCL_MOVE_WITH_ON_OFF_COMMAND_ID : ZCL_MOVE_COMMAND_ID);
    zclSimpleClientCommand(ZCL_LEVEL_CONTROL_CLUSTER_ID, commandId);
}
...
{
    uint8_t commandId = (emberCurrentCommand->name[0] == 'o' ? ZCL_STEP_WITH_ON_OFF_COMMAND_ID : ZCL_STEP_COMMAND_ID);
    zclSimpleClientCommand(ZCL_LEVEL_CONTROL_CLUSTER_ID, commandId);
}
...
{
    uint8_t commandId = (emberCurrentCommand->name[0] == 'o' ? ZCL_STOP_WITH_ON_OFF_COMMAND_ID : ZCL_STOP_COMMAND_ID);
    zclSimpleClientCommand(ZCL_LEVEL_CONTROL_CLUSTER_ID, commandId);
}
#endif // ZCL_USING_LEVEL_CONTROL_CLUSTER_CLIENT
...
void zclColorControlMoveToHueCommand(void)
{
    zclSimpleClientCommand(
        ...
        (emberCurrentCommand->name[0] != 'e' ? ZCL_MOVE_TO_HUE_COMMAND_ID : ZCL_ENHANCED_MOVE_TO_HUE_COMMAND_ID));
}
...
void zclColorControlMoveHueCommand(void)
{
    zclSimpleClientCommand(ZCL_COLOR_CONTROL_CLUSTER_ID,
                           (emberCurrentCommand->name[0] != 'e' ? ZCL_MOVE_HUE_COMMAND_ID : ZCL_ENHANCED_MOVE_HUE_COMMAND_ID));
}
...
{
    zclSimpleClientCommand(ZCL_COLOR_CONTROL_CLUSTER_ID,
                           (emberCurrentCommand->name[0] != 'e' ? ZCL_STEP_HUE_COMMAND_ID : ZCL_ENHANCED_STEP_HUE_COMMAND_ID));
}
...
{
    zclSimpleClientCommand(ZCL_COLOR_CONTROL_CLUSTER_ID,
                           (emberCurrentCommand->name[0] != 'e' ? ZCL_MOVE_TO_HUE_AND_SATURATION_COMMAND_ID
...
void zclColorControlMoveColorCommand(void)
{
    zclSimpleClientCommand(
        ZCL_COLOR_CONTROL_CLUSTER_ID,
        (emberCurrentCommand->argumentTypes[0] == 'r' ? ZCL_MOVE_COLOR_COMMAND_ID : ZCL_MOVE_COLOR_TEMPERATURE_COMMAND_ID));
}

// zcl color-control stepcolor <step x:2> <step y:2> <transition time:2>
// zcl color-control stepcolortemp <step mode:1> <step size:2> <transition time:2> <color temperature min:2> <color temperature
// max:2>
void zclColorControlStepColorCommand(void)
{
    zclSimpleClientCommand(
        ZCL_COLOR_CONTROL_CLUSTER_ID,
        (emberCurrentCommand->argumentTypes[0] == 'r' ? ZCL_STEP_COLOR_COMMAND_ID : ZCL_STEP_COLOR_TEMPERATURE_COMMAND_ID));
}
...
void zclPollControlSetPollIntervalCommand(void)
{
    zclSimpleClientCommand(
        ...
        (emberCurrentCommand->name[0] == 'l' ? ZCL_SET_LONG_POLL_INTERVAL_COMMAND_ID : ZCL_SET_SHORT_POLL_INTERVAL_COMMAND_ID));
}
#endif
...
{
    zclSimpleClientCommand(ZCL_POWER_PROFILE_CLUSTER_ID, ZCL_POWER_PROFILE_REQUEST_COMMAND_ID);
}

// zcl power-profile state
void zclPowerProfileStateRequestCommand(void)
{
    zclSimpleClientCommand(ZCL_POWER_PROFILE_CLUSTER_ID, ZCL_POWER_PROFILE_STATE_REQUEST_COMMAND_ID);
}

// zcl power-profile energy-phases-schedule <profile id:1> <num scheduled phases:1> [<energy phase ID:1> <scheduled time:2> * num
// scheduled phases]
void zclPowerProfileEnergyPhasesScheduleNotificationCommand(void)
{
    ...
    {
        zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, ZCL_POWER_PROFILE_CLUSTER_ID,
...
{
    zclSimpleClientCommand(ZCL_POWER_PROFILE_CLUSTER_ID, ZCL_POWER_PROFILE_SCHEDULE_CONSTRAINTS_REQUEST_COMMAND_ID);
}
...
{
    zclSimpleClientCommand(ZCL_POWER_PROFILE_CLUSTER_ID, ZCL_ENERGY_PHASES_SCHEDULE_STATE_REQUEST_COMMAND_ID);
}
...
{
    zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_SERVER_TO_CLIENT, ZCL_DEMAND_RESPONSE_LOAD_CONTROL_CLUSTER_ID,
...
{
    zclSimpleServerCommand(ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_MIRROR_REMOVED_COMMAND_ID);
}
...
{
    zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, ZCL_PRICE_CLUSTER_ID,
...
{
    zclSimpleClientCommand(ZCL_GENERIC_TUNNEL_CLUSTER_ID, ZCL_MATCH_PROTOCOL_ADDRESS_COMMAND_ID);
}
...
void zclBacnetTransferRandomCommand(void)
{
    ...
    zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, ZCL_BACNET_PROTOCOL_TUNNEL_CLUSTER_ID,
...
void zclDoorLockCommand(void)
{
    zclSimpleClientCommand(ZCL_DOOR_LOCK_CLUSTER_ID,
                           (emberCurrentCommand->name[0] == 'l' ? ZCL_LOCK_DOOR_COMMAND_ID : ZCL_UNLOCK_DOOR_COMMAND_ID));
}
...
{
    zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, ZCL_TUNNELING_CLUSTER_ID,
...
void zclTunnelingRandomToServerCommand(void)
{
    ...
    zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, ZCL_TUNNELING_CLUSTER_ID,
                   ZCL_TRANSFER_DATA_CLIENT_TO_SERVER_COMMAND_ID);
    zclBufferAddWordFromArgument(0); // tunnel id
    appZclBufferLen = (appZclBufferLen + length < APP_ZCL_BUFFER_SIZE ? appZclBufferLen + length : APP_ZCL_BUFFER_SIZE);
    globalApsFrame.options |= EMBER_APS_OPTION_SOURCE_EUI64;
    cliBufferPrint();
}
...
{
    zclBufferSetup(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_SERVER_TO_CLIENT, ZCL_TUNNELING_CLUSTER_ID,
...
void emAfCliSendCommand(void)
{
    ...
    if (emberAfPreCliSendCallback(&globalApsFrame, emberAfGetNodeId(), destination, appZclBuffer, appZclBufferLen))
    {
        return;
    }
    ...
    {
        label  = 'M';
        status = emberAfSendMulticast(destination, &globalApsFrame, appZclBufferLen, appZclBuffer);
    }
    else if (destination >= EMBER_BROADCAST_ADDRESS)
    {
        label  = 'B';
        status = emberAfSendBroadcast(destination, &globalApsFrame, appZclBufferLen, appZclBuffer);
    }
    else
    {
        label  = 'U';
        status = emberAfSendUnicast(EMBER_OUTGOING_DIRECT, destination, &globalApsFrame, appZclBufferLen, appZclBuffer);
    }
    ...
    UNUSED_VAR(label);
    emberAfDebugPrintln("T%4x:TX (%p) %ccast 0x%x%p", emberAfGetCurrentTime(), "CLI", label, status,
                        ((globalApsFrame.options & EMBER_APS_OPTION_ENCRYPTION) ? " w/ link key" : ""));
...
void emAfCliBsendCommand(void)
{
    ...
    emberAfCorePrintln("src ep %x, clus %2x", srcEndpointToUse, globalApsFrame.clusterId);
    ...
    if (useMulticastBinding)
    {
        ...
        status = emberAfSendMulticastToBindings(&globalApsFrame, appZclBufferLen, appZclBuffer);

        emberAfDebugPrintln("T%4x:TX (%p) %ccast 0x%x%p", emberAfGetCurrentTime(), "CLI", 'U', status,
                            ((globalApsFrame.options & EMBER_APS_OPTION_ENCRYPTION) ? " w/ link key" : ""));
        emberAfDebugPrint("TX buffer: [");
        emberAfDebugFlush();
        emberAfDebugPrintBuffer(appZclBuffer, appZclBufferLen, true);
    ...
    else
    {
        ...
        for (i = 0; i < EMBER_BINDING_TABLE_SIZE; i++)
        {
            ...
            if (status == EMBER_SUCCESS && candidate.type == EMBER_UNICAST_BINDING &&
                (srcEndpointToUse == 0 || candidate.local == srcEndpointToUse) && candidate.clusterId == globalApsFrame.clusterId)
            {
                emberAfCorePrintln("sending to bind %x", i);

                status = emberAfSendUnicast(EMBER_OUTGOING_VIA_BINDING, i, &globalApsFrame, appZclBufferLen, appZclBuffer);
...
void emAfCliTimesyncCommand(void)
{
    ...
    zclBufferSetup(ZCL_GLOBAL_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, ZCL_TIME_CLUSTER_ID, ZCL_READ_ATTRIBUTES_COMMAND_ID);
...
void interpanCommand(void)
{
    ...
    status = emberAfInterpanSendMessageCallback(&header, appZclBufferLen, appZclBuffer);
    if (status != EMBER_SUCCESS)
    {
        emberAfDebugPrintln("%psend failed: 0x%X", "ERR: Inter-PAN ", status);
    }
}
...
void interpanLongCommand(void)
{
    ...
    header.panId     = (uint16_t) emberUnsignedCommandArgument(1);
    header.profileId = (uint16_t) emberUnsignedCommandArgument(2);
    header.options   = ((uint16_t) emberUnsignedCommandArgument(3) | EMBER_AF_INTERPAN_OPTION_MAC_HAS_LONG_ADDRESS);
    header.clusterId = globalApsFrame.clusterId;
...
void emAfCliReadCommand(void)
{
    ...
    emberAfCorePrint("%p: ep: %d, cl: 0x%2X, attr: 0x%2X", "read", endpoint, cluster, attribute);
    emberAfCorePrintln(", svr: %c", (serverAttribute ? 'y' : 'n'));

    status = emberAfReadAttribute(endpoint, cluster, attribute, (serverAttribute ? CLUSTER_MASK_SERVER : CLUSTER_MASK_CLIENT), data,
...
void emAfCliWriteCommand(void)
{
    ...
    emberAfCorePrint("%p: ep: %d, cl: 0x%2X, attr: 0x%2X", "write", endpoint, cluster, attribute);
    emberAfCorePrintln(", svr: %c, dtype: 0x%X", (serverAttribute ? 'y' : 'n'), dataType);
    ...
    if (emberAfIsStringAttributeType(dataType))
    {
        ...
        data[0] = emberCopyStringArgument(5, data + 1, ATTRIBUTE_LARGEST - 1, false);
    }
    else if (emberAfIsLongStringAttributeType(dataType))
    {
        ...
        data[0] = emberCopyStringArgument(5, data + 2, ATTRIBUTE_LARGEST - 2, false);
        data[1] = 0;
    }
    else
    {
        emberCopyStringArgument(5, data, emberAfGetDataSize(dataType), false);
    }

    status = emberAfWriteAttribute(endpoint, cluster, attribute, (serverAttribute ? CLUSTER_MASK_SERVER : CLUSTER_MASK_CLIENT),
                                   data, dataType);
...
void eraseKeyTableEntry(uint8_t index)
{
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfCorePrintln("%perase key %d: 0x%x", "ERROR: ", index, status);
    }
}
...
{
    emberAfCorePrintln("%pSECURITY_LEVEL: %x", "EMBER_", EMBER_SECURITY_LEVEL);
    printKeyInfo();
}
...
{
    zclBufferSetup(ZCL_GLOBAL_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER, 0x0000, ZCL_READ_ATTRIBUTES_COMMAND_ID);
...
/** @} END addtogroup */
EmberCommandEntry zclGlobalCommands[] = { { "read", zclGlobalReadCommand, "vv" },
...
EmberCommandEntry zclTestResponseCommands[] = { { "on", zclTestResponseOnCommand, "" },
                                                { "off", zclTestResponseOffCommand, "" },
                                                { NULL } };

EmberCommandEntry zclTestCommands[] = { { "response", NULL, (const char *) zclTestResponseCommands }, { NULL } };

#ifdef ZCL_USING_BASIC_CLUSTER_CLIENT
EmberCommandEntry zclBasicCommands[] = { { "rtfd", zclBasicRtfdCommand, "" }, { NULL } };
#endif
...
#ifdef ZCL_USING_GROUPS_CLUSTER_CLIENT
EmberCommandEntry zclGroupsCommands[] = { { "add", zclGroupsAddCommand, "vb" },
...
#ifdef ZCL_USING_SCENES_CLUSTER_CLIENT
EmberCommandEntry zclScenesSetCommands[] = { { "on", zclScenesSetCommand, "u" }, { "off", zclScenesSetCommand, "u" }, { NULL } };

EmberCommandEntry zclScenesCommands[] = { { "add", zclScenesAddCommand, "vuvb" },
...
#ifdef ZCL_USING_ON_OFF_CLUSTER_CLIENT
EmberCommandEntry zclOnOffCommands[] = { { "on", zclOnOffCommand, "" },
                                         { "off", zclOnOffCommand, "" },
...
#ifdef ZCL_USING_LEVEL_CONTROL_CLUSTER_CLIENT
EmberCommandEntry zclLevelControlCommands[] = { { "mv-to-level", zclLevelControlMoveToLevelCommand, "uv" },
...
#ifdef ZCL_USING_THERMOSTAT_CLUSTER_CLIENT
EmberCommandEntry zclThermostatCommands[] = { { "set", zclThermostatSetCommand, "uu" }, { NULL } };
#endif

#ifdef ZCL_USING_IAS_ZONE_CLUSTER_SERVER
EmberCommandEntry zclIasZoneCommands[] = { { "enroll", zclIasZoneEnrollCommand, "vv" },
                                           { "sc", zclIasZoneStatusChangeCommand, "vuuv" },
                                           { NULL } };
#endif

#ifdef ZCL_USING_IAS_ACE_CLUSTER_CLIENT
EmberCommandEntry zclIasAceCommands[] = { { "a", zclIasAceArmCommand, "ubu" },
...
#ifdef ZCL_USING_COLOR_CONTROL_CLUSTER_CLIENT
EmberCommandEntry zclColorControlCommands[] = { { "movetohue", zclColorControlMoveToHueCommand, "uuv" },
...
EmberCommandEntry zclPowerProfileCommands[] = {
    emberCommandEntryAction("profile", zclPowerProfileRequestCommand, "u", "Request Profile"),
    emberCommandEntryAction("state", zclPowerProfileStateRequestCommand, "", "Request State"),
    emberCommandEntryAction("energy-phases-schedule", zclPowerProfileEnergyPhasesScheduleNotificationCommand, "?",
                            "Notify Energy Phase Schedule"),
    emberCommandEntryAction("schedule-constraints", zclPowerProfileScheduleConstraintsRequestCommand, "u",
                            "Request Schedule Constraints"),
    emberCommandEntryAction("energy-phases-schedule-states", zclPowerProfileEnergyPhasesScheduleStateRequestCommand, "u",
                            "Request Energy Phases Schedule States"),
    emberCommandEntryTerminator(),
};
#endif // ZCL_USING_POWER_PROFILE_CLUSTER_CLIENT
...
EmberCommandEntry zclWindowCoveringCommands[] = {
    emberCommandEntryAction("up", zclWindowCoveringUpCommand, "", "Up"),
    emberCommandEntryAction("down", zclWindowCoveringDownCommand, "", "Down"),
    emberCommandEntryAction("stop", zclWindowCoveringStopCommand, "", "Stop"),
    emberCommandEntryAction("go-to-lift-value", zclWindowCoveringGoToLiftValueCommand, "v", "Go To Lift Value"),
    emberCommandEntryAction("go-to-lift-percent", zclWindowCoveringGoToLiftPercentageCommand, "u", "Go To Lift Percentage"),
    emberCommandEntryAction("go-to-tilt-value", zclWindowCoveringGoToTiltValueCommand, "v", "Go To Tilt Value"),
    emberCommandEntryAction("go-to-tilt-percentage", zclWindowCoveringGoToTiltPercentageCommand, "u", "Go To Tilt Percentage"),
    emberCommandEntryTerminator(),
};
#endif // ZCL_USING_WINDOW_COVERING_CLUSTER_CLIENT
...
#ifdef ZCL_USING_PRICE_CLUSTER_CLIENT
EmberCommandEntry zclPriceClientCommands[] = { { "cu", zclPriceGetCurrentCommand, "" },
...
EmberCommandEntry zclBacnetCommands[] = { { "transfer-npdu", NULL, (const char *) zclBacnetTransferCommands }, { NULL } };
#endif

#ifdef ZCL_USING_DOOR_LOCK_CLUSTER_CLIENT
EmberCommandEntry zclDoorLockCommands[] = { { "lock", zclDoorLockCommand, "" }, { "unlock", zclDoorLockCommand, "" }, { NULL } };
#endif // ZCL_USING_DOOR_LOCK_CLUSTER_CLIENT
...
EmberCommandEntry interpanCommands[] = {
    { "group", interpanCommand, "vvv" }, { "short", interpanCommand, "vvv" }, { "long", interpanLongCommand, "bvvv" }, { NULL }
};
