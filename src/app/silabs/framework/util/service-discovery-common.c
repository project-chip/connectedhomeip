...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Service discovery code that is common to
                                                                               *different types of service discovery, e.g. match
                                                                               *descriptor, NWK address lookup, and IEEE address
                                                                               * lookup.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
#define MAX_SIMPLE_DESCRIPTOR_RESPONSE_PAYLOAD                                                                                     \
    (128                                  /* 802.15.4 MTU */                                                                       \
     ...
     - 8                                  /* APS header */                                                                         \
     - EMBER_AF_ZDO_RESPONSE_OVERHEAD - 3 /* Active Endpoint Response overhead: Address(2), length(1) */                           \
    )

#define SIMPLE_DESCRIPTOR_RESPONSE_ENDPOINT_OFFSET                                                                                 \
    (EMBER_AF_ZDO_RESPONSE_OVERHEAD + 2 /* address of interest */                                                                  \
     + 1                                /* length value */                                                                         \
    )
...
static void setupDiscoveryData(ServiceDiscoveryState * state, EmberNodeId messageDest, EmberAfServiceDiscoveryCallback * callback,
                               uint16_t zdoClusterId, uint8_t sequenceNumber);
                                                  ...
                                                  bool serverCluster, EmberAfServiceDiscoveryCallback * callback)
{
    ...
    if (EMBER_BROADCAST_ADDRESS <= target && target != EMBER_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS)
    {
        ...
        emberAfServiceDiscoveryPrintln("%pIllegal broadcast address, remapping to valid one.", PREFIX);
    ...
    {
        emberAfServiceDiscoveryPrintln("%pFailed to send match discovery: 0x%x", PREFIX, status);
        return status;
    }

    emberAfServiceDiscoveryPrintln("%pStarting discovery for cluster 0x%2x", PREFIX, clusterId);
    setupDiscoveryData(state, target, callback, MATCH_DESCRIPTORS_REQUEST, sequenceNumber);
...
EmberStatus emberAfFindActiveEndpoints(EmberNodeId target, EmberAfServiceDiscoveryCallback * callback)
{
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfServiceDiscoveryPrintln("%pFailed to send active endpoint request: 0x%x", PREFIX, status);
        return status;
    }

    setupDiscoveryData(state, target, callback, ACTIVE_ENDPOINTS_REQUEST, sequenceNumber);
    return status;
}
                                                   ...
                                                   EmberAfServiceDiscoveryCallback * callback)
{
    ...
    status = emberSimpleDescriptorRequest(target, targetEndpoint, EMBER_AF_DEFAULT_APS_OPTIONS);
    // Get the sequence number sent in the request.
    uint8_t sequenceNumber = emberGetLastAppZigDevRequestSequence();

    if (status != EMBER_SUCCESS)
    {
        emberAfServiceDiscoveryPrintln("%pFailed to send simple descriptor request: 0x%x", PREFIX, status);
        return status;
    }

    setupDiscoveryData(state, target, callback, SIMPLE_DESCRIPTOR_REQUEST, sequenceNumber);
...
EmberStatus emberAfFindIeeeAddress(EmberNodeId shortAddress, EmberAfServiceDiscoveryCallback * callback)
{
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfServiceDiscoveryPrintln("%pFailed to send IEEE address request: 0x%x", PREFIX, status);
        return status;
    }

    setupDiscoveryData(state, shortAddress, callback, IEEE_ADDRESS_REQUEST, sequenceNumber);
...
EmberStatus emberAfFindNodeId(EmberEUI64 longAddress, EmberAfServiceDiscoveryCallback * callback)
{
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfServiceDiscoveryPrintln("%pFailed to send NWK address request: 0x%x", PREFIX, status);
        return status;
    }

    setupDiscoveryData(state, EMBER_BROADCAST_ADDRESS, callback, NETWORK_ADDRESS_REQUEST, sequenceNumber);

    return status;
}

static void setupDiscoveryData(ServiceDiscoveryState * state, EmberNodeId messageDest, EmberAfServiceDiscoveryCallback * callback,
                               uint16_t zdoClusterRequest, uint8_t sequenceNumber)
{
    state->active      = true;
    ...
    emberAfServiceDiscoveryPrintln("%pWaiting %d sec for discovery to complete", PREFIX, EMBER_AF_DISCOVERY_TIMEOUT_QS >> 2);
    emberAfNetworkEventControlSetDelayQS(emAfServiceDiscoveryEventControls[state->stateIndex], EMBER_AF_DISCOVERY_TIMEOUT_QS);
...
static void serviceDiscoveryComplete(ServiceDiscoveryState * state)
{
    ...
    if (state->callback != NULL)
    {
        ...
        result.status =
            (isUnicastQuery(state) ? EMBER_AF_UNICAST_SERVICE_DISCOVERY_TIMEOUT : EMBER_AF_BROADCAST_SERVICE_DISCOVERY_COMPLETE);
...
static void executeCallback(ServiceDiscoveryState * state, const EmberAfServiceDiscoveryResult * result)
{
    (*state->callback)(result);
    if (isUnicastQuery(state) || result->status == EMBER_AF_BROADCAST_SERVICE_DISCOVERY_COMPLETE_WITH_RESPONSE)
    {
...
static bool processMatchDescriptorResponse(ServiceDiscoveryState * state, const uint8_t * message, uint16_t length)
{
    EmberNodeId matchId;
    uint8_t listLength;
    ...
    {
        emberAfServiceDiscoveryPrintln("%pMessage too short for num. endpoints", PREFIX);
        return true;
    }

    emberAfServiceDiscoveryPrintln("%pMatch%p found from 0x%2x.", PREFIX, (listLength > 0 ? "" : " NOT"), matchId);
    ...
    if (listLength)
    {
        result.status = (isUnicastQuery(state) ? EMBER_AF_UNICAST_SERVICE_DISCOVERY_COMPLETE_WITH_RESPONSE
    ...
    else
    {
        ...
        result.status = (isUnicastQuery(state) ? EMBER_AF_UNICAST_SERVICE_DISCOVERY_COMPLETE_WITH_EMPTY_RESPONSE
...
static bool processSimpleDescriptorResponse(ServiceDiscoveryState * state, const uint8_t * message, uint16_t length)
{
    EmberAfServiceDiscoveryResult result;
    EmberAfClusterList clusterList;

    clusterList.profileId =
        message[SIMPLE_DESCRIPTOR_RESPONSE_PROFILE_ID_OFFSET] + (message[SIMPLE_DESCRIPTOR_RESPONSE_PROFILE_ID_OFFSET + 1] << 8);
    clusterList.deviceId =
        message[SIMPLE_DESCRIPTOR_RESPONSE_DEVICE_ID_OFFSET] + (message[SIMPLE_DESCRIPTOR_RESPONSE_DEVICE_ID_OFFSET + 1] << 8);
    ...
    clusterList.outClusterCount =
        message[SIMPLE_DESCRIPTOR_RESPONSE_INPUT_CLUSTER_LIST_COUNT_INDEX + 1 + (clusterList.inClusterCount * 2)];

    if ((length - index - 1) % 2 != 0)
    { // subtract 1 for the output cluster count.
...
static bool processAddressResponse(ServiceDiscoveryState * state, const uint8_t * message, uint16_t length)
{
    EmberAfServiceDiscoveryResult result;
    ...
    result.matchAddress        = (message[ADDRESS_RESPONSE_NODE_ID_OFFSET] + (message[ADDRESS_RESPONSE_NODE_ID_OFFSET + 1] << 8));
    result.zdoRequestClusterId = getRequestCluster(state);
...
static bool processActiveEndpointResponse(ServiceDiscoveryState * state, const uint8_t * message, uint16_t length)
{
    EmberAfServiceDiscoveryResult result;
    ...
    result.matchAddress =
        (message[ACTIVE_ENDPOINT_RESPONSE_NODE_ID_OFFSET] + (message[ACTIVE_ENDPOINT_RESPONSE_NODE_ID_OFFSET + 1] << 8));
    ...
    {
        emberAfServiceDiscoveryPrintln("Error: %p invalid length (%d != %d) for Active Endpoint response", PREFIX, expectedLength,
                                       length);
        return true;
    }
...
bool emAfServiceDiscoveryIncoming(EmberNodeId sender, EmberApsFrame * apsFrame, const uint8_t * message, uint16_t length)
{
    ...
    if (!(serviceDiscoveryInProgress(state) &&
          (apsFrame->profileId == EMBER_ZDO_PROFILE_ID
           ...
           && (apsFrame->clusterId == (CLUSTER_ID_RESPONSE_MINIMUM | getRequestCluster(state))))))
    {
        return false;
    }
...
static ServiceDiscoveryState * findStateForResponse(uint8_t sequenceNumber, uint8_t networkIndex)
{
    ...
    for (int si = 0; si < stateLimit; si++)
    {
        if (states[si][networkIndex].requestSequence == sequenceNumber && states[si][networkIndex].active)
        {
