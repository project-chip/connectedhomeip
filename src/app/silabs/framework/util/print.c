...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Utilities and command line interface for
                                                                               *printing, and enabling/disabling printing to
                                                                               *different areas.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static uint16_t convertUserNumberAreaToInternalArea(uint16_t userNumberedArea)
{
    ...
    index          = (uint16_t)(index << 8) + (uint16_t)(((uint16_t) 0x0001) << (userNumberedArea % 8));
    return index;
}
...
static void reallyPrintAreaName(uint16_t area)
{
    ...
    if (area != 0xFFFF && index < EMBER_AF_PRINT_NAME_NUMBER)
    {
...
static void emAfPrintInternalVarArg(uint16_t area, bool newLine, const char * formatString, va_list ap)
{
...
void emberAfPrintStatus(void)
{
    ...
    {
        emberSerialPrintfLine(EMBER_AF_PRINT_OUTPUT, "[%d] %p : %p", i, areaNames[i],
                              (emberAfPrintEnabled(convertUserNumberAreaToInternalArea(i)) ? "YES" : "no"));
