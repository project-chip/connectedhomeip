...
void emberAfStackDown(void)
{
    ...
    if (emberStackIsPerformingRejoin() == false && emberNetworkState() == EMBER_NO_NETWORK)
    {
...
EmberStatus emberAfSendResponseWithCallback(EmberAfMessageSentFunction callback)
{
    ...
    {
        label  = 'B';
        status = emberAfSendBroadcastWithCallback(emberAfResponseDestination, &emberAfResponseApsFrame, appResponseLength,
                                                  appResponseData, callback);
    }
    UNUSED_VAR(label);
