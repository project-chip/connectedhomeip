...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Utilities for printing in common formats:
                                                                               *buffers, EUI64s, keys
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static void emAfPrintBuffer(uint16_t area, const uint8_t * buffer, uint16_t bufferLen, const char * formatString)
{
...
{
    emberAfPrint(emberAfPrintActiveArea, "(%c)%X%X%X%X%X%X%X%X", '<', eui64[0], eui64[1], eui64[2], eui64[3], eui64[4], eui64[5],
                 eui64[6], eui64[7]);
}

void emberAfPrintBigEndianEui64(const EmberEUI64 eui64)
{
    emberAfPrint(emberAfPrintActiveArea, "(%c)%X%X%X%X%X%X%X%X", '>', eui64[7], eui64[6], eui64[5], eui64[4], eui64[3], eui64[2],
                 eui64[1], eui64[0]);
}
...
void emberAfPrint8ByteBlocks(uint8_t numBlocks, const uint8_t * block, bool crBetweenBlocks)
{
    ...
    for (i = 0; i < numBlocks; i++)
    {
        ...
        emberAfPrint(emberAfPrintActiveArea, " %p", (crBetweenBlocks || ((i + 1) == numBlocks) ? "\r\n" : ""));
    }
}
...
void emberAfPrintChannelListFromMask(uint32_t channelMask)
{
    if (emberAfPrintEnabled(emberAfPrintActiveArea))
    {
        ...
        for (i = 0; i < EMBER_MAX_CHANNELS_PER_PAGE; i++)
        {
