...
{
    emberAfSetExternalBuffer(appResponseData, EMBER_AF_RESPONSE_BUFFER_LEN, &appResponseLength, &emberAfApsResponseFrame);
}
                                    ...
                                    uint8_t * dataPtr, EmberAfAttributeType dataType)
{
    ...
    AttributeCallbackData data = {
        false, // read?
        endpoint,  cluster, attributeID, dataPtr,
        0, // max length
        &dataType,
    };
                                   ...
                                   uint8_t * dataPtr, uint8_t readLength, EmberAfAttributeType * dataType)
{
    ...
    AttributeCallbackData data = {
        true, // read?
        endpoint, cluster, attributeID, dataPtr, readLength, dataType,
    };
    return attributeCallbackFunction(&data);
}
...
EmberStatus emberAfDeactivateClusterTick(uint8_t endpoint, uint16_t clusterId, bool isClient)
{
...
void sendZclMessage(bool request)
{
    ...
    uint8_t sequenceNumber = (request ? emberAfNextSequence() : 0); // last sequence number.  Should default to 0
...
void emberAfFormatMfgString(uint8_t * mfgString)
{
    ...
    MEMMOVE(mfgString, string, strlen((char *) string));
}
...
static void printMessage(const char * prefix, bool debug, uint16_t length, const uint8_t * message)
{
                            ...
                            uint8_t * actualData, const char * file, int lineNumber)
{
    ...
    if (expectedLength > MAX_DATA_VERIFY_LENGTH || actualLength > MAX_DATA_VERIFY_LENGTH)
    {
...
void verifyMessage(const char * name, uint16_t length, const uint8_t * message)
{
    ...
    if (length != messageToSend.length || 0 != MEMCOMPARE(message, messageToSend.message, length))
    {
    ...
    debug("[Verified TX of %s] Dest: 0x%2X, Profile: 0x%2X, Cluster: 0x%2X, Length: %d\n", name, messageToSend.destination,
          messageToSend.apsFrame.profileId, messageToSend.apsFrame.clusterId, messageToSend.length);
    ...
    if (messageToSend.callback)
    {
        ...
        (messageToSend.callback)(
            ((emberAfResponseDestination < EMBER_BROADCAST_ADDRESS) ? EMBER_OUTGOING_DIRECT : EMBER_OUTGOING_BROADCAST),
            messageToSend.destination, &(messageToSend.apsFrame), messageToSend.length, messageToSend.message, EMBER_SUCCESS);
    }
...
static void testSend(EmberNodeId destination, EmberApsFrame * apsFrame, uint16_t messageLength, uint8_t * message,
                     EmberAfMessageSentFunction callback)
{
    ...
    debug("[TX] Dest: 0x%2X, Profile: 0x%2X, Cluster: 0x%2X, EP: %d->%d, Length: %d\n", messageToSend.destination,
          messageToSend.apsFrame.profileId, messageToSend.apsFrame.clusterId, messageToSend.apsFrame.sourceEndpoint,
          messageToSend.apsFrame.destinationEndpoint, messageToSend.length);
    printMessage("Message: ", true, messageToSend.length, messageToSend.message);
}
...
{
    testSend(indexOrDestination, apsFrame, messageLength, message, callback);
    return EMBER_SUCCESS;
}
