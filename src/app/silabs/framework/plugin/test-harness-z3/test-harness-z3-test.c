...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Unit test for ZigBee 3.0 Test Harness
                                                                               *functionality.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static uint8_t myExtPanId[] = {
    0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA,
};
static EmberEUI64 trustCenterEui64 = {
...
PRINTER(cliCall)
{
    ...
    fprintf(stderr, " running function %s with format %s and args", functionName, format);
...
#define addCliCallAction(function, format, args)                                                                                   \
    ...
    addAction(&cliCallActionType, (function), #function, (format), (args))
...
static bool checkZllChannelMasks(uint32_t primary, uint32_t secondary)
{
    if (scriptDebug)
    {
        fprintf(stderr, "[checking primary (0x%08X) and secondary (0x%08X) masks]", primary, secondary);
    }

    return (emAfZllPrimaryChannelMask == primary && emAfZllSecondaryChannelMask == secondary);
}
...
{
    functionCallCheck("emberZllStartScan", "ii", channelMask, nodeType);
    return EMBER_SUCCESS;
}

EmberStatus emberZllJoinTarget(const EmberZllNetwork * zllNetwork)
{
    functionCallCheck("emberZllJoinTarget", "ii", zllNetwork->securityAlgorithm.transactionId, zllNetwork->nodeType);
    return EMBER_SUCCESS;
}
...
static EmberTokTypeStackZllData zllToken = {
    0,
};
...
{
    functionCallCheck("emberZllSetTokenStackZllData", "iiiii", data->bitmask, data->freeNodeIdMin, data->freeNodeIdMax,
                      data->freeGroupIdMin, data->freeGroupIdMax);
...
{
    functionCallCheck("interpanOut", "ipiiip", panId, makeMessage("s", destinationLongId, EUI64_SIZE), destinationShortId,
                      multicastId, profileId, makeMessage("s", messageBytes, messageLength));
    return EMBER_SUCCESS;
}

#define addInterpanOutCheck(panId, destLongId, destShortId, multicastId, profileId, messageBytes)                                  \
    addSimpleCheck("interpanOut", "ipiiip", (panId), (destLongId), (destShortId), (multicastId), (profileId), (messageBytes))
...
EmberStatus emberSendBroadcast(EmberNodeId destination, EmberApsFrame * apsFrame, uint8_t radius, EmberMessageBuffer message)
{
    ...
    functionCallCheck("apsOut", "iiiiiip", destination, apsFrame->profileId, apsFrame->clusterId, apsFrame->sourceEndpoint,
                      apsFrame->destinationEndpoint, ((apsFrame->options & EMBER_APS_OPTION_ENCRYPTION) ? true : false),
                      messageParcel);
                             ...
                             EmberMessageBuffer message)
{
    ...
    functionCallCheck("apsOut", "iiiiiip", indexOrDestination, apsFrame->profileId, apsFrame->clusterId, apsFrame->sourceEndpoint,
                      apsFrame->destinationEndpoint, ((apsFrame->options & EMBER_APS_OPTION_ENCRYPTION) ? true : false),
                      messageParcel);
...
#define addApsOutCheck(destination, profileId, clusterId, sourceEndpoint, destinationEndpoint, encryption, message)                \
    addSimpleCheck("apsOut", "iiiiiip", (destination), (profileId), (clusterId), (sourceEndpoint), (destinationEndpoint),          \
                   (encryption), (message))

#define addZdoOutCheck(destination, clusterId, message)                                                                            \
    addApsOutCheck((destination), EMBER_ZDO_PROFILE_ID, (clusterId), EMBER_ZDO_ENDPOINT, EMBER_ZDO_ENDPOINT, false, (message))
...
EmberStatus emberSendRawMessage(EmberMessageBuffer message)
{
    ...
    functionCallCheck("rawMessageOut", "p", messageParcel);
...
#define addRawMessageOutCheck(message) addSimpleCheck("rawMessageOut", "p", (message))

// -----------------------------------------------------------------------------
// NETWORK OUT CHECK.

bool emNetworkSendCommand(EmberNodeId destination, uint8_t * commandFrame, uint8_t length, bool tryToInsertLongDest,
                          EmberEUI64 destinationEui)
{
    functionCallCheck("networkOut", "pii", makeMessage("s", commandFrame, length), destination, tryToInsertLongDest);

    return true;
}

#define addNetworkOutCheck(data, dstShort, tryToInsertLongDest)                                                                    \
    addSimpleCheck("networkOut", "pii", (data), (dstShort), (tryToInsertLongDest))

// -----------------------------------------------------------------------------
// NETWORK OUT CHECK.

bool emSendApsCommand(EmberNodeId destination, EmberEUI64 longDestination, EmberMessageBuffer payload, uint8_t options)
{
    functionCallCheck("emSendApsCommand", "ippi", destination, makeMessage("s", longDestination, EUI64_SIZE),
                      bufferToParcel(payload), options);

    return true;
}

#define addApsCommandOutCheck(destShort, destLong, payload, options)                                                               \
    addSimpleCheck("emSendApsCommand", "ippi", (destShort), (destLong), (payload), (options))
...
{
    functionCallCheck("emberAddOrUpdateKeyTableEntry", "pp", makeMessage("s", address, EUI64_SIZE),
                      makeMessage("s", emberKeyContents(keyData), EMBER_ENCRYPTION_KEY_SIZE));
    return EMBER_SUCCESS;
}

EmberStatus emSetKeyTableEntry(bool erase, uint8_t index, EmberEUI64 address, EmberKeyData * keyData)
{
    return EMBER_SUCCESS;
}

#define addSetKeyCheck(partner, keyData) addSimpleCheck("emberAddOrUpdateKeyTableEntry", "pp", partner, keyData)
...
static void macCommandsTest(void)
{
    ...
    addCliCallAction(emAfPluginTestHarnessZ3BeaconBeaconReqCommand, "", makeParcel(0));
    addSimpleCheck("emberStartScan", "ii", EMBER_ACTIVE_SCAN, BIT32(MY_CHANNEL));

    addSimpleAction("setGetNetworkParametersReturn(0x%02X)", setGetNetworkParametersReturn, 1, EMBER_ERR_FATAL);
    addCliCallAction(emAfPluginTestHarnessZ3BeaconBeaconReqCommand, "", makeParcel(0));
    addSimpleCheck("emberStartScan", "ii", EMBER_ACTIVE_SCAN, emAfPluginNetworkSteeringPrimaryChannelMask);

    addSimpleAction("setGetNetworkParametersReturn(0x%02X)", setGetNetworkParametersReturn, 1, EMBER_SUCCESS);

    // plugin test-harness z3 beacon beacon-config <options:4>
    addCliCallAction(emAfPluginTestHarnessZ3BeaconBeaconsConfigCommand, "1", makeMessage("1", 0));
    addRawMessageOutCheck(makeMessage("<21<2<2<211111p1111",
                                      ...
                                      0x84,       // capacity bits
                                      makeMessage("s", myExtPanId, EXTENDED_PAN_ID_SIZE), 0xFF, 0xFF, 0xFF, // tx offset
...
static void nwkCommandsTest(void)
{
    ...
    addCliCallAction(emAfPluginTestHarnessZ3NwkNwkRejoinRequestCommand, "2", makeMessage("2", remoteNodeId));
    addNetworkOutCheck(makeMessage("11", 0x06, BIT(1) | BIT(2) | BIT(3) | BIT(7)), remoteNodeId, true);

    // plugin test-harness z3 nwk nwk-rejoin-response <networkAddress:2>
    // <rejoinStatus:1> <dstShort:2>
    addCliCallAction(emAfPluginTestHarnessZ3NwkNwkRejoinResponseCommand, "212", makeMessage("212", 0x1234, WHATEVER, remoteNodeId));
    addNetworkOutCheck(makeMessage("1<21", 0x07, 0x1234, WHATEVER), remoteNodeId, true);

    // plugin test-harness z3 nwk nwk-leave <rejoin:1> <request:1>
    // <removeChildren:1> <dstShort:2> <options:4>
    addCliCallAction(emAfPluginTestHarnessZ3NwkNwkLeaveCommand, "11124", makeMessage("11124", true, false, false, remoteNodeId, 0));
    addNetworkOutCheck(makeMessage("11", 0x04, BIT(5)), remoteNodeId, true);

    addCliCallAction(emAfPluginTestHarnessZ3NwkNwkLeaveCommand, "11124",
                     makeMessage("11124", false, true, false, remoteNodeId, BIT32(0)));
    addNetworkOutCheck(makeMessage("11", 0x04, BIT(6)), remoteNodeId, false);
...
static void apsCommandsTest(void)
{
    ...
    addCliCallAction(emAfPluginTestHarnessZ3ApsApsRemoveDevice, "881",
                     makeMessage("ss",
                                 ...
                                 EUI64_SIZE, 0));
    addSimpleCheck("emberSendRemoveDevice", "ipp", remoteNodeId, makeMessage("s", remoteEui64, EUI64_SIZE),
    ...
    addSetKeyCheck(makeMessage("s", remoteEui64, EUI64_SIZE),
                   makeMessage("s", emberKeyContents(&zigbeeAlliance09), EMBER_ENCRYPTION_KEY_SIZE));
    addSimpleCheck("emberSendRemoveDevice", "ipp", remoteNodeId, makeMessage("s", remoteEui64, EUI64_SIZE),
                   makeMessage("s", remoteEui64, EUI64_SIZE));

    // plugin test-harness z3 aps aps-request-key <dstShort:2> <keyType:1>
    // <parentLong:8> <options:4>
    addCliCallAction(emAfPluginTestHarnessZ3ApsApsRequestKeyCommand, "2184",
                     makeMessage("21s4", remoteNodeId,
                                 0x04, // trust center link key type
                                 remoteEui64, EUI64_SIZE, 0));
    addApsCommandOutCheck(remoteNodeId, makeMessage("s", trustCenterEui64, EUI64_SIZE),
                          makeMessage("11",
    ...
    addSetKeyCheck(makeMessage("s", trustCenterEui64, EUI64_SIZE),
                   makeMessage("s", emberKeyContents(&zigbeeAlliance09), EMBER_ENCRYPTION_KEY_SIZE));
    addApsCommandOutCheck(remoteNodeId, makeMessage("s", trustCenterEui64, EUI64_SIZE),
                          makeMessage("11",
                                      0x08,  // aps request key command
...
static void zllCommandsTest(void)
{
    ...
    addCliCallAction(emAfPluginTestHarnessZ3SetDeviceModeCommand, "1",
                     makeMessage("1", EM_AF_PLUGIN_TEST_HARNESS_Z3_DEVICE_MODE_ZR_ADDRESS_ASSIGNABLE));
    addSimpleAction("checkDeviceMode(0x%02X)", checkDeviceMode, 1, EM_AF_PLUGIN_TEST_HARNESS_Z3_DEVICE_MODE_ZR_ADDRESS_ASSIGNABLE);

    // plugin test-harness z3 set-device-mode <mode:1>
    // plugin test-harness z3 touchlink scan-request-process <linkInitiator:1>
    // <unused:1> <options:4>
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkScanRequestProcessCommand, "114", makeMessage("114", 0, 0, BIT(3)));
    addSimpleCheck("emberZllSetPolicy", "i", EMBER_ZLL_POLICY_DISABLED);

    addCliCallAction(emAfPluginTestHarnessZ3SetDeviceModeCommand, "1",
                     makeMessage("1", EM_AF_PLUGIN_TEST_HARNESS_Z3_DEVICE_MODE_ZR_NOT_ADDRESS_ASSIGNABLE));
    addSimpleAction("checkDeviceMode(0x%02X)", checkDeviceMode, 1,
                    EM_AF_PLUGIN_TEST_HARNESS_Z3_DEVICE_MODE_ZR_NOT_ADDRESS_ASSIGNABLE);
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkScanRequestProcessCommand, "114", makeMessage("114", 0, 0, 0));
    addSimpleCheck("emberZllSetPolicy", "i", EMBER_ZLL_POLICY_ENABLED);
    addSimpleCheck("emberZllGetTokenStackZllData", "");
    addSimpleCheck("emberZllSetTokenStackZllData", "iiiii", zllToken.bitmask & ~EMBER_ZLL_STATE_ADDRESS_ASSIGNMENT_CAPABLE,
                   zllToken.freeNodeIdMin, zllToken.freeNodeIdMin, zllToken.freeGroupIdMin, zllToken.freeGroupIdMax);
    addSimpleCheck("emberZllStartScan", "ii", emAfZllPrimaryChannelMask | emAfZllSecondaryChannelMask, EMBER_ROUTER);
    addFunctionCheck("checkZllChannelMasks(0x%08X, 0x%08X)", true, checkZllChannelMasks, 2, EMBER_ZLL_PRIMARY_CHANNEL_MASK,
                     EMBER_ZLL_SECONDARY_CHANNEL_MASK);

    addCliCallAction(emAfPluginTestHarnessZ3SetDeviceModeCommand, "1",
                     makeMessage("1", EM_AF_PLUGIN_TEST_HARNESS_Z3_DEVICE_MODE_ZR_ADDRESS_ASSIGNABLE));
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkScanRequestProcessCommand, "114",
                     makeMessage("114", 0, 0, BIT32(1))); // scan current channel
    addSimpleCheck("emberZllSetPolicy", "i", EMBER_ZLL_POLICY_ENABLED);
    addSimpleCheck("emberZllStartScan", "ii", BIT32(MY_CHANNEL), EMBER_ROUTER);
    addFunctionCheck("checkZllChannelMasks(0x%08X, 0x%08X)", true, checkZllChannelMasks, 2,
    ...
    addSimpleCheck("emberZllSetPolicy", "i", EMBER_ZLL_POLICY_ENABLED);
    addSimpleCheck("emberZllStartScan", "ii", EMBER_ZLL_SECONDARY_CHANNEL_MASK, EMBER_ROUTER);
    addFunctionCheck("checkZllChannelMasks(0x%08X, 0x%08X)", true, checkZllChannelMasks, 2,
                     0,                                 // primary channel mask
                     EMBER_ZLL_SECONDARY_CHANNEL_MASK); // secondary channel mask

    addCliCallAction(emAfPluginTestHarnessZ3SetDeviceModeCommand, "1",
                     makeMessage("1", EM_AF_PLUGIN_TEST_HARNESS_Z3_DEVICE_MODE_ZED_ADDRESS_ASSIGNABLE));
    ...
    // process above.
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkDeviceInformationRequestCommand, "14", makeMessage("14", 0x00, 0));
    addSimpleAction("callScanCompleteCallbacks", callScanCompleteCallbacks, 0);
    ...
    addInterpanOutCheck(0xFFFF, // dst pan id
                        ...
                        makeMessage("111<41", EM_AF_PLUGIN_TEST_HARNESS_Z3_ZLL_CLIENT_TO_SERVER_FRAME_CONTROL,
                                    0x00, // zcl sequence number
                                    ZCL_DEVICE_INFORMATION_REQUEST_COMMAND_ID, MY_TRANSACTION_ID,
                                    0x00)); // start index

    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkDeviceInformationRequestCommand, "14", makeMessage("14", 0x00, BIT(0)));
    addInterpanOutCheck(0xFFFF, // dst pan id
                        makeMessage("s", remoteEui64, EUI64_SIZE),
                        EMBER_NULL_NODE_ID, // node id - ignored
                        0x0000,             // group id - ignored
                        ...
                        makeMessage("111<41", EM_AF_PLUGIN_TEST_HARNESS_Z3_ZLL_CLIENT_TO_SERVER_FRAME_CONTROL,
                                    0x00, // zcl sequence number
                                    ZCL_DEVICE_INFORMATION_REQUEST_COMMAND_ID, MY_TRANSACTION_ID - 1,
                                    0x00)); // start index

    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkDeviceInformationRequestCommand, "14", makeMessage("14", 0xDD, 0x00));
    addInterpanOutCheck(0xFFFF, // dst pan id
                        makeMessage("s", remoteEui64, EUI64_SIZE),
                        EMBER_NULL_NODE_ID, // node id - ignored
                        0x0000,             // group id - ignored
    ...
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkIdentifyRequestCommand, "24", makeMessage("24", 0x4321, 0));
    addInterpanOutCheck(0xFFFF, // dst pan id
                        makeMessage("s", remoteEui64, EUI64_SIZE),
                        EMBER_NULL_NODE_ID, // node id - ignored
                        0x0000,             // group id - ignored
                        EMBER_ZLL_PROFILE_ID,
                        makeMessage("111<4<2", EM_AF_PLUGIN_TEST_HARNESS_Z3_ZLL_CLIENT_TO_SERVER_FRAME_CONTROL,
                                    0x00, // zcl sequence number
                                    ZCL_IDENTIFY_REQUEST_COMMAND_ID, MY_TRANSACTION_ID,
                                    0x4321)); // identify duration

    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkIdentifyRequestCommand, "24", makeMessage("24", 0x4321, BIT(0)));
    addInterpanOutCheck(0xFFFF, // dst pan id
                        makeMessage("s", remoteEui64, EUI64_SIZE),
                        EMBER_NULL_NODE_ID, // node id - ignored
                        0x0000,             // group id - ignored
                        ...
                        makeMessage("111<4<2", EM_AF_PLUGIN_TEST_HARNESS_Z3_ZLL_CLIENT_TO_SERVER_FRAME_CONTROL,
                                    0x00, // zcl sequence number
                                    ZCL_IDENTIFY_REQUEST_COMMAND_ID, MY_TRANSACTION_ID - 1,
                                    0x4321)); // identify duration

    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkIdentifyRequestCommand, "24", makeMessage("24", 0xDCBA, 0));
    addInterpanOutCheck(0xFFFF, // dst pan id
                        makeMessage("s", remoteEui64, EUI64_SIZE),
                        EMBER_NULL_NODE_ID, // node id - ignored
                        0x0000,             // group id - ignored
    ...
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkRTFNRequestCommand, "4", makeMessage("4", 0));
    addInterpanOutCheck(0xFFFF, // dst pan id
                        makeMessage("s", remoteEui64, EUI64_SIZE),
                        EMBER_NULL_NODE_ID, // node id - ignored
                        0x0000,             // group id - ignored
                        EMBER_ZLL_PROFILE_ID,
                        makeMessage("111<4", EM_AF_PLUGIN_TEST_HARNESS_Z3_ZLL_CLIENT_TO_SERVER_FRAME_CONTROL,
                                    0x00, // zcl sequence number
                                    ZCL_RESET_TO_FACTORY_NEW_REQUEST_COMMAND_ID, MY_TRANSACTION_ID));
    ...
    addSimpleAction("setLastCommandArgument(%s)", setLastCommandArgument, 1, "network-start-request");
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkNetworkCommand, "222224",
                     makeMessage("222224", remoteNodeId, 0x0010, 0x0012, 0x1000, 0x1200, 0));
    addSimpleCheck("emberZllGetTokenStackZllData", "");
    addSimpleCheck("emberZllSetTokenStackZllData", "iiiii", zllToken.bitmask | EMBER_ZLL_STATE_FACTORY_NEW, 0x0010, 0x0012, 0x1000,
                   0x1200);
    addSimpleCheck("emberZllJoinTarget", "ii", MY_TRANSACTION_ID, EMBER_ROUTER);

    // plugin test-harness z3 touchlink network-join-router-request <dstAddress:2>
    // <freeAddrBegin:2> <freeAddrEnd:2> <groupIdBegin:2> <groupIdEnd:2> <options:4>
    addSimpleAction("setLastCommandArgument(%s)", setLastCommandArgument, 1, "network-join-router-request");
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkNetworkCommand, "222224",
                     makeMessage("222224", remoteNodeId, 0x0010, 0x0012, 0x1000, 0x1200, 0));
    addSimpleCheck("emberZllGetTokenStackZllData", "");
    addSimpleCheck("emberZllSetTokenStackZllData", "iiiii", zllToken.bitmask & ~EMBER_ZLL_STATE_FACTORY_NEW, 0x0010, 0x0012, 0x1000,
    ...
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkNetworkCommand, "222224",
                     makeMessage("222224", remoteNodeId, 0x0010, 0x0012, 0x1000, 0x1200, BIT32(3)));
    addSimpleCheck("emberZllGetTokenStackZllData", "");
    addSimpleCheck("emberZllSetTokenStackZllData", "iiiii", zllToken.bitmask & ~EMBER_ZLL_STATE_FACTORY_NEW, 0x0010, 0x0012, 0x1000,
                   0x1200);
    addSimpleCheck("emberZllJoinTarget", "ii", MY_TRANSACTION_ID - 1, EMBER_ROUTER);

    // plugin test-harness z3 touchlink network-join-end-device-request <dstAddress:2>
    // <freeAddrBegin:2> <freeAddrEnd:2> <groupIdBegin:2> <groupIdEnd:2> <options:4>
    addSimpleAction("setLastCommandArgument(%s)", setLastCommandArgument, 1, "network-join-end-device-request");
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkNetworkCommand, "222224",
                     makeMessage("222224", remoteNodeId, 0x0010, 0x0012, 0x1000, 0x1200, 0));
    addSimpleCheck("emberZllGetTokenStackZllData", "");
    addSimpleCheck("emberZllSetTokenStackZllData", "iiiii", zllToken.bitmask & ~EMBER_ZLL_STATE_FACTORY_NEW, 0x0010, 0x0012, 0x1000,
                   0x1200);
    addSimpleCheck("emberZllJoinTarget", "ii", MY_TRANSACTION_ID, EMBER_END_DEVICE);

    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkNetworkCommand, "222224",
                     makeMessage("222224", remoteNodeId, 0x000A, 0x000B, 0x000C, 0x000D, BIT32(3)));
    addSimpleCheck("emberZllGetTokenStackZllData", "");
    addSimpleCheck("emberZllSetTokenStackZllData", "iiiii", zllToken.bitmask & ~EMBER_ZLL_STATE_FACTORY_NEW, 0x000A, 0x000B, 0x000C,
                   0x000D);
    addSimpleCheck("emberZllJoinTarget", "ii", MY_TRANSACTION_ID - 1, EMBER_END_DEVICE);

    // plugin test-harness z3 touchlink network-update-request <options:4>
    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkNetworkUpdateRequestCommand, "4", makeMessage("4", 0));
    addInterpanOutCheck(0xFFFF, // dst pan id
                        makeMessage("s", remoteEui64, EUI64_SIZE),
                        EMBER_NULL_NODE_ID, // node id - ignored
                        0x0000,             // group id - ignored
                        EMBER_ZLL_PROFILE_ID,
                        makeMessage("111<4p11<2<2", EM_AF_PLUGIN_TEST_HARNESS_Z3_ZLL_CLIENT_TO_SERVER_FRAME_CONTROL,
                                    0x00, // zcl sequence number
                                    ZCL_NETWORK_UPDATE_REQUEST_COMMAND_ID, MY_TRANSACTION_ID,
                                    makeMessage("s", myExtPanId, EXTENDED_PAN_ID_SIZE), MY_NETWORK_UPDATE_ID, MY_CHANNEL, MY_PAN_ID,
                                    MY_NODE_ID));

    addCliCallAction(emAfPluginTestHarnessZ3TouchlinkNetworkUpdateRequestCommand, "4", makeMessage("4", BIT32(0)));
    addInterpanOutCheck(0xFFFF, // dst pan id
                        makeMessage("s", remoteEui64, EUI64_SIZE),
                        EMBER_NULL_NODE_ID, // node id - ignored
                        0x0000,             // group id - ignored
                        ...
                        makeMessage("111<4p11<2<2", EM_AF_PLUGIN_TEST_HARNESS_Z3_ZLL_CLIENT_TO_SERVER_FRAME_CONTROL,
                                    0x00, // zcl sequence number
                                    ZCL_NETWORK_UPDATE_REQUEST_COMMAND_ID, MY_TRANSACTION_ID,
                                    makeMessage("s", myExtPanId, EXTENDED_PAN_ID_SIZE), MY_NETWORK_UPDATE_ID + 5, MY_CHANNEL,
                                    MY_PAN_ID, MY_NODE_ID));
...
static void zdoCommandsTest(void)
{
    ...
    addCliCallAction(emAfPluginTestHarnessZ3MgmtPermitJoiningReqCommand, "224", makeMessage("224", 123, remoteNodeId, 0));
    addSimpleCheck("emberPermitJoiningRequest", "iii", remoteNodeId,
                   0x01, // TC_Significance
                   123);
    addCliCallAction(emAfPluginTestHarnessZ3MgmtPermitJoiningReqCommand, "224", makeMessage("224", 123, remoteNodeId, BIT32(0)));
    addSimpleCheck("emberPermitJoiningRequest", "iii", remoteNodeId,
    ...
    addCliCallAction(emAfPluginTestHarnessZ3MgmtLeaveCommand, "2114", makeMessage("2114", remoteNodeId, true, false, 0));
    addZdoOutCheck(remoteNodeId, LEAVE_REQUEST,
                   makeMessage("1s1",
                               emberNextZigDevRequestSequence(), // cheating...
                               remoteEui64, EUI64_SIZE, BIT(6)));

    addCliCallAction(emAfPluginTestHarnessZ3MgmtLeaveCommand, "2114", makeMessage("2114", remoteNodeId, true, false, BIT(0)));
    addZdoOutCheck(remoteNodeId, LEAVE_REQUEST,
                   makeMessage("11",
                               emberNextZigDevRequestSequence(), // cheating...
                               BIT(6)));

    addCliCallAction(emAfPluginTestHarnessZ3MgmtLeaveCommand, "2114", makeMessage("2114", remoteNodeId, true, false, BIT(1)));
    addZdoOutCheck(remoteNodeId, LEAVE_REQUEST,
                   makeMessage("1s",
                               emberNextZigDevRequestSequence(), // cheating...
                               remoteEui64, EUI64_SIZE));
    ...
    addCliCallAction(emAfPluginTestHarnessZ3MgmtNwkUpdateRequestCommand, "22124", makeMessage("22124", 11, 5, 3, remoteNodeId, 0));
    ...
    addCliCallAction(emAfPluginTestHarnessZ3MgmtNwkUpdateRequestCommand, "22124",
                     makeMessage("22124", 11, 0xFF, 3, remoteNodeId, 0));
    addZdoOutCheck(remoteNodeId,
                   0x0038, // network update cluster
                   makeMessage("1<4111",
                               emberNextZigDevRequestSequence(), // cheating...
                               BIT32(11), 0xFF, MY_NETWORK_UPDATE_ID, MY_NETWORK_MANAGER_ID));

    addCliCallAction(emAfPluginTestHarnessZ3MgmtNwkUpdateRequestCommand, "22124",
                     makeMessage("22124", 11, 0xFE, 3, remoteNodeId, BIT32(0)));
    addZdoOutCheck(remoteNodeId,
                   0x0038, // network update cluster
                   makeMessage("1<411",
                               emberNextZigDevRequestSequence(), // cheating...
                               BIT32(11), 0xFE, MY_NETWORK_UPDATE_ID - 1));

    addCliCallAction(emAfPluginTestHarnessZ3MgmtNwkUpdateRequestCommand, "22124",
                     makeMessage("22124", 11, 0xFE, 3, remoteNodeId, BIT32(1)));
    addZdoOutCheck(remoteNodeId,
                   0x0038, // network update cluster
                   makeMessage("1<411",
                               emberNextZigDevRequestSequence(), // cheating...
    ...
    addCliCallAction(emAfPluginTestHarnessZ3ZdoActiveEndpointRequestCommand, "224", makeMessage("224", remoteNodeId, 0x1234, 0));
    addZdoOutCheck(remoteNodeId, ACTIVE_ENDPOINTS_REQUEST,
                   makeMessage("1<2",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234));

    addCliCallAction(emAfPluginTestHarnessZ3ZdoActiveEndpointRequestCommand, "224",
                     makeMessage("224", remoteNodeId, 0x1234, BIT(0)));
    addZdoOutCheck(remoteNodeId, ACTIVE_ENDPOINTS_REQUEST, makeMessage("1",
                                                                       emberNextZigDevRequestSequence())); // cheating...
    ...
    addCliCallAction(emAfPluginTestHarnessZ3ZdoMatchDescReqCommand, "2224",
                     makeMessage("2224", remoteNodeId, 0x1234, HA_PROFILE_ID, 0));
    addZdoOutCheck(remoteNodeId, MATCH_DESCRIPTORS_REQUEST,
                   makeMessage("1<2<21p1p",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234, HA_PROFILE_ID, 1, makeMessage("<2", 0x0001), 2, makeMessage("<2<2", 0x0101, 0x0102)));
    ...
    addZdoOutCheck(remoteNodeId, MATCH_DESCRIPTORS_REQUEST,
                   makeMessage("1<2<21p1p",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x4321, HA_PROFILE_ID, 2, makeMessage("<2<2", 0x0001, 0x0002), 0, makeParcel(0)));
    ...
    addZdoOutCheck(remoteNodeId, MATCH_DESCRIPTORS_REQUEST,
                   makeMessage("1<21p1p",
                               emberNextZigDevRequestSequence(), // cheating...
                               HA_PROFILE_ID, 1, makeMessage("<2", 0x0001), 1, makeMessage("<2", 0x0101)));
    ...
    addZdoOutCheck(remoteNodeId, MATCH_DESCRIPTORS_REQUEST,
                   makeMessage("1<2<211",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234, HA_PROFILE_ID, 1, 1));

    addSimpleAction("setClCounts(%d, %d)", setClCounts, 2, 1, 1);
    addCliCallAction(emAfPluginTestHarnessZ3ZdoMatchDescReqCommand, "2224", makeMessage("2224", remoteNodeId, 0x1234, 0xACAC, 0));
    addZdoOutCheck(remoteNodeId, MATCH_DESCRIPTORS_REQUEST,
                   makeMessage("1<2<21<21<2",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234, 0xACAC, 1, 0x0001, 1, 0x0101));
    ...
    addZdoOutCheck(remoteNodeId, MATCH_DESCRIPTORS_REQUEST,
                   makeMessage("1<2<21<21<2<2",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234, 0xACAC, 2, 0x0001, 2, 0x0101, 0x0102));
    ...
    addZdoOutCheck(remoteNodeId, MATCH_DESCRIPTORS_REQUEST,
                   makeMessage("1<2<21<2<21<2",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234, 0xACAC, 2, 0x0001, 0x0002, 2, 0x0101));

    // plugin test-harness z3 simple-desc-req <dstShort:2> <dstEndpoint:1>
    // <nwkAddrOfInterest:2> <options:4>
    addCliCallAction(emAfPluginTestHarnessZ3ZdoSimpleDescReqCommand, "2124", makeMessage("2124", remoteNodeId, 8, 0x1234, 0));
    addZdoOutCheck(remoteNodeId, SIMPLE_DESCRIPTOR_REQUEST,
                   makeMessage("1<21",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234, 8));

    addCliCallAction(emAfPluginTestHarnessZ3ZdoSimpleDescReqCommand, "2124", makeMessage("2124", remoteNodeId, 8, 0x1234, BIT(0)));
    addZdoOutCheck(remoteNodeId, SIMPLE_DESCRIPTOR_REQUEST,
                   makeMessage("11",
                               emberNextZigDevRequestSequence(), // cheating...
                               8));

    addCliCallAction(emAfPluginTestHarnessZ3ZdoSimpleDescReqCommand, "2124", makeMessage("2124", remoteNodeId, 8, 0x1234, BIT(1)));
    addZdoOutCheck(remoteNodeId, SIMPLE_DESCRIPTOR_REQUEST,
                   makeMessage("1<2",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234));
    ...
    addCliCallAction(emAfPluginTestHarnessZ3ZdoBindGroupCommand, "211228",
                     makeMessage("21122s", remoteNodeId, 1, 2, 0xABCD, 0x1234, emLocalEui64, EUI64_SIZE));
    addZdoOutCheck(remoteNodeId, BIND_REQUEST,
                   makeMessage("1s1<21<2",
                               emberNextZigDevRequestSequence(), // cheating...
                               emLocalEui64, EUI64_SIZE, 1, 0x1234, 0x01, 0xABCD));

    // plugin test-harness z3 zdo nwk-addr-req <ieee:8> <requestType:1>
    // <startIndex:1> <dstShort:2> <options:4>
    addCliCallAction(emAfPluginTestHarnessZ3ZdoNwkAddrReqCommand, "81124",
                     makeMessage("s1124", remoteEui64, EUI64_SIZE, 0, 1, remoteNodeId, 0));
    addZdoOutCheck(remoteNodeId, NETWORK_ADDRESS_REQUEST,
                   makeMessage("1s11",
                               emberNextZigDevRequestSequence(), // cheating...
                               remoteEui64, EUI64_SIZE, 0, 1));

    addCliCallAction(emAfPluginTestHarnessZ3ZdoNwkAddrReqCommand, "81124",
                     makeMessage("s1124", remoteEui64, EUI64_SIZE, 0, 1, remoteNodeId, BIT(0)));
    addZdoOutCheck(remoteNodeId, NETWORK_ADDRESS_REQUEST, makeMessage("1",
                                                                      emberNextZigDevRequestSequence())); // cheating...

    // plugin test-harness z3 zdo ieee-addr-req <nwkAddrOfInterest:2>
    // <requestType:1> <startIndex:1> <dstShort:2> <options:4>
    addCliCallAction(emAfPluginTestHarnessZ3ZdoIeeeAddrReqCommand, "21124", makeMessage("21124", 0x1234, 0, 1, remoteNodeId, 0));
    addZdoOutCheck(remoteNodeId, IEEE_ADDRESS_REQUEST,
                   makeMessage("1<211",
                               emberNextZigDevRequestSequence(), // cheating...
                               0x1234, 0, 1));

    addCliCallAction(emAfPluginTestHarnessZ3ZdoIeeeAddrReqCommand, "21124",
                     makeMessage("21124", 0x1234, 0, 1, remoteNodeId, BIT(0)));
    addZdoOutCheck(remoteNodeId, IEEE_ADDRESS_REQUEST, makeMessage("1",
                                                                   emberNextZigDevRequestSequence())); // cheating...

    // plugin test-harness z3 zdo bc-device-announce <shortAddress:2> <ieeeAddress:8>
    addCliCallAction(emAfPluginTestHarnessZ3ZdoBcDeviceAnnounceCommand, "28",
                     makeMessage("2s", remoteNodeId, remoteEui64, EUI64_SIZE));
    addZdoOutCheck(EMBER_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS, END_DEVICE_ANNOUNCE,
                   makeMessage("1<2s1",
                               emberNextZigDevRequestSequence(), // cheating...
                               remoteNodeId, remoteEui64, EUI64_SIZE,
...
static void zclCommandsTest(void)
{
    ...
    addCliCallAction(emAfPluginTestHarnessZ3ZclReadCommand, "222112",
                     makeMessage("222112", 0x0001, 0x0002, remoteNodeId, 0x04, 0x05, 0x0006));
    addApsOutCheck(remoteNodeId, 0x0006, 0x0001, 0x04, 0x05, false, makeMessage("111<2", 0x00, 0xAC, 0x00, 0x0002));
...
PERFORMER(incomingCommand)
{
    ...
    emberAfIncomingPacketFilterCallback(commandType, packet, &length, data);

    commandAftermath = bufferToParcel(reallyCommandBuffer);
    simpleScriptCheck("emberAfIncomingPacketFilterCallback", "incomingCommandAftermath", "p", commandAftermath);
...
#define addIncomingCommandAction(commandType, commandBuffer, indexOfCommand, data)                                                 \
    addAction(&incomingCommandActionType, (commandType), (commandBuffer), (indexOfCommand), (data))
...
void incomingCommandTest(void)
{
    ...
    addSimpleAction("setLastCommandArgument(%s)", setLastCommandArgument, 1, "on");
    addCliCallAction(emAfPluginTestHarnessZ3NwkNwkLeaveSupression, "", makeParcel(0));
    addIncomingCommandAction(EMBER_ZIGBEE_COMMAND_TYPE_NWK,
                             makeMessage("11",
                                         0x04,    // leave command
                                         BIT(6)), // request
                             0, makeParcel(0));
    addSimpleCheck("incomingCommandAftermath", "p",
                   makeMessage("11",
                               0x04, // leave command
                               0));  // no options

    addSimpleAction("setLastCommandArgument(%s)", setLastCommandArgument, 1, "off");
    addCliCallAction(emAfPluginTestHarnessZ3NwkNwkLeaveSupression, "", makeParcel(0));
    addIncomingCommandAction(EMBER_ZIGBEE_COMMAND_TYPE_NWK,
                             makeMessage("11",
                                         0x04,    // leave command
...
int main(int argc, char * argv[])
{
    ...
    {
        // AF initialization.
        emberAfSetExternalBuffer(zclBuffer, sizeof(zclBuffer), &zclBufferLength, &zclApsFrame);
    }
...
{
    functionCallCheck("emberFindAndRejoinNetwork", "ii", haveCurrentNetworkKey, channelMask);
    return EMBER_SUCCESS;
}
...
EmberStatus emberSendZigbeeLeave(EmberNodeId nodeId, EmberLeaveRequestFlags flags)
{
    functionCallCheck("emberSendZigbeeLeave", "ii", nodeId, flags);
    return EMBER_SUCCESS;
}
EmberStatus emberStartScan(EmberNetworkScanType type, uint32_t channelMask, uint8_t scanDuration)
{
    functionCallCheck("emberStartScan", "ii", type, channelMask);
    return EMBER_SUCCESS;
}
EmberStatus emberSendRemoveDevice(EmberNodeId destShort, EmberEUI64 destLong, EmberEUI64 deviceToRemoveLong)
{
    functionCallCheck("emberSendRemoveDevice", "ipp", destShort, makeMessage("s", destLong, EUI64_SIZE),
                      makeMessage("s", deviceToRemoveLong, EUI64_SIZE));
    return EMBER_SUCCESS;
}
EmberStatus emberZllSetPolicy(EmberZllPolicy p)
{
    functionCallCheck("emberZllSetPolicy", "i", p);
    return EMBER_SUCCESS;
}
...
{
    functionCallCheck("emberIeeeAddressRequest", "iiii", target, reportKids, childStartIndex, options);
    return EMBER_SUCCESS;
}
EmberStatus emberNetworkAddressRequest(EmberEUI64 target, bool reportKids, uint8_t childStartIndex)
{
    functionCallCheck("emberNetworkAddressRequest", "pii", makeMessage("s", target, EUI64_SIZE), reportKids, childStartIndex);
    return EMBER_SUCCESS;
}
EmberStatus emberPermitJoiningRequest(EmberNodeId target, uint8_t duration, uint8_t authentication, EmberApsOption options)
{
    functionCallCheck("emberPermitJoiningRequest", "iii", target, authentication, duration);
...
{
    return ((WHATEVER << 0x00) | (WHATEVER << 0x08) | (WHATEVER << 0x10) | (WHATEVER << 0x18));
}
