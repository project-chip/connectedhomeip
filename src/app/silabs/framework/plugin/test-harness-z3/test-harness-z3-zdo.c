...
static EmberStatus sendZdoCommand(EmberNodeId destination, uint16_t clusterId, uint8_t * frame, uint8_t frameLength)
{
...
static EmberStatus sendZdoCommand(EmberNodeId destination, uint16_t clusterId, uint8_t * frame, uint8_t frameLength)
{
    ...
    {
        status = emberSendBroadcast(destination, &apsFrame, EMBER_MAX_HOPS, message);
    }
    else
    {
...
void emAfPluginTestHarnessZ3MgmtPermitJoiningReqCommand(void)
{
    ...
    status =
        emberPermitJoiningRequest(destination, permitDurationS, (options == BIT32(0) ? false : true), (EMBER_APS_OPTION_RETRY));

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Permit joining request", status);
}
...
void emAfPluginTestHarnessZ3MgmtLeaveCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Mgmt leave", status);
}
...
void emAfPluginTestHarnessZ3MgmtNwkUpdateRequestCommand(void)
{
    ...
    *finger++ = (uint8_t)(0xFF & scanDuration); // ANDREW: Q3.
    ...
    {
        *finger++ = options == BIT(1) ? 0x55 : networkParameters.nwkUpdateId - (options == BIT(0) ? 1 : 0);
    }
    ...
    status = sendZdoCommand(destination, ZDO_NETWORK_UPDATE_REQUEST, frame, finger - &frame[0]);
...
void emAfPluginTestHarnessZ3ZdoZdoResetCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "ZDO reset", EMBER_SUCCESS);
}
...
void emAfPluginTestHarnessZ3ZdoNodeDescReqCommand(void)
{
    ...
    status = sendZdoCommand(destination, NODE_DESCRIPTOR_REQUEST, frame, 1);

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Node descriptor request", status);
}
...
void emAfPluginTestHarnessZ3ZdoZdoNodeDescRspConfigCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Node descriptor response config", status);
}
...
void emAfPluginTestHarnessZ3ZdoActiveEndpointRequestCommand(void)
{
    ...
    status = sendZdoCommand(destination, ACTIVE_ENDPOINTS_REQUEST, frame, frameLength);

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Active endpoint request", status);
}
...
void emAfPluginTestHarnessZ3ZdoMatchDescReqCommand(void)
{
    ...
    if (options != BIT(1))
    {
        uint8_t reallyInClustersCount  = (options == BIT(2) ? 1 : emAfCliInClCount);
        uint8_t reallyOutClustersCount = (options == BIT(3) ? 1 : emAfCliOutClCount);

        // In clusters.
        *finger++ = emAfCliInClCount;
        MEMMOVE(finger, emAfCliZdoInClusters, reallyInClustersCount * sizeof(uint16_t));
        finger += (reallyInClustersCount * sizeof(uint16_t));

        // Out clusters.
        *finger++ = emAfCliOutClCount;
        MEMMOVE(finger, emAfCliZdoOutClusters, reallyOutClustersCount * sizeof(uint16_t));
    ...
    status = sendZdoCommand(destination, MATCH_DESCRIPTORS_REQUEST, frame, finger - &frame[0]);

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Match descriptor request", status);
}
...
void emAfPluginTestHarnessZ3ZdoMatchDescRspConfigCommand(void)
{
    ...
    case BIT(0):
        // match list absent
        negativeZdoCommandFlags = (NEGATIVE_ZDO_COMMAND_FLAGS_MATCH_LIST_ABSENT);
        break;
    case BIT(1):
        // second response device not found
        negativeZdoCommandFlags =
            (NEGATIVE_ZDO_COMMAND_FLAGS_MATCH_LIST_ABSENT | NEGATIVE_ZDO_COMMAND_FLAGS_SEND_POSITIVE_RESPONSE);
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Match descriptor response config", status);
}
...
void emAfPluginTestHarnessZ3ZdoSimpleDescReqCommand(void)
{
    ...
    status = sendZdoCommand(destination, SIMPLE_DESCRIPTOR_REQUEST, frame, finger - &frame[0]);

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Simple descriptor request", status);
}
...
void emAfPluginTestHarnessZ3ZdoSimpleDescRspConfigCommand(void)
{
    ...
    case BIT(5):
        ...
        negativeZdoCommandFlags =
            (NEGATIVE_ZDO_COMMAND_FLAGS_SEND_POSITIVE_RESPONSE | NEGATIVE_ZDO_COMMAND_FLAGS_NO_SIMPLE_DESCRIPTOR);
        currentZdoNegativeCommandFrame[1] = EMBER_ZDP_DEVICE_NOT_FOUND;
    ...
    case BIT(7):
        ...
        break;
    default:; // options of 0x00 is ok
    }

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Simple descriptor response config", status);
}
...
void emAfPluginTestHarnessZ3ZdoBindGroupCommand(void)
{
    ...
    *finger++ = 0x01; // multicast
    ...
    status = sendZdoCommand(shortAddress, BIND_REQUEST, frame, finger - &frame[0]);

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "ZDO bind group", status);
}
...
void emAfPluginTestHarnessZ3ZdoNwkAddrReqCommand(void)
{
    ...
    status = sendZdoCommand(destinationShort, NETWORK_ADDRESS_REQUEST, frame, finger - &frame[0]);

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Network address request", status);
}
...
void emAfPluginTestHarnessZ3ZdoIeeeAddrReqCommand(void)
{
    ...
    status = sendZdoCommand(destination, IEEE_ADDRESS_REQUEST, frame, finger - &frame[0]);

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "IEEE address request", status);
}
...
void emAfPluginTestHarnessZ3ZdoIeeeAddrRspConfigCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "IEEE address response config", status);
}
...
void emAfPluginTestHarnessZ3ZdoBcDeviceAnnounceCommand(void)
{
    ...
    status = sendZdoCommand(EMBER_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS, END_DEVICE_ANNOUNCE, frame, sizeof(frame));

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Broadcast device announce", status);
}
...
EmberStatus emAfPluginTestHarnessZ3ZdoCommandResponseHandler(uint8_t * requestBuffer, uint8_t length, EmberApsFrame * apsFrame)
{
    ...
    if (clusterId == IEEE_ADDRESS_REQUEST)
    {
        ...
        EmberNodeId nwkAddressOfInterest = HIGH_LOW_TO_INT(requestBuffer[2], requestBuffer[1]);
        ...
        status = emberLookupEui64ByNodeId(nwkAddressOfInterest, &currentZdoNegativeCommandFrame[2]);
    }
    ...
    {
        if (negativeZdoCommandFlags & NEGATIVE_ZDO_COMMAND_FLAGS_MATCH_LIST_ABSENT)
        {
    ...
    else if (clusterId == SIMPLE_DESCRIPTOR_REQUEST && !(negativeZdoCommandFlags & NEGATIVE_ZDO_COMMAND_FLAGS_NO_SIMPLE_DESCRIPTOR))
    {
        ...
        if (endpointIndex != 0xFF)
        {
            ...
            currentZdoNegativeCommandFrame[currentZdoNegativeCommandFrameLength++] = EMBER_ZDP_SUCCESS; // Status - Index 1
            if (!(negativeZdoCommandFlags & NEGATIVE_ZDO_COMMAND_FLAGS_NO_NWK_ADDR))
            {
                currentZdoNegativeCommandFrame[currentZdoNegativeCommandFrameLength++] = LOW_BYTE(emberAfGetNodeId());  // Index 2
                currentZdoNegativeCommandFrame[currentZdoNegativeCommandFrameLength++] = HIGH_BYTE(emberAfGetNodeId()); // Index 3
            }
            // Length field index 2 or 4 depending the above.
            // Increment the length to account the length field itself.
            currentZdoNegativeCommandFrameLength++;

            if (!(negativeZdoCommandFlags & NEGATIVE_ZDO_COMMAND_FLAGS_NO_ENDPOINT_FIELDS))
            {
                // endpoint
                currentZdoNegativeCommandFrame[currentZdoNegativeCommandFrameLength++] = endpoint;
            }

            // profile id
            emberAfCopyInt16u(currentZdoNegativeCommandFrame, currentZdoNegativeCommandFrameLength,
            ...
            // device version
            currentZdoNegativeCommandFrame[currentZdoNegativeCommandFrameLength++] = emberAfDeviceVersionFromIndex(endpointIndex);
            ...
            if (negativeZdoCommandFlags & NEGATIVE_ZDO_COMMAND_FLAGS_CLUSTER_LIST_SWAP)
            {
            ...
            if (!(negativeZdoCommandFlags & NEGATIVE_ZDO_COMMAND_FLAGS_CLUSTER_LIST_ABSENT))
            {
                currentZdoNegativeCommandFrame[currentZdoNegativeCommandFrameLength++] = firstCount;
                for (int i = 0; i < firstCount; i++)
                {
                    emberAfCopyInt16u(currentZdoNegativeCommandFrame, currentZdoNegativeCommandFrameLength, firstList[i]);
                    currentZdoNegativeCommandFrameLength += sizeof(uint16_t);
                }
                currentZdoNegativeCommandFrame[currentZdoNegativeCommandFrameLength++] = secondCount;
                for (int i = 0; i < secondCount; i++)
                {
                    emberAfCopyInt16u(currentZdoNegativeCommandFrame, currentZdoNegativeCommandFrameLength, secondList[i]);
                    currentZdoNegativeCommandFrameLength += sizeof(uint16_t);
                }
            }
    ...
    // ZDO command.
    if (!(negativeZdoCommandFlags & NEGATIVE_ZDO_COMMAND_FLAGS_SEND_POSITIVE_RESPONSE))
    {
...
void emberAfPluginTestHarnessZ3ZdoSendEventHandler(void)
{
    ...
    if (!(negativeZdoCommandFlags & NEGATIVE_ZDO_COMMAND_FLAGS_NO_NEGATIVE_RESPONSE))
    {
        emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "ZDO send event handler",
                           sendZdoCommand(currentZdoNegativeCommandDestination, currentZdoNegativeCommandCluster,
