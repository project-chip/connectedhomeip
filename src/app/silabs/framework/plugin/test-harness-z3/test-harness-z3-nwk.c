...
extern bool emNetworkSendCommand(EmberNodeId destination, uint8_t * commandFrame, uint8_t length, bool tryToInsertLongDest,
                                 EmberEUI64 destinationEui);
extern uint8_t emSetNwkUpdateId(uint8_t id);

#endif /* EZSP_HOST */

static EmberStatus sendNetworkCommand(EmberNodeId destinationShort, uint8_t * commandFrame, uint8_t length)
{
    EmberEUI64 destinationLong;
    bool haveDestinationLong = false;

    haveDestinationLong = (emberLookupEui64ByNodeId(destinationShort, destinationLong) == EMBER_SUCCESS);

    return (emNetworkSendCommand(destinationShort, commandFrame, length, haveDestinationLong, destinationLong)
                ? EMBER_SUCCESS
...
void emAfPluginTestHarnessZ3NwkNwkRejoinRequestCommand(void)
{
    ...
    status = sendNetworkCommand(destinationShort, commandFrame, sizeof(commandFrame));

done:
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Rejoin request", status);
}
...
void emAfPluginTestHarnessZ3NwkNwkRejoinResponseCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Rejoin response", status);
}
...
void emAfPluginTestHarnessZ3NwkNwkLeaveCommand(void)
{
    ...
    case BIT(0): // no ieee addr field
        status =
            (emNetworkSendCommand(destinationShort, frame, sizeof(frame), false, NULL) ? EMBER_SUCCESS : EMBER_DELIVERY_FAILED);
        break;
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Leave", status);
}
...
{
    emAfPluginTestHarnessZ3IgnoreLeaveCommands = (emberStringCommandArgument(-1, NULL)[1] == 'n');
    emberAfCorePrintln("%p: %p: %p", TEST_HARNESS_Z3_PRINT_NAME, "Supress network leave",
                       (emAfPluginTestHarnessZ3IgnoreLeaveCommands ? "YES" : "NO"));
}
...
void emAfPluginTestHarnessZ3NwkGetNetworkUpdateId(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Network Update Id", networkParameters.nwkUpdateId);
}
...
void emAfPluginTestHarnessZ3NwkSetNetworkUpdateId(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Set network update id", updateId);
}
