...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief ZigBee 3.0 mac layer test harness
                                                                               *functionality
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
void emAfPluginTestHarnessZ3BeaconBeaconReqCommand(void)
{
    ...
    {
        status = emberStartScan(EMBER_ACTIVE_SCAN, BIT32(networkParameters.radioChannel),
    ...
    else
    {
        ...
        status = emberStartScan(EMBER_ACTIVE_SCAN, emAfPluginNetworkSteeringPrimaryChannelMask,
                                2); // scan duration, whatever
    }

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Beacon request", status);
}
...
void emAfPluginTestHarnessZ3BeaconBeaconsConfigCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Beacon", status);
}
...
EmberPacketAction emAfPluginTestHarnessZ3ModifyBeaconPayload(uint8_t * beaconPayload, uint8_t * payloadLength)
{
    if (beaconConfigNegativeBehaviorMask)
    {
        ...
        if (beaconConfigNegativeBehaviorMask & NEGATIVE_BEHAVIOR_SWITCH_OFF_RADIO)
        {
            ...
            emberEventControlSetDelayMS(emberAfPluginTestHarnessZ3BeaconSendEventControl, BEACON_RADIO_OFF_DELAY_MS);
        }
