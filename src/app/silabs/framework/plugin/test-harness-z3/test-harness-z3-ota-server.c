...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief ZigBee 3.0 ota server test harness
                                                                               *functionality
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
EmberAfOtaImageId emberAfOtaStorageSearchCallback(int16u manufacturerId, int16u imageTypeId, const int16u * hardwareVersion)
{
    ...
    if (loadedOTAFileHeader.manufacturerId == manufacturerId && loadedOTAFileHeader.imageTypeId == imageTypeId)
    {
...
{
    if (id->manufacturerId == loadedOtaImageId.manufacturerId && id->imageTypeId == loadedOtaImageId.imageTypeId)
    {
...
bool emberAfIsOtaImageIdValid(const EmberAfOtaImageId * idToCompare)
{
    ...
    if (idToCompare != NULL)
    {
        ...
        if (idToCompare->manufacturerId != InvalidId.manufacturerId && idToCompare->imageTypeId != InvalidId.imageTypeId
...
// OTA Server commands
// plugin test-harness z3 ota-server activate <shortAddress:2> <endpoint:1> <manufacturereCode:2> <imageType:2> <fileVersion:4>
// <fileSize:4>
...
void emAfPluginTestHarnessZ3OtaServerActivateCommand(void)
{
    ...
    status = emberAfOtaServerSendImageNotifyCallback(shortAddress, endpoint,
                                                     0x03, // payloadType,
                                                     60,   // jitter,
                                                     &loadedOtaImageId);

#endif /* EZSP_HOST */
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Ota Server activate", status);
}
...
void emAfPluginTestHarnessZ3OtaServerDeActivateCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Ota Server Deactivate", status);
}
