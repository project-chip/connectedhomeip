...
uint32_t emberGetTestHarnessConfigurationOptions(uint8_t commandType, uint8_t commandId)
{
    switch (commandType)
    {
...
void emberAfPluginTestHarnessZ3ResetKeyEventHandler(void)
{
    ...
    if (mostRecentKeyIndex != 0xFF)
    {
        ...
        emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Reset key handler", status);
...
static EmberStatus setNextKey(EmberEUI64 partner, bool linkKey, uint8_t keyType)
{
    ...
    status = emberAddOrUpdateKeyTableEntry(partner, linkKey, (keyType > KEY_TYPE_MAX ? keys[KEY_TYPE_MAX] : keys[keyType]));
...
// Internal stack API.
extern bool emSendApsCommand(EmberNodeId destination, EmberEUI64 longDestination, EmberMessageBuffer payload, uint8_t options);
...
void emAfPluginTestHarnessZ3ApsApsRemoveDevice(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Remove device", status);
}
...
void emAfPluginTestHarnessZ3ApsApsRemoveDeviceConfig(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Remove device configuration", 0);
}
...
void emAfPluginTestHarnessZ3ApsApsRequestKeyCommand(void)
{
    ...
    apsCommandOptions = (APS_COMMAND_OPTION_NWK_ENCRYPTION | APS_COMMAND_OPTION_APS_ENCRYPTION);
    ...
    *finger++ = keyType;
    ...
    status = (emSendApsCommand(destShort, trustCenterEui64, commandBuffer, apsCommandOptions) ? EMBER_SUCCESS : EMBER_ERR_FATAL);
    emberReleaseMessageBuffer(commandBuffer);

done:
#endif /* EZSP_HOST */

    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Request key", status);
}
