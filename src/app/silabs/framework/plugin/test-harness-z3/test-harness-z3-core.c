                                                      ...
                                                      void * data)
{
    ...
    case EMBER_ZIGBEE_PACKET_TYPE_BEACON: {
        if (printingMask & PRINTING_MASK_BEACONS)
        {
            ...
            emberAfCorePrint("beacon:rx 0x%2X, AP 0x%p, EP ", panId,
                             ((packetData[BEACON_HEADER_SIZE + 1] & BEACON_ASSOCIATION_PERMIT_BIT) ? "1" : "0"));
            emberAfCorePrintBuffer(packetData + BEACON_HEADER_SIZE + BEACON_EXTENDED_PAN_ID_INDEX, EXTENDED_PAN_ID_SIZE,
                                   true); // spaces?
            emberAfCorePrintln("");

            // Print out the full payload except beacon header, for the benefit of the beanshell tests
            emberAfCorePrint("payload[");
            emberAfCorePrintBuffer(packetData + BEACON_HEADER_SIZE, packetLength - BEACON_HEADER_SIZE,
                                   true); // spaces?
            emberAfCorePrintln("]");
        }
        break;
    ...
    case EMBER_ZIGBEE_PACKET_TYPE_NWK_COMMAND: {
        ...
        {
            emberAfCorePrint("nwk:rx seq AC sec 28 cmd %X payload[", commandId);
            emberAfCorePrintBuffer(packetData + 1, packetLength - 1, true); // spaces?
            emberAfCorePrintln("]");
        }
        if (commandId == NWK_LEAVE_COMMAND && emAfPluginTestHarnessZ3IgnoreLeaveCommands)
        {
    ...
    case EMBER_ZIGBEE_PACKET_TYPE_APS_COMMAND: {
        if (printingMask & PRINTING_MASK_APS)
        {
            ...
            emberAfCorePrint("aps:rx seq AC fc %X cmd %X payload[", fc, commandId);
            emberAfCorePrintBuffer(packetData + 1, packetLength - 1, true); // spaces?
            emberAfCorePrintln("]");
        }
        break;
    ...
    case EMBER_ZIGBEE_PACKET_TYPE_ZDO: {
        ...
        {
            emberAfCorePrint("zdo:t%4X:%p seq %X cmd %2X payload[", emberAfGetCurrentTime(), "rx", packetData[0],
                             apsFrame->clusterId);
            emberAfCorePrintBuffer(packetData + ZDO_MESSAGE_OVERHEAD, packetLength - ZDO_MESSAGE_OVERHEAD,
                                   true); // spaces?
            emberAfCorePrintln("]");
        }
        status = emAfPluginTestHarnessZ3ZdoCommandResponseHandler(packetData, packetLength, apsFrame);
        if (status != EMBER_INVALID_CALL)
        {
            emberAfCorePrintln("%p: %p: cluster: 0x%02X status: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "ZDO negative command",
                               apsFrame->clusterId | 0x8000, status);
        }
        break;
    }
                                                      ...
                                                      void * data)
{
    ...
    case EMBER_ZIGBEE_PACKET_TYPE_BEACON: {
        ...
        act                   = emAfPluginTestHarnessZ3ModifyBeaconPayload(packetData + MAC_BEACON_SIZE, &payloadLength);
    ...
    case EMBER_ZIGBEE_PACKET_TYPE_MAC_COMMAND: {
        ...
        if (packetData[macHeaderLength + 2] == 0x03)
        {
            ...
            act = emAfPluginTestHarnessZ3ZllModifyInterpanCommand(packetData, macHeaderLength);
        }
        break;
    }
    default:
...
void emAfPluginTestHarnessZ3PrintingCommand(void)
{
    ...
    emberAfCorePrintln("%puccessfully %p printing.", (mask == 0xFF ? "Uns" : "S"), (enabled ? "enabled" : "disabled"));
}
...
void emberAfPluginTestHarnessZ3OpenNetworkEventHandler(void)
{
    ...
    if (emberAfGetNodeId() == EMBER_TRUST_CENTER_NODE_ID)
    {
        ...
        ezspGetConfigurationValue(EZSP_CONFIG_TRANSIENT_KEY_TIMEOUT_S, &transientKeyTimeoutS);
#else
        ...
        emberEventControlSetDelayMS(emberAfPluginTestHarnessZ3OpenNetworkEventControl,
                                    ((transientKeyTimeoutS * MILLISECOND_TICKS_PER_SECOND) - MILLISECOND_TICKS_PER_SECOND));
    }
}
...
void emAfPluginTestHarnessZ3ResetCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Reset", EMBER_SUCCESS);
}
...
void emAfPluginTestHarnessZ3SetDeviceModeCommand(void)
{
    ...
    EmAfPluginTestHarnessZ3DeviceMode mode = (EmAfPluginTestHarnessZ3DeviceMode) emberUnsignedCommandArgument(0);
    ...
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Set device mode", status);
}

// plugin test-harness z3 set-short-address
void emAfPluginTestHarnessZ3SetShortAddressCommand(void)
{
    emberAfCorePrintln("%p: %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Set short address", EMBER_INVALID_CALL);
}
...
void emAfPluginTestHarnessZ3LegacyProfileCommand(void)
{
    emAfPluginTestHarnessZ3TouchlinkProfileId =
        (emberStringCommandArgument(-1, NULL)[0] == 'e') ? EMBER_ZLL_PROFILE_ID : HA_PROFILE_ID;
}
...
#endif
#else
    emberAfCorePrintln("%p: %p %p: 0x%X", TEST_HARNESS_Z3_PRINT_NAME, "Install code", (doClear ? "clear" : "set"),
                       EMBER_INVALID_CALL);
#endif
}
