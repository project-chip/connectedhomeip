...
static void optionallyChangeState(EmberNodeId nodeId, EmberAfPluginDeviceTableDeviceState state);
...
{
    emberAfCorePrintln("ROUTE REPAIR SERVICE RETURN RESULT: status=%d", result->status);
}
...
bool emAfPluginDeviceTablePreCommandReceivedCallback(EmberAfClusterCommand * cmd)
{
    ...
    if (getCurrentState(nodeId) >= EMBER_AF_PLUGIN_DEVICE_TABLE_STATE_LEAVE_SENT &&
...
void emberAfPluginDeviceTableSendLeave(uint16_t index)
{
    ...
    emberLeaveRequest(nodeId, destinationEui64,
...
static void optionallyChangeState(EmberNodeId nodeId, EmberAfPluginDeviceTableDeviceState state)
{
    ...
    if (originalState != state)
    {
        ...
        emberAfPluginDeviceTableStateChangeCallback(deviceTable[index].nodeId, state);
    }
}

static bool isZdoLeaveCommand(uint16_t profileId, uint16_t clusterId)
{
    return profileId == EMBER_ZDO_PROFILE_ID && clusterId == LEAVE_REQUEST ? true : false;
}
...
void emberAfPluginDeviceTableMessageSentStatus(EmberNodeId nodeId, EmberStatus status, uint16_t profileId, uint16_t clusterId)
{
    ...
    {
        optionallyChangeState(nodeId, EMBER_AF_PLUGIN_DEVICE_TABLE_STATE_LEAVE_SENT);
    }
    else
    {
        ...
        {
            optionallyChangeState(nodeId, EMBER_AF_PLUGIN_DEVICE_TABLE_STATE_UNRESPONSIVE);
        }
    }
}
