...
static void checkNullEui64(EmberEUI64 eui64a, EmberEUI64 eui64b)
{
    ...
    for (i = 0; i < EUI64_SIZE; i++)
    {
        if (eui64a[i] != 0xff || eui64b[i] != 0xff)
        {
            return;
        }
    }
...
uint16_t emberAfDeviceTableGetIndexFromEui64AndEndpoint(EmberEUI64 eui64, uint8_t endpoint)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_DEVICE_TABLE_DEVICE_TABLE_SIZE; i++)
    {
        if (matchEui64(deviceTable[i].eui64, eui64) && deviceTable[i].endpoint == endpoint)
        {
            return i;
        }
    }
...
uint16_t emberAfDeviceTableGetEndpointFromNodeIdAndEndpoint(EmberNodeId emberNodeId, uint8_t endpoint)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_DEVICE_TABLE_DEVICE_TABLE_SIZE; i++)
    {
        if (deviceTable[i].nodeId == emberNodeId && deviceTable[i].endpoint == endpoint)
        {
            return i;
        }
    }
...
void emAfDeviceTableCopyDeviceTableEntry(uint16_t fromIndex, uint16_t toIndex)
{
    ...
    assert(deviceTable[fromIndex].nodeId != EMBER_AF_PLUGIN_DEVICE_TABLE_NULL_NODE_ID);
...
uint16_t emAfDeviceTableFindIndexNodeIdEndpoint(uint16_t nodeId, uint8_t endpoint)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_DEVICE_TABLE_DEVICE_TABLE_SIZE; i++)
    {
        if (deviceTable[i].nodeId == nodeId && deviceTable[i].endpoint == endpoint)
        {
            return i;
        }
    }
...
void emberAfPluginDeviceTableStackStatusCallback(EmberStatus status)
{
    ...
    if (status == EMBER_NETWORK_DOWN && emberNetworkState() == EMBER_NO_NETWORK)
    {
...
void emAfDeviceTableSave(void)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_DEVICE_TABLE_DEVICE_TABLE_SIZE; i++)
    {
        if (deviceTable[i].nodeId != EMBER_AF_PLUGIN_DEVICE_TABLE_NULL_NODE_ID)
        {
            fprintf(fp, "%x %x %x ", deviceTable[i].nodeId, deviceTable[i].endpoint, deviceTable[i].deviceId);
            for (j = 0; j < 8; j++)
            {
...
void emAfDeviceTableLoad(void)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_DEVICE_TABLE_DEVICE_TABLE_SIZE && feof(fp) == false; i++)
    {
...
void emAfDeviceTableSendCieAddressWrite(EmberNodeId nodeId, uint8_t endpoint)
{
    ...
    emberAfSendUnicast(EMBER_OUTGOING_DIRECT, nodeId, &globalApsFrame, 14, outgoingBuffer);
}
...
void emberAfDeviceTableCliIndexSendWithEndpoint(uint16_t index, uint8_t endpoint)
{
    ...
    status = emberAfSendUnicast(EMBER_OUTGOING_DIRECT, nodeId, &globalApsFrame, appZclBufferLen, appZclBuffer);
...
void emberAfDeviceTableCommandIndexSendWithEndpoint(uint16_t index, uint8_t endpoint)
{
    ...
    emberAfCorePrintln("device table send with ep: 0x%2X, %d", nodeId, endpoint);
