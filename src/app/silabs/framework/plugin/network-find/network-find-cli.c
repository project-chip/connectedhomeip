...
const char * const emberCommandTableNetworkFindMaskCommandArguments[] = {
    "channel page (0 and, if sub-GHz support included, 28-31)", "channel mask", NULL
};

const char * const emberCommandTableNetworkFindModeCommandArguments[] = { "Bit 0 = enable 2.4GHz, Bit 1 = enable sub-GHz", NULL };
...
void findPrintCommand(void)
{
    ...
    emberAfCorePrintln("%s%s scan is %sABLED", "2.4 ", "GHz", (mode & FIND_AND_JOIN_MODE_ALLOW_2_4_GHZ) ? "EN" : "DIS");
    emberAfCorePrintln("%s%s scan is %sABLED", "Sub-", "GHz", (mode & FIND_AND_JOIN_MODE_ALLOW_SUB_GHZ) ? "EN" : "DIS");

    allowAllChannels = emberAfPluginNetworkFindGetEnableScanningAllChannelsCallback();
    emberAfCorePrintln("%s%s scan is %sABLED", "Fallback on ", "all channels", allowAllChannels ? "EN" : "DIS");
}
