...
#endif
static EmberStatus startSearchForJoinableNetwork(uint32_t channelMask, bool allChannels);
...
static uint32_t page28to31mask[EMBER_MAX_SUGBHZ_PAGE_NUMBER - EMBER_MIN_SUGBHZ_PAGE_NUMBER + 1] =
    EMBER_AF_PLUGIN_NETWORK_FIND_SUB_GHZ_CHANNEL_MASK;
...
#define CHANNEL_28_TO_31_PAGE_MASK(page) CHANNEL_28_TO_31_PAGE_MASK_WITH_PARAMETER(page, page28to31mask)

static const uint32_t page28to31maskAllChannels[] = EMBER_AF_PLUGIN_NETWORK_FIND_SUB_GHZ_ALL_CHANNELS_MASK;
#define CHANNEL_28_TO_31_PAGE_ALL_CHANNELS_MASK(page) CHANNEL_28_TO_31_PAGE_MASK_WITH_PARAMETER(page, page28to31maskAllChannels)
...
static uint8_t searchMode = FIND_AND_JOIN_MODE_ALLOW_2_4_GHZ | FIND_AND_JOIN_MODE_ALLOW_SUB_GHZ;
...
#define GET_RADIO_TX_POWER(pgChan)                                                                                                 \
    (emberAfGetPageFrom8bitEncodedChanPg(pgChan) == 0 ? EMBER_AF_PLUGIN_NETWORK_FIND_RADIO_TX_POWER                                \
...
void emberAfUnusedPanIdFoundCallback(EmberPanId panId, uint8_t channel)
{
    ...
    {
        emberAfAppPrintln("%p error 0x%x %p", "Form", status, "aborting");
        emberAfAppFlush();
        emberAfScanErrorCallback(status);
    }
}

void emberAfJoinableNetworkFoundCallback(EmberZigbeeNetwork * networkFound, uint8_t lqi, int8_t rssi)
{
    ...
    if (emberAfPluginNetworkFindJoinCallback(networkFound, lqi, rssi))
    {
        ...
        MEMMOVE(networkParams.extendedPanId, networkFound->extendedPanId, EXTENDED_PAN_ID_SIZE);
        ...
        emberAfAppPrintln("Nwk found, ch %d, panId 0x%2X, joining", networkFound->channel, networkFound->panId);
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfAppPrintln("%p error 0x%x, %p", "Join", status, "aborting");
        emberAfAppFlush();
...
void emberAfPluginNetworkFindTickEventHandler(void)
{
    ...
    else
    {
        ...
        if ((state == NETWORK_FIND_FORM || state == NETWORK_FIND_FORM_ALL_CHANNELS) && currentPage == 0 &&
            (searchMode & FIND_AND_JOIN_MODE_ALLOW_SUB_GHZ) != 0)
        {
            ...
            if (phyInterfaceCount > 1 && phyInterfaceCount != 0xFF)
            {
                ...
                status = emberScanForUnusedPanId(CHANNEL_28_TO_31_PAGE_MASK(currentPage), EMBER_AF_PLUGIN_NETWORK_FIND_DURATION);
                if (status == EMBER_SUCCESS)
                {
...
{
    emberAfAppPrintln("%p, no suitable primary channel found.", "Network find complete");
...
void emberAfScanErrorCallback(EmberStatus status)
{
    ...
    case EMBER_NO_BEACONS:
        ...
        if ((searchMode & FIND_AND_JOIN_MODE_ALLOW_SUB_GHZ) && currentPage < EMBER_MAX_SUGBHZ_PAGE_NUMBER)
        {
            ...
            newPageMask =
                allChannels ? CHANNEL_28_TO_31_PAGE_ALL_CHANNELS_MASK(currentPage) : CHANNEL_28_TO_31_PAGE_MASK(currentPage);
...
#endif
        if (state == NETWORK_FIND_JOIN && emberAfPluginNetworkFindGetEnableScanningAllChannelsCallback())
        {
    ...
    emberAfCoreFlush();
    state = NETWORK_FIND_NONE;
    emberAfAppPrintln("%p (scan error).", "Network find complete");
    emberAfPluginNetworkFindFinishedCallback(status);
}

static bool findInitialChannelMask(bool allChannels, uint32_t * initialChannelMask)
{
#ifdef EMBER_AF_PLUGIN_NETWORK_FIND_SUB_GHZ
    ...
    *initialChannelMask = allChannels ? EMBER_ALL_802_15_4_CHANNELS_MASK : page0mask;
    return true;
}
...
static EmberStatus startSearchForUnusedNetwork(bool allChannels)
{
    ...
    status = emberScanForUnusedPanId(initialChannelMask, EMBER_AF_PLUGIN_NETWORK_FIND_DURATION);
    if (status == EMBER_SUCCESS)
    {
        state = allChannels ? NETWORK_FIND_FORM_ALL_CHANNELS : NETWORK_FIND_FORM;
    }
    return status;
}
#endif
...
static EmberStatus startSearchForJoinableNetwork(uint32_t channelMask, bool allChannels)
{
    ...
    if (status == EMBER_SUCCESS)
    {
        state = allChannels ? NETWORK_FIND_JOIN_ALL_CHANNELS : NETWORK_FIND_JOIN;
    }
    return status;
}
...
#ifdef EMBER_AF_HAS_COORDINATOR_NETWORK
#ifdef EMBER_AF_PLUGIN_NETWORK_FIND_SUB_GHZ
    return state == NETWORK_FIND_FORM_ALL_CHANNELS || state == NETWORK_FIND_PHY2_ALL_CHANNELS;
#else
