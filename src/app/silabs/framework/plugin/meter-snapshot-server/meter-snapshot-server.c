...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Code to handle meter snapshot server
                                                                               *behavior.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
                                                          ...
                                                          uint8_t * snapshotSchedulePayload)
{
    ...
    emberAfPluginMeterSnapshotServerScheduleSnapshotCallback(cmd->apsFrame->destinationEndpoint, cmd->apsFrame->sourceEndpoint,
                                                             cmd->source, snapshotSchedulePayload, (uint8_t *) responsePayload);

    emberAfFillCommandSimpleMeteringClusterScheduleSnapshotResponse(issuerEventId, responsePayload, 2);
...
bool emberAfSimpleMeteringClusterTakeSnapshotCallback(uint32_t snapshotCause)
{
    ...
    snapshotId = emberAfPluginMeterSnapshotServerTakeSnapshotCallback(endpoint, snapshotCause, &snapshotConfirmation);
...
bool emberAfSimpleMeteringClusterGetSnapshotCallback(uint32_t startTime, uint32_t latestEndTime, uint8_t numberOfSnapshots,
                                                     uint32_t snapshotCause)
{
    ...
    emberAfCorePrintln("Start Time %u Endpoint %u snapshot Offset %u SnapShotCause %u", startTime, latestEndTime, numberOfSnapshots,
                       snapshotCause);
    // Get / publish the snapshot
    emberAfPluginMeterSnapshotServerGetSnapshotCallback(cmd->apsFrame->destinationEndpoint, cmd->apsFrame->sourceEndpoint,
                                                        cmd->source, (uint8_t *) snapshotCriteria);
