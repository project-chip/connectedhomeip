...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This file defines the proprietary Ember
                                                                               *standalone bootloader messages.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
bool emberAfPluginStandaloneBootloaderCommonCheckIncomingMessage(uint8_t length, uint8_t * message)
{
    ...
    {
        bootloadPrintln("Error: Protocol version in bootload message (%d) does not match mine (%d).", message[OFFSET_VERSION],
...
EmberStatus emberAfPluginStandaloneBootloaderCommonSendMessage(bool isBroadcast, EmberEUI64 targetEui, uint8_t length,
                                                               uint8_t * message)
{
    EmberStatus status = emAfSendBootloadMessage(isBroadcast, targetEui, length, message);
    if (EMBER_SUCCESS != status)
    {
        bootloadPrintln("Failed to send bootload message type: 0x%X, status: 0x%X", message[1], status);
    }
    return status;
}
