...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Routines for the GBCS Message Controller
                                                                               *plugin.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static uint16_t emAfPluginGbzMessageControllerAppendInNewMem(EmberAfGbzMessageCreatorState * state, EmberAfGbzZclCommand * zclCmd,
                                                             EmberAfGbzMessageData * data);
static uint16_t emAfPluginGbzMessageControllerAppendInPlace(EmberAfGbzMessageCreatorState * state, EmberAfGbzZclCommand * zclCmd,
                                                            EmberAfGbzMessageData * data);
...
static uint16_t emAfPluginGbzMessageControllerAppendResponses(uint8_t * dst, uint16_t maxLen,
...
void emberAfPluginGbzMessageControllerNextCommand(EmberAfGbzMessageParserState * state, EmberAfGbzZclCommand * gbzZclCommand)
{
    ...
    if (state->componentsParsed < state->componentsSize)
    {
        ...
        gbzZclCommand->direction =
            ((gbzZclCommand->frameControl & ZCL_FRAME_CONTROL_DIRECTION_MASK) ? ZCL_DIRECTION_SERVER_TO_CLIENT
        ...
        if (encryption)
        {
            ...
            if (data.encryption)
            {
                ...
                emberAfPluginGbzMessageControllerPrintBuffer(data.encryptedPayload, data.encryptedPayloadLength, true);
 ...
 */
static uint16_t emAfPluginGbzMessageControllerGetCommandLength(EmberAfGbzZclCommand * cmd, EmberAfGbzMessageData * data)
{
...
bool emberAfPluginGbzMessageControllerParserInit(EmberAfGbzMessageParserState * state, EmberAfGbzMessageType type,
                                                 uint8_t * gbzCommand, uint16_t gbzCommandLength, bool copyGbzCommand,
...
uint16_t emberAfPluginGbzMessageControllerCreatorInit(EmberAfGbzMessageCreatorState * state, EmberAfGbzMessageType type,
                                                      uint16_t alertCode, uint32_t timestamp, uint16_t messageCode,
                                                      uint8_t * gbzCommand, uint16_t gbzCommandLength)
{
    ...
    if (gbzCommand == NULL)
    { // inits for appends that requires mem allocation
        // inits
...
uint16_t emberAfPluginGbzMessageControllerAppendCommand(EmberAfGbzMessageCreatorState * creator, EmberAfGbzZclCommand * zclCmd)
{
    ...
    {
        result = emAfPluginGbzMessageControllerAppendInNewMem(creator, zclCmd, &data);
    }
    else
    {
        result = emAfPluginGbzMessageControllerAppendInPlace(creator, zclCmd, &data);
    }
    return result;
}
...
EmberAfGbzMessageCreatorResult * emberAfPluginGbzMessageControllerCreatorAssemble(EmberAfGbzMessageCreatorState * state)
{
    ...
    if (state->allocateMemoryForResponses)
    {
        ...
        emAfPluginGbzMessageControllerAppendResponses(&responsePayload[state->header->payloadLength],
                                                      len - state->header->payloadLength, state->responses);
    ...
    if (len > 0)
    {
        ...
        *state->componentsCount += 1;
        ...
        if (data->encryption)
        {
            ...
            MEMCOPY(&payload[index], data->encryptedPayload, data->encryptedPayloadLength);
                                                            ...
                                                            EmberAfGbzMessageData * data)
{
    ...
    if (state->commandLength < state->commandIndex + zclCommandLength)
    {
        emberAfPluginGbzMessageControllerPrintln("GBZ: ERR: Unable to append ZCL command to GBZ message: out of space (%d/%d).",
                                                 state->commandIndex + zclCommandLength, state->commandLength);
        return 0;
    }
    ...
    *state->componentsCount += 1;
    ...
    else if (zclCmd->hasFromDateTime)
    {
        ...
        emberAfCopyInt32u(state->command, state->commandIndex, zclCmd->fromDateTime);
    ...
    if (zclCmd->encryption)
    {
        ...
        // ZCl payload
        MEMCOPY(&state->command[state->commandIndex], data->encryptedPayload, data->encryptedPayloadLength);
    ...
    {
        // ZCl payload
        MEMCOPY(&state->command[state->commandIndex], zclCmd->payload, zclCmd->payloadLength);
...
bool emberAfPluginGbzMessageControllerGetEncryptPayloadFlag(EmberAfGbzMessageCreatorState * creator, EmberAfGbzZclCommand * resp)
{
    ...
    if (!resp->clusterSpecific)
    {
        ...
        {
            if ((creator->messageCode == GCS13a_MESSAGE_CODE) || (creator->messageCode == GCS13b_MESSAGE_CODE) ||
                (creator->messageCode == GCS13c_MESSAGE_CODE))
            {
                return true;
            }
            else if ((creator->messageCode == GCS21b_MESSAGE_CODE) || (creator->messageCode == GCS21f_MESSAGE_CODE) ||
                     (creator->messageCode == GCS21j_MESSAGE_CODE) || (creator->messageCode == GCS33_MESSAGE_CODE) ||
                     (creator->messageCode == GCS38_MESSAGE_CODE) || (creator->messageCode == GCS46_MESSAGE_CODE) ||
            ...
            else if (creator->messageCode == GCS14_MESSAGE_CODE)
            {
                ...
                while (bufIndex + 3 <= bufLen)
                {
                    ...
                    if (status == EMBER_ZCL_STATUS_SUCCESS)
                    {
                        ...
                        {
                            if (attributeId == ZCL_DEBT_AMOUNT_1_ATTRIBUTE_ID || (attributeId == ZCL_DEBT_AMOUNT_2_ATTRIBUTE_ID) ||
    ...
    else
    {
        ...
        case GCS15b_MESSAGE_CODE:
            if ((*clusterId == ZCL_PREPAYMENT_CLUSTER_ID) && (*commandId == ZCL_PUBLISH_PREPAY_SNAPSHOT_COMMAND_ID))
            {
                return true;
            }
            else if ((*clusterId == ZCL_SIMPLE_METERING_CLUSTER_ID) && (*commandId == ZCL_PUBLISH_SNAPSHOT_COMMAND_ID))
            {
                return true;
            }
            break;
        case GCS15c_MESSAGE_CODE:
            if ((*clusterId == ZCL_SIMPLE_METERING_CLUSTER_ID) && (*commandId == ZCL_PUBLISH_SNAPSHOT_COMMAND_ID))
            {
                return true;
            }
            else if ((*clusterId == ZCL_PREPAYMENT_CLUSTER_ID) && (*commandId == ZCL_PUBLISH_PREPAY_SNAPSHOT_COMMAND_ID))
            {
                return true;
            }
            break;
        case GCS15d_MESSAGE_CODE:
            if ((*clusterId == ZCL_PREPAYMENT_CLUSTER_ID) && (*commandId == ZCL_PUBLISH_DEBT_LOG_COMMAND_ID))
            {
                return true;
            }
            break;
        ...
        case GCS17_MESSAGE_CODE:
            if ((*clusterId == ZCL_SIMPLE_METERING_CLUSTER_ID) && (*commandId == ZCL_GET_SAMPLED_DATA_RESPONSE_COMMAND_ID))
            {
                return true;
            }
            break;
        ...
        case GCS21d_MESSAGE_CODE:
            if ((*clusterId == ZCL_PRICE_CLUSTER_ID) && (*commandId == ZCL_PUBLISH_BILLING_PERIOD_COMMAND_ID))
            {
                return false;
            }
            break;
