...
void emAfPluginMeterMirrorStatusCommand(void)
{
    ...
    for (endpoint = EMBER_AF_PLUGIN_METER_MIRROR_ENDPOINT_START;
         endpoint < EMBER_AF_PLUGIN_METER_MIRROR_ENDPOINT_START + EMBER_AF_PLUGIN_METER_MIRROR_MAX_MIRRORS; endpoint++)
    {
        bool validEndpoint;
        ...
        validEndpoint               = emberAfPluginMeterMirrorGetEui64ByEndpoint(endpoint, mirrorEui);
        ...
        if (emberAfPluginMeterMirrorIsMirrorUsed(endpoint))
        {
            ...
            emberAfReadServerAttribute(endpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_METERING_DEVICE_TYPE_ATTRIBUTE_ID,
                                       &meteringDeviceTypeAttribute, sizeof(meteringDeviceTypeAttribute));
        }
    ...
    emberAfSimpleMeteringClusterPrintln("%d of %d mirrors used.", emAfPluginMeterMirrorGetMirrorsAllocated(),
