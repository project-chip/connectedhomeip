...
void emberAfPluginCoulombCounterTick(void)
{
    ...
    if (computeElapsed(currentTime, tokenTimeMarker) >= (EMBER_AF_PLUGIN_COULOMB_COUNTER_TOKEN_SAVE_INTERVAL * 1000))
    {
        DISABLE_INTERRUPTS();
...
void halSleepCallback(boolean enter, SleepModes sleepMode)
{
    ...
    if (enter == true)
    {
        ...
        {
            emberAfPluginCoulombCounterEventCallback(EMBER_AF_PLUGIN_COULOMB_COUNTER_EM_MODE_MASK,
...
{
    uint32_t eepromStateShifted = ((uint32_t) newEepromState << EMBER_AF_PLUGIN_COULOMB_COUNTER_EEPROM_STATE_SHIFT) &
        EMBER_AF_PLUGIN_COULOMB_COUNTER_EEPROM_MODE_MASK;

    emberAfPluginCoulombCounterEventCallback(EMBER_AF_PLUGIN_COULOMB_COUNTER_EEPROM_MODE_MASK, eepromStateShifted);
}
