...
{
#ifdef EZSP_HOST
    EzspStatus status = ezspSetEndpointFlags(endpoint, EZSP_ENDPOINT_DISABLED);
 ...
 *  */
void emAfPluginGasProxyFunctionLogEvent(uint8_t * gbzCmd, uint16_t gbzCmdLen, uint8_t * gbzResp, uint16_t gbzRespLen,
                                        uint16_t eventId, EmberAfGPFMessageType cmdType, uint16_t messageCode)
{
    EmberAfEvent event;
    ...
    if (eventId == GBCS_EVENT_ID_IMM_HAN_CMD_RXED_ACTED || eventId == GBCS_EVENT_ID_IMM_HAN_CMD_RXED_NOT_ACTED)
    {
        logMsg[0] = 0x02;
    ...
    if ((messageCode == GCS01a_MESSAGE_CODE) || (messageCode == GCS01b_MESSAGE_CODE) || (messageCode == GCS05_MESSAGE_CODE) ||
        (messageCode == GCS07_MESSAGE_CODE) || (messageCode == GCS23_MESSAGE_CODE) || (messageCode == GCS25_MESSAGE_CODE) ||
    ...
    emberAfEventsServerAddEvent(endpoint, EMBER_ZCL_EVENT_LOG_ID_GENERAL_EVENT_LOG, &event);
}

static EmberStatus emberAfPluginGasProxyFunctionReplayZclCommand(uint8_t srcEndpoint, uint8_t dstEndpoint,
                                                                 EmberAfGbzZclCommand * cmd)
{
    ...
    if (dstEndpoint == EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_MIRROR_ENDPOINT && !emberAfIsDeviceEnabled(dstEndpoint))
    {
        emberAfPluginGasProxyFunctionPrintln("GPF: ERR: Cannot replay command. Mirror is not available.");
        emberAfFillExternalBuffer(((cmd->direction == ZCL_DIRECTION_SERVER_TO_CLIENT) ? ZCL_FRAME_CONTROL_CLIENT_TO_SERVER
                                                                                      : ZCL_FRAME_CONTROL_SERVER_TO_CLIENT),
                                  cmd->clusterId, ZCL_DEFAULT_RESPONSE_COMMAND_ID, "uu", cmd->commandId, EMBER_ZCL_STATUS_FAILURE);
        emberAfSetCommandEndpoints(srcEndpoint, srcEndpoint);
    }
    else
    {
        emberAfFillExternalBuffer(cmd->frameControl, cmd->clusterId, cmd->commandId, "");
...
static bool matchingZclCommands(EmberAfGbzZclCommand * cmd, EmberAfGbzZclCommand * cmdResp)
{
    ...
    {
        emberAfPluginGasProxyFunctionPrintln("GPF: ERR: No matching ZCL tran seq number (0x%X/0x%X) in ZCL cmd/cmd resp pair!",
                                             cmd->transactionSequenceNumber, cmdResp->transactionSequenceNumber);
        result = false;
    }

    if (cmd->clusterId != cmdResp->clusterId)
    {
        emberAfPluginGasProxyFunctionPrintln("GPF: ERR: No matching ZCL cluster id (%2X/%2X) in ZCL cmd/cmd resp pair!",
                                             cmd->clusterId, cmdResp->clusterId);
        result = false;
    }
    ...
    /*emberAfPluginGasProxyFunctionPrintln("GPF: ERR: No matching ZCL command id (%x/%x) in ZCL cmd/cmd resp pair!", cmd->commandId,
     * cmdResp->commandId);*/
    /*result = false;*/
    /*}*/
 ...
 */
EmberStatus emberAfPluginGasProxyFunctionTapOffMessageHandler(uint8_t * gbzCommands, uint16_t gbzCommandsLength,
                                                              uint8_t * gbzCommandsResponse, uint16_t gbzCommandsResponseLength,
                                                              uint16_t messageCode)
{
    ...
    emberAfPluginGbzMessageControllerParserInit(&gbzCommandHandler, EMBER_AF_GBZ_MESSAGE_COMMAND, gbzCommands, gbzCommandsLength,
                                                false, messageCode);
    emberAfPluginGbzMessageControllerParserInit(&gbzCommandResponseHandler, EMBER_AF_GBZ_MESSAGE_RESPONSE, gbzCommandsResponse,
                                                gbzCommandsResponseLength, false, messageCode);
    ...
    while (emberAfPluginGbzMessageControllerHasNextCommand(&gbzCommandHandler))
    {
            ...
            emberAfPluginGbzMessageControllerGetZclDefaultResponse(&cmdResp) == EMBER_ZCL_STATUS_SUCCESS)
        {
            ...
            // zcl direction.
            if ((cmd.clusterId == ZCL_EVENTS_CLUSTER_ID) && (cmd.commandId == ZCL_CLEAR_EVENT_LOG_REQUEST_COMMAND_ID))
            {
            ...
            status = emberAfPluginGasProxyFunctionValidateIncomingZclCommandCallback(&clusterCmd, messageCode);
            ...
            else if (status == EMBER_AF_GPF_ZCL_COMMAND_PERMISSION_IGNORED)
            {
                emberAfPluginGasProxyFunctionPrintln(
                    "GPF: Info: Command ignored: ZCL command(clus 0x%2X, cmd 0x%x) embedded within Tap Off Message.",
                    clusterCmd.apsFrame->clusterId, clusterCmd.commandId);
            }
            else
            {
                ...
                // here.
                emberAfPluginGasProxyFunctionPrintln(
                    "GPF: ERR: Unsupported ZCL command(clus 0x%2X, cmd 0x%x) embedded within Tap Off Message!",
                    clusterCmd.apsFrame->clusterId, clusterCmd.commandId);
                status = EMBER_ERR_FATAL;
                goto kickout;
            }
...
kickout:
    ...
    {
        emberAfPluginGasProxyFunctionPrintln(
            "GPF: TOM message has been successfully processed and will be logged to Event cluster on ep(%d).",
            EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT);
    }

    // time to log these lovely TOM commands to the Event log.
    emAfPluginGasProxyFunctionLogEvent(gbzCommands, gbzCommandsLength, gbzCommandsResponse, gbzCommandsResponseLength,
                                       (status == EMBER_SUCCESS) ? GBCS_EVENT_ID_IMM_HAN_CMD_RXED_ACTED
                                                                 : GBCS_EVENT_ID_IMM_HAN_CMD_RXED_NOT_ACTED,
                                       EMBER_AF_GPF_MESSAGE_TYPE_TOM, messageCode);
...
static bool sendNextNonTomZclCmd(void)
{
    ...
    if (cmd.clusterId == ZCL_PRICE_CLUSTER_ID || cmd.clusterId == ZCL_EVENTS_CLUSTER_ID || cmd.clusterId == ZCL_CALENDAR_CLUSTER_ID)
    {
        status = emberAfPluginGasProxyFunctionReplayZclCommand(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_HIDDEN_CLIENT_SIDE_ENDPOINT,
                                                               EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, &cmd);
    }
    else if (cmd.clusterId == ZCL_SIMPLE_METERING_CLUSTER_ID || cmd.clusterId == ZCL_PREPAYMENT_CLUSTER_ID)
    {
        status = emberAfPluginGasProxyFunctionReplayZclCommand(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_HIDDEN_CLIENT_SIDE_ENDPOINT,
                                                               EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_MIRROR_ENDPOINT, &cmd);
    }
    ...
    {
        if ((cmd.commandId == ZCL_PUBLISH_CHANGE_OF_TENANCY_COMMAND_ID) && (cmd.direction == ZCL_DIRECTION_SERVER_TO_CLIENT))
        {
            status =
                emberAfPluginGasProxyFunctionReplayZclCommand(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_HIDDEN_CLIENT_SIDE_ENDPOINT,
                                                              EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_HIDDEN_CLIENT_SIDE_ENDPOINT, &cmd);
        }
        else
        {
            status = emberAfPluginGasProxyFunctionReplayZclCommand(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_HIDDEN_CLIENT_SIDE_ENDPOINT,
...
void emAfGasProxyFunctionAlert(uint16_t alertCode, EmberAfClusterCommand * cmd, uint16_t messageCode)
{
    ...
    emberAfPluginGbzMessageControllerCreatorInit(&creator, EMBER_AF_GBZ_MESSAGE_ALERT, alertCode, emberAfGetCurrentTime(),
                                                 messageCode, NULL, 0);
    emberAfPluginGbzMessageControllerAppendCommand(&creator, &gbzZclCmd);
                                                                 ...
                                                                 uint16_t messageCode)
{
    ...
    if (!emberAfPluginGbzMessageControllerParserInit(&nonTomGbzRequestParser, EMBER_AF_GBZ_MESSAGE_COMMAND, gbzCommands,
                                                     gbzCommandsLength, true, messageCode))
    {
        status = EMBER_ERR_FATAL;
        goto kickout;
    }

    if (!emberAfPluginGbzMessageControllerCreatorInit(&nonTomGbzResponseCreator, EMBER_AF_GBZ_MESSAGE_RESPONSE, 0, 0, messageCode,
                                                      NULL, 0))
    {
        status = EMBER_ERR_FATAL;
        goto kickout;
...
kickout:
    if (status != EMBER_SUCCESS)
    {
        ...
        emAfPluginGasProxyFunctionLogEvent(gbzCommands, gbzCommandsLength, NULL, 0, GBCS_EVENT_ID_IMM_HAN_CMD_RXED_NOT_ACTED,
                                           EMBER_AF_GPF_MESSAGE_TYPE_NON_TOM, messageCode);
    }
    return status;
}
...
static void captureNonTomZclCmdResp(EmberAfClusterCommand * cmd)
{
    EmberAfGbzZclCommand gbzZclCmd = { 0 };
    EmberAfGbzMessageCreatorResult * gbzResponse;
    emberAfPluginGasProxyFunctionPrintln("GPF: Intercepting ZCL cmd(0x%X) on endpoint: %d", cmd->commandId,
    ...
    emberAfPluginGbzMessageControllerAppendCommand(&nonTomGbzResponseCreator, &gbzZclCmd);

    // allocate final GBZ Non-TOM response buffer
    if (nonTomExpectedZclResps == ++nonTomResponsesCount && !sendNextNonTomZclCmd())
    {
        ...
        emAfPluginGasProxyFunctionLogEvent(nonTomGbzRequestParser.command, nonTomGbzRequestParser.length, gbzResponse->payload,
                                           gbzResponse->payloadLength, GBCS_EVENT_ID_IMM_HAN_CMD_RXED_ACTED,
                                           EMBER_AF_GPF_MESSAGE_TYPE_NON_TOM, nonTomGbzRequestParser.messageCode);
...
{
    if (!nonTomHandlingActive || (cmd->source != emberGetNodeId()))
    {
        return false;
    }
                                                      ...
                                                      uint8_t calorificValueUnit, uint8_t calorificValueTrailingDigit)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishCalorificValue 0x%4X, 0x%4X, 0x%4X, 0x%X, 0x%X", issuerEventId, startTime,
    ...
    status = emberAfPluginPriceServerCalorificValueAdd(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, issuerEventId, startTime,
                                                       calorificValue, calorificValueUnit, calorificValueTrailingDigit);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
                                                        ...
                                                        uint8_t conversionFactorTrailingDigit)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishConversionFactor 0x%4X, 0x%4X, 0x%4X, 0x%X", issuerEventId, startTime,
    ...
    status = emberAfPluginPriceServerConversionFactorAdd(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, issuerEventId, startTime,
                                                         conversionFactor, conversionFactorTrailingDigit);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
                                                   ...
                                                   uint8_t numberOfDayProfiles)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishCalendar 0x%4x, 0x%4x, 0x%4x, 0x%4x, 0x%x, \"", providerId, issuerEventId,
                                         issuerCalendarId, startTime, calendarType);
    emberAfPluginGasProxyFunctionPrintString(calendarName);
    emberAfPluginGasProxyFunctionPrintln("\", %d, %d, %d", numberOfSeasons, numberOfWeekProfiles, numberOfDayProfiles);
    ...
    status = emberAfCalendarCommonAddCalInfo(providerId, issuerEventId, issuerCalendarId, startTime, calendarType, calendarName,
                                                     ...
                                                     uint8_t * dayScheduleEntries)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishDayProfile 0x%4x, 0x%4x, 0x%4x, %d, %d, %d, %d, 0x%x", providerId,
                                         issuerEventId, issuerCalendarId, dayId, totalNumberOfScheduleEntries, commandIndex,
                                         totalNumberOfCommands, calendarType);

    status = emberAfCalendarCommonAddDayProfInfo(issuerCalendarId, dayId, dayScheduleEntries, dayScheduleEntriesLength);
    return true;
}
...
{
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishBlockThresholds 0x%4X, 0x%4X, 0x%4X, 0x%4X, 0x%X, 0x%X, 0x%X", providerId,
                                         issuerEventId, startTime, issuerTariffId, commandIndex, numberOfCommands,
                                         subpayloadControl);
    ...
    emberAfPriceAddBlockThresholdsTableEntry(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, providerId, issuerEventId, startTime,
                                             issuerTariffId, commandIndex, numberOfCommands, subpayloadControl, payload);
                                                         ...
                                                         uint32_t blockThresholdDivisor)
{
    ...
    emberAfPriceAddTariffTableEntry(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, &info, &tariff);
    return true;
}
...
bool emberAfPriceClusterPublishPriceMatrixCallback(uint32_t providerId, uint32_t issuerEventId, uint32_t startTime,
                                                   ...
                                                   uint8_t subPayloadControl, uint8_t * payload)
{
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishPriceMatrix 0x%4X, 0x%4X, 0x%4X, 0x%4X, 0x%X, 0x%X, 0x%X", providerId,
                                         issuerEventId, startTime, issuerTariffId, commandIndex, numberOfCommands,
                                         subPayloadControl);
    ...
    emberAfPriceAddPriceMatrixRaw(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, providerId, issuerEventId, startTime,
                                  issuerTariffId, commandIndex, numberOfCommands, subPayloadControl, payload);
                                                  ...
                                                  uint8_t commandIndex, uint8_t totalNumberOfCommands, uint8_t * seasonEntries)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishSeasons 0x%4x, 0x%4x, 0x%4x, %d, %d", providerId, issuerEventId,
                                         issuerCalendarId, commandIndex, totalNumberOfCommands);

    success = emberAfCalendarServerAddSeasonsInfo(issuerCalendarId, seasonEntries, seasonEntriesLength, &unknownWeekIdSeasonsMask);

    if (!success && unknownWeekIdSeasonsMask != 0)
    {
        emberAfPluginGasProxyFunctionUnknownSeasonWeekIdCallback(issuerCalendarId, seasonEntries, seasonEntriesLength,
                                                      ...
                                                      uint8_t * specialDayEntries)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishSpecialDays 0x%4x, 0x%4x, 0x%4x, 0x%4x, 0x%x, %d, %d, %d, [", providerId,
                                         issuerEventId, issuerCalendarId, startTime, calendarType, totalNumberOfSpecialDays,
    ...
    success = emberAfCalendarCommonAddSpecialDaysInfo(issuerCalendarId, totalNumberOfSpecialDays, specialDayEntries,
                                                      speicalDaysEntriesLength, &unknownSpecialDaysMask);
    if (!success && unknownSpecialDaysMask != 0)
    {
        emberAfPluginGasProxyFunctionUnknownSpecialDaysDayIdCallback(issuerCalendarId, specialDayEntries, speicalDaysEntriesLength,
                                                      ...
                                                      uint8_t dayIdRefSaturday, uint8_t dayIdRefSunday)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishWeekProfile 0x%4x, 0x%4x, 0x%4x, %d, %d, %d, %d, %d, %d, %d, %d",
                                         providerId, issuerEventId, issuerCalendarId, weekId, dayIdRefMonday, dayIdRefTuesday,
                                         dayIdRefWednesday, dayIdRefThursday, dayIdRefFriday, dayIdRefSaturday, dayIdRefSunday);

    status = emberAfCalendarServerAddWeekProfInfo(issuerCalendarId, weekId, dayIdRefMonday, dayIdRefTuesday, dayIdRefWednesday,
                                                   ...
                                                   uint8_t optionalExtendedMessageControl)
{
    ...
    emberAfPluginMessagingServerSetMessage(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, &message);
    return true;
}
                                                                   ...
                                                                   uint8_t * proposedProviderContactDetails)
{
    ...
    emberAfCopyString(supplier.proposedProviderName, proposedProviderName,
                      EMBER_AF_DEVICE_MANAGEMENT_MAXIMUM_PROPOSED_PROVIDER_NAME_LENGTH);
    emberAfCopyString(supplier.proposedProviderContactDetails, proposedProviderContactDetails,
                                                                  ...
                                                                  uint32_t proposedTenancyChangeControl)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("RX: PublishChangeOfTenancy: 0x%4X, 0x%4X, 0x%X, 0x%4X, 0x%4X", providerId, issuerEventId,
                                                   ...
                                                   uint8_t tariffResolutionPeriod)
{
    ...
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishBlockPeriod 0x%4X, 0x%4X, 0x%4X, 0x%4X, 0x%X, 0x%X, 0x%X, 0x%X",
                                         providerId, issuerEventId, blockPeriodStartTime, blockPeriodDuration, blockPeriodControl,
    ...
    emberAfReadAttribute(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, ZCL_PRICE_CLUSTER_ID,
                         ZCL_THRESHOLD_MULTIPLIER_ATTRIBUTE_ID, CLUSTER_MASK_SERVER, (uint8_t *) &(thresholdMultiplier),
                         emberAfGetDataSize(ZCL_INT24U_ATTRIBUTE_TYPE), NULL);

    emberAfReadAttribute(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, ZCL_PRICE_CLUSTER_ID, ZCL_THRESHOLD_DIVISOR_ATTRIBUTE_ID,
                         CLUSTER_MASK_SERVER, (uint8_t *) &(thresholdDivisor), emberAfGetDataSize(ZCL_INT24U_ATTRIBUTE_TYPE), NULL);

    emberAfPluginPriceServerBlockPeriodAdd(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, providerId, issuerEventId,
                                           blockPeriodStartTime, blockPeriodDuration, blockPeriodControl, blockPeriodDurationType,
                                           thresholdMultiplier, thresholdDivisor, tariffType, tariffResolutionPeriod);
    return true;
}
...
bool emberAfPriceClusterPublishBillingPeriodCallback(uint32_t providerId, uint32_t issuerEventId, uint32_t billingPeriodStartTime,
                                                     ...
                                                     uint8_t tariffType)
{
    emberAfPluginGasProxyFunctionPrintln("GPF: RX: PublishBillingPeriod 0x%4X, 0x%4X, 0x%4X, 0x%4X, 0x%X, 0x%X", providerId,
                                         issuerEventId, billingPeriodStartTime, billingPeriodDuration, billingPeriodDurationType,
                                         tariffType);
    ...
    emberAfPluginPriceServerBillingPeriodAdd(EMBER_AF_PLUGIN_GAS_PROXY_FUNCTION_ESI_ENDPOINT, billingPeriodStartTime, issuerEventId,
                                             providerId, billingPeriodDuration, billingPeriodDurationType, tariffType);
    return true;
}

EmberAfGpfMessage * emberAfPluginGasProxyFunctionGetCurrentMessage()
{
    if ((currentGpfMessage.gbzCommands == NULL) && (currentGpfMessage.gbzCommandsLength == 0))
    {
        return NULL;
    }
...
bool emberAfPluginGasProxyFunctionIgnoreFutureCommand(uint32_t startTime)
{
    ...
    if ((startTime > now) && (emberAfPluginGasProxyFunctionGetCurrentMessage() != NULL))
    {
