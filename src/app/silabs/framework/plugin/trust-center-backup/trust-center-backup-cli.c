...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief User interface for backing up and restoring
                                                                               *a trust center.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
EmberCommandEntry emberAfPluginTrustCenterBackupCommands[] = {
    ...
    emberCommandEntryAction("print-export", printExportDataCommand, "", "Print the set of export data that a TC must backup"),
    emberCommandEntryAction("import-key", importKeyCommand, "ubb", "Set a key in the import data set"),
    emberCommandEntryAction("set-ext-pan", setExtendedPanIdCommand, "b", "Set the extended PAN ID in the import data set."),
    emberCommandEntryAction("print-import", printImportDataCommand, "", "Print the import data set"),
    emberCommandEntryAction("clear-import", importClearCommand, "", "Clear the import data set"),
    ...
    emberCommandEntryAction("file-export", emAfTcExportCommand, "b", "Write the TC backup data to a file"),
...
static EmberAfTrustCenterBackupData importData = { { 0, 0, 0, 0, 0, 0, 0, 0 }, // extended PAN ID (filled in later)
...
void importClearCommand(void)
{
    ...
    MEMSET(importKeyList, 0, sizeof(EmberAfLinkKeyBackupData) * MAX_CLI_SIZE);
...
void importKeyCommand(void)
{
    ...
    {
        emberAfSecurityPrintln("%p: EUI64 cannot be all zeroes.", "Error");
        return;
    }

    if (index > MAX_CLI_SIZE)
    {
        emberAfSecurityPrintln("%p: index %d is greater than max import limit of %d.", "Error", index, MAX_CLI_SIZE);
        return;
    }

    MEMMOVE(importKeyList[index].deviceId, partnerEUI64, EUI64_SIZE);
    MEMMOVE(emberKeyContents(&(importKeyList[index].key)), emberKeyContents(&newKey), EMBER_ENCRYPTION_KEY_SIZE);
...
void restoreFromBackupCommand(void)
{
    ...
    if (0 == MEMCOMPARE(importData.extendedPanId, emberAfNullEui64, EUI64_SIZE))
    {
        emberAfSecurityPrintln("%p: Import contains NULL extended PAN ID", "Error");
        return;
    }
