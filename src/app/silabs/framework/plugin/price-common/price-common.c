...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Routines for the Price Common plugin, which
                                                                               *implement the common features of the Price cluster.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
                                  ...
                                  uint16_t dataArraySize)
{
    ...
    for (j = 0; j < dataArraySize - 1; j++)
    {
        ...
        for (i = j + 1; i < dataArraySize; i++)
        {
            ...
            else if (commonInfos[i].valid && commonInfos[iMin].valid && (commonInfos[i].startTime < commonInfos[iMin].startTime))
            {
        ...
        if (iMin != j)
        {
            ...
            // swap block size
            swap(dataArray + j * dataArrayBlockSizeInByte, dataArray + iMin * dataArrayBlockSizeInByte, dataArrayBlockSizeInByte);
        }
    }
}
                                                               ...
                                                               bool expireTimedOut)
{
    ...
    for (i = 0; i < numberOfEntries; i++)
    {
        ...
        if ((!commonInfos[i].valid) || (expireTimedOut && (endTime <= timeNow)))
        {
        ...
        else if ((commonInfos[i].startTime == newStartTime) && (commonInfos[i].issuerEventId == newIssuerEventId))
        {
            // Match found
            smallestEventIdIndex = i;
            break;
        }
        else if ((commonInfos[i].issuerEventId < smallestEventId) && (commonInfos[i].issuerEventId < newIssuerEventId))
        {
                                                 ...
                                                 uint32_t minIssuerEventId, uint8_t numberOfCommands)
{
    ...
    for (i = 0; i < numberOfEntries; i++)
    {
        ...
        { // keep track of the startTime before currentTime.
          ...
          // the last startTime will be valid.
            ...
            else if (startTrackingLastStartTime == 1)
            { // update lastStartTime
    ...
    if ((numberOfCommands != 0x0) && (numberOfCommands < numberOfValidCommands))
    {
...
uint8_t emberAfPluginPriceCommonServerGetActiveIndex(EmberAfPriceCommonInfo * commonInfos, uint8_t numberOfEntries)
{
    ...
    for (i = 0; i < numberOfEntries; i++)
    {
        if (commonInfos[i].valid && (commonInfos[i].startTime <= timeNow))
        {
            if (commonInfos[i].durationSec == ZCL_PRICE_CLUSTER_DURATION_SEC_UNTIL_CHANGED)
            {
                endTime = ZCL_PRICE_CLUSTER_END_TIME_NEVER;
            ...
            if ((endTime >= timeNow) && (commonInfos[i].issuerEventId > maxCurrentEventId))
            {
