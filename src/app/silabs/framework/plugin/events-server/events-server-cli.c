...
void emAfEventsServerCliSet(void)
{
    ...
    length             = emberCopyStringArgument(5, event.eventData + 1, EMBER_AF_PLUGIN_EVENTS_SERVER_EVENT_DATA_LENGTH, false);
...
void emAfEventsServerCliAdd(void)
{
    ...
    length             = emberCopyStringArgument(4, event.eventData + 1, EMBER_AF_PLUGIN_EVENTS_SERVER_EVENT_DATA_LENGTH, false);
    event.eventData[0] = length;
...
void emAfEventsServerCliPublish(void)
{
    ...
    emberAfEventsServerPublishEventMessage(nodeId, srcEndpoint, dstEndpoint, logId, index, eventControl);
}
