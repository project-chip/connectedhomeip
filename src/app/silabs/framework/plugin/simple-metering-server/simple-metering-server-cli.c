...
static uint8_t getEndpointArgument(uint8_t index)
{
    ...
    return (endpoint == 0 ? emberAfPrimaryEndpointForCurrentNetworkIndex() : endpoint);
}
...
void emAfPluginSimpleMeteringServerCliRate(void)
{
    afTestMeterSetConsumptionRate((uint16_t) emberUnsignedCommandArgument(0), (uint8_t) emberUnsignedCommandArgument(1));
}
...
void emAfPluginSimpleMeteringServerCliSetError(void)
{
    ...
    afTestMeterSetError(getEndpointArgument(1), (uint8_t) emberUnsignedCommandArgument(0));
}
#endif // EMBER_AF_PLUGIN_SIMPLE_METERING_SERVER_TEST_METER_ERRORS
...
void emAfPluginSimpleMeteringServerCliConfigureMirror(void)
{
    ...
    emberAfFillCommandSimpleMeteringClusterConfigureMirror(issuerId, reportingInterval, mirrorNotificationReporting,
                                                           notificationScheme);
