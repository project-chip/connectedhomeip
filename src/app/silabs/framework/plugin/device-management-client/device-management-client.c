...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Routines for the Device Management Client
                                                                               *plugin, which is based on the SE 1.2 cluster.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static void sendDeviceManagementClusterReportWildCardAttribute(uint8_t attributeSet, uint8_t endpoint)
{
    uint8_t attrSet, j, k;
    ...
    emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER), ZCL_DEVICE_MANAGEMENT_CLUSTER_ID,
                              ZCL_REPORT_EVENT_CONFIGURATION_COMMAND_ID, "uu", 0, 1);
    ...
    for (attrSet = 0; attrSet < numberOfAttrSetsToReport; attrSet++)
    {
        for (j = 0; j < 7; j++)
        {
            ...
            for (k = attributeTable[actualSet].attributeRange[j].startIndex;
                 k < attributeTable[actualSet].attributeRange[j].endIndex; k++)
            {
                uint8_t topByte      = (attributeSet == 0xFF) ? attributeTable[attrSet].attributeSetId : attributeSet;
                uint16_t attributeId = ((topByte << 8) | k);
                if (emberAfLocateAttributeMetadata(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_CLIENT,
                                                   EMBER_AF_NULL_MANUFACTURER_CODE))
                {
                    status = emberAfReadClientAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, attributeId,
                                                        (uint8_t *) &eventConfiguration, sizeof(eventConfiguration));
...
static void writeDeviceManagementClusterWildCardAttribute(uint8_t attributeSet, uint8_t endpoint, uint8_t attributeConfiguration)
{
    ...
    {
        for (attrId = attributeTable[attributeSet].attributeRange[attrRange].startIndex;
             attrId < attributeTable[attributeSet].attributeRange[attrRange].endIndex; attrId++)
        {
            uint16_t attributeId = ((attributeSet << 8) | attrId);
            if (emberAfLocateAttributeMetadata(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_CLIENT,
                                               EMBER_AF_NULL_MANUFACTURER_CODE))
            {
                status = emberAfWriteAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_CLIENT,
                                               (uint8_t *) &attributeConfiguration, ZCL_BITMAP8_ATTRIBUTE_TYPE);
            }
        }
    }
}

static void writeDeviceManagementClusterByLogTypeAttribute(uint8_t logType, uint8_t endpoint, uint8_t attributeConfiguration)
{
    uint8_t i, j, k;
    uint8_t eventConfiguration;
    EmberAfStatus status;
            ...
            for (k = attributeTable[i].attributeRange[j].startIndex; k < attributeTable[i].attributeRange[j].endIndex; k++)
            {
                                                   ...
                                                   EMBER_AF_NULL_MANUFACTURER_CODE))
                {
                    ...
                    if (status == EMBER_ZCL_STATUS_SUCCESS)
                    {
                        ...
                        {
                            EmberAfStatus status =
                                emberAfWriteAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_CLIENT,
                                                      (uint8_t *) &attributeConfiguration, ZCL_BITMAP8_ATTRIBUTE_TYPE);
                            if (status == EMBER_ZCL_STATUS_SUCCESS)
                            {
...
static void writeDeviceManagementClusterByMatchingAttribute(uint8_t currentConfiguration, uint8_t endpoint,
...
static void writeDeviceManagementAttribute(uint16_t attributeId, uint8_t attributeConfiguration)
{
    uint8_t endpoint     = emberAfCurrentEndpoint();
    EmberAfStatus status = emberAfWriteAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, attributeId, CLUSTER_MASK_CLIENT,
                                                 (uint8_t *) &attributeConfiguration, ZCL_BITMAP8_ATTRIBUTE_TYPE);
    if (status == EMBER_ZCL_STATUS_SUCCESS)
    {
...
void emberAfDeviceManagementClusterClientTickCallback(uint8_t endpoint)
{
    ...
    if (pmInfo.pendingUpdates & EMBER_AF_DEVICE_MANAGEMENT_CHANGE_OF_TENANCY_PENDING_MASK)
    {
        if (currentTime >= pmInfo.tenancy.implementationDateTime)
        {
            emberAfDeviceManagementClusterPrintln("DEVICE MANAGEMENT CLIENT: Enacting change of tenancy at time 0x%4x\n",
                                                  currentTime);
            emberAfPluginDeviceManagementClientEnactChangeOfTenancyCallback(endpoint, &(pmInfo.tenancy));
    ...
    if (pmInfo.pendingUpdates & EMBER_AF_DEVICE_MANAGEMENT_CHANGE_OF_SUPPLIER_PENDING_MASK)
    {
        if (currentTime >= pmInfo.supplier.implementationDateTime)
        {
            emberAfDeviceManagementClusterPrintln("DEVICE MANAGEMENT CLIENT: Enacting change of supplier at time 0x%4x\n",
                                                  currentTime);
            emberAfPluginDeviceManagementClientEnactChangeOfSupplierCallback(endpoint, &(pmInfo.supplier));
    ...
    if (pmInfo.pendingUpdates & EMBER_AF_DEVICE_MANAGEMENT_UPDATE_SITE_ID_PENDING_MASK)
    {
        ...
        {
            EmberAfStatus status =
                emberAfWriteAttribute(endpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_SITE_ID_ATTRIBUTE_ID, CLUSTER_MASK_SERVER,
                                      (uint8_t *) &(pmInfo.siteId.siteId), ZCL_OCTET_STRING_ATTRIBUTE_TYPE);
            if (status == EMBER_ZCL_STATUS_SUCCESS)
            {
    ...
    if (pmInfo.pendingUpdates & EMBER_AF_DEVICE_MANAGEMENT_UPDATE_CIN_PENDING_MASK)
    {
        ...
        {
            EmberAfStatus status =
                emberAfWriteAttribute(endpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_CUSTOMER_ID_NUMBER_ATTRIBUTE_ID,
                                      CLUSTER_MASK_SERVER, (uint8_t *) &(pmInfo.cin.cin), ZCL_OCTET_STRING_ATTRIBUTE_TYPE);
            if (status == EMBER_ZCL_STATUS_SUCCESS)
            {
                pmInfo.pendingUpdates &= ~(EMBER_AF_DEVICE_MANAGEMENT_UPDATE_CIN_PENDING_MASK);
            }
            emberAfDeviceManagementClusterPrintln(
                "DEVICE MANAGEMENT CLIENT: Enacting customer id number update at time 0x%4x: %d\n", currentTime, status);
        }
    ...
    {
        emberAfDeviceManagementClusterPrintln(
            "DEVICE MANAGEMENT CLIENT: Service password current time 0x%4x: implementationTime 0x%4x\n", currentTime,
    ...
    if (pmInfo.pendingUpdates & EMBER_AF_DEVICE_MANAGEMENT_UPDATE_CONSUMER_PASSWORD_PENDING_MASK)
    {
        if (currentTime >= consumerPassword.implementationDateTime)
        {
            ...
            MEMCOPY(&pmInfo.consumerPassword.password, &consumerPassword.password,
                    emberAfStringLength(consumerPassword.password) + 1);
    ...
    {
        emberAfScheduleClientTick(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, nextTick * MILLISECOND_TICKS_PER_SECOND);
    }
}

bool emberAfDeviceManagementClusterPublishChangeOfTenancyCallback(uint32_t supplierId, uint32_t issuerEventId, uint8_t tariffType,
                                                                  ...
                                                                  uint32_t proposedTenancyChangeControl)
{
    ...
    emberAfDeviceManagementClusterPrintln("RX: PublishChangeOfTenancy: 0x%4X, 0x%4X, 0x%X, 0x%4X, 0x%4X", supplierId, issuerEventId,
                                          tariffType, implementationDateTime, proposedTenancyChangeControl);
    ...
    status = emberAfWriteAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, ZCL_CHANGE_OF_TENANCY_UPDATE_DATE_TIME_ATTRIBUTE_ID,
                                   CLUSTER_MASK_CLIENT, (uint8_t *) &implementationDateTime, ZCL_UTC_TIME_ATTRIBUTE_TYPE);
    ...
    status = emberAfWriteAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, ZCL_PROPOSED_TENANCY_CHANGE_CONTROL_ATTRIBUTE_ID,
                                   CLUSTER_MASK_CLIENT, (uint8_t *) &proposedTenancyChangeControl, ZCL_INT32U_ATTRIBUTE_TYPE);
    ...
    else
    {
        ...
        emberAfScheduleClientTick(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID,
                                  (implementationDateTime - currentTime) * MILLISECOND_TICKS_PER_SECOND);
        pmInfo.pendingUpdates |= EMBER_AF_DEVICE_MANAGEMENT_CHANGE_OF_TENANCY_PENDING_MASK;
    }
                                                                   ...
                                                                   uint8_t * proposedProviderContactDetails)
{
    ...
    emberAfDeviceManagementClusterPrintln("RX: PublishChangeOfSupplier: 0x%4X, 0x%4X, 0x%X, 0x%4X, 0x%4X, 0x%4X, ",
                                          currentProviderId, issuerEventId, tariffType, proposedProviderId,
                                          providerChangeImplementationTime, providerChangeControl);
    ...
    emberAfCopyString(pmInfo.supplier.proposedProviderName, proposedProviderName,
                      EMBER_AF_DEVICE_MANAGEMENT_MAXIMUM_PROPOSED_PROVIDER_NAME_LENGTH);
    emberAfCopyString(pmInfo.supplier.proposedProviderContactDetails, proposedProviderContactDetails,
    ...
    else
    {
        // Otherwise, wait until the time of implementation
        emberAfScheduleClientTick(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID,
                                  (providerChangeImplementationTime - currentTime) * MILLISECOND_TICKS_PER_SECOND);
        pmInfo.pendingUpdates |= EMBER_AF_DEVICE_MANAGEMENT_CHANGE_OF_SUPPLIER_PENDING_MASK;
    }
                                                                      ...
                                                                      uint8_t * password)
{
    ...
    emberAfCopyString(pass->password, password, EMBER_AF_DEVICE_MANAGEMENT_MAXIMUM_PASSWORD_LENGTH);
...
bool emberAfDeviceManagementClusterUpdateSiteIdCallback(uint32_t issuerEventId, uint32_t implementationDateTime,
                                                        uint32_t providerId, uint8_t * siteId)
{
    uint32_t currentTime = emberAfGetCurrentTime();
    uint8_t endpoint     = emberAfCurrentEndpoint();
    ...
    emberAfCopyString(pmInfo.siteId.siteId, siteId, EMBER_AF_DEVICE_MANAGEMENT_MAXIMUM_SITE_ID_LENGTH);
    ...
    {
        if ((pmInfo.pendingUpdates & EMBER_AF_DEVICE_MANAGEMENT_UPDATE_SITE_ID_PENDING_MASK) && (pmInfo.providerId == providerId) &&
    ...
    { // schedule siteid update or immediate update
        if (issuerEventId > pmInfo.issuerEventId)
        {
            pmInfo.issuerEventId = issuerEventId;
        }
        ...
        if (implementationDateTime == 0x00000000)
        {
            ...
            status = emberAfWriteAttribute(endpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_SITE_ID_ATTRIBUTE_ID, CLUSTER_MASK_SERVER,
                                           (uint8_t *) siteId, ZCL_OCTET_STRING_ATTRIBUTE_TYPE);
...
bool emberAfDeviceManagementClusterSetSupplyStatusCallback(uint32_t issuerEventId, uint8_t supplyTamperState,
                                                           ...
                                                           uint8_t loadLimitSupplyState)
{
    ...
    emberAfPluginDeviceManagementClientSetSupplyStatusCallback(endpoint, flags);

    emberAfSendImmediateDefaultResponse(EMBER_ZCL_STATUS_SUCCESS);
    return true;
}

bool emberAfDeviceManagementClusterPublishUncontrolledFlowThresholdCallback(uint32_t providerId, uint32_t issuerEventId,
                                                                            uint16_t uncontrolledFlowThreshold,
                                                                            uint8_t unitOfMeasure, uint16_t multiplier,
                                                                            uint16_t divisor, uint8_t stabilisationPeriod,
                                                                            uint16_t measurementPeriod)
{
    ...
    emberAfPluginDeviceManagementClientEnactUpdateUncontrolledFlowThresholdCallback(endpoint, threshold);

    emberAfSendImmediateDefaultResponse(EMBER_ZCL_STATUS_SUCCESS);
    return true;
}

bool emberAfDeviceManagementClusterUpdateCINCallback(uint32_t issuerEventId, uint32_t implementationDateTime, uint32_t providerId,
                                                     uint8_t * customerIdNumber)
{
    ...
    if (implementationDateTime == 0xFFFFFFFF)
    {
        if ((pmInfo.pendingUpdates & EMBER_AF_DEVICE_MANAGEMENT_UPDATE_CIN_PENDING_MASK) && (pmInfo.providerId == providerId) &&
    ...
    { // schedule CIN update or immediate update
        if (issuerEventId > pmInfo.issuerEventId)
        {
            pmInfo.issuerEventId = issuerEventId;
        }
        ...
        if (implementationDateTime == 0x00000000)
        {
            ...
            status = emberAfWriteAttribute(endpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_CUSTOMER_ID_NUMBER_ATTRIBUTE_ID,
                                           CLUSTER_MASK_SERVER, (uint8_t *) customerIdNumber, ZCL_OCTET_STRING_ATTRIBUTE_TYPE);
            if (status != EMBER_ZCL_STATUS_SUCCESS)
            {
...
bool emberAfDeviceManagementClusterGetEventConfigurationCallback(uint16_t eventId)
{
    ...
    if ((eventId & 0xFF) == 0xFF)
    {
        ...
        {
            sendDeviceManagementClusterReportWildCardAttribute(attributeSet, endpoint);
            return true;
        }
    }
    else
    {
        ...
        status = emberAfReadClientAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, eventId, (uint8_t *) &eventConfiguration,
        ...
        if (status == EMBER_ZCL_STATUS_SUCCESS)
        {
            emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER),
                                      ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, ZCL_REPORT_EVENT_CONFIGURATION_COMMAND_ID, "uu", 0, 1);
                                                                 ...
                                                                 uint8_t * eventConfigurationPayload)
{
    ...
    case EMBER_ZCL_EVENT_CONFIGURATION_CONTROL_APPLY_BY_EVENT_GROUP: {
        ...
        writeDeviceManagementClusterWildCardAttribute(attributeSet, emberAfCurrentEndpoint(), eventConfiguration);
        break;
    }
    case EMBER_ZCL_EVENT_CONFIGURATION_CONTROL_APPLY_BY_LOG_TYPE: {
        uint8_t logType = eventConfigurationPayload[0];
        writeDeviceManagementClusterByLogTypeAttribute(logType, emberAfCurrentEndpoint(), eventConfiguration);
        break;
    }
    case EMBER_ZCL_EVENT_CONFIGURATION_CONTROL_APPLY_BY_CONFIGURATION_MATCH: {
        ...
        writeDeviceManagementClusterByMatchingAttribute(currentConfiguration, emberAfCurrentEndpoint(), eventConfiguration);
        break;
    }
    default:
