...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Routines for the Power Consumption Model
                                                                               *plugin, which implements the Light Bulb Electrical
...
// 160 Watt Maximum
#define DEFAULT_POWER_RATING_MIILI_WATT (EMBER_AF_PLUGIN_POWER_CONSUMPTION_MODEL_DEFAULT_POWER_RATING * WATT_TO_MILLI_WATT)
#define SAMPLE_PERIOD_MILLI_SECOND                                                                                                 \
    ((EMBER_AF_PLUGIN_POWER_CONSUMPTION_MODEL_POLLING_TIME_INTERVAL_MS * MILLISECOND_TICKS_PER_SECOND) / MILLI_SEC_TO_SECONDS)
#define OFF_POWER_CONSUMPTION_MILLI_WATT EMBER_AF_PLUGIN_POWER_CONSUMPTION_MODEL_OFF_POWER_CONSUMPTION_MILLI_WATT
#define ON_POWER_CONSUMPTION_MILLI_WATT (EMBER_AF_PLUGIN_POWER_CONSUMPTION_MODEL_ON_POWER_CONSUMPTION * WATT_TO_MILLI_WATT)
...
static uint32_t instantaneousPowerToMilliWattPerPeriod(uint32_t instantPowerMilliWatt, uint16_t pollingTimeIntervalMs);
static uint32_t energyConsumptionWattHour(uint32_t energyConsumptionPerPeriod);
static void updateEnergyConsumptionDataAttribute(uint32_t energyConsumedWhr, uint32_t instantPowerMilliWatt);
...
static uint8_t readLevelControl(uint8_t deviceEndpoint)
{
    ...
    {
        emberAfReadServerAttribute(deviceEndpoint, ZCL_ON_OFF_CLUSTER_ID, ZCL_ON_OFF_ATTRIBUTE_ID, (uint8_t *) &lighbulbOn,
                                   sizeof(bool));
        if (lighbulbOn)
        {
            uint8_t levelControl;
            emberAfReadServerAttribute(deviceEndpoint, ZCL_LEVEL_CONTROL_CLUSTER_ID, ZCL_CURRENT_LEVEL_ATTRIBUTE_ID,
                                       (uint8_t *) &levelControl, sizeof(levelControl));
            return levelControl;
        }
    }
    return 0;
...
static uint32_t computeInstantaneousPowerMilliWatt(uint8_t levelControl)
{
    ...
    {
        scaler = ((DEFAULT_POWER_RATING_MIILI_WATT - ON_POWER_CONSUMPTION_MILLI_WATT) * POWER_SCALE_FACTOR) /
            (LEVEL_CONTROL_AT_MAX - LEVEL_CONTROL_AT_ON);
        shifter =
            (((LEVEL_CONTROL_AT_MAX * ON_POWER_CONSUMPTION_MILLI_WATT) - LEVEL_CONTROL_AT_ON * DEFAULT_POWER_RATING_MIILI_WATT) *
             POWER_SCALE_FACTOR) /
            (LEVEL_CONTROL_AT_MAX - LEVEL_CONTROL_AT_ON);
    }
    else
    {
        scaler =
            ((ON_POWER_CONSUMPTION_MILLI_WATT - OFF_POWER_CONSUMPTION_MILLI_WATT) * POWER_SCALE_FACTOR) / (LEVEL_CONTROL_AT_ON - 1);
...
static uint32_t instantaneousPowerToMilliWattPerPeriod(uint32_t instantPowerMilliWatt, uint16_t pollingTimeIntervalMs)
{
    ...
    milliWattSecond = (instantPowerMilliWatt * pollingTimeIntervalMs) / MILLISECOND_TICKS_PER_SECOND;
...
static uint32_t energyConsumptionWattHour(uint32_t energyConsumptionPerPeriod)
{
    ...
    energyConsumptionWh                   = totalEnergyConsumptionMilliWattSecond / (WATT_TO_MILLI_WATT * SECONDS_PER_HOUR);
    totalEnergyConsumptionMilliWattSecond = totalEnergyConsumptionMilliWattSecond % (WATT_TO_MILLI_WATT * SECONDS_PER_HOUR);
...
{
    uint8_t i;
    uint8_t previousCumulativeEnergy[CUMULATIVE_ENERGY_CONSUMPTION_ATTRIBUTE_IN_BYTE];
    ...
    if (emberAfContainsServer(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID))
    {
        emberAfReadServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_CURRENT_SUMMATION_DELIVERED_ATTRIBUTE_ID,
                                   (uint8_t *) (&previousCumulativeEnergy), CUMULATIVE_ENERGY_CONSUMPTION_ATTRIBUTE_IN_BYTE);
        ...
        emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_CURRENT_SUMMATION_DELIVERED_ATTRIBUTE_ID,
                                    (uint8_t *) (&totalEnergyConsumption), ZCL_INT48U_ATTRIBUTE_TYPE);

        emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_INSTANTANEOUS_DEMAND_ATTRIBUTE_ID,
                                    (uint8_t *) (&instantaneousDemandPowerMilliWatt), ZCL_INT24S_ATTRIBUTE_TYPE);
    }
}
...
static void refreshingEnergyConsumptionData(void)
{
    ...
    instantaneousPowerMilliWatt = computeInstantaneousPowerMilliWatt(levelControl);
    energyConsumptionMilliWattSecond =
        instantaneousPowerToMilliWattPerPeriod(instantaneousPowerMilliWatt, SAMPLE_PERIOD_MILLI_SECOND);
    energyConsumptionWh = energyConsumptionWattHour(energyConsumptionMilliWattSecond);
    updateEnergyConsumptionDataAttribute(energyConsumptionWh, instantaneousPowerMilliWatt);
}
...
void emberAfPowerConsumptionModelPrintAttributes(void)
{
    ...
    if (emberAfContainsServer(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID) == false)
    {
        emberAfAppPrintln("Error:  need to include the simple metering cluster");
        return;
    }
    assert(EMBER_ZCL_STATUS_SUCCESS ==
           emberAfReadServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID,
                                      ZCL_CURRENT_SUMMATION_DELIVERED_ATTRIBUTE_ID, (uint8_t *) (&cumulativeEnergy),
    ...
    assert(EMBER_ZCL_STATUS_SUCCESS ==
           emberAfReadServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_INSTANTANEOUS_DEMAND_ATTRIBUTE_ID,
                                      (uint8_t *) (&instantPowerMilliW), INSTANTANEOUS_POWER_DEMAND_ATTRIBUTE_IN_BYTE));
    for (i = 0; i < INSTANTANEOUS_POWER_DEMAND_ATTRIBUTE_IN_BYTE; i++)
    {
        instantDemandPower += (uint32_t)(instantPowerMilliW[i] << (i * 8));
    }
    emberAfAppPrintln("Instant Demand Power: %d.%d W", (instantDemandPower * SIMPLE_METERING_MULTIPLIER) / SIMPLE_METERING_DIVISOR,
                      (instantDemandPower * SIMPLE_METERING_MULTIPLIER) % SIMPLE_METERING_DIVISOR);
}
...
void emberAfPluginPowerConsumptionModelInitCallback(void)
{
    ...
    emberEventControlSetActive(emberAfPluginPowerConsumptionModelReadEventControl);
}

// After event triggered, this function is called
void emberAfPluginPowerConsumptionModelReadEventHandler(void)
{
    emberEventControlSetDelayMS(emberAfPluginPowerConsumptionModelReadEventControl, SAMPLE_PERIOD_MILLI_SECOND);
...
static void simpleMeteringClusterSetup(void)
{
    uint8_t cumulativeEnergyFormat = CUMULATIVE_ENERGY_FORMAT, instantaneousPowerFormat = INSTANTANEOUS_POWER_FORMAT,
            electricalMeasurementUnit    = ELECTRICAL_UNIT_MEASUREMENT_KWH_AND_KW,
            electricalMeteringDeviceType = ELECTRICAL_METERING_DEVICE_TYPE, meteringDeviceStatus = METERING_DEVICE_GOOD_STATUS,
            historicalConsumptionFormatting = HISTORICAL_CONSUMPTION_FORMATTING;

    uint32_t attributeDivisor = SIMPLE_METERING_DIVISOR, attributeMultiplier = SIMPLE_METERING_MULTIPLIER;

    if (emberAfContainsServer(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID))
    {
        // Setting up formatting for cumulative energy (kWh) and instantaneous power (Wh)
        assert(EMBER_ZCL_STATUS_SUCCESS ==
               emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_SUMMATION_FORMATTING_ATTRIBUTE_ID,
                                           (uint8_t *) (&cumulativeEnergyFormat), ZCL_BITMAP8_ATTRIBUTE_TYPE));
        assert(EMBER_ZCL_STATUS_SUCCESS ==
               emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_DEMAND_FORMATTING_ATTRIBUTE_ID,
                                           (uint8_t *) (&instantaneousPowerFormat), ZCL_BITMAP8_ATTRIBUTE_TYPE));
        // Setting up device status (no errors)
        assert(EMBER_ZCL_STATUS_SUCCESS ==
               emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_STATUS_ATTRIBUTE_ID,
                                           (uint8_t *) (&meteringDeviceStatus), ZCL_BITMAP8_ATTRIBUTE_TYPE));

        // Setting up meter type (electrical metering)
        assert(EMBER_ZCL_STATUS_SUCCESS ==
               emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_METERING_DEVICE_TYPE_ATTRIBUTE_ID,
                                           (uint8_t *) (&electricalMeteringDeviceType), ZCL_BITMAP8_ATTRIBUTE_TYPE));

        // setting up unit of measurement (kWh for cumulative and W for instant)
        assert(EMBER_ZCL_STATUS_SUCCESS ==
               emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_UNIT_OF_MEASURE_ATTRIBUTE_ID,
                                           (uint8_t *) (&electricalMeasurementUnit), ZCL_ENUM8_ATTRIBUTE_TYPE));

        // setting up historical consumption formatting
        assert(EMBER_ZCL_STATUS_SUCCESS ==
               emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID,
                                           ZCL_HISTORICAL_CONSUMPTION_FORMATTING_ATTRIBUTE_ID,
                                           (uint8_t *) (&historicalConsumptionFormatting), ZCL_BITMAP8_ATTRIBUTE_TYPE));

        // setting up muliplier and divisor attribute
        assert(EMBER_ZCL_STATUS_SUCCESS ==
               emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_DIVISOR_ATTRIBUTE_ID,
                                           (uint8_t *) (&attributeDivisor), ZCL_INT24U_ATTRIBUTE_TYPE));
        assert(EMBER_ZCL_STATUS_SUCCESS ==
               emberAfWriteServerAttribute(lightbulbEndpoint, ZCL_SIMPLE_METERING_CLUSTER_ID, ZCL_MULTIPLIER_ATTRIBUTE_ID,
                                           (uint8_t *) (&attributeMultiplier), ZCL_INT24U_ATTRIBUTE_TYPE));
    }
}
