...
void emAfDelayedJoinActivateCommand(void)
{
    ...
    EzspStatus status = emberAfSetEzspPolicy(
        EZSP_TRUST_CENTER_POLICY, (EZSP_DECISION_ALLOW_JOINS | EZSP_DECISION_ALLOW_UNSECURED_REJOINS | EZSP_DECISION_DEFER_JOINS),
        "Trust Center Policy", "Delayed joins");

    if (EZSP_SUCCESS != status)
    {
        emberAfCorePrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_DELAYED_JOIN_PLUGIN_NAME, "failed to configure delayed joining", status);
    }
#endif // EZSP_HOST
}
...
void emAfDelayedJoinSendNetworkKeyCommand(void)
{
    ...
    if (EMBER_SUCCESS != emberUnicastCurrentNetworkKey(targetShort, targetLong, parentShort))
    {
        emberAfCorePrintln("%p: %p 0x%X", EMBER_AF_PLUGIN_DELAYED_JOIN_PLUGIN_NAME, "failed to send the encrypted network key to ",
                           targetShort);
    }
}
