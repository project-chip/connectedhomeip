...
{
    emberAfCorePrintln("%p: 0x%2X %p %X %p: 0x%2X", EMBER_AF_PLUGIN_DELAYED_JOIN_PLUGIN_NAME, newNodeId, " is attempting to do ",
                       status, " with parentId", parentOfNewNode);
    emberAfPluginDelayedJoinCallback(newNodeId, newNodeEui64, parentOfNewNode, status, decision);
}
    ...
    if (status == EMBER_NETWORK_UP)
    {
        EzspStatus status =
            emberAfSetEzspPolicy(EZSP_TRUST_CENTER_POLICY,
                                 (EZSP_DECISION_ALLOW_JOINS | EZSP_DECISION_ALLOW_UNSECURED_REJOINS | EZSP_DECISION_DEFER_JOINS),
                                 "Trust Center Policy", "Delay joins");

        if (EZSP_SUCCESS != status)
        {
            emberAfCorePrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_DELAYED_JOIN_PLUGIN_NAME, "failed to configure delayed joining",
                               status);
        }
    }
#endif // EZSP_HOST
...
void emberAfPluginDelayedJoinSetNetworkKeyTimeout(uint8_t timeout)
{
    ...
    {
        emberAfCorePrintln("%p: %p", EMBER_AF_PLUGIN_DELAYED_JOIN_PLUGIN_NAME,
