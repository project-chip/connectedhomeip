...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Commands for executing manufacturing related
                                                                               *tests
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
bool emberAfMfglibEnabled(void)
{
    ...
    emberSerialPrintf(APP_SERIAL, "MFG_LIB Enabled %x\r\n", enabled);
...
void emberAfPluginManufacturingLibraryCliCheckSendCompleteEventHandler(void)
{
    ...
    if (savedPacketCount == mfgTotalPacketCounter)
    {
        ...
        emberAfCorePrintln("%p Send Complete %d packets", PLUGIN_NAME, mfgCurrentPacketCounter);
        emberAfCorePrintln("First packet: lqi %d, rssi %d, len %d", savedLinkQuality, savedRssi, savedPktLength);
        mfgCurrentPacketCounter = 0;
    }
    else
    {
        savedPacketCount = mfgTotalPacketCounter;
        emberEventControlSetDelayQS(checkSendCompleteEventControl, CHECK_SEND_COMPLETE_DELAY_QS);
    }
}
...
void ezspMfglibRxHandler(uint8_t linkQuality, int8_t rssi, uint8_t packetLength, uint8_t * packetContents)
{
...
void emberAfMfglibStart(bool wantCallback)
{
    ...
    emberAfCorePrintln("%p start, status 0x%X", PLUGIN_NAME, status);
...
void emberAfMfglibStop(void)
{
    ...
    emberAfCorePrintln("%p end, status 0x%X", PLUGIN_NAME, status);
