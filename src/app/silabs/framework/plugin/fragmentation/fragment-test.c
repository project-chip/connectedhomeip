...
EmberEventData fragmentationEvents[] = { EMBER_AF_FRAGMENTATION_EVENTS{ NULL, NULL } };
...
Parcel * parcelFragment(Parcel * wholeMessage, uint8_t startIndex, uint8_t length);
Parcel * makeIncrementingParcel(uint8_t length);
void callFragmentInit(void);
void callFragmentTick(void);
EmberStatus callSendFragmentedMessage(EmberOutgoingMessageType type, uint16_t indexOrDestination, EmberApsFrame * apsFrame,
                                      Parcel * payload, uint8_t maxFragmentSize, bool sourceRoute);
bool callIsOutgoingFragment(EmberApsFrame * apsFrame, EmberMessageBuffer * messageLoc, EmberStatus status);
bool callIsIncomingFragment(EmberApsFrame * apsFrame, EmberNodeId sender, Parcel * payloadBefore, Parcel * payloadAfter);
...
#define initWindowSize(windowSize)                                                                                                 \
    do                                                                                                                             \
    {                                                                                                                              \
        addSimpleAction("FragmentInit", callFragmentInit, 0);                                                                      \
        addSimpleAction("SetWindowSize", callSetWindowSize, 1, windowSize);                                                        \
    } while (0)

#define tick()                                                                                                                     \
    do                                                                                                                             \
    {                                                                                                                              \
        addSimpleAction("FragmentTick", callFragmentTick, 0);                                                                      \
    } while (0)

#define sendFragmented(wholeMessage, fragmentSize, sourceRoute)                                                                    \
    do                                                                                                                             \
    {                                                                                                                              \
        addFunctionCheck("SendFragmentedMessage", EMBER_SUCCESS, callSendFragmentedMessage, 6, EMBER_OUTGOING_DIRECT,              \
                         remoteNodeId, &apsFrame, wholeMessage, fragmentSize, sourceRoute);                                        \
    } while (0)

#define checkSend(messageFragment, totalTxBlocks, blockNumber, data, sourceRoute)                                                  \
    do                                                                                                                             \
    {                                                                                                                              \
        ...
        addSimpleCheck("emberSendUnicast", "iipp!", EMBER_OUTGOING_DIRECT, remoteNodeId, apsStructParcel(newFrame),                \
...
#define outgoingFragment(data, status)                                                                                             \
    ...
    {                                                                                                                              \
        ...
        memcpy(newFrame, &apsFrame, sizeof(EmberApsFrame));                                                                        \
        newFrame->options = EMBER_APS_OPTION_RETRY | EMBER_APS_OPTION_FRAGMENT;                                                    \
        addFunctionCheck("IsOutgoingFragment", true, callIsOutgoingFragment, 3, newFrame, data, status);                           \
    } while (0)

#define fragmentedMessageSent(status)                                                                                              \
    do                                                                                                                             \
    {                                                                                                                              \
        addSimpleCheck("emAfFragmentationMessageSentHandler", "i", status);                                                        \
    } while (0)

#define incomingFragment(payloadBefore, totalTxBlocks, blockNumber, payloadAfter, newMessage)                                      \
    do                                                                                                                             \
    {                                                                                                                              \
        EmberApsFrame * newFrame = (EmberApsFrame *) malloc(sizeof(EmberApsFrame));                                                \
        memcpy(newFrame, &apsFrame, sizeof(EmberApsFrame));                                                                        \
        newFrame->options = EMBER_APS_OPTION_RETRY | EMBER_APS_OPTION_FRAGMENT;                                                    \
        newFrame->groupId = HIGH_LOW_TO_INT(totalTxBlocks, blockNumber);                                                           \
        addFunctionCheck("IsIncomingFragment", payloadAfter == NULL, callIsIncomingFragment, 4, newFrame, remoteNodeId,            \
...
#define checkSendReply(blockMask, rxWindowBase)                                                                                    \
    ...
    {                                                                                                                              \
        ...
        addSimpleCheck("emberSetReplyFragmentData", "i", newFrame->groupId);                                                       \
        addSimpleCheck("emberSendReply", "ip", newFrame->clusterId, makeMessage(""));                                              \
    } while (0)

#define expectFragInProgress(inProgress)                                                                                           \
    do                                                                                                                             \
    {                                                                                                                              \
        addFunctionCheck("FragInProgress", inProgress, callIsFragInProgress, 0);                                                   \
    } while (0)
...
static void simpleSend(void)
{
    ...
    checkSend(parcelFragment(wholeMessage, 0, 20), 2, 0, fragmentData, false);
    checkSend(parcelFragment(wholeMessage, 20, 20), 2, 1, fragmentData + 1, false);
...
static void sendWindow(void)
{
    ...
    checkSend(parcelFragment(wholeMessage, 0, 10), 3, 0, fragmentData, false);
    addRunAction(10);
    outgoingFragment(fragmentData, EMBER_SUCCESS);
    ...
    checkSend(parcelFragment(wholeMessage, 10, 10), 3, 1, fragmentData + 1, false);
    addRunAction(10);
    outgoingFragment(fragmentData + 1, EMBER_SUCCESS);
    checkSend(parcelFragment(wholeMessage, 20, 10), 3, 2, fragmentData, false);
    addRunAction(10);
    outgoingFragment(fragmentData, EMBER_SUCCESS);
    fragmentedMessageSent(EMBER_SUCCESS);
    addRunAction(100);
...
static void ackMask1(void)
{
    ...
    initWindowSize(3);
    expectFragInProgress(false);
    incomingFragment(parcelFragment(wholeMessage, 0, 20), 2, 0, NULL, true);
    expectFragInProgress(true);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 20, 20), 2, 1, wholeMessage, false);
...
static void ackMask2(void)
{
    ...
    initWindowSize(4);
    expectFragInProgress(false);
    incomingFragment(parcelFragment(wholeMessage, 0, 10), 5, 0, NULL, true);
    expectFragInProgress(true);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 10, 10), 5, 1, NULL, false);
    expectFragInProgress(true);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 20, 10), 5, 2, NULL, false);
    expectFragInProgress(true);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 30, 10), 5, 3, NULL, false);
    checkSendReply(0xFF, 0);
    expectFragInProgress(true);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 40, 5), 5, 4, wholeMessage, false);
...
static void ackRetries(void)
{
    ...
    incomingFragment(parcelFragment(wholeMessage, 0, 40), 3, 0, NULL, true);
    checkSendReply(0xFF, 0);
    expectFragInProgress(true);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 0, 40), 3, 0, NULL, false);
    checkSendReply(0xFF, 0);
    expectFragInProgress(true);
    addRunAction(10);

    incomingFragment(parcelFragment(wholeMessage, 40, 40), 3, 1, NULL, false);
    ...
    incomingFragment(parcelFragment(wholeMessage, 80, 20), 3, 2, wholeMessage, false);
    checkSendReply(0xFF, 2);
    expectFragInProgress(false);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 80, 20), 3, 2, NULL, false);
...
static void ackMissingFragment(void)
{
    ...
    incomingFragment(parcelFragment(wholeMessage, 0, 10), 4, 0, NULL, true);
    expectFragInProgress(true);
    addRunAction(10);
    ...
    //                 4, 1, NULL, false);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 20, 10), 4, 2, NULL, false);
    ...
    incomingFragment(parcelFragment(wholeMessage, 0, 10), 4, 0, NULL, false);
    expectFragInProgress(true);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 10, 10), 4, 1, NULL, false);
    checkSendReply(0xFF, 0);
    expectFragInProgress(true);
    addRunAction(10);
    incomingFragment(parcelFragment(wholeMessage, 20, 10), 4, 2, NULL, false);
    ...
    incomingFragment(parcelFragment(wholeMessage, 30, 10), 4, 3, wholeMessage, false);
...
static void timeout(void)
{
    ...
    incomingFragment(parcelFragment(wholeMessage, 0, 10), 2, 0, NULL, true);
    expectFragInProgress(true);
    addRunAction(emberApsAckTimeoutMs * 2);
    tick();
    expectFragInProgress(true);
    incomingFragment(parcelFragment(wholeMessage, 10, 10), 2, 1, wholeMessage, false);
    checkSendReply(0xFF, 0);
    expectFragInProgress(false);
    ...
    incomingFragment(parcelFragment(wholeMessage, 10, 10), 2, 1, NULL, true);
    expectFragInProgress(true);
...
static void sourceRoute(void)
{
    ...
    checkSend(parcelFragment(wholeMessage, 0, 20), 2, 0, fragmentData, true);
    checkSend(parcelFragment(wholeMessage, 20, 20), 2, 1, fragmentData + 1, true);
    expectFragInProgress(true);
    addRunAction(10);
    outgoingFragment(fragmentData, EMBER_SUCCESS);
    expectFragInProgress(true);
...
static void txRxOverlap(void)
{
    ...
    incomingFragment(parcelFragment(rxMessage, 0, 20), 2, 0, NULL, true);
    expectFragInProgress(true);
    addRunAction(10);

    // Tx fragments 1 and 2 - Tx fragmentation begins
    sendFragmented(txMessage, 20, false);
    checkSend(parcelFragment(txMessage, 0, 20), 2, 0, fragmentData, false);
    checkSend(parcelFragment(txMessage, 20, 20), 2, 1, fragmentData + 1, false);
    expectFragInProgress(true);
    addRunAction(10);

    // Rx fragment 2 - Rx fragmentation completes
    incomingFragment(parcelFragment(rxMessage, 20, 20), 2, 1, rxMessage, false);
    checkSendReply(0xFF, 0);
    expectFragInProgress(true);
    addRunAction(10);
...
{
    return makeMessage("<2<2<211<21", apsStruct->options, apsStruct->profileId, apsStruct->clusterId, apsStruct->sourceEndpoint,
                       apsStruct->destinationEndpoint, apsStruct->groupId, apsStruct->sequence);
}
                                      ...
                                      Parcel * payload, uint8_t maxFragmentSize, bool sourceRoute)
{
    ...
    status = emAfFragmentationSendUnicast(type, indexOrDestination, apsFrame, payload->contents, payload->length, &messageTag);
...
bool callIsIncomingFragment(EmberApsFrame * apsFrame, EmberNodeId sender, Parcel * payloadBefore, Parcel * payloadAfter)
{
    ...
    result = emAfFragmentationIncomingMessage(EMBER_INCOMING_UNICAST, apsFrame, sender, &payloadBuffer, &payloadLength);
    ...
    if (payloadAfter != NULL && !sameParcel(actualPayloadAfter, payloadAfter))
    {
        fprintf(stderr, "\n[wrong payload\n have [");
...
long callIsFragInProgress(void)
{
    return ((READBITS(currentAppTasks, EMBER_AF_FRAGMENTATION_IN_PROGRESS) == EMBER_AF_FRAGMENTATION_IN_PROGRESS) ? (long) true
                                                                                                                  : (long) false);
}
...
EmberStatus emberSendUnicast(EmberOutgoingMessageType type, uint16_t indexOrDestination, EmberApsFrame * apsFrame,
                             EmberMessageBuffer message)
{
    functionCallCheck("emberSendUnicast", "iipp", type, indexOrDestination, apsStructParcel(apsFrame), bufferToParcel(message));
