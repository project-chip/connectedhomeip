...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Splits long messages into smaller fragments
                                                                               *for transmission and reassembles received fragments
                                                                               *into full messages.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static uint8_t txMessageBuffers[EMBER_AF_PLUGIN_FRAGMENTATION_MAX_OUTGOING_PACKETS][EMBER_AF_PLUGIN_FRAGMENTATION_BUFFER_SIZE];
...
EmberStatus emAfFragmentationSendUnicast(EmberOutgoingMessageType type, uint16_t indexOrDestination, EmberApsFrame * apsFrame,
                                         uint8_t * buffer, uint16_t bufLen, uint8_t * messageTag)
{
    EmberStatus status;
    uint16_t fragments;
    ...
    txPacket->apsFrame.options |= (EMBER_APS_OPTION_FRAGMENT | EMBER_APS_OPTION_RETRY);

    emAfPluginFragmentationHandleSourceRoute(txPacket, indexOrDestination);

    MEMMOVE(txPacket->bufferPtr, buffer, bufLen);
    txPacket->bufLen      = bufLen;
    txPacket->fragmentLen = emberAfMaximumApsPayloadLength(type, indexOrDestination, &txPacket->apsFrame);
...
bool emAfFragmentationMessageSent(EmberApsFrame * apsFrame, EmberStatus status)
{
...
static EmberStatus sendNextFragments(txFragmentedPacket * txPacket)
{
    ...
    emberAfCorePrintln("Sending fragment %d of %d", txPacket->fragmentBase, txPacket->fragmentCount);

    offset = txPacket->fragmentBase * txPacket->fragmentLen;

    // Send fragments until the window is full.
    for (i = txPacket->fragmentBase; i < txPacket->fragmentBase + emberFragmentWindowSize && i < txPacket->fragmentCount; i++)
    {
        EmberStatus status;
        ...
        uint8_t fragmentLen =
            (offset + txPacket->fragmentLen < txPacket->bufLen ? txPacket->fragmentLen : txPacket->bufLen - offset);

        txPacket->apsFrame.groupId = HIGH_LOW_TO_INT(txPacket->fragmentCount, i);

        status = emAfPluginFragmentationSend(txPacket, i, fragmentLen, offset);
        if (status != EMBER_SUCCESS)
        {
            return status;
        }
    ...
    if (txPacket->fragmentsInTransit == 0)
    {
        emAfFragmentationMessageSentHandler(txPacket->messageType, txPacket->indexOrDestination, &txPacket->apsFrame,
                                            txPacket->bufferPtr, txPacket->bufLen, EMBER_SUCCESS, messageTag(txPacket));
        txPacket->messageType = UNUSED_TX_PACKET_ENTRY;
    }
    else if (freeOutgoingMessagePriorToFinalAck && txPacket->bufferPtr != NULL && offset >= txPacket->bufLen &&
             emberFragmentWindowSize == 1 && txPacketAwaitingFinalAck.messageType == UNUSED_TX_PACKET_ENTRY)
    {
...
static void abortTransmission(txFragmentedPacket * txPacket, EmberStatus status)
{
    ...
    {
        emAfFragmentationMessageSentHandler(txPacket->messageType, txPacket->indexOrDestination, &txPacket->apsFrame,
                                            txPacket->bufferPtr, txPacket->bufLen, status, messageTag(txPacket));
        txPacket->messageType = UNUSED_TX_PACKET_ENTRY;
    }
}
...
static bool storeRxFragment(rxFragmentedPacket * rxPacket, uint8_t fragment, uint8_t * buffer, uint16_t bufLen);
static void moveRxWindow(rxFragmentedPacket * rxPacket);
static rxFragmentedPacket * getFreeRxPacketEntry(void);
static rxFragmentedPacket * rxPacketLookUp(EmberApsFrame * apsFrame, EmberNodeId sender);
...
static uint16_t retryTimeoutMs(EmberNodeId nodeId)
{
    ...
    if (emberLookupEui64ByNodeId(nodeId, eui64) == EMBER_SUCCESS && emberGetExtendedTimeout(eui64))
    {
        retryTimeoutMs += emberMacIndirectTimeout;
    }
...
bool emAfFragmentationIncomingMessage(EmberIncomingMessageType type, EmberApsFrame * apsFrame, EmberNodeId sender,
                                      uint8_t ** buffer, uint16_t * bufLen)
{
    ...
    if (rxPacket == NULL)
    {
        ...
        emberEventControlSetDelayMS(*(rxPacket->fragmentEventControl), (retryTimeoutMs(sender) * ZIGBEE_APSC_MAX_TRANSMIT_RETRIES));
        emberAfAddToCurrentAppTasks(EMBER_AF_FRAGMENTATION_IN_PROGRESS);
    }
    ...
    if (rxPacket->fragmentMask == 0xFF && rxPacket->fragmentBase + emberFragmentWindowSize <= fragment)
    {
    ...
    if (fragment < rxPacket->fragmentBase || fragment >= rxPacket->fragmentBase + emberFragmentWindowSize)
    {
        return true;
    }
    else
    { // Fragment inside the rx window.
        if (rxWindowMoved)
        {
        ...
        else
        {
            ...
            if (fragment < rxPacket->fragmentsExpected - 1 && rxPacket->fragmentLen != (uint8_t)(*bufLen))
            {
                goto kickout;
            }
        }
    ...
    if (fragment == rxPacket->fragmentsExpected - 1 ||
        (rxPacket->fragmentMask | lowBitMask(fragment % emberFragmentWindowSize)) == 0xFF)
    {
        emAfPluginFragmentationSendReply(sender, apsFrame, rxPacket);
    }

    // Received all the expected fragments.
    if (rxPacket->fragmentsReceived == rxPacket->fragmentsExpected)
    {
        uint8_t fragmentsInLastWindow = rxPacket->fragmentsExpected % emberFragmentWindowSize;
        ...
        if (rxPacket->status == EMBER_AF_PLUGIN_FRAGMENTATION_RX_PACKET_IN_USE)
        {
            ...
            // fragment.
            *bufLen = rxPacket->windowFinger + rxPacket->lastfragmentLen + (fragmentsInLastWindow - 1) * rxPacket->fragmentLen;
            *buffer = rxPacket->buffer;

            updateCurrentAppTasksForFragmentationState();
            return false;
        }
        else if (rxPacket->status == EMBER_AF_PLUGIN_FRAGMENTATION_RX_PACKET_PAYLOAD_TOO_LARGE)
        {
            // Send a default response with INSUFFICIENT_SPACE
            *bufLen = rxPacket->windowFinger + rxPacket->lastfragmentLen + (fragmentsInLastWindow - 1) * rxPacket->fragmentLen;
            *buffer = rxPacket->buffer;
            emberAfProcessMessageIntoZclCmd(apsFrame, type, *buffer, *bufLen, sender,
...
static void setFragmentMask(rxFragmentedPacket * rxPacket)
{
    ...
    if (rxPacket->fragmentsExpected < rxPacket->fragmentBase + emberFragmentWindowSize)
    {
...
static rxFragmentedPacket * getFreeRxPacketEntry(void)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_FRAGMENTATION_MAX_INCOMING_PACKETS; i++)
    {
        ...
        {
            if (ackedPacket == NULL || ackedPacket->ackedPacketAge < rxPacket->ackedPacketAge)
            {
...
static rxFragmentedPacket * rxPacketLookUp(EmberApsFrame * apsFrame, EmberNodeId sender)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_FRAGMENTATION_MAX_INCOMING_PACKETS; i++)
    {
        ...
        if (apsFrame->sequence == rxPacket->fragmentSequenceNumber && sender == rxPacket->fragmentSource)
        {
            return rxPacket;
        }
    }
