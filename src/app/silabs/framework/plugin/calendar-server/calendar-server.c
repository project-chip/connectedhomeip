...
void emberAfCalendarClusterServerTickCallback(uint8_t endpoint)
{
    ...
    {
        emberAfPluginCalendarServerPublishInfoCallback(publishInfo.publishCommandId, publishInfo.nodeId, publishInfo.clientEndpoint,
    ...
    if (publishInfo.publishCommandId == ZCL_PUBLISH_CALENDAR_COMMAND_ID)
    {
        ...
        emberAfCalendarServerPublishCalendarMessage(publishInfo.nodeId, publishInfo.serverEndpoint, publishInfo.clientEndpoint,
                                                    calendarIndex);
        commandSent = true;
    }
    else if (publishInfo.publishCommandId == ZCL_PUBLISH_DAY_PROFILE_COMMAND_ID)
    {
        ...
        emberAfCalendarServerPublishDayProfilesMessage(publishInfo.nodeId, publishInfo.serverEndpoint, publishInfo.clientEndpoint,
                                                       publishInfo.calendarIndex, dayIndex);
        commandSent = true;
    }
    else if (publishInfo.publishCommandId == ZCL_PUBLISH_WEEK_PROFILE_COMMAND_ID)
    {
        ...
        emberAfCalendarServerPublishWeekProfileMessage(publishInfo.nodeId, publishInfo.serverEndpoint, publishInfo.clientEndpoint,
                                                       publishInfo.calendarIndex, weekIndex);
        commandSent = true;
    }
    else if (publishInfo.publishCommandId == ZCL_PUBLISH_SEASONS_COMMAND_ID)
    {
        emberAfCalendarServerPublishSeasonsMessage(publishInfo.nodeId, publishInfo.serverEndpoint, publishInfo.clientEndpoint,
    ...
    {
        uint8_t calendarIndex = publishInfo.calendarsToPublish[publishInfo.commandIndex];
        emberAfCalendarServerPublishSpecialDaysMessage(publishInfo.nodeId, publishInfo.serverEndpoint, publishInfo.clientEndpoint,
                                                       calendarIndex);
        commandSent = true;
    }
    ...
    {
        emberAfScheduleServerTick(endpoint, ZCL_CALENDAR_CLUSTER_ID, MILLISECOND_TICKS_PER_QUARTERSECOND);
    }
    else
    {
        publishInfo.commandIndex = EMBER_AF_PLUGIN_CALENDAR_COMMON_INVALID_INDEX;
                                               ...
                                               uint8_t calendarType, uint32_t providerId)
{
    ...
    emberAfCalendarClusterPrintln("RX: GetCalendar 0x%4x, 0x%4x, 0x%x, 0x%x, 0x%4x", earliestStartTime, minIssuerEventId,
                                  numberOfCalendars, calendarType, providerId);
    ...
    if (publishInfo.commandIndex != EMBER_AF_PLUGIN_CALENDAR_COMMON_INVALID_INDEX)
    {
        emberAfCalendarClusterPrintln("%p%p%p",
                                      "Error: ", "Cannot get calendar: ", "only one Get command can be processed at a time");
    ...
    else
    {
        ...
        emberAfScheduleServerTick(emberAfCurrentEndpoint(), ZCL_CALENDAR_CLUSTER_ID, MILLISECOND_TICKS_PER_QUARTERSECOND);
    }
                                                  ...
                                                  uint8_t numberOfDays)
{
    ...
    uint8_t calendarIndex = emberAfPluginCalendarCommonGetCalendarById(issuerCalendarId, providerId);

    emberAfCalendarClusterPrintln("RX: GetDayProfiles 0x%4x, 0x%4x, 0x%x, 0x%x", providerId, issuerCalendarId, startDayId,
                                  numberOfDays);
    ...
    {
        emberAfCalendarClusterPrintln("%p%p%p",
                                      "Error: ", "Cannot get day profiles: ", "only one Get command can be processed at a time");
        emberAfSendDefaultResponse(cmd, EMBER_ZCL_STATUS_FAILURE);
        return true;
    }
                                                   ...
                                                   uint8_t numberOfWeeks)
{
    ...
    emberAfCalendarClusterPrintln("RX: GetWeekProfiles 0x%4x, 0x%4x, 0x%x, 0x%x", providerId, issuerCalendarId, startWeekId,
                                  numberOfWeeks);
    ...
    {
        emberAfCalendarClusterPrintln("%p%p%p",
                                      "Error: ", "Cannot get week profile: ", "only one Get command can be processed at a time");
        emberAfSendDefaultResponse(cmd, EMBER_ZCL_STATUS_FAILURE);
        return true;
    }
...
bool emberAfCalendarClusterGetSeasonsCallback(uint32_t providerId, uint32_t issuerCalendarId)
{
    ...
    uint8_t calendarIndex = emberAfPluginCalendarCommonGetCalendarById(issuerCalendarId, providerId);
    emberAfCalendarClusterPrintln("RX: GetSeasons 0x%4x, 0x%4x", providerId, issuerCalendarId);

    if (calendarIndex == EMBER_AF_PLUGIN_CALENDAR_COMMON_INVALID_INDEX || calendars[calendarIndex].numberOfSeasons == 0)
    {
        emberAfSendImmediateDefaultResponse(EMBER_ZCL_STATUS_NOT_FOUND);
        return true;
    }
    ...
    {
        emberAfCalendarClusterPrintln("%p%p%p",
                                      "Error: ", "Cannot get seasons: ", "only one Get command can be processed at a time");
        emberAfSendDefaultResponse(cmd, EMBER_ZCL_STATUS_FAILURE);
        return true;
    }
                                                  ...
                                                  uint32_t providerId, uint32_t issuerCalendarId)
{
    ...
    emberAfCalendarClusterPrintln("RX: GetSpecialDays 0x%4x, 0x%x, 0x%x, 0x%4x, 0x%4x", startTime, numberOfEvents, calendarType,
    ...
    {
        emberAfCalendarClusterPrintln("%p%p%p",
                                      "Error: ", "Cannot get special days: ", "only one Get command can be processed at a time");
        emberAfSendDefaultResponse(cmd, EMBER_ZCL_STATUS_FAILURE);
        return true;
    }
    ...
    while (numberOfEvents == 0 || numberOfSpecialDaysSent < numberOfEvents)
    {
        ...
        for (i = 0; i < EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS; i++)
        {
            if (calendars[i].calendarId != EMBER_AF_PLUGIN_CALENDAR_COMMON_INVALID_CALENDAR_ID &&
                ...
                startTime < emberAfPluginCalendarCommonEndTimeUtc(&(calendars[i])) && calendars[i].startTimeUtc < referenceUtc)
            {
                referenceUtc = calendars[i].startTimeUtc;
                indexToSend  = i;
            }
...
bool emberAfCalendarClusterGetCalendarCancellationCallback(void)
{
    ...
    {
        emberAfFillCommandCalendarClusterCancelCalendar(lastCancelCalendar->providerId, lastCancelCalendar->issuerEventId,
                                                 ...
                                                 uint8_t calendarIndex)
{
    ...
    {
        emberAfCalendarClusterPrintln("%p%p%p", "Error: ", "Cannot publish calendar: ", "invalid calendar index");
        return;
    }

    emberAfFillCommandCalendarClusterPublishCalendar(
        calendars[calendarIndex].providerId, calendars[calendarIndex].issuerEventId, calendars[calendarIndex].calendarId,
        calendars[calendarIndex].startTimeUtc, calendars[calendarIndex].calendarType, EMBER_ZCL_CALENDAR_TIME_REFERENCE_UTC_TIME,
        calendars[calendarIndex].name, calendars[calendarIndex].numberOfSeasons, calendars[calendarIndex].numberOfWeekProfiles,
    ...
    {
        emberAfCalendarClusterPrintln("%p%p0x%x", "Error: ", "Cannot publish calendar: ", status);
    }
}
                                                    ...
                                                    uint8_t calendarIndex, uint8_t dayIndex)
{
    ...
    if (EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS <= calendarIndex)
    {
        emberAfCalendarClusterPrintln("%p%p%p", "Error: ", "Cannot publish day profiles: ", "invalid calendar index");
        return;
    }

    if (calendars[calendarIndex].numberOfDayProfiles <= dayIndex)
    {
        emberAfCalendarClusterPrintln("%p%p%p", "Error: ", "Cannot publish day profiles: ", "invalid day index");
        return;
    }
    ...
    emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_SERVER_TO_CLIENT
                              ...
                              ZCL_CALENDAR_CLUSTER_ID, ZCL_PUBLISH_DAY_PROFILE_COMMAND_ID, "wwwuuuuu",
                              calendars[calendarIndex].providerId, calendars[calendarIndex].issuerEventId,
                              calendars[calendarIndex].calendarId, calendars[calendarIndex].normalDays[dayIndex].id,
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfCalendarClusterPrintln("%p%p0x%x", "Error: ", "Cannot publish day profiles: ", status);
    }
}
                                                    ...
                                                    uint8_t calendarIndex, uint8_t weekIndex)
{
    ...
    if (EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS <= calendarIndex)
    {
        emberAfCalendarClusterPrintln("%p%p%p", "Error: ", "Cannot publish week profile: ", "invalid calendar index");
        return;
    }

    if (calendars[calendarIndex].numberOfWeekProfiles <= weekIndex)
    {
        emberAfCalendarClusterPrintln("%p%p%p", "Error: ", "Cannot publish week profile: ", "invalid week index");
        return;
    }
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfCalendarClusterPrintln("%p%p0x%x", "Error: ", "Cannot publish week profile: ", status);
    }
}
...
void emberAfCalendarServerPublishSeasonsMessage(EmberNodeId nodeId, uint8_t srcEndpoint, uint8_t dstEndpoint, uint8_t calendarIndex)
{
    ...
    if (EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS <= calendarIndex)
    {
        emberAfCalendarClusterPrintln("%p%p%p", "Error: ", "Cannot publish seasons: ", "invalid calendar index");
        return;
    }
    ...
    emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_SERVER_TO_CLIENT
                              ...
                              ZCL_CALENDAR_CLUSTER_ID, ZCL_PUBLISH_SEASONS_COMMAND_ID, "wwwuu", calendars[calendarIndex].providerId,
                              calendars[calendarIndex].issuerEventId, calendars[calendarIndex].calendarId,
                              0,  // command index
                              1); // total commands
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfCalendarClusterPrintln("%p%p0x%x", "Error: ", "Cannot publish seasons: ", status);
    }
}
                                                    ...
                                                    uint8_t calendarIndex)
{
    ...
    if (EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS <= calendarIndex)
    {
        emberAfCalendarClusterPrintln("%p%p%p", "Error: ", "Cannot publish special days: ", "invalid calendar index");
        return;
    }
    ...
    emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_SERVER_TO_CLIENT
                              ...
                              ZCL_CALENDAR_CLUSTER_ID, ZCL_PUBLISH_SPECIAL_DAYS_COMMAND_ID, "wwwwuuuu",
                              calendars[calendarIndex].providerId, calendars[calendarIndex].issuerEventId,
                              calendars[calendarIndex].calendarId, calendars[calendarIndex].startTimeUtc,
                              calendars[calendarIndex].calendarType, daysToSend,
                              0,  // command index
                              1); // total commands
    ...
    while (daysSent < daysToSend)
    {
        ...
        else
        {
            ...
            emberAfPutInt8uInResp(
                calendars[calendarIndex].normalDays[calendars[calendarIndex].specialDays[indexToAdd].normalDayIndex].id);
            daysSent++;
    ...
    if (status != EMBER_SUCCESS)
    {
        emberAfCalendarClusterPrintln("%p%p0x%x", "Error: ", "Cannot publish special days: ", status);
    }
...
void emberAfCalendarServerCancelCalendarMessage(EmberNodeId nodeId, uint8_t srcEndpoint, uint8_t dstEndpoint, uint8_t calendarIndex)
{
    ...
    if (EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS <= calendarIndex)
    {
        emberAfCalendarClusterPrintln("%p%p%p", "Error: ", "Cannot send cancel calendar: ", "invalid calendar index");
        return;
    }

    emberAfFillCommandCalendarClusterCancelCalendar(calendars[calendarIndex].providerId, calendars[calendarIndex].issuerEventId,
                                                    calendars[calendarIndex].calendarType);
    emberAfSetCommandEndpoints(srcEndpoint, dstEndpoint);
    emberAfGetCommandApsFrame()->options |= EMBER_APS_OPTION_SOURCE_EUI64;
    status = emberAfSendCommandUnicast(EMBER_OUTGOING_DIRECT, nodeId);
