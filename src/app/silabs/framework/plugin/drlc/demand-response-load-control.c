 ...
 **/
static bool afMatchUegOrDeviceClass(uint8_t endpoint, uint8_t utilityEnrollmentGroup, uint16_t deviceClass)
{
    uint8_t ueg = 0;
    uint16_t dc;
    uint8_t dcArray[2];
    ...
    status = emberAfReadAttribute(endpoint, ZCL_DEMAND_RESPONSE_LOAD_CONTROL_CLUSTER_ID, ZCL_UTILITY_ENROLLMENT_GROUP_ATTRIBUTE_ID,
                                  CLUSTER_MASK_CLIENT, (uint8_t *) &ueg, 1, &dataType);
    ...
    dataType = 0;
    status   = emberAfReadAttribute(endpoint, ZCL_DEMAND_RESPONSE_LOAD_CONTROL_CLUSTER_ID, ZCL_DEVICE_CLASS_VALUE_ATTRIBUTE_ID,
                                  CLUSTER_MASK_CLIENT, (uint8_t *) dcArray, 2, &dataType);
...
// -------------------------------------------------------
static void fillReportEventData(uint8_t commandStatus, uint8_t sequenceNumber, EmberAfLoadControlEvent * e, uint32_t statusTime)
{
    // emberAfDemandResponseLoadControlClusterFlush();
    emberAfDemandResponseLoadControlClusterPrintln("= TX Report Event Status =");
    // emberAfDemandResponseLoadControlClusterFlush();
    emberAfDemandResponseLoadControlClusterPrintln("  eid: %4x", e->eventId);
    // emberAfDemandResponseLoadControlClusterFlush();
    emberAfDemandResponseLoadControlClusterPrintln("   es: %x", commandStatus);
    emberAfDemandResponseLoadControlClusterPrintln("   st: T%4x", statusTime);
    // emberAfDemandResponseLoadControlClusterFlush();
    emberAfDemandResponseLoadControlClusterPrintln("  cla: %x", e->criticalityLevel);
    emberAfDemandResponseLoadControlClusterPrintln("   ec: %x", e->eventControl);
    emberAfDemandResponseLoadControlClusterPrintln("ctspa: %2x (%d)", e->coolingTempSetPoint, e->coolingTempSetPoint);
    // emberAfDemandResponseLoadControlClusterFlush();
    emberAfDemandResponseLoadControlClusterPrintln("htspa: %2x (%d)", e->heatingTempSetPoint, e->heatingTempSetPoint);
    ...
    appResponseLength = 0;
    emberAfPutInt8uInResp(ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_CLIENT_TO_SERVER);
 ...
 * randomization.
 */
static uint16_t afGetRandomizationTime(uint8_t endpoint, bool startRand)
{
    uint8_t maxRandMin, dataType;
    EmberStatus status = emberAfReadAttribute(
        endpoint, ZCL_DEMAND_RESPONSE_LOAD_CONTROL_CLUSTER_ID,
        (startRand ? ZCL_START_RANDOMIZATION_MINUTES_ATTRIBUTE_ID : ZCL_DURATION_RANDOMIZATION_MINUTES_ATTRIBUTE_ID),
        CLUSTER_MASK_CLIENT, (uint8_t *) &maxRandMin, 1, &dataType);
...
void emberAfDemandResponseLoadControlClusterClientInitCallback(uint8_t endpoint)
{
    ...
    emberAfWriteAttribute(endpoint, ZCL_DEMAND_RESPONSE_LOAD_CONTROL_CLUSTER_ID, ZCL_DEVICE_CLASS_VALUE_ATTRIBUTE_ID,
                          CLUSTER_MASK_CLIENT, (uint8_t *) (&deviceClass), ZCL_INT16U_ATTRIBUTE_TYPE);

    emAfLoadControlEventTableInit(endpoint);

    // Start tick
    emberAfScheduleClientTick(endpoint, ZCL_DEMAND_RESPONSE_LOAD_CONTROL_CLUSTER_ID, MILLISECOND_TICKS_PER_SECOND);
}
...
bool emberAfDemandResponseLoadControlClusterLoadControlEventCallback(
    uint32_t eventId, uint16_t deviceClass, uint8_t utilityEnrollmentGroup, uint32_t startTime, uint16_t duration,
    uint8_t criticalityLevel, uint8_t coolingTempOffset, uint8_t heatingTempOffset, int16_t coolingTempSetPoint,
    int16_t heatingTempSetPoint, int8_t averageLoadAdjustPercent, uint8_t dutyCycle, uint8_t eventControl)
{
    ...
    emberAfDemandResponseLoadControlClusterPrintln(" dc/g: %2x / %x", deviceClass, utilityEnrollmentGroup);
    ...
    emberAfDemandResponseLoadControlClusterPrintln("co/ho: %x / %x", coolingTempOffset, heatingTempOffset);
    emberAfDemandResponseLoadControlClusterFlush();
    emberAfDemandResponseLoadControlClusterPrintln("ctspa: %2x (%d)", coolingTempSetPoint, coolingTempSetPoint);
    emberAfDemandResponseLoadControlClusterPrintln("htspa: %2x (%d)", heatingTempSetPoint, heatingTempSetPoint);
    emberAfDemandResponseLoadControlClusterFlush();
    emberAfDemandResponseLoadControlClusterPrintln("  avg: %x", averageLoadAdjustPercent);
    ...
    if (!afMatchUegOrDeviceClass(emberAfCurrentEndpoint(), utilityEnrollmentGroup, deviceClass))
    {
        emberAfDemandResponseLoadControlClusterPrintln("Neither UEG nor Device class matched.  Ignoring.");
    ...
    {
        emberAfDemandResponseLoadControlClusterPrintln("Reserved criticality level.  Ignoring.");
        return true;
    }
    ...
    emberAfDemandResponseLoadControlClusterPrintln("schedule -- start: %4x, start-random: %2x duration-random: %2x", e.startTime,
                                                   e.startRand, e.durationRand);

    emAfScheduleLoadControlEvent(emberAfCurrentEndpoint(), &e);
    return true;
}

bool emberAfDemandResponseLoadControlClusterCancelLoadControlEventCallback(uint32_t eventId, uint16_t deviceClass,
                                                                           ...
                                                                           uint32_t effectiveTime)
{
    ...
    emAfCancelLoadControlEvent(emberAfCurrentEndpoint(), eventId, cancelControl, effectiveTime);
    return true;
}
...
void emberAfEventAction(EmberAfLoadControlEvent * event, uint8_t eventStatus, uint8_t sequenceNumber, uint8_t esiIndex)
{
    ...
    // the even.
    sendReport = emberAfPluginDrlcEventActionCallback(event, (EmberAfAmiEventStatus) eventStatus, sequenceNumber);
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_ESI_MANAGEMENT_ESI_TABLE_SIZE; i++)
    {
        ...
        if (esiEntry != NULL)
        {
            fillReportEventData(eventStatus, sequenceNumber, event, emberAfGetCurrentTime());
