...
void emberAfPluginNetworkCreatorStartCommand(void)
{
    ...
    emberAfCorePrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_NETWORK_CREATOR_PLUGIN_NAME, "Form", status);
}
...
void emberAfPluginNetworkCreatorFormCommand(void)
{
    ...
    EmberStatus status = emberAfPluginNetworkCreatorNetworkForm(centralizedSecurity, panId, radioTxPower, channel);
...
void emberAfPluginNetworkCreatorChannelMaskCommand(void)
{
    ...
    uint32_t * channelMask =
        (channelMaskIsPrimary ? &emAfPluginNetworkCreatorPrimaryChannelMask : &emAfPluginNetworkCreatorSecondaryChannelMask);

    // Check if operation is add or subtract first.
    if (emberStringCommandArgument(-1, NULL)[1] != 'e')
    {
        if (channelOrNewMask < EMBER_MIN_802_15_4_CHANNEL_NUMBER || channelOrNewMask > EMBER_MAX_802_15_4_CHANNEL_NUMBER)
        {
            emberAfCorePrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_NETWORK_CREATOR_PLUGIN_NAME, "Illegal 802.15.4 channel",
                               channelOrNewMask);
        }
    ...
    emberAfCorePrint("%p channel mask now: 0x%4X [", (channelMaskIsPrimary ? "Primary" : "Secondary"), *channelMask);
...
{
    uint8_t i;
    uint32_t masks[] = { 0, emAfPluginNetworkCreatorPrimaryChannelMask, emAfPluginNetworkCreatorSecondaryChannelMask };

    emberAfCorePrintln("%p: %p:", EMBER_AF_PLUGIN_NETWORK_CREATOR_PLUGIN_NAME, "Channel mask");
    for (i = 1; i <= 2; i++)
    {
        emberAfCorePrint("    (%d) 0x%4X [", i, masks[i]);
