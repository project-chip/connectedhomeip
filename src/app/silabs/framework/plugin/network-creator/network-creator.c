...
/***************************************************************************/ /**
                                                                               ...
                                                                               *plugin, which handles forming a network per the
...
#define SECONDARY_CHANNEL_MASK (EMBER_ALL_802_15_4_CHANNELS_MASK ^ EMBER_AF_PLUGIN_NETWORK_CREATOR_CHANNEL_MASK)
...
static void updateChannelComposites(int8_t rssi, EmAfPluginNetworkCreatorChannelCompositeMetric metric, uint8_t channel);
...
EmberStatus emberAfPluginNetworkCreatorStart(bool centralizedNetwork)
{
    ...
    {
        emberAfCorePrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_NETWORK_CREATOR_PLUGIN_NAME, "Cannot start. State", stateFlags);
...
EmberStatus emberAfPluginNetworkCreatorNetworkForm(bool centralizedNetwork, EmberPanId panId, int8_t radioTxPower, uint8_t channel)
{
    ...
    if ((channel > EMBER_MAX_802_15_4_CHANNEL_NUMBER) || (channel < EMBER_MIN_802_15_4_CHANNEL_NUMBER))
    {
        emberAfCorePrintln("%p: Error: Channel %d is out of range", EMBER_AF_PLUGIN_NETWORK_CREATOR_PLUGIN_NAME, channel);
    ...
    if (status == EMBER_SUCCESS)
    {
        ...
        emberAfCorePrintln("%p: Form. Channel: %d. Status: 0x%X", EMBER_AF_PLUGIN_NETWORK_CREATOR_PLUGIN_NAME, channel, status);
    }
...
static EmberStatus tryToFormNetwork(void)
{
    EmberNetworkParameters networkParameters;
    EmberStatus status;
    uint8_t channel  = (emberGetPseudoRandomNumber() & 0x0F) + EMBER_MIN_802_15_4_CHANNEL_NUMBER;
    ...
    if (status == EMBER_SUCCESS)
    {
        ...
        while (currentChannelMask != 0 && status != EMBER_SUCCESS)
        {
            ...
            channel = (channel == EMBER_MAX_802_15_4_CHANNEL_NUMBER ? EMBER_MIN_802_15_4_CHANNEL_NUMBER : channel + 1);
            ...
            if (status == EMBER_SUCCESS)
            {
                emberAfPluginNetworkCreatorCompleteCallback(&networkParameters, (maskIsSecondary() ? true : false));
            }
            else
            {
...
{
    EmberNetworkScanType scanType = emberAfPluginScanDispatchScanResultsGetScanType(results);
...
HIDDEN void scanHandler(EmberAfPluginScanDispatchScanResults * results)
{
    ...
    { // success
        ...
        {
            debugPrintln("Scan complete. Channel: %d. Status: 0x%X", results->channel, results->status);
        ...
        { // results
            if (scanType == EMBER_ACTIVE_SCAN)
            {
                ...
                debugPrintln("  PanId: 0x%2X, Channel: %d, PJoin: %p", results->network->panId, results->network->channel,
                             (results->network->allowingJoin ? "YES" : "NO"));
                debugPrintln("  lqi:  %d", results->lqi);
                debugPrintln("  rssi: %d", results->rssi);

                updateChannelComposites(results->rssi, EM_AF_PLUGIN_NETWORK_CREATOR_CHANNEL_COMPOSITE_METRIC_BEACONS,
                                        results->network->channel);
            }
            else if (scanType == EMBER_ENERGY_SCAN)
            {
                debugPrintln("Energy scan results.");
                debugPrintln("%p: Channel: %d. Rssi: %d", EMBER_AF_PLUGIN_NETWORK_CREATOR_PLUGIN_NAME, results->channel,
                             results->rssi);

                updateChannelComposites(results->rssi, EM_AF_PLUGIN_NETWORK_CREATOR_CHANNEL_COMPOSITE_METRIC_RSSI,
                                        results->channel);
            }
        }
    }
...
static void cleanupAndStop(EmberStatus status)
{
    emberAfCorePrintln("%p: Stop. Status: 0x%X. State: 0x%X", EMBER_AF_PLUGIN_NETWORK_CREATOR_PLUGIN_NAME, status, stateFlags);
}
...
static void cleanChannelComposites(void)
{
    ...
    {
        channelComposites[i].beaconsHeard = 0;
        channelComposites[i].maxRssiHeard = EM_AF_PLUGIN_NETWORK_CREATOR_CHANNEL_COMPOSITE_INVALID_RSSI;
    }
}
...
static void maybeClearChannelBitOfMaxRssiReading(uint8_t channel)
{
    ...
    for (i = 0, channelsConsideredSoFar = 0, maxIndex = 0; i < EMBER_NUM_802_15_4_CHANNELS; i++)
    {
        // If we have received an RSSI reading from this channel,
        // and we are still considering this channel...
        if ((channelComposites[i].maxRssiHeard != EM_AF_PLUGIN_NETWORK_CREATOR_CHANNEL_COMPOSITE_INVALID_RSSI) &&
            (READBIT(currentChannelMask, i + EMBER_MIN_802_15_4_CHANNEL_NUMBER)))
        {
            // ...increment the channelsConsideredSoFar by 1 and see if this
            // channel has the maximum RSSI.
            channelsConsideredSoFar++;
            if (channelComposites[i].maxRssiHeard > channelComposites[maxIndex].maxRssiHeard)
            {
                maxIndex = i;
            }
        }
    ...
    if ((channelsConsideredSoFar > EM_AF_PLUGIN_NETWORK_CREATOR_CHANNELS_TO_CONSIDER))
    {
        uint8_t channelToThrowAway = ((channelComposites[channel - EMBER_MIN_802_15_4_CHANNEL_NUMBER].maxRssiHeard >
