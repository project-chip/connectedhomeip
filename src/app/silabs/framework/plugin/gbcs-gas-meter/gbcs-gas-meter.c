...
static EmberAfAttributeId basicClusterAttributes[] = {
    ZCL_VERSION_ATTRIBUTE_ID,    ZCL_MODEL_IDENTIFIER_ATTRIBUTE_ID, ZCL_MANUFACTURER_NAME_ATTRIBUTE_ID,
    ZCL_HW_VERSION_ATTRIBUTE_ID, ZCL_POWER_SOURCE_ATTRIBUTE_ID,
};
...
#define GAS_METER_PREPAYMENT_CLUSTER_ATTRIBUTE_COUNT                                                                               \
    ((sizeof(prepaymentClusterAttributes) / sizeof(prepaymentClusterAttributes[0])))
...
static bool tunnelCreate(EmberNodeId remoteNodeId, uint8_t remoteEndpoint);
static bool tunnelDestroy(void);
static bool tunnelSendData(uint16_t dataLen, uint8_t * data);
...
void emberAfPluginGbcsGasMeterReportAttributes(void)
{
    ...
    if (state == MIRROR_READY)
    {
        ...
        emberAfWriteServerAttribute(GSME_ENDPOINT, ZCL_SIMPLE_METERING_CLUSTER_ID,
                                    ZCL_SIMPLE_METERING_CLUSTER_REPORTING_STATUS_SERVER_ATTRIBUTE_ID, &reportingStatus,
...
void emberAfClusterInitCallback(uint8_t endpointId, uint16_t clusterId)
{
    ...
    if ((endpointId == GSME_ENDPOINT) && (clusterId == ZCL_SIMPLE_METERING_CLUSTER_ID))
    {
...
void emberAfPluginGbcsGasMeterSleepyMeterEventHandler(void)
{
    ...
    emberAfEventControlSetDelay(&emberAfPluginGbcsGasMeterSleepyMeterEventControl, 2 * MILLISECOND_TICKS_PER_SECOND);
    ...
    case REGISTRATION_COMPLETE:
        ...
        // support mirroring.
        status = emberAfFindDevicesByProfileAndCluster(EMBER_RX_ON_WHEN_IDLE_BROADCAST_ADDRESS, SE_PROFILE_ID,
                                                       ZCL_SIMPLE_METERING_CLUSTER_ID, EMBER_AF_CLIENT_CLUSTER_DISCOVERY,
    ...
    case MIRROR_READY:
        ...
        {
            emberAfPluginGbcsGasMeterPrintln("Retrying tunnel creation to node ID 0x%2x", tunnel.remoteNodeId);
            requestTunnel();
        }
    ...
    case MIRROR_UPDATE_IN_PROGRESS:
        while (clusterToReport < GAS_METER_REPORT_LIST_COUNT)
        {
            if (reportList[clusterToReport].report)
            {
                ...
                {
                    EmberAfStatus status = emberAfAppendAttributeReportFields(
                        GSME_ENDPOINT, reportList[clusterToReport].clusterId,
                        reportList[clusterToReport].attributeIds[attributeToReport], CLUSTER_MASK_SERVER, data,
                        GAS_METER_ATTRIBUTE_RECORD_BUFFER_SIZE, &bufIndex);
                ...
                emberAfFillCommandGlobalServerToClientReportAttributes(reportList[clusterToReport].clusterId, data, bufIndex);
...
bool emberAfReadAttributesResponseCallback(EmberAfClusterId clusterId, uint8_t * buffer, uint16_t bufLen)
{
    ...
    // * request.
    if ((state == SERVICE_DISCOVERY_STARTED || state == MIRROR_CONFIGURATION_STARTED) && clusterId == ZCL_BASIC_CLUSTER_ID)
    {
        if (bufLen < 5)
        {
            return false;
            ...
            /* && (emberAfGetInt8u(buffer, 4, bufLen) & 0x01) */)
        {
            ...
            emberAfSetCommandEndpoints(GSME_ENDPOINT, currentCommand->apsFrame->sourceEndpoint);
...
void emberAfPluginSimpleMeteringServerProcessNotificationFlagsCallback(EmberAfAttributeId attributeId, uint32_t bitMap)
{
    uint32_t requestMessages = 0;

    emberAfPluginGbcsGasMeterPrintln("GSME: ProcessNotificationFlags: 0x%2x, 0x%4x", attributeId, bitMap);
    ...
    if (state == MIRROR_READY || state == MIRROR_UPDATE_IN_PROGRESS)
    {
        ...
        {
            requestMessages = bitMap &
                (EMBER_AF_METERING_NF4_GET_PREPAY_SNAPSHOT | EMBER_AF_METERING_NF4_GET_TOP_UP_LOG |
        ...
        {
            emberAfPluginGbcsGasMeterPrintln("GSME: GetNotifiedMessage: 0x%2x, 0x%4x", attributeId, requestMessages);
            emberAfFillCommandSimpleMeteringClusterGetNotifiedMessage(
                EMBER_ZCL_NOTIFICATION_SCHEME_PREDEFINED_NOTIFICATION_SCHEME_B, attributeId, requestMessages);
            emberAfSetCommandEndpoints(GSME_ENDPOINT, mirrorEndpoint);
            emberAfSendCommandUnicast(EMBER_OUTGOING_DIRECT, mirrorAddress);
        }
    }
...
bool emberAfSimpleMeteringClusterRequestMirrorResponseCallback(uint16_t endpointId)
{
    ...
    else
    {
        if (state != MIRROR_READY)
        {
            ...
            emberAfFillCommandSimpleMeteringClusterConfigureMirror(issuerEventId, MIRROR_UPDATE_INTERVAL_SECONDS, true,
...
{
    emberAfPluginGbcsGasMeterPrintln("GSME: Smart Energy Registration %p.", (success ? "completed" : "FAILED"));
    if (!success)
    {
        return;
    }
...
{
    emberAfPluginGbcsGasMeterPrintln("GSME: ClientTunnelOpened:0x%x,0x%x,0x%2x", tunnelId, tunnelStatus,
...
void emberAfPluginTunnelingClientDataReceivedCallback(uint8_t tunnelId, uint8_t * data, uint16_t dataLen)
{
    ...
    if (tunnel.tunnelId != tunnelId || dataLen < 2 || *data != TUNNEL_MANAGER_HEADER_GET_RESPONSE)
    {
        return;
    }
...
static void setSleepyMeterState(uint8_t newState)
{
    ...
    case MIRROR_UPDATE_IN_PROGRESS:
    default:
        emberEventControlSetDelayQS(emberAfPluginGbcsGasMeterSleepyMeterEventControl,
                                    MIRROR_UPDATE_IN_PROGRESS_INTERVAL_QUARTER_SECONDS);
        break;
    }
}
...
static bool tunnelCreate(EmberNodeId remoteNodeId, uint8_t remoteEndpoint)
{
    emberAfPluginGbcsGasMeterPrintln("GSME: TunnelCreate 0x%2x 0x%x", remoteNodeId, remoteEndpoint);
...
static bool tunnelSendData(uint16_t dataLen, uint8_t * data)
{
    ...
    if (!success)
    {
        emberAfAppPrintln("%p%p%p0x%x", "Error: ", "Tunnel SendData failed: ", "Tunneling Status: ", status);
    }
    return success;
}
...
static bool requestTunnel(void)
{
    ...
    status = emberAfPluginTunnelingClientRequestTunnel(tunnel.remoteNodeId, GSME_ENDPOINT, tunnel.remoteEndpoint,
                                                       GBCS_TUNNELING_PROTOCOL_ID, GBCS_TUNNELING_MANUFACTURER_CODE,
                                                       GBCS_TUNNELING_FLOW_CONTROL_SUPPORT);
    if (status != EMBER_AF_PLUGIN_TUNNELING_CLIENT_SUCCESS && !handleRequestTunnelFailure(status))
    {
        return false;
    }
...
static bool handleRequestTunnelFailure(EmberAfPluginTunnelingClientStatus status)
{
    emberAfPluginGbcsGasMeterPrintln("GSME: handleRequestTunnelFailure 0x%x", status);
    ...
    else if (status == EMBER_AF_PLUGIN_TUNNELING_CLIENT_NO_MORE_TUNNEL_IDS)
    {
        ...
        // to the CHF.
        emberAfAppPrintln("%p%p%p", "Error: ", "Tunnel Create failed: ", "No more tunnel ids");
        tunnel.state = CLOSED_TUNNEL;
        return false;
    }
    ...
    emberAfAppPrintln("%p%p%p0x%x", "Error: ", "Tunnel Create failed: ", "Tunneling Client Status: ", status);
    tunnel.state = CLOSED_TUNNEL;
    return false;
}
...
static void handleRemotePartyMessage(uint16_t gbzCommandLen, uint8_t * gbzCommand)
{
    ...
    validGbzMessage =
        emberAfPluginGbzMessageControllerParserInit(&gbzParser, EMBER_AF_GBZ_MESSAGE_COMMAND, gbzCommand, gbzCommandLen, false, 0);
    if (validGbzMessage)
    {
        // TODO: properly handle the request.  For now we just send a default response to all
        // all requests.

        emberAfPluginGbzMessageControllerCreatorInit(&gbzCreator, EMBER_AF_GBZ_MESSAGE_RESPONSE, 0, 0, 0, &message[headerLen],
        ...
        while (emberAfPluginGbzMessageControllerHasNextCommand(&gbzParser))
        {
            ...
            zclResponse.direction = (zclRequest.direction == ZCL_DIRECTION_SERVER_TO_CLIENT) ? ZCL_DIRECTION_CLIENT_TO_SERVER
                                                                                             : ZCL_DIRECTION_SERVER_TO_CLIENT;
            zclResponse.frameControl |=
                ((zclResponse.direction == ZCL_DIRECTION_SERVER_TO_CLIENT) ? ZCL_FRAME_CONTROL_SERVER_TO_CLIENT : 0);
