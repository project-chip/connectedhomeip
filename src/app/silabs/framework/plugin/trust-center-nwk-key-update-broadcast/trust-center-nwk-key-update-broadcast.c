 ...
 */
/***************************************************************************/ /**
                                                                               * @brief Functionality for rolling the network key by
                                                                               *broadcasting the key update and the NWK key switch
                                                                               *messages. This is the preferred mechanism for Home
                                                                               *Automation and another networks without individual
                                                                               *link keys per device.
                                                                               *
                                                                               * The process is pretty straightforward.
                                                                               *   1. Broadcast new NWK key (randomly generated)
                                                                               *   2. Wait until the broadcast fades out of the
                                                                               *network (~20 seconds)
                                                                               *   3. Broadcast NWK key switch message
                                                                               *
                                                                               * Note: This does not PERIODICALLY update the NWK
                                                                               *key.  It just manages the process when it is told to
                                                                               *initate the key change.  Another software module
                                                                               *must determine the policy for periodically
                                                                               *initiating this process (such as the 'Trust Center
                                                                               *NWK Key Update Periodic' App Framework Plugin).
                                                                               *
                                                                               *******************************************************************************
                                                                               * # License
...
static bool nextNetworkKeyIsNewer(EmberKeyStruct * nextNwkKey)
{
    ...
    emberGetKey(EMBER_CURRENT_NETWORK_KEY, &currentNwkKey);

    status = emberGetKey(EMBER_NEXT_NETWORK_KEY, nextNwkKey);
    if (status != EMBER_SUCCESS || (timeGTorEqualInt8u(currentNwkKey.sequenceNumber, nextNwkKey->sequenceNumber)))
    {
        return false;
    }
...
EmberStatus emberAfTrustCenterStartNetworkKeyUpdate(void)
{
    ...
    if (emberAfGetNodeId() != EMBER_TRUST_CENTER_NODE_ID || emberAfNetworkState() != EMBER_JOINED_NETWORK ||
    ...
    if (!nextNetworkKeyIsNewer(&nextNwkKey))
    {
        ...
        MEMSET(emberKeyContents(&(nextNwkKey.key)), 0, EMBER_ENCRYPTION_KEY_SIZE);
    }
    ...
    else
    {
        ...
        emberEventControlSetDelayQS(myEvent, EMBER_BROADCAST_TABLE_TIMEOUT_QS + 2);
    }
    return status;
}
