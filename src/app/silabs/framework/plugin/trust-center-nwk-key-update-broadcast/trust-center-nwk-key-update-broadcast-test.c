...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Test code for the Trust Center Network Key
...
EmberStatus emberBroadcastNextNetworkKey(EmberKeyData * newKey)
{
    ...
    expectMessage((0 == MEMCOMPARE(newKey, &testFrameworkZeroKey, EMBER_ENCRYPTION_KEY_SIZE)),
                  "Error: New key does not match expected key of 0's.\n");
    return (broadcastFails ? EMBER_NO_BUFFERS : EMBER_SUCCESS);
}
...
int main(int argc, char * argv[])
{
    ...
    return parseCommandLineAndExecuteTest(argc, argv, argv[0], allTests);
}
