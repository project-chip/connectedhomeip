...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Routines for the Network Creator Security
                                                                               *plugin, which configures network permit join state
                                                                               *and key policy for a Zigbee 3.0 network.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
#ifdef EZSP_HOST
// NCP
#define allowTrustCenterLinkKeyRequests() ezspSetPolicy(EZSP_TC_KEY_REQUEST_POLICY, EZSP_ALLOW_TC_KEY_REQUESTS_AND_SEND_CURRENT_KEY)
...
#define setTransientKeyTimeout(timeS) ezspSetValue(EZSP_VALUE_TRANSIENT_KEY_TIMEOUT_S, 2, (uint8_t *) (&timeS));
...
#define setTransientKeyTimeout(timeS) emberTransientKeyTimeoutS = (timeS)
...
void emberAfPluginNetworkCreatorSecurityInitCallback(void)
{
    ...
    EzspStatus status =
        emberAfSetEzspPolicy(EZSP_TRUST_CENTER_POLICY, (EZSP_DECISION_ALLOW_JOINS | EZSP_DECISION_JOINS_USE_INSTALL_CODE_KEY),
                             "Trust Center Policy", "Joins using install code only");

    if (EZSP_SUCCESS != status)
    {
        emberAfCorePrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_PLUGIN_NAME,
                           "failed to configure joining using install code only", status);
    }
...
void emberAfPluginNetworkCreatorSecurityStackStatusCallback(EmberStatus status)
{
    ...
    if (status == EMBER_NETWORK_UP && emberAfGetNodeId() == EMBER_TRUST_CENTER_NODE_ID)
    {
...
#if defined(EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_TRUST_CENTER_SUPPORT) || defined(EMBER_AF_PLUGIN_TEST_HARNESS_Z3)
...
void emberAfPluginNetworkCreatorSecurityZigbeeKeyEstablishmentCallback(EmberEUI64 eui64, EmberKeyStatus keyStatus)
{
#if defined(EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_TRUST_CENTER_SUPPORT) || defined(EMBER_AF_PLUGIN_TEST_HARNESS_Z3)
         ...
         keyStatus == EMBER_TC_REQUESTER_VERIFY_KEY_FAILURE))
    {
        ...
        if (destinationId != EMBER_NULL_NODE_ID)
        {
            ...
            status = emberLeaveRequest(destinationId, eui64,
                                       0, // no leave flags
        ...
        emberAfCorePrintln("%p: Remove node 0x%2X for failed key verification: 0x%X",
                           EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_PLUGIN_NAME, destinationId, status);
    }
#endif /* defined(EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_TRUST_CENTER_SUPPORT)                                                   \
       || defined(EMBER_AF_PLUGIN_TEST_HARNESS_Z3) */
}
...
EmberStatus emberAfPluginNetworkCreatorSecurityStart(bool centralizedNetwork)
{
    ...
    state.bitmask = (EMBER_TRUST_CENTER_GLOBAL_LINK_KEY | EMBER_HAVE_PRECONFIGURED_KEY | EMBER_HAVE_NETWORK_KEY |
                     EMBER_NO_FRAME_COUNTER_RESET | EMBER_REQUIRE_ENCRYPTED_KEY);

    extended = EMBER_JOINER_GLOBAL_LINK_KEY;

    if (!centralizedNetwork)
    {
        MEMMOVE(&(state.preconfiguredKey), emberKeyContents(&distributedKey), EMBER_ENCRYPTION_KEY_SIZE);
...
kickout:
    emberAfCorePrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_PLUGIN_NAME, "Start", status);
    return status;
}
...
#define EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_ALLOW_TC_REJOINS_USING_WELL_KNOWN_KEY_TIMEOUT_SEC 0
    ...
    setTcRejoinsUsingWellKnownKeyTimeout(
        EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_ALLOW_TC_REJOINS_USING_WELL_KNOWN_KEY_TIMEOUT_SEC);
...
void emberAfPluginNetworkCreatorSecurityOpenNetworkEventHandler(void)
{
    ...
    if (openNetworkTimeRemainingS > EMBER_AF_PERMIT_JOIN_MAX_TIMEOUT)
    {
        ...
        emberEventControlSetDelayQS(openNetworkEventControl, EMBER_AF_PERMIT_JOIN_MAX_TIMEOUT << 2);
    }
    else
    {
    ...
    emberAfCorePrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_NETWORK_CREATOR_SECURITY_PLUGIN_NAME, "Open network", status);
}
