...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               * 1. After each device announce, add the device to a
                                                                               *list of knonwn devices.
                                                                               * 2. Query the device for all Active Endpoints
                                                                               *(Active Endpoint Request)
                                                                               * 3. Query each endpoint for its list of clusters and
                                                                               *device information.
                                                                               *
                                                                               * Also periodically map the network to find all
                                                                               *devices.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static void scheduleEvent(bool withDelay)
{
    if (withDelay)
    {
        ...
        emberEventControlSetDelayQS(emberAfPluginDeviceQueryServiceMyEventControl, DISCOVERY_DELAY_QS);
    }
    else
    {
...
bool emAfPluginDeviceQueryPreZDOMessageReceived(EmberNodeId sender, EmberApsFrame * apsFrame, uint8_t * message, uint16_t length)
{
    ...
    if (apsFrame->clusterId == END_DEVICE_ANNOUNCE)
    {
        ...
        device = emberAfPluginDeviceDatabaseAdd(tempEui64, message[DEVICE_ANNOUNCE_CAPABILITIES_OFFSET]);
        if (device == NULL)
        {
            emberAfCorePrint("Error: %p failed to add device to database: ", PLUGIN_NAME);
    ...
    else if (apsFrame->clusterId == NODE_DESCRIPTOR_RESPONSE)
    {
        ...
        if (device && (currentNodeId == sender) && (EMBER_AF_DEVICE_DISCOVERY_STATUS_FIND_STACK_REVISION == device->status))
        {
            serverMask = message[12] | (message[13] << 8);
            serverMask &= SERVER_MASK_STACK_COMPLIANCE_REVISION_MASK;
            stackRevision = (serverMask >> SERVER_MASK_STACK_COMPLIANCE_REVISION_BIT_POSITION);

            debugPrintln("%p got Node Descriptor response from 0x%2X", PLUGIN_NAME, currentNodeId);

            emAfPluginDeviceDatabaseUpdateNodeStackRevision(currentEui64, stackRevision);

            // This is the last state - we're done
            emberAfPluginDeviceDatabaseSetStatus(currentEui64, EMBER_AF_DEVICE_DISCOVERY_STATUS_DONE);
            scheduleEvent(WITH_DELAY);
            clearCurrentDevice();
        }
    }
...
static void serviceDiscoveryCallback(const EmberAfServiceDiscoveryResult * result)
{
    ...
    else if (result->zdoRequestClusterId == ACTIVE_ENDPOINTS_REQUEST)
    {
        ...
        if (!emberAfPluginDeviceDatabaseSetEndpoints(device->eui64, listStruct->list, listStruct->count))
        {
    ...
    else if (result->zdoRequestClusterId == SIMPLE_DESCRIPTOR_REQUEST)
    {
        ...
        debugPrintln("%p found %d server clusters and %d client clusters on EP %d", PLUGIN_NAME, clusterInfo->inClusterCount,
                     clusterInfo->outClusterCount, clusterInfo->endpoint);
        if (!emberAfPluginDeviceDatabaseSetClustersForEndpoint(device->eui64, clusterInfo))
        {
...
static void sendSimpleDescriptorRequest(const EmberAfDeviceInfo * device)
{
    ...
    if (endpoint == 0xFF)
    {
        ...
        emberAfPluginDeviceDatabaseSetStatus(device->eui64, EMBER_AF_DEVICE_DISCOVERY_STATUS_FIND_STACK_REVISION);
...
static void sendNodeDescriptorRequest(const EmberAfDeviceInfo * device)
{
    ...
    debugPrintln("%p initiating node descriptor request for: 0x%2X", PLUGIN_NAME, currentNodeId);

    status = emberNodeDescriptorRequest(currentNodeId, EMBER_AF_DEFAULT_APS_OPTIONS);
...
void emberAfPluginDeviceQueryServiceMyEventHandler(void)
{
    ...
    if (emberAfMemoryByteCompare(currentEui64, EUI64_SIZE, 0xFF))
    {
        ...
        device = emberAfPluginDeviceDatabaseFindDeviceByStatus(
            EMBER_AF_DEVICE_DISCOVERY_STATUS_NEW | EMBER_AF_DEVICE_DISCOVERY_STATUS_FIND_ENDPOINTS |
            EMBER_AF_DEVICE_DISCOVERY_STATUS_FIND_CLUSTERS | EMBER_AF_DEVICE_DISCOVERY_STATUS_FIND_STACK_REVISION);
        if (device != NULL)
        {
            debugPrintln("%p found device with status (0x%X): %p", PLUGIN_NAME, device->status,
