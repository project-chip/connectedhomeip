...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief A list of all devices known in the network
                                                                               *and their services.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
void emAfPluginDeviceDatabasePrintAll(void)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_DEVICE_DATABASE_MAX_DEVICES; i++)
    {
        ...
        emberAfCorePrintln(" - Capabilities: 0x%X - EPs: %d - Status: %p - Failures:%d - Stack Revision: %d", device->capabilities,
                           device->endpointCount, emberAfPluginDeviceDatabaseGetStatusString(device->status),
                           device->discoveryFailures, device->stackRevision);
    }
...
void emAfPluginDeviceDatabasePrintDevice(void)
{
    ...
    {
        emberAfCorePrintln("EP: %d - Profile ID: 0x%2X - Device ID: 0x%2X - Clusters: %d", device->endpoints[i].endpoint,
                           device->endpoints[i].profileId, device->endpoints[i].deviceId, device->endpoints[i].clusterCount);
        uint8_t j;
        for (j = 0; j < device->endpoints[i].clusterCount; j++)
        {
            emberAfCorePrintln("  Cluster: 0x%2X %p", device->endpoints[i].clusters[j].clusterId,
                               (device->endpoints[i].clusters[j].server ? "Server" : "Client"));
        }
    }
...
void emAfPluginDeviceDatabaseAddDummyDevice(void)
{
    ...
    {
        emberAfCorePrintln("Error:  Base endpoint of %d + number of endpoints %d would exceed 254", DUMMY_BASE_ENDPOINT,
                           dummy.endpointCount);
        return;
    }
    ...
    {
        emberAfCorePrintln("Error: Base cluster ID 0x%2X + number of clusters %d would exceed 65534", DUMMY_BASE_CLUSTER_ID,
                           clusterCount);
        return;
    }
