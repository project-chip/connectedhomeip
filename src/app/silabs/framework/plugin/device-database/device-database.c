...
const EmberAfDeviceInfo * emberAfPluginDeviceDatabaseFindDeviceByStatus(EmberAfDeviceDiscoveryStatus status)
{
    ...
    {
        if (!emberAfMemoryByteCompare(deviceDatabase[i].eui64, EUI64_SIZE, 0xFF) && (deviceDatabase[i].status & status))
        {
...
bool emberAfPluginDeviceDatabaseSetEndpoints(const EmberEUI64 eui64, const uint8_t * endpointList, uint8_t endpointCount)
{
    ...
    MEMSET(device->endpoints, 0xFF, sizeof(EmberAfEndpointInfoStruct) * EMBER_AF_MAX_ENDPOINTS_PER_DEVICE);

    device->endpointCount = (endpointCount < EMBER_AF_MAX_ENDPOINTS_PER_DEVICE ? endpointCount : EMBER_AF_MAX_ENDPOINTS_PER_DEVICE);
...
{
    EmberAfDeviceInfo * device = findDeviceByEui64(eui64);
    if (device != NULL && index < EMBER_AF_MAX_ENDPOINTS_PER_DEVICE)
    {
...
{
    EmberAfDeviceInfo * device = findDeviceByEui64(eui64);
    uint8_t index              = (device != NULL ? findEndpoint(device, endpoint) : 0xFF);
    return index;
}
...
bool emberAfPluginDeviceDatabaseSetClustersForEndpoint(const EmberEUI64 eui64, const EmberAfClusterList * clusterList)
{
    ...
    {
        emberAfCorePrintln("%p too many clusters (%d) for endpoint.  Limiting to %d", PLUGIN_NAME,
    ...
    for (doServer = 0; doServer < 2; doServer++)
    {
        ...
        for (clusterPointerIndex = 0; (clusterPointerIndex < count) && (deviceClusterIndex < device->endpoints[index].clusterCount);
             clusterPointerIndex++)
        {
...
static void doesDeviceHaveCluster(const EmberAfDeviceInfo * device, EmberAfClusterId clusterToFind, bool server,
                                  uint8_t * returnEndpoint)
{
    ...
    *returnEndpoint = EMBER_AF_INVALID_ENDPOINT;
    for (i = 0; i < EMBER_AF_MAX_ENDPOINTS_PER_DEVICE; i++)
    {
        ...
        {
            if (device->endpoints[i].clusters[j].clusterId == clusterToFind && device->endpoints[i].clusters[j].server == server)
            {
...
EmberStatus emberAfPluginDeviceDatabaseDoesDeviceHaveCluster(EmberEUI64 deviceEui64, EmberAfClusterId clusterToFind, bool server,
                                                             uint8_t * returnEndpoint)
{
    EmberAfDeviceInfo * device = findDeviceByEui64(deviceEui64);
    if (device == NULL)
...
EmberStatus emberAfPluginDeviceDatabaseFindDeviceSupportingCluster(EmberAfDeviceDatabaseIterator * iterator,
                                                                   EmberAfClusterId clusterToFind, bool server,
                                                                   uint8_t * returnEndpoint)
{
...
void emAfPluginDeviceDatabaseUpdateNodeStackRevision(EmberEUI64 eui64, uint8_t stackRevision)
{
    EmberAfDeviceInfo * device = findDeviceByEui64(eui64);
    if (device != NULL)
    {
