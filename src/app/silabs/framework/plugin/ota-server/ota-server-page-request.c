...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               * This handles the optional feature of a device
                                                                               *requesting a a full page (of EEPROM) and getting
                                                                               *multiple image block responses.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
#define pageRequestTickCallback(x, y) emAfServerPageRequestTickCallback(x, y)
#else
                                  ...
                                  uint8_t maxDataSize, uint16_t pageSize, uint16_t responseSpacing)
{
    ...
    emberAfOtaBootloadClusterPrintln(
        "RX ImagePageReq mfgId:%2x imageType:%2x, file:%4x, offset:%4x dataSize:%d pageSize%2x spacing:%d", id->manufacturerId,
        id->imageTypeId, id->firmwareVersion, offset, maxDataSize, pageSize, responseSpacing);
    ...
    requesterResponseSpacing = (responseSpacing < SHORTEST_SEND_RATE ? SHORTEST_SEND_RATE : responseSpacing);
    totalBytesSent           = 0;
...
void emAfOtaPageRequestTick(uint8_t endpoint)
{
    ...
    sendBlockRequest();
    emberAfScheduleServerTickExtended(endpoint, ZCL_OTA_BOOTLOAD_CLUSTER_ID, requesterResponseSpacing, EMBER_AF_SHORT_POLL,
...
static void sendBlockRequest(void)
{
    ...
    {
        maxDataToSend = (bytesLeft > requesterMaxDataSize ? requesterMaxDataSize : (uint8_t) bytesLeft);
    }
    else
    {
    ...
    if (pageRequestTickCallback(totalBytesSent, requesterMaxDataSize))
    {
        ...
        callbackStruct.bitmask        = EMBER_AF_IMAGE_BLOCK_REQUEST_SIMULATED_FROM_PAGE_REQUEST;
    ...
    else
    {
        ...
        if (totalBytesSent >= totalSize || totalBytesSent >= requesterPageSize)
        {
