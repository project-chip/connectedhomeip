...
const char * pluginEepromDataPrintCommandArguments[] = { "The offset within the EEPROM to print.", NULL };
...
void emAfEepromDataPrintCommand(void)
{
    ...
    if (status)
    {
        emberAfCorePrintln("Error:  Failed to read from EEPROM at 0x%4X, status: %d", offset, status);
        return;
    }

    emberAfPrint8ByteBlocks(DATA_BLOCK >> 3, data,
...
{
    uint8_t i;
    emberAfCorePrintln("EEPROM Initialized: %c", (emAfIsEepromInitialized() ? 'y' : 'n'));
    ...
    emberAfCorePrintln("EEPROM Busy: %c", (!emAfIsEepromInitialized() ? '?' : (emberAfPluginEepromBusy() ? 'y' : 'n')));
    emberAfCorePrintln("Partial Word Storage Count: %d", EMBER_AF_PLUGIN_EEPROM_PARTIAL_WORD_STORAGE_COUNT);

    for (i = 0; i < EMBER_AF_PLUGIN_EEPROM_PARTIAL_WORD_STORAGE_COUNT; i++)
    {
        emberAfCorePrintln("Address: 0x%4X, Partial Word: 0x%X", emAfEepromSavedPartialWrites[i].address,
...
void emAfEepromInfoCommand(void)
{
    ...
    else
    {
        ...
        emberAfCorePrintln("Word size (bytes):         %d (%p)", wordSize, (confirmedWordSize ? "confirmed" : "assumed"));
    }
}
