...
// -------------------------------------------------------
EmberAfOtaImageId emAfOtaCreateEmberAfOtaImageIdStruct(uint16_t manufacturerId, uint16_t imageType, uint32_t fileVersion);

uint8_t emAfOtaParseImageIdFromMessage(EmberAfOtaImageId * returnId, const uint8_t * buffer, uint8_t length);

#if defined(EMBER_AF_PRINT_CORE)
void emAfPrintPercentageSetStartAndEnd(uint32_t startingOffset, uint32_t endOffset);
uint8_t emAfPrintPercentageUpdate(const char * prefixString, uint8_t updateFrequency, uint32_t currentOffset);
