...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Server side implementation of the SE 1.2
...
bool emberAfPluginDeviceManagementSetTenancy(EmberAfDeviceManagementTenancy * tenancy, bool validateOptionalFields)
{
    ...
    {
        if ((tenancy->providerId != pmInfo.providerId) || (tenancy->issuerEventId < pmInfo.issuerEventId) ||
...
bool emberAfPluginDeviceManagementSetSupplier(uint8_t endpoint, EmberAfDeviceManagementSupplier * supplier)
{
    ...
    {
        EmberStatus status =
            emberAfWriteAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, ZCL_PROVIDER_NAME_ATTRIBUTE_ID, CLUSTER_MASK_SERVER,
                                  (uint8_t *) &(supplier->proposedProviderName), ZCL_OCTET_STRING_ATTRIBUTE_TYPE);
        if (status == EMBER_ZCL_STATUS_SUCCESS)
        {
            status = emberAfWriteAttribute(endpoint, ZCL_DEVICE_MANAGEMENT_CLUSTER_ID, ZCL_PROVIDER_CONTACT_DETAILS_ATTRIBUTE_ID,
                                           CLUSTER_MASK_SERVER, (uint8_t *) &(supplier->proposedProviderContactDetails),
...
bool emberAfDeviceManagementClusterGetChangeOfTenancyCallback(void)
{
    ...
    {
        emberAfFillCommandDeviceManagementClusterPublishChangeOfTenancy(pmInfo.providerId, pmInfo.issuerEventId, pmInfo.tariffType,
                                                                        tenancy->implementationDateTime, tenancy->tenancy);
        emberAfSendResponse();
    }
...
bool emberAfDeviceManagementClusterGetChangeOfSupplierCallback(void)
{
    ...
    else
    {
        emberAfFillCommandDeviceManagementClusterPublishChangeOfSupplier(
            pmInfo.providerId, pmInfo.issuerEventId, pmInfo.tariffType, supplier->proposedProviderId,
            supplier->implementationDateTime, supplier->providerChangeControl, supplier->proposedProviderName,
...
bool emberAfDeviceManagementClusterPublishChangeOfSupplier(EmberNodeId dstAddr, uint8_t srcEndpoint, uint8_t dstEndpoint)
{
    ...
    EmberAfDeviceManagementSupplier * supplier = &(pmInfo.supplier);
    emberAfFillCommandDeviceManagementClusterPublishChangeOfSupplier(
        pmInfo.providerId, pmInfo.issuerEventId, pmInfo.tariffType, supplier->proposedProviderId, supplier->implementationDateTime,
        supplier->providerChangeControl, supplier->proposedProviderName, supplier->proposedProviderContactDetails);
    emberAfSetCommandEndpoints(srcEndpoint, dstEndpoint);
    status = emberAfSendCommandUnicast(EMBER_OUTGOING_DIRECT, dstAddr);
...
bool emberAfDeviceManagementClusterGetSiteIdCallback(void)
{
    ...
    {
        emberAfFillCommandDeviceManagementClusterUpdateSiteId(pmInfo.issuerEventId, siteId->implementationDateTime,
                                                              pmInfo.providerId, siteId->siteId);
        emberAfSendResponse();
    }
    else
    {
...
bool emberAfDeviceManagementClusterGetCINCallback(void)
{
    ...
    {
        emberAfFillCommandDeviceManagementClusterUpdateCIN(pmInfo.issuerEventId, cin->implementationDateTime, pmInfo.providerId,
                                                           cin->cin);
        emberAfSendResponse();
    }
    else
...
bool emberAfDeviceManagementClusterRequestNewPasswordCallback(uint8_t passwordType)
{
    ...
    {
        emberAfFillCommandDeviceManagementClusterRequestNewPasswordResponse(pmInfo.issuerEventId, password->implementationDateTime,
                                                                            password->durationInMinutes, password->passwordType,
                                                                            password->password);
...
bool emberAfDeviceManagementClusterSendRequestNewPasswordResponse(uint8_t passwordType, EmberNodeId dstAddr, uint8_t srcEndpoint,
                                                                  uint8_t dstEndpoint)
{
    EmberStatus status;
    EmberAfDeviceManagementPassword * password;
...
bool emberAfDeviceManagementClusterUpdateSiteId(EmberNodeId dstAddr, uint8_t srcEndpoint, uint8_t dstEndpoint)
{
    ...
    emberAfFillCommandDeviceManagementClusterUpdateSiteId(pmInfo.issuerEventId, siteId->implementationDateTime, pmInfo.providerId,
                                                          siteId->siteId);
    status = emberAfSendCommandUnicast(EMBER_OUTGOING_DIRECT, dstAddr);
