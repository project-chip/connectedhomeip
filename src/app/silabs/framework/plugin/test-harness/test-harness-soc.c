...
static void hashTheFirstPage(uint8_t * address)
{
    ...
    assert(EMBER_SUCCESS == (emberAesMmoHashUpdate(&context, AAT_MAX_SIZE, aatBuff)));
    ...
    assert(EMBER_SUCCESS == (emberAesMmoHashUpdate(&context, FLASH_PAGE_SIZE_35X - AAT_MAX_SIZE, address + AAT_MAX_SIZE)));
#if defined(PRINT_INTERMEDIATE_HASH_RESULTS)
...
void emberAfPluginTestHarnessFlashPageHashEventHandler(void)
{
...
#if defined(PRINT_INTERMEDIATE_HASH_RESULTS)
    emberAfCorePrintln("Processing Page Range Index %d, page %d, address: 0x%4X", pageRangeIndex, pageBeg + pageWithinRangeIndex,
                       (uint32_t) address);
#endif
    assert(EMBER_SUCCESS == (emberAesMmoHashUpdate(&context, FLASH_PAGE_SIZE_35X, address)));
#if defined(PRINT_INTERMEDIATE_HASH_RESULTS)
    ...
    {
        pageRangeIndex++;
        if (pageRangeIndex >= 6 || (halAppAddressTable.pageRanges[pageRangeIndex].begPg == 0xFF))
        {
            const uint8_t unsetImageStamp[] = {
                0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
            };
            assert(EMBER_SUCCESS == emberAesMmoHashFinal(&context, 0, NULL));
            ...
            else
            {
                ...
                emberAfCorePrintln(
                    "Image stamp %pmatch first 8 bytes in hash result.",
                    (0 == MEMCOMPARE(halAppAddressTable.imageStamp, context.result, IMAGE_STAMP_SIZE)) ? "" : "DOES NOT ");
            }

            return;
        }
    }
    emberEventControlSetDelayMS(emberAfPluginTestHarnessFlashPageHashEventControl, 1);
#endif // HASH_CODE_SUPPORT
}
...
void emAfTestHarnessStartImageStampCalculation(void)
{
    ...
    for (i = 0; i < 6; i++)
    {
        emberAfCorePrintln("Index %d: Beg Page: %d, End Page: %d", i, halAppAddressTable.pageRanges[i].begPg,
