...
void emAfOtaServerPolicyPrint(void)
{
    ...
    otaPrintln("Query Policy: %p (%d)", nextVersionPolicyStrings[nextVersionPolicy], nextVersionPolicy);

#if defined EM_AF_TEST_HARNESS_CODE
    otaPrintln("Image Block Request Policy: %p (%d)", imageBlockRequestPolicyStrings[imageBlockRequestPolicy],
               imageBlockRequestPolicy);
    if (imageBlockRequestPolicy == DELAY_DOWNLOAD_ONCE)
    {
        otaPrintln("  Delay time: %d seconds", IMAGE_BLOCK_REQUEST_DELAY_TIME_SECONDS);
    }
    otaPrintln("Page Request Block Missed Modulus: %d", missedBlockModulus);
#else
    otaPrintln("Image Block Request Policy: Send block");
#endif
    otaPrintln("Image Block Request Min Period: %d milliseconds", otaMinimumBlockPeriodMs);
    emberAfCoreFlush();

    otaPrintln("Page Request Code Compiled in: %p", PAGE_REQUEST_COMPILE_TIME_SUPPORT);
    otaPrintln("Page Request Runtime Status Code: 0x%X", pageRequestStatus);
    emberAfCoreFlush();

    otaPrintln("Upgrade Request Policy: %p (%d)", upgradePolicyStrings[upgradePolicy], upgradePolicy);
...
static bool determineNextSoftwareVersion(uint32_t versionServerHas, uint32_t versionClientHas)
{
                                      ...
                                      EmberAfOtaImageId * nextUpgradeImageId)
{
    ...
    otaPrintln("QueryNextImageRequest mfgId:0x%2x imageTypeId:0x%2x, fw:0x%4x", currentImageId->manufacturerId,
               currentImageId->imageTypeId, currentImageId->firmwareVersion);
    *nextUpgradeImageId =
        emberAfOtaStorageSearchCallback(currentImageId->manufacturerId, currentImageId->imageTypeId, hardwareVersion);
    ...
    if (emberAfIsOtaImageIdValid(nextUpgradeImageId))
    {
        ...
        if (hardwareVersion)
        {
            ...
            emberAfOtaStorageGetFullHeaderCallback(nextUpgradeImageId, &header);
            if (header.fieldControl & HARDWARE_VERSIONS_PRESENT_MASK)
            {
                if (*hardwareVersion < header.minimumHardwareVersion || header.maximumHardwareVersion < *hardwareVersion)
                {
                    otaPrintln("Hardware version 0x%02X does not fall within the min (0x%02X) and max (0x%02X) hardware versions "
                               "in the file.",
                               *hardwareVersion, header.minimumHardwareVersion, header.maximumHardwareVersion);
        ...
        {
            status = (determineNextSoftwareVersion(nextUpgradeImageId->firmwareVersion, currentImageId->firmwareVersion)
                          ? EMBER_ZCL_STATUS_SUCCESS
                          : EMBER_ZCL_STATUS_NO_IMAGE_AVAILABLE);
            if (status == EMBER_ZCL_STATUS_SUCCESS)
            {
                otaPrintln("Next fw version is: 0x%4X", nextUpgradeImageId->firmwareVersion);
            }
        }
    }
    return status;
...
uint8_t emberAfOtaServerImageBlockRequestCallback(EmberAfImageBlockRequestCallbackStruct * data)
{
    ...
    if (SERVER_AND_CLIENT_SUPPORT_MIN_BLOCK_REQUEST == (data->bitmask & SERVER_AND_CLIENT_SUPPORT_MIN_BLOCK_REQUEST))
    {
        clientDelayUnit = emberAfPluginOtaServerPolicyGetClientDelayUnits(data->source, data->sourceEui);
...
{
    otaPrintln("Client 0x%2X indicated upgrade status: 0x%X", source, status);
...
void emAfOtaServerSetPageRequestPolicy(uint8_t value)
{
    ...
    pageRequestStatus = (value ? EMBER_ZCL_STATUS_SUCCESS : EMBER_ZCL_STATUS_UNSUP_CLUSTER_COMMAND);
}
...
bool emAfServerPageRequestTickCallback(uint16_t relativeOffset, uint8_t dataSize)
{
    uint16_t block = (relativeOffset + dataSize) / dataSize;
    emberAfCoreFlush();
    if (missedBlockModulus && (block % missedBlockModulus == 0))
    {
        emberAfCoreFlush();
