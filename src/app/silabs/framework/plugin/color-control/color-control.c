...
static EmberAfStatus colorControlReadCurrentSaturation(uint8_t endpoint, uint8_t * saturation);
...
void emberAfColorControlClusterServerTickCallback(uint8_t endpoint)
{
    ...
    {
        emberAfColorControlClusterPrintln("ERR: could not read current hue %x", status);
        return;
    }
    status = colorControlReadCurrentSaturation(endpoint, &sat);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        emberAfColorControlClusterPrintln("ERR: could not read current saturation %x", status);
        return;
    }
    ...
    case ZCL_MOVE_SATURATION_COMMAND_ID:
        ...
        {
            state->active = (sat != state->saturationMoveToLevel || sat == 0x00 || sat == 0xFE);
        }
        break;
    default:
...
#ifdef ZCL_USING_COLOR_CONTROL_CLUSTER_COLOR_CONTROL_REMAINING_TIME_ATTRIBUTE
    {
        ...
        state->elapsedTime = (state->active ? state->elapsedTime + state->eventDuration : state->transitionTime);
        ...
        status = emberAfWriteServerAttribute(endpoint, ZCL_COLOR_CONTROL_CLUSTER_ID, ZCL_COLOR_CONTROL_REMAINING_TIME_ATTRIBUTE_ID,
    ...
    emberAfScheduleServerTick(endpoint, ZCL_COLOR_CONTROL_CLUSTER_ID, state->eventDuration);
}
...
static void colorControlClearRemainingTime(uint8_t endpoint)
{
    ...
    EmberAfStatus status =
        emberAfWriteServerAttribute(endpoint, ZCL_COLOR_CONTROL_CLUSTER_ID, ZCL_COLOR_CONTROL_REMAINING_TIME_ATTRIBUTE_ID,
                                    (uint8_t *) &data, ZCL_INT16U_ATTRIBUTE_TYPE);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        emberAfColorControlClusterPrintln("ERR: writing remaining time %x", status);
        return;
...
static void colorControlSetHue(uint8_t endpoint, uint8_t hue)
{
    ...
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        emberAfColorControlClusterPrintln("ERR: writing current hue %x", status);
...
static void colorControlSetSaturation(uint8_t endpoint, uint8_t saturation)
{
    ...
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        emberAfColorControlClusterPrintln("ERR: writing current saturation %x", status);
        return;
    }
...
static void colorControlSetColorModeToZero(void)
{
    ...
    uint8_t colorMode = 0;
    EmberAfStatus status =
        emberAfWriteServerAttribute(emberAfCurrentEndpoint(), ZCL_COLOR_CONTROL_CLUSTER_ID,
                                    ZCL_COLOR_CONTROL_COLOR_MODE_ATTRIBUTE_ID, &colorMode, ZCL_ENUM8_ATTRIBUTE_TYPE);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        emberAfColorControlClusterPrintln("ERR: writing color mode%x", status);
    }
    emberAfDebugPrintln("colorMode=%x", colorMode);
    emberAfColorControlClusterFlush();
#else
    emberAfDebugPrintln("no color mode attribute");
    emberAfColorControlClusterFlush();
...
static EmberAfStatus colorControlReadCurrentHue(uint8_t endpoint, uint8_t * hue)
{
    EmberAfStatus status = emberAfReadServerAttribute(endpoint, ZCL_COLOR_CONTROL_CLUSTER_ID,
                                                      ZCL_COLOR_CONTROL_CURRENT_HUE_ATTRIBUTE_ID, hue, sizeof(uint8_t));
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
...
static EmberAfStatus colorControlReadCurrentSaturation(uint8_t endpoint, uint8_t * saturation)
{
    EmberAfStatus status = emberAfReadServerAttribute(
        endpoint, ZCL_COLOR_CONTROL_CLUSTER_ID, ZCL_COLOR_CONTROL_CURRENT_SATURATION_ATTRIBUTE_ID, saturation, sizeof(uint8_t));
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        emberAfColorControlClusterPrintln("ERR: reading current saturation %x", status);
    }
    return status;
}
                                                 ...
                                                 uint8_t optionsOverride)
{
    ...
    emberAfColorControlClusterPrintln("ColorControl: MoveToHue (%x, %x, %2x)", hue, direction, transitionTime);
    ...
    status = colorControlReadCurrentSaturation(emberAfCurrentEndpoint(), &currentSaturation);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        goto send_default_response;
    }
    ...
    state->eventDuration  = state->transitionTime / ((state->hueMoveDirection) ? (hue - currentHue) : (currentHue - hue));
    // Set the Color Mode attribute to zero as per the spec, ignoring any errors.
    colorControlSetColorModeToZero();

    // Schedule the next tick
    if (emberAfScheduleServerTick(emberAfCurrentEndpoint(), ZCL_COLOR_CONTROL_CLUSTER_ID, state->eventDuration) != EMBER_SUCCESS)
    {
...
bool emberAfColorControlClusterMoveHueCallback(uint8_t moveMode, uint8_t rate, uint8_t optionsMask, uint8_t optionsOverride)
{
    ...
    emberAfColorControlClusterPrintln("ColorControl: MoveHue (%x, %x)", moveMode, rate);
    ...
    case EMBER_ZCL_HUE_MOVE_MODE_STOP:
        state->active = false;
        emberAfDeactivateServerTick(emberAfCurrentEndpoint(), ZCL_COLOR_CONTROL_CLUSTER_ID);
#ifdef ZCL_USING_COLOR_CONTROL_CLUSTER_COLOR_CONTROL_REMAINING_TIME_ATTRIBUTE
                                               ...
                                               uint8_t optionsOverride)
{
    ...
    emberAfColorControlClusterPrintln("ColorControl: StepHue (%x, %x, %x)", stepMode, stepSize, transitionTime);
    ...
    if (!emberAfPluginColorControlIsColorSupportedCallback(state->hueMoveToLevel, currentSaturation) || stepSize == 0)
    {
        status = EMBER_ZCL_STATUS_INVALID_VALUE;
        goto send_default_response;
    }
                                                        ...
                                                        uint8_t optionsOverride)
{
    ...
    emberAfColorControlClusterPrintln("ColorControl: MoveToSaturation (%x, %2x)", saturation, transitionTime);
    ...
    state->satMoveDirection      = (saturation > currentSaturation ? true : false);
    state->eventDuration =
        state->transitionTime / ((state->satMoveDirection) ? (saturation - currentSaturation) : (currentSaturation - saturation));
...
bool emberAfColorControlClusterMoveSaturationCallback(uint8_t moveMode, uint8_t rate, uint8_t optionsMask, uint8_t optionsOverride)
{
    ...
    emberAfColorControlClusterPrintln("ColorControl: MoveSaturation (%x, %x)", moveMode, rate);
                                                      ...
                                                      uint8_t optionsMask, uint8_t optionsOverride)
{
    ...
    emberAfColorControlClusterPrintln("ColorControl: StepSaturation (%x, %x, %x)", stepMode, stepSize, transitionTime);
    ...
    case EMBER_ZCL_SATURATION_STEP_MODE_UP:
        state->saturationMoveToLevel = (stepSize > 0xFE - currentSaturation ? 0xFE : currentSaturation + stepSize);
        state->satMoveDirection      = true;
        break;
    case EMBER_ZCL_SATURATION_STEP_MODE_DOWN:
        state->saturationMoveToLevel = (stepSize > currentSaturation ? 0x00 : currentSaturation - stepSize);
        state->satMoveDirection      = false;
        break;
    default:
        status = EMBER_ZCL_STATUS_INVALID_FIELD;
    ...
    if (!emberAfPluginColorControlIsColorSupportedCallback(currentHue, state->saturationMoveToLevel) || stepSize == 0)
    {
        status = EMBER_ZCL_STATUS_INVALID_VALUE;
        goto send_default_response;
    }
                                                              ...
                                                              uint8_t optionsMask, uint8_t optionsOverride)
{
    ...
    emberAfColorControlClusterPrintln("ColorControl: MoveToHueAndSaturation (%x, %x, %2x)", hue, saturation, transitionTime);
    ...
    satDiff                      = (state->satMoveDirection ? saturation - currentSaturation : currentSaturation - saturation);

    if (hueDiff == 0)
    {
        return emberAfColorControlClusterMoveToSaturationCallback(saturation, transitionTime,
                                                                  0x00,  // optionsMask
                                                                  0x00); // optionsOverride
    }
    else if (satDiff == 0)
    {
        return emberAfColorControlClusterMoveToHueCallback(hue, EMBER_ZCL_HUE_DIRECTION_SHORTEST_DISTANCE, transitionTime,
                                                           0x00,  // optionsMask
                                                           0x00); // optionsOverride
    }
    ...
    state->acceleratedMoveRate = (state->acceleratedHue ? hueDiff / satDiff : satDiff / hueDiff);

    state->transitionTime = transitionTime * MILLISECOND_TICKS_PER_SECOND / 10;
    state->eventDuration  = state->transitionTime / ((state->acceleratedHue) ? hueDiff : satDiff);
...
static bool colorControlHueAndSaturationHandler(uint8_t endpoint)
{
    ...
    hueDiff = (hue > state->hueMoveToLevel) ? (0xFF - (hue - state->hueMoveToLevel)) : (state->hueMoveToLevel - hue);
    satDiff =
        (sat > state->saturationMoveToLevel) ? (0xFF - (sat - state->saturationMoveToLevel)) : (state->saturationMoveToLevel - sat);
