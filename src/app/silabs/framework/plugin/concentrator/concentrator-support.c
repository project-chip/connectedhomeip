...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Code common to SOC and host to handle
                                                                               *periodically broadcasting many-to-one route requests
                                                                               *(MTORRs).
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
#if (MIN_QS > MAX_QS)
#error                                                                                                                             \
    "Minimum broadcast time must be less than max (EMBER_PLUGIN_CONCENTRATOR_MIN_TIME_BETWEEN_BROADCASTS_SECONDS < EMBER_PLUGIN_CONCENTRATOR_MAX_TIME_BETWEEN_BROADCASTS_SECONDS)"
#endif
...
EmberAfPluginConcentratorRouterBehavior emAfPluginConcentratorRouterBehavior = EMBER_AF_PLUGIN_CONCENTRATOR_DEFAULT_ROUTER_BEHAVIOR;
...
void emberAfPluginConcentratorStackStatusCallback(EmberStatus status)
{
    ...
    {
        // now we clear/init the source route table everytime the network is up , therefore we do clear the source route table on
        // rejoin.
    }
    else if (status == EMBER_NETWORK_UP)
    {
        if ((emAfPluginConcentratorRouterBehavior == FULL) ||
            (emberAfGetNodeType(&nodeType) == EMBER_SUCCESS && nodeType == EMBER_COORDINATOR))
        {
            emberSetConcentrator(true, EMBER_AF_PLUGIN_CONCENTRATOR_CONCENTRATOR_TYPE,
                                 ...
                                 EMBER_AF_PLUGIN_CONCENTRATOR_DELIVERY_FAILURE_THRESHOLD, EMBER_AF_PLUGIN_CONCENTRATOR_MAX_HOPS);
        }
    }
}
