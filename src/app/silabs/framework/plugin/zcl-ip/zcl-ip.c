...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This file provides an interface to send and
                                                                               *receive zcl messages over an IP interface.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
//#define DEBUG_ON
#if defined(DEBUG_ON)
#define debugPrintBlock(blocks, pointer, printCarriageReturns) emberAfPrint8ByteBlocks(blocks, pointer, printCarriageReturns)
...
{
    uint16_t index = createOutgoingZclIpPacket(EM_AF_ZCL_IP_KEEPALIVE_PACKET_ID, EM_AF_ZCL_IP_KEEPALIVE_PACKET_SIZE);
...
// is used.
EmberStatus emberAfPluginZclIpGenerateOutgoingProxyPacket(EmberApsFrame * apsFrame, EmberNodeId nodeId, EmberEUI64 eui64,
                                                          uint8_t * zclMessageBuffer, uint16_t zclMessageBufferLength)
{
    uint16_t index        = 0;
    ...
    {
        emberAfCorePrintln("Error: %p can't encapsulate.  Packet size %d > %d", PLUGIN_NAME, packetLength,
    ...
    else if (EMBER_SUCCESS != emberLookupEui64ByNodeId(nodeId, &outgoingZclIpPacket[index]))
    {
...
EmberStatus emberAfPluginZclIpGenerateOutgoingConfigureRemoteBinding(EmberEUI64 targetEUI, uint8_t sourceEndpoint,
                                                                     uint8_t destEndpoint, uint16_t clusterId, EmberEUI64 destEUI,
                                                                     EmberEUI64 sourceEUI)
{
    uint16_t index =
        createOutgoingZclIpPacket(EM_AF_ZCL_IP_CONFIGURE_REMOTE_BINDING_ID, EM_AF_ZCL_IP_CONFIGURE_REMOTE_BINDING_COMMAND_LENGTH);
...
{
    uint16_t index = createOutgoingZclIpPacket(EM_AF_ZCL_IP_ADD_DEVICE_ID, EM_AF_ZCL_IP_ADD_DEVICE_COMMAND_LENGTH);
...
EmberStatus emberAfPluginZclIpGenerateOutgoingDeviceDiscoveryUpdate(const EmberAfDeviceInfo * device)
{
    ...
    if (packetLength > EMBER_AF_PLUGIN_ZCL_IP_MAX_PAYLOAD_SIZE)
    {
        emberAfCorePrintln("Error: Device Discovery Update packet too long (%d > %d)", packetLength,
                           EMBER_AF_PLUGIN_ZCL_IP_MAX_PAYLOAD_SIZE);
        return EMBER_NO_BUFFERS;
    }

    uint16_t index = createOutgoingZclIpPacket(EM_AF_ZCL_IP_DEVICE_DISCOVERY_UPDATE_ID, packetLength);
...
static bool checkCommandIdAndLength(uint16_t expectedCommandId, uint16_t expectedLength, bool exactLength,
                                    const char * expectedCommandIdString)
{
    ...
    {
        emberAfCorePrintln("Error: %p expected %p (id: 0x%2X) but got id: 0x%02X", PLUGIN_NAME, expectedCommandIdString,
    ...
    if (exactLength)
    {
        ...
        {
            emberAfCorePrintln("Error: %p command %p (id 0x%2X) expected exact length of %d but got %d", PLUGIN_NAME,
                               expectedCommandIdString, commandId, expectedLength, receivedLength);
            return false;
        }
    }
    else
    {
        if (receivedLength < expectedLength)
        {
            emberAfCorePrintln("Error: %p command %p (id 0x%) expected min length of %d but got %d", PLUGIN_NAME,
                               expectedCommandIdString, expectedLength, receivedLength);
            return false;
        }
    }
...
{
    if (!checkCommandIdAndLength(EM_AF_ZCL_IP_KEEPALIVE_PACKET_ID, EM_AF_ZCL_IP_KEEPALIVE_PACKET_SIZE,
                                 true, // exact length?
                                 "Keepalive packet"))
    {
        return EMBER_ERR_FATAL;
    ...
    *nextKeepaliveDelaySeconds =
        emAfGetLittleEndianInt32uFromBuffer(incomingZclIpPacket + EM_AF_ZCL_IP_KEEPALIVE_DELAY_SECONDS_INDEX);
...
{
    if (!checkCommandIdAndLength(EM_AF_ZCL_IP_COMMAND_ZCL_PROXY_PACKET_ID, EM_AF_ZCL_IP_PROXY_PACKET_MINIMUM_SIZE,
                                 false, // exact length?
                                 "ZCL Proxy Packet"))
    {
        return EMBER_ERR_FATAL;
    ...
    {
        emberAfCorePrintln("Error: %p encapsulated packet length %d exceeds max ZCL message length %d", PLUGIN_NAME,
                           returnCmd->bufLen, EMBER_AF_PLUGIN_ZCL_IP_MAX_PAYLOAD_SIZE);
        return EMBER_NO_BUFFERS;
    }
...
EmberStatus emberAfPluginZclIpRetrieveIncomingConfigureRemoteClusters(EmberAfRemoteClusterStruct * returnData, uint16_t maxCount,
                                                                      uint16_t * returnedCount)
{
    if (!checkCommandIdAndLength(EM_AF_ZCL_IP_CONFIGURE_REMOTE_CLUSTERS_ID, EM_AF_ZCL_IP_CONFIGURE_REMOTE_CLUSTERS_COMMAND_OVERHEAD,
                                 false, // exact length?
    ...
    if (count > maxCount)
    {
        emberAfCorePrintln("Error: %p the remote cluster count of %d exceeded local max of %d", PLUGIN_NAME, count, maxCount);
        return EMBER_ERR_FATAL;
    }
    *returnedCount = count;
    ...
    {
        emberAfCorePrintln("Error: %p expected packet length of %d (%d records * %d length) but got %d", PLUGIN_NAME,
                           expectedLength, count, EM_AF_ZCL_IP_CONFIGURE_REMOTE_CLUSTERS_RECORD_LENGTH, packetLength);
        return EMBER_ERR_FATAL;
    }
...
{
    if (!checkCommandIdAndLength(EM_AF_ZCL_IP_ADD_DEVICE_ID, EM_AF_ZCL_IP_ADD_DEVICE_COMMAND_LENGTH,
                                 true, // exact length?
                                 "Add device"))
    {
        return EMBER_ERR_FATAL;
    ...
    MEMMOVE(newDeviceEui64, incomingZclIpPacket + EM_AF_ZCL_IP_ADD_DEVICE_EUI64_INDEX, EUI64_SIZE);
    MEMMOVE(emberKeyContents(newDeviceKeyData), incomingZclIpPacket + EM_AF_ZCL_IP_ADD_DEVICE_LINK_KEY_INDEX,
...
{
    if (!checkCommandIdAndLength(EM_AF_ZCL_IP_CONFIGURE_REMOTE_BINDING_ID, EM_AF_ZCL_IP_CONFIGURE_REMOTE_BINDING_COMMAND_LENGTH,
                                 true, // exact length?
    ...
    MEMMOVE(returnData->targetEUI64, incomingZclIpPacket + EM_AF_ZCL_IP_CONFIGURE_REMOTE_BINDING_COMMAND_TARGET_EUI_OFFSET,
            EUI64_SIZE);
    ...
    MEMMOVE(returnData->destEUI64, incomingZclIpPacket + EM_AF_ZCL_IP_CONFIGURE_REMOTE_BINDING_COMMAND_DEST_EUI_OFFSET, EUI64_SIZE);

    MEMMOVE(returnData->sourceEUI64, incomingZclIpPacket + EM_AF_ZCL_IP_CONFIGURE_REMOTE_BINDING_COMMAND_SOURCE_EUI_OFFSET,
            EUI64_SIZE);
...
static bool calculateIncomingDeviceDiscoveryPacketLength(uint16_t * packetLength)
{
    *packetLength = EM_AF_ZCL_IP_DEVICE_DISCOVERY_UPDATE_COMMAND_MIN_LENGTH;
    ...
    debugPrintln("%p %d endpoints in Device Discovery Update message.", PLUGIN_NAME,
                 incomingZclIpPacket[EM_AF_ZCL_IP_DEVICE_DISCOVERY_UPDATE_COMMAND_ENDPOINT_COUNT_INDEX]);
    ...
    for (i = 0; i < incomingZclIpPacket[EM_AF_ZCL_IP_DEVICE_DISCOVERY_UPDATE_COMMAND_ENDPOINT_COUNT_INDEX] &&
         *packetLength < EMBER_AF_PLUGIN_ZCL_IP_MAX_PAYLOAD_SIZE + 3;
         i++)
    {
        uint8_t clusterCount =
            incomingZclIpPacket[*packetLength + EM_AF_ZCL_IP_DEVICE_DISCOVERY_UPDATE_COMMAND_CLUSTER_COUNT_OFFSET];
        debugPrintln("%p cluster count is %d (offset %d)", PLUGIN_NAME, clusterCount,
        ...
        *packetLength += (EM_AF_ZCL_IP_DEVICE_DISCOVERY_UPDATE_COMMAND_ENDPOINT_INFO_MIN_LENGTH + (clusterCount * (2 + 1)));
    }
    if (*packetLength > EMBER_AF_PLUGIN_ZCL_IP_MAX_PAYLOAD_SIZE)
    {
        return false;
...
EmberStatus emberAfPluginZclIpRetrieveIncomingDeviceDiscoveryUpdate(EmberAfDeviceInfo * device)
{
    ...
    if (!checkCommandIdAndLength(EM_AF_ZCL_IP_DEVICE_DISCOVERY_UPDATE_ID, packetLength,
                                 true, // exact length?
    ...
    MEMMOVE(device->eui64, incomingZclIpPacket + EM_AF_ZCL_IP_DEVICE_DISCOVERY_UPDATE_COMMAND_EUI64_INDEX, EUI64_SIZE);
    ...
    uint16_t i;
    for (i = 0;
         i < device->endpointCount && i < EMBER_AF_MAX_ENDPOINTS_PER_DEVICE && index < EMBER_AF_PLUGIN_ZCL_IP_MAX_PAYLOAD_SIZE; i++)
    {
        ...
        uint8_t j;
        for (j = 0; j < device->endpoints[i].clusterCount && j < EMBER_AF_MAX_CLUSTERS_PER_ENDPOINT &&
