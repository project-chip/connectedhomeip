...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This file provides unit tests for functions
                                                                               *defined in permit-join-manager.c
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
bool emberAfPluginPermitJoinManagerZdoMessageReceivedCallback(EmberNodeId emberNodeId, EmberApsFrame * apsFrame, uint8_t * message,
                                                              uint16_t length);
void emberAfPluginPermitJoinManagerTrustCenterJoinCallback(EmberNodeId newNodeId, EmberEUI64 newNodeEui64,
                                                           EmberNodeId parentOfNewNode, EmberDeviceUpdate status,
                                                           EmberJoinDecision decision);
//------------------------------------------------------------------------------
// Event System stubs.
EmberEventData afEvents[] = { { &emberAfPluginPermitJoinManagerTimeoutNetworkEventControls[0],
...
{
    simpleScriptCheck("emberPermitJoiningRequest", "emberPermitJoiningRequest", "iiii", 43981, 40,
...
static void deviceAnnounceWithoutJoiningListEntry(void)
{
    ...
    addSimpleAction("emberAfPluginPermitJoinManagerZdoMessageReceivedCallback()",
                    emberAfPluginPermitJoinManagerZdoMessageReceivedCallback, 4, 0xABCD, &apsFrame, message, 12);
    runScript();
}
...
static void deviceAnnounceTest(bool deviceAnnounceFound)
{
    ...
    emberAfPluginPermitJoinManagerTrustCenterJoinCallback(0xABCD, newNodeEui64, parentNodeId,
                                                          EMBER_STANDARD_SECURITY_UNSECURED_JOIN, EMBER_USE_PRECONFIGURED_KEY);
    ...
    addSimpleCheck("emberPermitJoiningRequest", "iiii", 0xABCD, EMBER_AF_PLUGIN_PERMIT_JOIN_MANAGER_PERMIT_JOINS_DURATION,
                   1, // TC significance is always 1
                   0);
    runScript();
}
