...
void emberAfPriceClearBlockThresholdsTable(uint8_t endpoint)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE; i++)
    {
        MEMSET(&priceServerInfo.scheduledBlockThresholdsTable.scheduledBlockThresholds[ep][i], 0x00,
...
bool emberAfPriceGetPriceMatrix(uint8_t endpoint, uint8_t index, EmberAfPriceCommonInfo * info, EmberAfScheduledPriceMatrix * pm)
{
    ...
    if (index < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE)
    {
        ...
        MEMCOPY(pm, &priceServerInfo.scheduledPriceMatrixTable.scheduledPriceMatrix[ep][index],
...
bool emberAfPriceGetBlockThresholdsTableEntry(uint8_t endpoint, uint8_t index, EmberAfScheduledBlockThresholds * bt)
{
    ...
    if (index < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE)
    {
        ...
        MEMCOPY(bt, &priceServerInfo.scheduledBlockThresholdsTable.scheduledBlockThresholds[ep][index],
                sizeof(EmberAfScheduledBlockThresholds));
        return true;
    }
                                           ...
                                           EmberAfScheduledTariff * tariff)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE; i++)
    {
        ...
        if (lookup->status != 0 && lookup->issuerTariffId == issuerTariffId)
        {
                                                ...
                                                EmberAfScheduledPriceMatrix * pm)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE; i++)
    {
        ...
        if (lookup_info->valid && (lookup_pm->status != 0) && lookup_pm->issuerTariffId == issuerTariffId)
        {
                                                    ...
                                                    EmberAfScheduledBlockThresholds * bt)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE; i++)
    {
        ...
        EmberAfScheduledBlockThresholds * lookup_bt =
            &priceServerInfo.scheduledBlockThresholdsTable.scheduledBlockThresholds[ep][i];
        if ((lookup_info->valid) && (lookup_bt->status != 0) && lookup_bt->issuerTariffId == issuerTariffId)
        {
            if (info != NULL)
            {
                MEMCOPY(info, lookup_info, sizeof(EmberAfPriceCommonInfo));
...
bool emberAfPriceAddTariffTableEntry(uint8_t endpoint, EmberAfPriceCommonInfo * info, const EmberAfScheduledTariff * curTariff)
{
    ...
    tableIndex = emberAfPluginPriceCommonGetCommonMatchingOrUnusedIndex(priceServerInfo.scheduledTariffTable.commonInfos[ep],
                                                                        ...
                                                                        info->issuerEventId, info->startTime, false);
    ...
    return emberAfPriceSetTariffTableEntry(endpoint, tableIndex, info, curTariff);
}
...
kickout:
    if (update)
    {
        emberAfPluginPriceCommonSort((EmberAfPriceCommonInfo *) &priceServerInfo.scheduledTariffTable.commonInfos[ep],
                                     ...
                                     sizeof(EmberAfScheduledTariff), EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE);
        emberAfPriceClusterScheduleTickCallback(endpoint, EMBER_AF_PRICE_SERVER_CHANGE_TARIFF_INFORMATION_EVENT_MASK);
    }
    return update;
}
...
bool emberAfPriceAddPriceMatrix(uint8_t endpoint, EmberAfPriceCommonInfo * info, EmberAfScheduledPriceMatrix * pm)
{
    ...
    return emberAfPriceSetPriceMatrix(endpoint, tableIndex, info, pm);
}
                                   ...
                                   uint8_t subPayloadControl, uint8_t * payload)
{
    ...
    while (payloadIndex + ZCL_PRICE_CLUSTER_PRICE_MATRIX_SUBPAYLOAD_BLOCK_SIZE <= payloadLength)
    {
        ...
        if ((subPayloadControl & 0x01) == 0x01)
        {
            ...
            emberAfPriceClusterPrintln("Info: Updating PriceMatrix tier[%d] = 0x%4X", tier, price);
        }
        else if ((subPayloadControl & 0x01) == 0x00)
        {
            ...
            emberAfPriceClusterPrintln("Info: Updating PriceMatrix blockAndTier[%d][%d] = 0x%4X", tier, blockNumber, price);
        }
                                ...
                                const EmberAfScheduledPriceMatrix * pm)
{
    ...
    if (index < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE)
    {
        ...
        emberAfPluginPriceCommonSort((EmberAfPriceCommonInfo *) &priceServerInfo.scheduledPriceMatrixTable.commonInfos[ep],
                                     ...
                                     sizeof(EmberAfScheduledPriceMatrix), EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE);
        emberAfPriceClusterScheduleTickCallback(endpoint, EMBER_AF_PRICE_SERVER_ACTIVATED_PRICE_MATRIX_EVENT_MASK);
                                              ...
                                              uint8_t subpayloadControl, uint8_t * payload)
{
    ...
    tableIndex = emberAfPluginPriceCommonGetCommonMatchingOrUnusedIndex(
        priceServerInfo.scheduledBlockThresholdsTable.commonInfos[ep], EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE,
    ...
    if ((subpayloadControl & 0x01) == 0x01)
    {
        ...
        payloadIndex++;
        if (payloadIndex + numberOfBt * ZCL_PRICE_CLUSTER_BLOCK_THRESHOLDS_PAYLOAD_SIZE > payloadLength)
        {
            emberAfPriceClusterPrintln("ERR: Invalid number of thresholds(%d) within BlockThresholds payload!", numberOfBt);
            status = false;
            goto kickout;
        }
        ...
        {
            emberAfPriceClusterPrintln("ERR: The number(%d) of block threshold is larger than the max (%d)!", numberOfBt,
        ...
        {
            MEMCOPY(&bt.thresholds.block[i], &payload[payloadIndex + i * ZCL_PRICE_CLUSTER_BLOCK_THRESHOLDS_PAYLOAD_SIZE],
                    ZCL_PRICE_CLUSTER_BLOCK_THRESHOLDS_PAYLOAD_SIZE);
            emberAfPriceClusterPrint("Info: Updating block[%d] = 0x", i);
            emberAfPriceClusterPrintBuffer(&payload[payloadIndex + i * ZCL_PRICE_CLUSTER_BLOCK_THRESHOLDS_PAYLOAD_SIZE],
                                           ZCL_PRICE_CLUSTER_BLOCK_THRESHOLDS_PAYLOAD_SIZE, false);
        ...
        while (payloadIndex < payloadLength)
        {
            ...
            for (i = 0; i < numberOfBt; i++)
            {
                MEMCOPY(&bt.thresholds.blockAndTier[tier][i],
                        ...
                        ZCL_PRICE_CLUSTER_BLOCK_THRESHOLDS_PAYLOAD_SIZE);
                emberAfPriceClusterPrint("Info: Updating blockAndTier[%d][%d] = 0x", tier, i);
                emberAfPriceClusterPrintBuffer(&payload[payloadIndex + i * ZCL_PRICE_CLUSTER_BLOCK_THRESHOLDS_PAYLOAD_SIZE],
                                               ZCL_PRICE_CLUSTER_BLOCK_THRESHOLDS_PAYLOAD_SIZE, false);
                emberAfPriceClusterPrintln("");
            }
...
kickout:
    if (status)
    {
        return emberAfPriceSetBlockThresholdsTableEntry(endpoint, tableIndex, &inf, &bt);
    }
    else
    {
        return false;
                                              ...
                                              const EmberAfScheduledBlockThresholds * bt)
{
    ...
    if (index < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE)
    {
        ...
        MEMCOPY(inf, info, sizeof(EmberAfPriceCommonInfo));
        MEMCOPY(blockThresholds, bt, sizeof(EmberAfScheduledBlockThresholds));
        ...
        emberAfPluginPriceCommonSort((EmberAfPriceCommonInfo *) &priceServerInfo.scheduledBlockThresholdsTable.commonInfos[ep],
                                     ...
                                     sizeof(EmberAfScheduledBlockThresholds), EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE);
        emberAfPriceClusterScheduleTickCallback(endpoint, EMBER_AF_PRICE_SERVER_ACTIVATED_BLOCK_THRESHOLD_EVENT_MASK);
...
void emberAfPricePrintTariff(const EmberAfPriceCommonInfo * info, const EmberAfScheduledTariff * tariff)
{
    ...
    emberAfPriceClusterPrint("\n uom/cur: 0x%X/0x%2X"
                             ...
                             "\r\n st/tt: 0x%4X/0x%X",
                             tariff->unitOfMeasure, tariff->currency, tariff->providerId, info->issuerEventId,
                             tariff->issuerTariffId, info->startTime, tariff->tariffTypeChargingScheme);
    emberAfPriceClusterFlush();
    emberAfPriceClusterPrint("\r\n ptu/btu: 0x%X/0x%X"
                             "\r\n ptd/sc/tbm: 0x%X/0x%4X/0x%X"
                             "\r\n btm/btd: 0x%4X/0x%4X",
                             tariff->numberOfPriceTiersInUse, tariff->numberOfBlockThresholdsInUse, tariff->priceTrailingDigit,
                             tariff->standingCharge, tariff->tierBlockMode, tariff->blockThresholdMultiplier,
...
void emberAfPricePrintPriceMatrix(uint8_t endpoint, const EmberAfPriceCommonInfo * info, const EmberAfScheduledPriceMatrix * pm)
{
    ...
    found = emberAfPriceGetTariffByIssuerTariffId(endpoint, pm->issuerTariffId, &tariffInfo, &t);

    if (!found)
    {
        emberAfPriceClusterPrint("  No corresponding valid tariff found; price matrix cannot be printed.\n");
        emberAfPriceClusterPrint(
            "  (NOTE: If printing from command line, be sure the tariff has been pushed to the tariff table.)\n");
        return;
    }
                                      ...
                                      const EmberAfScheduledBlockThresholds * bt)
{
    ...
    found = emberAfPriceGetTariffByIssuerTariffId(endpoint, bt->issuerTariffId, &newInfo, &t);
    ...
    case 1: // ActiveBlockPriceTier
        ...
        {
            emberAfPriceClusterPrint("  block threshold %d: 0x%x%x%x%x%x%x\r\n", j, bt->thresholds.block[j][0],
                                     bt->thresholds.block[j][1], bt->thresholds.block[j][2], bt->thresholds.block[j][3],
                                     bt->thresholds.block[j][4], bt->thresholds.block[j][5]);
        }
        break;
        ...
        for (i = 0; i < t.numberOfPriceTiersInUse; i++)
        {
            for (j = 0; j < t.numberOfBlockThresholdsInUse; j++)
            {
                emberAfPriceClusterPrint("  tier %d block threshold %d: 0x%x%x%x%x%x%x\r\n", i, j,
                                         bt->thresholds.blockAndTier[i][j][0], bt->thresholds.blockAndTier[i][j][1],
                                         bt->thresholds.blockAndTier[i][j][2], bt->thresholds.blockAndTier[i][j][3],
                                         bt->thresholds.blockAndTier[i][j][4], bt->thresholds.blockAndTier[i][j][5]);
            }
        }
        break;
    case 0xFF: // Not used: in case of TOU tariff or Block tariff charging scheme only
        if ((t.tariffTypeChargingScheme & CHARGING_SCHEME_MASK) == 0x10)
        { // block tariff only
            for (j = 0; j < t.numberOfBlockThresholdsInUse; j++)
            {
                emberAfPriceClusterPrint("  block threshold %d: 0x%x%x%x%x%x%x\r\n", j, bt->thresholds.block[j][0],
        ...
        else if ((t.tariffTypeChargingScheme & CHARGING_SCHEME_MASK) == 0x00)
        { // TOU tariff only
            for (i = 0; i < t.numberOfPriceTiersInUse; i++)
            {
                for (j = 0; j < t.numberOfBlockThresholdsInUse; j++)
...
void emberAfPricePrintTariffTable(uint8_t endpoint)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE; i++)
    {
        emberAfPriceClusterPrintln("= TARIFF %d =", i);
...
void emberAfPricePrintPriceMatrixTable(uint8_t endpoint)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE; i++)
    {
        emberAfPriceClusterPrintln("=PRICE MATRIX %x =", i);
        emberAfPricePrintPriceMatrix(endpoint, &priceServerInfo.scheduledPriceMatrixTable.commonInfos[ep][i],
...
void emberAfPricePrintBlockThresholdsTable(uint8_t endpoint)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE; i++)
    {
        emberAfPriceClusterPrintln("=BLOCK THRESHOLDS %x =", i);
        emberAfPricePrintBlockThresholds(endpoint, &priceServerInfo.scheduledBlockThresholdsTable.commonInfos[ep][i],
                                         &priceServerInfo.scheduledBlockThresholdsTable.scheduledBlockThresholds[ep][i]);
    }
                                                     ...
                                                     uint8_t numberOfCommands, uint8_t tariffType)
{
    ...
    emberAfDebugPrintln("RX: GetTariffInformation, 0x%4X, 0x%4X, 0x%X, 0x%X", earliestStartTime, minIssuerEventId, numberOfCommands,
                        tariffType);
    ...
    entriesCount =
        emberAfPluginPriceCommonFindValidEntries(validCmds, EMBER_AF_PLUGIN_PRICE_SERVER_TARIFF_TABLE_SIZE,
    ...
    // upper nibble is reserved. we'll ignore them.
    {
        ...
        for (i = 0; i < entriesCount; i++)
        {
            ...
            if ((priceServerInfo.scheduledTariffTable.scheduledTariffs[ep][index].tariffTypeChargingScheme & TARIFF_TYPE_MASK) !=
    ...
    sendValidCmdEntries(ZCL_PUBLISH_TARIFF_INFORMATION_COMMAND_ID, ep, validCmds, entriesCount);
...
bool emberAfPriceClusterGetPriceMatrixCallback(uint32_t issuerTariffId)
{
    ...
    uint8_t subPayload[ZCL_PRICE_CLUSTER_MAX_TOU_BLOCKS * ZCL_PRICE_CLUSTER_MAX_TOU_BLOCK_TIERS *
    ...
    found = emberAfPriceGetTariffByIssuerTariffId(endpoint, issuerTariffId, &tariffInfo, &tariff);

    if (!found)
    {
        emberAfDebugPrintln("GetPriceMatrix: no corresponding tariff for id 0x%4x found", issuerTariffId);
        emberAfSendImmediateDefaultResponse(EMBER_ZCL_STATUS_NOT_FOUND);
        return true;
    }

    // Grab the actual price matrix
    found = emberAfPriceGetPriceMatrixByIssuerTariffId(endpoint, issuerTariffId, &pmInfo, &pm);

    if (!found)
    {
        emberAfDebugPrintln("GetPriceMatrix: no corresponding price matrix for id 0x%4x found", issuerTariffId);
        emberAfSendImmediateDefaultResponse(EMBER_ZCL_STATUS_NOT_FOUND);
        return true;
    }
    ...
    case 0: // TOU only
        ...
        for (i = 0; i < tariff.numberOfPriceTiersInUse; i++)
        {
            ...
            emberAfCopyInt32u(subPayload, i * ZCL_PRICE_CLUSTER_PRICE_MATRIX_SUB_PAYLOAD_ENTRY_SIZE + 1, pm.matrix.tier[i]);
    ...
    case 1: // Block only
        ...
        for (i = 0; i < tariff.numberOfPriceTiersInUse; i++)
        {
            ...
            emberAfCopyInt32u(subPayload, i * ZCL_PRICE_CLUSTER_PRICE_MATRIX_SUB_PAYLOAD_ENTRY_SIZE + 1,
    ...
    case 3: // TOU / Block combined
        ...
        for (i = 0; i < tariff.numberOfPriceTiersInUse; i++)
        {
            for (j = 0; j < tariff.numberOfBlockThresholdsInUse + 1; j++)
            {
                ...
                emberAfCopyInt32u(subPayload, size + 1, pm.matrix.blockAndTier[i][j]);
                size += ZCL_PRICE_CLUSTER_PRICE_MATRIX_SUB_PAYLOAD_ENTRY_SIZE;
            }
        }
        break;
    ...
    emberAfFillCommandPriceClusterPublishPriceMatrix(pm.providerId, pmInfo.issuerEventId, pmInfo.startTime, pm.issuerTariffId, 0, 1,
                                                     payloadControl, subPayload, size);
    emberAfSendResponse();
...
bool emberAfPriceClusterGetBlockThresholdsCallback(uint32_t issuerTariffId)
{
    ...
    if (!found)
    {
        emberAfDebugPrintln("GetBlockThresholds: no corresponding tariff for id 0x%4x found", issuerTariffId);
        emberAfSendImmediateDefaultResponse(EMBER_ZCL_STATUS_NOT_FOUND);
        return true;
    }

    // Grab the actual block thresholds
    found = emberAfPriceGetBlockThresholdsByIssuerTariffId(endpoint, issuerTariffId, &btInfo, &bt);

    if (!found)
    {
        emberAfDebugPrintln("GetBlockThresholds: no corresponding block thresholds for id 0x%4x found", issuerTariffId);
        emberAfSendImmediateDefaultResponse(EMBER_ZCL_STATUS_NOT_FOUND);
        return true;
    }
    ...
    emberAfFillExternalBuffer((ZCL_CLUSTER_SPECIFIC_COMMAND | ZCL_FRAME_CONTROL_SERVER_TO_CLIENT), ZCL_PRICE_CLUSTER_ID,
                              ZCL_PUBLISH_BLOCK_THRESHOLDS_COMMAND_ID, "wwwwuu", bt.providerId, btInfo.issuerEventId,
                              btInfo.startTime, bt.issuerTariffId, 0, 1);
    ...
    case 2:                       // ActiveBlockPriceTierThreshold
        ...
        break;
    case 0xFF: // Not used: in case of TOU tariff or Block tariff charging scheme only
        if ((tariff.tariffTypeChargingScheme & CHARGING_SCHEME_MASK) == 0x10)
        {                             // block tariff only
            emberAfPutInt8uInResp(1); // payload control
            emberAfPutInt8uInResp(tariff.numberOfBlockThresholdsInUse);
            size += 1;
        ...
        else if ((tariff.tariffTypeChargingScheme & CHARGING_SCHEME_MASK) == 0x00)
        {                             // TOU tariff only
            emberAfPutInt8uInResp(0); // payload control
            for (i = 0; i < tariff.numberOfPriceTiersInUse; i++)
            {
