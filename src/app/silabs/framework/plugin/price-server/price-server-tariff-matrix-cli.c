...
void emAfPriceServerCliTWho(void)
{
    ...
    length = emberCopyStringArgument(1, scheduledTariff.tariffLabel + 1, ZCL_PRICE_CLUSTER_MAXIMUM_RATE_LABEL_LENGTH, false);
...
void emAfPriceServerCliTAdd(void)
{
    ...
    if (!emberAfPriceAddTariffTableEntry(endpoint, &scheduledTariffInfo, &scheduledTariff))
    {
...
void emAfPriceServerCliTGet(void)
{
    ...
    if (!emberAfPriceGetTariffTableEntry(endpoint, index, &scheduledTariffInfo, &scheduledTariff))
    {
...
void emAfPriceServerCliTSet(void)
{
    ...
    if (!emberAfPriceSetTariffTableEntry(endpoint, index, &scheduledTariffInfo, &scheduledTariff))
    {
        emberAfPriceClusterPrintln("tariff entry %d not present", index);
    }
}
...
void emAfPriceServerCliPmSetMetadata(void)
{
    ...
    if (!(emberAfPriceGetTariffByIssuerTariffId(endpoint, tariffId, &i, &t) || scheduledTariff.issuerTariffId == tariffId))
    {
...
void emAfPriceServerCliPmPut(void)
{
    ...
    if (tier >= t.numberOfPriceTiersInUse || block > t.numberOfBlockThresholdsInUse)
    {
