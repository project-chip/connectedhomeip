...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This file calls ECC curve specific functions
                                                                               *from the cbke library based on whether the 163k1 ECC
                                                                               *curve or the 283k1 ECC curve is being used for key
                                                                               *establishment.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
bool storePublicPartnerData(bool isCertificate, uint8_t * data)
{
    if (isCbkeKeyEstablishmentSuite163k1())
    {
                                       ...
                                       EmberPublicKey283k1Data * partnerEphemeralPublicKey)
{
    if (isCbkeKeyEstablishmentSuite163k1())
    {
        return retrieveAndClearPublicPartnerData163k1(
            (EmberCertificateData *) emberCertificate283k1Contents(partnerCertificate),
...
EmberAfKeyEstablishmentNotifyMessage sendCertificate(void)
{
    if (isCbkeKeyEstablishmentSuite163k1())
    {
        ...
        sendNextKeyEstablishMessage(ZCL_INITIATE_KEY_ESTABLISHMENT_REQUEST_COMMAND_ID, emberCertificateContents(&localCert));
    }
    else if (isCbkeKeyEstablishmentSuite283k1())
    {
        EmberCertificate283k1Data localCert;
        ...
        sendNextKeyEstablishMessage(ZCL_INITIATE_KEY_ESTABLISHMENT_REQUEST_COMMAND_ID, emberCertificate283k1Contents(&localCert));
    }
    else
    {
...
EmberStatus emCalculateSmacsForCurve(bool amInitiator, EmberCertificate283k1Data * partnerCert,
                                     EmberPublicKey283k1Data * partnerEphemeralPublicKey)
{
    EmberStatus status;
    ...
    if (isCbkeKeyEstablishmentSuite163k1())
    {
        status = emberCalculateSmacs(amInitiator, (EmberCertificateData *) emberCertificate283k1Contents(partnerCert),
                                     (EmberPublicKeyData *) emberPublicKey283k1Contents(partnerEphemeralPublicKey));
    }
    else if (isCbkeKeyEstablishmentSuite283k1())
    {
        status = emberCalculateSmacs283k1(amInitiator, partnerCert, partnerEphemeralPublicKey);
    }
    else
    {
        status = EMBER_ERR_FATAL;
