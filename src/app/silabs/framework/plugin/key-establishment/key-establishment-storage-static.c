...
{
    uint8_t * ptr = (isCertificate ? emberCertificateContents(&partnerCert) : emberPublicKeyContents(&partnerPublicKey));
    uint8_t size  = (isCertificate ? EMBER_CERTIFICATE_SIZE : EMBER_PUBLIC_KEY_SIZE);
                                            ...
                                            EmberPublicKeyData * partnerEphemeralPublicKey)
{
    ...
    {
        MEMMOVE(partnerCertificate, &partnerCert, EMBER_CERTIFICATE_SIZE);
    }
    if (partnerEphemeralPublicKey != NULL)
    {
        MEMMOVE(partnerEphemeralPublicKey, &partnerPublicKey, EMBER_PUBLIC_KEY_SIZE);
    }
...
{
    uint8_t * ptr =
        (isCertificate ? emberCertificate283k1Contents(&partnerCert283k1) : emberPublicKey283k1Contents(&partnerPublicKey283k1));
    uint8_t size = (isCertificate ? EMBER_CERTIFICATE_283K1_SIZE : EMBER_PUBLIC_KEY_283K1_SIZE);
    MEMMOVE(ptr, data, size);
    return true;
}
                                            ...
                                            EmberPublicKey283k1Data * partnerEphemeralPublicKey)
{
    if (partnerCertificate != NULL)
    {
        MEMMOVE(partnerCertificate, &partnerCert283k1, EMBER_CERTIFICATE_283K1_SIZE);
    }
    if (partnerEphemeralPublicKey != NULL)
    {
        MEMMOVE(partnerEphemeralPublicKey, &partnerPublicKey283k1, EMBER_PUBLIC_KEY_283K1_SIZE);
    }
