...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Routines for the Key Establishment plugin,
                                                                               *which handles establishment of link keys per the
                                                                               *Smart Energy Key Establishment cluster.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
void emberAfPluginKeyEstablishmentApsDuplicateDetectionEventHandler(EmberEventControl * control);
...
#define EMBER_AF_CUSTOM_KE_EPHEMERAL_DATA_GENERATE_TIME_SECONDS DEFAULT_EPHEMERAL_DATA_GENERATE_TIME_SECONDS
#endif

#if !defined(EMBER_AF_CUSTOM_KE_GENERATE_SHARED_SECRET_TIME_SECONDS)
#define EMBER_AF_CUSTOM_KE_GENERATE_SHARED_SECRET_TIME_SECONDS DEFAULT_GENERATE_SHARED_SECRET_TIME_SECONDS
#endif
...
const uint8_t emAfKeyEstablishMessageToDataSize[] = {
    EM_AF_KE_INITIATE_SIZE,       EM_AF_KE_EPHEMERAL_SIZE,       EM_AF_KE_SMAC_SIZE, EM_AF_KE_TERMINATE_SIZE,
    EM_AF_KE_INITIATE_SIZE_283K1, EM_AF_KE_EPHEMERAL_SIZE_283K1, EM_AF_KE_SMAC_SIZE, EM_AF_KE_TERMINATE_SIZE,
};
...
#define sendEphemeralData(publicKey) (sendNextKeyEstablishMessage(ZCL_EPHEMERAL_DATA_RESPONSE_COMMAND_ID, (publicKey)))
#define sendConfirmKey(smac) (sendNextKeyEstablishMessage(ZCL_CONFIRM_KEY_DATA_REQUEST_COMMAND_ID, (smac)))
...
static void keyEstablishStateMachine(KeyEstablishEvent newEvent, uint8_t * data1, uint8_t * data2);
static void clearKeyEstablishmentState(void);
static bool askApplicationWithDelay(EmberAfKeyEstablishmentNotifyMessage message, uint8_t delayInSec);
...
static void sendTerminateMessage(EmberAfAmiKeyEstablishmentStatus status, uint8_t backOffTime);
...
static void setupEventTimeouts(uint8_t theirGenerateKeyTimeSeconds, uint8_t theirConfirmKeyTimeSeconds);
static bool commandReceivedHandler(EmberAfClusterCommand * cmd);
static void messageSentHandler(EmberOutgoingMessageType type, uint16_t indexOrDestination, EmberApsFrame * apsFrame,
...
static void checkSupportedCurve(void)
{
    ...
    emberAfFillCommandGlobalClientToServerReadAttributes(ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, &attributeIds, sizeof(attributeIds));
...
static bool checkValidByteValue283k1(uint8_t byte, uint8_t requiredValue, uint8_t mask)
{
    if (isCbkeKeyEstablishmentSuite283k1())
    {
        ...
        {
            emberAfKeyEstablishmentClusterPrintln("Error: Invalid byte in cert: byteValue %u requiredValue %u", byte,
                                                  requiredValue);
...
static bool checkKeyTable(uint8_t * bigEndianEui64)
{
    ...
    if (keyEstPartner.isIntraPan && keyEstPartner.pan.intraPan.nodeId == EMBER_TRUST_CENTER_NODE_ID)
    {
        if (emberGetKey(EMBER_TRUST_CENTER_LINK_KEY, &keyStruct) == EMBER_SUCCESS &&
            (keyStruct.bitmask & EMBER_KEY_HAS_PARTNER_EUI64) && MEMCOMPARE(eui64, keyStruct.partnerEUI64, EUI64_SIZE) == 0)
        {
    ...
    else
    {
        ...
        else if (0xFF != emberFindKeyTableEntry((uint8_t *) emberAfNullEui64, true))
        {
            return true;
        }
        else
...
static void keyEstablishStateMachine(KeyEstablishEvent newEvent, uint8_t * data1, uint8_t * data2)
{
    ...
    emberAfKeyEstablishmentClusterPrintln("Key Establish State Machine event:%d", newEvent);
    ...
    if (cmd != NULL)
    {
        ...
        if ((cmd->interPanHeader == NULL) && (lastNodeSuccessfulCbke == cmd->source) &&
            (lastConfirmKeyDataResponseApsSequence == cmd->apsFrame->sequence) && (newEvent == INITIATOR_RECEIVED_CONFIRM_KEY))
        {
        ...
        else if (commandIsFromOurPartner(cmd) && (keyEstPartner.isInitiator || lastEvent != NO_KEY_ESTABLISHMENT_EVENT))
        {
        ...
        else if (!isCbkeKeyEstablishmentSuiteValid() || (lastEvent != NO_KEY_ESTABLISHMENT_EVENT))
        {
            ...
            if (setPartnerFromCommand(cmd))
            {
                ...
                sendTerminateMessage(EMBER_ZCL_AMI_KEY_ESTABLISHMENT_STATUS_NO_RESOURCES, BACK_OFF_TIME_REPORTED_TO_PARTNER);
            }
    ...
    if (newEvent != lastEvent + 1)
    {
        if (keyEstPartner.isInitiator && lastEvent == NO_KEY_ESTABLISHMENT_EVENT)
        {
            ...
            {
                emberAfKeyEstablishmentClusterPrintln("Got wrong message in the sequence. last event %u, new event %u", lastEvent,
                                                      newEvent);
    ...
    case BEGIN_KEY_ESTABLISHMENT: {
        ...
        if (keyEstPartner.isInitiator)
        {
            if (!checkRequestedSuite(data1) || !checkIssuer(data2 + certIssuerOffset) ||
            ...
            {
                setupEventTimeouts(data1[EPHEMERAL_DATA_TIME_OFFSET], data1[CONFIRM_KEY_TIME_OFFSET]);
            }
        }
    ...
    case GENERATE_KEYS:
        ...
        if (!askApplication(GENERATING_EPHEMERAL_KEYS) ||
            !storePublicPartnerData(!keyEstPartner.isInitiator,           // certificate?
                                    (!keyEstPartner.isInitiator ? data2   // partner cert
                                                                : data1)) // partner key
    ...
    case GENERATE_SHARED_SECRET: {
        ...
        if (!askApplication(GENERATING_SHARED_SECRET) ||
            (!keyEstPartner.isInitiator ? !storePublicPartnerData(false, data1) : !storeSmac((EmberSmacData *) data1)) ||
            !retrieveAndClearPublicPartnerData(&partnerCert, &partnerEphemeralPublicKey) ||
            (EMBER_OPERATION_IN_PROGRESS !=
             emCalculateSmacsForCurve(!keyEstPartner.isInitiator, &partnerCert, &partnerEphemeralPublicKey)))
        {
            cleanupAndStop(NO_LOCAL_RESOURCES);
            return;
        }
    ...
    case SEND_CONFIRM_KEY_MESSAGE: {
        ...
        if (!askApplication(GENERATE_SHARED_SECRET_DONE) || (!keyEstPartner.isInitiator && !storeSmac(responderSmac)))
        {
            cleanupAndStop(NO_LOCAL_RESOURCES);
            return;
        }
        ...
        sendConfirmKey(emberSmacContents(!keyEstPartner.isInitiator ? initiatorSmac : responderSmac));
    ...
    case INITIATOR_RECEIVED_CONFIRM_KEY: {
        ...
        if (keyEstPartner.isIntraPan)
        {
            ...
            emberEventControlSetDelayMS(emberAfPluginKeyEstablishmentApsDuplicateDetectionEventControl,
                                        KEY_ESTABLISHMENT_APS_DUPLICATE_DETECTION_TIMEOUT_SEC << 10);
        }

        // For both success and failure, stop key establishment.
        cleanupAndStop(message == NO_APP_MESSAGE ? LINK_KEY_ESTABLISHED : message);
...

    {
        ...
        emberAfScheduleServerTick(keyEstablishmentEndpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, timeMs);
    }
    lastEvent = newEvent;
    return;
}
...
static void clearKeyEstablishmentState(void)
{
    ...
    emberAfDeactivateServerTick(keyEstablishmentEndpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID);
...
static bool askApplicationWithDelay(EmberAfKeyEstablishmentNotifyMessage message, uint8_t delayInSec)
{
    ...
    emberAfKeyEstablishmentClusterPrint(
        "%p: %p %p: %p (%d), %p ", (!keyEstPartner.isInitiator ? "Initiator" : "Responder"), "Key Establish",
        (message >= APP_NOTIFY_ERROR_CODE_START ? "Error" : (message == LINK_KEY_ESTABLISHED ? "Success" : "Notify")),
        appNotifyText[message], message, "keyEstPartner");
    if (keyEstPartner.isIntraPan)
    {
        emberAfKeyEstablishmentClusterPrintln("0x%2x", keyEstPartner.pan.intraPan.nodeId);
        return emberAfKeyEstablishmentCallback(message, !keyEstPartner.isInitiator, keyEstPartner.pan.intraPan.nodeId, delayInSec);
    }
    else
    {
        emberAfKeyEstablishmentClusterDebugExec(emberAfPrintBigEndianEui64(keyEstPartner.pan.interPan.eui64));
        emberAfKeyEstablishmentClusterPrintln("");
        return emberAfInterPanKeyEstablishmentCallback(message, !keyEstPartner.isInitiator, keyEstPartner.pan.interPan.panId,
                                                       keyEstPartner.pan.interPan.eui64, delayInSec);
    }
}
...
static void sendKeyEstablishMessage(KeyEstablishMessage message)
{
    ...
    *ptr++        = (ZCL_CLUSTER_SPECIFIC_COMMAND |
              (!keyEstPartner.isInitiator ? ZCL_FRAME_CONTROL_CLIENT_TO_SERVER : ZCL_FRAME_CONTROL_SERVER_TO_CLIENT));
    ...
    *ptr          = message;
    ...
    if (keyEstPartner.isIntraPan)
    {
        ...
        emberAfSendUnicast(EMBER_OUTGOING_DIRECT, keyEstPartner.pan.intraPan.nodeId, &apsFrame, appResponseLength, appResponseData);
    }
    else
    {
        emberAfSendInterPan(keyEstPartner.pan.interPan.panId, keyEstPartner.pan.interPan.eui64, EMBER_NULL_NODE_ID,
                            0, // multicast id - unused
                            ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, SE_PROFILE_ID, appResponseLength, appResponseData);
    }
}
...
static EmberAfKeyEstablishmentNotifyMessage verifySmac(EmberSmacData * otherSmac)
{
    ...
    return (MEMCOMPARE(emberSmacContents(ptr), otherSmac, EMBER_SMAC_SIZE) == 0 ? NO_APP_MESSAGE // success
                                                                                : KEY_CONFIRM_FAILURE);
}
...
static void setupEventTimeouts(uint8_t theirGenerateKeyTimeSeconds, uint8_t theirConfirmKeyTimeSeconds)
{
    ...
    eventTimeoutsSec[BEGIN_KEY_ESTABLISHMENT] += (!keyEstPartner.isInitiator ? 0 : theirGenerateKeyTimeSeconds);
    ...
    eventTimeoutsSec[SEND_EPHEMERAL_DATA_MESSAGE] +=
        (!keyEstPartner.isInitiator ? theirGenerateKeyTimeSeconds : theirConfirmKeyTimeSeconds);
...
static bool commandReceivedHandler(EmberAfClusterCommand * cmd)
{
    ...
    EmberAfStatus status =
        (cmd->direction == ZCL_DIRECTION_CLIENT_TO_SERVER ? emberAfKeyEstablishmentClusterServerCommandParse(cmd)
                                                          : emberAfKeyEstablishmentClusterClientCommandParse(cmd));
    if (status == EMBER_ZCL_STATUS_MALFORMED_COMMAND)
    {
        emberAfKeyEstablishmentClusterPrintln("Invalid length for KE command: %d (expected: %d, actual: %d)", cmd->commandId,
...
static void messageSentHandler(EmberOutgoingMessageType type, uint16_t indexOrDestination, EmberApsFrame * apsFrame,
                               uint16_t msgLen, uint8_t * message, EmberStatus status)
{
    if (!keyEstPartner.isIntraPan || lastEvent == NO_KEY_ESTABLISHMENT_EVENT ||
...
static bool setPartnerFromCommand(const EmberAfClusterCommand * cmd)
{
    ...
    {
        if (!(cmd->interPanHeader->options & EMBER_AF_INTERPAN_OPTION_MAC_HAS_LONG_ADDRESS))
        {
            return false;
        }
        keyEstPartner.pan.interPan.panId = cmd->interPanHeader->panId;
        MEMMOVE(keyEstPartner.pan.interPan.eui64, cmd->interPanHeader->longAddress, EUI64_SIZE);
...
static bool commandIsFromOurPartner(const EmberAfClusterCommand * cmd)
{
    ...
    return (((keyEstPartner.isInitiator == (cmd->direction == ZCL_DIRECTION_CLIENT_TO_SERVER)) ||
             (cmd->commandId == ZCL_TERMINATE_KEY_ESTABLISHMENT_COMMAND_ID)) &&
            keyEstPartner.isIntraPan == (cmd->interPanHeader == NULL) &&
            (keyEstPartner.isIntraPan
                 ? keyEstPartner.pan.intraPan.nodeId == cmd->source
                 : ((cmd->interPanHeader->options & EMBER_AF_INTERPAN_OPTION_MAC_HAS_LONG_ADDRESS) &&
                    (MEMCOMPARE(keyEstPartner.pan.interPan.eui64, cmd->interPanHeader->longAddress, EUI64_SIZE) == 0))));
}
...
static void writeKeyEstablishmentClusterAttribute(uint8_t endpoint)
{
    if (isCbkeKeyEstablishmentSuiteValid())
    {
        ...
        {
            attrWriteStatus = emberAfWriteAttribute(endpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID,
                                                    // Conveniently the server and client attribute
                                                    // ID are the same.  So we pass just the server ID.
                                                    ZCL_KEY_ESTABLISHMENT_SUITE_SERVER_ATTRIBUTE_ID, (uint8_t) mask,
                                                    (uint8_t *) &keSuiteId, ZCL_ENUM16_ATTRIBUTE_TYPE);
            if (attrWriteStatus != EMBER_ZCL_STATUS_SUCCESS)
            {
                emberAfKeyEstablishmentClusterPrintln("%p%p%p%X", "Key Est. Suite attr write failed (",
                                                      ((mask == CLUSTER_MASK_SERVER) ? "server" : "client"), "): 0x",
                                                      attrWriteStatus);
            }
        }
    }
...
static void debugPrintSmac(bool initiatorSmac, uint8_t * smac)
{
    emberAfKeyEstablishmentClusterPrintln("%p SMAC", (initiatorSmac ? "Initiator" : "Responder"));
...
static void debugPrintOtherSmac(bool received, uint8_t * smac)
{
    emberAfKeyEstablishmentClusterPrintln("%p SMAC", (received ? "Received" : "Calculated"));
    // Conveniently Zigbee Keys are the same length as the SMAC
    emberAfPrintZigbeeKey(smac);
}
...
static void debugPrintCert(bool initiatorCert, uint8_t * cert)
{
    emberAfKeyEstablishmentClusterPrintln("%p Cert", (initiatorCert ? "Initiator" : "Responder"));
...
static void debugPrintKey(bool initiatorKey, uint8_t * key)
{
    emberAfKeyEstablishmentClusterPrintln("%p public key", (initiatorKey ? "Initiator" : "Responder"));
    if (isCbkeKeyEstablishmentSuite163k1())
    {
...
static EmberStatus initiateKeyEstablishment(const EmberEUI64 eui64, uint16_t nodeIdOrPanId, uint8_t endpoint)
{
    ...
    if (isCbkeKeyEstablishmentSuiteValid() && lastEvent == NO_KEY_ESTABLISHMENT_EVENT)
    {
        ...
        if (!keyEstPartner.isIntraPan)
        {
            ...
            emberAfKeyEstablishmentClusterPrintln("Skipping read attributes of supported ECC curves for inter-PAN"
                                                  " (using %sk1)",
            ...
            return ((lastEvent == validLastEvent) ? EMBER_ERR_FATAL : EMBER_SUCCESS);
        }
...
{
    EmberAfClusterCommand * cmd = emberAfCurrentCommand();
    uint8_t validSize           = isCertificate ? INITIATE_KEY_ESTABLISHMENT_LENGTH_283K1 : EMBER_PUBLIC_KEY_283K1_SIZE;
    if (isCbkeKeyEstablishmentSuite283k1())
    {
        emberAfKeyEstablishmentClusterPrintln("Checking size Certificate size %u, expected %u",
                                              cmd->bufLen - cmd->payloadStartIndex, validSize);
...
static EmberAfStatus validateNewCbkeSuite(uint8_t endpoint, uint16_t newCbkeSuite)
{
...
void cleanupAndStopWithDelay(EmberAfKeyEstablishmentNotifyMessage message, uint8_t delayInSec)
{
    ...
    scriptTestCheckpoint("CBKE: link key %s", linkKeyEstablished ? "established" : "failure");
    ...
    emberAfKeyEstablishmentClusterFlush();
    emberAfKeyEstablishmentClusterPrintln("End Key Establishment Status: 0x%x, Store Link Key Status: 0x%x", message,
                                          storeLinkKeyStatus);
    ...
    else if (status != EMBER_ZCL_AMI_KEY_ESTABLISHMENT_STATUS_SUCCESS && message != PARTNER_SENT_DEFAULT_RESPONSE_ERROR &&
             message != BAD_KEY_ESTABLISHMENT_SUITE && (keyEstPartner.isInitiator || lastEvent != NO_KEY_ESTABLISHMENT_EVENT))
    {
...
EmberAfStatus emberAfKeyEstablishmentClusterClientPreAttributeChangedCallback(uint8_t endpoint, EmberAfAttributeId attributeId,
                                                                              ...
                                                                              uint8_t * value)
{
    return emberAfKeyEstablishmentClusterServerPreAttributeChangedCallback(endpoint, attributeId, attributeType, size, value);
}
...
void emberAfKeyEstablishmentClusterClientAttributeChangedCallback(uint8_t endpoint, EmberAfAttributeId attributeId)
{
    ...
    EmberAfStatus status = emberAfReadClientAttribute(endpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, attributeId,
                                                      (uint8_t *) &availableSuites, sizeof(availableSuites));
    ...
    status = emberAfReadServerAttribute(endpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, attributeId, (uint8_t *) &serverSuites,
                                        sizeof(serverSuites));
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
    ...
    {
        status = emberAfWriteServerAttribute(endpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, attributeId, (uint8_t *) &availableSuites,
...
void emberAfKeyEstablishmentClusterServerAttributeChangedCallback(uint8_t endpoint, EmberAfAttributeId attributeId)
{
    ...
    uint16_t availableSuites;
    EmberAfStatus status = emberAfReadServerAttribute(endpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, attributeId,
                                                      (uint8_t *) &availableSuites, sizeof(availableSuites));
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
    ...
    status = emberAfReadClientAttribute(endpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, attributeId, (uint8_t *) &clientSuites,
                                        sizeof(clientSuites));
    ...
    {
        status = emberAfWriteClientAttribute(endpoint, ZCL_KEY_ESTABLISHMENT_CLUSTER_ID, attributeId, (uint8_t *) &availableSuites,
                                             ZCL_ENUM16_ATTRIBUTE_TYPE);
        if (status != EMBER_ZCL_STATUS_SUCCESS)
        {
...
void emberAfPluginKeyEstablishmentReadAttributesCallback(EmberAfCbkeKeyEstablishmentSuite suite)
{
    ...
    emberAfKeyEstablishmentClusterPrintln("keyEstPartner.isInitiator emberAfPluginKeyEstablishmentReadAttributesCallback %u",
                                          keyEstPartner.isInitiator);
    if (!keyEstPartner.isInitiator)
    {
        // We identify the highest available suite to use.
        if ((suite & emAfAvailableCbkeSuite) >= EMBER_AF_CBKE_KEY_ESTABLISHMENT_SUITE_283K1)
        {
        ...
        else if ((suite & emAfAvailableCbkeSuite) <= EMBER_AF_CBKE_KEY_ESTABLISHMENT_SUITE_163K1)
        {
...
{
    return initiateKeyEstablishment(eui64, panId,
                                    0); // endpoint - unused
}
...
void emberAfKeyEstablishmentClusterServerInitCallback(uint8_t endpoint)
{
    ...
    emberAfCorePrintln("Key Est. Init %p 0x%x", (isCbkeKeyEstablishmentSuiteValid() ? "Success" : "FAILED!"),
                       LOW_BYTE(emAfAvailableCbkeSuite));
    emberAfKeyEstablishmentClusterPrintln("Key Est. Init %p", (isCbkeKeyEstablishmentSuiteValid() ? "Success" : "FAILED!"));
                                                                           ...
                                                                           uint8_t confirmKeyGenerateTime, uint8_t * identity)
{
    ...
    keyEstablishStateMachine(BEGIN_KEY_ESTABLISHMENT, cmd->buffer + cmd->payloadStartIndex, identity);
    return true;
}
...
{
    keyEstablishStateMachine(GENERATE_SHARED_SECRET, secureMessageAuthenticationCode, NULL);
    return true;
}
                                                                     ...
                                                                     uint16_t keyEstablishmentSuite)
{
    ...
    if (lastEvent != NO_KEY_ESTABLISHMENT_EVENT && cmd != NULL && commandIsFromOurPartner(cmd))
    {
        ...
        emberAfKeyEstablishmentClusterPrintln(
            "Terminate Received, Status(%d): %p", statusCode,
            terminateStatus[((statusCode > UNKNOWN_TERMINATE_STATUS) ? UNKNOWN_TERMINATE_STATUS : statusCode)]);
...
static void defaultResponseCallback(uint8_t endpoint, uint8_t commandId, EmberAfStatus status)
{
    EmberAfClusterCommand * cmd = emberAfCurrentCommand();
    ...
    if (status != EMBER_ZCL_STATUS_SUCCESS && lastEvent != NO_KEY_ESTABLISHMENT_EVENT && cmd != NULL &&
        commandIsFromOurPartner(cmd))
    {
        emberAfKeyEstablishmentClusterPrintln("Got Default Response with error code: %d", status);
                                                                            ...
                                                                            uint8_t confirmKeyGenerateTime, uint8_t * identity)
{
    ...
    keyEstablishStateMachine(GENERATE_KEYS, cmd->buffer + cmd->payloadStartIndex, identity);
    return true;
}
...
{
    keyEstablishStateMachine(INITIATOR_RECEIVED_CONFIRM_KEY, secureMessageAuthenticationCode, NULL);
    return true;
}
...
void emAfPluginKeyEstablishmentGenerateCbkeKeysHandler(EmberStatus status, EmberPublicKeyData * ephemeralPublicKey)
{
    emberAfKeyEstablishmentClusterPrintln("GenerateCbkeKeysHandler() returned: 0x%x", status);
    ...
    if (emAfKeyEstablishmentTestHarnessCbkeCallback(CBKE_OPERATION_GENERATE_KEYS, ephemeralPublicKey->contents, NULL))
    {
        return;
    }

    emberAfPushEndpointNetworkIndex(keyEstablishmentEndpoint);
    keyEstablishStateMachine(SEND_EPHEMERAL_DATA_MESSAGE, emberPublicKeyContents(ephemeralPublicKey), NULL);
    emberAfPopNetworkIndex();
}

void emAfPluginKeyEstablishmentCalculateSmacsHandler(EmberStatus status, EmberSmacData * initiatorSmac,
                                                     EmberSmacData * responderSmac)
{
    emberAfKeyEstablishmentClusterPrintln("CalculateSmacsHandler() returned: 0x%x", status);
    emAfCryptoOperationComplete();
    debugPrintSmac(true, emberSmacContents(initiatorSmac));
    debugPrintSmac(false, emberSmacContents(responderSmac));
    ...
    if (emAfKeyEstablishmentTestHarnessCbkeCallback(CBKE_OPERATION_GENERATE_SECRET, initiatorSmac->contents,
    ...
    emberAfPushEndpointNetworkIndex(keyEstablishmentEndpoint);
    keyEstablishStateMachine(SEND_CONFIRM_KEY_MESSAGE, emberKeyContents(initiatorSmac), emberKeyContents(responderSmac));
    emberAfPopNetworkIndex();
}

void emAfPluginKeyEstablishmentGenerateCbkeKeysHandler283k1(EmberStatus status, EmberPublicKey283k1Data * ephemeralPublicKey)
{
    emberAfKeyEstablishmentClusterPrintln("GenerateCbkeKeysHandler283k1() returned: 0x%x", status);
    emAfCryptoOperationComplete();
    ...
    emberAfPushEndpointNetworkIndex(keyEstablishmentEndpoint);
    keyEstablishStateMachine(SEND_EPHEMERAL_DATA_MESSAGE, emberPublicKey283k1Contents(ephemeralPublicKey), NULL);
    emberAfPopNetworkIndex();
}

void emAfPluginKeyEstablishmentCalculateSmacsHandler283k1(EmberStatus status, EmberSmacData * initiatorSmac,
                                                          EmberSmacData * responderSmac)
{
    emberAfKeyEstablishmentClusterPrintln("CalculateSmacsHandler() returned: 0x%x", status);
    emAfCryptoOperationComplete();
