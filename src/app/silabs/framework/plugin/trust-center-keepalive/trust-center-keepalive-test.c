...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Test code for the Trust Center Keepalive
                                                                               *plugin.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static uint8_t readAttributeRequestMessage[] = { 0, // frame control
                                                 TEST_ZCL_SEQUENCE,
                                                  ...
                                                  bool serverCluster, EmberAfServiceDiscoveryCallback * callback)
{
    ...
    if (profileId != 0xFFFF)
    {
        expectComparisonHex(serviceDiscoveryParameters.profileId, profileId, "Expected Profile ID", "Actual Profile ID");
    }
    expectComparisonHex(serviceDiscoveryParameters.clusterId, clusterId, "Expected Cluster ID", "Actual Cluster ID");
    expectComparisonDecimal(serviceDiscoveryParameters.serverCluster, serverCluster, "Expected Server Cluster",
                            "Actual Server Cluster");
    expectComparisonHex(serviceDiscoveryParameters.target, target, "Expected Node ID", "Target Node ID");
...
static void findAndQueryTest(void)
{
    ...
    expectComparisonDecimal(
        emberAfPluginTrustCenterKeepaliveTickNetworkEventControls[EM_AF_DEFAULT_NETWORK_INDEX_FOR_UNIT_TESTS].status,
        EMBER_EVENT_MS_TIME, "Event Status", "EMBER_EVENT_MS_TIME");
    ...
    EmberAfServiceDiscoveryResult result = {
        EMBER_AF_UNICAST_SERVICE_DISCOVERY_TIMEOUT, serviceDiscoveryParameters.clusterId, serviceDiscoveryParameters.target,
        NULL, // response data
    };
    ...
    serviceDiscoveryParameters.callback(&result);
    verifyMessage("Read Attributes Request", sizeof(readAttributeRequestMessage), readAttributeRequestMessage);
}
...
int main(int argc, char * argv[])
{
    ...
    return parseCommandLineAndExecuteTest(argc, argv, "af-trust-center-keepalive-test", allTests);
}
