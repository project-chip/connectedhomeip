...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief CLI for the Partner Link Key Exchange
                                                                               *plugin.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
extern EzspStatus emberAfSetEzspPolicy(EzspPolicyId policyId, EzspDecisionId decisionId, const char * policyName,
                                       const char * decisionName);
#endif

extern void emAfPrintStatus(const char * task, EmberStatus status);
...
void cbkeAllowPartner(void)
{
    ...
    emberAfSetEzspPolicy(EZSP_APP_KEY_REQUEST_POLICY,
                         (emAfAllowPartnerLinkKey ? EZSP_ALLOW_APP_KEY_REQUESTS : EZSP_DENY_APP_KEY_REQUESTS),
                         "App Link Key Request Policy", (emAfAllowPartnerLinkKey ? "Allow" : "Deny"));
...
void cbkePartnerCommand(void)
{
    ...
    EmberStatus status = emberAfInitiatePartnerLinkKeyExchange(target, endpoint,
                                                               NULL); // callback
