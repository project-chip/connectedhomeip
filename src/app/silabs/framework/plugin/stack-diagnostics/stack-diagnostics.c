...
static const char * nodeTypeStrings[] = {
    "Unknown", "Coordinator", "Router", "End Device", "Sleep End Device",
};
...
void printChildTable(void)
{
    ...
    uint8_t i;
    const char * types[] = {
        "Unknown", "Coordin", "Router ", "RxOn   ", "Sleepy ", "???    ",
    };
    uint8_t used                = 0;
    ...
    emberAfAppPrintln("\n%d of %d entries used.", used, emberAfGetChildTableSize());
}
...
#endif // RECORD_LQI_RSSI
                        "0x%4X  ",
                        i, n.shortId, n.averageLqi, n.inCost, n.outCost, n.age,
#ifdef RECORD_LQI_RSSI
                        neighborRssiTable[i], neighborLqiTable[i],
#endif // RECORD_LQI_RSSI
                        returnFrameCounter);
...
void printRouteTable(void)
{
    ...
    {
        if (EMBER_SUCCESS != emberGetRouteTableEntry(i, &entry) || entry.destination == EMBER_NULL_NODE_ID)
        {
            continue;
        }
        used++;
        emberAfAppPrintln("%d: 0x%2X  0x%2X  %d   %p  %p", i, entry.destination, entry.nextHop, entry.age,
                          concentratorNames[entry.concentratorType], zigbeeRouteStatusNames[entry.status]);
    }
...
void printInfo(void)
{
    ...
    emberAfAppPrintln("Configured Node Type (%d): %p", emAfCurrentZigbeeProNetwork->nodeType,
                      nodeTypeStrings[emAfCurrentZigbeeProNetwork->nodeType]);
    emberAfAppPrintln("Running Node Type    (%d): %p\n", type, nodeTypeStrings[type]);
    ...
    else if (rejoinReason >= EMBER_AF_REJOIN_FIRST_REASON && rejoinReason <= EMBER_AF_REJOIN_LAST_REASON)
    {
    ...
    emberAfAppPrintln("Last Rejoin Reason (%d): %p", rejoinReason, rejoinReasonStrings[index]);
    ...
    emberAfAppPrintln("Last Leave Reason  (%d): %p", leaveReason, leaveReasonStrings[index]);
