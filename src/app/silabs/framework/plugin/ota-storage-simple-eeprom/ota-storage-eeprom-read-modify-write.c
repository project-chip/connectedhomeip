...
/***************************************************************************/ /**
                                                                               ...
                                                                               *that support read-modify-write of arbitrary page
                                                                               *sizes, and across page boundaries.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
uint32_t emberAfOtaStorageDriverRetrieveLastStoredOffsetCallback(void)
{
    ...
    offset = emAfOtaStorageReadInt32uFromEeprom(otaStorageEepromGetImageInfoStartAddress() + SAVED_DOWNLOAD_OFFSET_INDEX);
...
{
    uint32_t oldDownloadOffset = emberAfOtaStorageDriverRetrieveLastStoredOffsetCallback();

    if (finalOffset || offset == 0 || (offset > SAVE_RATE && (oldDownloadOffset + SAVE_RATE) <= offset))
    {
        ...
        emAfOtaStorageWriteInt32uToEeprom(offset, otaStorageEepromGetImageInfoStartAddress() + SAVED_DOWNLOAD_OFFSET_INDEX);
...
void emAfOtaStorageEepromInit(void)
{
    ...
    if (emberAfPluginEepromInfo() != NULL)
    {
        ...
        assert((emberAfPluginEepromInfo()->capabilitiesMask & EEPROM_CAPABILITIES_PAGE_ERASE_REQD) == 0);
    }
}
