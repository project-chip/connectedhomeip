...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This is an integration of the simple OTA
                                                                               *storage driver with the low-level EEPROM driver.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static uint32_t gOtaImageInfoStart = EEPROM_END - MAX_IMAGE_INFO_AND_OTA_HEADER_SIZE;
#else
...
void emAfSetupFakeEepromForSimulation(void)
{
    setupFakeEeprom(gOtaEepromSize,
                    ...
                    2048,             // page size
                    emberAfPluginEepromInfo()->pageEraseMs, (COMPILED_FOR_READ_MODIFY_WRITE == false),
...
{
    return emAfOtaStorageReadInt32uFromEeprom(gOtaImageInfoStart + EBL_START_OFFSET_INDEX);
}

static void setEblStartOffset(uint32_t eblStart)
{
    debugPrint("Writing EBL start offset of 0x%4X to EEPROM offset 0x%4X", eblStart, gOtaImageInfoStart + EBL_START_OFFSET_INDEX);
    debugFlush();
    emAfOtaStorageWriteInt32uToEeprom(eblStart, gOtaImageInfoStart + EBL_START_OFFSET_INDEX);
...
static bool readWritePrimitive(bool read, const uint8_t * writeData, uint32_t offset, uint32_t length, uint8_t * readData)
{
    uint8_t count = 1;
    uint8_t i;
    ...
    for (i = 0; i < count; i++)
    {
        ...
        debugFlush();
        debugPrint("%p realOffset: 0x%4X, realLength: %l", (read ? "read" : "write"), realOffset, realLength);
        debugFlush();
        ...
        else
        {
            ...
            if ((status == EMBER_SUCCESS) && spansBreak && (i == 0) &&
                ((realLength + realOffset) % emberAfPluginEepromGetWordSize()))
            {
                ...
                status                          = emberAfPluginEepromWrite(realOffset + realLength, zeroData, remainingLen);
            }
        }
...
bool emAfOtaStorageCheckDownloadMetaData(void)
{
    ...
    emberAfPluginEepromRead(gOtaImageInfoStart + MAGIC_NUMBER_OFFSET, magicNumberActual, MAGIC_NUMBER_SIZE + VERSION_NUMBER_SIZE);
    if (0 != MEMCOMPARE(magicNumberExpected, magicNumberActual, MAGIC_NUMBER_SIZE + VERSION_NUMBER_SIZE))
    {
...
void emAfOtaStorageWriteDownloadMetaData(void)
{
    ...
    debugFlush();
    emberAfPluginEepromWrite(gOtaImageInfoStart + MAGIC_NUMBER_OFFSET, magicNumber, MAGIC_NUMBER_SIZE + VERSION_NUMBER_SIZE);
}
...
bool emberAfOtaStorageDriverReadCallback(uint32_t offset, uint32_t length, uint8_t * returnData)
{
    return readWritePrimitive(true, // read?
                              ...
                              offset, length, returnData);
}

static bool socBootloaderSupportWriteHandler(const uint8_t * dataToWrite, uint32_t offset, uint32_t length)
{
#if defined(SOC_BOOTLOADING_SUPPORT)
    ...
    if (offset == 0)
    {
        ...
        headerLength = ((dataToWrite[HEADER_LENGTH_OFFSET] + (dataToWrite[HEADER_LENGTH_OFFSET + 1] << 8)) + TAG_OVERHEAD);
...
{
    if (!socBootloaderSupportWriteHandler(dataToWrite, offset, length))
    {
        return false;
    }

    if (readWritePrimitive(false, // read?
                           dataToWrite, offset, length, NULL))
    { // readData pointer
...
void calculateSlotAndEepromOffsets()
{
    ...
    if (SLOT_STRATEGY != DO_NOT_USE_SLOTS)
    {
        ...
        if ((SLOT_MANAGER_SUCCESS == status) && (numSlots != 0))
        {
            ...
            { // (SLOT_STRATEGY == USE_SPECIFIC_SLOT)
...
#if defined(SOC_BOOTLOADING_SUPPORT)
                gOtaImageInfoStart = gOtaStorageEnd - MAX_IMAGE_INFO_AND_OTA_HEADER_SIZE;
#else
    ...
    { // DO_NOT_USE_SLOTS
        ...
        if ((SLOT_MANAGER_SUCCESS == status) && (numSlots != 0))
        {
            ...
            if (SLOT_MANAGER_SUCCESS == status)
            {
                ...
                defaultSlotEndAddress   = defaultSlotStartAddress + slotInfo.slotStorageInfo.length;
...
void emAfOtaStorageDriverInfoPrint(void)
{
    ...
    otaPrintFlush();
    otaPrintln("Offset of download offset   0x%4X", gOtaImageInfoStart + SAVED_DOWNLOAD_OFFSET_INDEX);
    otaPrintFlush();
    otaPrintln("Offset of EBL offset:       0x%4X", gOtaImageInfoStart + EBL_START_OFFSET_INDEX);
    otaPrintFlush();
    otaPrintln("Offset of image start:      0x%4X", gOtaImageInfoStart + OTA_HEADER_INDEX);
    otaPrintFlush();
...
static void printImageInfoStartData(void)
{
    uint8_t data[DATA_SIZE];
    uint8_t i;
    uint32_t maxSize = (MAX_IMAGE_INFO_AND_OTA_HEADER_SIZE < 128 ? MAX_IMAGE_INFO_AND_OTA_HEADER_SIZE : 128);
...
static void printDataBlock(const uint8_t * block)
{
    ...
    {
        otaPrintFlush();
        otaPrintln("%X %X %X %X %X %X %X %X", block[i], block[i + 1], block[i + 2], block[i + 3], block[i + 4], block[i + 5],
                   block[i + 6], block[i + 7]);
        otaPrintFlush();
    }
}
...
void emberAfPluginEepromTest(void)
{
    ...
    addressOffset = 0;
    value         = 0x02;
    otaPrintln("Re-writing value 0x%X of length %d to address 0x%4X", value, length, addressOffset);
    MEMSET(data, value, DATA_SIZE);
