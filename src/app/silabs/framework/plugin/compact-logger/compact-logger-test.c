...
#define MILLISECOND_TICKS_SINCE_BOOT                                                                                               \
    (MILLISECOND_TICKS_PER_SECOND * 60 /* seconds */                                                                               \
     * 60                              /* minutes */                                                                               \
     * 3)                              /* hours */
...
#define UPDATED_UTC_TIME_SECONDS (60 * 60 * 24 * 31)

//------------------------------------------------------------------------------
// Forward Declarations

#define verifyCompactLogInfo(index, info) verifyCompactLogInfoWithFileAndLineNumber(index, info, __FILE__, __LINE__)
...
static void verifyCompactLogInfoWithFileAndLineNumber(uint16_t index, EmberAfPluginCompactLoggerMessageInfo * expectedInfo,
                                                      const char * file, int lineNumber)
{
    EmberAfPluginCompactLoggerMessageInfo actualInfo;
    expectComparisonHex(EMBER_SUCCESS, emberAfPluginCompactLoggerGetEntryByNumber(index, &actualInfo), "EMBER_SUCCESS",
...
{
    expectComparisonHex(EMBER_SUCCESS, emberAfPluginCompactLoggerAdd(globalLogMessageId, dataLength, data), "EMBER_SUCCESS",
...
static void runAllTests(void)
{
    ...
    // 2. After time sync, rewrite all logs with updated time stamps
    expectComparisonHex(EMBER_SUCCESS, emberAfPluginCompactLoggerUpdateAllLogsWithUtcTime(UPDATED_UTC_TIME_SECONDS),
                        "EMBER_SUCCESS", "emberAfPluginCompactLoggerUpdateAllLogsWithUtcTime()");
...
int main(int argc, char * argv[])
{
    ...
    return parseCommandLineAndExecuteTest(argc, argv, "af-plugin-compact-logger-test", allTests);
}
