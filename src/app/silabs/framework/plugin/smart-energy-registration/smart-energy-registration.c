...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This defines the state machine and procedure
                                                                               *for a newly joined device to perform the necessary
                                                                               *steps to register in a network.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
uint32_t emAfPluginSmartEnergyRegistrationDiscoveryPeriod = EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_DELAY_PERIOD;
...
static bool checkErrorCountAndSetEventControl(uint32_t delayMs, bool errorFlag, bool resetErrorCount);
static bool resumeAfterDelay(EmberStatus status, uint32_t delayMs);

#define resumeAfterFixedDelay(status) resumeAfterDelay((status), EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_DELAY_RETRY)

#define transition(next)                                                                                                           \
    transitionAfterDelay((next), EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_DELAY_TRANSITION, true) // reset error count
static bool transitionAfterDelay(EmAfPluginSmartEnergyRegistrationState next, uint32_t delay, bool resetErrorCount);
...
EmberStatus emberAfRegistrationStartCallback(void)
{
    ...
    {
        transitionAfterDelay(STATE_DISCOVER_KEY_ESTABLISHMENT_CLUSTER, EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_DELAY_INITIAL,
                             true);
    }
...
static bool checkErrorCountAndSetEventControl(uint32_t delayMs, bool errorFlag, bool resetErrorCount)
{
    ...
    {
        state->errors++;
        emberAfRegistrationPrintln("Registration error count %d of %d", state->errors,
    ...
    emberAfNetworkEventControlSetDelayMS(emberAfPluginSmartEnergyRegistrationTickNetworkEventControls, delayMs);
    return true;
}
...
static bool resumeAfterDelay(EmberStatus status, uint32_t delayMs)
{
    ...
    return checkErrorCountAndSetEventControl((errorFlag ? delayMs : EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_DELAY_RESUME),
                                             errorFlag, false);
}
...
static void performKeyEstablishment(void)
{
    ...
    status = emberAfInitiateKeyEstablishment(EMBER_TRUST_CENTER_NODE_ID, state->trustCenterKeyEstablishmentEndpoint);
    if (status != EMBER_SUCCESS)
    {
        emberAfRegistrationPrintln("ERR: Failed to start key establishment (0x%x)", status);
                                     ...
                                     uint8_t delayInSeconds)
{
    ...
    if (state->state == STATE_PERFORM_KEY_ESTABLISHMENT)
    {
        ...
        else if (status >= APP_NOTIFY_ERROR_CODE_START)
        {
            ...
            resumeAfterDelay(EMBER_ERR_FATAL, delayMs);
        }
    }
...
static bool findNextNodeForDiscoveryOrPartnerKeyExchange(State * state, bool skipCurrent)
{
#ifdef EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_ESI_DISCOVERY_REQUIRED
    while (state->esiEntry != NULL)
    {
        assert(state->esiEntry->nodeId != EMBER_NULL_NODE_ID);
        if (skipCurrent || state->esiEntry->nodeId == EMBER_TRUST_CENTER_NODE_ID)
        {
    ...
    {
        if (skipCurrent || state->timeSourceCandidates[state->currentCandidate].nodeId == EMBER_TRUST_CENTER_NODE_ID
#ifdef EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_ESI_DISCOVERY_REQUIRED
            || emberAfPluginEsiManagementEsiLookUpByShortIdAndEndpoint(
                   state->timeSourceCandidates[state->currentCandidate].nodeId,
...
static bool retrieveNodeForDiscoveryOrPartnerKeyExchange(const State * state, EmberNodeId * nodeId, uint8_t * endpoint)
{
#ifdef EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_ESI_DISCOVERY_REQUIRED
...
static void performPartnerLinkKeyExchange(void)
{
    ...
    if (findNextNodeForDiscoveryOrPartnerKeyExchange(state, false))
    {
        ...
        emberAfRegistrationPrintln("Perform%p partner link key exchange"
                                   ...
                                   "ing", nodeId, endpoint);
        status = emberAfInitiatePartnerLinkKeyExchange(nodeId, endpoint, partnerLinkKeyExchangeCallback);
        if (status != EMBER_SUCCESS)
        {
            emberAfRegistrationPrintln("ERR: Failed to %p partner link key request"
                                       " with node 0x%2x endpoint 0x%x (0x%x)",
                                       "initiate", nodeId, endpoint, status);
            resumeAfterFixedDelay(status);
        }
    }
    else
...
static void performBinding(void)
{
    ...
    for (; state->esiEntry != NULL; state->esiEntry = emberAfPluginEsiManagementGetNextEntry(state->esiEntry, 0))
    {
        assert(state->esiEntry->nodeId != EMBER_NULL_NODE_ID);
        for (; state->endpointIndex < emberAfEndpointCount(); state->endpointIndex++)
        {
            uint8_t endpoint = emberAfEndpointFromIndex(state->endpointIndex);
            if (networkIndex != emberAfNetworkIndexFromEndpointIndex(state->endpointIndex))
            {
                continue;
            }
            for (; clusterList[state->clusterIndex] != UNDEFINED_CLUSTER_ID; state->clusterIndex++)
            {
                ...
                if (emberAfContainsClient(endpoint, clusterId))
                {
                    ...
                    emberAfRegistrationPrintln("Performing binding"
                                               ...
                                               " from endpoint 0x%x"
                                               " for cluster 0x%2x",
                                               state->esiEntry->nodeId, state->esiEntry->endpoint, endpoint, clusterId);
                    status = emberBindRequest(state->esiEntry->nodeId, state->esiEntry->eui64, state->esiEntry->endpoint, clusterId,
                                              UNICAST_BINDING, eui64,
                                              0, // multicast group identifier - ignored
                                              endpoint, EMBER_AF_DEFAULT_APS_OPTIONS);
                    if (status == EMBER_SUCCESS)
                    {
                        state->clusterIndex++;
                    }
                    else
                    {
                        emberAfRegistrationPrintln("ERR: Failed to send bind request"
                                                   ...
                                                   state->esiEntry->nodeId, state->esiEntry->endpoint, endpoint, clusterId, status);
                    }
...
static void determineAuthoritativeTimeSource(void)
{
    ...
    static const uint8_t attributeIds[] = {
        ...
        LOW_BYTE(ZCL_LAST_SET_TIME_ATTRIBUTE_ID),
        HIGH_BYTE(ZCL_LAST_SET_TIME_ATTRIBUTE_ID),
        LOW_BYTE(ZCL_VALID_UNTIL_TIME_ATTRIBUTE_ID),
        HIGH_BYTE(ZCL_VALID_UNTIL_TIME_ATTRIBUTE_ID),
    };
    ...
    while (state->esiEntry != NULL)
    {
        ...
        emberAfRegistrationPrintln("Requesting time attributes"
                                   ...
                                   TIME_SOURCE_CANDIDATE(state).nodeId, TIME_SOURCE_CANDIDATE(state).endpoint);

        emberAfFillCommandGlobalClientToServerReadAttributes(ZCL_TIME_CLUSTER_ID, attributeIds, sizeof(attributeIds));
        emberAfSetCommandEndpoints(sourceEndpoint, TIME_SOURCE_CANDIDATE(state).endpoint);
        status = emberAfSendCommandUnicast(EMBER_OUTGOING_DIRECT, TIME_SOURCE_CANDIDATE(state).nodeId);
        ...
        else
        {
            emberAfRegistrationPrintln("ERR: Failed to request time attributes"
                                       ...
                                       TIME_SOURCE_CANDIDATE(state).nodeId, TIME_SOURCE_CANDIDATE(state).endpoint, status);
        }
...
void emAfPluginSmartEnergyRegistrationReadAttributesResponseCallback(uint8_t * buffer, uint16_t bufLen)
{
    ...
    emberAfRegistrationPrintln("Received time attributes from node 0x%2x", cmd->source);
    ...
    {
        if (state->source.valid && state->source.nodeId == cmd->source)
        {
    ...
    else
    {
        ...
        if ((IS_TIME_STATUS(timeStatus, MASTER_CLOCK) || IS_TIME_STATUS(timeStatus, SYNCHRONIZED))
...
static void stopRegistration(bool success)
{
    ...
    if (success)
    {
        transitionAfterDelay(STATE_DISCOVER_ENERGY_SERVICE_INTERFACES, emAfPluginSmartEnergyRegistrationDiscoveryPeriod, true);
        return;
    }
#endif // EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_DELAY_PERIOD
...
static void performDiscovery(void)
{
    ...
    {
        emberAfRegistrationPrintln("ERR: Invalid state for discovery (0x%x)", state->state);
        emberAfRegistrationAbortCallback();
        return;
    }
    ...
    status = emberAfFindDevicesByProfileAndCluster(target, SE_PROFILE_ID, clusterId, EMBER_AF_SERVER_CLUSTER_DISCOVERY,
                                                   discoveryCallback);
#ifdef PARTNER_KE_REQUIRED
kickout:
#endif
...
static void discoveryCallback(const EmberAfServiceDiscoveryResult * result)
{
        ...
        if (result->zdoRequestClusterId == MATCH_DESCRIPTORS_REQUEST)
        {
            ...
            for (i = 0; i < endpointList->count; i++)
            {
                if (state->state == STATE_DISCOVER_KEY_ESTABLISHMENT_CLUSTER)
                {
                    ...
                    {
                        emberAfRegistrationPrintln("%p %p on node 0x%2x endpoint 0x%x%p", "Discovered", "Key Establishment cluster",
                                                   result->matchAddress, endpointList->list[i], "");
                        state->trustCenterKeyEstablishmentEndpoint = endpointList->list[i];
                    }
                    else
                    {
                        emberAfRegistrationPrintln("%p %p on node 0x%2x endpoint 0x%x%p", "INFO: Ignored",
                                                   "Key Establishment cluster", result->matchAddress, endpointList->list[i], "");
                    }
#ifdef EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_ESI_DISCOVERY_REQUIRED
                }
                else if (state->state == STATE_DISCOVER_ENERGY_SERVICE_INTERFACES)
                {
                    emberAfRegistrationPrintln("%p %p on node 0x%2x endpoint 0x%x%p", "Discovered", "Energy Server Interface",
                                               result->matchAddress, endpointList->list[i], "");

                    state->esiEntry =
                        emberAfPluginEsiManagementEsiLookUpByShortIdAndEndpoint(result->matchAddress, endpointList->list[i]);
                    if (state->esiEntry == NULL)
                    {
                    ...
                    {
                        emberAfRegistrationPrintln("%p %p on node 0x%2x endpoint 0x%x%p", "INFO: Ignored",
                                                   "Energy Server Interface", result->matchAddress, endpointList->list[i],
                ...
                {
                    emberAfRegistrationPrintln("%p %p on node 0x%2x endpoint 0x%x%p", "Discovered", "Time server",
                                               result->matchAddress, endpointList->list[i], "");
                    ...
                    {
                        emberAfRegistrationPrintln("%p %p on node 0x%2x endpoint 0x%x%p", "INFO: Ignored", "Time server",
                                                   result->matchAddress, endpointList->list[i], " because table is full");
                    }
    ...
    if (state->state == STATE_DISCOVER_KEY_ESTABLISHMENT_CLUSTER)
    {
        if (state->trustCenterKeyEstablishmentEndpoint == UNDEFINED_ENDPOINT)
        {
            emberAfRegistrationPrintln("ERR: Failed to find %p", "Key Establishment cluster");
        ...
        else
        {
            ...
            {
                emberAfRegistrationFlush();
                emberAfRegistrationPrintln("%pSkipping key establishment%p", "WARN: ",
            ...
            {
                emberAfRegistrationPrintln("%pSkipping key establishment%p", "", " because key is already authorized");
                transition(NEXT_STATE_AFTER_KE);
            }
            else
            {
    ...
    else if (state->state == STATE_DISCOVER_ENERGY_SERVICE_INTERFACES)
    {
        if (result->status == EMBER_AF_BROADCAST_SERVICE_DISCOVERY_COMPLETE)
        {
            ...
            if (state->esiEntry == NULL)
            {
                ...
                emberAfRegistrationPrintln("ERR: Failed to find %p", "Energy Service Interfaces");
                resumeAfterFixedDelay(EMBER_ERR_FATAL);
            }
            else
            {
    ...
    else if (state->state == STATE_DISCOVER_TIME_SERVERS)
    {
        if (result->status == EMBER_AF_BROADCAST_SERVICE_DISCOVERY_COMPLETE)
        {
            ...
            {
                emberAfRegistrationPrintln("ERR: Failed to find %p", "Time servers");
                resumeAfterFixedDelay(EMBER_ERR_FATAL);
            }
            else
            {
    ...
    else if (state->state == STATE_DISCOVER_IEEE_ADDRESSES)
    {
        ...
        {
            emberAfRegistrationPrintln("%p IEEE address for node 0x%2x", "Discovered", nodeId);
#ifdef EMBER_AF_PLUGIN_SMART_ENERGY_REGISTRATION_ESI_DISCOVERY_REQUIRED
            if (state->esiEntry != NULL)
            {
...
#endif
            if (emberAfAddAddressTableEntry((uint8_t *) result->responseData, nodeId) == EMBER_NULL_ADDRESS_TABLE_INDEX)
            {
