...
void emberAfPluginEzModeCommissioningClientCommand(void)
{
    ...
    {
        clusterIds[clusterIdLength] = (EmberAfClusterId) emberUnsignedCommandArgument(clusterIdLength + 2);
    }
    status = emberAfEzmodeClientCommission(endpoint, direction, clusterIds, clusterIdLength);
    UNUSED_VAR(status);
