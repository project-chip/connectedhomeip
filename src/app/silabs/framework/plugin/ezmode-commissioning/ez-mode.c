...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This file provides a function set for
                                                                               *initiating ez-mode commissioning as both a client
                                                                               *and a server.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static void identifyRequestMessageSentCallback(EmberOutgoingMessageType type, uint16_t indexOrDestination, EmberApsFrame * apsFrame,
                                               uint16_t msgLen, uint8_t * message, EmberStatus status)
{
    ...
    if (ezModeState == EZMODE_IDENTIFY)
    {
        if (status == EMBER_SUCCESS)
        {
            ...
            emberEventControlSetDelayMS(stateEvent, (10 * MILLISECOND_TICKS_PER_SECOND));
        }
        else
        {
            complete();
...
void emberAfPluginEzmodeCommissioningStateEventHandler(void)
{
    ...
    case EZMODE_IDENTIFY:
        ...
        status = emberAfSendCommandBroadcastWithCallback(EMBER_SLEEPY_BROADCAST_ADDRESS, identifyRequestMessageSentCallback);
        if (status != EMBER_SUCCESS)
        {
            complete();
        }
    ...
    case EZMODE_MATCH:
        ...
        status =
            emberAfFindClustersByDeviceAndEndpoint(currentIdentifyingAddress, currentIdentifyingEndpoint, serviceDiscoveryCallback);
        if (status != EMBER_SUCCESS)
        {
            complete();
        }
...
bool emberAfIdentifyClusterIdentifyQueryResponseCallback(uint16_t timeout)
{
    ...
    {
        if ((ezModeState == EZMODE_IDENTIFY_WAIT || ezModeState == EZMODE_IDENTIFY) && timeout != 0)
        {
...
static void createBinding(uint8_t * address)
{
    ...
    for (i = 0; i < EMBER_BINDING_TABLE_SIZE; i++)
    {
        if (emberGetBinding(i, &candidate) == EMBER_SUCCESS && candidate.type == EMBER_UNICAST_BINDING &&
            candidate.local == ezmodeClientEndpoint && candidate.clusterId == ezmodeClientCluster &&
            candidate.remote == currentIdentifyingEndpoint && MEMCOMPARE(candidate.identifier, address, EUI64_SIZE) == 0)
        {
            bindingIndex = i;
...
EmberStatus emberAfEzmodeServerCommissionWithTimeout(uint8_t endpoint, uint16_t identifyTimeoutSeconds)
{
    ...
    afStatus = emberAfWriteAttribute(endpoint, ZCL_IDENTIFY_CLUSTER_ID, ZCL_IDENTIFY_TIME_ATTRIBUTE_ID, CLUSTER_MASK_SERVER,
                                     (uint8_t *) &identifyTimeoutSeconds, ZCL_INT16U_ATTRIBUTE_TYPE);
