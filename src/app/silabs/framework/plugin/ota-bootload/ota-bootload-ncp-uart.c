...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Routines for bootloading from a linux host
                                                                               *connected via UART to an NCP
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
bool emAfStartNcpBootloaderCommunications(void)
{
    ...
    if (EZSP_SUCCESS != ezspSetupSerialPort(&serialFd, errorString, MAX_ERROR_LENGTH, true))
    { // bootloader mode?
...
{
    if (length != write(serialFd, data, length))
    { // count
        errorPrint("Error: Failed to write to serial port: %s", strerror(errno));
        return false;
    }
    fsync(serialFd);
    return true;
