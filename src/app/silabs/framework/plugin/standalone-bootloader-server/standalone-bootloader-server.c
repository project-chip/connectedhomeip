...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This file defines the server behavior for
                                                                               *the Ember proprietary bootloader protocol.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static const EmberEUI64 broadcastEui64 = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };
...
static EmberStatus sendQuery(const EmberEUI64 targetEui)
{
    ...
    return emberAfPluginStandaloneBootloaderCommonSendMessage(isBroadcast, eui64Copy, length, outgoingBlock);
}
...
static void decodeAndPrintServerMessageType(uint8_t command)
{
    ...
    const char * commandStrings[] = {
        "Query Response", "Auth Challenge", "SOH", "EOT", "ACK", "NAK", "CANCEL", "BLOCK_OK", "FILEDONE",
    };
    ...
    {
        bootloadPrintln("Received Standalone Bootloader message (%d): %p", command, (id == 0xFF ? "??" : commandStrings[id]));
    }
}
...
static void printTargetClientInfo(const char * prefixString)
{
    ...
    if (isTargetClientDataValid())
    {
        ...
        bootloadPrintln("\n  Bootloader Active: %p", (targetClient.bootloaderActive ? "yes" : "no"));
...
static void sendLaunchRequest(void)
{
    ...
    MEMMOVE(&(outgoingBlock[index]), targetClient.hardwareTag, EMBER_AF_STANDALONE_BOOTLOADER_HARDWARE_TAG_LENGTH);
    ...
    status = emberAfPluginStandaloneBootloaderCommonSendMessage(false, // is broadcast?
                                                                targetClient.eui64, index, outgoingBlock);
...
static EmberStatus xmodemSendRoutine(uint8_t * data, uint8_t length)
{
    ...
    index += length;
    return emberAfPluginStandaloneBootloaderCommonSendMessage(false, targetClient.eui64, index, outgoingBlock);
}

static EmberStatus getNextBootloaderBlock(uint32_t address, // relative to 0
                                          uint8_t length, uint8_t * returnData, uint8_t * returnLength, bool * done)
{
    ...
    status = emberAfOtaStorageReadImageDataCallback(&imageToBootload, address + tagOffset, length, returnData, &realReturnLength);
    *returnLength = (uint8_t) realReturnLength;
    ...
    return (status == EMBER_AF_OTA_STORAGE_SUCCESS ? EMBER_SUCCESS : EMBER_ERR_FATAL);
}
...
static void startSendingXmodemData(void)
{
    ...
    if (EMBER_SUCCESS !=
        emberAfPluginXmodemSenderStart(xmodemSendRoutine, getNextBootloaderBlock, xmodemComplete, OTA_XMODEM_MESSAGE_SIZE, false))
    { // wait for ready char?
...
static void processQueryResponse(EmberEUI64 longId, uint8_t * message)
{
    ...
    targetClient.mfgId            = HIGH_LOW_TO_INT(message[QRESP_OFFSET_MFG_ID], message[QRESP_OFFSET_MFG_ID + 1]);
    MEMMOVE(targetClient.hardwareTag, &(message[QRESP_OFFSET_HARDWARE_TAG]), EMBER_AF_STANDALONE_BOOTLOADER_HARDWARE_TAG_LENGTH);
    ...
    targetClient.bootloaderVersion = HIGH_LOW_TO_INT(message[QRESP_OFFSET_BL_VERSION], message[QRESP_OFFSET_BL_VERSION + 1]);
...
static void processChallenge(EmberEUI64 longId, uint8_t * message)
{
    uint8_t outgoingBlock[XMODEM_AUTH_RESPONSE_LENGTH];
    EmberStatus status;
    uint8_t index = emberAfPluginStandaloneBootloaderCommonMakeHeader(outgoingBlock, XMODEM_AUTH_RESPONSE);
    ...
    if (message[XMODEM_AUTH_CHALLENGE_REQUEST_INDEX] != CHALLENGE_REQUEST_VERSION)
    {
        bootloadPrintln("Error: Challenge request version (%d) not the same as mine (%d)",
                        message[XMODEM_AUTH_CHALLENGE_REQUEST_INDEX], CHALLENGE_REQUEST_VERSION);
        resetServerState(false);
        return;
    }

    MEMMOVE(&(outgoingBlock[index]), &(message[BOOTLOAD_MESSAGE_OVERHEAD]), BOOTLOAD_AUTH_CHALLENGE_SIZE);

    emAfStandaloneBootloaderClientEncrypt(&(outgoingBlock[index]), (uint8_t *) emberKeyContents(&bootloadKey));

    index += BOOTLOAD_AUTH_RESPONSE_SIZE;
    status = emberAfPluginStandaloneBootloaderCommonSendMessage(false, longId, index, outgoingBlock);
    if (status == EMBER_SUCCESS)
    {
...
EmberStatus emberAfPluginStandaloneBootloaderServerStartClientBootloadWithCurrentTarget(const EmberAfOtaImageId * id, uint16_t tag)
{
    ...
    return emberAfPluginStandaloneBootloaderServerStartClientBootload(targetClient.eui64, id, tag);
}
                                                                       ...
                                                                       uint16_t tag)
{
    ...
    if (EMBER_AF_OTA_STORAGE_SUCCESS != emAfOtaStorageGetTagOffsetAndSize(id, tag, &tagOffset, &tagSize))
    {
    ...
    if (status == EMBER_SUCCESS)
    {
        ...
        emberEventControlSetDelayQS(emberAfPluginStandaloneBootloaderServerMyEventEventControl, QUERY_TIMEOUT_SECONDS << 2);
    }
    else
    {
        resetServerState(false);
...
bool emberAfPluginStandaloneBootloaderCommonIncomingMessageCallback(EmberEUI64 longId, uint8_t length, uint8_t * message)
{
    ...
    case (XMODEM_QRESP): {
            ...
            serverBootloadState == SERVER_BOOTLOAD_STATE_CHECK_IF_BOOTLOADER_ACTIVE_SENT)
        {
            ...
            processQueryResponse(longId, message);
        }
        break;
    }
    case (XMODEM_AUTH_CHALLENGE): {
        if (serverBootloadState == SERVER_BOOTLOAD_STATE_LAUNCH_REQUEST_SENT)
        {
            ...
            {
                bootloadPrintln("Error: Bootloader Challenge too short (%d < %d)", length, XMODEM_AUTH_CHALLENGE_LENGTH);
            }
            else
            {
                processChallenge(longId, message);
            }
        }
        break;
    }
