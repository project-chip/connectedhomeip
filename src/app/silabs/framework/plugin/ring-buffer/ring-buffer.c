...
/***************************************************************************/ /**
                                                                               ...
                                                                               * An implementation of a basic ring buffer where each
                                                                               *entry in the buffer has the following format:
                                                                               *
                                                                               *   length - 1 or 2 bytes, little-endian.
                                                                               *            This is the length of the data only and
                                                                               *does not include the length field itself. data -
                                                                               *variable length
                                                                               *
                                                                               * This plugin does NOT allocate the data for the
                                                                               *buffer, but acts on a data space allocated by
                                                                               *another plugin.  Therefore it can act on multiple
                                                                               * ring buffers.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static EmberStatus addOrAppend(EmberRingBuffer * ring, uint8_t * dataToAdd, uint16_t dataToAddSize, RingBufferAction action);

static void copyFromRingBuffer(EmberRingBuffer * ring, uint16_t index, uint16_t length, uint8_t * returnData);

static uint16_t getNextEntryIndex(EmberRingBuffer * ring, uint16_t index);

static uint16_t calculateMaxWriteLengthWithoutOverlap(EmberRingBuffer * ring);

static uint16_t calculateNewIndexWriteData(EmberRingBuffer * ring, uint16_t index, uint16_t length, uint8_t * newData);
...
static uint16_t getIndexOfDataForEntry(EmberRingBuffer * ring, uint16_t indexOfEntry);

static EmberStatus internalGetEntry(EmberRingBuffer * ring, uint16_t index, uint16_t dataIndexInEntry,
                                    uint16_t * returnEntryTotalSize, uint16_t maxReturnSize, uint16_t * returnDataSize,
                                    uint8_t * returnData);
...
EmberStatus emberAfPluginRingBufferInitStruct(EmberRingBuffer * ringBufferStruct, bool wideLengthField,
                                              EmberAfPluginRingBufferDeleteFunction * deleteCallback, uint8_t * data)
{
    debug("%s\n", __PRETTY_FUNCTION__);
    if (data == NULL)
    {
    ...
    ringBufferStruct->lengthFieldWidthBytes = (wideLengthField ? EMBER_AF_PLUGIN_RING_BUFFER_WIDE_LENGTH_ENTRY_OVERHEAD
...
{
    return addOrAppend(ring, dataToAdd, dataToAddSize, RING_BUFFER_ADD_NEW_ENTRY);
}
                                                         ...
                                                         uint16_t * returnDataSize, uint8_t * returnData)
{
    ...
    status = internalGetEntry(ring, index, dataIndexInEntry, returnEntryTotalSize, maxReturnSize, returnDataSize, returnData);
...
{
    return emberAfPluginRingBufferGetEntryByEntryNumber(ring, ring->entryCount - 1, dataIndexInEntry, returnEntryTotalSize,
                                                        maxReturnSize, returnDataSize, returnData);
}
                                                      ...
                                                      uint16_t * returnDataSize, uint8_t * returnData)
{
    ...
    return internalGetEntry(ring, ring->iteratorIndex, dataIndexInEntry, returnEntryTotalSize, maxReturnSize, returnDataSize,
                            returnData);
}
                                                         ...
                                                         uint16_t updatedDataLength)
{
    ...
    calculateNewIndexWriteData(ring, index, updatedDataLength, updatedData);
...
EmberStatus emberAfPluginRingBufferAppendLastEntry(EmberRingBuffer * ring, uint8_t * dataToAdd, uint16_t dataToAddSize)
{
    ...
    status = addOrAppend(ring, dataToAdd, dataToAddSize, RING_BUFFER_APPEND_LAST_ENTRY);
    ...
    {
        indexOfLastEntry = calculateNewIndexWriteData(ring, indexOfLastEntry, 1, &serializedLengthData[i]);
    }
...
static EmberStatus addOrAppend(EmberRingBuffer * ring, uint8_t * dataToAdd, uint16_t dataToAddSize, RingBufferAction action)
{
    ...
    while (dataToAddSize > 0)
    {
        ...
        if (action == RING_BUFFER_ADD_NEW_ENTRY && maxWriteLength > 0 && lengthDataWritten < getLengthFieldWidth(ring))
        {
        ...
        else if (maxWriteLength > 0)
        {
            ...
            newTailIndex = calculateNewIndexWriteData(ring, ring->tailIndex, maxWriteLength, dataToAdd);
            debug("Moved tail index from %d to %d\n", ring->tailIndex, newTailIndex);
        ...
        if (dataToAddSize > 0 && ring->tailIndex == ring->firstItemIndex)
        {
            uint16_t newFirstItemIndex = getNextEntryIndex(ring, ring->firstItemIndex);
...
static uint16_t getNextEntryIndex(EmberRingBuffer * ring, uint16_t indexOfEntry)
{
    uint16_t length;
    EMBER_TEST_ASSERT(indexOfEntry < ring->maxSize);
                                    ...
                                    uint8_t * returnData)
{
    ...
    *returnDataSize = *returnEntryTotalSize - dataIndexInEntry;
    ...
    copyFromRingBuffer(ring, index, *returnDataSize, returnData);
