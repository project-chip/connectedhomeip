...
#define verifyLastEntry(length) verifyLastEntryWithFileInfo(length, __FILE__, __LINE__)

#define verifyAddEntryNumber(ringBufferEntryNumber, length, numberOfAddEntry)                                                      \
    verifyAddEntryNumberWithFileInfo(ringBufferEntryNumber, length, numberOfAddEntry, __FILE__, __LINE__)
...
static void addEntry(uint16_t size)
{
    ...
    snprintf(formattedString, MAX_STRING_SIZE, "Calling emberAfPluginRingBufferAddEntry() addEntryNumber %d of %d bytes",
             addEntryNumber, size);
    ...
    expectComparisonHex(EMBER_SUCCESS, emberAfPluginRingBufferAddEntry(&ring, &(testItem[addEntryNumber]), size), "EMBER_SUCCESS",
                        formattedString);
    addEntryNumber++;
}

// Array of sizes must be NULL terminated (since 0 is an invalid size)
static void setupRingBufferWithVariousSizes(bool initializeRing, bool wideLengthField, uint16_t * arrayOfSizes)
{
    uint8_t i = 0;
    if (initializeRing)
    {
        expectComparisonHex(EMBER_SUCCESS, emberAfPluginRingBufferInitStruct(&ring, wideLengthField, deleteCallback, buffer),
                            "EMBER_SUCCESS", "emberAfPluginRingBufferInitStruct()");
    }
...
void printAllEntries(void)
{
    ...
    expectComparisonHex(EMBER_SUCCESS, emberAfPluginRingBufferInitIterator(&ring), "EMBER_SUCCESS",
    ...
    do
    {
        ...
        expectComparisonHex(EMBER_SUCCESS,
                            emberAfPluginRingBufferGetEntryByIterator(&ring,
                                                                      0, // index in entry
                                                                      &entryTotalSize, MAX_TEST_SIZE, &returnDataSize, returnData),
                            "EMBER_SUCCESS", "emberAfPluginRingBufferGetEntryByIterator()");

        expectComparisonDecimal(entryTotalSize, returnDataSize, "entryTotalSize", "returnDataSize");
...
static void verifyAddEntryNumberWithFileInfo(uint16_t ringBufferEntryNumber, uint16_t length, uint16_t numberOfAddEntry,
                                             const char * file, int lineNumber)
{
    ...
    expect(EMBER_SUCCESS ==
           emberAfPluginRingBufferGetEntryByEntryNumber(&ring, ringBufferEntryNumber,
                                                        0, // index in entry
                                                        &entryTotalSize, MAX_TEST_SIZE, &returnDataSize, returnData));
    snprintf(formattedString, MAX_STRING_SIZE, "Verifying ring buffer data at entry %d", ringBufferEntryNumber);
    verifyDataWithFileInfo(formattedString, length, &(testItem[numberOfAddEntry]), returnDataSize, returnData, file, lineNumber);
}
...
static void verifyLastEntryWithFileInfo(uint16_t length, const char * file, int lineNumber)
{
    ...
    verifyAddEntryNumberWithFileInfo(ring.entryCount - 1, length, addEntryNumber - 1, file, lineNumber);
}
...
static void runAddTests(void)
{
    ...
    setupRingBufferWithVariousSizes(true, // initialize ring?
                                    wideLengthField, arrayOfSizes);

    expectComparisonHex(EMBER_MESSAGE_TOO_LONG, emberAfPluginRingBufferAddEntry(&ring, NULL, RING_BUFFER_SIZE + 1),
                        "EMBER_MESSAGE_TOO_LONG", "emberAfPluginRingBufferAddEntry() with RING_BUFFER_SIZE + 1");
    ...
    expectComparisonDecimal(11, ring.data[MAX_TEST_SIZE - 2], "Item 4 size", "ring buffer data");

    printAllEntries();

    expectComparisonHex(EMBER_SUCCESS,
                        emberAfPluginRingBufferGetEntryByEntryNumber(&ring, 3,
                                                                     0, // index into entry
                                                                     &entryTotalSize, MAX_TEST_SIZE, &returnDataSize, returnData),
                        "EMBER_SUCCESS", "emberAfPluginRingBufferGetEntryByEntryNumber(3)");

    verifyData("Verifying ring buffer data at entry 3", testItemLength, &(testItem[4]), returnDataSize, returnData);
...
static void runAppendTests(void)
{
    ...
    expectComparisonHex(EMBER_SUCCESS,
                        emberAfPluginRingBufferAppendLastEntry(&ring, &(testItem[addEntryNumber - 1 + testItemLength]),
                                                               2), // append length
                        "EMBER_SUCCESS", "emberAfPluginRingBufferAppendLastEntry()");
    testItemLength += 2;
    printAllEntries();
    verifyLastEntry(testItemLength);
    ...
    expectComparisonDecimal(ring.tailIndex, ring.maxSize - 1, "ring.tailIndex", "ring.maxSize - 1");
    ...
    expectComparisonDecimal(ring.tailIndex, 43, "ring.tailIndex", "43");
...
static void runGetEntryTests(void)
{
    ...
    expectComparisonHex(EMBER_SUCCESS,
                        emberAfPluginRingBufferGetEntryByEntryNumber(&ring,
                                                                     1, // entry number
                                                                     indexWithinEntry, &entryTotalSize, limitedMaxReturnSize,
                                                                     &returnDataSize, returnData),
                        "EMBER_SUCCESS", "emberAfPluginRingBufferGetEntryByEntryNumber()");

    // Now verify we didn't write any MORE data than the limited
    // return size by checking that the returned data is still 0xFF.
    expectComparisonHex(0xFF, returnData[limitedMaxReturnSize], "0xFF", "returnData[limitedMaxReturnSize]");
    ...
    verifyDataWithFileInfo("Verify of get entry where limitedMaxReturnSize < size of entry", limitedMaxReturnSize,
                           &(testItem[1]), // See comment above
                           returnDataSize, returnData, __FILE__, __LINE__);
    ...
    verifyDataWithFileInfo("Verify an entry where the index within the entry is > 0", limitedMaxReturnSize,
    ...
    expectComparisonHex(EMBER_INDEX_OUT_OF_RANGE,
                        emberAfPluginRingBufferGetEntryByEntryNumber(&ring,
                                                                     ...
                                                                     &entryTotalSize, limitedMaxReturnSize, &returnDataSize,
                                                                     returnData),
                        "EMBER_INDEX_OUT_OF_RANGE", "emberAfPluginRingBufferGetEntryByEntryNumber()");
    ...
    expectComparisonHex(EMBER_OPERATION_IN_PROGRESS, emberAfPluginRingBufferIteratorNextEntry(&ring), "EMBER_OPERATION_IN_PROGRESS",
    ...
    expectComparisonHex(EMBER_INVALID_CALL, emberAfPluginRingBufferIteratorNextEntry(&ring), "EMBER_OPERATION_IN_PROGRESS",
                        "emberAfPluginRingBufferIteratorNextEntry()");
...
int main(int argc, char * argv[])
{
    ...
    return parseCommandLineAndExecuteTest(argc, argv, "af-plugin-ring-buffer-test", allTests);
}
