...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This file defines the client behavior for
...
void emAfStandaloneBootloaderClientPrintStatus(void)
{
    ...
    emAfStandaloneBootloaderClientGetInfo(&bootloaderVersion, &platformId, &microId, &phyId);
    emAfStandaloneBootloaderClientGetMfgInfo(&mfgId, hardwareTag);
    getBootloadKey(key);
...
static EmberStatus sendChallenge(EmberEUI64 targetEui)
{
    ...
    uint8_t index = emberAfPluginStandaloneBootloaderCommonMakeHeader(outgoingBlock, XMODEM_AUTH_CHALLENGE);
    uint8_t platformId;
    uint8_t microId;
    uint8_t phyId;
    uint32_t macTimer;
    ...
    MEMMOVE(challenge, &(outgoingBlock[BOOTLOAD_MESSAGE_OVERHEAD]), BOOTLOAD_AUTH_CHALLENGE_SIZE);
    ...
    status = emberAfPluginStandaloneBootloaderCommonSendMessage(false, // is broadcast?
                                                                targetEui, index, outgoingBlock);

    if (status == EMBER_SUCCESS)
    {
        clientBootloadState = CLIENT_BOOTLOAD_STATE_CHALLENGE_SENT;
        emberEventControlSetDelayQS(emberAfPluginStandaloneBootloaderClientMyEventEventControl, CHALLENGE_TIMEOUT_SECONDS << 2);
    }
    return status;
}
...
{
    uint8_t outgoingBlock[MAX_BOOTLOAD_MESSAGE_SIZE];
    uint8_t index = emberAfPluginStandaloneBootloaderCommonMakeHeader(outgoingBlock, XMODEM_QRESP);
    uint16_t mfgId;
    uint8_t platformId;
    uint8_t microId;
    uint8_t phyId;
    ...
    emAfStandaloneBootloaderClientGetMfgInfo(&mfgId, &(outgoingBlock[QRESP_OFFSET_HARDWARE_TAG]));
    emAfStandaloneBootloaderClientGetInfo(&bootloaderVersion, &platformId, &microId, &phyId);
...
bool emberAfPluginStandaloneBootloaderCommonIncomingMessageCallback(EmberEUI64 longId, uint8_t length, uint8_t * message)
{
    ...
    if (!emberAfPluginStandaloneBootloaderClientAllowIncomingMessageCallback(longId, message[OFFSET_MESSAGE_TYPE]))
    {
