...
uint8_t emberAfPluginCalendarCommonGetCalendarById(uint32_t calendarId, uint32_t providerId)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS; i++)
    {
        if (calendarId == calendars[i].calendarId &&
            (providerId == EMBER_AF_PLUGIN_CALENDAR_COMMON_WILDCARD_PROVIDER_ID || providerId == calendars[i].providerId))
        {
            return i;
        }
    }
...
uint32_t emberAfPluginCalendarCommonEndTimeUtc(const EmberAfCalendarStruct * calendar)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS; i++)
    {
        if (calendar->providerId == calendars[i].providerId && calendar->issuerEventId < calendars[i].issuerEventId &&
            calendar->startTimeUtc < calendars[i].startTimeUtc && calendar->calendarType == calendars[i].calendarType &&
                                     ...
                                     uint8_t numberOfWeekProfiles, uint8_t numberOfDayProfiles)
{
    ...
    {
        EmberAfCalendarStruct * cal = &calendars[i];
        if (cal->providerId == providerId && cal->calendarId == issuerCalendarId)
        {
    ...
    *unknownSpecialDaysMask = 0;
    ...
    if (totalNumberOfSpecialDays > EMBER_AF_PLUGIN_CALENDAR_COMMON_SPECIAL_DAY_PROFILE_MAX)
    {
        emberAfCalendarClusterPrintln("ERR: %d number of special days being added is above the current maximum(%d).",
                                      totalNumberOfSpecialDays, EMBER_AF_PLUGIN_CALENDAR_COMMON_SPECIAL_DAY_PROFILE_MAX);
        status = false;
        goto kickout;
    }
    ...
    for (i = 0; i < numberOfSpecialDaysEntries; i++)
    {
        ...
        emberAfGetDate(specialDaysEntries, SPECIAL_DAY_ENTRY_SIZE * i, specialDaysEntriesLength, &startDate);
        normalDayId =
            emberAfGetInt8u(specialDaysEntries, SPECIAL_DAY_ENTRY_SIZE * i + sizeof(EmberAfDate), specialDaysEntriesLength);
        if (cal->numberOfSpecialDayProfiles < EMBER_AF_PLUGIN_CALENDAR_COMMON_SPECIAL_DAY_PROFILE_MAX)
        {
            ...
            else if (update)
            {
                ...
                emberAfCalendarClusterPrintln("Updated: Calendar(calId=0x%4X)", cal->calendarId);
                ...
                emberAfCalendarClusterPrintln("           normalDayIndex: %d",
                                              cal->specialDays[cal->numberOfSpecialDayProfiles].normalDayIndex);
                                         ...
                                         uint16_t dayScheduleEntriesLength)
{
    ...
    if ((dayScheduleEntries == NULL) || (dayScheduleEntriesLength == 0) ||
    ...
    emberAfCalendarClusterPrintln("Updated: DayProfile[%d]", cal->numberOfReceivedDayProfiles);
    emberAfCalendarClusterPrintln("           DayId=%d", dayId);
    ...
    for (scheduleEntryIndex = 0; scheduleEntryIndex < numberOfScheduledEntries; scheduleEntryIndex++)
    {
        ...
        minutesFromMidnight =
            emberAfGetInt16u(dayScheduleEntries, scheduleEntryIndex * SCHEDULE_ENTRY_SIZE, dayScheduleEntriesLength);
        priceTier = emberAfGetInt8u(dayScheduleEntries, scheduleEntryIndex * SCHEDULE_ENTRY_SIZE + 2, dayScheduleEntriesLength);
        normalDay->minutesFromMidnight = minutesFromMidnight;
        normalDay->data                = priceTier;
        emberAfCalendarClusterPrintln("           ScheduledEntries[%d]", scheduleEntryIndex);
        emberAfCalendarClusterPrintln("             startTime: 0x%2X from midnight", minutesFromMidnight);
        emberAfCalendarClusterPrintln("             price tier: 0x%X", priceTier);
    }
...
bool emberAfCalendarCommonSetDayProfInfo(uint8_t index, uint8_t dayId, uint8_t entryId, uint16_t minutesFromMidnight, uint8_t data)
{
    ...
    {
        emberAfCalendarClusterPrintln("EntryId must be in the range of 0 to %d",
                                          ...
                                          uint8_t dayIdRefSun)
{
    ...
    if ((cal == NULL) || (cal->numberOfReceivedWeekProfiles >= EMBER_AF_PLUGIN_CALENDAR_COMMON_WEEK_PROFILE_MAX))
    {
        return false;
    }
    ...
    emberAfCalendarClusterPrintln("Updated: WeekProfile[%d]", cal->numberOfReceivedWeekProfiles);
    ...
    for (dayIdRefsIndex = 0; dayIdRefsIndex < dayCount; dayIdRefsIndex++)
    {
        ...
        for (normalDayIndex = 0; normalDayIndex < EMBER_AF_PLUGIN_CALENDAR_COMMON_DAY_PROFILE_MAX; normalDayIndex++)
        {
            if (normalDays[normalDayIndex].id == dayId)
            {
                ...
                emberAfCalendarClusterPrintln("           normalDayIndexes[%d]=%d", dayIdRefsIndex, normalDayIndex);
            }
        }
    }
                                          ...
                                          uint8_t dayIdRefSun)
{
    ...
    {
        emberAfCalendarClusterPrintln("WeekId must be in the range of 0 to %d",
                                         ...
                                         uint8_t * unknownWeekIdSeasonsMask)
{
    ...
    *unknownWeekIdSeasonsMask = 0;
    ...
    for (seasonEntryIndex = 0; seasonEntryIndex < seasonEntryCount; seasonEntryIndex++)
    {
        ...
        uint8_t weekId;
        emberAfGetDate(seasonsEntries, seasonEntryIndex * SEASON_ENTRY_SIZE, seasonsEntriesLength, &startDate);
        weekId = emberAfGetInt8u(seasonsEntries, seasonEntryIndex * SEASON_ENTRY_SIZE + sizeof(EmberAfDate), seasonsEntriesLength);
...
bool emberAfCalendarServerSetSeasonsInfo(uint8_t index, uint8_t seasonId, EmberAfDate startDate, uint8_t weekIdRef)
{
    if (index >= EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS)
    {
        emberAfCalendarClusterPrintln("Index must be in the range of 0 to %d", EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS - 1);
    ...
    {
        emberAfCalendarClusterPrintln("SeasonId must be in the range of 0 to %d",
                                      EMBER_AF_PLUGIN_CALENDAR_COMMON_SEASON_PROFILE_MAX - 1);
        return false;
    }
