...
uint8_t calendarName0[EMBER_AF_PLUGIN_CALENDAR_MAX_CALENDAR_NAME_LENGTH + 1] = { 0x0C, 'a', 'b', 'c', 'd', 'e', 'f',
                                                                                 'g',  'h', 'i', 'j', 'k', 'l' };
uint8_t calendarName1[EMBER_AF_PLUGIN_CALENDAR_MAX_CALENDAR_NAME_LENGTH + 1] = { 0x0C, 'm', 'n', 'o', 'p', 'q', 'r',
                                                                                 's',  't', 'u', 'v', 'w', 'x' };

const char * calendarTypeStrings[] = {
    "Delivered", "Received", "Delivered and Received", "Friendly Credit", "Auxilliary Load Switch",
};
...
{
    emberAfCalendarClusterPrintln("Selected Calendar Index: %d", calendarIndexForPrinting);
...
static void printScheduleEntryData(uint8_t calendarType, uint8_t data)
{
    ...
    case EMBER_ZCL_CALENDAR_TYPE_FRIENDLY_CREDIT_CALENDAR:
        emberAfCalendarClusterPrint("Friendly credit: %p", (data == 0 ? "not available" : "enabled"));
        break;
    case EMBER_ZCL_CALENDAR_TYPE_AUXILLIARY_LOAD_SWITCH_CALENDAR:
        ...
        break;
    default:
        emberAfCalendarClusterPrint("Invalid calendar type (%d), data: %d", calendarType, data);
        break;
    }
}

static void printTime(const char * prefix, uint32_t zigbeeUtcTime)
{
    EmberAfTimeStruct time;
    emberAfFillTimeStructFromUtc(zigbeeUtcTime, &time);
    emberAfCalendarClusterPrint("%p", prefix);
    emberAfCalendarClusterPrintln("%4X (%d/%p%d/%p%d, %p%d:%p%d:%p%d)", zigbeeUtcTime, time.year, (time.month < 10 ? "0" : ""),
                                  time.month, (time.day < 10 ? "0" : ""), time.day, (time.hours < 10 ? "0" : ""), time.hours,
                                  (time.minutes < 10 ? "0" : ""), time.minutes, (time.seconds < 10 ? "0" : ""), time.seconds);
}

static void printScheduleEntry(const EmberAfCalendarDayScheduleEntryStruct * scheduleEntry, uint8_t calendarType)
{
    uint8_t hours   = 0;
    uint8_t minutes = 0;
    ...
    emberAfCalendarClusterPrint("%s%d:%s%d - ", (hours < 10 ? "0" : ""), hours, (minutes < 10 ? "0" : ""), minutes);
    printScheduleEntryData(calendarType, scheduleEntry->data);
    emberAfCalendarClusterPrintln("");
}

static void printDayProfile(const EmberAfCalendarDayStruct * day, uint8_t calendarType)
{
    uint8_t i;
    for (i = 0; i < day->numberOfScheduleEntries; i++)
    {
        emberAfCalendarClusterPrint("  ");
        printScheduleEntry(&(day->scheduleEntries[i]), calendarType);
    }
}

static void printSpecialDayProfile(const EmberAfCalendarSpecialDayStruct * specialDay, const EmberAfCalendarDayStruct * normalDays)
{
...
static void printDayProfiles(const EmberAfCalendarStruct * calendar, bool printSpecialDays)
{
    uint8_t i;
    uint8_t max = (printSpecialDays ? calendar->numberOfSpecialDayProfiles : calendar->numberOfDayProfiles);
    for (i = 0; i < max; i++)
    {
        if (printSpecialDays)
        {
        ...
        {
            emberAfCalendarClusterPrintln("Day ID: %d - Schedule", calendar->normalDays[i].id);
            printDayProfile(&(calendar->normalDays[i]), calendar->calendarType);
        }
    }
}
...
static void printCalendar(const EmberAfCalendarStruct * calendar)
{
    ...
    emberAfCalendarClusterPrintln(
        "Type:            %d (%p)", calendar->calendarType,
        (calendar->calendarType <= MAX_CALENDAR_TYPE ? calendarTypeStrings[calendar->calendarType] : "???"));
...
void emberAfPluginCalendarCommonSelectCommand(void)
{
    ...
    {
        emberAfCalendarClusterPrintln("Error: Invalid index.  Max is %d.", EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS - 1);
        return;
    }
...
void emberAfPluginCalendarCommonPrintWeeksCommand(void)
{
    ...
    for (i = 0; i < calendars[calendarIndexForPrinting].numberOfWeekProfiles; i++)
    {
        ...
        emberAfCalendarClusterPrintln("    Monday    (Day ID Ref):      0x%x",
                                      calendars[calendarIndexForPrinting].normalDays[week->normalDayIndexes[0]].id);
        emberAfCalendarClusterPrintln("    Tuesday   (Day ID Ref):      0x%x",
                                      calendars[calendarIndexForPrinting].normalDays[week->normalDayIndexes[1]].id);
        emberAfCalendarClusterPrintln("    Wednesday (Day ID Ref):      0x%x",
                                      calendars[calendarIndexForPrinting].normalDays[week->normalDayIndexes[2]].id);
        emberAfCalendarClusterPrintln("    Thursday  (Day ID Ref):      0x%x",
                                      calendars[calendarIndexForPrinting].normalDays[week->normalDayIndexes[3]].id);
        emberAfCalendarClusterPrintln("    Friday    (Day ID Ref):      0x%x",
                                      calendars[calendarIndexForPrinting].normalDays[week->normalDayIndexes[4]].id);
        emberAfCalendarClusterPrintln("    Saturday  (Day ID Ref):      0x%x",
                                      calendars[calendarIndexForPrinting].normalDays[week->normalDayIndexes[5]].id);
        emberAfCalendarClusterPrintln("    Sunday    (Day ID Ref):      0x%x",
                                      calendars[calendarIndexForPrinting].normalDays[week->normalDayIndexes[6]].id);
    }
}
...
static void setupTestWeeks(EmberAfCalendarStruct * calendar, uint8_t offset)
{
    ...
    for (i = 0; i < calendar->numberOfWeekProfiles; i++)
    {
        ...
        {
            calendar->weeks[i].normalDayIndexes[j] =
                (j < EMBER_AF_PLUGIN_CALENDAR_COMMON_DAY_PROFILE_MAX ? j : EMBER_AF_PLUGIN_CALENDAR_COMMON_INVALID_ID);
        }
    }
}
...
void emberAfPluginCalendarCommonSetupTestCalendarsCommand(void)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        emberAfCopyString(calendars[i].name, (i == 0 ? calendarName0 : calendarName1),
...
void emberAfPluginCalendarCommonPrintSeasonsCommand(void)
{
    ...
    for (i = 0; i < calendars[calendarIndexForPrinting].numberOfSeasons; i++)
    {
        ...
        else
        {
            ...
            emberAfCalendarClusterPrintln(
                "  Week ID: %d",
                calendars[calendarIndexForPrinting].weeks[calendars[calendarIndexForPrinting].seasons[i].weekIndex].id);
        }
    }
}
...
void emberAfPluginCalendarCommonLoadSimpleCalendarCommand(void)
{
    ...
    if (index < EMBER_AF_PLUGIN_CALENDAR_COMMON_TOTAL_CALENDARS)
    {
        ...
        length = emberCopyStringArgument(6, calendars[index].name + 1, EMBER_AF_PLUGIN_CALENDAR_MAX_CALENDAR_NAME_LENGTH, false);
...
void emberAfCalendarCommonCliSetCalInfo(void)
{
    ...
    uint8_t length = emberCopyStringArgument(6, calName + 1, EMBER_AF_PLUGIN_CALENDAR_MAX_CALENDAR_NAME_LENGTH, false);
    calName[0]     = length;
    emberAfCalendarCommonSetCalInfo((uint8_t) emberUnsignedCommandArgument(0), (uint32_t) emberUnsignedCommandArgument(1),
                                    (uint32_t) emberUnsignedCommandArgument(2), (uint32_t) emberUnsignedCommandArgument(3),
                                    (uint32_t) emberUnsignedCommandArgument(4), (uint8_t) emberUnsignedCommandArgument(5), calName,
                                    (uint8_t) emberUnsignedCommandArgument(7), (uint8_t) emberUnsignedCommandArgument(8),
...
{
    uint8_t calName[EMBER_AF_PLUGIN_CALENDAR_MAX_CALENDAR_NAME_LENGTH + 1];
    uint8_t length = emberCopyStringArgument(5, calName + 1, EMBER_AF_PLUGIN_CALENDAR_MAX_CALENDAR_NAME_LENGTH, false);
    calName[0]     = length;
    emberAfCalendarCommonAddCalInfo((uint32_t) emberUnsignedCommandArgument(0), (uint32_t) emberUnsignedCommandArgument(1),
                                    (uint32_t) emberUnsignedCommandArgument(2), (uint32_t) emberUnsignedCommandArgument(3),
                                    (uint8_t) emberUnsignedCommandArgument(4), calName, (uint8_t) emberUnsignedCommandArgument(6),
                                    (uint8_t) emberUnsignedCommandArgument(7), (uint8_t) emberUnsignedCommandArgument(8));
}
...
{
    emberAfCalendarCommonSetDayProfInfo((uint8_t) emberUnsignedCommandArgument(0), (uint8_t) emberUnsignedCommandArgument(1),
                                        (uint8_t) emberUnsignedCommandArgument(2), (uint16_t) emberUnsignedCommandArgument(3),
...
{
    emberAfCalendarServerSetWeekProfInfo((uint8_t) emberUnsignedCommandArgument(0), (uint8_t) emberUnsignedCommandArgument(1),
                                         (uint8_t) emberUnsignedCommandArgument(2), (uint8_t) emberUnsignedCommandArgument(3),
                                         (uint8_t) emberUnsignedCommandArgument(4), (uint8_t) emberUnsignedCommandArgument(5),
...
void emberAfCalendarCommonCliSetSeasonsInfo(void)
{
    ...
    emberAfCalendarServerSetSeasonsInfo((uint8_t) emberUnsignedCommandArgument(0), (uint8_t) emberUnsignedCommandArgument(1), date,
