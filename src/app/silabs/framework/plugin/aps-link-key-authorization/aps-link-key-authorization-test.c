...
static void testApsEnableAndExemptCount(bool expectedEnableState, uint8_t expectedCount)
{
    ...
    expectComparisonDecimal(expectedEnableState, apsAuthorizationEnabled, "expected APS authorization enable state",
                            "observed APS authorization enable state");

    expectComparisonDecimal(expectedCount, exemptClusterListCount, "expected exempt cluster list count",
...
static void emptyListTest(void)
{
    ...
    expectMessage(EMBER_SUCCESS == emberAfApsLinkKeyAuthorizationAddClusterExempted(0xACDC),
                  "ACDC cannot be added to the exempt list!\n");
    expectMessage(EMBER_SUCCESS == emberAfApsLinkKeyAuthorizationAddClusterExempted(0xBEEF),
                  "BEEF cannot be added to the exempt list!\n");
...
static void addQueryRemoveExemptionTest(void)
{
    ...
    for (exemptClusterIdx = 0; exemptClusterIdx < (EMBER_AF_PLUGIN_APS_LINK_KEY_AUTHORIZATION_MAX_EXEMPT_CLUSTERS + 1);
         exemptClusterIdx++)
    {
    ...
    expectMessage(EMBER_TABLE_FULL ==
    ...
    for (exemptClusterIdx = 0; exemptClusterIdx < EMBER_AF_PLUGIN_APS_LINK_KEY_AUTHORIZATION_MAX_EXEMPT_CLUSTERS;
         exemptClusterIdx++)
    {
        isClusterExempted = emberAfApsLinkKeyAuthorizationIsClusterExempted(exemptClusterIdx);
        expectComparisonDecimal(true, isClusterExempted, "expected cluster exemption", "observer cluster exemption");
    }

    // Test for clusters existing and missing
    expectMessage(true == emberAfApsLinkKeyAuthorizationIsClusterExempted(0), "Cluster is supposed to be in the exempt list !\n");
    expectMessage(true == emberAfApsLinkKeyAuthorizationIsClusterExempted(10), "Cluster is supposed to be in the exempt list !\n");
    expectMessage(
        false ==
