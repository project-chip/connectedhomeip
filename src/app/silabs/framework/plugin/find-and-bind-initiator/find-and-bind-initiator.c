...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Initiator routines for the Find and Bind
                                                                               *Initiator plugin.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static void processClusterList(bool clientList, uint8_t clusterCount, const EmberAfClusterId * clusters, EmberStatus * status);

static EmberStatus writeSimpleDescriptorResponse(EmberBindingTableEntry * entry, uint8_t * groupName);
...
#define targetInfosAreEqual(targetInfo1, targetInfo2)                                                                              \
    ((targetInfo1).nodeId == (targetInfo2).nodeId && (targetInfo1).endpoint == (targetInfo2).endpoint)

#define currentTargetInfoIsNull() (currentTargetResponsesIndex == INVALID_TARGET_RESPONSES_INDEX)

#define currentTargetInfoNodeId (targetResponses[currentTargetResponsesIndex].nodeId)
#define currentTargetInfoEndpoint (targetResponses[currentTargetResponsesIndex].endpoint)
#define currentTargetInfoIeeeAddr (currentTargetResponseIeeeAddr)
...
void emberAfPluginFindAndBindInitiatorCheckTargetResponsesEventHandler(void)
{
    ...
    case STATE_NONE:
    default:
        debugPrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_PLUGIN_NAME, "Bad state", state);
...
bool emberAfIdentifyClusterIdentifyQueryResponseCallback(uint16_t timeout)
{
    ...
    if (currentCommand->apsFrame->destinationEndpoint == initiatorEndpoint)
    {
        ...
        for (i = 0; (i < targetResponsesReceived && !targetInfosAreEqual(targetInfo, targetResponses[i])); i++)
        {
            ; // pass
        }
        if (i < EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_TARGET_RESPONSES_COUNT && i == targetResponsesReceived)
        {
...
static EmberStatus broadcastIdentifyQuery(void)
{
    ...
    debugPrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_PLUGIN_NAME, "Broadcast", status);
...
static EmberStatus sendIeeeAddrRequest(void)
{
    ...
    status = emberAfFindIeeeAddress(currentTargetInfoNodeId, handleIeeeAddrResponse);

    debugPrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_PLUGIN_NAME, "Ieee request", status);
...
static void handleIeeeAddrResponse(const EmberAfServiceDiscoveryResult * result)
{
    debugPrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_PLUGIN_NAME, "Ieee response", result->status);

    if (currentTargetInfoIsNull() || (result->status != EMBER_AF_UNICAST_SERVICE_DISCOVERY_COMPLETE_WITH_RESPONSE))
    {
        cleanupAndStop(EMBER_ERR_FATAL);
        return;
    }

    MEMCOPY(currentTargetInfoIeeeAddr, result->responseData, EUI64_SIZE);
...
static EmberStatus sendSimpleDescriptorRequest()
{
    ...
    status =
        emberAfFindClustersByDeviceAndEndpoint(currentTargetInfoNodeId, currentTargetInfoEndpoint, handleSimpleDescriptorResponse);

    debugPrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_PLUGIN_NAME, "Descriptor request", status);
...
static void handleSimpleDescriptorResponse(const EmberAfServiceDiscoveryResult * result)
{
    ...
    debugPrintln("%p: %p: 0x%X", EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_PLUGIN_NAME, "Descriptor response", result->status);

    if (currentTargetInfoIsNull() || (result->status != EMBER_AF_UNICAST_SERVICE_DISCOVERY_COMPLETE_WITH_RESPONSE) || !clusterList)
    {
        cleanupAndStop(EMBER_ERR_FATAL);
        return;
    }
    ...
    processClusterList(false, // server list
                       clusterList->inClusterCount, clusterList->inClusterList, &status);
    processClusterList(true, // client list
                       clusterList->outClusterCount, clusterList->outClusterList, &status);
...
static void processClusterList(bool clientList, uint8_t clusterCount, const EmberAfClusterId * clusters, EmberStatus * status)
{
    ...
    for (i = 0; (*status != EMBER_TABLE_FULL) && (i < clusterCount); i++)
    {
        ...
        if (((clientList && emberAfContainsServer(initiatorEndpoint, clusters[i])) ||
            ...
            emberAfPluginFindAndBindInitiatorBindTargetCallback(currentTargetInfoNodeId, &bindingEntry, groupName))
        {
            *status = writeSimpleDescriptorResponse(&bindingEntry, groupName);
            debugPrintln("%p: write cluster 0x%2X: 0x%X", EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_PLUGIN_NAME, clusters[i],
                         *status);
        }
    }
}

static EmberStatus writeSimpleDescriptorResponse(EmberBindingTableEntry * newEntry, uint8_t * groupName)
{
    ...
    for (i = 0, goodIndex = bindingTableSize; i < bindingTableSize; i++)
    {
        if (emberGetBinding(i, &oldEntry) == EMBER_SUCCESS)
        {
            ...
            else
            {
                ...
                if (oldEntry.clusterId == newEntry->clusterId && oldEntry.remote == newEntry->remote &&
                    oldEntry.type == newEntry->type &&
                    !MEMCOMPARE(oldEntry.identifier, newEntry->identifier,
                                (oldEntry.type == EMBER_MULTICAST_BINDING ? 2 // group id
                                                                          : EUI64_SIZE)))
                {
                    status = EMBER_SUCCESS;
                }
    ...
    if (goodIndex < bindingTableSize)
    {
        ...
        {
            uint16_t groupId = HIGH_LOW_TO_INT(newEntry->identifier[1], newEntry->identifier[0]);
...
static void cleanupAndStop(EmberStatus status)
{
    debugPrintln("%p: Stop. Status: 0x%X. State: 0x%X", EMBER_AF_PLUGIN_FIND_AND_BIND_INITIATOR_PLUGIN_NAME, status, state);
