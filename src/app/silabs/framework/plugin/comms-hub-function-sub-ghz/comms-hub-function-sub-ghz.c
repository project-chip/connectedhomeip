...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief Routines for the Comms Hub Function Sub Ghz
                                                                               *plugin.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
static bool isChildSubGhz(EmberEUI64 eui64)
{
    ...
    for (i = 0; i < childTableSize; ++i)
    {
        ...
        if (emberAfGetChildData(i, &childData) == EMBER_SUCCESS && MEMCOMPARE(childData.eui64, eui64, EUI64_SIZE) == 0)
        {
...
{
    uint8_t mirrorIndex;
    for (mirrorIndex = 0; mirrorIndex < sizeof channelChangeState / sizeof channelChangeState[0]; mirrorIndex++)
    {
        if (channelChangeState[mirrorIndex] != CHANNEL_CHANGE_STATE_IGNORED && channelChangeState[mirrorIndex] != expectedState)
        {
 ...
 */
static void updateStateForAllMirrors(ChannelChangeState newState, bool includeIgnored)
{
    uint8_t mirrorIndex;
    for (mirrorIndex = 0; mirrorIndex < sizeof channelChangeState / sizeof channelChangeState[0]; mirrorIndex++)
    {
        if (channelChangeState[mirrorIndex] != CHANNEL_CHANGE_STATE_IGNORED || includeIgnored)
        {
 ...
 */
static EmberAfPluginCommsHubFunctionStatus updateFlagsAndStateForAllMirrors(uint32_t setMask, ChannelChangeState newState)
{
    uint8_t mirrorIndex;
    for (mirrorIndex = 0; mirrorIndex < sizeof channelChangeState / sizeof channelChangeState[0]; mirrorIndex++)
    {
        ...
        {
            EmberAfPluginCommsHubFunctionStatus status = emAfUpdateFunctionalNotificationFlagsByEndpoint(
                mirrorIndex + EMBER_AF_PLUGIN_METER_MIRROR_ENDPOINT_START, 0xFFFFFFFF, setMask);
 ...
 */
static bool checkStateAndClearFlag(uint8_t mirrorEndpoint, ChannelChangeState expectedState, ChannelChangeState newState,
                                   uint32_t resetMask)
{
    const int8_t mirrorIndex = mirrorEndpoint - EMBER_AF_PLUGIN_METER_MIRROR_ENDPOINT_START;
    if (channelChangeState[mirrorIndex] != expectedState)
    {
        return false; // not the flag we are expecting
    }
    if (emAfUpdateFunctionalNotificationFlagsByEndpoint(mirrorEndpoint, resetMask, 0) == EMBER_AF_CHF_STATUS_SUCCESS)
    {
        channelChangeState[mirrorIndex] = newState;
    }
...
static EmberAfStatus executeChannelChange(void)
{
    ...
    status =
        emberAfReadAttribute(ZCL_SUB_GHZ_CLUSTER_ENDPOINT, ZCL_SUB_GHZ_CLUSTER_ID, ZCL_SUB_GHZ_CLUSTER_CHANNEL_CHANGE_ATTRIBUTE_ID,
                             CLUSTER_MASK_SERVER, (uint8_t *) &pageChannelMask, sizeof pageChannelMask, &dataType);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        return status;
    }

    status = emberEnergyScanRequest(EMBER_SLEEPY_BROADCAST_ADDRESS, pageChannelMask, 0xFE, 0);
    if (status != EMBER_ZCL_STATUS_SUCCESS)
    {
        return status;
    }
    ...
    pageChannelMask = 0;
    status =
        emberAfWriteAttribute(ZCL_SUB_GHZ_CLUSTER_ENDPOINT, ZCL_SUB_GHZ_CLUSTER_ID, ZCL_SUB_GHZ_CLUSTER_CHANNEL_CHANGE_ATTRIBUTE_ID,
                              CLUSTER_MASK_SERVER, (uint8_t *) &pageChannelMask, ZCL_BITMAP32_ATTRIBUTE_TYPE);
    return status;
}
...
executeChannelChangeAndResetStates(EmberAfPluginCommsHubFunctionSubGhzChannelChangeOutcome outcomeIfSuccess,
                                   EmberAfPluginCommsHubFunctionSubGhzChannelChangeOutcome outcomeIfFailed, EmberAfStatus * status)
{
    updateStateForAllMirrors(CHANNEL_CHANGE_STATE_IDLE, true);

    *status = executeChannelChange();
    return *status == EMBER_ZCL_STATUS_SUCCESS ? outcomeIfSuccess : outcomeIfFailed;
}
...
EmberAfPluginCommsHubFunctionStatus emberAfCommsHubFunctionSubGhzStartChannelChangeSequence(void)
{
    ...
    for (mirrorIndex = 0; mirrorIndex < sizeof channelChangeState / sizeof channelChangeState[0]; mirrorIndex++)
    {
        const uint8_t mirrorEndpoint = mirrorIndex + EMBER_AF_PLUGIN_METER_MIRROR_ENDPOINT_START;
        ...
        if (emberAfPluginMeterMirrorIsMirrorUsed(mirrorEndpoint) &&
            emberAfPluginMeterMirrorGetEui64ByEndpoint(mirrorEndpoint, mirrorEui) && isChildSubGhz(mirrorEui))
        {
        ...
        {
            emberAfPluginCommsHubFunctionPrintln("unallocated, unknown or not %p", "sub-GHz device");
    ...
    emberAfEventControlSetDelay(&emberAfPluginCommsHubFunctionSubGhzGsmeTimeOutEventControl,
                                EMBER_AF_PLUGIN_COMMS_HUB_FUNCTION_SUB_GHZ_GSME_WAKE_UP_PERIOD * MILLISECOND_TICKS_PER_MINUTE);
    return status;
}
...
EmberAfPluginCommsHubFunctionStatus emberAfCommsHubFunctionSubGhzCompleteChannelChangeSequence(uint32_t pageChannelMask)
{
    ...
    {
        EmberAfStatus status;
        const EmberAfPluginCommsHubFunctionSubGhzChannelChangeOutcome outcome = executeChannelChangeAndResetStates(
            EMBER_AF_CHF_SUB_GHZ_CHANNEL_CHANGE_SUCCESS_NORMAL, EMBER_AF_CHF_SUB_GHZ_CHANNEL_CHANGE_FAIL_NORMAL, &status);
    ...
    if (emberAfWriteAttribute(ZCL_SUB_GHZ_CLUSTER_ENDPOINT, ZCL_SUB_GHZ_CLUSTER_ID, ZCL_SUB_GHZ_CLUSTER_CHANNEL_CHANGE_ATTRIBUTE_ID,
                              CLUSTER_MASK_SERVER, (uint8_t *) &pageChannelMask,
...
void emAfCommsHubFunctionSubGhzReadAttributeNotification(EmberNodeId source, EmberAfClusterId clusterId, uint16_t attrId)
{
    ...
    if (clusterId == ZCL_SIMPLE_METERING_CLUSTER_ID && attrId == ZCL_FUNCTIONAL_NOTIFICATION_FLAGS_ATTRIBUTE_ID)
    {
        ...
        {
            emberAfPluginCommsHubFunctionPrintln("CHF: GSME 0x%2x reading Functional Notification flags", source);
            // There are two notification flags. Work out which one this is about.
            if (checkStateAndClearFlag(mirrorEndpoint, CHANNEL_CHANGE_STATE_ENERGY_SCAN_PENDING_FLAG_SET,
                                       CHANNEL_CHANGE_STATE_ENERGY_SCAN_PENDING_FLAG_READ,
            ...
            else if (checkStateAndClearFlag(mirrorEndpoint, CHANNEL_CHANGE_STATE_CHANNEL_CHANGE_PENDING_FLAG_AND_ATTRIBUTE_SET,
                                            CHANNEL_CHANGE_STATE_CHANNEL_CHANGE_PENDING_FLAG_READ,
    ...
    else if (clusterId == ZCL_SUB_GHZ_CLUSTER_ID && attrId == ZCL_SUB_GHZ_CLUSTER_CHANNEL_CHANGE_ATTRIBUTE_ID)
    {
        ...
        if (emberLookupEui64ByNodeId(source, eui64) == EMBER_SUCCESS &&
            emberAfPluginMeterMirrorGetEndpointByEui64(eui64, &mirrorEndpoint))
        {
            emberAfPluginCommsHubFunctionPrintln("CHF: GSME 0x%2x reading Sub-GHz Cluster attributes", source);
            const int8_t mirrorIndex = mirrorEndpoint - EMBER_AF_PLUGIN_METER_MIRROR_ENDPOINT_START;
...
void emberAfPluginCommsHubFunctionSubGhzGsmeTimeOutEventHandler(void)
{
    ...
    if (checkStateForAllMirrors(CHANNEL_CHANGE_STATE_CHANNEL_CHANGE_ATTRIBUTE_READ))
    {
        ...
        outcome = executeChannelChangeAndResetStates(EMBER_AF_CHF_SUB_GHZ_CHANNEL_CHANGE_SUCCESS_NORMAL,
                                                     EMBER_AF_CHF_SUB_GHZ_CHANNEL_CHANGE_FAIL_NORMAL, &status);
    }
    else if (checkStateForAnyMirror(CHANNEL_CHANGE_STATE_CHANNEL_CHANGE_PENDING_FLAG_AND_ATTRIBUTE_SET))
    {
        ...
        outcome = executeChannelChangeAndResetStates(EMBER_AF_CHF_SUB_GHZ_CHANNEL_CHANGE_SUCCESS_TIMEOUT,
                                                     EMBER_AF_CHF_SUB_GHZ_CHANNEL_CHANGE_FAIL_TIMEOUT, &status);
    }
