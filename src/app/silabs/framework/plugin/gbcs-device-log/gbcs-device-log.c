...
#define EMBER_AF_PLUGIN_GBCS_DEVICE_LOG_NULL_EUI                                                                                   \
    {                                                                                                                              \
        0, 0, 0, 0, 0, 0, 0, 0                                                                                                     \
    }
...
bool emberAfPluginGbcsDeviceLogStore(EmberEUI64 deviceId, EmberAfGBCSDeviceLogInfo * deviceInfo)
{
    ...
    emberAfPluginGbcsDeviceLogPrintln("%p%p%p", "Error: ", "Cannot add device to GBCS Device Log: ", "Too many entries");
    return false;
}
...
bool emberAfPluginGbcsDeviceLogRemove(EmberEUI64 deviceId)
{
    ...
    emberAfPluginGbcsDeviceLogPrintln("%p%p%p", "Error: ", "Cannot remove device from GBCS Device Log: ", "Device does not exist");
    return false;
}
...
bool emberAfPluginGbcsDeviceLogGet(EmberEUI64 deviceId, EmberAfGBCSDeviceLogInfo * deviceInfo)
{
    ...
    emberAfPluginGbcsDeviceLogPrintln("%p%p%p",
                                      "Error: ", "Cannot retrieve device info from GBCS Device Log: ", "Device does not exist");
    return false;
}
...
bool emberAfPluginGbcsDeviceLogExists(EmberEUI64 deviceId, EmberAfGBCSDeviceType deviceType)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_GBCS_DEVICE_LOG_DEVICE_LOG_SIZE; i++)
    {
        if (deviceLogEntryInUse(&deviceLog[i]) && (deviceLog[i].deviceInfo.deviceType == deviceType) &&
...
static uint8_t findDeviceLogEntry(EmberEUI64 deviceId)
{
    ...
    for (i = 0; i < EMBER_AF_PLUGIN_GBCS_DEVICE_LOG_DEVICE_LOG_SIZE; i++)
    {
        if (deviceLogEntryInUse(&deviceLog[i]) && (MEMCOMPARE(deviceId, deviceLog[i].deviceId, EUI64_SIZE) == 0))
        {
            return i;
        }
    }
