...
static void addOrSubtractChannel(uint8_t maskToAddTo, uint8_t channelToAdd, bool operationIsAdd)
{
    if (channelToAdd < EMBER_MIN_802_15_4_CHANNEL_NUMBER || channelToAdd > EMBER_MAX_802_15_4_CHANNEL_NUMBER)
    {
    ...
    else if (maskToAddTo == 1)
    {
        ...
        emberAfCorePrintln("%p mask now 0x%4X", "Primary", emAfPluginNetworkSteeringPrimaryChannelMask);
    }
    else if (maskToAddTo == 2)
    {
        ...
        emberAfCorePrintln("%p mask now 0x%4X", "Secondary", emAfPluginNetworkSteeringSecondaryChannelMask);
    }
    else
    {
...
void emberAfPluginNetworkSteeringChannelAddOrSubtractCommand(void)
{
    ...
    addOrSubtractChannel(maskToAddTo, channelToAdd, operationIsAdd);
}
...
void emberAfPluginNetworkSteeringChannelSetCommand(void)
{
    ...
    uint32_t * mask =
        (maskIsPrimary ? &emAfPluginNetworkSteeringPrimaryChannelMask : &emAfPluginNetworkSteeringSecondaryChannelMask);

    emberAfCorePrintln("%p: Set %p mask to 0x%4X", emAfNetworkSteeringPluginName, (maskIsPrimary ? "primary" : "secondary"),
                       (*mask = value));
}
...
{
    emberAfCorePrintln("%p: %p:", emAfNetworkSteeringPluginName, "Status");

    emberAfCorePrintln("Channel mask:");
    emberAfCorePrint("    (1) 0x%4X [", emAfPluginNetworkSteeringPrimaryChannelMask);
    emberAfPrintChannelListFromMask(emAfPluginNetworkSteeringPrimaryChannelMask);
    emberAfCorePrintln("]");
    emberAfCorePrint("    (2) 0x%4X [", emAfPluginNetworkSteeringSecondaryChannelMask);
    emberAfPrintChannelListFromMask(emAfPluginNetworkSteeringSecondaryChannelMask);
    emberAfCorePrintln("]");

    emberAfCorePrintln("State: 0x%X (%p)", emAfPluginNetworkSteeringState,
                       emAfPluginNetworkSteeringStateNames[emAfPluginNetworkSteeringState]);
#ifndef EMBER_AF_PLUGIN_NETWORK_STEERING_OPTIMIZE_SCANS
    emberAfCorePrintln("Pan ID index: %d", emAfPluginNetworkSteeringPanIdIndex);
#endif // #ifndef EMBER_AF_PLUGIN_NETWORK_STEERING_OPTIMIZE_SCANS
    emberAfCorePrintln("Current channel: %d", emAfPluginNetworkSteeringGetCurrentChannel());
    emberAfCorePrintln("Total beacons: %d", emAfPluginNetworkSteeringTotalBeacons);
    emberAfCorePrintln("Join attempts: %d", emAfPluginNetworkSteeringJoinAttempts);
    emberAfCorePrintln("Network state: 0x%X", emberAfNetworkState());
}
...
{
    emberAfCorePrintln("%p: %p: 0x%X", emAfNetworkSteeringPluginName, "Stop", emberAfPluginNetworkSteeringStop());
}
