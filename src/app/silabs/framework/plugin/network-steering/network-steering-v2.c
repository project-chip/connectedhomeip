...
/***************************************************************************/ /**
                                                                               * @file
                                                                               * @brief This file implements the Zigbee 3.0 network
                                                                               *steering procedure using only one 15.4 scan.
                                                                               ******************************************************************************/
...
#define EMBER_AF_PLUGIN_NETWORK_STEERING_CHANNEL_MASK (BIT32(11) | BIT32(14))
#endif
...
EmberAfPluginNetworkSteeringJoiningState emAfPluginNetworkSteeringState = EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_NONE;
...
#define SECONDARY_CHANNEL_MASK                                                                                                     \
    ((EMBER_ALL_802_15_4_CHANNELS_MASK ^ EMBER_AF_PLUGIN_NETWORK_STEERING_CHANNEL_MASK) & EMBER_ALL_802_15_4_CHANNELS_MASK)
...
static const EmberKeyData defaultLinkKey = { { 0x5A, 0x69, 0x67, 0x42, 0x65, 0x65, 0x41, 0x6C, 0x6C, 0x69, 0x61, 0x6E, 0x63, 0x65,
                                               0x30, 0x39 } };
static const EmberKeyData distributedTestKey = { { 0xD0, 0xD1, 0xD2, 0xD3, 0xD4, 0xD5, 0xD6, 0xD7, 0xD8, 0xD9, 0xDA, 0xDB, 0xDC,
                                                   0xDD, 0xDE, 0xDF } };
...
uint32_t emAfPluginNetworkSteeringPrimaryChannelMask   = EMBER_AF_PLUGIN_NETWORK_STEERING_CHANNEL_MASK;
uint32_t emAfPluginNetworkSteeringSecondaryChannelMask = SECONDARY_CHANNEL_MASK;
...
#define randomJitterMS()                                                                                                           \
    ((emberGetPseudoRandomNumber() % (FINISH_STEERING_JITTER_MAX_MS - FINISH_STEERING_JITTER_MIN_MS)) +                            \
...
EmberAfPluginNetworkSteeringOptions emAfPluginNetworkSteeringOptionsMask = EMBER_AF_PLUGIN_NETWORK_STEERING_OPTIONS_NONE;
...
void tryToJoinNetwork()
{
    ...
    emberAfCorePrintln("Examining beacon on channel %d with panId 0x%2X", BEACON_ITERATOR.beacon.channel,
    ...
    {
        if (0 != MEMCOMPARE(BEACON_ITERATOR.beacon.extendedPanId, gExtendedPanIdToFilterOn, COUNTOF(gExtendedPanIdToFilterOn)))
        {
    ...
    while ((!(CURRENT_CHANNEL_MASK & BIT32(BEACON_ITERATOR.beacon.channel))) || filteredOut)
    {
        ...
        filteredOut =
            (gFilterByExtendedPanId &&
             MEMCOMPARE(BEACON_ITERATOR.beacon.extendedPanId, gExtendedPanIdToFilterOn, COUNTOF(gExtendedPanIdToFilterOn)));
    }
    ...
    emberAfCorePrintln("%p joining 0x%2x on channel %d", PLUGIN_NAME, BEACON_ITERATOR.beacon.panId, BEACON_ITERATOR.beacon.channel);
    ...
    status       = emberJoinNetworkDirectly(nodeType, &BEACON_ITERATOR.beacon, radioTxPower,
                                      true); // clearBeaconsAfterNetworkUp
    emAfPluginNetworkSteeringJoinAttempts++;
    if (EMBER_SUCCESS != status)
    {
        emberAfCorePrintln("Error: %p could not attempt join: 0x%X", PLUGIN_NAME, status);
...
static uint32_t jitterTimeDelayMs()
{
    ...
    uint32_t jitterDelayMs =
        (emberGetPseudoRandomNumber() % (UPDATE_TC_LINK_KEY_JITTER_MAX_MS - UPDATE_TC_LINK_KEY_JITTER_MIN_MS + 1)) +
...
static EmberStatus scheduleScan(uint32_t channelMask)
{
    ...
    emberAfCorePrintln("%s: issuing scan on %s channels (mask 0x%4X)", PLUGIN_NAME,
                       channelMask == emAfPluginNetworkSteeringPrimaryChannelMask ? "primary" : "secondary", channelMask);
    return status;
}
...
HIDDEN void scanResultsHandler(EmberAfPluginScanDispatchScanResults * results)
{
    if (emberAfPluginScanDispatchScanResultsAreComplete(results) || emberAfPluginScanDispatchScanResultsAreFailure(results))
    {
        if (results->status != EMBER_SUCCESS)
        {
            emberAfCorePrintln("Error: Scan complete handler returned 0x%X", results->status);
        ...
        emberAfCorePrintln("%s scan complete. Beacons heard: %d", PLUGIN_NAME, emAfPluginNetworkSteeringTotalBeacons);
...
static EmberStatus goToNextState(void)
{
    ...
    if ((emAfPluginNetworkSteeringState == EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_PRIMARY_USE_ALL_KEYS) ||
        (emAfPluginNetworkSteeringState == EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_FINISHED))
    {
    ...
    if ((emAfPluginNetworkSteeringState == EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_SECONDARY_INSTALL_CODE) ||
        (emAfPluginNetworkSteeringState == EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_SECONDARY_USE_ALL_KEYS))
    {
...
static void cleanupAndStop(EmberStatus status)
{
    ...
    emberAfPluginNetworkSteeringCompleteCallback(status, emAfPluginNetworkSteeringTotalBeacons,
                                                 emAfPluginNetworkSteeringJoinAttempts, emAfPluginNetworkSteeringState);
...
{
    EmberStatus status;
    emberAfCorePrintln("%p State: %p", PLUGIN_NAME, emAfPluginNetworkSteeringStateNames[emAfPluginNetworkSteeringState]);

    status = setupSecurity();

    while (status != EMBER_SUCCESS)
    {
        emberAfCorePrintln("Error: %p could not setup security: 0x%X", PLUGIN_NAME, status);
...
static EmberStatus setupSecurity(void)
{
    ...
    bool tryAllKeys = ((emAfPluginNetworkSteeringState == EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_PRIMARY_USE_ALL_KEYS) ||
                       (emAfPluginNetworkSteeringState == EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_SECONDARY_USE_ALL_KEYS));

    state.bitmask = (EMBER_TRUST_CENTER_GLOBAL_LINK_KEY | EMBER_HAVE_PRECONFIGURED_KEY | EMBER_REQUIRE_ENCRYPTED_KEY |
                     EMBER_NO_FRAME_COUNTER_RESET |
                     (emAfPluginNetworkSteeringStateUsesInstallCodes() ? EMBER_GET_PRECONFIGURED_KEY_FROM_INSTALL_CODE : 0) |
                     (emAfPluginNetworkSteeringStateUsesDistributedKey() ? EMBER_DISTRIBUTED_TRUST_CENTER_MODE : 0));

    if (!emberAfPluginNetworkSteeringGetDistributedKeyCallback(&emberPluginNetworkSteeringDistributedKey))
    {
        MEMMOVE(emberKeyContents(&emberPluginNetworkSteeringDistributedKey), emberKeyContents(&distributedTestKey),
                EMBER_ENCRYPTION_KEY_SIZE);
    }
    MEMMOVE(emberKeyContents(&(state.preconfiguredKey)),
            (emAfPluginNetworkSteeringStateUsesDistributedKey() ? emberKeyContents(&emberPluginNetworkSteeringDistributedKey)
    ...
    extended = (EMBER_JOINER_GLOBAL_LINK_KEY | EMBER_EXT_NO_FRAME_COUNTER_RESET);
    ...
    if (tryAllKeys)
    {
        ...
        status = emberAddTransientLinkKey(wildcardEui, (EmberKeyData *) &defaultLinkKey);
        if (status == EMBER_SUCCESS)
        {
            status = emberAddTransientLinkKey(wildcardEui, (EmberKeyData *) &distributedTestKey);
        }
    }
...
EmberStatus emberAfPluginNetworkSteeringStart(void)
{
    ...
    if ((0 == emAfPluginNetworkSteeringPrimaryChannelMask) && (0 == emAfPluginNetworkSteeringSecondaryChannelMask))
    {
        return EMBER_INVALID_CALL;
    }

    if (emAfProIsCurrentNetwork() && (emAfPluginNetworkSteeringState == EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_NONE))
    {
    ...
    emberAfCorePrintln("%p: %p: 0x%X", emAfNetworkSteeringPluginName, "Start", status);
...
void emberAfPluginNetworkSteeringFinishSteeringEventHandler(void)
{
    ...
    if (emAfPluginNetworkSteeringStateVerifyTclk())
    {
        ...
        emberAfPluginUpdateTcLinkKeyStop();
        emberLeaveNetwork();
        emberAfCorePrintln("%p: %p", PLUGIN_NAME, "Key verification failed. Leaving network");
        cleanupAndStop(EMBER_ERR_FATAL);
    }
    ...
    else
    {
        ...
        status = emberAfPermitJoin(EMBER_AF_PLUGIN_NETWORK_STEERING_COMMISSIONING_TIME_S,
                                   true); // Broadcast permit join?
        emberAfCorePrintln("%p: %p: 0x%X", PLUGIN_NAME, "Broadcasting permit join", status);
        cleanupAndStop(status);
    }
}
...
void emberAfPluginUpdateTcLinkKeyStatusCallback(EmberKeyStatus keyStatus)
{
    ...
    {
        emberAfCorePrintln("%p: %p: 0x%X", PLUGIN_NAME, "Trust center link key update status", keyStatus);
        switch (keyStatus)
        {
...
void emAfPluginNetworkSteeringSetExtendedPanIdFilter(uint8_t * extendedPanId, bool turnFilterOn)
{
    ...
    MEMCOPY(gExtendedPanIdToFilterOn, extendedPanId, COUNTOF(gExtendedPanIdToFilterOn));
