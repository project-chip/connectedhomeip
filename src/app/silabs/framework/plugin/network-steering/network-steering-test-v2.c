...
static EmAfZigbeeProNetwork zigbeeNetwork                = { EMBER_ROUTER, EMBER_AF_SECURITY_PROFILE_Z3 };
...
{
    long * contents = functionCallCheck("getNodeTypeCallback", "i!", state);
    return contents[1];
}
...
{
    functionCallCheck("permitJoin", "ii", duration, broadcast);
    return EMBER_SUCCESS;
}
...
void emberAfPluginNetworkSteeringCompleteCallback(EmberStatus status, uint8_t totalBeacons, uint8_t joinAttempts,
                                                  uint8_t finalState)
{
    functionCallCheck("networkSteeringCompleteCallback", "iiii", status, totalBeacons, joinAttempts, finalState);
}
...
EmberStatus emberAfPluginScanDispatchScheduleScan(EmberAfPluginScanDispatchScanData * data)
{
    ...
    if (scriptDebug)
    {
        simPrint("scheduleScan with scanType 0x%02X and channelMask 0x%08X", data->scanType, data->channelMask);
    }
...
EmberStatus emberJoinNetworkDirectly(EmberNodeType nodeType, EmberBeaconData * beacon, int8_t radioTxPower,
                                     bool clearBeaconsAfterNetworkUp)
{
    functionCallCheck("joinNetworkDirectly",
                      "iiiii", // nodeType, target, panId, channel
                      nodeType,
                      beacon->sender, // sender of the beacon is the join target
                      beacon->panId, beacon->channel, clearBeaconsAfterNetworkUp);
...
EmberStatus emberSetInitialSecurityState(EmberInitialSecurityState * state)
{
    long * contents          = functionCallCheck("setInitialSecurityState",
                                        ...
                                        state->bitmask, makeMessage("s", &state->preconfiguredKey, EMBER_ENCRYPTION_KEY_SIZE));
...
#define addSetInitialSecurityStateCheck(mask, preconfiguredKey, returnStatus)                                                      \
    addSimpleCheck("setInitialSecurityState", "ip!i", (mask), (preconfiguredKey), (returnStatus))
...
#define addStartAction(expectedReturnStatus) addAction(&startActionType, (expectedReturnStatus))
...
PRINTER(scanResults)
{
    ...
    fprintf(stderr, " statusOrRssi: 0x%02X, channelOrLqi: %d, panId: 0x%04X", statusOrRssi, channelOrLqi, panId);
...
PERFORMER(scanResults)
{
    ...
    EmberAfPluginScanDispatchScanResults results = {
        ...
        .lqi     = channelOrLqi,
        .network = &network,
        .mask    = (EMBER_ACTIVE_SCAN | (complete ? EM_AF_PLUGIN_SCAN_DISPATCH_SCAN_RESULTS_MASK_COMPLETE : 0)),
    };
...
#define addScanResultsAction(statusOrRssi, channelOrLqi, panId, complete)                                                          \
    addAction(&scanResultsActionType, (statusOrRssi), (channelOrLqi), (panId), (complete))
...
#define addStackStatusAction(status) addAction(&stackStatusActionType, (status))
...
#define NETWORK_STEERING_TEST_CONFIGURED_KEY                                                                                       \
    {                                                                                                                              \
        {                                                                                                                          \
            0x63, 0x6F, 0x6E, 0x67, 0x72, 0x65, 0x73, 0x73, 0x20, 0x73, 0x74, 0x72, 0x65, 0x65, 0x74, 0x2E                         \
        }                                                                                                                          \
    }
...
static void init(void)
{
    ...
    centralizedKey = makeMessage("s", emberKeyContents(&zigbeeAlliance09), EMBER_ENCRYPTION_KEY_SIZE);
    distributedKey = makeMessage("s", emberKeyContents(&d0D1D2Etc), EMBER_ENCRYPTION_KEY_SIZE);
    configuredKey  = makeMessage("s", emberKeyContents(&congressStreet), EMBER_ENCRYPTION_KEY_SIZE);
}

#define SECURITY_BITMASK                                                                                                           \
    (EMBER_TRUST_CENTER_GLOBAL_LINK_KEY | EMBER_HAVE_PRECONFIGURED_KEY | EMBER_REQUIRE_ENCRYPTED_KEY | EMBER_NO_FRAME_COUNTER_RESET)
...
static void noNetworksTest(void)
{
    ...
    // We scan on the primary mask
    addSimpleCheck("scheduleScan", "ii", EMBER_ACTIVE_SCAN, emAfPluginNetworkSteeringPrimaryChannelMask);

    // We don't find any networks
    addScanResultsAction(EMBER_SUCCESS, 0, 0, true);

    // ...so we go to the secondary mask
    addSimpleCheck("scheduleScan", "ii", EMBER_ACTIVE_SCAN, emAfPluginNetworkSteeringSecondaryChannelMask);

    // We don't find any networks
    addScanResultsAction(EMBER_SUCCESS, 0, 0, true);

    // No beacons are received, so we never set up security. The ending state is
    // EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_SECONDARY_INSTALL_CODE because
    // we never progress past it due to lack of beacons.
    addSimpleCheck("networkSteeringCompleteCallback", "iiii", EMBER_NO_BEACONS,
                   0, // beacons
                   0, // join attempts
...
static void primaryChannelsInstallCodeTest(void)
{
    ...
    addSimpleAction("updateTcLinkKeyCompleteCallback(0x%02X)", emberAfPluginUpdateTcLinkKeyStatusCallback, 1,
    ...
    // Done!
    addSimpleCheck("networkSteeringCompleteCallback", "iiii", EMBER_SUCCESS,
                   2, // beacons
                   2, // join attempts
...
static void failedTclkExchangeTest(void)
{
    ...
    // Done!
    addSimpleCheck(
        "networkSteeringCompleteCallback", "iiii", EMBER_NO_LINK_KEY_RECEIVED,
        1, // beacons
        1, // join attempts
        (EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_PRIMARY_INSTALL_CODE | EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_UPDATE_TCLK));
...
static void failedSecuritySetupTest(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (i == 0)
        {
            addSetInitialSecurityStateCheck(securityBitmask, key, EMBER_SECURITY_CONFIGURATION_INVALID);
        }
        else
        {
            addSetInitialSecurityStateCheck(securityBitmask, key, EMBER_SUCCESS);
...
static void failedVerifyKey(void)
{
    ...
    uint32_t securityBitmask = (SECURITY_BITMASK | EMBER_GET_PRECONFIGURED_KEY_FROM_INSTALL_CODE);
    ...
    // Done!
    addSimpleCheck("networkSteeringCompleteCallback", "iiii", EMBER_ERR_FATAL,
                   1, // beacons
                   1, // join attempts
                   (EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_PRIMARY_INSTALL_CODE |
                    EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_UPDATE_TCLK | EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_VERIFY_TCLK));
...
static void failedScan(void)
{
    ...
    // We report failure and reset our state.
    addSimpleCheck("networkSteeringCompleteCallback", "iiii", EMBER_MAC_COMMAND_TRANSMIT_FAILURE,
                   0, // beacons
                   0, // join attempts
                   EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_PRIMARY_INSTALL_CODE);
    addFunctionCheck("state = %d", EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_NONE, getNetworkSteeringState, 0);

    // We get a scan complete call saying we are done with the scan. Make sure
    // that we don't keep trying to scan.
    addScanResultsAction(EMBER_SUCCESS, 11, 0, true);
...
{
    Test tests[] = {
        { "already-on-network", alreadyOnNetwork },

        { "no-networks", noNetworksTest },
        /* TODO EMZIGBEE-EMZIGBEE-3745
           The following tests need update. Network steering has changed such that
           we previously used to do:
           Set up security
           Scan
           Wait for scan complete. Collect scan results.
           Join networks one by one
           Move to next channel if applicable, scan + join again
           Move to next state if applicable, setup security, scan, and join again
           And with the new network-steering, we do
           Scan all channels
           Wait for scan complete
           Set up security
           Join networks on by one
           Move to next channel if applicable, join again
           Move to next state if applicable, setup security and join again
           I've fixed noNetworksTest to follow this new routine, but the following tests
           need update. */
        { "primary-install-code", primaryChannelsInstallCodeTest },

        { "secondary-centralized", secondaryChannelsCentralizedTest },

        { "primary-distributed", primaryChannelsDistributedTest },

        { "failed-tclk-update", failedTclkExchangeTest },

        { "failed-security-setup", failedSecuritySetupTest },

        { "failed-verify-key", failedVerifyKey },

        { "failed-scan", failedScan },

        { "pre-r21-trust-center", preR21TrustCenter },

        { "configured-key", configuredKeyTest },
