...
/***************************************************************************/ /**
                                                                               ...
                                                                               *
                                                                               * 1. Setup stack security with an Install code key
                                                                               *(if available) If not available, skip to step 9.
                                                                               * 2. Set the channel mask to the Primary set of
                                                                               *channels.
                                                                               * 3. Perform a scan of open networks on a single
                                                                               *channel.
                                                                               * 4. For all beacons collected on that channel,
                                                                               *filter out PAN IDs that are not open for joining or
                                                                               *don't have the right stack profile (2), or are
                                                                               *duplicates.
                                                                               * 5. For each discovered network, try to join.
                                                                               * 6. Repeat steps 3-5 until all channels in the mask
                                                                               *are tried.
                                                                               * 7. If no networks are successfully joined, setup
                                                                               *secondary channel mask (all channels) and repeat
                                                                               *steps 3-6 with.
                                                                               * 8. If no networks are successfully joined, change
                                                                               *the security to use the default well-known
                                                                               *"ZigbeeAlliance09" key.
                                                                               * 9. Repeat steps 2-7
                                                                               *    (This will again try primary channel mask first,
                                                                               *then secondary).
                                                                               * 10. If no networks are successsully joined, setup
                                                                               *security to use distributed link key.
                                                                               * 11. Repeat steps 2.7.
                                                                               * 12. If no networks are joined successfully fail the
                                                                               *whole process and return control to the application.
                                                                               *******************************************************************************
                                                                               ******************************************************************************/
...
#define LAST_JOINING_STATE EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_SECONDARY_DISTRIBUTED
...
static EmberKeyData gConfiguredKey = { { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                         0x00 } };
...
static bool addPanIdCandidate(uint16_t panId)
{
    ...
    {
        emberAfCorePrintln("Error: %p could not get memory pointer for stored PAN IDs", PLUGIN_NAME);
        return false;
    }
    ...
    {
        emberAfCorePrintln("Warning: %p Max PANs reached (%d)", PLUGIN_NAME, maxNetworks);
...
static uint16_t getNextCandidate(void)
{
    ...
    uint16_t * pointer = emAfPluginNetworkSteeringGetStoredPanIdPointer(emAfPluginNetworkSteeringPanIdIndex);
    if (pointer == NULL)
    {
...
static void networkFoundCallback(EmberZigbeeNetwork * networkFound, uint8_t lqi, int8_t rssi)
{
    emAfPluginNetworkSteeringTotalBeacons++;

    if (!(networkFound->allowingJoin && networkFound->stackProfile == REQUIRED_STACK_PROFILE))
    {
        return;
    }

    debugPrint("%p nwk found ch: %d, panID 0x%2X, xpan: ", PLUGIN_NAME, networkFound->channel, networkFound->panId);
    ...
    {
        if (0 != MEMCOMPARE(networkFound->extendedPanId, gExtendedPanIdToFilterOn, COUNTOF(gExtendedPanIdToFilterOn)))
        {
    ...
    {
        emberAfCorePrintln("Error: %p could not add candidate network.", PLUGIN_NAME);
...
HIDDEN void scanResultsHandler(EmberAfPluginScanDispatchScanResults * results)
{
    ...
    {
        networkFoundCallback(results->network, results->lqi, results->rssi);
    }
}
...
static EmberStatus tryNextMethod(void)
{
    ...
    {
        EmberStatus status = (emAfPluginNetworkSteeringTotalBeacons > 0 ? EMBER_JOIN_FAILED : EMBER_NO_BEACONS);
        cleanupAndStop(status);
        return status;
    }
...
static uint8_t getNextChannel(void)
{
    if (emAfPluginNetworkSteeringCurrentChannel == 0)
    {
        emAfPluginNetworkSteeringCurrentChannel = (emberGetPseudoRandomNumber() & 0x0F) + EMBER_MIN_802_15_4_CHANNEL_NUMBER;
...
static EmberStatus setupSecurity(void)
{
    ...
    MEMMOVE(emberKeyContents(&(state.preconfiguredKey)),
            gUseConfiguredKey
                ? emberKeyContents(&(gConfiguredKey))
    ...
    if ((status = emberSetInitialSecurityState(&state)) != EMBER_SUCCESS)
    {
        goto done;
    }
...
EmberStatus emberAfPluginNetworkSteeringStart(void)
{
    ...
    if (emAfProIsCurrentNetwork() && (emAfPluginNetworkSteeringState == EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_NONE))
    {
        ...
        {
            emAfPluginNetworkSteeringState = gUseConfiguredKey ? EMBER_AF_PLUGIN_NETWORK_STEERING_STATE_SCAN_PRIMARY_CONFIGURED
