...
const kGlobalAttributes =
    ...
    function asReadTypeLength(type) {
      ...
      function fn(pkgId)
      {
        ...
        const enumResolver = zclHelper.isEnum(db, type, pkgId)
                                 .then(result => { return result == 'unknown'
                                               ? null
                                               : zclQuery.selectEnumByName(db, type, pkgId)
                                                     .then(rec => { return zclQuery.selectAtomicType(db, pkgId, rec.type) }) })

        const bitmapResolver = zclHelper.isBitmap(db, type, pkgId)
                                   .then(result => { return result == 'unknown'
                                                 ? null
                                                 : zclQuery.selectBitmapByName(db, pkgId, type)
                                                       .then(rec => { return zclQuery.selectAtomicType(db, pkgId, rec.type) }) })

        const typeResolver = Promise.all([
          defaultResolver,
          enumResolver,
          bitmapResolver,
        ])
...
function asReadType(type) {
  ...
  {
    const options
        = { hash : {} } return zclHelper.asUnderlyingZclType.call(this, type, options).then(zclType => {
            const basicType = ChipTypesHelper.asBasicType(zclType)
            switch (basicType)
            {
            case 'bool':
              return 'Int8u'
              case 'int8_t': return 'Int8s'
              case 'uint8_t': return 'Int8u'
              case 'int16_t': return 'Int16s'
              case 'uint16_t': return 'Int16u'
              case 'int24_t': return 'Int24s'
              case 'uint24_t': return 'Int24u'
              case 'int32_t': return 'Int32s'
              case 'uint32_t': return 'Int32u'
              case 'int64_t': return 'Int64s'
              case 'uint64_t': return 'Int64u' default: error
...
var endpointClusterWithInit
    ...
    = [ 'IAS Zone' ] var endpointClusterWithMessageSent =
        [ 'IAS Zone' ]
            ...
            if (c.comment.includes('server')) {
              ...
              if (endpointClusterWithInit.includes(clusterName))
              {
                ...
                functionList     = functionList.concat(
                    `  (EmberAfGenericClusterFunction) emberAf${
                        cHelper.asCamelCased(
                            clusterName,
                            false,
                ...
                functionList = functionList.concat(
                    `  (EmberAfGenericClusterFunction) emberAf${
                        cHelper.asCamelCased(
                            clusterName,
                            false,
                            )}ClusterServerMessageSentCallback,\\\n`,
                )
                hasFunctionArray = true
              }
              ...
              if (hasFunctionArray) {
                ret = ret.concat(
                    `const EmberAfGenericClusterFunction chipFuncArray${
                        cHelper.asCamelCased(
                            clusterName,
                            false,
                            )}Server[] = {\\\n`,
        ...
        chip_endpoint_cluster_list() {
          let ret = '{ \\\n' this.clusterList.forEach(c => {
            ...
            ret = ret.concat(
                `  { ${c.clusterId}, ZAP_ATTRIBUTE_INDEX(${c.attributeIndex}), ${c.attributeCount}, ${c.attributeSize}, ${mask}, ${
                    functionArray} }, /* ${c.comment} */ \\\n`,
            )
          })
          return ret.concat('}\n')
        }
          ...
          function fn(pkgId)
          {
            const options
                = { hash : {} } return zclHelper.asUnderlyingZclType.call(this, type, options).then(zclType => {
                    ...
                    case 'bool':
                      return '%d'
                      case 'int8_t': return '%" PRId8 "'
                      case 'uint8_t': return '%" PRIu8 "'
                      case 'int16_t': return '%" PRId16 "'
                      case 'uint16_t': return '%" PRIu16 "'
                      case 'int24_t': return '%" PRId32 "'
                      case 'uint24_t': return '%" PRIu32 "'
                      case 'int32_t': return '%" PRId32 "'
                      case 'uint32_t': return '%" PRIu32 "'
                      case 'int64_t': return '%" PRId64 "'
                      case 'uint64_t': return '%" PRIu64 "' default: return '%p'
                    }
                  })
          }
    ...
    const promise = templateUtil
        ...
        function asTypeLiteralSuffix(type) {
          ...
          case 'int32_t':
            return 'L'
            case 'int64_t': return 'LL'
            case 'uint16_t': return 'U'
            case 'uint32_t': return 'UL'
            case 'uint64_t': return 'ULL' default: return ''
          }
        }
