...
{
  console.error(
      'Error in: ' + test.filename + '.yaml for test with label: "' + test.label + '"\n',
  )
...
{
  const type = test[kCommandName] switch (type)
  {
  case 'readAttribute':
    ...
    break
        ...
        case 'writeAttribute': test.isAttribute
        = true
    test.isWriteAttribute = true
    break

        default: test.isCommand
        = true
    break
  }
}
...
{
  const defaultArguments = {} setDefault(test, kArgumentsName, defaultArguments)
  ...
  test[kArgumentsName]
      ...
      }) delete test[kArgumentsName]
      .value
}

function setDefaultResponse(test)
{
  const defaultResponse = {} setDefault(test, kResponseName, defaultResponse)
  ...
  const defaultResponseConstraints = {} setDefault(
      test[kResponseName],
      kConstraintsName,
      defaultResponseConstraints,
  )

  const hasResponseValue = 'value' in test[kResponseName] const hasResponseConstraints
      = 'constraints' in test[kResponseName] && Object.keys(test[kResponseName].constraints).length
...
function assertCommandOrAttribute(context)
{
  ...
  return Clusters.getClusters().then(clusters => {
    if (!clusters.find(cluster => cluster.name == clusterName)) {
      ...
      printErrorAndExit(
          context,
          'Missing cluster "' + clusterName + '" in: \n\t* ' + names.join('\n\t* '),
      )
    }
    ...
    return items.then(items => {
      ...
      if (!item)
      {
        ...
        printErrorAndExit(
            context,
            'Missing ' + itemType + ' "' + filterName + '" in: \n\t* ' + names.join('\n\t* '),
        )
      }
...
function chip_tests_item_response_parameters(options)
{
  const responseValues = this.response.values.slice()
    ...
    const responses = responseArgs.map(responseArg => {
      ...
      if (expectedIndex != -1)
      {
        const expected = responseValues.splice(expectedIndex, 1)[0] if ('value' in expected)
        {
      ...
      unusedResponseValues.forEach(unusedResponseValue => { printErrorAndExit(
                                       this,
