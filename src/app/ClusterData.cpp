...
ClusterSchemaEngine::StoreData(AttributePathHandle aHandle, TLV::TLVReader & aReader, ISetDataDelegate * apDelegate) const
{
    ...
    if (IsLeaf(curHandle))
    {
        err = apDelegate->SetData(curHandle, aReader, aReader.GetType() == TLV::kTLVType_Null);
        SuccessOrExit(err);
    }
    else
    {
...
AttributePathHandle ClusterSchemaEngine::GetNextChild(AttributePathHandle aParentHandle, AttributePathHandle aChildHandle) const
{
    ...
    // aParentId.
    for (pathHandleIndex = (uint16_t) aChildHandle - 1; pathHandleIndex < mSchema.mNumAttributePathHandleEntries; pathHandleIndex++)
    {
    ...
    {
        return (pathHandleIndex + kHandleTableOffset);
    }
}
...
bool ClusterSchemaEngine::MatchesClusterId(uint32_t aClusterId) const
{
    return (aClusterId == mSchema.mClusterId);
}
...
void ClusterDataSink::SetVersion(DataVersion aVersion)
{
    ...
    {
        ChipLogDetail(DataManagement, "Cluster version not change");
    }
    mVersion         = aVersion;
...
CHIP_ERROR ClusterDataSink::SetData(AttributePathHandle aHandle, chip::TLV::TLVReader & aReader, bool aIsnullptr)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    // if a Cluster has no nullptrable handles, aIsnullptr will always be false
    // and serves no purpose. this is true for the default implementation.
    IgnoreUnusedVariable(aIsnullptr);
