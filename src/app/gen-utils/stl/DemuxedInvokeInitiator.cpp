...
DemuxedInvokeInitiator::DemuxedInvokeInitiator(onDoneFuncDef onDoneFunc) : mOnDoneFunc(onDoneFunc) {}

CHIP_ERROR DemuxedInvokeInitiator::AddCommand(
    IEncodable * request, CommandParams params,
...
void DemuxedInvokeInitiator::OnResponse(InvokeInitiator & initiator, CommandParams & params, TLV::TLVReader * payload)
{
    ...
    if (!foundMatch)
    {
        ChipLogProgress(DataManagement,
                        "Could not find a matching demuxed handler for command! (ClusterId = %04x, Endpoint = %lu, Command = %lu)",
                        params.ClusterId, (unsigned long) params.EndpointId, (unsigned long) params.CommandId);
    }
}

void DemuxedInvokeInitiator::OnError(InvokeInitiator & initiator, CommandParams * params, CHIP_ERROR error,
