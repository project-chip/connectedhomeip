...
#ifndef CHIP_CONFIG_INTERACTION_MODEL_ENABLE_SCHEMA_CHECK
#define CHIP_CONFIG_INTERACTION_MODEL_ENABLE_SCHEMA_CHECK 1
...
CHIP_ERROR AttributePath::Parser::CheckSchemaValidity() const
{
    ...
    if (CHIP_END_OF_TLV == err)
    {
        ...
        {
            err = CHIP_ERROR_INTERACTION_MODEL_MALFORMED_ATTRIBUTE_PATH;
        }
    }
    SuccessOrExit(err);
...
CHIP_ERROR StatusCode::Parser::CheckSchemaValidity() const
{
    ...
    while (CHIP_NO_ERROR == (err = reader.Next()))
    {
        ...
        else if (!(TagPresenceMask & (1 << kCsTag_DetailedCode)))
        {
            // The second element has to be the StatusCode.
            TagPresenceMask |= (1 << kCsTag_DetailedCode);
            VerifyOrExit(chip::TLV::kTLVType_UnsignedInteger == reader.GetType(), err = CHIP_ERROR_WRONG_TLV_TYPE);
...
CHIP_ERROR AttributeDataElement::Parser::CheckSchemaValidity() const
{
    ...
    if (CHIP_END_OF_TLV == err)
    {
        ...
        {
            err = CHIP_ERROR_INTERACTION_MODEL_MALFORMED_ATTRIBUTE_DATA_ELEMENT;
        }
    }
...
CHIP_ERROR CommandDataElement::Parser::CheckSchemaValidity() const
{
    ...
    if (CHIP_END_OF_TLV == err)
    {
        ...
        {
            err = CHIP_ERROR_INTERACTION_MODEL_MALFORMED_COMMAND_DATA_ELEMENT;
        }
    }
    ...
    while (CHIP_NO_ERROR == (err = reader.Next()))
    {
        ...
        if (chip::TLV::ContextTag(kBit_RequestResponse) == tag)
        {
...
#if CHIP_DETAIL_LOGGING
            {
                ...
                SuccessOrExit(err);
                PRETTY_PRINT("\tRequestResponse = %u,", RequestResponse);
            }
#endif // CHIP_DETAIL_LOGGING
        }
