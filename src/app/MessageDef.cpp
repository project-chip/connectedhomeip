...
CHIP_ERROR StatusCode::Parser::CheckSchemaValidity() const
{
    ...
    while (CHIP_NO_ERROR == (err = reader.Next()))
    {
        ...
        else if (!(TagPresenceMask & (1 << kCsTag_ClusterId)))
        {
            ...
            VerifyOrExit(reader.GetType() == chip::TLV::kTLVType_Structure, err = CHIP_ERROR_WRONG_TLV_TYPE);
...
StatusCode::Builder & StatusCode::Builder::Create(const uint16_t aGeneralCode, const uint32_t aProtocolId, const uint16_t aStatusCode, const chip::ClusterId aClusterId)
{
    ...
    mError = mpWriter->StartContainer(tag, kTLVType_Structure, metaDataContainer);
    SuccessOrExit(mError);

    mpWriter->Put(tag, aClusterId);

    mError = mpWriter->EndContainer(kTLVType_Structure);
    SuccessOrExit(mError);
...
CHIP_ERROR AttributeStatusElement::Parser::GetStatusCode(StatusCode::Parser * const apStatusCode) const
{
    ...
    err = mReader.FindElementWithTag(chip::TLV::ContextTag(kCsTag_StatusCode), reader);
    SuccessOrExit(err);
