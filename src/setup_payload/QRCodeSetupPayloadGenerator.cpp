                                                                    ...
                                                                    uint32_t tlvDataStartSize)
{
    ...
    MutableByteSpan bitsSpan;
    std::vector<char> buffer;
    MutableCharSpan bufferSpan;
    ...
    bitsSpan = MutableByteSpan(bits.data(), bits.capacity());
    buffer.reserve(base38EncodedLength(bits.capacity()) + strlen(kQRCodePrefix));
    bufferSpan = MutableCharSpan(buffer.data(), buffer.capacity());
