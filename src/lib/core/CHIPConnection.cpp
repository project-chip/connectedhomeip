 ...
 *
 */
CHIP_ERROR ChipConnection::Connect(uint64_t peerNodeId, ChipAuthMode authMode, const IPAddress &peerAddr, uint16_t peerPort, InterfaceId intf)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    VerifyOrExit(State == kState_ReadyToConnect, err = CHIP_ERROR_INCORRECT_STATE);

    VerifyOrExit(authMode == kChipAuthMode_Unauthenticated || IsCASEAuthMode(authMode) || IsPASEAuthMode(authMode), err = CHIP_ERROR_INVALID_ARGUMENT);

    // Can't request authentication if the security manager is not initialized.
    VerifyOrExit(authMode == kChipAuthMode_Unauthenticated || MessageLayer->SecurityMgr != NULL, err = CHIP_ERROR_UNSUPPORTED_AUTH_MODE);
 ...
 *
 */
CHIP_ERROR ChipConnection::Connect(uint64_t peerNodeId, ChipAuthMode authMode, const char *peerAddr,
                                     ...
                                     uint16_t peerAddrLen, uint8_t dnsOptions, uint16_t defaultPort)
{
    ...
    err = MessageLayer->Inet->ResolveHostAddress(hostName, hostNameLen, dnsOptions, CHIP_CONFIG_CONNECT_IP_ADDRS, mPeerAddrs, HandleResolveComplete, this);
 ...
 *
 */
CHIP_ERROR ChipConnection::Connect(uint64_t peerNodeId, ChipAuthMode authMode, HostPortList hostPortList,
                                     InterfaceId intf)
{
#if CHIP_CONFIG_ENABLE_DNS_RESOLVER
    const uint8_t dnsOptions = ::nl::Inet::kDNSOption_Default;
...
void ChipConnection::HandleConnectComplete(TCPEndPoint *endPoint, INET_ERROR conRes)
{
    ...
    if (conRes == INET_NO_ERROR)
    {
        ...
        if (!localAddr.IsIPv6ULA() || IPv6InterfaceIdToChipNodeId(localAddr.InterfaceId()) != con->MessageLayer->FabricState->LocalNodeId)
        {
...
void ChipConnection::HandleSecureSessionError(ChipSecurityManager *sm, ChipConnection *con, void *reqState,
