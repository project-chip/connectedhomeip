...
public:
    ...
    CHIP_ERROR AddTrustedKey(uint64_t caId, uint32_t curveId, const EncodedECPublicKey& pubKey,
    ...
    CHIP_ERROR FindValidCert(const ChipDN& subjectDN, const CertificateKeyId& subjectKeyId,
            ValidationContext& context, ChipCertificateData *& cert);

    CHIP_ERROR GenerateECDSASignature(const uint8_t *msgHash, uint8_t msgHashLen,
                                       ChipCertificateData& cert,
                                       const EncodedECPrivateKey& privKey,
                                       EncodedECDSASignature& encodedSig);
    CHIP_ERROR VerifyECDSASignature(const uint8_t *msgHash, uint8_t msgHashLen,
...
protected:
    ...
    CHIP_ERROR FindValidCert(const ChipDN& subjectDN, const CertificateKeyId& subjectKeyId,
            ValidationContext& context, uint16_t validateFlags, uint8_t depth, ChipCertificateData *& cert);
...
extern CHIP_ERROR GenerateOperationalDeviceCert(uint64_t deviceId, EncodedECPublicKey& devicePubKey,
...
extern CHIP_ERROR ConvertX509CertToChipCert(const uint8_t *x509Cert, uint32_t x509CertLen, uint8_t *weaveCertBuf, uint32_t weaveCertBufSize, uint32_t& weaveCertLen);
extern CHIP_ERROR ConvertChipCertToX509Cert(const uint8_t *weaveCert, uint32_t weaveCertLen, uint8_t *x509CertBuf, uint32_t x509CertBufSize, uint32_t& x509CertLen);
...
{
    return (oid == chip::ASN1::kOID_AttributeType_ChipDeviceId ||
            oid == chip::ASN1::kOID_AttributeType_ChipServiceEndpointId ||
            oid == chip::ASN1::kOID_AttributeType_ChipCAId ||
