...
enum
{
    ...
    kStatusCode_OperationalNodeIdInUse          = 20, // The specified operational node Id is already used by another CHIP node (indication of node id collision).
...
enum
{
    ...
    kTag_ChipSignature_SigningCertificateRef   = 3,    // [ structure ] A CHIP certificate reference structure identifying the certificate
    ...
    kTag_ChipSignature_RelatedCertificates     = 4,    // [ array ] Array of certificates needed to validate the signature.  May be omitted if
    ...
    kTag_ChipSignature_SignatureAlgorithm      = 5,    // [ unsigned int ] Enumerated value identifying the signature algorithm.
    ...
    kTag_ChipCertificateRef_PublicKeyId        = 2,    // [ byte string ] Unique identifier for referenced certificate's public key, per RFC5280.

    // ---- Context-specific Tags for CHIP CASE Certificate Information Structure ----
    kTag_CASECertificateInfo_EntityCertificate    = 1,  // [ structure ] A CHIP certificate object representing the authenticating entity.
    kTag_CASECertificateInfo_EntityCertificateRef = 2,  // [ structure ] A CHIP certificate reference object identifying the authenticating entity.
    kTag_CASECertificateInfo_RelatedCertificates  = 3,  // [ path ] A collection of certificates related to the authenticating entity.
    kTag_CASECertificateInfo_TrustAnchors         = 4,  // [ path ] A collection of CHIP certificate reference identifying certificates trusted
    ...
    kTag_GetCertReqMsg_OpRelatedCerts           = 5,    // [ array, optional ] An optional array of CHIP operational intermediate CA certificates,
    ...
    kTag_GetCertReqMsg_MfrAttest_ChipCert      = 6,    // [ structure ] CHIP device certificate provisioned at the factory at manufacturing time.
                                                        //   This has the same internal structure as an kTag_ChipCertificate.
    kTag_GetCertReqMsg_MfrAttest_ChipRelCerts  = 7,    // [ array, optional ] An optional array of CHIP intermediate CA certificates, which are needed
    ...
    kTag_GetCertRespMsg_OpDeviceCert            = 1,    // [ structure ] Service assigned CHIP operational device certificate.
                                                        //   This has the same internal structure as an kTag_ChipCertificate.
    kTag_GetCertRespMsg_OpRelatedCerts          = 2,    // [ array, optional ] An optional array of CHIP certificates related to the
