...
CHIP_ERROR ChipMessageLayer::EncodeMessage(const IPAddress & destAddr, uint16_t destPort, InterfaceId sendIntId,
                                           ChipMessageInfo * msgInfo, const PacketBufferHandle & payload)
{
 ...
 *
 */
CHIP_ERROR ChipMessageLayer::DecodeHeader(const PacketBufferHandle & msgBuf, ChipMessageInfo * msgInfo, uint8_t ** payloadStart)
{
    CHIP_ERROR err     = CHIP_NO_ERROR;
...
CHIP_ERROR ChipMessageLayer::ReEncodeMessage(const PacketBufferHandle & msgBuf)
{
 ...
 *
 */
CHIP_ERROR ChipMessageLayer::EncodeMessage(ChipMessageInfo * msgInfo, const System::PacketBufferHandle & msgBuf,
                                           ChipConnection * con, uint16_t maxLen, uint16_t reserve)
{
    CHIP_ERROR err;
    uint8_t * p1;
...
CHIP_ERROR ChipMessageLayer::DecodeMessage(System::PacketBufferHandle msgBuf, uint64_t sourceNodeId, ChipConnection * con,
...
CHIP_ERROR ChipMessageLayer::EncodeMessageWithLength(ChipMessageInfo * msgInfo, const PacketBufferHandle & msgBuf,
...
CHIP_ERROR ChipMessageLayer::DecodeMessageWithLength(const PacketBufferHandle & msgBuf, uint64_t sourceNodeId, ChipConnection * con,
...
void ChipMessageLayer::HandleUDPMessage(UDPEndPoint * endPoint, System::PacketBufferHandle msg, const IPPacketInfo * pktInfo)
{
    CHIP_ERROR err              = CHIP_NO_ERROR;
    ...
    {
        msgLayer->OnMessageReceived(msgLayer, &msgInfo, std::move(msg));
    }
    else
    {
