...
enum
{
    kChipPeerDescription_MaxLength = 100,  /**< Maximum length of string (including NUL character) returned by ChipMessageLayer::GetPeerDescription(). */
};
...
enum
{
    ...
    kMsgHeaderField_ReservedFlagsMask                   = kMsgHeaderField_FlagsMask &
                                                          ~kChipHeaderFlag_DestNodeId &
                                                          ~kChipHeaderFlag_SourceNodeId &
                                                          ~kChipHeaderFlag_TunneledData &
                                                          ~kChipHeaderFlag_MsgCounterSyncReq,

    kMsgHeaderField_MessageHMACMask                     = ~((kChipHeaderFlag_DestNodeId |
                                                             kChipHeaderFlag_SourceNodeId |
                                                             kChipHeaderFlag_MsgCounterSyncReq) << kMsgHeaderField_FlagsShift)
};
...
struct ChipMessageInfo
{
    ...
    ChipAuthMode PeerAuthMode;        /**< The means by which the sender of the message was authenticated. Only meaningful for incoming messages. */
    ChipConnection *InCon;            /**< The connection (if any) over which the message was received. Only meaningful for incoming messages.*/
...
typedef enum ChipMessageFlags
{
    ...
    kChipMessageFlag_DefaultMulticastSourceAddress     = 0x00002000, /**< Indicates that default IPv6 source address selection should be used when
    ...
    kChipMessageFlag_DuplicateMessage                  = 0x00008000, /**< Indicates that the message is a duplicate of a previously received message. */
    kChipMessageFlag_PeerGroupMsgIdNotSynchronized     = 0x00010000, /**< Indicates that the peer's group key message counter is not synchronized. */
	kChipMessageFlag_FromInitiator                     = 0x00020000, /**< Indicates that the source of the message is the initiator of the
																		   CHIP exchange. */
    kChipMessageFlag_ViaEphemeralUDPPort               = 0x00040000, /**< Indicates that message is being sent/received via the local ephemeral UDP port. */
...
enum
{
    kMaxAlwaysLocalChipNodeId  = 0x000000000000FFFFULL,    /**< CHIP node identifiers less or equal than this value are considered local for testing convenience. */
};
 ...
 *
 */
inline uint64_t ChipFabricIdToIPv6GlobalId(uint64_t fabricId) { return (fabricId & 0xFFFFFFFFFFULL); }
