...
inline bool IsPASEAuthMode(ChipAuthMode authMode) { return (authMode & kChipAuthModeCategory_Mask) == kChipAuthModeCategory_PASE; }

/** True if the authentication mode is based on the CHIP CASE protocol. */
inline bool IsCASEAuthMode(ChipAuthMode authMode) { return (authMode & kChipAuthModeCategory_Mask) == kChipAuthModeCategory_CASE; }

/** True if the authentication mode is based on the CHIP TAKE protocol. */
inline bool IsTAKEAuthMode(ChipAuthMode authMode) { return (authMode & kChipAuthModeCategory_Mask) == kChipAuthModeCategory_TAKE; }

/** True if the authentication mode is based possession of a shared password. */
inline bool IsPasswordAuthMode(ChipAuthMode authMode) { return IsPASEAuthMode(authMode); }

/** True if the authentication mode is based on possession of a private key associated with a certificate. */
inline bool IsCertAuthMode(ChipAuthMode authMode) { return IsCASEAuthMode(authMode); }

/** True if the authentication mode is based on possession of a shared group key. */
inline bool IsGroupKeyAuthMode(ChipAuthMode authMode) { return (authMode & kChipAuthModeCategory_Mask) == kChipAuthModeCategory_GroupKey; }
 ...
 */
inline uint8_t PasswordSourceFromAuthMode(ChipAuthMode authMode) { return (uint8_t)(authMode & kChipAuthMode_PASE_PasswordSourceMask); }

/** Returns the password source for the given authentication mode.
 *
 *  @note The result is only valid when the supplied mode is one of the certificate authentication modes.
 */
inline uint8_t CertTypeFromAuthMode(ChipAuthMode authMode) { return (uint8_t)(authMode & kChipAuthMode_CASE_CertTypeMask); }
 ...
 */
inline uint8_t AppGroupMasterKeyIdFromAuthMode(ChipAuthMode authMode) { return ChipKeyId::MakeAppGroupMasterKeyId(authMode & kChipAuthMode_GroupKey_AppGroupLocalNumber); }

/** Returns a corresponding PASE authentication mode for a given password source. */
inline ChipAuthMode PASEAuthMode(uint8_t pwSource) { return kChipAuthModeCategory_PASE | pwSource; }

/** Returns a corresponding CASE authentication mode for a given certificate type. */
inline ChipAuthMode CASEAuthMode(uint8_t certType) { return kChipAuthModeCategory_CASE | certType; }
 ...
 */
inline ChipAuthMode GroupKeyAuthMode(uint32_t keyId) { return kChipAuthModeCategory_GroupKey | ChipKeyId::GetAppGroupLocalNumber(keyId); }
...
public:
    ...
    ChipSessionState(ChipMsgEncryptionKey *msgEncKey, ChipAuthMode authMode,
...
public:
    ...
    CHIP_ERROR SetSessionKey(uint16_t keyId, uint64_t peerNodeId, uint8_t encType, ChipAuthMode authMode, const ChipEncryptionKey *encKey);
    ...
    CHIP_ERROR GetSharedSessionEndNodeIds(const ChipSessionKey *sessionKey, uint64_t *endNodeIds,
                                           uint8_t endNodeIdsBufSize, uint8_t& endNodeIdsCount);
    void RemoveSharedSessionEndNodes(const ChipSessionKey *sessionKey);

    CHIP_ERROR SuspendSession(uint16_t keyId, uint64_t peerNodeId, uint8_t * buf, uint16_t bufSize, uint16_t & serializedSessionLen);
    CHIP_ERROR RestoreSession(uint8_t * serializedSession, uint16_t serializedSessionLen);

    CHIP_ERROR GetSessionState(uint64_t remoteNodeId, uint16_t keyId, uint8_t encType, ChipConnection *con, ChipSessionState& outSessionState);
    ...
    CHIP_ERROR CheckMsgEncForAppGroup(const ChipMessageInfo *msgInfo, uint32_t appGroupGlobalId, uint32_t rootKeyId, bool requireRotatingKey);
