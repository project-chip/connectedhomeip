...
void TestInit(nlTestSuite * inSuite, void * inContext)
{
    ...
    NL_TEST_ASSERT(inSuite, dataset.Init(nullptr, 255) == CHIP_ERROR_INVALID_ARGUMENT);
    NL_TEST_ASSERT(inSuite, dataset.Init(nullptr, 0) == CHIP_NO_ERROR);

    {
        uint8_t data[] = { 0x01, 0x02, 0x03 };

        NL_TEST_ASSERT(inSuite, dataset.Init(data, sizeof(data)) == CHIP_ERROR_INVALID_ARGUMENT);
    }
