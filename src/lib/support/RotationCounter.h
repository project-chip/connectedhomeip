 ...
 * Example:
 *
 * - Assuming epoch is 100 via RotationCounter::Init(_, 100) and GetValue +
 *   AdvanceValue is called, we get the following outputs:
 *
 *   - Output: 0, 1, 2, 3, 4  <factory resets>
 *
 */
class RotationCounter : public MonotonicallyIncreasingCounter
{
public:
    RotationCounter();
    ~RotationCounter() override;

    /**
     *  @brief
     *    Initialize a RotationCounter object.
     *
     *  @param[in] aId     The identifier of this RotationCounter instance.
