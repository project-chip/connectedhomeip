...
struct PersistentArray : public PersistentData<TLV::EstimateStructOverhead(sizeof(uint16_t), kMaxListSize * kMaxSerializedEntrySize)>
{
    ...
    CHIP_ERROR Set(size_t index, const EntryType & value)
    {
        ...
        if (index < this->mCount)
        {
            ...
            EntryType old;
            ReturnErrorOnFailure(Copy(old, mEntries[index]));
