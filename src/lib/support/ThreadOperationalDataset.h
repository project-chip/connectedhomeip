 ...
 *
 */
class OperationalDataset : public DeviceLayer::Internal::ThreadOperationalDataset
{
public:
    /**
     * This method initializes the dataset with the given dataset.
     *
     * @param[in]   aData       The pointer to the dataset buffer.
     ...
     *
     */
    CHIP_ERROR Init(const uint8_t * aData, uint8_t aLength);
    ...
    /**
     * This method checks if the dataset is valid.
     *
     */
    bool IsValid(void) const;

private:
    Tlv * Locate(uint8_t aType);
    const Tlv * Locate(uint8_t aType) const;
    const Tlv & End(void) const { return *reinterpret_cast<const Tlv *>(&mData[mLength]); };
    Tlv & End(void) { return const_cast<Tlv &>(const_cast<const OperationalDataset *>(this)->End()); }
    void Remove(uint8_t aType);
    void Remove(Tlv & aTlv);
    Tlv * MakeRoom(uint8_t aType, size_t aSize);
