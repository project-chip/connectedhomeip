...
public:
    ...
    /**
     * Wraps an iterator pointer and auto-releases it when going out of scope.
     */
    class AutoReleasing
    {
    public:
        AutoReleasing(CommonIterator * iterator) : mIterator(iterator) {}
        ~AutoReleasing()
        {
        ...
        AutoReleasing(const AutoReleasing &)             = delete;
        AutoReleasing & operator=(const AutoReleasing &) = delete;

        bool IsValid() const { return mIterator != nullptr; }
        size_t Count() const { return mIterator != nullptr ? mIterator->Count() : 0; }
        bool Next(T & value) { return (mIterator == nullptr) ? false : mIterator->Next(value); }

    private:
        CommonIterator * mIterator;
    };
