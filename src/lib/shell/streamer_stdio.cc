...
int streamer_stdio_init(const struct streamer * streamer)
{
    int ret   = 0;
    int in_fd = fileno(stdin);
...
int streamer_stdio_read(const struct streamer * streamer, char * buf, size_t len)
{
    return read(STDIN_FILENO, buf, len);
}

int streamer_stdio_write(const struct streamer * streamer, const char * buf, size_t len)
{
    return write(STDOUT_FILENO, buf, len);
}

static const struct streamer streamer_stdio = {
