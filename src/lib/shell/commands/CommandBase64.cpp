...
int CommandBase64Help(int argc, char ** argv)
{
    sShellBase64Commands.ForEachCommand(PrintCommandHelp, nullptr);
    return 0;
}

int CommandBase64Decode(int argc, char ** argv)
{
...
int CommandBase64Encode(int argc, char ** argv)
{
    streamer_t * sout = streamer_get();
    char base64[256];
    uint8_t binary[256];
...
int CommandBase64Dispatch(int argc, char ** argv)
{
    CHIP_ERROR error = CHIP_NO_ERROR;

    if (argc == 0)
    {
        CommandBase64Help(argc, argv);
        return error;
    }
...
void CommandBase64Init()
{
    ...
    static const shell_command_t sBase64SubCommands[] = {
        { &CommandBase64Help, "help", "Usage: base64 <subcommand>" },
        { &CommandBase64Encode, "encode", "Encode a hex sting as base64. Usage: base64 encode <hex_string>" },
        { &CommandBase64Decode, "decode", "Decode a base64 sting as hex. Usage: base64 decode <base64_string>" },
    };

    static const shell_command_t sBase64Command = { &CommandBase64Dispatch, "base64", "Base64 encode / decode utilities" };
