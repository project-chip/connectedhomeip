...
class BdxDownloader : public bdx::Initiator
{
public:
    ...
    void HandleTransferSessionOutput(bdx::TransferSession::OutputEvent & event) override
    {
        ...
        case OutputEventType::kNone:
            if (mIsTransferComplete)
            {
                ...
                status              = dfu_target_done(true);
                VerifyOrReturn(error == CHIP_NO_ERROR, ChipLogError(BDX, "dfu_target_done() failed: %d", status));
            }
            break;
        case OutputEventType::kMsgToSend: {
            ...
            error = mExchangeCtx->SendMessage(event.msgTypeData.ProtocolId, event.msgTypeData.MessageType, std::move(event.MsgData),
                                              flags);
