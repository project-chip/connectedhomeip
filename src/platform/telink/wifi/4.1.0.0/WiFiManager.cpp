        ...
        if (!mInternalScan) // don't erase in the middle of Connect procedure
        {
            ...
            if (ssid.size() <= DeviceLayer::Internal::kMaxWiFiSSIDLength)
            {
                ...
                ChipLogProgress(DeviceLayer, "Directed Scanning, looking for: %s",
                                InlineString(100, static_cast<int>(ssid.size()), ssid.data()));
            }
            else
            {
...
void WiFiManager::ScanResultHandler(Platform::UniquePtr<uint8_t> data, size_t length)
{
    ...
    ChipLogDetail(DeviceLayer, "Found SSID: %s", InlineString(100, static_cast<int>(scanResult->ssid_length), scanResult->ssid));
