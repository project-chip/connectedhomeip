    ...
    if (!ssid.empty()) // Directed Scanning, Main Spec, 11.9.7.1. "ScanNetworks Command"
    {
        if (!mInternalScan) // don't erase in the middle of Connect procedure
        {
            ...
            if (ssid.size() <= DeviceLayer::Internal::kMaxWiFiSSIDLength)
            {
                ...
                ChipLogProgress(DeviceLayer, "Directed Scanning, looking for: %s", ChipLogFormat(100, "%.*s", static_cast<int>(ssid.size()), ssid.data()));
            }
            else
            {
...
void WiFiManager::ScanResultHandler(Platform::UniquePtr<uint8_t> data, size_t length)
{
    ...
    ChipLogDetail(DeviceLayer, "Found SSID: %s", ChipLogFormat(100, "%.*s", static_cast<int>(scanResult->ssid_length), scanResult->ssid).c_str());
