        ...
        mGattCallback =
                new BluetoothGattCallback() {
                    ...
                    public void onCharacteristicWrite(
                        ...
                        if (connId > 0) {
                            AndroidChipPlatform.getInstance().handleWriteConfirmation(
                    ...
                    public void onCharacteristicChanged(
                        ...
                        int connId = getConnId(gatt);
                        if (connId > 0) {
                            AndroidChipPlatform.getInstance().handleIndicationReceived(connId, svcIdBytes, charIdBytes, characteristic.getValue());
                        } else {
                    ...
                    public void onDescriptorWrite(
                        ...
                        if (desc.getValue() == BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE) {
                            AndroidChipPlatform.getInstance().handleSubscribeComplete(
                                    connId, svcIdBytes, charIdBytes, status == BluetoothGatt.GATT_SUCCESS);
                        } else if (desc.getValue() == BluetoothGattDescriptor.DISABLE_NOTIFICATION_VALUE) {
                            AndroidChipPlatform.getInstance().handleUnsubscribeComplete(
                                    connId, svcIdBytes, charIdBytes, status == BluetoothGatt.GATT_SUCCESS);
                        } else {
