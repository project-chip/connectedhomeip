...
public class AndroidNfcCommissioningManager implements NfcCommissioningManager {
  ...
  private static final int TYPE4_SIMPLE_APDU_MAX_TX_SIZE  = 245;   // NB: 245 is the optimum size to get the biggest TX chunks and thus the max throughput
  private static final int TYPE4_SIMPLE_APDU_MAX_RX_SIZE  = 250;   // NB: 250 is the optimum size to get the biggest RX chunks and thus the max throughput
  ...
  // response packet. The next response packet can be read thanks to a call to getResponse() command.
  public byte[] processAPDUResponse(byte[] response) throws IOException {
      ...
      if ((sw1 == ((byte) 0x90)) && (sw2 == 0x00)) {
          ...
          while (sw1 == 0x61) {
              ...
              byte nextBlockLength = ( (sw2 == 0x00) || ((int) sw2 > TYPE4_SIMPLE_APDU_MAX_RX_SIZE)) ? (byte) TYPE4_SIMPLE_APDU_MAX_RX_SIZE : sw2;
  ...
  public byte[] sendTransportAPDU(byte[] data, boolean isLastBlock, int totalLength) throws IOException {
  ...
  public byte[] transceive(String commandName, byte[] data) throws IOException {
    ...
    Log.d(TAG, "==> Send " + commandName + " command (" + data.length + " bytes) : " + convertHexByteArrayToString(data));
  ...
  public byte[] getChainedResponseBufferData() {
      ...
      Log.d(TAG, "chainedResponse (" + chainedResponseLen + " bytes): " + convertHexByteArrayToString(result));
