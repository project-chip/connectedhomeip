...
void OTAImageProcessorImpl::HandleFinalize(intptr_t context)
{
    ...
    {
        // write last tempBUF
        if (!WriteFlashChunk(mFlashWriteOffset + SLOT_DWL_A_START, reinterpret_cast<std::uint8_t *>(tempBUF),
        ...
        else
        {
            ...
            if (nb_blocks != 0U)
            {
                ...
                if (!WriteFlashChunk(mFlashWriteOffset + SLOT_DWL_A_START,
                                     ...
                                     static_cast<std::uint32_t>(nb_blocks * 16U), imageProcessor))
                {
                    ChipLogError(SoftwareUpdate, "Flash write failed");
                    imageProcessor->mDownloader->EndDownload(CHIP_ERROR_WRITE_FAILED);
                    return;
    ...
    { // new block received
        ...
        if (extra_bytes != 0U)
        {
            ...
            if ((blocksize + extra_bytes) < TEMPBUF_SIZE)
            {
                ...
                memcpy(tempBUF + extra_bytes, reinterpret_cast<std::uint8_t *>(imageProcessor->mBlock.data()), tempBuf_complement);
        ...
        // ChipLogProgress(SoftwareUpdate, "tempBuf_complement=%lu imageProcessor->mParams.downloadedBytes=%lu ",
        // tempBuf_complement, imageProcessor->mParams.downloadedBytes);
