...
/* OEMiROT Magic value */
const uint32_t MagicTrailerValue[] = { 0xf395c277, 0x7fefd260, 0x0f505235,
		0x8079b62c, };
...
bool OTAImageProcessorImpl::WriteMagicValue(uint32_t dest) {
	...
	status =
			F_OTA_FLASH_WriteChunk(reinterpret_cast<uint32_t*>(dest),
...
bool OTAImageProcessorImpl::WriteFlashChunk(uint32_t dest, uint8_t *pSrc,
		uint32_t size, OTAImageProcessorImpl *imageProcessor) {

	ChipLogProgress(DeviceLayer, "WriteFlashChunk:  @ %p , size = %lu",
			(void* )dest, size);

	STM_OTA_StatusTypeDef status;
	status = F_OTA_FLASH_WriteChunk(reinterpret_cast<uint32_t*>(dest),
			reinterpret_cast<uint32_t*>(pSrc), static_cast<uint32_t>(size));
	if (status != STM_OTA_FLASH_OK) {
		// retry 5 times
		for (int i = 0; i < 5; i++) {
...
bool OTAImageProcessorImpl::DeleteImages(
...
CHIP_ERROR OTAImageProcessorImpl::ConfirmCurrentImage() {
	...
	ReturnErrorOnFailure(
			DeviceLayer::ConfigurationMgr().GetSoftwareVersion(currentVersion));
...
CHIP_ERROR OTAImageProcessorImpl::PrepareDownload() {
	...
	return (DeviceLayer::PlatformMgr().ScheduleWork(HandlePrepareDownload,
...
CHIP_ERROR OTAImageProcessorImpl::ProcessHeader(ByteSpan &block) {
	...
	if (mHeaderParser.IsInitialized()) {
		...
		VerifyOrReturnError(error != CHIP_ERROR_BUFFER_TOO_SMALL,
				CHIP_NO_ERROR);
		...
		OtaImgHeader.minApplicableVersion =
				header.mMinApplicableVersion.ValueOr(0);
		OtaImgHeader.maxApplicableVersion =
				header.mMaxApplicableVersion.ValueOr(0);
...
CHIP_ERROR OTAImageProcessorImpl::Finalize() {
	...
	return (DeviceLayer::PlatformMgr().ScheduleWork(HandleFinalize,
			reinterpret_cast<intptr_t>(this)));
...
CHIP_ERROR OTAImageProcessorImpl::Apply() {
	...
	if (mCPU1Size != 0) {
		...
		if (mCPU1Size <= SLOT_DWL_A_SIZE) {
			uint32_t MagicAddress = SLOT_DWL_A_END - sizeof(MagicTrailerValue)
					+ 1;
			...
			ChipLogError(SoftwareUpdate,
					"Not enough space to write Magic value");
			return CHIP_ERROR_INTERNAL;
		}
	}
	//write  software version in nvm for validation for next reboot
	error = NVM_SetKeyValue((char*) X_CUBE_MATTER_VERSION, SOFTWARE_VERSION_KEY,
			sizeof(X_CUBE_MATTER_VERSION), SECTOR_MATTER);
	if (error != 0) {
		return CHIP_ERROR_INTERNAL;
	}
	NVM_Dump();
...
CHIP_ERROR OTAImageProcessorImpl::Abort() {
	return (DeviceLayer::PlatformMgr().ScheduleWork(HandleAbort,
			reinterpret_cast<intptr_t>(this)));
...
CHIP_ERROR OTAImageProcessorImpl::ProcessBlock(ByteSpan &block) {
	...
	if (err != CHIP_NO_ERROR) {
		ChipLogError(SoftwareUpdate, "Matter image header parser error %s",
				chip::ErrorStr(err));
	...
	if (err != CHIP_NO_ERROR) {
		ChipLogError(SoftwareUpdate,
				"Cannot set block data: %" CHIP_ERROR_FORMAT, err.Format());
		return err;
	}

	err = DeviceLayer::PlatformMgr().ScheduleWork(HandleProcessBlock,
			reinterpret_cast<intptr_t>(this));
	return err;
}
...
void OTAImageProcessorImpl::HandleFinalize(intptr_t context) {
	...
	if (extra_bytes != 0U) {
		// write last tempBUF
		if (!WriteFlashChunk(mFlashWriteOffset + SLOT_DWL_A_START,
				reinterpret_cast<std::uint8_t*>(tempBUF),
				static_cast<std::uint32_t>(TEMPBUF_SIZE), imageProcessor)) {
			ChipLogError(SoftwareUpdate, "Flash write failed");
			imageProcessor->mDownloader->EndDownload(CHIP_ERROR_WRITE_FAILED);
		}
	}
...
void OTAImageProcessorImpl::HandleProcessBlock(intptr_t context) {
	...
	if (!stm_header_decoded) {
		...
		mCPU1Size = STMHeader[0] + ((STMHeader[1]) << 8)
				+ ((STMHeader[2]) << 16) + ((STMHeader[3]) << 24);
		mCPU2Size = STMHeader[4] + ((STMHeader[5]) << 8)
				+ ((STMHeader[6]) << 16) + ((STMHeader[7]) << 24);
		...
		// check the header
		if ((mCPU1Size > SLOT_DWL_A_SIZE) || (mCPU2Size != 0)) {
		...
		if (mCPU1Size == 0) {
			...
			if (nb_blocks != 0U) {
				...
				if (!WriteFlashChunk(mFlashWriteOffset + SLOT_DWL_A_START,
						...
						imageProcessor)) {
					ChipLogError(SoftwareUpdate, "Flash write failed");
					imageProcessor->mDownloader->EndDownload(
		...
		if (extra_bytes != 0U) {
			memcpy(tempBUF,
					reinterpret_cast<std::uint8_t*>(imageProcessor->mBlock.data())
							+ tempBuf_complement + (nb_blocks * 16U),
					extra_bytes);
		} else {
			memset(tempBUF, TEMPBUF_PADDING, TEMPBUF_SIZE);
		}
...
CHIP_ERROR OTAImageProcessorImpl::SetBlock(ByteSpan &block) {
	...
	if (mBlock.size() < block.size()) {
		...
		uint8_t *mBlock_ptr = static_cast<uint8_t*>(chip::Platform::MemoryAlloc(
				block.size()));
	...
	if (err != CHIP_NO_ERROR) {
		ChipLogError(SoftwareUpdate,
				"Cannot copy block data: %" CHIP_ERROR_FORMAT, err.Format());
		return err;
	}
