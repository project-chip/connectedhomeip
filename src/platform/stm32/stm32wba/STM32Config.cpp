...
CHIP_ERROR STM32Config::ReadConfigValueBin(Key key, uint8_t * buf, size_t bufSize, size_t & outLen)
{
    ...
    return PrintError(NVM_GetKeyValue(buf, (char *) buffer_key, bufSize, &outLen, SECTOR_MATTER));
}
...
CHIP_ERROR STM32Config::ReadConfigValue(Key key, bool & val)
{
    ...
    sprintf((char *) buffer_key, "Config%li", key);
    return PrintError(
        NVM_GetKeyValue(reinterpret_cast<uint8_t *>(&val), (char *) buffer_key, sizeof(bool), &Out_Length, SECTOR_MATTER));
}
...
CHIP_ERROR STM32Config::ReadConfigValue(Key key, uint32_t & val)
{
    ...
    sprintf((char *) buffer_key, "Config%li", key);
    return PrintError(
        NVM_GetKeyValue(reinterpret_cast<uint8_t *>(&val), (char *) buffer_key, sizeof(uint32_t), &Out_Length, SECTOR_MATTER));
}
...
CHIP_ERROR STM32Config::ReadConfigValue(Key key, uint64_t & val)
{
    ...
    sprintf((char *) buffer_key, "Config%li", key);
    return PrintError(
        NVM_GetKeyValue(reinterpret_cast<uint8_t *>(&val), (char *) buffer_key, sizeof(uint64_t), &Out_Length, SECTOR_MATTER));
}
