...
CHIP_ERROR OTAImageProcessorImpl::ProcessHeader(ByteSpan & block)
{
    if (mHeaderParser.IsInitialized())
    {
        ...
        if (false == qvCHIP_OtaImageDownloadInProgress())
        {
            ...
            uint16_t headerSize;
            VerifyOrReturnError(qvCHIP_OtaStatusSuccess == qvCHIP_OtaValidateandStoreHeader(block.data(), block.size(),
                                &headerSize, nullptr, 128), CHIP_ERROR_INTEGRITY_CHECK_FAILED);
