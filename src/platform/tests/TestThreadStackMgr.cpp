...
int TestThreadStackManager()
{
    ...
    chip::DeviceLayer::Internal::ThreadOperationalDataset info;
    uint16_t masterKey[16] = { 0x11, 0x22, 0x33, 0x44, 0x55, 0x66, 0x77, 0x88, 0x99, 0xaa, 0xbb, 0xcc, 0xdd, 0xee, 0xff };

    info.SetNetworkName("CHIP-TEST");
    info.SetChannel(UINT8_MAX);
    info.SetPanId(0x3455);
    info.SetMasterKey(&masterKey);
