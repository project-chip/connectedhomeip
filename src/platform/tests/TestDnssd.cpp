...
static void HandleBrowse(void * context, DnssdService * services, size_t servicesSize, CHIP_ERROR error)
{
    ...
    if (services)
    {
        ...
        NL_TEST_ASSERT(suite, ChipDnssdResolve(services, chip::Inet::InterfaceId(), HandleResolve, suite) == CHIP_NO_ERROR);
    }
}
...
static void InitCallback(void * context, CHIP_ERROR error)
{
    ...
    service.mInterface = chip::Inet::InterfaceId();
    service.mPort      = 80;
    ...
    ChipDnssdBrowse("_mock", DnssdServiceProtocol::kDnssdProtocolTcp, chip::Inet::IPAddressType::kAny, chip::Inet::InterfaceId(),
                    HandleBrowse, suite);
}
