...
CHIP_ERROR CC13X2_26X2Config::ReadConfigValueBin(Key key, uint8_t * buf, size_t bufSize, size_t & outLen)
{
    ...
    VerifyOrExit(sNvoctpFps.readItem(key.nvID, 0, (uint16_t)len, buf) == NVINTF_SUCCESS, err = CHIP_ERROR_PERSISTED_STORAGE_FAILED);
...
CHIP_ERROR CC13X2_26X2Config::WriteConfigValueBin(Key key, const uint8_t * data, size_t dataLen)
{
    ...
    VerifyOrExit(sNvoctpFps.writeItem(key.nvID, (uint16_t)dataLen, data) == NVINTF_SUCCESS, err = CHIP_ERROR_PERSISTED_STORAGE_FAILED);
exit:
    return err;
}
