...
CHIP_ERROR ConnectivityManagerImplWiFiPAF::_WiFiPAFPublish(ConnectivityManager::WiFiPAFAdvertiseParam & args)
{
    ...
    if (sscanf(params.resp, "%u", &publishId) == 1 && publishId <= UINT8_MAX)
    {
        {
            NanMutexLock lock(mNanMutex);
...
#endif
{
    ...
    case NET_EVENT_WIFI_CONNECT_RESULT: {
        ...
        {
            NanMutexLock lock(self.mNanMutex);
            self.mPafChannelAvailable = false;
            ChipLogProgress(DeviceLayer, "WiFi-PAF: WiFi connected to AP, PAF channel unavailable");
        }
        break;
    }
    ...
    case NET_EVENT_WIFI_DISCONNECT_RESULT: {
        NanMutexLock lock(self.mNanMutex);
        self.mPafChannelAvailable = true;
        ChipLogProgress(DeviceLayer, "WiFi-PAF: WiFi disconnected from AP, PAF channel available");
    }
    break;
...
{
    mPafAdvParam.freq_list_len = (pafAdvParam.freq_list != nullptr) ? pafAdvParam.freq_list_len : 0;
