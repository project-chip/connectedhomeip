...
CHIP_ERROR ConnectivityManagerImplWiFiPAF::_WiFiPAFPublish(ConnectivityManager::WiFiPAFAdvertiseParam & args) 
{
    ...
    if (args.freq_list_len > 0 && args.freq_list != nullptr)
    {
        ...
        size_t remaining = sizeof(params.publish.freq_list) - 1;  // Reserve for null terminator
        for (size_t i = 0; i < copyLen && remaining > 0; i++)
        {
            char freqStr[16];
            int written = snprintf(freqStr, sizeof(freqStr), "%u%s", 
                                args.freq_list[i], (i < copyLen - 1) ? "," : "");
            if (written < 0 || static_cast<size_t>(written) >= remaining)
                break;  // Stop if buffer full
            
            strncat(params.publish.freq_list, freqStr, remaining);
            remaining -= written;
        }
        params.publish.freq_list[sizeof(params.publish.freq_list) - 1] = '\0';
    }

    // Service Specific Information (SSI)
    static_assert(sizeof(ssi) <= sizeof(params.publish.ssi), "SSI too large");
...
CHIP_ERROR ConnectivityManagerImplWiFiPAF::_WiFiPAFSend(const WiFiPAF::WiFiPAFSession & TxInfo, System::PacketBufferHandle && msgBuf)
{
    ...
    VerifyOrReturnError(dataLen <= sizeof(params.transmit.ssi), CHIP_ERROR_BUFFER_TOO_SMALL,
                        ChipLogError(DeviceLayer, "WiFi-PAF: Payload too large: %zu > %zu", 
                                     dataLen, sizeof(params.transmit.ssi)));
    VerifyOrReturnError(dataLen <= UINT8_MAX, CHIP_ERROR_INVALID_ARGUMENT,
                        ChipLogError(DeviceLayer, "WiFi-PAF: SSI length overflow: %zu > %u",
                                     dataLen, UINT8_MAX));

    memcpy(params.transmit.ssi, msgBuf->Start(), dataLen);
    params.transmit.ssi_len = static_cast<uint8_t>(dataLen);
...
void ConnectivityManagerImplWiFiPAF::OnNanReceive(const wifi_nan_receive_event * event)
{
    ...
    VerifyOrReturn(event->ssi_len > 0 && event->ssi_len <= sizeof(event->ssi),
                   ChipLogError(DeviceLayer, "WiFi-PAF: OnNanReceive: invalid SSI length: %u", 
