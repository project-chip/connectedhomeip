...
namespace {
// RAII guard for mNanMutex.
class NanMutexLock
{
public:
    explicit NanMutexLock(struct k_mutex & m) : mMutex(m) { k_mutex_lock(&mMutex, K_FOREVER); }
    ~NanMutexLock() { k_mutex_unlock(&mMutex); }
    NanMutexLock(const NanMutexLock &)             = delete;
    NanMutexLock & operator=(const NanMutexLock &) = delete;
...
CHIP_ERROR ConnectivityManagerImplWiFiPAF::InitWiFiPAF()
{
    ...
    mNanRecoveryId.store(0);
    mArmedNanRecoveryId.store(0);
    ...
    // Register WiFi STA event callbacks to track AP connection state
    net_mgmt_init_event_callback(&mWifiStaMgmtClbk, WifiStaMgmtEventHandler, kWifiStaManagementEvents);
    net_mgmt_add_event_callback(&mWifiStaMgmtClbk);

    // Learn when a station join starts so PAF frames can be held during the join.
    WiFiManager::Instance().SetOnConnectStartedCallback([]() { GetInstance().OnWiFiConnectStarted(); });
    WiFiManager::Instance().SetOnConnectFailedCallback([]() { GetInstance().OnWiFiConnectFailed(); });

    ChipLogProgress(DeviceLayer, "WiFi-PAF: NAN and WiFi STA event callbacks registered");
...
CHIP_ERROR ConnectivityManagerImplWiFiPAF::_WiFiPAFPublish(ConnectivityManager::WiFiPAFAdvertiseParam & args)
{
    ...
    ssi.DevInfo = discriminator & 0x0FFF; // b15:b12 = advertisement version (0x0), b11:b0 = discriminator
    ...
    // Service Specific Information (SSI) - copy into inline array
    static_assert(sizeof(ssi) <= sizeof(params.publish.ssi), "SSI too large for NAN publish buffer");
    memcpy(params.publish.ssi, &ssi, sizeof(ssi));
...
CHIP_ERROR ConnectivityManagerImplWiFiPAF::_WiFiPAFCancelPublish(uint32_t PublishId)
{
...

    {
        ...
        // Verify the ID matches the current publish
        if (mCurrentPublishId != PublishId)
        {
            ChipLogError(DeviceLayer, "WiFi-PAF: Publish ID mismatch (current=%u, requested=%u)", mCurrentPublishId, PublishId);
            return CHIP_ERROR_INCORRECT_STATE;
        }
    }
                                                        ...
                                                        System::PacketBufferHandle && msgBuf)
{
    ...
    // Set SSI payload (copy into inline array; net_mgmt is synchronous so lifetime is safe)
    size_t dataLen = msgBuf->DataLength();
    static_assert(sizeof(params.transmit.ssi) >= CHIP_PAF_DEFAULT_MTU, "SSI buffer too small for PAF MTU");
    VerifyOrReturnError(dataLen <= sizeof(params.transmit.ssi), CHIP_ERROR_MESSAGE_TOO_LONG,
                        ChipLogError(DeviceLayer, "WiFi-PAF: Frame too large: %u", static_cast<unsigned>(dataLen)));
    memcpy(params.transmit.ssi, msgBuf->Start(), dataLen);
    params.transmit.ssi_len = static_cast<uint16_t>(dataLen);
...
CHIP_ERROR ConnectivityManagerImplWiFiPAF::_WiFiPAFShutdown(uint32_t id, WiFiPAF::WiFiPafRole role)
{
    ...
    // Deregister event callbacks to prevent handler from firing after shutdown
    net_mgmt_del_event_callback(&mNanMgmtClbk);
    net_mgmt_del_event_callback(&mWifiStaMgmtClbk);
    ChipLogProgress(DeviceLayer, "WiFi-PAF: Event callbacks deregistered");
...
void ConnectivityManagerImplWiFiPAF::OnNanReceive(const wifi_nan_receive_event * event)
{
    ...
    {
        NanMutexLock lock(mNanMutex);
        isActiveSession = (mPublishActive && event->id == mCurrentPublishId);
    }
    VerifyOrReturn(isActiveSession, ChipLogProgress(DeviceLayer, "WiFi-PAF: Ignoring event for inactive session %u", event->id));

    VerifyOrReturn(
        event->ssi_len > 0 && event->ssi_len <= sizeof(event->ssi),
        ChipLogError(DeviceLayer, "WiFi-PAF: OnNanReceive: invalid SSI length %u", static_cast<unsigned>(event->ssi_len)));
    ...
    // Create packet buffer (copies data from inline SSI array)
...
void ConnectivityManagerImplWiFiPAF::OnNanPublishTerminated(const wifi_nan_terminated_event * event)
{
    ...
    // Marshal to CHIP thread to safely access PAF layer and post events
    uint8_t terminatedId = event->id;
    CHIP_ERROR schedErr  = DeviceLayer::SystemLayer().ScheduleLambda([this, terminatedId]() {
        // Clean up state
        if (terminatedId == mCurrentPublishId)
        {
            {
                NanMutexLock lock(mNanMutex);
                mCurrentPublishId = 0;
    ...
    if (schedErr != CHIP_NO_ERROR)
    {
        ChipLogError(DeviceLayer, "WiFi-PAF: Failed to schedule OnNanPublishTerminated: %" CHIP_ERROR_FORMAT, schedErr.Format());
    }
}
...
#endif
{
    ...
    case NET_EVENT_WIFI_NAN_REPLIED: {
        ...
        if (event != nullptr)
        {
            // NAN traffic observed: release the post-association hold, if any.
            self.PafChannelNoteNanActivity();
...
#if ZEPHYR_VERSION_CODE >= ZEPHYR_VERSION(4, 2, 0)
void ConnectivityManagerImplWiFiPAF::WifiStaMgmtEventHandler(net_mgmt_event_callback * cb, uint64_t mgmtEvent, net_if * iface)
#else
void ConnectivityManagerImplWiFiPAF::WifiStaMgmtEventHandler(net_mgmt_event_callback * cb, uint32_t mgmtEvent, net_if * iface)
#endif
{
    ConnectivityManagerImplWiFiPAF & self = GetInstance();
    ...
    case NET_EVENT_WIFI_CONNECT_RESULT: {
        const wifi_status * status = static_cast<const wifi_status *>(cb->info);
        if (status != nullptr && status->conn_status == WIFI_STATUS_CONN_SUCCESS)
        {
            // Only transition to kAwaitingNan if we were in a PAF-initiated join (kConnecting).
            // Unrelated reconnects (e.g., WiFi recovery) should not block PAF sends.
            PafChannelState prior = self.mPafChannelState.load();
            if (prior != PafChannelState::kConnecting)
            {
                break;
            }

            // Station associated. NAN needs a little longer than the link to resume, so hold PAF
            // frames until NAN traffic is seen or the recovery timer fires. Bump the recovery id
            // first so a timer left over from a previous join cannot release this one.
            self.mNanRecoveryId++;
            self.mPafChannelState.store(PafChannelState::kAwaitingNan);
            ChipLogProgress(DeviceLayer, "WiFi-PAF: station associated, awaiting NAN recovery");
            // StartTimer must run on the CHIP thread; this handler runs on the net_mgmt thread.
            LogErrorOnFailure(DeviceLayer::SystemLayer().ScheduleLambda([]() { GetInstance().ArmNanRecoveryTimer(); }));
        }
        else
        {
            // Association failed. PAF does not need the station link, so release held frames now.
            self.mPafChannelState.store(PafChannelState::kAvailable);
            ChipLogProgress(DeviceLayer, "WiFi-PAF: station association failed, PAF channel available");
            LogErrorOnFailure(
    ...
    case NET_EVENT_WIFI_DISCONNECT_RESULT: {
        // Only release if an established link went down (kAwaitingNan). Do not release during a
        // join (kConnecting): WiFiManager disconnects before (re)associating, and that deliberate
        // disconnect also lands here - releasing then would let frames out mid-association.
        PafChannelState expected = PafChannelState::kAwaitingNan;
        if (self.mPafChannelState.compare_exchange_strong(expected, PafChannelState::kAvailable))
        {
            ChipLogProgress(DeviceLayer, "WiFi-PAF: station disconnected, PAF channel available");
            LogErrorOnFailure(
                DeviceLayer::SystemLayer().ScheduleLambda([]() { WiFiPAF::WiFiPAFLayer::GetWiFiPAFLayer().DrivePendingSends(); }));
        }
        break;
...
void ConnectivityManagerImplWiFiPAF::OnWiFiConnectStarted()
{
    // Flush any owed ack, then hold PAF frames before the internal scan moves the radio off the
    // NAN channel. Set kConnecting here (not after the scan) so the whole join is covered.
    WiFiPAF::WiFiPAFLayer::GetWiFiPAFLayer().FlushPendingAcks();
    mPafChannelState.store(PafChannelState::kConnecting);
    ChipLogProgress(DeviceLayer, "WiFi-PAF: station join starting, holding PAF frames");
}

void ConnectivityManagerImplWiFiPAF::OnWiFiConnectFailed()
{
    // The join never reached the driver, so no connect-result event will arrive to release the
    // channel. Release it and wake queued sends rather than waiting on the resource timer.
    mPafChannelState.store(PafChannelState::kAvailable);
    ChipLogProgress(DeviceLayer, "WiFi-PAF: station join did not start, PAF channel available");
    WiFiPAF::WiFiPAFLayer::GetWiFiPAFLayer().DrivePendingSends();
}

void ConnectivityManagerImplWiFiPAF::ArmNanRecoveryTimer()
{
    // Only meaningful while awaiting NAN after an association.
    VerifyOrReturn(mPafChannelState.load() == PafChannelState::kAwaitingNan);

    // Record which association this timer belongs to, so two joins in quick succession settle on
    // the later one. StartTimer() cancels any timer already registered for this handler+context.
...
void ConnectivityManagerImplWiFiPAF::HandleNanRecoveryTimeout(System::Layer * layer, void * context)
{
    auto * self = static_cast<ConnectivityManagerImplWiFiPAF *>(context);
    ...
    ChipLogProgress(DeviceLayer, "WiFi-PAF: no NAN activity after association; releasing PAF frames anyway");
    WiFiPAF::WiFiPAFLayer::GetWiFiPAFLayer().DrivePendingSends();
}

bool ConnectivityManagerImplWiFiPAF::TryReleaseNanRecoveryWait()
{
    PafChannelState expected = PafChannelState::kAwaitingNan;
    return mPafChannelState.compare_exchange_strong(expected, PafChannelState::kAvailable);
}

void ConnectivityManagerImplWiFiPAF::PafChannelNoteNanActivity()
{
    // False on most frames: only the first NAN activity after an association has a wait to release.
    VerifyOrReturn(TryReleaseNanRecoveryWait());

    ChipLogProgress(DeviceLayer, "WiFi-PAF: NAN active again after association; releasing PAF frames");

    // Runs on the net_mgmt thread, so drive the queued sends on the CHIP thread.
    LogErrorOnFailure(
        DeviceLayer::SystemLayer().ScheduleLambda([]() { WiFiPAF::WiFiPAFLayer::GetWiFiPAFLayer().DrivePendingSends(); }));
}
