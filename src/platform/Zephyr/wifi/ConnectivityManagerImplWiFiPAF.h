...
private:
    ...
    // Checked right before sending a PAF frame: true only when the radio is on the NAN channel.
    ...
    // Station-join hooks invoked on the CHIP thread from WiFiManager; hold/release frames across a join.
    void OnWiFiConnectStarted();
    void OnWiFiConnectFailed();
    // Arm/handle the timer that releases held PAF frames if NAN traffic does not resume after
    // the station associates.
    void ArmNanRecoveryTimer();
    static void HandleNanRecoveryTimeout(System::Layer * layer, void * context);
    // Atomically release the post-association NAN wait (kAwaitingNan -> kAvailable). Returns true
    // if this call performed the transition.
    bool TryReleaseNanRecoveryWait();
    // Called when NAN activity is observed; releases the post-association wait and drives pending sends.
    ...
    // Written from both the net_mgmt thread and the CHIP thread.
    enum class PafChannelState : uint8_t
    {
        kAvailable,   // The radio is free and PAF frames may be sent
        kConnecting,  // Station join in progress (scan, auth, assoc, key handshake); radio off NAN channel
        kAwaitingNan, // STA link up but NAN traffic not yet observed after the join
        kNoInterface, // No Wi-Fi interface to send on (unused on Zephyr today)
    };
    std::atomic<PafChannelState> mPafChannelState{ PafChannelState::kAvailable };

    // Recovery-timer generation counters. mNanRecoveryId is bumped on each association so a stale
    // timer from an earlier join cannot release a later one; mArmedNanRecoveryId records the id the
    // currently pending timer was armed for.
