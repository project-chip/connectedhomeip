 ...
 *          for the scm1612s (NimBLE) platform.
 */
...
#include <platform/senscomm/scm1612s/BLEManagerImpl.h>
...
uint8_t own_addr_type = BLE_OWN_ADDR_PUBLIC; /* BLE_OWN_ADDR_RANDOM; */
...
void BLEManagerImpl::HandleConnectFailed(CHIP_ERROR error)
{
    if (sInstance.mIsCentral)
    {
        ...
        event.Type                                    = DeviceEventType::kPlatformSCMBLECentralConnectFailed;
...
void BLEManagerImpl::HandlePlatformSpecificBLEEvent(const ChipDeviceEvent * apEvent)
{
    ...
    switch (apEvent->Type)
    {
    case DeviceEventType::kPlatformSCMBLECentralConnected:
    ...
    case DeviceEventType::kPlatformSCMBLECentralConnectFailed:
        if (BLEManagerImpl::mBLEScanConfig.mBleScanState == BleScanState::kConnecting)
        {
    ...
    case DeviceEventType::kPlatformSCMBLEIndicationReceived:
                                     ...
                                     void * arg)
{
    ...
    ChipDeviceEvent event;
    event.Type                                          = DeviceEventType::kPlatformSCMBLESubscribeOpComplete;
...
bool BLEManagerImpl::CloseConnection(BLE_CONNECTION_OBJECT conId)
{
    ...
    // Signal the SCM BLE layer to close the conneecction.
                                    ...
                                    chip::System::PacketBufferHandle data)
{
    ...
    WISE_LOGD(TAG, "Sending indication for CHIPoBLE TX characteristic (con %u, len %u)", conId, data->DataLength());
...
static int OnWriteComplete(uint16_t conn_handle, const struct ble_gatt_error * error, struct ble_gatt_attr * attr, void * arg)
{
    ...
    ChipDeviceEvent event;
    event.Type                                  = DeviceEventType::kPlatformSCMBLEWriteComplete;
...
CHIP_ERROR BLEManagerImpl::MapBLEError(int bleErr)
{
    ...
    case WISE_ERR_INVALID_ARG:
        return CHIP_ERROR_INVALID_ARGUMENT;
    default:
        return CHIP_ERROR(ChipError::Range::kPlatform, CHIP_DEVICE_CONFIG_BLE_ERROR_MIN + bleErr);
    }
}
...
void BLEManagerImpl::DriveBLEState(void)
{
    ...
    // Initializes the SCM BLE layer if needed.
    if (mServiceMode == ConnectivityManager::kCHIPoBLEServiceMode_Enabled && !mFlags.Has(Flags::kSCMBLELayerInitialized))
    {
        err = InitSCMBleLayer();
        SuccessOrExit(err);
    ...
    )
    {
        ...
        // SCM BLE layer needs to be refreshed.
        if (!mFlags.Has(Flags::kAdvertising) || mFlags.Has(Flags::kAdvertisingRefreshNeeded))
        {
            ...
            WISE_LOGD(TAG, "NimBLE start advertising...");
...
{
    WISE_LOGE(TAG, "Resetting state; reason=%d\n", reason);
}
...
CHIP_ERROR BLEManagerImpl::bleprph_set_random_addr(void)
{
    ...
    if (rc != 0)
    {
        WISE_LOGE(TAG, "Failed to generate random address err: %d", rc);
    ...
    if (rc != 0)
    {
        WISE_LOGE(TAG, "Failed to set random address err: %d", rc);
        return CHIP_ERROR_INTERNAL;
    }
    ...
    if (rc != 0)
    {
        WISE_LOGE(TAG, "Failed to configure random address err: %d", rc);
        return CHIP_ERROR_INTERNAL;
    }
    return CHIP_NO_ERROR;
}
...
{
    WISE_LOGI(TAG, "BLE host-controller synced");
...
{
    WISE_LOGD(TAG, "BLE Host Task Started");
    /* This function will return only when nimble_port_stop() is executed */
    nimble_port_run();
    ble_npl_task_remove(&s_task_host);
    return NULL;
}

CHIP_ERROR BLEManagerImpl::InitSCMBleLayer(void)
{
    CHIP_ERROR err = CHIP_NO_ERROR;

    VerifyOrExit(!mFlags.Has(Flags::kSCMBLELayerInitialized), /* */);

    semaphoreHandle = xSemaphoreCreateBinary();
    if (semaphoreHandle == NULL)
    {
        err = CHIP_ERROR_NO_MEMORY;
        WISE_LOGE(TAG, "Failed to create semaphore");
        ExitNow();
    }
    ...
    /* XXX: this has already been done in soc_init(). */
#if 0
    nimble_port_init();
#endif
    ...
    // Register the CHIPoBLE GATT attributes with the SCM BLE layer if needed.
    if (mServiceMode == ConnectivityManager::kCHIPoBLEServiceMode_Enabled)
    {
        ble_svc_gap_init();
        ble_svc_gatt_init();
    ...
    sInstance.mFlags.Set(Flags::kSCMBLELayerInitialized);
...
void BLEManagerImpl::DeinitSCMBleLayer()
{
...
CHIP_ERROR BLEManagerImpl::ConfigureAdvertisingData(void)
{
    ...
    advData[index++] = kServiceDataTypeSize + sizeof(SCMChipServiceData);                 // length
...
void BLEManagerImpl::OnGattDiscComplete(const struct peer * peer, int status, void * arg)
{
    ...
    ChipDeviceEvent event;
    event.Type                                     = DeviceEventType::kPlatformSCMBLECentralConnected;
...
int BLEManagerImpl::ble_svr_gap_event(struct ble_gap_event * event, void * arg)
{
    ...
    case BLE_GAP_EVENT_MTU:
        WISE_LOGD(TAG, "BLE_GAP_EVENT_MTU = %d channel id = %d", event->mtu.value, event->mtu.channel_id);
        break;

    case BLE_GAP_EVENT_NOTIFY_RX:
        WISE_LOGD(TAG, "BLE_GAP_EVENT_NOTIFY_RX received %s conn_handle:%d attr_handle:%d attr_len:%d",
...
CHIP_ERROR BLEManagerImpl::HandleRXNotify(struct ble_gap_event * ble_event)
{
    ...
    ChipDeviceEvent event;
    event.Type                                       = DeviceEventType::kPlatformSCMBLEIndicationReceived;
