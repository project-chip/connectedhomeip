...
static struct peer_svc * peer_svc_find(struct peer *peer,
		uint16_t svc_start_handle,
		struct peer_svc **out_prev);
static int peer_svc_is_empty(const struct peer_svc *svc);

static uint16_t chr_end_handle(const struct peer_svc *svc,
		const struct peer_chr *chr);
static int chr_is_empty(const struct peer_svc *svc,
		const struct peer_chr *chr);
...
peer_init(int max_peers, int max_svcs, int max_chrs, int max_dscs)
{
	...
	peer_svc_mem = malloc(OS_MEMPOOL_BYTES(max_svcs,
				sizeof (struct peer_svc)));
	...
	peer_chr_mem = malloc(OS_MEMPOOL_BYTES(max_chrs,
				sizeof (struct peer_chr)));
	...
	peer_dsc_mem = malloc(OS_MEMPOOL_BYTES(max_dscs,
				sizeof (struct peer_dsc)));
