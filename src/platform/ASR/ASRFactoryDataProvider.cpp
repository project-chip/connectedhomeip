...
#else
    err = ASRConfig::ReadFactoryConfigValue(ASR_SALT_PARTITION, reinterpret_cast<uint8_t *>(saltB64), kSpake2pSalt_MaxBase64Len, saltB64Len);
#endif
...
#else
    err = ASRConfig::ReadFactoryConfigValue(ASR_VERIFIER_PARTITION, reinterpret_cast<uint8_t *>(verifierB64), kSpake2pSerializedVerifier_MaxBase64Len, verifierB64Len);
#endif
    ReturnErrorOnFailure(err);
...
CHIP_ERROR ASRFactoryDataProvider::GetCertificationDeclaration(MutableByteSpan & outBuffer)
{
    ...
    ReturnErrorOnFailure(
        ASRConfig::ReadFactoryConfigValue(ASR_CERT_DCLRN_PARTITION, outBuffer.data(), outBuffer.size(), certSize));
...
#else
    ReturnErrorOnFailure(
        ASRConfig::ReadFactoryConfigValue(ASR_DAC_CERT_PARTITION, outBuffer.data(), outBuffer.size(), certSize));
#endif
    outBuffer.reduce_size(certSize);
    return CHIP_NO_ERROR;
}
...
#else
    ReturnErrorOnFailure(
        ASRConfig::ReadFactoryConfigValue(ASR_PAI_CERT_PARTITION, outBuffer.data(), outBuffer.size(), certSize));
#endif
    outBuffer.reduce_size(certSize);
    return CHIP_NO_ERROR;
}
