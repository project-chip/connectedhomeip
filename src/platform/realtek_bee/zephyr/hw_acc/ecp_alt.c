...
#if ENABLE_HW_ECC_VERIFY == 1
    ecp_group_count--;
    // check if there is other ecp group
    if (ecp_group_count == 0)
    {
        set_pke_clock(false);
    }
#endif
                         ...
                         mbedtls_mpi tmp[4])
{
    ...
    hw_ecp_init(mbedtls_mpi_bitlen(&grp->P), ECC_PRIME_MODE, GO_TO_END_LOOP, !RR_MOD_N_READY);
    hw_ecp_set_sub_operand((ECC_N_ADDR), grp->P.p, mbedtls_mpi_size(&grp->P));
    hw_ecp_compute(temp, (ECC_X_RESULT_ADDR), RTK_PUBKEY_N_INV_ENTRY);

    hw_ecp_init(mbedtls_mpi_bitlen(&grp->P), ECC_PRIME_MODE, GO_TO_END_LOOP, !RR_MOD_N_READY);
    hw_ecp_compute(temp, (ECC_X_RESULT_ADDR), RTK_PUBKEY_R_SQAR_ENTRY);
    ...
    // a is null means a = -3, and calculate a mod P
    if (grp->A.p == NULL)
    {
        mbedtls_mpi_sub_int(&A_mod_P, &grp->P, 3);
    }
    else
    {
        mbedtls_mpi_copy(&A_mod_P, &grp->A);
    }

    hw_ecp_set_sub_operand((ECC_CURVE_A_ADDR), A_mod_P.p, mbedtls_mpi_size(&A_mod_P));
    hw_ecp_set_sub_operand((ECC_CURVE_B_ADDR), grp->B.p, mbedtls_mpi_size(&grp->B));

    hw_ecp_set_sub_operand(ECC_X_ADDR, P->X.p, mbedtls_mpi_size(&P->X));
    hw_ecp_set_sub_operand(ECC_Y_ADDR, P->Y.p, mbedtls_mpi_size(&P->Y));
    hw_ecp_set_sub_operand(ECC_Z_ADDR, P->Z.p, mbedtls_mpi_size(&P->Z));

    hw_ecp_set_sub_operand(ECC_X_RESULT_ADDR, Q->X.p, mbedtls_mpi_size(&Q->X));
    hw_ecp_set_sub_operand(ECC_Y_RESULT_ADDR, Q->Y.p, mbedtls_mpi_size(&Q->Y));
    hw_ecp_set_sub_operand(ECC_Z_RESULT_ADDR, Q->Z.p, mbedtls_mpi_size(&Q->Z));

    hw_ecp_point result;
    int ret = hw_ecp_compute(&result, (ECC_X_RESULT_ADDR), RTK_PUBKEY_ECC_ADD_POINT_ENTRY);

    if (ret == 0)
    {
        mbedtls_mpi_read_binary_le(&R->X, (const unsigned char *)result.x, mbedtls_mpi_size(&grp->P));
        mbedtls_mpi_read_binary_le(&R->Y, (const unsigned char *)result.y, mbedtls_mpi_size(&grp->P));
        mbedtls_mpi_lset(&R->Z, (mbedtls_mpi_sint)0x1);
    }
...
#if ENABLE_HW_ECC_VERIFY == 1
static int ecp_mul_hw_accelerate(mbedtls_ecp_group *grp, mbedtls_ecp_point *R,
                                 const mbedtls_mpi *m, const mbedtls_ecp_point *P,
                                 mbedtls_ecp_curve_type grp_type)
{
    ...
    hw_ecp_group ecc_group =
    {
        .N = grp->P.p,
        .A = mbedtls_calloc(grp->P.n, sizeof(mbedtls_mpi)),
        .B = mbedtls_calloc(grp->P.n, sizeof(mbedtls_mpi)),
        .n = mbedtls_calloc(grp->P.n, sizeof(mbedtls_mpi)),
        .key_bits = grp->pbits,
        .mode = (grp_type == MBEDTLS_ECP_TYPE_MONTGOMERY) ? ECC_MONTGOMERY_CURVE : ECC_PRIME_MODE,
    };
    ...
    for (uint8_t i = 0; i < grp->P.n; i++)
    {
        ecc_group.A[i] = (i < A_mod_P.n) ? A_mod_P.p[i] : 0;
        ecc_group.B[i] = (i < grp->B.n)  ? grp->B.p[i]  : 0;
        ecc_group.n[i] = (i < grp->N.n)  ? grp->N.p[i]  : 0;
    }

    memcpy(ecc_group.G.x, P->X.p, P->X.n << 2);
    memcpy(ecc_group.G.y, P->Y.p, P->Y.n << 2);
    memcpy(ecc_group.G.z, P->Z.p, P->Z.n << 2);

    hw_ecp_point result;

    hw_ecp_init(ecc_group.key_bits, ecc_group.mode, !GO_TO_END_LOOP, !RR_MOD_N_READY);

    if (hw_ecp_set_all_operands((void *)&ecc_group, (uint32_t *)m->p, mbedtls_mpi_size(m)) == false)
    {
        ret = -1;
        goto cleanup;
    }

    ret = hw_ecp_compute(&result, (ECC_X_RESULT_ADDR), RTK_PUBKEY_ECC_MUL_ENTRY);
...
cleanup:
    ...
    mbedtls_free(ecc_group.A);
    mbedtls_free(ecc_group.B);
    mbedtls_free(ecc_group.n);
                                        ...
                                        mbedtls_ecp_restart_ctx *rs_ctx)
{
    ...
    {
#if ENABLE_HW_ECC_VERIFY == 1
        MBEDTLS_MPI_CHK(ecp_mul_hw_accelerate(grp, R, m, P, MBEDTLS_ECP_TYPE_MONTGOMERY));
#else
    ...
    {
#if ENABLE_HW_ECC_VERIFY == 1
        MBEDTLS_MPI_CHK(ecp_mul_hw_accelerate(grp, R, m, P, MBEDTLS_ECP_TYPE_SHORT_WEIERSTRASS));
#else
