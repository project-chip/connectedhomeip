...
void LinuxWiFiDriver::ConnectNetwork(ByteSpan networkId, ConnectCallback * callback)
{
    ...
    if (network.UsingPDC())
    {
        ChipLogProgress(NetworkProvisioning, "LinuxWiFiDriver: ConnectNetwork (PDC) '%s'", ChipLogFormat(100, "%.*s", static_cast<int>(network.ssidLen), network.ssid));
...
#endif // CHIP_DEVICE_CONFIG_ENABLE_WIFI_PDC
    {
        ChipLogProgress(NetworkProvisioning, "LinuxWiFiDriver: ConnectNetwork '%s'", ChipLogFormat(100, "%.*s", static_cast<int>(network.ssidLen), network.ssid));
