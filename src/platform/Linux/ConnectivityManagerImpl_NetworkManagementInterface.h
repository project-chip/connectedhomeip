...
public:
     ...
     *  not permit concurrent Wi-Fi station and "Soft Access Point
     *  (AP)" operation (if supported by the platform or
     *  implementation). Concrete implementations may interpret this
     *  as "station-only" or as a reduced-feature startup used during
     ...
     *
     *  @param[in]  inSsid
     *    A reference to the immutable byte span for the Service Set
     *    Identifier (SSID) of the Wi-Fi remote access point to
     *    connect to. @a inSsid may or may not be known to the
     *    underlying Wi-Fi control plane at the time this method is
     *    invoked.
     *
     *  @param[in]  inCredentials
     *    A reference to an optional immutable byte span for the
     *    preshared key (PSK) credentials associated with @a inSsid,
     *    if any.
     *
     ...
     *    A reference to the immutable network identity used to select
     *    the credential and/or trust context for the target network.
     *    This is typically an identifier meaningful to the underlying
     *    control plane (for example, an EAP "realm", an SSID-scoped
     *    identity label, or other network-identifier string encoded as
     *    bytes).
     *
     *  @param[in]  inClientIdentity
     *    A reference to the immutable client identity presented by the
     *    device during certificate-based authentication (for example,
     *    an EAP identity / username, often encoded as a UTF-8 string).
     *
     ...
     */
    virtual CHIP_ERROR
    ConnectWiFiNetworkWithPDCAsync(const ByteSpan & inSsid, const ByteSpan & inNetworkIdentity, const ByteSpan & inClientIdentity,
     ...
     *    A reference to the immutable byte span for the Service Set
     *    Identifier (SSID) of the Wi-Fi remote access point to
     *    scan for. @a inSsid may or may not be known to the
     *    underlying Wi-Fi control plane at the time this method is
     *    invoked.
     *
     ...
     *
     */
    virtual CHIP_ERROR StartWiFiScan(const ByteSpan & inSsid, NetworkCommissioning::WiFiDriver::ScanCallback * inScanCallback) = 0;
...
protected:
    ...
    NetworkManagementInterface(void) = default;
};
