...
CHIP_ERROR ConnectivityManagerImpl::_WiFiPAFCancelPublish(uint32_t PublishId)
{
    ...
    std::lock_guard<std::mutex> lock(mWpaSupplicantMutex);
    wpa_fi_w1_wpa_supplicant1_interface_call_nancancel_publish_sync(mWpaSupplicant.iface, PublishId, nullptr, &err.GetReceiver());
    return CHIP_NO_ERROR;
}
...
CHIP_ERROR ConnectivityManagerImpl::_WiFiPAFCancelSubscribe(uint32_t SubscribeId)
{
    ...
    std::lock_guard<std::mutex> lock(mWpaSupplicantMutex);
    wpa_fi_w1_wpa_supplicant1_interface_call_nancancel_subscribe_sync(mWpaSupplicant.iface, SubscribeId, nullptr,
                                                                      &err.GetReceiver());
    return CHIP_NO_ERROR;
}
