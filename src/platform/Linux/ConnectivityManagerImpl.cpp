...
#if CHIP_DEVICE_CONFIG_ENABLE_WIFIPAF_CTRL_IFACE
...
CHIP_ERROR ConnectivityManagerImpl::_WiFiPAFPublish(ConnectivityManager::WiFiPAFAdvertiseParam & InArgs)
{
    ...
    {
#if CHIP_DEVICE_CONFIG_ENABLE_WIFIPAF_CTRL_IFACE
        mWiFiPAFDriver = std::make_unique<WiFiPAFDriverCtrlIface>();
#else
        mWiFiPAFDriver = std::make_unique<WiFiPAFDriverDbus>(&mWpaSupplicant, &mWpaSupplicantMutex);
#endif
        mWiFiPAFDriver->Initialize();
    }

    return mWiFiPAFDriver->Publish(std::move(InArgs.freq_list), InArgs.freq_list_len);
}
                                                      ...
                                                      OnConnectionCompleteFunct onSuccess, OnConnectionErrorFunct onError)
{
    ...
    return mWiFiPAFDriver->Subscribe(connDiscriminator, mApFreq);
}
