...
static void escape_value(const char * val, char * buf, size_t bufSize)
{
    ...
    for (idx = 0; (idx < len) && (bufSize > 6); idx++)
    {
...
static PRIntn settings_writer(PLHashEntry * he, PRIntn index, void * arg)
{
    ...
    if ((he->key != NULL) && (he->value != NULL))
    {
        ...
        if (strlen((const char *) he->key) + strlen(esc) + 26 > 2048)
        {
