...
CHIP_ERROR ThreadStackManagerImpl::_InitThreadStack()
{
    ...
    static_cast<chip::System::LayerImpl &>(SystemLayer()).AddSource(this);

exit:
    return error;
}

void ThreadStackManagerImpl::PrepareEvents(int * aMaxFd, fd_set * aReadFdSet, fd_set * aWriteFdSet, fd_set * aErrorFdSet,
                                           struct timeval * aTimeout)
{
    otTaskletsProcess(OTInstance());
    otSysUpdateEvents(OTInstance(), aMaxFd, aReadFdSet, aWriteFdSet, aErrorFdSet, aTimeout);
}

void ThreadStackManagerImpl::ProcessEvents(const fd_set * aReadFdSet, const fd_set * aWriteFdSet, const fd_set * aErrorFdSet)
{
    otSysProcessEvents(OTInstance(), aReadFdSet, aWriteFdSet, aErrorFdSet);
}
