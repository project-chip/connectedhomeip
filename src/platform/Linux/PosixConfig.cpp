...
static ChipLinuxSettings gChipLinuxFacotryStorage;
static ChipLinuxSettings gChipLinuxConfigStorage;
static ChipLinuxSettings gChipLinuxCountersStorage;
...
ChipLinuxSettings * PosixConfig::GetStorageForNamespace(Key key)
{
    if (strcmp(key.Namespace, kConfigNamespace_ChipFactory) == 0)
        return &gChipLinuxFacotryStorage;
...
{
    CHIP_ERROR err              = CHIP_NO_ERROR;
    ChipLinuxSettings * storage = NULL;

    if (strcmp(ns, kConfigNamespace_ChipFactory) == 0)
    {
        storage = &gChipLinuxFacotryStorage;
        err     = storage->Init(CHIP_DEFAULT_FACTORY_PATH);
        storage->SetWritability(false);
    }
