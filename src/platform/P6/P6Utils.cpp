...
bool P6Utils::IsStationProvisioned(void)
{
    ...
    return (p6_wifi_get_config(WIFI_IF_STA, &stationConfig) == CY_RSLT_SUCCESS && strlen((const char*)stationConfig.sta.ssid) != 0);
}
...
cy_rslt_t P6Utils::p6_wifi_set_config(wifi_interface_t interface, wifi_config_t *conf)
{
   ...
   else
   {
        ...
        ChipLogProgress(DeviceLayer, "AP %s : [%s] [%s] channel %d\r\n", __func__,
            wifi_conf.ap.ssid, wifi_conf.ap.password, wifi_conf.ap.channel);
   }
...
CHIP_ERROR P6Utils::p6_wifi_connect(void)
{
    ...
    ChipLogProgress(DeviceLayer, "p6_wifi_connect ssid %s pass %s sec %d \r\n",
        ...
        connect_param.ap_credentials.password,
        connect_param.ap_credentials.security);
...
CHIP_ERROR P6Utils::p6_start_ap(void)
{
    ...
    ChipLogProgress(DeviceLayer, "p6_start_ap %s %s \r\n", ap_conf.ap_credentials.SSID,
        ap_conf.ap_credentials.password);
...
void P6Utils::populate_wifi_config_t(wifi_config_t *wifi_config, wifi_interface_t interface, const cy_wcm_ssid_t *ssid, const cy_wcm_passphrase_t *password, cy_wcm_security_t security)
{
