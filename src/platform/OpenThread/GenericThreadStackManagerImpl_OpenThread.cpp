...
void DispatchResolve(intptr_t context)
{
    ...
    chip::ThreadStackMgrImpl().mDnsResolveCallback(dnsResult->context, &(dnsResult->mMdnsService), dnsResult->error);
    chip::Platform::Delete<DnsResult>(context);
}

void DispatchBrowseEmpty(intptr_t context)
{
    auto * dnsResult = reinterpret_cast<DnsResult *>(context);
    chip::ThreadStackMgrImpl().mDnsBrowseCallback(dnsResult->context, nullptr, 0, dnsResult->error);
    chip::Platform::Delete<DnsResult>(context);
}
...
{
    auto * dnsResult = reinterpret_cast<DnsResult *>(context);
    chip::ThreadStackMgrImpl().mDnsBrowseCallback(dnsResult->context, &dnsResult->mMdnsService, 1, dnsResult->error);
    chip::Platform::Delete<DnsResult>(context);
}
} // namespace
                                                                            ...
                                                                            void * aContext)
{
    ...
    while (otDnsBrowseResponseGetServiceInstance(aResponse, index, serviceName, sizeof(service.mName)) == OT_ERROR_NONE)
    {
        ...
        DnsResult * dnsResult = Platform::New<DnsResult>(context, MapOpenThreadError(aError));
        ...
        if (CHIP_NO_ERROR == error)
        {
            ...
            // allocate on
            // stack.
            strncpy(dnsResult->mName, serviceName, sizeof(serviceName));
