...
private:
    ...
    CHIP_ERROR StoreSpake2pIterationCount(uint32_t iterationCount) override { return CHIP_ERROR_NOT_IMPLEMENTED; }
    CHIP_ERROR GetSpake2pSalt(uint8_t * buf, size_t bufSize, size_t & saltLen) override { return CHIP_ERROR_NOT_IMPLEMENTED; }
    CHIP_ERROR StoreSpake2pSalt(const uint8_t * salt, size_t saltLen) override { return CHIP_ERROR_NOT_IMPLEMENTED; }
    CHIP_ERROR GetSpake2pVerifier(uint8_t * buf, size_t bufSize, size_t & verifierLen) override
    {
        return CHIP_ERROR_NOT_IMPLEMENTED;
    }
    CHIP_ERROR StoreSpake2pVerifier(const uint8_t * verifier, size_t verifierLen) override { return CHIP_ERROR_NOT_IMPLEMENTED; }
