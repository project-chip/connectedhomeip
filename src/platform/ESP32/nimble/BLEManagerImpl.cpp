          ...
          (struct ble_gatt_chr_def[]){
              {
                  ...
                  .flags     = BLE_GATT_CHR_F_WRITE | BLE_GATT_CHR_F_WRITE | BLE_GATT_CHR_F_WRITE_NO_RSP,
              },
              {
...
void BLEManagerImpl::bleprph_on_sync(void)
{
    ...
    ESP_LOGD(TAG, "BLE Host Controller synced");
...
void BLEManagerImpl::HandleRXCharWrite(struct ble_gatt_char_context * param)
{
    ...
    HandleWriteReceived(param->conn_handle, &CHIP_BLE_SVC_ID, &chipUUID_CHIPoBLEChar_RX, buf);
    buf = NULL;
...
int BLEManagerImpl::gatt_svr_chr_access(uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt * ctxt, void * arg)
{
    ...
    case BLE_GATT_ACCESS_OP_READ_CHR:
        ...
        param.ctxt        = ctxt;
        param.arg         = arg;
        // XXX sInstance.HandleTXCharRead(&param);
        break;
