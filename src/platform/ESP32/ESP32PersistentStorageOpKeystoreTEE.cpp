...
std::string PersistentStorageOpKeystoreTEE::GetKeyId(FabricIndex fabricIndex) const
{
    ...
    int written = snprintf(buf, sizeof(buf), "esp32_fabric_%u_key", fabricIndex);
...
esp_tee_sec_storage_key_cfg_t PersistentStorageOpKeystoreTEE::GetKeyCfg(FabricIndex fabricIndex) const
{
    static thread_local char keyIdBuf[KEY_ID_MAX_SZ];
    int written = snprintf(keyIdBuf, sizeof(keyIdBuf), "esp32_fabric_%u_key", fabricIndex);
    if (written < 0 || written >= static_cast<int>(sizeof(keyIdBuf)))
    {
        keyIdBuf[sizeof(keyIdBuf) - 1] = '\0';
    }
...
bool PersistentStorageOpKeystoreTEE::HasOpKeypairForFabric(FabricIndex fabricIndex) const
{
    ...
    auto cfg = GetKeyCfg(fabricIndex);
