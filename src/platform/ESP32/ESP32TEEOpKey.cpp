...
CHIP_ERROR ESP32TEEOpKeyGenerate(const char * keyId)
{
    ...
    // esp_tee_sec_storage_gen_key rejects an already-populated id, so clear any existing key
    // first. This makes the primitive idempotent (honoring the "overwrites" contract) and
    // recovers a slot left populated by a reboot-aborted fail-safe.
...
CHIP_ERROR ESP32TEEOpKeyNewCSR(const char * keyId, MutableByteSpan & outCsr)
{
    ...
    // Subject Name "O=CSR": a non-empty placeholder (x509 parsers reject an empty subject). The
    // commissioner uses only the embedded public key from the CSR, not the subject.
...
exit:
    // Cleanup is part of the "every step verified" contract: if the checks above passed,
    // a failure to delete the throwaway key must fail the self-test rather than report PASSED.
    CHIP_ERROR cleanupErr = ESP32TEEOpKeyRemove(kId);
