...
Interface for `AttributePersistenceProvider` has changed to not depend on
ember types and its location was updated to `src/app/persistence`.

- `WriteValue` now has access to value metadata for symmetry. Only signature of
  method needs to be updated for backwards COMPATIBILITY
- `ReadValue` metadata has been updated from full Ember metadata to specific
  types of `ShortPascal/LongPascal/Fixed/Variable`.
