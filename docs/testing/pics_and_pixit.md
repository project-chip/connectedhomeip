...
endpoint the test talks to over Matter; it does **not** select which
...
single-endpoint XML directory, a directory with **per-endpoint subdirectories**
of XML, or a zip of that tree. The harness does **not** need to flatten that XML
