...
for that purpose. The base implementation and the full set of overridable
`*Impl()` APIs live in that example's source tree under `include/AppTaskImpl.h`
and `src/AppTask.cpp`. Any `*Impl()` you do not override keeps the Silicon Labs
default behavior.
...
What used to live in `DataModelCallbacks.cpp` before Matter Extension 2.9.0 now
lives in `AppTask.cpp`. The Matter SDK's `MatterPostAttributeChangeCallback` is
implemented in
...
-   **Methods that already exist in the AppTask** — Customize them by overriding
    the matching `*Impl()` method in `CustomerAppTask`. Do not edit
