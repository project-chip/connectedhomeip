...
```
user@ubuntu:~/Desktop/git/connectedhomeip$ west build -d <out_dir> -b frdmmcxw72 third_party/simw-top-mini/repo/demos/se051h_nfc_comm_prov/mcu/ -Dcore_id=cm33_core0 -DCONFIG_CHIP_SE05X=y -DCONFIG_SE05X_DO_AES_KEY_PROVISION=y
```
Open AES session and delete all objects ==>
```
user@ubuntu:~/Desktop/git/connectedhomeip$ west build -d <out_dir> -b frdmmcxw72 third_party/simw-top-mini/repo/demos/se051h_nfc_comm_prov/mcu/ -Dcore_id=cm33_core0 -DCONFIG_CHIP_SE05X=y -DCONFIG_SE05X_AES_KEY=y -DCONFIG_SE05X_DO_RESET=y
```
