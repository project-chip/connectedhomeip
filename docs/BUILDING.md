...
Run the below commands on the linux machine that runs controller everytime you restart the RPi
sudo btmgmt -i hci0 power off
sudo btmgmt -i hci0 bredr off
sudo btmgmt -i hci0 power on
(note: raspberry pi has a dual mode bluetooth chip. RPi or android phones by default tries to connect using a classic 
mode(BREDR). Therefore we must disable bredr in /etc/bluetooth/main.conf or use btmgmt to disable bredr as the 
