...
The script `./scripts/py_matter_idl/scripts/matter-data-model-xml-parser.py` has
the ability to parse one or more CSA data model XML files and output their
content in `.matter` format. For example:

```sh
./scripts/py_matter_idl/scripts/matter-data-model-xml-parser.py data_model/clusters/BooleanState.xml
```
